...
function createVariableInliner() {
  ...
  const variableUsages = new Map<ts.Symbol, {
    declaration: ts.VariableDeclaration,
    references: ts.Identifier[],
    canInline: boolean
  }>();

  function visitNode(node: ts.Node, program: ts.Program, typeChecker: ts.TypeChecker): ts.Node {
  ...
  function isInlinable(node: ts.Expression): boolean {
    ...
    switch (node.kind) {
      ...
      case ts.SyntaxKind.ParenthesizedExpression:
        return isInlinable((node as ts.ParenthesizedExpression).expression);
  ...
  function inlineVariables(node: ts.Node, program: ts.Program, typeChecker: ts.TypeChecker): ts.Node {
      ...
      const visit = (node: ts.Node): ts.Node => {
        ...
        if (ts.isVariableStatement(node)) {
          const declarations = node.declarationList.declarations.filter(decl => {
            ...
            const usage = variableUsages.get(symbol);
            return !usage || usage.references.length !== 1 || !usage.canInline;
          });
          ...
          if (declarations.length !== node.declarationList.declarations.length) {
            return ts.factory.updateVariableStatement(
              node,
              node.modifiers,
              ts.factory.createVariableDeclarationList(declarations, node.declarationList.flags)
            );
          }
        }
        ...
        if (ts.isIdentifier(node)) {
          ...
          if (symbol) {
            const usage = variableUsages.get(symbol);
            if (usage && usage.references.length === 1 && usage.canInline) {
...
// Example usage
function transformFile(sourceFile: string) {
