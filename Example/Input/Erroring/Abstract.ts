...
export function contiguousFuzzyScore(
	patternLower: string,
	wordLower: string,
...
export interface IFindFilter<T>
	extends ITreeFilter<T, FuzzyScore | LabelFuzzyScore> {
	filter(
		element: T,
...
class TreeFindToggle extends Toggle {
	readonly id: string;
	constructor(
		contribution: ITreeFindToggleContribution,
		opts: IToggleStyles,
...
class FindWidget<T, TFilterData> extends Disposable {
	...
	constructor(
		...
		this.toggles = toggleContributions.map((contribution) =>
			this._register(
				new TreeFindToggle(
					contribution,
					styles.toggleStyles,
...
export abstract class AbstractFindController<T, TFilterData>
...
class FindController<T, TFilterData> extends AbstractFindController<
	T,
	TFilterData
> {
	...
	set matchType(matchType: TreeFindMatchType) {
		...
		this.updateToggleState(
			DefaultTreeToggles.MatchType,
			matchType === TreeFindMatchType.Fuzzy,
		);
		this.tree.refilter();
		this.render();
	...
	constructor(
		...
		const defaultFindMode =
			options.defaultFindMode ?? TreeFindMode.Highlight;
		const defaultFindMatchType =
			options.defaultFindMatchType ?? TreeFindMatchType.Fuzzy;
	...
	private render(): void {
		const noMatches =
			this.filter.matchCount === 0 && this.filter.totalCount > 0;
	...
	constructor(
		...
		this._register(
			model.onDidSpliceRenderedNodes((e) => {
				...
				const hasRemovedStickyNode =
					e.deleteCount > 0 &&
					state.stickyNodes.some(
						(stickyNode) =>
							!this.model.has(
