...
export class NotebookVariablesView extends ViewPane {
	...
	static readonly NOTEBOOK_TITLE: ILocalizedString = nls.localize2(
		"notebook.notebookVariables",
		"Notebook Variables",
	);
	static readonly REPL_TITLE: ILocalizedString = nls.localize2(
	...
	private getActiveNotebook() {
		...
		const notebookDocument =
			getNotebookEditorFromEditorPane(notebookEditor)?.textModel;
		return notebookDocument && notebookEditor
			? { notebookDocument, notebookEditor }
			: undefined;
	}
	...
	private handleCloseEditor(e: IEditorCloseEvent) {
		if (
			e.editor.resource &&
			e.editor.resource.toString() === this.activeNotebook?.uri.toString()
		) {
	...
	private handleExecutionStateChange(
		...
		if (
			this.activeNotebook &&
			event.affectsNotebook(this.activeNotebook.uri)
		) {
			...
			this.editorService.visibleEditorPanes.forEach((editor) => {
				const notebookDocument =
					getNotebookEditorFromEditorPane(editor)?.textModel;
				if (
					notebookDocument &&
					event.affectsNotebook(notebookDocument.uri)
				) {
	...
	private handleVariablesChanged(notebookUri: URI) {
		if (
			...
			this.editorService.visibleEditorPanes.forEach((editor) => {
				...
				if (
					notebookDocument &&
					notebookDocument.uri.toString() === notebookUri.toString()
				) {
					this.setActiveNotebook(notebookDocument, editor);
				}
			});
