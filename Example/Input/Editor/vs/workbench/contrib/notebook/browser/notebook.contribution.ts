...
export class NotebookContribution
	extends Disposable
...
class CellInfoContentProvider {
	...
	async provideMetadataTextContent(
		resource: URI,
	...
	private parseStreamOutput(
		op?: ICellOutput,
	): { content: string; mode: ILanguageSelection } | undefined {
		if (!op) {
			return;
		}
	...
	private _getResult(
		...
		let result: { content: string; mode: ILanguageSelection } | undefined =
			undefined;

		const mode = this._languageService.createById("json");
		const op = cell.outputs.find(
			(op) =>
				op.outputId === data.outputId ||
...
class RegisterSchemasContribution
	extends Disposable
	implements IWorkbenchContribution
{
...
class NotebookEditorManager implements IWorkbenchContribution {
	...
	private _openMissingDirtyNotebookEditors(
		models: IResolvedNotebookEditorModel[],
	): void {
...
class SimpleNotebookWorkingCopyEditorHandler
	extends Disposable
	implements IWorkbenchContribution, IWorkingCopyEditorHandler
{
	...
	private _getViewType(workingCopy: IWorkingCopyIdentifier) {
		const notebookType = NotebookWorkingCopyTypeIdentifier.parse(
			workingCopy.typeId,
		);
		if (
			notebookType &&
...
class NotebookLanguageSelectorScoreRefine {
	static readonly ID =
		"workbench.contrib.notebookLanguageSelectorScoreRefine";
...
configurationRegistry.registerConfiguration({
	...
	properties: {
		...
		[NotebookSetting.cellEditorOptionsCustomizations]:
			editorOptionsCustomizationSchema,
