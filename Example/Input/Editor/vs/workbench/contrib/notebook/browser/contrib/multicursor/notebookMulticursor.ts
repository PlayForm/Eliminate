	...
	public async findAndTrackNextSelection(
		...
		if (this.state === NotebookMultiCursorState.Idle) {
			...
			if (findResult.cell.handle === cell.handle) {
				...
				if (
					this.notebookEditor.activeCellAndCodeEditor?.[0].handle !==
					resultCellViewModel.handle
				) {
					// should not happen
					throw new Error(
						"Focused cell does not match the find match data",
					);
				}
				...
				this.initializeMultiSelectDecorations(
					this.trackedMatches.find(
						(match) => match.cellViewModel.handle === cell.handle,
					),
				);
			} else {
				// should not happen
		...
		this.trackedMatches.forEach((match) => {
			...
			if (match.cellViewModel.handle !== this.anchorCell?.[0].handle) {
				...
				controller.setSelections(
					new ViewModelEventsCollector(),
					undefined,
					match.cellViewModel.getSelections(),
					CursorChangeReason.Explicit,
				);
			}
		});
	...
	private updateLazyDecorations() {
		...
		this.trackedMatches.forEach((match) => {
			...
			const matchingEditor = this.notebookEditor.codeEditors.find(
				(cellEditor) => cellEditor[0] === match.cellViewModel,
			);
			if (matchingEditor) {
