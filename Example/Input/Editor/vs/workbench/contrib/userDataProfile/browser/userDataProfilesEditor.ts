...
import {
	...
	IProfileChildElement,
	IProfileResourceTypeChildElement,
	IProfileResourceTypeElement,
		...
		>
{
	...
	renderTemplate(container: HTMLElement): IProfileElementTemplateData {
		...
		append(
			...
			$("span", undefined, localize("activeProfile", "Active")),
		);
...
class ProfileTreeDelegate extends CachedListVirtualDelegate<ProfileTreeElement> {
	...
	protected estimateHeight({ element, root }: ProfileTreeElement): number {
		switch (element) {
			...
			case "workspaces":
				return (
					(root.workspaces ? root.workspaces.length * 24 + 30 : 0) +
					112
				);
		}
	}
		...
		const renderName = (profileElement: ProfileTreeElement) => {
			...
			if (isDefaultProfile) {
				nameInput.setTooltip(
					localize(
						"defaultProfileName",
						"Name cannot be changed for the default profile",
					),
				);
			} else {
...
class ProfileIconRenderer extends ProfilePropertyRenderer {
	...
	renderTemplate(parent: HTMLElement): IProfilePropertyRendererTemplate {
		...
		const iconHover = disposables.add(
		...
		disposables.add(
			this.profilesContentTree.onContextMenu(async (e) => {
				...
				this.contextMenuService.showContextMenu({
					getAnchor: () => e.anchor,
					getActions: () =>
						e.element?.element?.actions?.contextMenu ?? [],
					getActionsContext: () => e.element,
				});
			}),
		);
...
class ProfileWorkspacesRenderer extends ProfilePropertyRenderer {
	...
	renderTemplate(parent: HTMLElement): IProfilePropertyRendererTemplate {
		...
		const profileWorkspacesRowContainer = append(
			parent,
			$(".profile-row-container"),
		);
		append(
			...
			$(
				".profile-label-element",
				undefined,
				localize("folders_workspaces", "Folders & Workspaces"),
			),
		);
		const profileWorkspacesDescriptionElement = append(
			profileWorkspacesRowContainer,
			$(".profile-description-element"),
		);

		const workspacesTableContainer = append(
			profileWorkspacesRowContainer,
			$(".profile-associations-table"),
		);
		const table = (this.workspacesTable = disposables.add(
			this.instantiationService.createInstance(
				WorkbenchTable<WorkspaceTableElement>,
				...
				[
					...
					},
					{
						...
						maximumWidth: 84,
						templateId:
							WorkspaceUriActionsColumnRenderer.TEMPLATE_ID,
						project(
							row: WorkspaceTableElement,
						): WorkspaceTableElement {
							return row;
		...
		const addButtonBarElement = append(
			profileWorkspacesRowContainer,
			$(".profile-workspaces-button-container"),
		);
		...
		disposables.add(
			table.onDidOpen((item) => {
				if (item?.element) {
					item.element.profileElement.openWorkspace(
						item.element.workspace,
					);
				}
			}),
		);
		...
		const updateTable = () => {
			if (
				...
				table.splice(
					0,
					table.length,
					profileElement.root.workspaces
						.map((workspace) => ({
							workspace,
							profileElement: <UserDataProfileElement>(
								profileElement!.root
							),
						}))
						.sort((a, b) =>
							this.uriIdentityService.extUri.compare(
								a.workspace,
								b.workspace,
							),
						),
	...
	layout(): void {
		if (this.workspacesTable) {
			this.workspacesTable.layout(
				this.workspacesTable.length * 24 + 30,
				undefined,
			);
		}
	}
		...
		>
{
	...
	renderElement(
		item: WorkspaceTableElement,
		index: number,
		templateData: IWorkspaceUriHostColumnTemplateData,
		height: number | undefined,
	): void {
		...
		>
{
	...
	renderTemplate(
		...
		const pathHover = disposables.add(
			this.hoverService.setupManagedHover(
				this.hoverDelegate,
				pathLabel,
				"",
	...
	renderElement(
		item: WorkspaceTableElement,
		index: number,
		templateData: IWorkspaceUriPathColumnTemplateData,
		height: number | undefined,
	): void {
		templateData.renderDisposables.clear();
		...
		ITableRenderer<WorkspaceTableElement, IActionsColumnTemplateData>
{
				...
				actionViewItemProvider: (action) => {
					if (action instanceof ChangeProfileAction) {
						return new DropdownMenuActionViewItem(
							...
							{
								getActions: () =>
									action.getSwitchProfileActions(),
							},
	...
	private createOpenAction(item: WorkspaceTableElement): IAction {
		return {
			...
			enabled: !this.uriIdentityService.extUri.isEqual(
				item.workspace,
				item.profileElement.getCurrentWorkspace(),
			),
			id: "openWorkspace",
	...
	private createDeleteAction(item: WorkspaceTableElement): IAction {
		return {
			...
			enabled:
				this.userDataProfileManagementService.getDefaultProfileToUse()
