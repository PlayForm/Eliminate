...
export abstract class AbstractUserDataProfileElement extends Disposable {
		...
		this._register(
			this.extensionManagementService.onProfileAwareDidUpdateExtensionMetadata(
				(e) => {
					const profile = this.getProfileToWatch();
					if (
						(profile && e.local.isApplicationScoped) ||
						this.uriIdentityService.extUri.isEqual(
							e.profileLocation,
							profile?.extensionsResource,
						)
	...
	openWorkspace(workspace: URI): void {
		if (
			this.uriIdentityService.extUri.extname(workspace) ===
			WORKSPACE_SUFFIX
		) {
	...
	protected override async getChildrenForResourceType(
		...
		if (resourceType === ProfileResourceType.Extensions) {
			const children = await this.instantiationService
				...
				.getChildren();
			return children.map<IProfileResourceTypeChildElement>((child) =>
				this.toUserDataProfileResourceChildElement(child, undefined, [
					{
		...
		this._register(
			toDisposable(() => {
				if (this.previewProfile) {
					this.userDataProfilesService.removeProfile(
						this.previewProfile,
					);
				}
			}),
