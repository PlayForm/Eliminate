		...
		>
{
	...
	private _renderBadges(
		...
		templateData.elementDisposables.add(
			autorun((reader) => {
				...
				const historyItemRefsByColor = groupBy2(references, (ref) =>
					ref.color ? ref.color : "",
				);

				for (const [key, historyItemRefs] of Object.entries(
					...
					const historyItemRefByIconId = groupBy2(
						historyItemRefs,
						(ref) =>
							ThemeIcon.isThemeIcon(ref.icon) ? ref.icon.id : "",
					);
					for (const [key, historyItemRefs] of Object.entries(
						historyItemRefByIconId,
					)) {
	...
	private _renderBadge(
		...
		elements.icon.classList.add(
			...ThemeIcon.asClassNameArray(historyItemRefs[0].icon),
		);
...
class SCMHistoryViewModel extends Disposable {
	...
	private readonly _repositoryState = new Map<
		ISCMRepository,
		RepositoryState
	>();
	private readonly _repositoryFilterState = new Map<
		string,
		HistoryItemRefsFilter
	>();
	...
	constructor(
		...
		this._extensionService.onWillStop(
		...
		this._scmHistoryItemCountCtx = ContextKeys.SCMHistoryItemCount.bindTo(
	...
	getHistoryItemsFilter(): "all" | "auto" | ISCMHistoryItemRef[] | undefined {
		...
		const filterState =
			this._repositoryFilterState.get(
				getProviderKey(repository.provider),
			) ?? "auto";
	...
	getCurrentHistoryItemTreeElement():
		...
		return state.viewModels.find(
			(viewModel) =>
				viewModel.historyItemViewModel.historyItem.id ===
	...
	async getHistoryItems(): Promise<SCMHistoryItemViewModelTreeElement[]> {
		...
		if (!state || state.loadMore !== false) {
			...
			const historyItemRefs =
				state?.historyItemsFilter ??
				(await this._resolveHistoryItemFilter(
					repository,
					historyProvider,
				));
	...
	setHistoryItemsFilter(filter: HistoryItemRefsFilter): void {
		...
		if (filter !== "auto") {
			this._repositoryFilterState.set(
				getProviderKey(repository.provider),
				filter,
			);
		} else {
			this._repositoryFilterState.delete(
				getProviderKey(repository.provider),
			);
		}
		this._saveHistoryItemsFilterState();
		...
		switch (historyItemsFilter) {
			...
			default: {
				...
				const refs = (
					...
					historyItemsFilter.some((filter) => filter === ref.id),
				);
				...
				quickPick.onDidAccept(() => {
					if (selectedItems.length === 0) {
						...
						resolve(
							selectedItems.map(
								(item) =>
									(item.historyItemRef as ISCMHistoryItemRef)
										.id,
							),
						);
					}
...
export class SCMHistoryViewPane extends ViewPane {
		...
		this.onDidChangeBodyVisibility(async (visible) => {
			...
			this._visibilityDisposables.add(
				autorunWithStore((reader, store) => {
					...
					store.add(
						runOnChange(
							this._treeViewModel.onDidChangeHistoryItemsFilter,
							() => {
								this.refresh();
							},
						),
	...
	override getActionViewItem(
		...
		if (action.id === PICK_REPOSITORY_ACTION_ID) {
			...
			const repository = this._treeViewModel?.repository.get();
			const historyItemsFilter =
				this._treeViewModel?.getHistoryItemsFilter();
	...
	async revealCurrentHistoryItem(): Promise<void> {
		...
		const revealTreeNode = (): boolean => {
			const historyItemTreeElement =
				this._treeViewModel.getCurrentHistoryItemTreeElement();

			if (
				historyItemTreeElement &&
