...
export class TerminalClipboardContribution
	extends Disposable
	implements ITerminalContribution
{
	...
	xtermReady(xterm: IXtermTerminal & { raw: RawXtermTerminal }): void {
		...
		this._register(
			xterm.onDidRequestCopyAsHtml((e) =>
...
if (BrowserFeatures.clipboard.readText) {
	registerActiveInstanceAction({
		...
		run: (activeInstance) =>
			TerminalClipboardContribution.get(activeInstance)?.paste(),
	});
}
...
if (BrowserFeatures.clipboard.readText && isLinux) {
	registerActiveInstanceAction({
		...
		run: (activeInstance) =>
			TerminalClipboardContribution.get(activeInstance)?.pasteSelection(),
	});
}
