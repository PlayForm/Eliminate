	...
	implements IAccessibleViewImplentation
{
	...
	getProvider(accessor: ServicesAccessor) {
		...
		return new AccessibleContentProvider(
			...
			() => helpText,
			() =>
				TerminalChatController.get(
					instance,
				)?.terminalChatWidget?.focus(),
...
export function getAccessibilityHelpText(accessor: ServicesAccessor): string {
	...
	const runCommandKeybinding = keybindingService
		.lookupKeybinding(TerminalChatCommandId.RunCommand)
		?.getAriaLabel();
	const insertCommandKeybinding = keybindingService
		.lookupKeybinding(TerminalChatCommandId.InsertCommand)
		?.getAriaLabel();
	const makeRequestKeybinding = keybindingService
		.lookupKeybinding(TerminalChatCommandId.MakeRequest)
		?.getAriaLabel();
	const startChatKeybinding = keybindingService
		.lookupKeybinding(TerminalChatCommandId.Start)
		?.getAriaLabel();
