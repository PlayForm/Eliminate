...
const terminalChatPlaceholder = localize(
	"default.placeholder",
...
export class TerminalChatWidget extends Disposable {
	...
	constructor(
		...
		this._promptHistory = JSON.parse(
	...
	async acceptInput(
		...
		let responseContent = "";
		const response = await this._inlineChatWidget.chatWidget.acceptInput(
			lastInput,
	...
	async viewInChat(): Promise<void> {
		...
		const currentRequest =
			this._inlineChatWidget.chatWidget.viewModel?.model
				.getRequests()
