	...
	reset(resourceMarkers: ResourceMarkers[]): void {
		...
		for (const resourceMarker of this.resourceMarkers) {
			for (const marker of resourceMarker.markers) {
				...
				// Exclude pattern
				if (
					this.filterOptions.excludesMatcher.matches(marker.resource)
				) {
					continue;
				}
				...
				// Include pattern
				if (
					this.filterOptions.includesMatcher.matches(marker.resource)
				) {
	...
	revealMarkers(
		...
		if (activeResource) {
			const activeResourceIndex =
				this.resourceMarkers.indexOf(activeResource);
