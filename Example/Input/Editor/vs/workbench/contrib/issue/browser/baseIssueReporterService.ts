...
import {
	IssueReporterModel,
	IssueReporterData as IssueReporterModelData,
...
export class BaseIssueReporterService extends Disposable {
	...
	setInitialFocus() {
		...
		if (fileOnExtension) {
			const issueTitle =
				this.window.document.getElementById("issue-title");
			issueTitle?.focus();
		} else {
	...
	private handleExtensionData(extensions: IssueReporterExtensionData[]) {
		...
		if (this.disableExtensions || installedExtensions.length === 0) {
			(<HTMLButtonElement>(
				this.getElementById("disableExtensions")
			)).disabled = true;
		}
	...
	private updateExtensionSelector(
		...
		const makeOption = (
			...
			const selected =
				selectedExtension && extension.id === selectedExtension.id;
		...
		if (extensionsSelector) {
			...
			reset(
				...
				...extensionOptions.map((extension) =>
					makeOption(extension, selectedExtension),
				),
			);
			...
			this.addEventListener(
				...
				async (e: Event) => {
					...
					const extensions =
						this.issueReporterModel.getData().allExtensions;
	...
	private async sendReporterMenu(
		extension: IssueReporterExtensionData,
	...
	public setEventHandlers(): void {
		...
		const showInfoElements =
			this.window.document.getElementsByClassName("showInfo");
		for (let i = 0; i < showInfoElements.length; i++) {
			const showInfo = showInfoElements.item(i)!;
			(showInfo as HTMLAnchorElement).addEventListener(
				"click",
				(e: MouseEvent) => {
					e.preventDefault();
		...
		this.addEventListener("issue-title", "input", (e: Event) => {
			...
			const issueSource =
				this.getElementById<HTMLSelectElement>("issue-source");
			if (!issueSource || issueSource.value === "") {
				return;
			}

			const { fileOnExtension, fileOnMarketplace } =
	...
	private setUpTypes(): void {
		const makeOption = (issueType: IssueType, description: string) =>
			$("option", { "value": issueType.valueOf() }, escape(description));
	...
	public renderBlocks(): void {
		...
		const processBlock =
			this.window.document.querySelector(".block-process");
		const workspaceBlock =
			this.window.document.querySelector(".block-workspace");
		const extensionsBlock =
			this.window.document.querySelector(".block-extensions");
