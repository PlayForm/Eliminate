...
const EXTENSIONS_DISMISSED_NOTIFICATIONS_KEY =
...
class Extensions extends Disposable {
	...
	private async updateMetadata(
		...
		return this.workbenchExtensionManagementService.updateMetadata(
			localExtension,
			{
	...
	implements IExtensionsWorkbenchService, IURLHandler
{
	...
	private async initialize(): Promise<void> {
		...
		this._register(
			this.storageService.onDidChangeValue(
				...
				this._store,
			)((e) => this.onDidDismissedNotificationsValueChange()),
		);
		this._register(
			this.storageService.onDidChangeValue(
	...
	private onDidDismissedNotificationsValueChange(): void {
		if (
			this.dismissedNotificationsValue !==
	...
	private updateExtensionsNotificaiton(): void {
		...
		if (computedNotificiations.length) {
			...
			for (const dismissedNotification of this.getDismissedNotifications()) {
				if (
					computedNotificiations.some(
						(e) => e.key === dismissedNotification,
					)
				) {
			...
			if (
				!dismissedNotifications.includes(computedNotificiations[0].key)
			) {
	...
	private computeExtensionsNotifications(): Array<
		Omit<IExtensionsNotification, "dismiss"> & { key: string }
	> {
		const computedNotificiations: Array<
			Omit<IExtensionsNotification, "dismiss"> & { key: string }
		> = [];
			...
			if (
				...
				computedNotificiations.push({
					...
					key:
						...
						invalidExtensions
							.sort((a, b) =>
								a.identifier.id.localeCompare(b.identifier.id),
							)
							.map(
								(e) =>
									`${e.identifier.id.toLowerCase()}@${e.local?.manifest.version}`,
							)
							.join("-"),
				});
			} else {
	...
	private set enabledAuotUpdateExtensionsValue(
		enabledAuotUpdateExtensionsValue: string,
	) {
		if (
			this.enabledAuotUpdateExtensionsValue !==
	...
	private set disabledAutoUpdateExtensionsValue(
		disabledAutoUpdateExtensionsValue: string,
	) {
		if (
			this.disabledAutoUpdateExtensionsValue !==
	...
	private setDismissedNotifications(dismissedNotifications: string[]): void {
		this.dismissedNotificationsValue = JSON.stringify(
	...
	private set dismissedNotificationsValue(
		dismissedNotificationsValue: string,
	) {
