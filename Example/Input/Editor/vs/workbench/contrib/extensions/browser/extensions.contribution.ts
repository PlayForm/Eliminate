	...
	metadata: {
		...
		args: [
			...
			},
			{
				name: "options",
				description:
					"(optional) Options for installing the extension. Object with the following properties: " +
		...
		try {
			if (typeof arg === "string") {
				...
				if (
					...
					const [gallery] =
						await extensionGalleryService.getExtensions(
							[
								{
									id,
									preRelease:
				...
				await extensionsWorkbenchService.install(vsix, {
					installOnlyNewlyAddedFromExtensionPack:
						options?.installOnlyNewlyAddedFromExtensionPackVSIX,
	...
	implements IWorkbenchContribution
{
	...
	private registerGlobalActions(): void {
						...
						[
							{
								...
								run: () =>
									extensionsWorkbenchService.updateRunningExtensions(),
							},
						],
					);
				} else {
		...
		this.registerExtensionAction({
			...
			run: () =>
				this.extensionsWorkbenchService.openSearch("@recommended "),
		});
			...
			this.registerExtensionAction({
				...
				run: async () => {
					const extensionsViewPaneContainer = (
						await this.viewsService.openViewContainer(
							VIEWLET_ID,
							true,
						)
					)?.getViewPaneContainer() as
						| IExtensionsViewPaneContainer
						| undefined;
		...
		this.registerExtensionAction({
			...
			run: async (accessor: ServicesAccessor) => {
				...
				if (viewPaneContainer) {
					const extensionsViewPaneContainer =
						viewPaneContainer as IExtensionsViewPaneContainer;
					extensionsViewPaneContainer.search("");
					extensionsViewPaneContainer.focus();
				}
			},
	...
	private registerContextMenuActions(): void {
		...
		this.registerExtensionAction({
			...
			run: async (accessor: ServicesAccessor, extensionId: string) => {
				...
				const action =
					instantiationService.createInstance(ClearLanguageAction);
				action.extension = extension;
				return action.run();
			},
		});
		...
		this.registerExtensionAction({
			...
			run: async (accessor: ServicesAccessor, id: string) =>
				accessor.get(IClipboardService).writeText(id),
		});
		...
		this.registerExtensionAction({
			...
			menu: {
				...
				when: ContextKeyExpr.and(
					ContextKeyExpr.equals("extensionStatus", "installed"),
					ContextKeyExpr.has("extensionHasAccountPreferences"),
				),
				order: 2,
			},
			run: (accessor: ServicesAccessor, id: string) =>
				accessor
					.get(ICommandService)
					.executeCommand(
		...
		this.registerExtensionAction({
			...
			async run(accessor: ServicesAccessor): Promise<any> {
				...
				const extensionId =
					editorService.activeEditor.extension.identifier.id.toLowerCase();
		...
		this.registerExtensionAction({
			...
			async run(accessor: ServicesAccessor): Promise<any> {
				...
				const unwantedRecommendations =
					await workspaceExtensionsConfigService.getUnwantedRecommendations();
	...
	private registerExtensionAction(
		...
		const menus = extensionActionOptions.menu
			? Array.isArray(extensionActionOptions.menu)
				...
				: [extensionActionOptions.menu]
			: [];
		let menusWithOutTitles: ({ id: MenuId } & Omit<
			IMenuItem,
			"command"
		>)[] = [];
