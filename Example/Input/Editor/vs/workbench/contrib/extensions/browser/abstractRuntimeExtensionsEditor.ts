...
export abstract class AbstractRuntimeExtensionsEditor extends EditorPane {
		...
		for (let i = 0, len = extensionsDescriptions.length; i < len; i++) {
			...
			if (profileInfo) {
				const extensionSegments =
					segments.get(extensionDescription.identifier) || [];
				let extensionTotalTime = 0;
				for (
					let j = 0, lenJ = extensionSegments.length / 2;
					j < lenJ;
					j++
				) {
	...
	protected createEditor(parent: HTMLElement): void {
		...
		const delegate = new (class
			implements IListVirtualDelegate<IRuntimeExtension>
		{
		...
		> = {
			...
			renderElement: (
				...
				data.name.textContent = (
					...
					element.description.identifier.value
				).substr(0, 50);
				...
				const slowExtensionAction =
					this._createSlowExtensionAction(element);
				...
				if (isNonEmptyArray(element.status.runtimeErrors)) {
					const reportExtensionIssueAction =
						this._createReportExtensionIssueAction(element);
				...
				if (activationTimes) {
					const activationId =
						activationTimes.activationReason.extensionId.value;
					const activationEvent =
						activationTimes.activationReason.activationEvent;
					if (activationEvent === "*") {
						title = nls.localize(
							{
								key: "starActivation",
				...
				if (
					this._getUnresponsiveProfile(element.description.identifier)
				) {
					const el = $(
						"span",
						undefined,
				...
				for (const feature of features) {
					const accessData =
						this._extensionFeaturesManagementService.getAccessData(
		...
		this._list = <WorkbenchList<IRuntimeExtension>>(
		...
		this._list.onContextMenu((e) => {
			...
			const reportExtensionIssueAction =
				this._createReportExtensionIssueAction(e.element);
			if (reportExtensionIssueAction) {
