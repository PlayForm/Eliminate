...
class Snapper {
	...
	private _themedTokenizeTreeSitter(
		...
		const colorThemeData =
			this.themeService.getColorTheme() as ColorThemeData;
		for (let i = 0, len = tokens.length; i < len; i++) {
			...
			const metadata = findMetadata(
				colorThemeData,
				scopes,
				this.languageService.languageIdCodec.encodeLanguageId(
					languageId,
				),
			);
			const color = TokenMetadata.getForeground(metadata);
	...
	private _treeSitterTokenize(
		...
		const tokenizationSupport =
			TreeSitterTokenizationRegistry.get(languageId);

		do {
			if (cursor.currentNode.childCount === 0) {
				const capture = tokenizationSupport?.captureAtPositionTree(
					cursor.currentNode.startPosition.row + 1,
	...
	public async captureTreeSitterSyntaxTokens(
		...
		if (languageId) {
			const tree = await this.treeSitterParserService.getTree(
				content,
				languageId!,
			);
			if (!tree) {
...
async function captureTokens(
	...
	const process = (resource: URI) => {
		...
		return fileService.readFile(resource).then((content) => {
			if (treeSitter) {
				return snapper.captureTreeSitterSyntaxTokens(
					fileName,
