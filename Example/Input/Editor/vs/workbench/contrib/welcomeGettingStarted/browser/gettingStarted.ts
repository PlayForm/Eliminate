...
import {
	...
	RestoreWalkthroughsConfigurationValue,
} from "./startupPage.js";
...
export class GettingStartedPage extends EditorPane {
	...
	constructor(
		...
		this._register(
			this.storageService.onWillSaveState((e) => {
				...
				const editorPane =
					this.groupsService.activeGroup.activeEditorPane;
	...
	private async buildMediaComponent(
		stepId: string,
	...
	private async selectStep(id: string | undefined, delayFocus = true) {
		if (id) {
			...
			const step = this.currentWalkthrough?.steps?.find(
				(step) => step.id === id,
			);
			if (step) {
				stepElement.setAttribute(
					"aria-label",
					`${this.provideScreenReaderUpdate()} ${step.title}`,
				);
			}
		} else {
	...
	protected createEditor(parent: HTMLElement) {
		...
		const prevButton = $(
			...
			$("span.moreText", {}, localize("goBack", "Go Back")),
		);
		this.stepsSlide = $(
	...
	private setSlide(
		...
		if (toEnable === "categories") {
			...
			const prevButton = this.container.querySelector<HTMLButtonElement>(
				".prev-button.button-link",
			);
