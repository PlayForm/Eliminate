...
export class TestingExplorerFilter extends BaseActionViewItem {
	...
	private readonly history: StoredValue<
		{ values: string[]; lastValue: string } | string[]
	> = this._register(
	...
	private getActions(): IAction[] {
		return [
			...
			new Separator(),
			{
				...
				run: () =>
					(this.filters.fuzzy.value = !this.filters.fuzzy.value),
				tooltip: "",
			},
			new Separator(),
			{
				...
				run: () =>
					this.filters.toggleFilteringFor(TestFilterTerm.Hidden),
				tooltip: "",
			},
			{
				class: undefined,
