					...
					new Action(
						...
						() => {
							const { include, exclude } =
								this.getTreeIncludeExclude(profile);
...
class TestingExplorerViewModel extends Disposable {
	...
	constructor(
		...
		this.tree = instantiationService.createInstance(
			...
			{
				identityProvider:
					instantiationService.createInstance(IdentityProvider),
		...
		this._register(
			this.crService.onDidChange((testId) => {
				if (testId) {
					...
					const elem =
						this.projection.value?.getElementByTestId(testId);
					this.tree.resort(
		...
		this._register(
			this.tree.onDidOpen((e) => {
				if (
					e.element instanceof TestItemTreeElement &&
		...
		this._register(
			autorun((reader) => {
				this.revealById(
					filterState.reveal.read(reader),
					undefined,
					false,
				);
			}),
		...
		this._register(
			this.tree.onDidChangeSelection((evt) => {
				if (
					dom.isMouseEvent(evt.browserEvent) &&
				...
				if (
					...
					filterState.didSelectTestInExplorer(
						selected.test.item.extId,
					);
					if (
		...
		this._register(
			configurationService.onDidChangeConfiguration((e) => {
				if (
					e.affectsConfiguration(TestingConfigKeys.FollowRunningTest)
				) {
