...
export class InlineCompletionLanguageStatusBarContribution extends Disposable {
	...
	private readonly _inlineCompletionInlineEdits = observableConfigValue(
		"editor.inlineSuggest.experimentalInlineEditsEnabled",
		false,
			...
			autorunWithStore((reader, store) => {
				...
				> = {
					...
					ghostText: {
						...
						label: "Inline Completion available",
						loading: false,
					},
					inlineEdit: {
						shortLabel: "$(lightbulb-sparkle)",
						label: "Inline Edit available",
						loading: false,
					},
					noSuggestion: {
						shortLabel: "$(circle-slash)",
						label: "No inline suggestion available",
						loading: false,
					},
				};
