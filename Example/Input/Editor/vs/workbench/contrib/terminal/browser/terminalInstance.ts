...
import {
	IRequestAddInstanceToGroupEvent,
...
export class TerminalInstance extends Disposable implements ITerminalInstance {
	...
	private _targetRef: ImmortalReference<TerminalLocation | undefined> =
		new ImmortalReference(undefined);
	...
	private readonly _onLineData = this._register(
		new Emitter<string>({
			onDidAddFirstListener: async () =>
				(this.xterm ?? (await this._xtermReadyPromise))?.raw.loadAddon(
					this._lineDataEventAddon!,
				),
		}),
	);
		...
		for (const desc of contributionDescs) {
			...
			try {
				contribution = this._register(
					this._scopedInstantiationService.createInstance(desc.ctor, {
						instance: this,
	...
	public static getXtermConstructor(
		...
		const keybinding = keybindingService.lookupKeybinding(
			TerminalContribCommandId.A11yFocusAccessibleBuffer,
			contextKeyService,
		);
		if (xtermConstructor) {
	...
	protected async _createXterm(): Promise<XtermTerminal | undefined> {
		...
		this._register(
			this.xterm.onDidRequestRefreshDimensions(() => {
