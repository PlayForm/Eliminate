...
export class TextSearchResultRenderer
	extends Disposable
	implements
		ICompressibleTreeRenderer<
			ITextSearchHeading,
			any,
			ITextSearchResultTemplate
		>
{
	...
	renderTemplate(container: HTMLElement): ITextSearchResultTemplate {
		...
		const textSearchResultElement = DOM.append(
			container,
			DOM.$(".textsearchresult"),
		);
	...
	async renderElement(
		node: ITreeNode<ITextSearchHeading, any>,
		index: number,
		...
		if (isPlainTextSearchHeading(node.element)) {
			...
			templateData.label.setLabel(
				`$(${Codicon.copilot.id}) ${localizedLabel}`,
			);
		}
	}
		...
		>
{
	...
	renderCompressedElements(
		node: ITreeNode<ICompressedTreeNode<ISearchTreeFolderMatch>, any>,
		index: number,
		templateData: IFolderMatchTemplate,
		height: number | undefined,
	): void {
	...
	renderElement(
		node: ITreeNode<ISearchTreeFolderMatch, any>,
		index: number,
		templateData: IFolderMatchTemplate,
	): void {
		...
		>
{
	...
	renderElement(
		node: ITreeNode<ISearchTreeFileMatch, any>,
		index: number,
		templateData: IFileMatchTemplate,
	): void {
		...
		ICompressibleTreeRenderer<ISearchTreeMatch, void, IMatchTemplate>
{
	...
	renderElement(
		node: ITreeNode<ISearchTreeMatch, any>,
		index: number,
		templateData: IMatchTemplate,
	): void {
