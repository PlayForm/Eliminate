	...
	implements ITextSearchHeading
{
	...
	isEmpty(): boolean {
		return this.folderMatches().every((folderMatch) =>
			folderMatch.isEmpty(),
		);
	}
	...
	protected clearQuery(): void {
		...
		this._folderMatchesMap =
			TernarySearchTree.forUris<ISearchTreeFolderMatchWithResource>(
	...
	implements IPlainTextSearchHeading
{
	...
	replace(match: ISearchTreeFileMatch): Promise<any> {
		return (
			this.getFolderMatch(match.resource)?.replace(match) ??
			Promise.resolve()
		);
	}
