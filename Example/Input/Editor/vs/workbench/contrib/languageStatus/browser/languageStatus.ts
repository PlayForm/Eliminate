...
class LanguageStatusViewModel {
	...
	isEqual(other: LanguageStatusViewModel) {
		return (
			equals(this.combined, other.combined) &&
...
export class LanguageStatusContribution
	extends Disposable
...
class LanguageStatus {
	...
	private _update(): void {
		...
		if (model.combined.length === 0) {
			...
			const userHasInteractedWithStatus =
	...
	private static _asStatusbarEntry(item: ILanguageStatus): IStatusbarEntry {
		...
		const textValue =
			typeof item.label === "string" ? item.label : item.label.shortValue;
