	...
	private async _participate(
		...
		if (sessions.size === 1) {
			...
			const labels = Array.from(
				Iterable.map(sessions.values(), (i) =>
					this._labelService.getUriBasenameLabel(
						i.session.textModelN.uri,
					),
				),
			);
			message = localize(
		...
		const result = await this._dialogService.confirm({
			...
			checkbox: {
				label: localize(
					"config",
					"Always save with AI-generated changes without asking",
				),
				checked: false,
			},
		});
	...
	private _isDisabled() {
		return (
			this._configService.getValue<boolean>(
				InlineChatConfigKeys.AcceptedOrDiscardBeforeSave,
			) === true || this._configService.getValue(key)
		);
	}
}
