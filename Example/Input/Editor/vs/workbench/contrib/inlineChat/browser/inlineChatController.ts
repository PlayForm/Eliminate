...
export class InlineChatController implements IEditorContribution {
	...
	constructor(
		...
		this._ui = new Lazy(() => {
			...
			const zone = _instaService.createInstance(
			...
			this._store.add(
				zone.widget.chatWidget.onDidClear(async () => {
	...
	saveViewState(): any {
		...
		const { selectedLanguageModelId } =
			this._ui.rawValue.widget.chatWidget.getViewState();
	...
	private async [State.INIT_UI](
		...
		this._ui.value.widget.setChatModel(
	...
	async reviewEdits(
		...
		await Event.toPromise(
			Event.filter(
				this._onDidEnterState.event,
				(candidate) => candidate === State.WAIT_FOR_INPUT,
			),
		);
