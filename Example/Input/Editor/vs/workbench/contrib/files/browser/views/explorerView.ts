...
import {
	...
	FileSorter,
	FilesRenderer,
	ICompressedNavigationController,
...
class ExplorerFindProvider implements IAsyncFindProvider<ExplorerItem> {
	...
	readonly placeholder: string = nls.localize(
	...
	async *getFindResults(
		pattern: string,
		sessionId: number,
		token: CancellationToken,
		toggleStates: ITreeFindToggleContribution[],
	): AsyncIterable<IAsyncFindResult<ExplorerItem>> {
		const isFuzzyMatch = toggleStates.find(
			(t) => t.id === explorerFuzzyMatch.id,
		)?.isChecked!!;

		const workspaceFolders =
			this.workspaceContextService.getWorkspace().folders;
		const folderPromises = Promise.all(
	...
	private async *createResultItems(
		folderResults: { folder: URI; result: ISearchComplete }[],
		pattern: string,
		...
		for (const { folder, result } of folderResults) {
			const folderRoot = new ExplorerItem(
				folder,
				this.fileService,
			...
			for (const file of result.results) {
				...
				if (isFuzzyMatch) {
					...
					filterdata = contiguousFuzzyScore(
						lowercasePattern,
						baseName.toLowerCase(),
					);
				}
...
export class ExplorerView extends ViewPane implements IExplorerView {
	...
	private createTree(container: HTMLElement): void {
		...
		const rootsSupportFindProvider = this.explorerService.roots.every(
			(root) =>
				root.resource.scheme === Schemas.file ||
