...
class CommandCenterCenterViewItem extends BaseActionViewItem {
	...
	constructor(
		...
		super(
			undefined,
			_submenu.actions.find(
				(action) => action.id === "workbench.action.quickOpenWithModes",
			) ?? _submenu.actions[0],
			options,
		);
		this._hoverDelegate =
			...
			const toolbar = this._instaService.createInstance(
				...
				container,
				{
					...
					actionViewItemProvider: (action, options) => {
						...
						if (
							action.id !==
							CommandCenterCenterViewItem._quickOpenCommandId
						) {
							return createActionViewItem(
								this._instaService,
								action,
						...
						return this._instaService.createInstance(
							class CommandCenterQuickPickItem extends BaseActionViewItem {
								...
								private _getLabel(): string {
									...
									if (
										that._windowTitle.isCustomTitleFormat()
									) {
										label =
											that._windowTitle.getWindowTitle();
									} else if (
										that._editorGroupService.partOptions
											.showTabs === "none"
									) {
										label =
