...
type Tree<I, E> =
	| WorkbenchDataTree<I, E, FuzzyScore>
...
class FileVirtualDelegate
	implements IListVirtualDelegate<IFileStat | IWorkspaceFolder>
{
...
class FileIdentityProvider
	implements
		IIdentityProvider<IWorkspace | IWorkspaceFolder | IFileStat | URI>
{
	getId(element: IWorkspace | IWorkspaceFolder | IFileStat | URI): {
		toString(): string;
	} {
...
class FileDataSource
	implements IAsyncDataSource<IWorkspace | URI, IWorkspaceFolder | IFileStat>
{
	constructor(@IFileService private readonly _fileService: IFileService) {}

	hasChildren(
		element: IWorkspace | URI | IWorkspaceFolder | IFileStat,
	): boolean {
		return (
			URI.isUri(element) ||
			isWorkspace(element) ||
	...
	async getChildren(
		element: IWorkspace | URI | IWorkspaceFolder | IFileStat,
	): Promise<(IWorkspaceFolder | IFileStat)[]> {
...
class FileRenderer
	implements
		ITreeRenderer<IFileStat | IWorkspaceFolder, FuzzyScore, IResourceLabel>
{
	...
	renderElement(
		node: ITreeNode<IWorkspaceFolder | IFileStat, [number, number, number]>,
		index: number,
		templateData: IResourceLabel,
	): void {
		const fileDecorations = this._configService.getValue<{
			colors: boolean;
			badges: boolean;
...
class FileNavigationLabelProvider
	implements IKeyboardNavigationLabelProvider<IWorkspaceFolder | IFileStat>
{
	getKeyboardNavigationLabel(element: IWorkspaceFolder | IFileStat): {
		toString(): string;
	} {
		return element.name;
	}
...
class FileAccessibilityProvider
	implements IListAccessibilityProvider<IWorkspaceFolder | IFileStat>
{
...
class FileFilter implements ITreeFilter<IWorkspaceFolder | IFileStat> {
	private readonly _cachedExpressions = new Map<
		string,
	...
	filter(
		element: IWorkspaceFolder | IFileStat,
		_parentVisibility: TreeVisibility,
	): boolean {
		if (isWorkspaceFolder(element)) {
			// not a file
			return true;
...
export class FileSorter implements ITreeSorter<IFileStat | IWorkspaceFolder> {
	compare(
		a: IFileStat | IWorkspaceFolder,
		b: IFileStat | IWorkspaceFolder,
	): number {
...
export class BreadcrumbsFilePicker extends BreadcrumbsPicker {
	...
	protected _createTree(container: HTMLElement) {
		...
		const labels = this._instantiationService.createInstance(
			ResourceLabels,
			DEFAULT_LABELS_CONTAINER /* TODO@Jo visibility propagation */,
		);
		...
		>this._instantiationService.createInstance(
			...
			this._instantiationService.createInstance(FileDataSource),
			{
				...
				keyboardNavigationLabelProvider:
					new FileNavigationLabelProvider(),
	...
	protected async _setInput(
		...
		const tree = this._tree as WorkbenchAsyncDataTree<
			IWorkspace | URI,
			IWorkspaceFolder | IFileStat,
			FuzzyScore
		>;
	...
	protected async _revealElement(
		element: IFileStat | IWorkspaceFolder,
		options: IEditorOptions,
		sideBySide: boolean,
	): Promise<boolean> {
...
export class BreadcrumbsOutlinePicker extends BreadcrumbsPicker {
	protected _createTree(container: HTMLElement, input: OutlineElement2) {
		...
		return <WorkbenchDataTree<IOutline<any>, any, FuzzyScore>>(
			this._instantiationService.createInstance(
				WorkbenchDataTree,
	...
	protected _setInput(input: OutlineElement2): Promise<void> {
		...
		const tree = this._tree as WorkbenchDataTree<
			IOutline<any>,
			any,
			FuzzyScore
		>;
