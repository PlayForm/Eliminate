...
export class BreadcrumbsControl {
	...
	constructor(
		...
		this._cfUseQuickPick =
			BreadcrumbsConfig.UseQuickPick.bindTo(configurationService);
		this._cfShowIcons =
			BreadcrumbsConfig.Icons.bindTo(configurationService);
		this._cfTitleScrollbarSizing =
			BreadcrumbsConfig.TitleScrollbarSizing.bindTo(configurationService);

		this._labels = this._instantiationService.createInstance(
			ResourceLabels,
		...
		this._widget.onDidSelectItem(
			this._onSelectEvent,
			this,
			this._disposables,
		);
		this._widget.onDidFocusItem(
			this._onFocusEvent,
			this,
			this._disposables,
		);
		this._widget.onDidChangeFocus(
			this._updateCkBreadcrumbsActive,
			this,
			this._disposables,
		);

		this._ckBreadcrumbsPossible =
			BreadcrumbsControl.CK_BreadcrumbsPossible.bindTo(
				this._contextKeyService,
			);
		this._ckBreadcrumbsVisible =
			BreadcrumbsControl.CK_BreadcrumbsVisible.bindTo(
				this._contextKeyService,
			);
		this._ckBreadcrumbsActive =
			BreadcrumbsControl.CK_BreadcrumbsActive.bindTo(
				this._contextKeyService,
			);
		...
		this._disposables.add(
			breadcrumbsService.register(this._editorGroup.id, this._widget),
		);
		this.hide();
	}
...
export class BreadcrumbsControlFactory {
	...
	constructor(
		...
		const config = this._disposables.add(
			BreadcrumbsConfig.IsEnabled.bindTo(configurationService),
		);
		this._disposables.add(
			config.onDidChange(() => {
		...
		this._disposables.add(
			fileService.onDidChangeFileSystemProviderRegistrations((e) => {
				if (
					this._control?.model &&
					this._control.model.resource.scheme !== e.scheme
				) {
