...
class CommentThreadWithDisposable {
	...
	constructor(
		public readonly thread: MainThreadCommentThread<IRange | ICellRange>,
	) {}
	dispose() {
...
export class MainThreadCommentController implements ICommentController {
	...
	private readonly _threads: DisposableMap<
		number,
		CommentThreadWithDisposable
	> = new DisposableMap<number, CommentThreadWithDisposable>();
	...
	async getNotebookComments(resource: URI, token: CancellationToken) {
		...
		for (const thread of [...this._threads.keys()]) {
			...
			if (commentThread.thread.resource === resource.toString()) {
				if (!commentThread.thread.isDocumentCommentThread()) {
					ret.push(
						commentThread.thread as languages.CommentThread<ICellRange>,
					);
				}
			}
		}
