...
type ViewContainerExtensionPointType = {
	[loc: string]: IUserFriendlyViewsContainerDescriptor[];
};
const viewsContainersExtensionPoint: IExtensionPoint<ViewContainerExtensionPointType> =
	ExtensionsRegistry.registerExtensionPoint<ViewContainerExtensionPointType>({
		extensionPoint: "viewsContainers",
		jsonSchema: viewsContainersContribution,
	});

type ViewExtensionPointType = { [loc: string]: IUserFriendlyViewDescriptor[] };
const viewsExtensionPoint: IExtensionPoint<ViewExtensionPointType> =
	ExtensionsRegistry.registerExtensionPoint<ViewExtensionPointType>({
...
class ViewsExtensionHandler implements IWorkbenchContribution {
	...
	private addCustomViewContainers(
		...
		let activityBarOrder =
			CUSTOM_VIEWS_START_ORDER +
			viewContainersRegistry.all.filter(
				(v) =>
					!!v.extensionId &&
					viewContainersRegistry.getViewContainerLocation(v) ===
	...
	private removeCustomViewContainers(
		...
		for (const viewContainer of viewContainersRegistry.all) {
			if (
				viewContainer.extensionId &&
				removedExtensions.has(viewContainer.extensionId)
			) {
	...
	private isValidViewsContainer(
		...
		for (const descriptor of viewsContainersDescriptors) {
			if (
				...
				isFalsyOrWhitespace(descriptor.id)
			) {
				collector.error(
					localize(
						"requireidstring",
			...
			Object.entries(value).forEach(([key, value]) => {
				...
				for (let index = 0; index < value.length; index++) {
					...
					const viewDescriptor: ICustomViewDescriptor = {
						...
						group: item.group,
						remoteAuthority:
							item.remoteName || (<any>item).remoteAuthority, // TODO@roblou - delete after remote extensions are updated
						virtualWorkspace: item.virtualWorkspace,
						hideByDefault:
							initialVisibility === InitialVisibility.Hidden,
						workspace:
							viewContainer?.id === REMOTE ? true : undefined,
						weight,
						accessibilityHelpContent,
					};
	...
	private removeViews(
		...
		for (const viewContainer of this.viewContainersRegistry.all) {
			const removedViews = this.viewsRegistry
				...
				.filter(
					(v) =>
						(v as ICustomViewDescriptor).extensionId &&
						removedExtensions.has(
							(v as ICustomViewDescriptor).extensionId,
						),
				);
...
class ViewContainersDataRenderer
	extends Disposable
