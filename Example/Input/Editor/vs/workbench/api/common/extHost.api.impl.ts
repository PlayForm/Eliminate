...
export function createApiFactoryAndRegisterActors(
	...
	return function (
		...
		function _asExtensionEvent<T>(
			actual: vscode.Event<T>,
		): vscode.Event<T> {
		...
		const extensions: typeof vscode.extensions = {
			getExtension(
				...
				const mine =
					extensionInfo.mine.getExtensionDescription(extensionId);
				if (mine) {
		...
		const languages: typeof vscode.languages = {
			...
			registerDocumentDropEditProvider(
				...
				return extHostLanguageFeatures.registerDocumentOnDropEditProvider(
					...
					selector,
					provider,
					isProposedApiEnabled(extension, "documentPaste")
						? metadata
						: undefined,
				);
			},
		...
		const window: typeof vscode.window = {
			...
			showInformationMessage(
				message: string,
				...rest: Array<
					vscode.MessageOptions | string | vscode.MessageItem
				>
			) {
				return <Thenable<any>>(
		...
		const workspace: typeof vscode.workspace = {
			...
			createFileSystemWatcher: (
				...
				let options: FileSystemWatcherCreateOptions | undefined =
					undefined;
		...
		const tasks: typeof vscode.tasks = {
			...
			fetchTasks: (
				filter?: vscode.TaskFilter,
			): Thenable<vscode.Task[]> => {
		...
		const lm: typeof vscode.lm = {
			...
			fileIsIgnored(uri: vscode.Uri, token: vscode.CancellationToken) {
				return extHostLanguageModels.fileIsIgnored(
					extension,
					uri,
					token,
				);
		...
		return <typeof vscode>{
			...
			DebugAdapterNamedPipeServer:
				extHostTypes.DebugAdapterNamedPipeServer,
			...
			DiagnosticRelatedInformation:
				extHostTypes.DiagnosticRelatedInformation,
			...
			InlineCompletionTriggerKind:
				extHostTypes.InlineCompletionTriggerKind,
			...
			TextEditorSelectionChangeKind:
				extHostTypes.TextEditorSelectionChangeKind,
			...
			RemoteAuthorityResolverError:
				extHostTypes.RemoteAuthorityResolverError,
			...
			NotebookVariablesRequestKind:
				extHostTypes.NotebookVariablesRequestKind,
			...
			ChatResponseCommandButtonPart:
				extHostTypes.ChatResponseCommandButtonPart,
			...
			LanguageModelChatMessageRole:
				extHostTypes.LanguageModelChatMessageRole,
