			...
			this._apiObject = Object.freeze<vscode.ChatResponseStream>({
				...
				push(part) {
					...
					if (
						...
						const dto =
							typeConvert.ChatResponseAnchorPart.from(part);
	...
	implements ExtHostChatAgentsShape2
{
	...
	private async getModelForRequest(
...
class ExtHostChatAgent {
	...
	async provideWelcomeMessage(
		...
		const content =
			await this._welcomeMessageProvider.provideWelcomeMessage(token);
