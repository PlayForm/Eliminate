			...
			if (diagnostics) {
				...
				if (diagnostics.length > this._maxDiagnosticsPerFile) {
						...
						for (const diagnostic of diagnostics) {
							if (diagnostic.severity === order[i]) {
								const len = marker.push({
									...
									modelVersionId:
										this._modelVersionIdProvider(uri),
								});
					...
					marker.push({
						...
						startLineNumber:
							marker[marker.length - 1].startLineNumber,
...
export class ExtHostDiagnostics implements ExtHostDiagnosticsShape {
	...
	private static readonly _maxDiagnosticsTotal: number =
		1.1 * this._maxDiagnosticsPerFile;

	private readonly _proxy: MainThreadDiagnosticsShape;
	private readonly _collections = new Map<string, DiagnosticCollection>();
	private readonly _onDidChangeDiagnostics = new DebounceEmitter<
		readonly vscode.Uri[]
	>({ merge: (all) => all.flat(), delay: 50 });
		...
		const result = new (class extends DiagnosticCollection {
			constructor() {
				super(
					...
					(uri) =>
						_extHostDocumentsAndEditors.getDocument(uri)?.version,
	...
	$acceptMarkersChange(data: [UriComponents, IMarkerData[]][]): void {
		if (!this._mirrorCollection) {
			...
			const collection = new DiagnosticCollection(
				...
				Number.MAX_SAFE_INTEGER, // no limits because this collection is just a mirror of "sanitized" data
				(_uri) => undefined,
