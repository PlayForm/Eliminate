...
export class ExtHostLanguageModels implements ExtHostLanguageModelsShape {
			...
			async (fragment) => {
				...
				if (
					...
					part = {
						...
						parameters:
							fragment.part.input ?? fragment.part.parameters,
	...
	async getDefaultLanguageModel(
		extension: IExtensionDescription,
	): Promise<vscode.LanguageModelChat | undefined> {
		const defaultModelId = Iterable.find(
			this._allLanguageModelData.entries(),
			([, value]) => !!value.metadata.isDefault,
		)?.[0];
		if (!defaultModelId) {
			return;
		}
	...
	async getLanguageModelByIdentifier(
		extension: IExtensionDescription,
		identifier: string,
	): Promise<vscode.LanguageModelChat | undefined> {
	...
	async selectLanguageModels(
		...
		for (const identifier of models) {
			const model = await this.getLanguageModelByIdentifier(
				extension,
				identifier,
			);
			if (model) {
	...
	async $isFileIgnored(
		...
		return (
			(await provider.provideFileIgnored(URI.revive(uri), token)) ?? false
		);
	}
