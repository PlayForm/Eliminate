...
import {
	...
	IThemeScopableCustomizations,
	IThemeScopedCustomizations,
...
export function findMetadata(
	...
	const tokenStyle = colorThemeData.resolveScopes(
		[captureNames],
		definitions,
	);

	if (captureNames.length > 0) {
		const standardToken = toStandardTokenType(
			captureNames[captureNames.length - 1],
		);
