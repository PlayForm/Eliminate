	...
	implements IAuthenticationExtensionsService
{
	...
	private _onDidAccountPreferenceChange: Emitter<{
		...
		new Emitter<{ providerId: string; extensionIds: string[] }>(),
	);
	...
	).reduce<{ [extensionId: string]: string }>((acc, [parent, children]) => {
	...
	updateAccountPreference(
		...
		const parentExtensionId =
			this._inheritAuthAccountPreferenceChildToParent[realExtensionId] ??
			realExtensionId;
