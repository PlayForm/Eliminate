...
import {
	ColorThemeData,
	findMetadata,
} from "../../themes/common/colorThemeData.js";
	...
	implements ITreeSitterTokenizationSupport
{
	...
	public tokenizeEncoded(
		...
		const encodedLanguageId = this._languageIdCodec.encodeLanguageId(
			this._languageId,
		);
		...
		for (
			...
			if (previousTokenEnd >= lineRelativeOffset) {
				const previousTokenStartOffset =
					tokenIndex >= 2
				...
				if (
					previousTokenStartOffset + currentTokenLength ===
					...
					endOffsetsAndScopes[tokenIndex - 1].scopes[
						endOffsetsAndScopes[tokenIndex - 1].scopes.length - 1
					] = capture.name;
				} else {
