...
} from "./rectangleRenderer.wgsl.js";
...
export class RectangleRenderer extends ViewEventHandler {
	...
	private readonly _shapeBindBuffer: MutableDisposable<
	...
	private readonly _shapeCollection: IObjectCollectionBuffer<RectangleRendererEntrySpec> =
		this._register(
			createObjectCollectionBuffer(
				[
					{ name: "x" },
	...
	private async _initWebgpu(device: Promise<GPUDevice>) {
		...
		this._register(
			Event.runAndSubscribe(
				this._shapeCollection.onDidChangeBuffer,
				() => {
					this._shapeBindBuffer.value = createShapeBindBuffer();
					if (this._pipeline) {
	...
	private _updateBindGroup(
		...
		layoutInfoUniformBuffer: GPUBuffer,
	) {
