					...
					this._languageFeaturesService.inlineCompletionsProvider.register(
						"*",
						{
							async provideInlineCompletions(
								...
								const allInlineEditProvider =
									_languageFeaturesService.inlineEditProvider.all(
										model,
									);
								const inlineEdits = await Promise.all(
									allInlineEditProvider.map(
										async (provider) => {
											const result =
												await provider.provideInlineEdit(
													model,
													{
														triggerKind:
								...
								const definedEdits = inlineEdits.filter(
									(e) => !!e,
								);
								return {
									edits: definedEdits,
