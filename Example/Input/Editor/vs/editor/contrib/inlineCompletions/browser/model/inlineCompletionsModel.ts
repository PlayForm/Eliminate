...
export class InlineCompletionsModel extends Disposable {
	...
	public stop(
		stopReason: "explicitCancel" | "automatic" = "automatic",
		tx?: ITransaction,
	): void {
		subtransaction(tx, (tx) => {
			if (stopReason === "explicitCancel") {
				const completion =
					this.state.get()?.inlineCompletion?.inlineCompletion;
				if (completion && completion.source.provider.handleRejection) {
					completion.source.provider.handleRejection(
	...
	private readonly _collapsedInlineEditId = observableValue<
		string | undefined
	>(this, undefined);
	...
	public collapseInlineEdit(): void {
		...
		this._collapsedInlineEditId.set(
			currentInlineEdit.semanticId,
			undefined,
		);
	}

	private readonly _inlineCompletionItems = derivedOpts(
		{ owner: this },
		(reader) => {
	...
	>(
		{
			...
			equalsFn: (a, b) => {
				...
				if (a.kind === "ghostText" && b.kind === "ghostText") {
					...
					return (
						a.inlineEdit.equals(b.inlineEdit) &&
		...
		(reader) => {
			...
			if (item?.inlineEditCompletion) {
				...
				const cursorAtInlineEdit = LineRange.fromRangeInclusive(
					edit.range,
				)
					.addMargin(1, 1)
				...
				const cursorDist = LineRange.fromRange(
					edit.range,
				).distanceToLine(this._primaryPosition.read(reader).lineNumber);
				const disableCollapsing = true;
				const currentItemIsCollapsed =
					!disableCollapsing &&
					cursorDist > 1 &&
				...
				const commands =
					item.inlineEditCompletion.inlineCompletion.source
						.inlineCompletions.commands;
				const inlineEdit = new InlineEdit(
					edit,
					currentItemIsCollapsed,
					false,
					commands ?? [],
				);
	...
	private readonly _alwaysShowInlineEdit = observableValue<boolean>(
		this,
		false,
	);

	protected readonly _resetAlwaysShowInlineEdit = this._register(
		autorun((reader) => {
