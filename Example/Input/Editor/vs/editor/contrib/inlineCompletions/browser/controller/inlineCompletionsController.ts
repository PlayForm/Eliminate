...
} from "../../../../../base/common/hotReloadHelpers.js";
import {
	Disposable,
	toDisposable,
...
export class InlineCompletionsController extends Disposable {
	...
	public static get(editor: ICodeEditor): InlineCompletionsController | null {
		return hotClassGetOriginalInstance(
			editor.getContribution<InlineCompletionsController>(
		...
		this._register(
			runOnChange(this._editorObs.selections, (_value, _, changes) => {
				if (
					...
					if (
						!this._hideInlineEditOnSelectionChange.get() &&
...
.monaco-editor .ghost-text {
	font-family: ${fontFamily};
		...
		const contextKeySvcObs = new ObservableContextKeyService(
			this._contextKeyService,
		);
