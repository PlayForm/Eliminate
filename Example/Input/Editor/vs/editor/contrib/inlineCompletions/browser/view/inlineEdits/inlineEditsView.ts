...
} from "./inlineDiffView.js";
import {
	IInlineEditsIndicatorState,
	InlineEditsIndicator,
} from "./inlineEditsIndicatorView.js";
import {
...
export class InlineEditsViewAndDiffProducer extends Disposable {
	...
	private readonly _modelUriGenerator = new UniqueUriGenerator(
		"inline-edits",
	);
	...
	private readonly _differ = new LRUCachedFunction(
		...
		(arg: { original: string; modified: string }) => {
			...
			return ObservablePromise.fromFn(async () => {
				const result = await diffAlgo.computeDiff(
	...
	private readonly _inlineEditPromise = derived<
		IObservable<InlineEditWithChanges | undefined> | undefined
	>(this, (reader) => {
		...
		return diffResult.promiseResult.map((p) => {
			...
			return new InlineEditWithChanges(
				...
				true,
				inlineEdit.commands,
			); //inlineEdit.showInlineIfPossible);
		});
	});

	private readonly _inlineEdit = derivedOpts(
		{ owner: this, equalsFn: equalsIfDefined(itemEquals()) },
		(reader) => this._inlineEditPromise.read(reader)?.read(reader),
	);

	constructor(
		private readonly _editor: ICodeEditor,
		private readonly _edit: IObservable<InlineEdit | undefined>,
		private readonly _model: IObservable<
...
export class InlineEditWithChanges {
	public readonly lineEdit = SingleLineEdit.fromSingleTextEdit(
...
export class InlineEditsView extends Disposable {
	...
	private readonly _elements = h(
		...
		[
			h(
				"div.editorContainer@editorContainer",
	...
	constructor(
		...
		const toolbarDropdownVisible = observableFromEvent(
			this,
			this._toolbar.onDidChangeDropdownVisibility,
			(e) => e ?? false,
		);
		...
		this._register(
			autorun((reader) => {
				this._elements.root.classList.toggle(
					...
					toolbarDropdownVisible.read(reader),
				);
			}),
		);
	}
	...
	private readonly _uiState = derived(this, (reader) => {
		...
		const indentationAdjustmentEdit = createReindentEdit(
			newText,
		...
		const diff = lineRangeMappingFromRangeMappings(
			mappings,
			edit.originalText,
		...
		const originalDisplayRange = edit.originalText.lineRange.intersect(
			edit.originalLineRange.join(
				LineRange.ofLength(
					edit.originalLineRange.startLineNumber,
					edit.lineEdit.newLines.length,
				),
			),
		)!;
		...
		if (edit.isCollapsed) {
			...
			edit.showInlineIfPossible &&
			diff.every((m) =>
				OriginalEditorInlineDiffView.supportsInlineDiffRendering(m),
			)
		) {
			state = "inline";
		}
	...
	private readonly _extraCommands = derivedOpts<readonly Command[]>(
	...
	protected readonly _updateToolbarAutorun = this._register(
		autorun((reader) => {
	...
	private readonly _previewEditorLeft = derived(this, (reader) => {
		...
		const maxLeft = maxLeftInRange(
			this._editorObs,
			state.originalDisplayRange,
			reader,
		);
	...
	private readonly _previewEditorLayoutInfo = derived(this, (reader) => {
		...
		const left =
			this._previewEditorLeft.read(reader)!.left + 20 - scrollLeft;
	...
	private readonly _inlineDiffViewState = derived<
		IOriginalEditorInlineDiffViewState | undefined
	>(this, (reader) => {
	...
	protected readonly _indicator = this._register(
		new InlineEditsIndicator(
			this._editorObs,
			derived<IInlineEditsIndicatorState | undefined>((reader) => {
				...
				return {
					editTopLeft: edit1,
					showAlways:
						state.state === "collapsed" || state.state === "inline",
				};
			}),
			this._model,
		),
