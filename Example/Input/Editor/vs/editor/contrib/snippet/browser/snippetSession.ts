...
export class OneSnippet {
	...
	get activeChoice(): { choice: Choice; range: Range } | undefined {
		...
		const placeholder =
			this._placeholderGroups[this._placeholderGroupsIdx][0];
			...
			for (const placeholder of this._placeholderGroups[
				...
				for (const placeholder of nested._snippet.placeholders) {
					const placeholderOffset =
						nested._snippet.offset(placeholder);
...
export class SnippetSession {
	...
	static createEditsAndSnippetsFromSelections(
		...
		const modelBasedVariableResolver = editor.invokeWithinContext(
			(accessor) =>
				new ModelBasedVariableResolver(
		...
		const firstLineFirstNonWhitespace =
		...
		for (const { selection, idx } of indexedSelections) {
			...
			const snippetLineLeadingWhitespace =
			...
			snippet.resolveVariables(
				new CompositeSnippetVariableResolver([
					...
					new ClipboardBasedVariableResolver(
						...
						editor.getOption(EditorOption.multiCursorPaste) ===
							"spread",
					),
	...
	static createEditsAndSnippetsFromEdits(
		...
		//
		snippetEdits = snippetEdits.sort((a, b) =>
