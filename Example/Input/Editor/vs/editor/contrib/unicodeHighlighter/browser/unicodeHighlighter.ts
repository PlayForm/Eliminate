	...
	implements IEditorContribution
{
		...
		if (state && state.hasMore) {
			...
			if (state.nonBasicAsciiCharacterCount >= max) {
				...
				data = {
					...
					command:
						new DisableHighlightingOfAmbiguousCharactersAction(),
				};
				...
				data = {
					...
					command:
						new DisableHighlightingOfInvisibleCharactersAction(),
				};
			} else {
	...
	private _updateHighlighter(): void {
		...
		if (
			this._editorWorkerService.canComputeUnicodeHighlights(
	...
	): UnicodeHighlighterDecorationInfo | null {
		if (this._highlighter) {
...
type Resolve<T> = T extends InUntrustedWorkspace
	? never
	: T extends "auto"
		? never
		: T;
...
type ResolvedOptions = {
	[TKey in keyof InternalUnicodeHighlightOptions]: Resolve<
		InternalUnicodeHighlightOptions[TKey]
	>;
};

function resolveOptions(
	trusted: boolean,
	options: InternalUnicodeHighlightOptions,
): ResolvedOptions {
	return {
		nonBasicASCII:
			options.nonBasicASCII === inUntrustedWorkspace
				? !trusted
				: options.nonBasicASCII,
		ambiguousCharacters: options.ambiguousCharacters,
		invisibleCharacters: options.invisibleCharacters,
		includeComments:
			options.includeComments === inUntrustedWorkspace
				? !trusted
				: options.includeComments,
		includeStrings:
			options.includeStrings === inUntrustedWorkspace
				? !trusted
				...
				if (!info.hasMore) {
					...
					for (const range of info.ranges) {
						decorations.push({
							range: range,
							options:
								Decorations.instance.getDecorationFromOptions(
									this._options,
								),
						});
					}
...
export class UnicodeHighlighterHoverParticipant
...
class Decorations {
	...
	private getDecoration(
		hideInComments: boolean,
		hideInStrings: boolean,
	): ModelDecorationOptions {
...
export class DisableHighlightingInCommentsAction
	extends EditorAction
	implements IDisableUnicodeHighlightAction
{
	public static ID =
...
export class DisableHighlightingInStringsAction
	extends EditorAction
	implements IDisableUnicodeHighlightAction
{
	public static ID =
...
export class DisableHighlightingOfInvisibleCharactersAction
	extends EditorAction
	implements IDisableUnicodeHighlightAction
{
	public static ID =
...
export class DisableHighlightingOfNonBasicAsciiCharactersAction
	extends EditorAction
	implements IDisableUnicodeHighlightAction
{
	public static ID =
...
export class ShowExcludeOptions extends EditorAction {
	...
	public async run(
		...
		const { codePoint, reason, inString, inComment } =
			args as ShowExcludeOptionsArgs;
		...
		options.push({
			...
			run: () =>
				excludeCharFromBeingHighlighted(configurationService, [
					codePoint,
				]),
		});
...
async function excludeCharFromBeingHighlighted(
	configurationService: IConfigurationService,
	charCodes: number[],
) {
...
async function excludeLocaleFromBeingHighlighted(
	configurationService: IConfigurationService,
	locales: string[],
) {
