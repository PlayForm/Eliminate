	...
	implements IEditorContribution
{
	...
	private _onEditorMouseDown(mouseEvent: IEditorMouseEvent): void {
		...
		const shouldNotHideCurrentHoverWidget =
			this._shouldNotHideCurrentHoverWidget(mouseEvent);
	...
	private _shouldNotRecomputeCurrentHoverWidget(
		...
		const isMouseOnStickyContentHoverWidget = (
			mouseEvent: IEditorMouseEvent,
			isHoverSticky: boolean,
		): boolean => {
			const isMouseOnContentHoverWidget =
				this._isMouseOnContentHoverWidget(mouseEvent);
			return isHoverSticky && isMouseOnContentHoverWidget;
		};
		const isMouseOnColorPicker = (
			mouseEvent: IEditorMouseEvent,
		): boolean => {
			const isMouseOnContentHoverWidget =
				this._isMouseOnContentHoverWidget(mouseEvent);
			const isColorPickerVisible =
				this._contentWidget?.isColorPickerVisible ?? false;
			return isMouseOnContentHoverWidget && isColorPickerVisible;
		};
		// TODO@aiday-mar verify if the following is necessary code
		const isTextSelectedWithinContentHoverWidget = (
			mouseEvent: IEditorMouseEvent,
			sticky: boolean,
		): boolean => {
			return (
				(sticky &&
					this._contentWidget?.containsNode(
						mouseEvent.event.browserEvent.view?.document
							.activeElement,
					) &&
					!mouseEvent.event.browserEvent.view?.getSelection()
						?.isCollapsed) ??
				false
			);
		};
	...
	private _onEditorMouseMove(mouseEvent: IEditorMouseEvent): void {
		...
		const shouldRescheduleHoverComputation =
			isContentHoverWidgetVisible && sticky && hidingDelay > 0;
	...
	private _reactToEditorMouseMove(
		...
		const activatedByDecoratorClick =
		...
		const contentHoverShowsOrWillShow =
			this._tryShowHoverWidget(mouseEvent);
	...
	private _onKeyDown(e: IKeyboardEvent): void {
		...
		const shouldKeepHoverVisible =
			...
			(resolvedKeyboardEvent.kind === ResultKind.KbFound &&
				(resolvedKeyboardEvent.commandId ===
					SHOW_OR_FOCUS_HOVER_ACTION_ID ||
					resolvedKeyboardEvent.commandId ===
						INCREASE_HOVER_VERBOSITY_ACTION_ID ||
					resolvedKeyboardEvent.commandId ===
	...
	public updateHoverVerbosityLevel(
		action: HoverVerbosityAction,
		index: number,
		focus?: boolean,
	): void {
