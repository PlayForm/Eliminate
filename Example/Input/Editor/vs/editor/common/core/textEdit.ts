...
export class TextEdit {
	public static fromOffsetEdit(
		edit: OffsetEdit,
		initialState: AbstractText,
	): TextEdit {
		const edits = edit.edits.map(
			(e) =>
				new SingleTextEdit(
					initialState.getTransformer().getRange(e.replaceRange),
					e.newText,
				),
		);
