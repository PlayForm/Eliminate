...
export class LineEdit {
	...
	public static fromTextEdit(
		...
		for (let i = 0; i < edits.length; i++) {
			const edit = edits[i];
			const nextEditRange =
				i + 1 < edits.length ? edits[i + 1] : undefined;
			currentEdits.push(edit);
			if (
				nextEditRange &&
				nextEditRange.range.startLineNumber === edit.range.endLineNumber
			) {
				continue;
			}
	...
	public static createFromUnsorted(
	...
	constructor(
		...
		assert(
			checkAdjacentItems(
				edits,
				(i1, i2) =>
					i1.lineRange.endLineNumberExclusive <=
					i2.lineRange.startLineNumber,
			),
		);
	}
	...
	public humanReadablePatch(originalLines: string[]): string {
		...
		function pushLine(
			...
			kind: "unmodified" | "deleted" | "added",
			content: string | undefined,
		) {
			const specialChar =
				kind === "unmodified" ? " " : kind === "deleted" ? "-" : "+";
			...
			const origLn =
				originalLineNumber === -1
					? "   "
					: originalLineNumber.toString().padStart(3, " ");
			const modLn =
				modifiedLineNumber === -1
					? "   "
					: modifiedLineNumber.toString().padStart(3, " ");
...
export class SingleLineEdit {
	...
	public static fromSingleTextEdit(
		edit: SingleTextEdit,
		initialValue: AbstractText,
	): SingleLineEdit {
		// 1: ab[cde
		...
		const editEndLineNumberMaxColumn =
			initialValue
				.getTransformer()
				.getLineLength(edit.range.endLineNumber) + 1;
		...
		newLines[newLines.length - 1] =
			newLines[newLines.length - 1] + survivingEndLineText;

		// Replacing [startLineNumber, endLineNumberEx) with newLines would be correct, however it might not be minimal.

		const startBeforeNewLine =
			edit.range.startColumn ===
			initialValue
				.getTransformer()
				.getLineLength(edit.range.startLineNumber) +
				1;
		const endAfterNewLine = edit.range.endColumn === 1;

		if (
			startBeforeNewLine &&
			newLines[0].length === survivingFirstLineText.length
		) {
	...
	public toSingleTextEdit(initialValue: AbstractText): SingleTextEdit {
		if (this.newLines.length === 0) {
			...
			if (
				...
				if (this.lineRange.startLineNumber > 1) {
					...
					const startColumn =
						initialValue
							.getTransformer()
							.getLineLength(startLineNumber) + 1;
			...
			if (
				insertionLine ===
				initialValue.getTransformer().textLength.lineCount + 2
			) {
				endLineNumber = insertionLine - 1;
				column =
					initialValue.getTransformer().getLineLength(endLineNumber) +
					1;
			...
			const endLineNumberMaxColumn =
				initialValue.getTransformer().getLineLength(endLineNumber) + 1;
	...
	public removeCommonSuffixPrefixLines(
		initialValue: AbstractText,
	): SingleLineEdit {
		...
		while (
			...
			this.newLines[trimStartCount] ===
				initialValue.getLineAt(startLineNumber)
		) {
			startLineNumber++;
			trimStartCount++;
		}
