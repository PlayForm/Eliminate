...
export class ViewModel extends Disposable implements IViewModel {
		...
		this._register(
			this.model.onDidChangeContentOrInjectedText((e) => {
				try {
					...
					const lineBreaksComputer =
						this._lines.createLineBreaksComputer();
						...
						switch (change.changeType) {
							...
							case textModelEvents.RawContentChangedType
								...
								const linesDeletedEvent =
									this._lines.onModelLinesDeleted(
										versionId,
							...
							case textModelEvents.RawContentChangedType
								...
								const insertedLineBreaks =
									lineBreakQueue.takeCount(
										change.detail.length,
									);
								const linesInsertedEvent =
									this._lines.onModelLinesInserted(
										versionId,
										change.fromLineNumber,
										change.toLineNumber,
										insertedLineBreaks,
	...
	public setHiddenAreas(
		...
		try {
			...
			const firstModelLineIsHidden =
				firstModelLineInViewPort &&
				mergedRanges.some(
					(range) =>
	...
	public saveState(): IViewState {
		...
		const firstViewLineNumber =
			this.viewLayout.getLineNumberAtVerticalOffset(scrollTop);
		const firstPosition =
			this.coordinatesConverter.convertViewPositionToModelPosition(
				new Position(
					firstViewLineNumber,
	...
	public getAllOverviewRulerDecorations(
		...
		for (const decoration of decorations) {
			const decorationOptions = <ModelDecorationOptions>(
				decoration.options
			);
...
class ViewportStart implements IDisposable {
	...
	public update(viewModel: IViewModel, startLineNumber: number): void {
		...
		const viewportStartLineTop =
			viewModel.viewLayout.getVerticalOffsetForLineNumber(
				startLineNumber,
			);
