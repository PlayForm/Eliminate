	...
	implements IExtensionManagementService
{
			...
			if (!URI.isUri(extension)) {
				...
				if (existingInstallingExtension) {
					if (
						...
						const identifier =
							existingInstallingExtension.task.identifier;
	...
	private async getOtherProfilesToUpdateExtension(
		tasks: IInstallExtensionTask[],
	): Promise<[URI, IInstallExtensionTask][]> {
		...
		for (const task of tasks) {
			...
			for (const profile of this.userDataProfilesService.profiles) {
				...
				const installedExtension = installedExtensions.find((e) =>
					areSameExtensions(e.identifier, task.identifier),
				);
