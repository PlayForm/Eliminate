...
export const { registerWindow, getWindow, getDocument, getWindows, getWindowsCount, getWindowId, getWindowById, hasWindow, onDidRegisterWindow, onWillUnregisterWindow, onDidUnregisterWindow, } = (function () {
    ...
    return {
        ...
        getWindow(e: Node | UIEvent | undefined | null): CodeWindow {
            if ((e as Node | undefined | null)
                ?.ownerDocument?.defaultView) {
                return (e as Node | undefined | null).ownerDocument.defaultView
        ...
        getDocument(e: Node | UIEvent | undefined | null): Document {
            return getWindow(e as Node | undefined | null).document;
        },
    };
})();
//#endregion
...
export const addStandardDisposableListener: IAddStandardDisposableListenerSignature = function addStandardDisposableListener(node: HTMLElement, type: string, handler: (event: any) => void, useCapture?: boolean): IDisposable {
    if (type === "click" ||
        ...
        type === "keypress" ||
        type === "keyup") {
        handler = _wrapAsStandardKeyboardEvent(handler);
    }
...
export function addDisposableGenericMouseDownListener(node: EventTarget, handler: (event: any) => void, useCapture?: boolean): IDisposable {
    return addDisposableListener(node, platform.isIOS && BrowserFeatures.pointerEvents
        ? EventType.POINTER_DOWN
        : EventType.MOUSE_DOWN, handler, useCapture);
}
export function addDisposableGenericMouseMoveListener(node: EventTarget, handler: (event: any) => void, useCapture?: boolean): IDisposable {
    return addDisposableListener(node, platform.isIOS && BrowserFeatures.pointerEvents
        ...
        : EventType.MOUSE_MOVE, handler, useCapture);
}
...
(function () {
    ...
    scheduleAtNextAnimationFrame = (targetWindow: Window, runner: () => void, priority: number = 0) => {
        ...
        if (!animFrameRequested.get(targetWindowId)) {
            ...
            targetWindow.requestAnimationFrame(() => ((targetWindowId: number) => {
...
class TimeoutThrottledDomListener<R, E extends Event> extends Disposable {
    constructor(node: any, type: string, handler: (event: R) => void, eventMerger: IEventMerger<R, E> = <any>function (lastEvent: Event | null, currentEvent: Event) {
        return currentEvent;
    }, minimumTimeMs: number = 8) {
        super();
...
function getRelativeLeft(element: HTMLElement, parent: HTMLElement): number {
    ...
    return getTopLeftOffset(element).left - getTopLeftOffset(parent).left;
}
export function getLargestChildWidth(parent: HTMLElement, children: HTMLElement[]): number {
    return Math.max(...children.map((child) => {
        return (Math.max(getTotalScrollWidth(child), getTotalWidth(child)) +
...
export function getActiveDocument(): Document {
    ...
    return Array.from(getWindows()).map(({ window }) => window.document).find((doc) => doc.hasFocus()) ?? mainWindow.document;
}
/**
...
export function createStyleSheet(container: HTMLElement = mainWindow.document.head, beforeAppend?: (style: HTMLStyleElement) => void, disposableStore?: DisposableStore): HTMLStyleElement {
    ...
    if (container === mainWindow.document.head) {
        ...
        for (const { window: targetWindow, disposables } of getWindows()) {
            ...
            disposableStore?.add(disposables.add(cloneGlobalStyleSheet(style, globalStylesheetClones, targetWindow)));
        }
    }
    return style;
}
...
function cloneGlobalStyleSheet(globalStylesheet: HTMLStyleElement, globalStylesheetClones: Set<HTMLStyleElement>, targetWindow: Window): IDisposable {
    ...
    })().observe(globalStylesheet, disposables, {
        childList: true,
    })(() => {
...
class FocusTracker extends Disposable implements IFocusTracker {
    ...
    constructor(element: HTMLElement | Window) {
        ...
        this._refreshStateHandler = () => {
            if (FocusTracker.hasFocusWithin(<HTMLElement>element)
                !== hasFocus) {
                if (hasFocus) {
                    onBlur();
                }
...
function _$<T extends Element>(namespace: Namespace, description: string, attrs?: {
    [key: string]: any;
...
export function $<T extends HTMLElement>(description: string, attrs?: {
    [key: string]: any;
}, ...children: Array<Node | string>): T {
    return _$(Namespace.HTML, description, attrs, ...children);
}
$.SVG = function <T extends SVGElement>(description: string, attrs?: {
    [key: string]: any;
}, ...children: Array<Node | string>): T {
...
export function removeTabIndexAndUpdateFocus(node: HTMLElement): void {
    ...
    if (node.ownerDocument.activeElement === node) {
        findParentWithAttribute(node.parentElement, "tabIndex")
            ?.focus();
    }
...
export function triggerDownload(dataOrUri: Uint8Array | URI, name: string): void {
    ...
    if (URI.isUri(dataOrUri)) {
        ...
        url = URL.createObjectURL(new Blob([dataOrUri]));
        // Ensure to free the data from DOM eventually
        setTimeout(() => URL.revokeObjectURL(url));
    }
    const anchor = document.createElement("a");
    getActiveWindow().document.body.appendChild(anchor);
...
export function safeInnerHtml(node: HTMLElement, value: string, extraDomPurifyConfig?: dompurify.Config): void {
    try {
        node.innerHTML = dompurify.sanitize(value, {
            ...Object.freeze<dompurify.Config & {
        ...
        hookDomPurifyHrefAndSrcSanitizer([Schemas.http, Schemas.https, Schemas.command]).dispose();
    }
}
/**
 ...
 * ```
 */
export function h<TTag extends string>(tag: TTag): TagToRecord<TTag> extends infer Y ? {
    [TKey in keyof Y]: Y[TKey];
} : never;
export function h<TTag extends string, T extends Child[]>(tag: TTag, children: [
    ...T
]): ArrayToObj<T> & TagToRecord<TTag> extends infer Y ? {
    [TKey in keyof Y]: Y[TKey];
} : never;
export function h<TTag extends string>(tag: TTag, attributes: Partial<ElementAttributes<TagToElement<TTag>>>): TagToRecord<TTag> extends infer Y ? {
    [TKey in keyof Y]: Y[TKey];
} : never;
export function h<TTag extends string, T extends Child[]>(tag: TTag, attributes: Partial<ElementAttributes<TagToElement<TTag>>>, children: [
    ...T
]): ArrayToObj<T> & TagToRecord<TTag> extends infer Y ? {
    [TKey in keyof Y]: Y[TKey];
} : never;
export function h(tag: string, ...args: [
] | [
    attributes: ({
        $: string;
    } & Partial<ElementAttributes<HTMLElement>>) | Record<string, any>,
    children?: any[]
] | [
    children: any[]
    ...
    let attributes: {
        $?: string;
    } & Partial<ElementAttributes<HTMLElement>>;
    ...
    for (const [key, value] of Object.entries(attributes)) {
        if (key === "className") {
            ...
            for (const [cssKey, cssValue] of Object.entries(value)) {
                el.style.setProperty(camelCaseToHyphenCase(cssKey), typeof cssValue === "number"
                    ? cssValue + "px"
                    : "" + cssValue);
            }
        }
...
export function svgElem<TTag extends string>(tag: TTag): TagToRecord<TTag> extends infer Y ? {
    [TKey in keyof Y]: Y[TKey];
} : never;
export function svgElem<TTag extends string, T extends Child[]>(tag: TTag, children: [
    ...T
]): ArrayToObj<T> & TagToRecord<TTag> extends infer Y ? {
    [TKey in keyof Y]: Y[TKey];
} : never;
export function svgElem<TTag extends string>(tag: TTag, attributes: Partial<ElementAttributes<TagToElement<TTag>>>): TagToRecord<TTag> extends infer Y ? {
    [TKey in keyof Y]: Y[TKey];
} : never;
export function svgElem<TTag extends string, T extends Child[]>(tag: TTag, attributes: Partial<ElementAttributes<TagToElement<TTag>>>, children: [
    ...T
]): ArrayToObj<T> & TagToRecord<TTag> extends infer Y ? {
    [TKey in keyof Y]: Y[TKey];
} : never;
export function svgElem(tag: string, ...args: [
] | [
    attributes: ({
        $: string;
    } & Partial<ElementAttributes<HTMLElement>>) | Record<string, any>,
...
export class SafeTriangle {
    ...
    constructor(private readonly originX: number, private readonly originY: number, target: HTMLElement) {
