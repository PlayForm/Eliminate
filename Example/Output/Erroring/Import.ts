...
class GitHubGistProfileContentHandler implements vscode.ProfileContentHandler {
        ...
        if (!this._octokit) {
            this._octokit = (async () => {
                ...
                return new Octokit({
                    ...
                    auth: `token ${(await vscode.authentication.getSession("github", ["gist", "user:email"], { createIfNone: true })).accessToken}`,
                });
            })();
        }
        return this._octokit;
    }
    async saveProfile(name: string, content: string): Promise<{
        readonly id: string;
        readonly vscode.Uri.parse(result.data.html_url): vscode.Uri;
    } | null> {
        ...
        if (result.data.id && result.data.html_url) {
            return { id: result.data.id, link: vscode.Uri.parse(result.data.html_url) };
        }
        return null;
    }
    ...
    async readProfile(arg: string | vscode.Uri): Promise<string | null> {
        ...
        try {
            const gist = await octokit.gists.get({ gist_id: (typeof arg === "string" ? arg : basename(arg.path)) });
            if (gist.data.files) {
