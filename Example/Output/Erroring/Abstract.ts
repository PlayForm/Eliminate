...
import { $, addDisposableListener, append, clearNode, createStyleSheet, getWindow, h, hasParentWithClass, isActiveElement, isEditableElement, isKeyboardEvent, } from "../../dom.js";
...
class TreeNodeListDragAndDrop<T, TFilterData, TRef> implements IListDragAndDrop<ITreeNode<T, TFilterData>> {
    ...
    onDragOver(data: IDragAndDropData, targetNode: ITreeNode<T, TFilterData> | undefined, targetIndex: number | undefined, targetSector: ListViewTargetSector | undefined, originalEvent: DragEvent, raw = true): boolean | IListDragOverReaction {
        ...
        if (typeof result === "boolean" ||
            ...
            result.feedback) {
            if (!raw) {
                return { accept: (typeof result === "boolean" ? result : result.accept), effect: (typeof result === "boolean" ? undefined : result.effect), feedback: [targetIndex!] };
            }
            return result;
        }
...
export class ComposedTreeDelegate<T, N extends {
    element: T;
...
export class TreeRenderer<T, TFilterData, TRef, TTemplateData> implements IListRenderer<ITreeNode<T, TFilterData>, ITreeListTemplateData<TTemplateData>> {
    ...
    private readonly disposables = new DisposableStore();
    constructor(private renderer: ITreeRenderer<T, TFilterData, TTemplateData>, private model: ITreeModel<T, TFilterData, TRef>, onDidChangeCollapseState: Event<ICollapseStateChangeEvent<T, TFilterData>>, private activeNodes: Collection<ITreeNode<T, TFilterData>>, private renderedIndentGuides: SetMap<ITreeNode<T, TFilterData>, HTMLDivElement>, options: ITreeRendererOptions = {}) {
...
class FindFilter<T> implements IFindFilter<T>, IDisposable {
    ...
    constructor(private tree: AbstractTree<T, any, any>, private keyboardNavigationLabelProvider: IKeyboardNavigationLabelProvider<T>, private _filter?: ITreeFilter<T, FuzzyScore>) {
    ...
    filter(element: T, parentVisibility: TreeVisibility): TreeFilterResult<FuzzyScore | LabelFuzzyScore> {
        ...
        for (const l of Array.isArray(label) ? label : [label]) {
            ...
            if (score) {
                this._matchCount++;
                return (Array.isArray(label) ? label : [label]).length === 1
...
class FindWidget<T, TFilterData> extends Disposable {
    ...
    constructor(container: HTMLElement, private tree: AbstractTree<T, TFilterData, any>, contextViewProvider: IContextViewProvider, placeholder: string, toggleContributions: ITreeFindToggleContribution[] = [], options?: IFindWidgetOptions) {
        super();
        ...
        this.onDidToggleChange = Event.any(...this.toggles.map((toggle) => Event.map(toggle.onChange, () => ({
            id: toggle.id,
...
export abstract class AbstractFindController<T, TFilterData> implements IDisposable {
    ...
    constructor(protected tree: AbstractTree<T, TFilterData, any>, protected filter: IFindFilter<T>, protected readonly contextViewProvider: IContextViewProvider, protected readonly options: IAbstractFindControllerOptions = {}) {
        this.toggles = new FindToggles(options.toggles ?? []);
        this._placeholder =
            options.placeholder ?? localize("type to search", "Type to search");
    }
    isOpened(): boolean {
        return !!this.widget;
    }
        ...
        if (showNotFound) {
            if (this.tree.options.showNotFoundMessage ?? true) {
                this.widget?.showMessage({
                    ...
                    content: localize("not found", "No results found."),
                });
            }
            else {
...
class FindController<T, TFilterData> extends AbstractFindController<T, TFilterData> {
    get mode(): TreeFindMode {
        return this.toggles.get(DefaultTreeToggles.Mode)
            ? TreeFindMode.Filter
    ...
    constructor(tree: AbstractTree<T, TFilterData, any>, protected override filter: FindFilter<T>, contextViewProvider: IContextViewProvider, options: IFindControllerOptions = {}) {
    ...
    protected override onDidToggleChange(e: ITreeFindToggleChangeEvent): void {
        if (e.id === DefaultTreeToggles.Mode) {
            this.mode = e.isChecked
                ? TreeFindMode.Filter
                : TreeFindMode.Highlight;
        }
        else if (e.id === DefaultTreeToggles.MatchType) {
            this.matchType = e.isChecked
                ? TreeFindMatchType.Fuzzy
                : TreeFindMatchType.Contiguous;
        }
    }
...
class StickyScrollState<T, TFilterData, TRef> {
    ...
    lastNodePartiallyVisible(): boolean {
        ...
        return (secondLastStickyNode.position + secondLastStickyNode.height !==
    ...
    animationStateChanged(previousState: StickyScrollState<T, TFilterData, TRef>): boolean {
        ...
        const lastStickyNode = this.stickyNodes[this.count - 1];
        return lastStickyNode.position !== previousState.stickyNodes[previousState.count - 1].position;
    }
}
...
class StickyScrollController<T, TFilterData, TRef> extends Disposable {
    ...
    constructor(private readonly tree: AbstractTree<T, TFilterData, TRef>, private model: ITreeModel<T, TFilterData, TRef>, private readonly view: List<ITreeNode<T, TFilterData>>, renderers: TreeRenderer<T, TFilterData, TRef, any>[], private readonly treeDelegate: IListVirtualDelegate<ITreeNode<T, TFilterData>>, options: IAbstractTreeOptions<T, TFilterData> = {}) {
        super();
    ...
    private nodeTopAlignsWithStickyNodesBottom(node: ITreeNode<T, TFilterData>, stickyNodesHeight: number): boolean {
        ...
        return this.view.scrollTop === elementTop -
            stickyNodesHeight;
    }
    ...
    nodePositionTopBelowWidget(node: ITreeNode<T, TFilterData>): number {
        ...
        for (let i = 0; i < ancestors.length && i < this.stickyScrollMaxItemCount; i++) {
            0
                += this.treeDelegate.getHeight(ancestors[i]);
        }
        return 0;
    }
    ...
    updateOptions(optionsUpdate: IAbstractTreeOptionsUpdate = {}): void {
        ...
        if (this.stickyScrollMaxItemCount !==
...
class StickyScrollWidget<T, TFilterData, TRef> implements IDisposable {
    ...
    get state(): StickyScrollState<T, TFilterData, TRef> | undefined {
        return this._previousState;
    }
    private stickyScrollFocus: StickyScrollFocus<T, TFilterData, TRef>;
    readonly onDidChangeHasFocus: Event<boolean>;
    readonly onContextMenu: Event<ITreeContextMenuEvent<T>>;
    constructor(container: HTMLElement, private readonly view: List<ITreeNode<T, TFilterData>>, private readonly tree: AbstractTree<T, TFilterData, TRef>, private readonly treeRenderers: TreeRenderer<T, TFilterData, TRef, any>[], private readonly treeDelegate: IListVirtualDelegate<ITreeNode<T, TFilterData>>, private readonly accessibilityProvider: IListAccessibilityProvider<T> | undefined) {
    ...
    private createElement(stickyNode: StickyScrollNode<T, TFilterData>, stickyIndex: number, stickyNodesTotal: number): {
        element: HTMLElement;
    ...
    private setAccessibilityAttributes(container: HTMLElement, element: T, stickyIndex: number, stickyNodesTotal: number): IDisposable {
        ...
        const result = autorun((reader) => {
            const value = reader.readObservable(ariaLabel && typeof ariaLabel !== "string"
                ? ariaLabel
        ...
        const ariaLevel = this.accessibilityProvider.getAriaLevel &&
...
class StickyScrollFocus<T, TFilterData, TRef> extends Disposable {
    ...
    constructor(private readonly container: HTMLElement, private readonly view: List<ITreeNode<T, TFilterData>>) {
        super();
    ...
    private handleContextMenu(e: IListContextMenuEvent<ITreeNode<T, TFilterData>>): void {
        ...
        if (!isStickyScrollContainer(target) &&
        ...
        if (this.domHasFocus && this.state) {
            ...
            // Move down, if last sticky node is focused, move focus into first child of last sticky node
                ...
                if (this.focusedIndex >= this.state.count - 1) {
                    const nodeIndexToFocus = this.state.stickyNodes[this.state.count - 1]
                        .startIndex + 1;
                    this.view.domFocus();
    ...
    updateElements(elements: HTMLElement[], state: StickyScrollState<T, TFilterData, TRef> | undefined): void {
        ...
        this.state = state;
        if (state) {
            const newFocusedIndex = clamp(this.focusedIndex, 0, state.count - 1);
    ...
    private scrollNodeUnderWidget(nodeIndex: number, state: StickyScrollState<T, TFilterData, TRef>) {
        ...
        this.view.scrollTop = elementScrollTop -
            (secondLastStickyNode
...
class Trait<T> {
    ...
    constructor(private getFirstViewElementWithTrait: () => ITreeNode<T, any> | undefined, private identityProvider?: IIdentityProvider<T>) { }
    ...
    onDidModelSplice({ insertedNodes, deletedNodes, }: ITreeModelSpliceEvent<T, any>): void {
        if (!this.identityProvider) {
            ...
            deletedNodes.forEach((node) => dfs(node, (node: ITreeNode<T, any>) => set.delete(node)));
            this.set([...set.values()]);
            return;
        }
        const deletedNodesIdSet = new Set<string>();
        deletedNodes.forEach((node) => dfs(node, (node: ITreeNode<T, any>) => deletedNodesIdSet.add(this.identityProvider!.getId(node.element).toString())));
        const insertedNodesMap = new Map<string, ITreeNode<T, any>>();
        insertedNodes.forEach((node) => dfs(node, (node: ITreeNode<T, any>) => insertedNodesMap.set(this.identityProvider!.getId(node.element).toString(), node)));
...
class TreeNodeListMouseController<T, TFilterData, TRef> extends MouseController<ITreeNode<T, TFilterData>> {
    constructor(list: TreeNodeList<T, TFilterData, TRef>, private tree: AbstractTree<T, TFilterData, TRef>, private stickyScrollProvider: () => StickyScrollController<T, TFilterData, TRef> | undefined) {
        super(list);
    }
    ...
    private handleStickyScrollMouseEvent(e: IListMouseEvent<ITreeNode<T, TFilterData>>, node: ITreeNode<T, TFilterData>): void {
        if (isMonacoCustomToggle(e.browserEvent.target as HTMLElement) ||
        ...
        this.tree.scrollTop = elementScrollTop -
            (secondLastStickyNode
                ? secondLastStickyNode.position +
                    secondLastStickyNode.height +
                    lastStickyNode.height
...
class TreeNodeList<T, TFilterData, TRef> extends List<ITreeNode<T, TFilterData>> {
    constructor(user: string, container: HTMLElement, virtualDelegate: IListVirtualDelegate<ITreeNode<T, TFilterData>>, renderers: IListRenderer<any /* TODO@joao */, any>[], private focusTrait: Trait<T>, private selectionTrait: Trait<T>, private anchorTrait: Trait<T>, options: ITreeNodeListOptions<T, TFilterData, TRef>) {
...
export abstract class AbstractTree<T, TFilterData, TRef> implements IDisposable {
    ...
    constructor(private readonly _user: string, container: HTMLElement, delegate: IListVirtualDelegate<T>, renderers: ITreeRenderer<T, TFilterData, any>[], private _options: IAbstractTreeOptions<T, TFilterData> = {}) {
        if (_options.keyboardNavigationLabelProvider) {
            ...
            _options = {
                ..._options,
                filter: this.findFilter as ITreeFilter<T, TFilterData>,
        ...
        if (_options.keyboardSupport !== false) {
            const onKeyDown = Event.chain(this.view.onKeyDown, ($) => $.filter((e) => !isEditableElement(e.target as HTMLElement)).map((e) => new StandardKeyboardEvent(e)));
        ...
        if ((_options.findWidgetEnabled ?? true) &&
            ...
            this.findController = this.disposables.add(new FindController(this, this.findFilter!, _options.contextViewProvider, {
    ...
    private updateStickyScroll(optionsUpdate: IAbstractTreeOptionsUpdate) {
        ...
        else if (this.stickyScrollController &&
    ...
    focusNext(n = 1, loop = false, browserEvent?: UIEvent, filter: ((node: ITreeNode<T, TFilterData>) => boolean) | undefined = isKeyboardEvent(browserEvent) && browserEvent.altKey
        ? undefined
        : this.focusNavigationFilter): void {
        this.view.focusNext(n, loop, browserEvent, filter);
    }
    focusPrevious(n = 1, loop = false, browserEvent?: UIEvent, filter: ((node: ITreeNode<T, TFilterData>) => boolean) | undefined = isKeyboardEvent(browserEvent) && browserEvent.altKey
        ? undefined
        : this.focusNavigationFilter): void {
        this.view.focusPrevious(n, loop, browserEvent, filter);
    }
    focusNextPage(browserEvent?: UIEvent, filter: ((node: ITreeNode<T, TFilterData>) => boolean) | undefined = isKeyboardEvent(browserEvent) && browserEvent.altKey
        ? undefined
        : this.focusNavigationFilter): Promise<void> {
        return this.view.focusNextPage(browserEvent, filter);
    }
    focusPreviousPage(browserEvent?: UIEvent, filter: ((node: ITreeNode<T, TFilterData>) => boolean) | undefined = isKeyboardEvent(browserEvent) && browserEvent.altKey
        ? undefined
        : this.focusNavigationFilter): Promise<void> {
        return this.view.focusPreviousPage(browserEvent, filter, () => this.stickyScrollController?.height ?? 0);
    }
    focusLast(browserEvent?: UIEvent, filter: ((node: ITreeNode<T, TFilterData>) => boolean) | undefined = isKeyboardEvent(browserEvent) && browserEvent.altKey
        ? undefined
        : this.focusNavigationFilter): void {
        this.view.focusLast(browserEvent, filter);
    }
    focusFirst(browserEvent?: UIEvent, filter: ((node: ITreeNode<T, TFilterData>) => boolean) | undefined = isKeyboardEvent(browserEvent) && browserEvent.altKey
        ? undefined
        : this.focusNavigationFilter): void {
    ...
    createNewModel(options: IAbstractTreeOptions<T, TFilterData> = {}): ITreeModel<T, TFilterData, TRef> {
        return this.createModel(this._user, {
            ...this._options,
            filter: this.findFilter as ITreeFilter<T, TFilterData> | undefined,
            ...options,
        });
    }
