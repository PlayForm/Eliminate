...
class TableListRenderer<TRow> implements IListRenderer<TRow, RowTemplateData> {
    ...
    constructor(private columns: ITableColumn<TRow, TCell>[], renderers: ITableRenderer<TCell, unknown>[], private getColumnSize: (index: number) => number) {
...
class ColumnHeader<TRow, TCell> extends Disposable implements IView {
    ...
    private _onDidLayout = new Emitter<[
        number,
        number
    ]>();
...
export class Table<TRow> implements ISpliceable<TRow>, IDisposable {
    ...
    constructor(user: string, container: HTMLElement, private virtualDelegate: ITableVirtualDelegate<TRow>, private columns: ITableColumn<TRow, TCell>[], renderers: ITableRenderer<TCell, unknown>[], _options?: ITableOptions<TRow>) {
