...
class AsyncDataTreeRenderer<TInput, T, TFilterData, TTemplateData> implements ITreeRenderer<IAsyncDataTreeNode<TInput, T>, TFilterData, IDataTreeListTemplateData<TTemplateData>> {
    ...
    constructor(protected renderer: ITreeRenderer<T, TFilterData, TTemplateData>, protected nodeMapper: AsyncDataTreeNodeMapper<TInput, T, TFilterData>, readonly onDidChangeTwistieState: Event<IAsyncDataTreeNode<TInput, T>>) {
...
class AsyncFindTreeNode<T> {
    ...
    constructor(public element: T | undefined, public id: string, private readonly sorter: ITreeSorter<T> | undefined) { }
...
class AsyncFindTree<TInput, T> {
    ...
    constructor(private readonly dataSource: IAsyncDataSource<TInput, T>, private readonly identityProvider: IIdentityProvider<T>, private readonly sorter?: ITreeSorter<T>) {
...
class AsyncFindFilter<TInput, T> implements IFindFilter<IAsyncDataTreeNode<TInput, T>>, IDisposable {
    ...
    constructor(private _filter?: ITreeFilter<IAsyncDataTreeNode<TInput, T>, FuzzyScore>) { }
...
class AsyncFindController<TInput, T, TFilterData> extends AbstractFindController<IAsyncDataTreeNode<TInput, T> | null, TFilterData> {
    ...
    constructor(private readonly user: string, protected override readonly tree: ObjectTree<IAsyncDataTreeNode<TInput, T>, TFilterData>, private readonly findProvider: IAsyncFindProvider<T>, filter: AsyncFindFilter<TInput, T>, private readonly sorter: ITreeSorter<T> | undefined, private readonly dataSource: IAsyncDataSource<TInput, T>, private readonly identityProvider: IIdentityProvider<T>, contextViewProvider: IContextViewProvider, options: IAbstractTreeOptions<IAsyncDataTreeNode<TInput, T>, TFilterData>, private readonly asTreeElement: (node: IAsyncDataTreeNode<TInput, T>) => IObjectTreeElement<IAsyncDataTreeNode<TInput, T>>) {
...
function asObjectTreeOptions<TInput, T, TFilterData>(options?: IAsyncDataTreeOptions<T, TFilterData>): IObjectTreeOptions<IAsyncDataTreeNode<TInput, T>, TFilterData> | undefined {
    return options && {
        ...
        sorter: undefined,
        expandOnlyOnTwistieClick: typeof options.expandOnlyOnTwistieClick === 'undefined' ? undefined : (typeof options.expandOnlyOnTwistieClick !== 'function' ? options.expandOnlyOnTwistieClick : (e => (options.expandOnlyOnTwistieClick as ((e: T) => boolean))(e.element as T))),
...
export class AsyncDataTree<TInput, T, TFilterData = void> implements IDisposable {
    ...
    private readonly getDefaultCollapseState: {
        (e: T): undefined | ObjectTreeElementCollapseState.PreserveOrCollapsed | ObjectTreeElementCollapseState.PreserveOrExpanded;
    };
    ...
    constructor(protected user: string, container: HTMLElement, delegate: IListVirtualDelegate<T>, renderers: ITreeRenderer<T, TFilterData, any>[], private dataSource: IAsyncDataSource<TInput, T>, options: IAsyncDataTreeOptions<T, TFilterData> = {}) {
    ...
    private setChildren(node: IAsyncDataTreeNode<TInput, T>, childrenElementsIterable: Iterable<T>, recursive: boolean, viewStateContext?: IAsyncDataTreeViewStateContext<TInput, T>): IAsyncDataTreeNode<TInput, T>[] {
        ...
        const childrenTreeNodesById = new Map<string, {
...
export class CompressibleAsyncDataTree<TInput, T, TFilterData = void> extends AsyncDataTree<TInput, T, TFilterData> {
    ...
    constructor(user: string, container: HTMLElement, virtualDelegate: IListVirtualDelegate<T>, private compressionDelegate: ITreeCompressionDelegate<T>, renderers: ICompressibleTreeRenderer<T, TFilterData, any>[], dataSource: IAsyncDataSource<TInput, T>, options: ICompressibleAsyncDataTreeOptions<T, TFilterData> = {}) {
    ...
    protected override createTree(user: string, container: HTMLElement, delegate: IListVirtualDelegate<T>, renderers: ICompressibleTreeRenderer<T, TFilterData, any>[], options: ICompressibleAsyncDataTreeOptions<T, TFilterData>): ObjectTree<IAsyncDataTreeNode<TInput, T>, TFilterData> {
        const objectTreeDelegate = new ComposedTreeDelegate<TInput | T, IAsyncDataTreeNode<TInput, T>>(delegate);
