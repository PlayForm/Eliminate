...
export namespace FuzzyScore {
    ...
    export function isDefault(score?: FuzzyScore): score is [
        -100,
        0
    ] {
...
export abstract class FuzzyScoreOptions {
    ...
    constructor(readonly firstMatchCanBeWeak: boolean, readonly boostFullMatch: boolean) { }
}
...
function _doScore(pattern: string, patternLow: string, patternPos: number, patternStart: number, word: string, wordLow: string, wordPos: number, wordLen: number, wordStart: number, newMatchStart: boolean, outFirstMatchStrong: boolean[]): number {
    if (patternLow[patternPos] !== wordLow[wordPos]) {
