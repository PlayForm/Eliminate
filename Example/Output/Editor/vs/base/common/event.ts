...
export namespace Event {
     ...
     * @param disposable A disposable store to add the new EventEmitter to.
     */
    export function split<T, U>(event: Event<T | U>, isT: (e: T | U) => e is T, disposable?: DisposableStore): [
        Event<T>,
        Event<U>
    ] {
        return [
...
class LeakageMonitor {
    ...
    constructor(private readonly _errorHandler: (err: Error) => void, readonly threshold: number, readonly name: string = (LeakageMonitor._idPool++).toString(16).padStart(3, '0')) { }
    dispose(): void {
        this._stacks?.clear();
    }
