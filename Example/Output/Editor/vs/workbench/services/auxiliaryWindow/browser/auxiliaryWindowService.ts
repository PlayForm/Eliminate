...
import { addDisposableListener, cloneGlobalStylesheets, copyAttributes, createLinkElement, createMetaElement, Dimension, EventHelper, EventType, getActiveWindow, getClientArea, getWindowId, isGlobalStylesheet, isHTMLElement, ModifierKeyEmitter, position, registerWindow, sharedMutationObserver, trackAttributes, } from "../../../../base/browser/dom.js";
...
export class AuxiliaryWindow extends BaseWindow implements IAuxiliaryWindow {
    ...
    readonly whenStylesHaveLoaded: Promise<void>;
    constructor(readonly window: CodeWindow, readonly container: HTMLElement, stylesHaveLoaded: Barrier, 
...
export class BrowserAuxiliaryWindowService extends Disposable implements IAuxiliaryWindowService {
    ...
    private async openWindow(options?: IAuxiliaryWindowOpenOptions): Promise<Window | undefined> {
        ...
        const width = Math.max(options?.bounds?.width ??
            BrowserAuxiliaryWindowService.DEFAULT_SIZE.width, WindowMinimumSize.WIDTH);
        const height = Math.max(options?.bounds?.height ??
            BrowserAuxiliaryWindowService.DEFAULT_SIZE.height, WindowMinimumSize.HEIGHT);
        let newWindowBounds: IRectangle = {
            x: options?.bounds?.x ??
                Math.max(activeWindowBounds.x +
                    activeWindowBounds.width / 2 -
                    width / 2, 0),
            y: options?.bounds?.y ??
                Math.max(activeWindowBounds.y +
    ...
    private applyHTML(auxiliaryWindow: CodeWindow, disposables: DisposableStore): HTMLElement {
        ...
        disposables.add(trackAttributes(this.layoutService.mainContainer, container, [
            "class",
