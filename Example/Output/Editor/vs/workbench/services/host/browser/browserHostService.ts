...
export class BrowserHostService extends Disposable implements IHostService {
    ...
    get onDidChangeFullScreen(): Event<{
        ...
        this._register(Event.runAndSubscribe(onDidRegisterWindow, ({ window, disposables }) => {
            ...
            // Fullscreen (Native)
            disposables.add(addDisposableThrottledListener(viewport, EventType.RESIZE, () => emitter.fire({
                windowId,
                fullscreen: !!detectFullscreen(window),
            }), undefined, isMacintosh
        ...
        for (const openable of toOpen) {
            ...
            if (isFolderToOpen(openable)) {
                if (options?.addMode) {
                    ...
                    this.doOpen({ folderUri: openable.folderUri }, {
                                        ...
                                        options: {
                                            selection: !isUndefined(pathColumnAware.line)
                                                ? {
                                                    ...
                                                    startColumn: pathColumnAware.column ||
                                                        1,
                                                }
                                                : undefined,
                                        },
    ...
    private shouldReuse(options: IOpenWindowOptions = Object.create(null), isFile: boolean): boolean {
        ...
        let openInNewWindow = (options.preferNewWindow || !!options.forceNewWindow) &&
    ...
    private async doOpen(workspace: IWorkspace, options?: {
        reuse?: boolean;
        payload?: object;
    }): Promise<void> {
