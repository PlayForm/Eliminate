...
import { EDITOR_EXPERIMENTAL_PREFER_TREESITTER, ITreeSitterParseResult, ITreeSitterParserService, } from "../../../../editor/common/services/treeSitterParserService.js";
...
class TreeSitterTokenizationSupport extends Disposable implements ITreeSitterTokenizationSupport {
    ...
    constructor(private readonly _queries: TreeSitterQueries, private readonly _languageId: string, private readonly _languageIdCodec: ILanguageIdCodec, 
        ...
        this._register(this._treeSitterService.onDidUpdateTree((e) => {
            ...
            this._onDidChangeTokens.fire({
                ...
                changes: {
                    ...
                    ranges: e.ranges.map((range) => ({
                        fromLineNumber: range.startLineNumber,
                        toLineNumber: range.endLineNumber < maxLine
                            ? range.endLineNumber
                            : maxLine,
                    })),
                },
    ...
    public tokenizeEncoded(lineNumber: number, textModel: ITextModel): Uint32Array | undefined {
        ...
        for (let captureIndex = 0; captureIndex < captures.length; captureIndex++) {
            ...
            const tokenEndIndex = capture.node.endIndex < lineStartOffset + lineLength
                ? capture.node.endIndex
                : lineStartOffset + lineLength;
            const tokenStartIndex = capture.node.startIndex < lineStartOffset
                ? lineStartOffset
            ...
            if (captureIndex > 0) {
                previousTokenEnd =
                    endOffsetsAndScopes[tokenIndex - 1].endOffset;
            }
            else {
            ...
            if (previousTokenEnd >= 0 &&
                previousTokenEnd < intermediateTokenOffset) {
            ...
            if (previousTokenEnd >= lineRelativeOffset) {
                const previousTokenStartOffset = tokenIndex >= 2
                    ? endOffsetsAndScopes[tokenIndex - 2].endOffset
                    : 0;
                ...
                if (previousTokenStartOffset + currentTokenLength ===
        ...
        if (captures[captures.length - 1].node.endPosition.column + 1 <
            lineLength) {
            increaseSizeOfTokensByOneToken();
