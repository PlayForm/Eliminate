...
export class UserDataProfileManagementService extends Disposable implements IUserDataProfileManagementService {
    ...
    private readonly logService: ILogService) {
        ...
        this._register(userDataProfilesService.onDidChangeProfiles((e) => {
            if (e.removed.some((profile) => profile.id ===
    ...
    private getProfileToUseForCurrentWorkspace(): IUserDataProfile {
        ...
        if (workspaceUri) {
            ...
            const currentProfile = this.userDataProfilesService.profiles.find((profile) => profile.id ===
                this.userDataProfileService.currentProfile.id);
            if (currentProfile) {
    ...
    async createAndEnterTransientProfile(): Promise<IUserDataProfile> {
        ...
        await this.changeCurrentProfile(profile);
        this.telemetryService.publicLog2<ProfileManagementActionExecutedEvent, ProfileManagementActionExecutedClassification>("profileManagementActionExecuted", {
        ...
        if (shouldRestartExtensionHosts) {
            if (!isRemoteWindow) {
                if (!(await this.extensionService.stopExtensionHosts(localize("switch profile", "Switching to a profile.")))) {
                    ...
                    if (this.userDataProfilesService.profiles.some((p) => p.id ===
                        this.userDataProfileService.currentProfile.id)) {
