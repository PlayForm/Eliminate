...
import { IProfileImportOptions, IProfileResourceTreeItem, ISaveProfileResult, isProfileURL, IUserDataProfileContentHandler, IUserDataProfileCreateOptions, IUserDataProfileImportExportService, IUserDataProfileManagementService, IUserDataProfileService, PROFILE_EXTENSION, PROFILE_FILTER, PROFILE_URL_AUTHORITY, PROFILE_URL_AUTHORITY_PREFIX, PROFILES_CATEGORY, toUserDataProfileUri, } from "../common/userDataProfile.js";
...
export class UserDataProfileImportExportService extends Disposable implements IUserDataProfileImportExportService {
    ...
    private readonly uriIdentityService: IUriIdentityService) {
        super();
        this.registerProfileContentHandler(Schemas.file, (this.fileUserDataProfileContentHandler =
            instantiationService.createInstance(FileUserDataProfileContentHandler)));
    }
    ...
    async createFromProfile(from: IUserDataProfile, options: IUserDataProfileCreateOptions, token: CancellationToken): Promise<IUserDataProfile | undefined> {
        ...
        return this.progressService
            ...
            const reportProgress = (message: string) => progress.report({
        ...
        try {
            ...
            await this.progressService.withProgress({
                ...
                const profile = await this.doCreateProfile(profileTemplate, true, false, {
                    useDefaultFlags: this.userDataProfileService.currentProfile
                        .useDefaultFlags,
                }, reportProgress);
                if (profile) {
                    ...
                    if (this.productService.webUrl) {
                        ...
                        buttons.push({
                            label: localize({
                                key: "open in",
                                comment: ["&& denotes a mnemonic"],
                            }, "&&Open in {0}", profileContentHandler.name),
                            run: async () => {
...
class UserDataProfileExportState extends UserDataProfileImportExportState {
    ...
    constructor(readonly profile: IUserDataProfile, private readonly exportFlags: ProfileResourceTypeFlags | undefined, 
    @IQuickInputService
