...
import { emptyProgressRunner, IProgressIndicator, IProgressRunner, } from "../../../../platform/progress/common/progress.js";
...
export class EditorProgressIndicator extends Disposable implements IProgressIndicator {
    constructor(private readonly progressBar: ProgressBar, private readonly group: IEditorGroupView) {
        super();
...
export class ScopedProgressIndicator extends Disposable implements IProgressIndicator {
    ...
    private onDidScopeActivate(): void {
        ...
        if (this.progressState.type === ProgressIndicatorState.Type.While) {
            ...
            if (this.progressState.whileDelay > 0) {
                const remainingDelay = this.progressState.whileDelay -
    ...
    show(infiniteOrTotal: true | number, delay?: number): IProgressRunner {
        ...
        return {
            total: (total: number) => {
                this.progressState = new ProgressIndicatorState.Work(total, this.progressState.type === ProgressIndicatorState.Type.Work
            ...
            worked: (worked: number) => {
                ...
                if (!this.scope.isActive || this.progressBar.hasTotal()) {
                    this.progressState = new ProgressIndicatorState.Work(this.progressState.type ===
                        ProgressIndicatorState.Type.Work
                        ? this.progressState.total
                        : undefined, this.progressState.type ===
                        ProgressIndicatorState.Type.Work &&
