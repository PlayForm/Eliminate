...
import { INotificationHandle, INotificationService, isNotificationSource, NotificationPriority, NotificationsFilter, Severity, } from "../../../../platform/notification/common/notification.js";
import { IProgress, IProgressCompositeOptions, IProgressDialogOptions, IProgressIndicator, IProgressNotificationOptions, IProgressOptions, IProgressRunner, IProgressService, IProgressStep, IProgressWindowOptions, Progress, ProgressLocation, } from "../../../../platform/progress/common/progress.js";
...
import { IStatusbarEntry, IStatusbarEntryAccessor, IStatusbarService, StatusbarAlignment, } from "../../statusbar/browser/statusbar.js";
...
export class ProgressService extends Disposable implements IProgressService {
    ...
    async withProgress<R = unknown>(options: IProgressOptions, originalTask: (progress: IProgress<IProgressStep>) => Promise<R>, onDidCancel?: (choice?: number) => void): Promise<R> {
        ...
        const handleStringLocation = (location: string) => {
            ...
            if (this.viewDescriptorService.getViewDescriptorById(location) !==
                null) {
        ...
        switch (location) {
            case ProgressLocation.Notification: {
                ...
                if (priority !== NotificationPriority.URGENT) {
                    if (this.notificationService.getFilter() ===
    ...
    private withWindowProgress<R = unknown>(options: IProgressWindowOptions, callback: (progress: IProgress<{
        message?: string;
    }>) => Promise<R>): Promise<R> {
        const task: [
            IProgressWindowOptions,
    ...
    private updateWindowProgress(idx: number = 0) {
        ...
        if (idx < this.windowProgressStack.length) {
            ...
            let text: string;
            let title: string;
            const source = options.source && typeof options.source !== "string"
            ...
            if (progressTitle && progressMessage) {
                ...
                text = progressTitle;
                title = source
                    ? localize("progress.title2", "[{0}]: {1}", source, progressTitle)
                    : text;
            }
                ...
                title = source
                    ? localize("progress.title2", "[{0}]: {1}", source, progressMessage)
                    : text;
            }
            else {
        ...
        const createDialog = (message: string) => {
            ...
            dialog = new Dialog(this.layoutService.activeContainer, message, buttons, {
                ...
                keyEventProcessor: (event: StandardKeyboardEvent) => {
                    ...
                    if (resolved.kind === ResultKind.KbFound &&
                        resolved.commandId) {
