...
export abstract class BaseConfigurationResolverService extends AbstractVariableResolverService {
        ...
        for (const variable of variables) {
            ...
            switch (type) {
                ...
                case "command": {
                    ...
                    const commandId = (variableToCommandMap
    ...
    private findVariables(object: any, variables: string[]) {
        if (typeof object === "string") {
            let matches;
            while ((matches =
                BaseConfigurationResolverService.INPUT_OR_COMMAND_VARIABLES_PATTERN.exec(object)) !== null) {
                if (matches.length === 4) {
            ...
            for (const contributed of this._contributedVariables.keys()) {
                if (variables.indexOf(contributed) < 0 &&
        ...
        if (info) {
            ...
            switch (info.type) {
                ...
                case "command": {
                    ...
                    return this.userInputAccessQueue
                        .queue(() => this.commandService.executeCommand<string>(info.command, info.args))
                        .then((result) => {
                        if (typeof result === "string" ||
                            Types.isUndefinedOrNull(result)) {
                            return result;
