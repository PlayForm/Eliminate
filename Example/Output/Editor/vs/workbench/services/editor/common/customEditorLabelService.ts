...
import { dirname, isAbsolute, ParsedPath, parse as parsePath, } from "../../../../base/common/path.js";
import { relativePath as getRelativePath, dirname as resourceDirname, } from "../../../../base/common/resources.js";
...
export class CustomEditorLabelService extends Disposable implements ICustomEditorLabelService {
    ...
    private applyTemplate(template: string, resource: URI, relevantPath: string): string {
        ...
        return template.replace(this._parsedTemplateExpression, (match: string, variable: string, ...args: any[]) => {
            ...
            // named group matches
            const { dirnameN = "0", extnameN = "0", }: {
                dirnameN?: string;
                extnameN?: string;
            } = args.pop();
            if (variable === "filename") {
                const { filename } = this._filenameCaptureExpression.exec(parsedPath.base)
                    ?.groups ?? {};
                if (filename) {
                    return filename;
                }
