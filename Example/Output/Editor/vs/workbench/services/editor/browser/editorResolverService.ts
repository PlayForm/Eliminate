...
import { EditorAssociation, EditorAssociations, EditorInputFactoryObject, editorsAssociationsSettingId, globMatchesResource, IEditorResolverService, priorityToRank, RegisteredEditorInfo, RegisteredEditorOptions, RegisteredEditorPriority, ResolvedEditor, ResolvedStatus, } from "../common/editorResolverService.js";
...
export class EditorResolverService extends Disposable implements IEditorResolverService {
    ...
    private resolveUntypedInputAndGroup(editor: IUntypedEditorInput, preferredGroup: PreferredGroup | undefined): Promise<[
        IUntypedEditorInput,
        IEditorGroup,
    ...
    getAssociationsForResource(resource: URI): EditorAssociations {
        ...
        let matchingAssociations = associations.filter((association) => association.filenamePattern &&
        ...
        matchingAssociations = matchingAssociations.sort((a, b) => (b.filenamePattern?.length ?? 0) -
    ...
    getAllUserAssociations(): EditorAssociations {
        const inspectedEditorAssociations = this.configurationService.inspect<{
    ...
    private findMatchingEditors(resource: URI): RegisteredEditor[] {
        ...
        return matchingEditors.sort((a, b) => {
            ...
            if (priorityToRank(b.editorInfo.priority) ===
            ...
            return (priorityToRank(b.editorInfo.priority) -
                priorityToRank(a.editorInfo.priority));
        });
    }
    public getEditors(resource?: URI): RegisteredEditorInfo[] {
        ...
        if (URI.isUri(resource)) {
            ...
            if (editors.find((e) => e.editorInfo.priority ===
                RegisteredEditorPriority.exclusive)) {
                return [];
            }
    ...
    private getEditor(resource: URI, editorId: string | EditorResolution.EXCLUSIVE_ONLY | undefined): {
        ...
        let possibleEditors = editors.filter((editor) => priorityToRank(editor.editorInfo.priority) >=
            priorityToRank(minPriority) &&
            editor.editorInfo.id !== DEFAULT_EDITOR_ASSOCIATION.id);
        if (possibleEditors.length === 0) {
            return {
                editor: associationsFromSetting[0] &&
        ...
        const selectedViewType = possibleEditors[0].editorInfo.priority ===
            RegisteredEditorPriority.exclusive
        ...
        possibleEditors = possibleEditors.filter((editor) => editor.editorInfo.priority !==
            RegisteredEditorPriority.exclusive);
        if (associationsFromSetting.length === 0 &&
     ...
     */
    private async moveExistingEditorForResource(existingEditorsForResource: Array<{
        editor: EditorInput;
        group: IEditorGroup;
    }>, targetGroup: IEditorGroup): Promise<EditorInput | undefined> {
     ...
     */
    private findExistingEditorsForResource(resource: URI, editorId: string): Array<{
        editor: EditorInput;
        group: IEditorGroup;
    }> {
        const out: Array<{
        ...
        const handle = this.notificationService.prompt(Severity.Warning, localize("editorResolver.conflictingDefaults", "There are multiple default editors available for the resource."), [
            {
                ...
                run: async () => {
                    ...
                    if (replacementEditor === ResolvedStatus.ABORT ||
    ...
    private mapEditorsToQuickPickEntry(resource: URI, showDefaultPicker?: boolean) {
        ...
        let registeredEditors = resource.scheme === Schemas.untitled
            ? this._registeredEditors.filter((e) => e.editorInfo.priority !==
                RegisteredEditorPriority.exclusive)
    ...
    private async doPickEditor(editor: IUntypedEditorInput, showDefaultPicker?: boolean): Promise<IEditorOptions | undefined> {
        ...
        const editorPicker = disposables.add(this.quickInputService.createQuickPick<IQuickPickItem>({
            useSeparators: true,
        }));
