...
import { IAuthenticationProvider, IResourcePreview, isAuthenticationProvider, IUserDataAutoSyncService, IUserDataSyncEnablementService, IUserDataSyncResource, IUserDataSyncService, IUserDataSyncStoreManagementService, SyncStatus, USER_DATA_SYNC_LOG_ID, USER_DATA_SYNC_SCHEME, } from "../../../../platform/userDataSync/common/userDataSync.js";
...
import { AccountStatus, CONTEXT_ACCOUNT_STATE, CONTEXT_ENABLE_ACTIVITY_VIEWS, CONTEXT_ENABLE_SYNC_CONFLICTS_VIEW, CONTEXT_HAS_CONFLICTS, CONTEXT_SYNC_ENABLEMENT, CONTEXT_SYNC_STATE, IUserDataSyncAccount, IUserDataSyncConflictsView, IUserDataSyncWorkbenchService, SHOW_SYNC_LOG_COMMAND_ID, SYNC_CONFLICTS_VIEW_ID, SYNC_TITLE, SYNC_VIEW_CONTAINER_ID, } from "../common/userDataSync.js";
...
export class UserDataSyncWorkbenchService extends Disposable implements IUserDataSyncWorkbenchService {
    ...
    private updateAuthenticationProviders(): void {
        this._authenticationProviders = (this.userDataSyncStoreManagementService.userDataSyncStore
            ?.authenticationProviders || []).filter(({ id }) => this.authenticationService.declaredProviders.some((provider) => provider.id === id));
    }
    ...
    private async initialize(): Promise<void> {
        if (isWeb) {
            ...
            if (this.currentSessionId === undefined &&
                ...
                if (this.environmentService.options?.settingsSyncOptions
        ...
        this._register(Event.filter(Event.any(this.authenticationService
            .onDidRegisterAuthenticationProvider, this.authenticationService
            .onDidUnregisterAuthenticationProvider), (info) => this.isSupportedAuthenticationProviderId(info.id))(() => this.update()));
    ...
    async synchroniseUserDataSyncStoreType(): Promise<void> {
        ...
        const userDataSyncStoreUrl = this.userDataSyncStoreManagementService.userDataSyncStore.type ===
            "insiders"
            ? this.userDataSyncStoreManagementService.userDataSyncStore
                .stableUrl
            : this.userDataSyncStoreManagementService.userDataSyncStore
    ...
    private async doTurnOnSync(token: CancellationToken): Promise<void> {
        ...
        try {
            await this.progressService.withProgress({
                ...
                if (this.userDataSyncService.status ===
            ...
            buttons: [
                {
                    ...
                    run: async () => {
                        const waitUntilConflictsAreResolvedPromise = raceCancellationError(Event.toPromise(Event.filter(this.userDataSyncService
                            .onDidChangeConflicts, (conficts) => conficts.length === 0)), token);
    ...
    private async replace(local: boolean): Promise<void> {
        for (const conflict of this.userDataSyncService.conflicts) {
            for (const preview of conflict.conflicts) {
                ...
                }, local ? preview.remoteResource : preview.localResource, undefined, { force: true });
            }
        }
    }
    async accept(resource: IUserDataSyncResource, conflictResource: URI, content: string | null | undefined, apply: boolean | {
        force: boolean;
    }): Promise<void> {
    ...
    async downloadSyncActivity(): Promise<URI | undefined> {
        ...
        return this.progressService.withProgress({ location: ProgressLocation.Window }, async () => {
            ...
            const name = (currentMachine ? currentMachine.name + " - " : "") +
            ...
            const folder = this.uriIdentityService.extUri.joinPath(result[0], indexes[0] !== 0
                ? name
    ...
    private createQuickpickItems(authenticationProviders: IAuthenticationProvider[], allAccounts: Map<string, UserDataSyncAccount[]>): (AccountQuickPickItem | IQuickPickSeparator)[] {
        ...
        for (const authenticationProvider of authenticationProviders) {
            ...
            if (!allAccounts.has(authenticationProvider.id) ||
    ...
    private get currentAuthenticationProviderId(): string | undefined {
        if (this._cachedCurrentAuthenticationProviderId === null) {
            this._cachedCurrentAuthenticationProviderId =
                this.storageService.get(UserDataSyncWorkbenchService.CACHED_AUTHENTICATION_PROVIDER_KEY, StorageScope.APPLICATION);
        }
    ...
    private set currentAuthenticationProviderId(currentAuthenticationProviderId: string | undefined) {
        if (this._cachedCurrentAuthenticationProviderId !==
