...
import { IRemoteUserData, IUserData, IUserDataSyncLogService, IUserDataSyncResourceInitializer, IUserDataSyncStoreManagementService, SyncResource, } from "../../../../platform/userDataSync/common/userDataSync.js";
                ...
                try {
                    ...
                    try {
                        authenticationSession =
                            await getCurrentAuthenticationSessionInfo(this.secretStorageService, this.productService);
                    }
                    catch (error) {
                    ...
                    const userDataSyncStore = this.userDataSyncStoreManagementService
                        .userDataSyncStore;
    ...
    private async initializeUserDataSyncStore(authenticationSession: AuthenticationSessionInfo): Promise<void> {
        ...
        try {
            ...
            this.globalStateUserData =
                await userDataSyncStoreClient.readResource(SyncResource.GlobalState, null);
            if (this.globalStateUserData) {
                ...
                if (userDataSyncStoreType) {
                    ...
                    if (!isEqual(userDataSyncStore.url, this.userDataSyncStoreManagementService
    ...
    private async initialize(syncResources: SyncResource[]): Promise<void> {
        ...
        await Promises.settled(syncResources.map(async (syncResource) => {
            try {
                ...
                const userData = await userDataSyncStoreClient.readResource(syncResource, syncResource === SyncResource.GlobalState
                    ? this.globalStateUserData
                    : null);
...
class ExtensionsPreviewInitializer extends AbstractExtensionsInitializer {
    ...
    constructor(private readonly extensionsData: IUserData, 
...
class InstalledExtensionsInitializer implements IUserDataSyncResourceInitializer {
    constructor(private readonly extensionsPreviewInitializer: ExtensionsPreviewInitializer, 
