    ...
    provideTextSearchResultsWithRgOptions(query: TextSearchQuery2, options: RipgrepTextSearchOptions, progress: Progress<TextSearchResult2>, token: CancellationToken): Promise<TextSearchComplete2> {
        ...
        return new Promise((resolve, reject) => {
            ...
            rgProc.on("close", () => {
                ...
                this.outputChannel.appendLine(gotResult
...
function buildRegexParseError(lines: string[]): string {
    ...
    if (pcre2ErrorLine.length >= 1) {
        ...
        if (pcre2ErrorMessage.indexOf(":") !== -1 &&
...
export function getRgArgs(query: TextSearchQuery2, options: RipgrepTextSearchOptions): string[] {
    ...
    const { doubleStarIncludes, otherIncludes } = groupBy(options.folderOptions.includes, (include: string) => include.startsWith("**") ? "doubleStarIncludes" : "otherIncludes");
    ...
    if (options.folderOptions.encoding &&
        ...
        onCharacterEnter(char) {
            ...
            if (!parent) {
                ...
                if (parent.negate) {
                    ...
                    const otherContent = pattern.slice(parent.start + 2, char.start) +
                    ...
                    const otherContent = pattern.slice(parent.start + 1, char.start) +
                        pattern.slice(char.end, parent.end - 1);
                    replace(parent.start, parent.end, otherContent === ""
                        ? "\\r?\\n"
                        : `(?:[${otherContent}]|\\r?\\n)`);
                }
            }
