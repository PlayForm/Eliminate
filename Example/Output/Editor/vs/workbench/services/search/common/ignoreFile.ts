...
export class IgnoreFile {
    ...
    constructor(contents: string, private readonly location: string, private readonly parent?: IgnoreFile) {
    ...
    isPathIncludedInTraversal(path: string, isDir: boolean): boolean {
        if (path[0] !== "/" || path[path.length - 1] === "/") {
            throw Error("Unexpected path format, expectred to begin with slash and end without. got:" +
                path);
        }
