...
export const getFileResults = (bytes: Uint8Array, pattern: RegExp, options: {
    ...
    if (patternIndecies.length) {
        ...
        for (const { matchStartIndex, matchedText } of patternIndecies) {
            ...
            while (Boolean(lineRanges[startLine + 1]) &&
            ...
            const fileRange = new Range(startLine, matchStartIndex - lineRanges[startLine].start, endLine, matchStartIndex +
                matchedText.length -
            ...
            results.push(match);
            if (options.surroundingContext) {
                for (let contextLine = endLine + 1; contextLine <=
                    Math.min(endLine + options.surroundingContext, lineRanges.length - 1); contextLine++) {
                    contextLinesNeeded.add(contextLine);
                }
            }
        }
