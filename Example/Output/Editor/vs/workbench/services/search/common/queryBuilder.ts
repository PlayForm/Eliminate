...
import { basename, isAbsolutePath, isEqual, relativePath, } from "../../../../base/common/resources.js";
...
export class QueryBuilder {
    ...
    text(contentPattern: IPatternInfo, folderResources?: uri[], options: ITextQueryBuilderOptions = {}): ITextQuery {
        ...
        const fallbackToPCRE = folderResources &&
            folderResources.some((folder) => {
                const folderConfig = this.configurationService.getValue<ISearchConfiguration>({
                    resource: folder,
                });
        ...
        return {
            ...
            userDisabledExcludesAndIgnoreFiles: options.disregardExcludeSettings &&
    ...
    private commonQuery(folderResources: (IWorkspaceFolderData | URI)[] = [], options: ICommonQueryBuilderOptions = {}): ICommonQueryProps<uri> {
        let excludePatterns: string | string[] | undefined = Array.isArray(options.excludePattern)
        ...
        const folderQueries = (includeSearchPathsInfo.searchPaths &&
            ...
            : folderResources.map((folder) => this.getFolderQueryForRoot(folder, options, excludeSearchPathsInfo, includeFolderName))).filter((query) => !!query) as IFolderQuery[];
        const queryProps: ICommonQueryProps<uri> = {
            _reason: options._reason,
            folderQueries,
            usingSearchPaths: !!(includeSearchPathsInfo.searchPaths &&
                includeSearchPathsInfo.searchPaths.length),
    ...
    private commonQueryFromFileList(files: URI[]): ICommonQueryProps<URI> {
        ...
        files.forEach((file) => {
            ...
            if (providerExists) {
                const searchRoot = this.workspaceContextService.getWorkspaceFolder(file)
    ...
    parseSearchPaths(pattern: string | string[]): ISearchPathsInfo {
        ...
        const segments = patterns.map((segment) => {
            ...
            if (userHome) {
                return untildify(segment, userHome.scheme === Schemas.file
                    ? userHome.fsPath
                    : userHome.path);
            }
            return segment;
    ...
    private getFolderQueryForRoot(folder: IWorkspaceFolderData | URI, options: ICommonQueryBuilderOptions, searchPathExcludes: ISearchPathsInfo, includeFolderName: boolean): IFolderQuery | null {
        ...
        let excludeFolderRoots = options.excludePattern?.map((excludePattern) => {
            const excludeRoot = options.excludePattern &&
                isISearchPatternBuilder(excludePattern)
                ? excludePattern.uri
                : undefined;
            const shouldUseExcludeRoot = !excludeRoot ||
                !(URI.isUri(folder) &&
                    this.uriIdentityService.extUri.isEqual(folder, excludeRoot));
        ...
        if (searchPathExcludes.searchPaths) {
            ...
            if (thisFolderExcludeSearchPath &&
