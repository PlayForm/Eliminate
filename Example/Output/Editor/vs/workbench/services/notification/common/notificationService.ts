...
import { INeverShowAgainOptions, INotification, INotificationActions, INotificationHandle, INotificationService, INotificationSource, INotificationSourceFilter, IPromptChoice, IPromptOptions, isNotificationSource, IStatusMessageOptions, NeverShowAgainScope, NoOpNotification, NotificationMessage, NotificationsFilter, Severity, } from "../../../../platform/notification/common/notification.js";
import { IStorageService, StorageScope, StorageTarget, } from "../../../../platform/storage/common/storage.js";
import { ChoiceAction, NotificationChangeType, NotificationsModel, } from "../../../common/notifications.js";
export class NotificationService extends Disposable implements INotificationService {
        ...
        this._register(this.model.onDidChangeNotification((e) => {
            switch (e.kind) {
                ...
                case NotificationChangeType.REMOVE: {
                    const source = typeof e.item.sourceId === "string" &&
    ...
    setFilter(filter: NotificationsFilter | INotificationSourceFilter): void {
        if (typeof filter === "number") {
            ...
            if (existing?.filter === filter.filter &&
    ...
    getFilter(source?: INotificationSource): NotificationsFilter {
        if (source) {
            return (this.mapSourceToFilter.get(source.id)?.filter ??
    ...
    private updateFilters(): void {
        this.model.setFilter({
            global: this.globalFilterEnabled
                ? NotificationsFilter.ERROR
                : NotificationsFilter.OFF,
            sources: new Map([...this.mapSourceToFilter.values()].map((source) => [
                source.id,
                source.filter,
            ])),
        });
