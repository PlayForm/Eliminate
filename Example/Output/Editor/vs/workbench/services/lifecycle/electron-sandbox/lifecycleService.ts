 ...
 *--------------------------------------------------------------------------------------------*/
import { disposableTimeout, Promises, raceCancellation, } from "../../../../base/common/async.js";
...
import { ILifecycleService, IWillShutdownEventJoiner, ShutdownReason, WillShutdownJoinerOrder, } from "../common/lifecycle.js";
...
export class NativeLifecycleService extends AbstractLifecycleService {
    ...
    private registerListeners(): void {
        ...
        ipcRenderer.on("vscode:onBeforeUnload", async (event: unknown, reply: {
            okChannel: string;
        ...
        ipcRenderer.on("vscode:onWillUnload", async (event: unknown, reply: {
            replyChannel: string;
            reason: ShutdownReason;
        }) => {
        ...
        this._onWillShutdown.fire({
            ...
            join(promiseOrPromiseFn, joiner) {
                ...
                if (joiner.order === WillShutdownJoinerOrder.Last) {
                    const promiseFn = typeof promiseOrPromiseFn === "function"
                        ? promiseOrPromiseFn
                        : () => promiseOrPromiseFn;
                    lastJoiners.push(() => promiseFn().finally(() => pendingJoiners.delete(joiner)));
                }
                else {
                    const promise = typeof promiseOrPromiseFn === "function"
                        ? promiseOrPromiseFn()
                        : promiseOrPromiseFn;
