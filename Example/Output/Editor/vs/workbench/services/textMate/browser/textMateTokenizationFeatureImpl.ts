...
import { missingTMGrammarErrorMessage, TMGrammarFactory, } from "../common/TMGrammarFactory.js";
import { grammarsExtPoint, ITMSyntaxExtensionPoint, } from "../common/TMGrammars.js";
...
export class TextMateTokenizationFeature extends Disposable implements ITextMateTokenizationService {
    ...
    private _validateGrammarDefinition(extension: IExtensionPointUser<ITMSyntaxExtensionPoint[]>, grammar: ITMSyntaxExtensionPoint): IValidGrammarDefinition | null {
        ...
        const validLanguageId = grammar.language &&
    ...
    private async _getOrCreateGrammarFactory(): Promise<TMGrammarFactory> {
        ...
        const [vscodeTextmate, vscodeOniguruma] = await Promise.all([
            importAMDNodeModule<typeof import("vscode-textmate")>("vscode-textmate", "release/main.js"),
    ...
    private async _createTokenizationSupport(languageId: string): Promise<(ITokenizationSupport & IDisposable) | null> {
        ...
        try {
            ...
            const maxTokenizationLineLength = observableConfigValue<number>("editor.maxTokenizationLineLength", languageId, -1, this._configurationService);
        ...
        if (!this._vscodeOniguruma) {
            this._vscodeOniguruma = (async () => {
                const [vscodeOniguruma, wasm] = await Promise.all([
                    importAMDNodeModule<typeof import("vscode-oniguruma")>("vscode-oniguruma", "release/main.js"),
