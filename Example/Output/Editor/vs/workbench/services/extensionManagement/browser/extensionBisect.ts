...
class BisectState {
    ...
    constructor(readonly extensions: string[], readonly low: number, readonly high: number, readonly mid: number = ((low + high) / 2) | 0) { }
}
class ExtensionBisectService implements IExtensionBisectService {
    ...
    async next(seeingBad: boolean): Promise<{
        id: string;
        bad: boolean;
    } | undefined> {
        if (!this._state) {
...
Registry.as<IWorkbenchContributionsRegistry>(Extensions.Workbench).registerWorkbenchContribution(ExtensionBisectUi, LifecyclePhase.Restored);
