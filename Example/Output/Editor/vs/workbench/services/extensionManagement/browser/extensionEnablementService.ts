...
import { DISABLED_EXTENSIONS_STORAGE_PATH, ENABLED_EXTENSIONS_STORAGE_PATH, IExtensionIdentifier, IExtensionManagementService, IGlobalExtensionEnablementService, InstallOperation, } from "../../../../platform/extensionManagement/common/extensionManagement.js";
...
import { EnablementState, ExtensionInstallLocation, IExtensionManagementServer, IExtensionManagementServerService, IWorkbenchExtensionEnablementService, IWorkbenchExtensionManagementService, } from "../common/extensionManagement.js";
...
export class ExtensionEnablementService extends Disposable implements IWorkbenchExtensionEnablementService {
    ...
    private throwErrorIfCannotChangeEnablement(extension: IExtension, donotCheckDependencies?: boolean): void {
        ...
        if (this.userDataSyncEnablementService.isEnabled() &&
            ...
            extension.manifest.contributes!.authentication!.some((a) => a.id ===
                this.userDataSyncAccountService.account!
    ...
    private throwErrorIfEnablementStateCannotBeChanged(extension: IExtension, enablementStateOfExtension: EnablementState, donotCheckDependencies?: boolean): void {
        switch (enablementStateOfExtension) {
            case EnablementState.DisabledByEnvironment:
                throw new Error(localize("cannot change disablement environment", "Cannot change enablement of {0} extension because it is disabled in environment", extension.manifest.displayName ||
                    extension.identifier.id));
            case EnablementState.DisabledByVirtualWorkspace:
                throw new Error(localize("cannot change enablement virtual workspace", "Cannot change enablement of {0} extension because it does not support virtual workspaces", extension.manifest.displayName ||
                    extension.identifier.id));
            case EnablementState.DisabledByExtensionKind:
                throw new Error(localize("cannot change enablement extension kind", "Cannot change enablement of {0} extension because of its extension kind", extension.manifest.displayName ||
                    extension.identifier.id));
            case EnablementState.DisabledByInvalidExtension:
                throw new Error(localize("cannot change invalid extension enablement", "Cannot change enablement of {0} extension because of it is invalid", extension.manifest.displayName ||
                    extension.identifier.id));
            case EnablementState.DisabledByExtensionDependency:
                ...
                for (const dependency of getExtensionDependencies(this.extensionsManager.extensions, extension)) {
                    ...
                    throw new Error(localize("cannot change enablement dependency", "Cannot enable '{0}' extension because it depends on '{1}' extension that cannot be enabled", extension.manifest.displayName ||
                        extension.identifier.id, dependency.manifest.displayName ||
    ...
    async setEnablement(extensions: IExtension[], newState: EnablementState): Promise<boolean[]> {
        ...
        if (newState === EnablementState.EnabledGlobally ||
        ...
        const workspace = newState === EnablementState.DisabledWorkspace ||
            newState === EnablementState.EnabledWorkspace;
        ...
        for (const extension of extensions) {
            ...
            if (enablementState ===
                ...
                (enablementState ===
                    ...
                    this.getDependenciesEnablementStates(extension).every(([, e]) => this.isEnabledEnablementState(e) ||
    ...
    private getExtensionsToEnableRecursively(extensions: IExtension[], allExtensions: ReadonlyArray<IExtension>, enablementState: EnablementState, options: {
    ...
    isEnabledEnablementState(enablementState: EnablementState): boolean {
        return (enablementState === EnablementState.EnabledByEnvironment ||
            ...
            for (const extensionKind of this.extensionManifestPropertiesService.getExtensionKind(extension.manifest)) {
                ...
                if (extensionKind === "web") {
                    if (this.extensionManagementServerService
                        ...
                        if (installLocation === ExtensionInstallLocation.Web ||
    ...
    private _isDisabledByExtensionDependency(extension: IExtension, extensions: ReadonlyArray<IExtension>, workspaceType: WorkspaceType, computedEnablementStates: Map<IExtension, EnablementState>): boolean {
        ...
        const dependencyExtensions = extensions.filter((e) => extension.manifest.extensionDependencies?.some((id) => areSameExtensions(e.identifier, { id }) &&
            (this.extensionManagementServerService.getExtensionManagementServer(e) ===
                ...
                ((e.manifest.main || e.manifest.browser) &&
                    e.manifest.api === "none"))));
    ...
    private _onDidChangeExtensions(added: ReadonlyArray<IExtension>, removed: ReadonlyArray<IExtension>, isProfileSwitch: boolean): void {
        ...
        this.extensionsDisabledByExtensionDependency =
            this.extensionsManager.extensions.filter((extension) => this.getEnablementState(extension) ===
                EnablementState.DisabledByExtensionDependency);
    ...
    public async updateExtensionsEnablementsWhenWorkspaceTrustChanges(): Promise<void> {
        await this.extensionsManager.whenInitialized();
        const computeEnablementStates = (workspaceType: WorkspaceType): [
            IExtension,
            EnablementState
        ][] => {
            const extensionsEnablements = new Map<IExtension, EnablementState>();
        ...
        const enablementChangedExtensionsBecauseOfTrust = enablementStatesWithTrustedWorkspace
            .filter(([, enablementState], index) => enablementState !==
...
class ExtensionsManager extends Disposable {
    ...
    private _onDidChangeExtensions = this._register(new Emitter<{
    ...
    private async initialize(): Promise<void> {
        ...
        this._register(this.extensionManagementService.onDidInstallExtensions((e) => this.updateExtensions(e.reduce<IExtension[]>((result, { local, operation }) => {
        ...
        if (added.length) {
            for (const extension of added) {
                ...
                const index = this._extensions.findIndex((e) => areSameExtensions(e.identifier, extension.identifier) &&
                    this.extensionManagementServerService.getExtensionManagementServer(e) === extensionServer);
                if (index !== -1) {
