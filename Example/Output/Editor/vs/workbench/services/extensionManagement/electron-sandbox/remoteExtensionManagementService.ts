...
import { EXTENSION_INSTALL_CLIENT_TARGET_PLATFORM_CONTEXT, ExtensionManagementError, ExtensionManagementErrorCode, IExtensionGalleryService, IGalleryExtension, ILocalExtension, InstallOperation, InstallOptions, } from "../../../../platform/extensionManagement/common/extensionManagement.js";
...
export class NativeRemoteExtensionManagementService extends RemoteExtensionManagementService {
    constructor(channel: IChannel, private readonly localExtensionManagementServer: IExtensionManagementServer, 
    ...
    private async downloadCompatibleAndInstall(extension: IGalleryExtension, installed: ILocalExtension[], installOptions: InstallOptions): Promise<ILocalExtension> {
        ...
        const location = await this.localExtensionManagementServer.extensionManagementService.download(compatible, installed.filter((i) => areSameExtensions(i.identifier, compatible.identifier))[0]
            ...
            : InstallOperation.Install, !!installOptions.donotVerifySignature);
    ...
    private async checkAndGetCompatible(extension: IGalleryExtension, includePreRelease: boolean): Promise<IGalleryExtension> {
        ...
        if (extension.hasPreReleaseVersion &&
            ...
            compatibleExtension =
                ...
                ], { targetPlatform, compatible: true }, CancellationToken.None))[0] || null;
        }
        ...
        if (!compatibleExtension) {
            compatibleExtension =
                await this.galleryService.getCompatibleExtension(extension, includePreRelease, targetPlatform);
        }
        if (!compatibleExtension) {
