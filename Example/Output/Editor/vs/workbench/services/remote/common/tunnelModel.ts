...
import { ALL_INTERFACES_ADDRESSES, isAllInterfaces, isLocalhost, ITunnelService, LOCALHOST_ADDRESSES, PortAttributesProvider, ProvidedOnAutoForward, ProvidedPortAttributes, RemoteTunnel, TunnelPrivacyId, TunnelProtocol, } from "../../../../platform/tunnel/common/tunnel.js";
...
export class PortsAttributes extends Disposable {
    ...
    private hasHostAndPort(value: number | PortRange | RegExp | HostAndPort): value is HostAndPort {
        return ((<any>value).host !== undefined &&
...
export class TunnelModel extends Disposable {
    ...
    private _onClosePort: Emitter<{
        host: string;
        port: number;
    }> = new Emitter();
    public onClosePort: Event<{
        host: string;
        port: number;
    }> = this._onClosePort.event;
    private _onPortName: Emitter<{
        host: string;
        port: number;
    }> = new Emitter();
    public onPortName: Event<{
        host: string;
        port: number;
    }> = this._onPortName.event;
    private _candidates: Map<string, CandidatePort> | undefined;
    private _onCandidatesChanged: Emitter<Map<string, {
        host: string;
        port: number;
    }>> = new Emitter();
    // onCandidateChanged returns the removed candidates
    public onCandidatesChanged: Event<Map<string, {
        host: string;
        port: number;
            ...
            for (const tunnel of tunnels) {
                if (tunnel.localAddress) {
                    ...
                    this.forwarded.set(key, {
                        ...
                        protocol: attributes?.get(tunnel.tunnelRemotePort)
    ...
    private async onTunnelClosed(address: {
        host: string;
        port: number;
        ...
        if (this.configurationService.getValue("remote.restoreForwardedPorts")) {
            ...
            if (tunnelRestoreValue &&
                ...
                for (const tunnel of tunnels) {
                    ...
                    if ((tunnel.source.source !== TunnelSource.Extension &&
                        !alreadyForwarded) ||
                    ...
                    else if (tunnel.source.source === TunnelSource.Extension &&
                        !alreadyForwarded) {
    ...
    private async storeForwarded() {
        if (this.configurationService.getValue("remote.restoreForwardedPorts")) {
            ...
            else if (valueToStore !== this.knownPortsRestoreValue &&
                key &&
                expirationKey) {
    ...
    private async showPortMismatchModalIfNeeded(tunnel: RemoteTunnel, expectedLocal: number, attributes: Attributes | undefined) {
        ...
        if (this.mismatchCooldown.getTime() + MISMATCH_LOCAL_PORT_COOLDOWN >
            newCooldown.getTime()) {
            return;
        }
        ...
        attributes =
            attributes ??
                (attributes !== null
                    ...
                    : undefined);
        const localPort = tunnelProperties.local !== undefined
    ...
    private updateInResponseToCandidates(candidates: CandidatePort[]): Map<string, {
        host: string;
        port: number;
    }> {
    ...
    async getAttributes(forwardedPorts: {
        host: string;
        port: number;
    }[], checkProviders: boolean = true): Promise<Map<number, Attributes> | undefined> {
