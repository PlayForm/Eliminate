...
import { CustomTitleBarVisibility, getMenuBarVisibility, hasCustomTitlebar, hasNativeTitlebar, TitleBarSetting, } from "../../../../platform/window/common/window.js";
...
export function shouldShowCustomTitleBar(configurationService: IConfigurationService, window: Window, menuBarToggled?: boolean): boolean {
    ...
    if (!isWeb) {
        ...
        if ((showCustomTitleBar === CustomTitleBarVisibility.NEVER &&
...
function isTitleBarEmpty(configurationService: IConfigurationService): boolean {
    ...
    if (activityBarPosition === ActivityBarPosition.TOP ||
    ...
    if (editorActionsLocation === EditorActionsLocation.TITLEBAR ||
