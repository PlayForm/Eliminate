...
import { isObject, isString, isStringArray, isUndefinedOrNull, } from "../../../../base/common/types.js";
...
function getStringValidators(prop: IConfigurationPropertySchema) {
    ...
    return [
        ...
        },
        {
            ...
            message: nls.localize("validations.invalidStringEnumValue", "Value is not accepted. Valid values: {0}.", prop.enum
...
function getNumericValidators(prop: IConfigurationPropertySchema): Validator<number>[] {
    const type: (string | undefined)[] = Array.isArray(prop.type)
        ...
        : [prop.type];
    const isNullable = canBeType(type, "null");
    const isIntegral = canBeType(type, "integer") &&
        (type.length === 1 || (type.length === 2 && isNullable));
    const isNumeric = canBeType(type, "number", "integer") &&
        (type.length === 1 || (type.length === 2 && isNullable));
    if (!isNumeric) {
        return [];
    }
    ...
    return [
        ...
        {
            enabled: prop.maximum !== undefined &&
        ...
        {
            enabled: prop.minimum !== undefined &&
    ...
    if (prop.type === "array" && prop.items && !Array.isArray(prop.items)) {
        ...
        if (propItems && !Array.isArray(propItems.type)) {
            ...
            return (value) => {
                    ...
                    if (propItemsEnum) {
                        arrayValue.forEach((v) => {
                            if (propItemsEnum.indexOf(v) === -1) {
                                message += nls.localize("validations.stringArrayItemEnum", "Value {0} is not one of {1}", withQuotes(v), "[" +
                                    propItemsEnum
                                        .map(withQuotes)
                                        .join(", ") +
                                    "]");
                ...
                else if (propItems.type === "integer" ||
