...
abstract class AbstractSettingsModel extends EditorModel {
    ...
    private removeDuplicateResults(): void {
        ...
        [...this._currentResultGroups.keys()]
            .sort((a, b) => this._currentResultGroups.get(a)!.order -
            ...
            for (const section of group.sections) {
                for (const setting of section.settings) {
                    ...
                    if (groupMatched || settingMatchResult) {
                        filterMatches.push({
                            setting,
                            matches: settingMatchResult &&
                                settingMatchResult.matches,
                            matchType: settingMatchResult?.matchType ??
...
export class DefaultSettings extends Disposable {
    ...
    constructor(private _mostCommonlyUsedSettingsKeys: string[], readonly target: ConfigurationTarget, readonly configurationService: IConfigurationService) {
        super();
        this._register(configurationService.onDidChangeConfiguration((e) => {
            if (e.source === ConfigurationTarget.DEFAULT) {
                this.reset();
    ...
    private parseConfig(config: IConfigurationNode, result: ISettingsGroup[], configurations: IConfigurationNode[], settingsGroup?: ISettingsGroup, seenSettings?: {
        [key: string]: boolean;
    }): ISettingsGroup[] {
        ...
        if (title) {
            if (!settingsGroup) {
                settingsGroup = result.find((g) => g.title === title &&
    ...
    private parseSettings(config: IConfigurationNode): ISetting[] {
        ...
        for (const key in settingsObject) {
            ...
            if (this.matchesScope(prop)) {
                ...
                const objectAdditionalProperties = prop.type === "object"
                ...
                if (!isLanguageTagSetting) {
                    ...
                    if (registeredConfigurationProp &&
                ...
                result.push({
                    ...
                    deprecationMessage: prop.markdownDeprecationMessage ||
...
export class DefaultSettingsEditorModel extends AbstractSettingsModel implements ISettingsEditorModel {
    ...
    private readonly _onDidChangeGroups: Emitter<void> = this._register(new Emitter<void>());
    readonly onDidChangeGroups: Event<void> = this._onDidChangeGroups.event;
    constructor(private _uri: URI, reference: IReference<ITextEditorModel>, private readonly defaultSettings: DefaultSettings) {
        super();
     ...
     */
    private writeResultGroups(groups: ISearchResultGroup[], startLine: number): {
        matches: IRange[];
            ...
            return {
                ...
                matches: filteredMatch.matches &&
                    filteredMatch.matches.map((match) => {
                        return new Range(match.startLineNumber -
                            filteredMatch.setting.range.startLineNumber, match.startColumn, match.endLineNumber -
                            filteredMatch.setting.range.startLineNumber, match.endColumn);
                    }),
            };
        });
