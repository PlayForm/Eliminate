...
class ExtensionUrlHandler implements IExtensionUrlHandler, IURLHandler {
    ...
    private uriBuffer = new Map<string, {
        timestamp: number;
        uri: URI;
    }[]>();
    ...
    private async handleURLByExtension(extensionId: ExtensionIdentifier | string, handler: IURLHandler, uri: URI, options?: IOpenURLOptions): Promise<boolean> {
        this.telemetryService.publicLog2<ExtensionUrlHandlerEvent, ExtensionUrlHandlerClassification>("uri_invoked/end", {
    ...
    private async handleUnhandledURL(uri: URI, extensionId: string, options?: IOpenURLOptions): Promise<void> {
        ...
        if (extension) {
            ...
            this.telemetryService.publicLog2<ExtensionUrlReloadHandlerEvent, ExtensionUrlReloadHandlerClassification>("uri_invoked/install_extension/reload", {
                extensionId,
    ...
    private garbageCollect(): void {
        ...
        const uriBuffer = new Map<string, {
            timestamp: number;
            uri: URI;
        }[]>();
