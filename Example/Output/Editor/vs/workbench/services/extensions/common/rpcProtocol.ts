...
class StringifiedJsonWithBufferRefs {
    constructor(public readonly jsonString: string, public readonly referencedBuffers: readonly VSBuffer[]) { }
}
    ...
    private _receiveOneMessage(rawmsg: VSBuffer): void {
        ...
        switch (messageType) {
            ...
            case MessageType.RequestMixedArgsWithCancellation: {
                ...
                this._receiveRequest(msgLength, req, rpcId, method, args, messageType ===
...
class PendingRPCReply {
    constructor(private readonly _promise: LazyPromise, private readonly _disposable: IDisposable) { }
...
type SerializedRequestArguments = {
    readonly type: SerializedRequestArgumentType.Simple;
    args: string;
} | {
    readonly type: SerializedRequestArgumentType.Mixed;
    args: MixedArg[];
};
class MessageIO {
    ...
    private static _requestJSONArgs(req: number, rpcId: number, method: string, args: string, usesCancellationToken: boolean): VSBuffer {
        ...
        const result = MessageBuffer.alloc(usesCancellationToken
            ...
            : MessageType.RequestJSONArgs, req, len);
    ...
    private static _requestMixedArgs(req: number, rpcId: number, method: string, args: readonly MixedArg[], usesCancellationToken: boolean): VSBuffer {
        ...
        const result = MessageBuffer.alloc(usesCancellationToken
            ? MessageType.RequestMixedArgsWithCancellation
            : MessageType.RequestMixedArgs, req, len);
        result.writeUInt8(rpcId);
        result.writeShortString(methodBuff);
...
type MixedArg = {
    readonly type: ArgType.String;
    readonly value: VSBuffer;
} | {
    readonly type: ArgType.VSBuffer;
    readonly value: VSBuffer;
} | {
