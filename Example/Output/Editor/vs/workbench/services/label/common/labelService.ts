...
import { basename, basenameOrAuthority, dirname, joinPath, } from "../../../../base/common/resources.js";
...
import { ISingleFolderWorkspaceIdentifier, isSingleFolderWorkspaceIdentifier, isTemporaryWorkspace, isUntitledWorkspace, isWorkspace, isWorkspaceIdentifier, IWorkspace, IWorkspaceContextService, IWorkspaceIdentifier, toWorkspaceIdentifier, WORKSPACE_EXTENSION, } from "../../../../platform/workspace/common/workspace.js";
...
export class LabelService extends Disposable implements ILabelService {
    ...
    private doGetUriLabel(resource: URI, formatting?: ResourceLabelFormatting, options: {
        relative?: boolean;
        noPrefix?: boolean;
    } = {}): string {
        if (!formatting) {
    ...
    private doGetWorkspaceLabel(workspaceUri: URI, options?: {
        verbose: Verbosity;
    }): string {
        // Workspace: Untitled
    ...
    private doGetSingleFolderWorkspaceLabel(folderUri: URI, options?: {
        verbose: Verbosity;
    }): string {
        let label: string;
        switch (options?.verbose) {
    ...
    registerCachedFormatter(formatter: ResourceLabelFormatter): IDisposable {
        ...
        let replace = list.findIndex((f) => f.scheme === formatter.scheme &&
    ...
    private formatUri(resource: URI, formatting: ResourceLabelFormatting, forceNoTildify?: boolean): string {
            ...
            switch (token) {
                ...
                case "path":
                    return formatting.stripPathStartingSeparator
                        ? resource.path.slice(resource.path[0] === formatting.separator
                            ? 1
                            : 0)
                        : resource.path;
                default: {
        ...
        if (formatting.normalizeDriveLetter &&
