...
export class LanguageDetectionService extends Disposable implements ILanguageDetectionService {
    ...
    public isEnabledForLanguage(languageId: string): boolean {
        return (!!languageId &&
            this._configurationService.getValue<boolean>(LanguageDetectionService.enablementSettingKey, { overrideIdentifier: languageId }));
    }
    private getLanguageBiases(): Record<string, number> {
        ...
        this._logService.trace("Historical Workspace Opened Languages:", JSON.stringify([
...
export class LanguageDetectionWorkerClient extends Disposable {
    ...
    constructor(private readonly _modelService: IModelService, private readonly _languageService: ILanguageService, private readonly _telemetryService: ITelemetryService, private readonly _indexJsUri: string, private readonly _modelJsonUri: string, private readonly _weightsUri: string, private readonly _regexpModelUri: string) {
        super();
    }
    private _getOrCreateLanguageDetectionWorker(): {
        ...
        if (!this.worker) {
            const workerClient = this._register(createWebWorker<ILanguageDetectionWorker>("vs/workbench/services/languageDetection/browser/languageDetectionSimpleWorker", "LanguageDetectionWorker"));
