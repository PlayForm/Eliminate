...
export class StoredFileWorkingCopy<M extends IStoredFileWorkingCopyModel> extends ResourceWorkingCopy implements IStoredFileWorkingCopy<M> {
    ...
    //#endregion
    constructor(readonly typeId: string, resource: URI, readonly name: string, private readonly modelFactory: IStoredFileWorkingCopyModelFactory<M>, private readonly externalResolver: IStoredFileWorkingCopyResolver, 
    @IFileService
    ...
    async save(options: IStoredFileWorkingCopySaveAsOptions = Object.create(null)): Promise<boolean> {
        ...
        if ((this.hasState(StoredFileWorkingCopyState.CONFLICT) ||
        ...
        return this.saveSequentializer.run(versionId, (async () => {
            ...
            if (this.isResolved() &&
                ...
                try {
                    ...
                    if (!saveCancellation.token.isCancellationRequested) {
                        ...
                        try {
                            await this.workingCopyFileService.runSaveParticipants(this, {
            ...
            return this.saveSequentializer.run(versionId, (async () => {
                try {
                    ...
                    if (typeof resolvedFileWorkingCopy.model.save ===
                        "function") {
                        try {
                            stat =
                                await resolvedFileWorkingCopy.model.save(writeFileOptions, saveCancellation.token);
                        }
                        catch (error) {
                            if (saveCancellation.token
                                .isCancellationRequested) {
                    ...
                    // Otherwise ask for a snapshot and save via file services
                        ...
                        if (options?.writeElevated &&
                            ...
                            stat =
                                await this.elevatedFileService.writeFileElevated(lastResolvedFileStat.resource, assertIsDefined(snapshot), writeFileOptions);
                        }
                        else {
    ...
    private handleSaveError(error: Error, versionId: number, options: IStoredFileWorkingCopySaveAsOptions): void {
        ...
        if ((error as FileOperationError).fileOperationResult ===
    ...
    private doHandleSaveError(error: Error, options: IStoredFileWorkingCopySaveAsOptions): void {
        ...
        if (fileOperationError.fileOperationResult ===
            FileOperationResult.FILE_MODIFIED_SINCE) {
        ...
        // Any other save error
            ...
            const isWriteLocked = fileOperationError.fileOperationResult ===
                FileOperationResult.FILE_WRITE_LOCKED;
            const triedToUnlock = isWriteLocked &&
                (fileOperationError.options as IWriteFileOptions | undefined)
                    ?.unlock;
            const isPermissionDenied = fileOperationError.fileOperationResult ===
