...
import { IStoredFileWorkingCopySaveEvent as IBaseStoredFileWorkingCopySaveEvent, IStoredFileWorkingCopy, IStoredFileWorkingCopyModel, IStoredFileWorkingCopyModelFactory, IStoredFileWorkingCopyResolveOptions, StoredFileWorkingCopy, StoredFileWorkingCopyState, } from "./storedFileWorkingCopy.js";
...
export class StoredFileWorkingCopyManager<M extends IStoredFileWorkingCopyModel> extends BaseFileWorkingCopyManager<M, IStoredFileWorkingCopy<M>> implements IStoredFileWorkingCopyManager<M> {
    ...
    private readonly mapCorrelationIdToWorkingCopiesToRestore = new Map<number, {
        source: URI;
        target: URI;
                        ...
                        for (const sourceWorkingCopy of sourceWorkingCopies) {
                            ...
                            workingCopiesToRestore.push({
                                ...
                                snapshot: sourceWorkingCopy.isDirty()
                                    ? await sourceWorkingCopy.model?.snapshot(SnapshotContext.Save, CancellationToken.None)
                                    : undefined,
                            });
                        }
                    }
