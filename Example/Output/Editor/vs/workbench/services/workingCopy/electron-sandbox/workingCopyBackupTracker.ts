...
import { ConfirmResult, getFileNamesMessage, IDialogService, IFileDialogService, } from "../../../../platform/dialogs/common/dialogs.js";
...
import { AutoSaveMode, IFilesConfigurationService, } from "../../filesConfiguration/common/filesConfigurationService.js";
...
export class NativeWorkingCopyBackupTracker extends WorkingCopyBackupTracker implements IWorkbenchContribution {
    ...
    protected async onBeforeShutdownWithModified(reason: ShutdownReason, modifiedWorkingCopies: readonly IWorkingCopy[]): Promise<boolean> {
        ...
        const workingCopiesToAutoSave = modifiedWorkingCopies.filter((wc) => !(wc.capabilities & WorkingCopyCapabilities.Untitled) &&
        ...
        switch (reason) {
            ...
            case ShutdownReason.CLOSE:
                ...
                if (isMacintosh ||
                    ...
                    if (this.contextService.getWorkbenchState() !==
                        WorkbenchState.EMPTY) {
                        return modifiedWorkingCopies.filter((modifiedWorkingCopy) => modifiedWorkingCopy.capabilities &
                            WorkingCopyCapabilities.Scratchpad); // backup scratchpads automatically to avoid user confirmation
                    }
    ...
    private async backupBeforeShutdown(modifiedWorkingCopies: readonly IWorkingCopy[]): Promise<{
    ...
    private doSaveAllBeforeShutdown(workingCopies: IWorkingCopy[], reason: SaveReason): Promise<void> {
        return this.withProgressAndCancellation(async () => {
            ...
            if (result !== false) {
                await Promises.settled(workingCopies.map((workingCopy) => workingCopy.isModified()
        ...
        workingCopies.some((workingCopy) => workingCopy.capabilities &
    ...
    private doRevertAllBeforeShutdown(modifiedWorkingCopies: IWorkingCopy[]): Promise<void> {
        return this.withProgressAndCancellation(async () => {
            ...
            await Promises.settled(modifiedWorkingCopies.map((workingCopy) => workingCopy.isModified()
    ...
    private onBeforeShutdownWithoutModified(): Promise<boolean> {
        ...
        return this.noVeto({
            except: this.contextService.getWorkbenchState() === WorkbenchState.EMPTY
                ? []
    ...
    private async noVeto(arg1: IWorkingCopyIdentifier[] | {
        except: IWorkingCopyIdentifier[];
    }): Promise<boolean> {
    ...
    private discardBackupsBeforeShutdown(backupsToDiscardOrKeep: IWorkingCopyIdentifier[] | {
        except: IWorkingCopyIdentifier[];
    }): Promise<void>;
    private async discardBackupsBeforeShutdown(arg1: IWorkingCopyIdentifier[] | {
        except: IWorkingCopyIdentifier[];
    }): Promise<void> {
