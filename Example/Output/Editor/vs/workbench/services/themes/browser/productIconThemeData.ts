...
import { getIconRegistry, IconContribution, IconDefinition, IconFontDefinition, IconFontSource, } from "../../../../platform/theme/common/iconRegistry.js";
import { fontFormatRegex, fontIdRegex, fontStyleRegex, fontWeightRegex, } from "../common/productIconThemeSchema.js";
...
export class ProductIconThemeData implements IWorkbenchProductIconTheme {
    ...
    static get defaultTheme(): ProductIconThemeData {
        ...
        if (!themeData) {
            themeData = ProductIconThemeData._defaultProductIconTheme =
                new ProductIconThemeData(DEFAULT_PRODUCT_ICON_THEME_ID, nls.localize("defaultTheme", "Default"), ThemeSettingDefaults.PRODUCT_ICON_THEME);
    ...
    static fromStorageData(storageService: IStorageService): ProductIconThemeData | undefined {
        ...
        try {
            ...
            if (Array.isArray(iconDefinitions) &&
...
function _loadProductIconThemeDocument(fileService: IExtensionResourceLoaderService, location: URI, warnings: string[]): Promise<ProductIconThemeDocument> {
    return fileService.readExtensionResource(location).then((content) => {
        ...
        if (parseErrors.length > 0) {
            return Promise.reject(new Error(nls.localize("error.cannotparseicontheme", "Problems parsing product icons file: {0}", parseErrors
