...
import { IColorTheme, IThemingRegistry, Extensions as ThemingExtensions, } from "../../../../platform/theme/common/themeService.js";
...
import { COLOR_THEME_DARK_INITIAL_COLORS, COLOR_THEME_LIGHT_INITIAL_COLORS, ExtensionData, IWorkbenchColorTheme, IWorkbenchFileIconTheme, IWorkbenchProductIconTheme, IWorkbenchThemeService, ThemeSettingDefaults, ThemeSettings, ThemeSettingTarget, VS_DARK_THEME, VS_HC_LIGHT_THEME, VS_HC_THEME, VS_LIGHT_THEME, } from "../common/workbenchThemeService.js";
import { FileIconThemeData, FileIconThemeLoader } from "./fileIconThemeData.js";
import { DEFAULT_PRODUCT_ICON_THEME_ID, ProductIconThemeData, } from "./productIconThemeData.js";
// implementation
...
export class WorkbenchThemeService extends Disposable implements IWorkbenchThemeService {
    ...
    private installRegistryListeners(): Promise<any> {
        ...
        this._register(this.colorThemeRegistry.onDidChange(async (event) => {
            ...
            if (await this.restoreColorTheme()) {
                ...
                // restore theme
                if (this.currentColorTheme.settingsId ===
                    ThemeSettingDefaults.COLOR_THEME_DARK &&
                ...
                else if (event.added.some((t) => t.settingsId ===
        ...
        this._register(this._register(this.fileIconThemeRegistry.onDidChange(async (event) => {
            ...
            if (await this.restoreFileIconTheme()) {
                // checks if theme from settings exists and is set
                // restore theme
                if (this.currentFileIconTheme.id ===
            ...
            else if (event.removed.some((t) => t.settingsId ===
                this.currentFileIconTheme.settingsId)) {
                // current theme is no longer available
        ...
        this._register(this.productIconThemeRegistry.onDidChange(async (event) => {
            ...
            if (await this.restoreProductIconTheme()) {
                // checks if theme from settings exists and is set
                // restore theme
                if (this.currentProductIconTheme.id ===
    ...
    private applyTheme(newTheme: ColorThemeData, settingsTarget: ThemeSettingTarget, silent = false): Promise<IWorkbenchColorTheme | null> {
        ...
        if (!this.colorThemingParticipantChangeListener) {
            this.colorThemingParticipantChangeListener =
                themingRegistry.onThemingParticipantAdded((_) => this.updateDynamicCSSRules(this.currentColorTheme));
        }
    ...
    private async internalSetFileIconTheme(iconThemeOrId: string | undefined | IWorkbenchFileIconTheme, settingsTarget: ThemeSettingTarget): Promise<IWorkbenchFileIconTheme> {
        ...
        const themeId = types.isString(iconThemeOrId)
            ? iconThemeOrId
            : iconThemeOrId.id;
        if (themeId !== this.currentFileIconTheme.id ||
    ...
    private async internalSetProductIconTheme(iconThemeOrId: string | undefined | IWorkbenchProductIconTheme, settingsTarget: ThemeSettingTarget): Promise<IWorkbenchProductIconTheme> {
        ...
        const themeId = types.isString(iconThemeOrId)
            ? iconThemeOrId
            : iconThemeOrId.id;
        if (themeId !== this.currentProductIconTheme.id ||
