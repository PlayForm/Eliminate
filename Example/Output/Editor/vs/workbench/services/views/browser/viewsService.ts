...
import { PaneComposite, PaneCompositeDescriptor, Extensions as PaneCompositeExtensions, PaneCompositeRegistry, } from "../../../browser/panecomposite.js";
...
import { IView, IViewDescriptor, IViewDescriptorService, IViewPaneContainer, ViewContainer, ViewContainerLocation, } from "../../../common/views.js";
...
export class ViewsService extends Disposable implements IViewsService {
    ...
    private readonly _onDidChangeViewContainerVisibility = this._register(new Emitter<{
        id: string;
        visible: boolean;
    ...
    private onDidChangeContainers(added: ReadonlyArray<{
    ...
    private getComposite(compositeId: string, location: ViewContainerLocation): {
        id: string;
        name: string;
    } | undefined {
    ...
    isViewContainerVisible(id: string): boolean {
        ...
        return (this.paneCompositeService
    ...
    private registerOpenViewContainerAction(viewContainer: ViewContainer): IDisposable {
        ...
        if (viewContainer.openCommandActionDescriptor) {
            ...
            const title = viewContainer.openCommandActionDescriptor.title ??
                viewContainer.title;
            const that = this;
                    ...
                    super({
                        ...
                        get title(): ICommandActionTitle {
                            ...
                            const localizedTitle = typeof title === "string"
                                ? title
                                : title.value;
                            const originalTitle = typeof title === "string"
                                ? title
                                : title.original;
                ...
                public async run(serviceAccessor: ServicesAccessor): Promise<any> {
                    ...
                    switch (viewContainerLocation) {
                        ...
                        case ViewContainerLocation.Sidebar: {
                            const part = viewContainerLocation ===
                                ViewContainerLocation.Sidebar
                                ? Parts.SIDEBAR_PART
            ...
            if (mnemonicTitle) {
                ...
                disposables.add(MenuRegistry.appendMenuItem(MenuId.MenubarViewMenu, {
                    ...
                    group: defaultLocation === ViewContainerLocation.Sidebar
                        ? "3_views"
                        : "4_panels",
    ...
    private registerOpenViewAction(viewDescriptor: IViewDescriptor): IDisposable {
        ...
        if (viewDescriptor.openCommandActionDescriptor) {
            const title = viewDescriptor.openCommandActionDescriptor.title ??
                viewDescriptor.name;
            ...
            if (viewDescriptor.openCommandActionDescriptor.mnemonicTitle) {
                ...
                if (defaultViewContainer) {
                    ...
                    disposables.add(MenuRegistry.appendMenuItem(MenuId.MenubarViewMenu, {
                        ...
                        order: viewDescriptor.openCommandActionDescriptor
    ...
    private registerFocusViewAction(viewDescriptor: IViewDescriptor, category?: string | ILocalizedString): IDisposable {
        return registerAction2(class FocusViewAction extends Action2 {
            constructor() {
                ...
                }, "Focus on {0} View", viewDescriptor.name.value);
                super({
                    ...
                    keybinding: {
                        when: ContextKeyExpr.has(`${viewDescriptor.id}.active`),
                        weight: KeybindingWeight.WorkbenchContrib,
                        primary: viewDescriptor.focusCommand?.keybindings
                            ?.primary,
                        secondary: viewDescriptor.focusCommand?.keybindings
                            ?.secondary,
    ...
    private registerResetViewLocationAction(viewDescriptor: IViewDescriptor): IDisposable {
        return registerAction2(class ResetViewLocationAction extends Action2 {
            ...
            run(accessor: ServicesAccessor): void {
                ...
                if (defaultContainer.hideIfEmpty &&
    ...
    private registerPaneComposite(viewContainer: ViewContainer, viewContainerLocation: ViewContainerLocation): void {
        ...
        Registry.as<PaneCompositeRegistry>(getPaneCompositeExtension(viewContainerLocation)).registerPaneComposite(PaneCompositeDescriptor.create(PaneContainer, viewContainer.id, typeof viewContainer.title === "string"
            ? viewContainer.title
            : viewContainer.title.value, isString(viewContainer.icon) ? viewContainer.icon : undefined, viewContainer.order, viewContainer.requestedIndex, viewContainer.icon instanceof URI
            ? viewContainer.icon
            : undefined));
    }
