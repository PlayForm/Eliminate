...
import { defaultViewIcon, IViewContainersRegistry, IViewDescriptor, IViewDescriptorService, IViewsRegistry, ViewContainer, ViewContainerLocation, ViewContainerLocationToString, Extensions as ViewExtensions, VIEWS_LOG_ID, VIEWS_LOG_NAME, ViewVisibilityState, } from "../../../common/views.js";
...
export class ViewDescriptorService extends Disposable implements IViewDescriptorService {
    ...
    private readonly viewContainerModels = this._register(new DisposableMap<ViewContainer, {
    ...
    private readonly _onDidChangeViewContainers = this._register(new Emitter<{
        added: ReadonlyArray<{
    ...
    loggerService: ILoggerService) {
        super();
        this.logger = new Lazy(() => loggerService.createLogger(VIEWS_LOG_ID, {
            name: VIEWS_LOG_NAME,
            hidden: true,
        }));
    ...
    private migrateToViewsCustomizationsStorage(): void {
        ...
        const viewsCustomizations: IViewsCustomizations = {
            viewContainerLocations: viewContainerLocations.reduce<IStringDictionary<ViewContainerLocation>>((result, [id, location]) => {
                result[id] = location;
                return result;
            }, {}),
    ...
    private registerGroupedViews(groupedViews: Map<string, IViewDescriptor[]>): void {
        for (const [containerId, views] of groupedViews.entries()) {
            ...
            const viewsToAdd = views.filter((view) => this.getViewContainerModel(viewContainer).allViewDescriptors.filter((vd) => vd.id === view.id)
                .length === 0);
    ...
    private onDidRegisterViews(views: {
        views: IViewDescriptor[];
        viewContainer: ViewContainer;
    }[]): void {
    ...
    private regroupViews(containerId: string, views: IViewDescriptor[]): Map<string, IViewDescriptor[]> {
        ...
        for (const viewDescriptor of views) {
            const correctContainerId = this.viewDescriptorsCustomLocations.get(viewDescriptor.id) ??
                containerId;
    ...
    private reportMovedViews(views: IViewDescriptor[], from: ViewContainer, to: ViewContainer): void {
        ...
        this.telemetryService.publicLog2<ViewDescriptorServiceMoveViewsEvent, ViewDescriptorServiceMoveViewsClassification>("viewDescriptorService.moveViews", {
            viewCount,
            fromContainer,
            toContainer,
            fromLocation,
    ...
    private cleanUpGeneratedViewContainer(viewContainerId: string): void {
        ...
        this.storageService.remove(getViewsStateStorageId(viewContainer?.storageId ||
            getViewContainerStorageId(viewContainerId)), StorageScope.PROFILE);
    }
    private registerGeneratedViewContainer(location: ViewContainerLocation, existingId?: string): ViewContainer {
        ...
        const container = this.viewContainersRegistry.registerViewContainer({
            ...
            icon: location === ViewContainerLocation.Sidebar
                ? defaultViewIcon
                : undefined,
    ...
    private onDidChangeActiveViews({ added, removed, }: {
    ...
    private onDidChangeVisibleViews({ added, removed, }: {
    ...
    private registerViewsVisibilityActions(viewContainer: ViewContainer, { viewContainerModel, disposables, }: {
        viewContainerModel: ViewContainerModel;
        disposables: DisposableStore;
    }): void {
        this.viewsVisibilityActionDisposables.deleteAndDispose(viewContainer);
                    ...
                    constructor() {
                        super({
                            ...
                            menu: [
                                {
                                    id: ViewsSubMenu,
                                    when: ContextKeyExpr.equals("viewContainer", viewContainerModel
                                        .viewContainer.id),
                                    order: index,
                                },
                                {
                                    id: MenuId.ViewContainerTitleContext,
                                    when: ContextKeyExpr.and(ContextKeyExpr.equals("viewContainer", viewContainerModel
                                        .viewContainer.id)),
                                    order: index,
    ...
    private addViews(container: ViewContainer, views: IViewDescriptor[], visibilityState: ViewVisibilityState = ViewVisibilityState.Default): void {
        ...
        this.getViewContainerModel(container).add(views.map((view) => {
            return {
                viewDescriptor: view,
                collapsed: visibilityState === ViewVisibilityState.Default
                    ? undefined
                    : false,
                visible: visibilityState === ViewVisibilityState.Default
                    ? undefined
                    : true,
            };
        }));
    ...
    private removeViews(container: ViewContainer, views: IViewDescriptor[]): void {
        ...
        this.contextKeyService.bufferChangeEvents(() => {
            views.forEach((view) => {
                if (this.viewDescriptorsCustomLocations.get(view.id) ===
                    container.id) {
                    this.viewDescriptorsCustomLocations.delete(view.id);
                }
