...
import { defaultViewIcon, IAddedViewDescriptorRef, IAddedViewDescriptorState, IViewContainerModel, IViewDescriptor, IViewDescriptorRef, IViewsRegistry, ViewContainer, Extensions as ViewExtensions, VIEWS_LOG_ID, VIEWS_LOG_NAME, } from "../../../common/views.js";
...
class ViewDescriptorsState extends Disposable {
    ...
    private _onDidChangeStoredState = this._register(new Emitter<{
        id: string;
        visible: boolean;
    }[]>());
    readonly onDidChangeStoredState = this._onDidChangeStoredState.event;
    private readonly logger: Lazy<ILogger>;
    constructor(viewContainerStorageId: string, private readonly viewContainerName: string, 
    @IStorageService
        ...
        for (const viewDescriptor of viewDescriptors) {
            ...
            if (viewState) {
                storedViewsStates[viewDescriptor.id] = {
                    ...
                    size: viewState.size,
                    order: viewDescriptor.workspace && viewState
                        ? viewState.order
                        : undefined,
                };
            }
    ...
    private updateGlobalState(viewDescriptors: ReadonlyArray<IViewDescriptor>): void {
        ...
        for (const viewDescriptor of viewDescriptors) {
            ...
            storedGlobalState.set(viewDescriptor.id, {
                ...
                isHidden: state && viewDescriptor.canToggleVisibility
                    ? !state.visibleGlobal
                    : false,
                order: !viewDescriptor.workspace && state
                    ? state.order
                    : undefined,
            });
        }
...
export class ViewContainerModel extends Disposable implements IViewContainerModel {
    ...
    private _onDidChangeContainerInfo = this._register(new Emitter<{
        title?: boolean;
        icon?: boolean;
    ...
    private _onDidChangeAllViewDescriptors = this._register(new Emitter<{
    ...
    private _onDidChangeActiveViewDescriptors = this._register(new Emitter<{
        added: ReadonlyArray<IViewDescriptor>;
        removed: ReadonlyArray<IViewDescriptor>;
    }>());
    ...
    private _onDidMoveVisibleViewDescriptors = this._register(new Emitter<{
    ...
    private updateContainerInfo(): void {
        ...
        const useDefaultContainerInfo = this.viewContainer.alwaysUseContainerInfo ||
            ...
            this.visibleViewDescriptors.some((v) => Registry.as<IViewsRegistry>(ViewExtensions.ViewsRegistry).getViewContainer(v.id) === this.viewContainer);
        ...
        const keybindingId = this.viewContainer.openCommandActionDescriptor?.id ??
            this.activeViewDescriptors.find((v) => v.openCommandActionDescriptor)?.openCommandActionDescriptor?.id;
    ...
    private isEqualIcon(icon: URI | ThemeIcon | undefined): boolean {
        if (URI.isUri(icon)) {
            ...
            return (ThemeIcon.isThemeIcon(this._icon) &&
    ...
    private updateVisibility(viewDescriptors: {
        id: string;
        visible: boolean;
    }[]): void {
    ...
    private updateViewDescriptorItemVisibility(viewDescriptorItem: IViewDescriptorItem, visible: boolean): boolean {
        ...
        if (this.isViewDescriptorVisibleWhenActive(viewDescriptorItem) ===
            visible) {
            return false;
        }
        // update visibility
