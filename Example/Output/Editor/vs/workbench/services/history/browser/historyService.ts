 ...
 *--------------------------------------------------------------------------------------------*/
import { addDisposableListener, EventHelper, EventType, WindowIdleValue, } from "../../../../base/browser/dom.js";
...
import { FileChangesEvent, FileChangeType, FileOperation, FileOperationEvent, FILES_EXCLUDE_CONFIG, IFileService, } from "../../../../platform/files/common/files.js";
...
import { EditorCloseContext, EditorPaneSelectionChangeReason, EditorPaneSelectionCompareResult, EditorResourceAccessor, EditorsOrder, GroupIdentifier, GroupModelChangeKind, IEditorCloseEvent, IEditorIdentifier, IEditorPane, IEditorPaneSelection, IEditorPaneSelectionChangeEvent, IEditorPaneWithSelection, IEditorWillMoveEvent, isEditorInput, isEditorPaneWithSelection, isResourceEditorInput, isSideBySideEditorInput, IUntypedEditorInput, SideBySideEditor, } from "../../../common/editor.js";
...
export class HistoryService extends Disposable implements IHistoryService {
    ...
    private onDidActiveEditorChange(): void {
        ...
        if (!activeEditorPane?.group.isTransient(activeEditorPane.input)) {
            ...
            const transientListener = activeEditorGroup.onDidModelChange((e) => {
                if (e.kind === GroupModelChangeKind.EDITOR_TRANSIENT &&
    ...
    private onDidFilesChange(event: FileChangesEvent | FileOperationEvent): void {
        ...
        // Internal file changes (e.g. explorer)
            ...
            // Move
            else if (event.isOperation(FileOperation.MOVE) &&
                event.target.isFile) {
                this.move(event);
            }
        }
    ...
    private readonly editorGroupScopedNavigationStacks = new Map<GroupIdentifier, {
        stack: IEditorNavigationStacks;
        disposable: IDisposable;
    }>();
    private readonly editorScopedNavigationStacks = new Map<GroupIdentifier, Map<EditorInput, {
        stack: IEditorNavigationStacks;
        disposable: IDisposable;
    }>>();
    ...
    private getStack(group = this.editorGroupService.activeGroup, editor = group.activeEditor): IEditorNavigationStacks {
        switch (this.editorNavigationScope) {
            ...
            case GoScope.EDITOR: {
                ...
                if (!stacksForGroup) {
                    stacksForGroup = new Map<EditorInput, {
                        stack: IEditorNavigationStacks;
                        disposable: IDisposable;
                    }>();
    ...
    private async doNavigateInRecentlyUsedEditorsStack(editorIdentifier: IEditorIdentifier | undefined, groupId?: GroupIdentifier): Promise<void> {
        if (editorIdentifier) {
            const acrossGroups = typeof groupId !== "number" ||
    ...
    private ensureRecentlyUsedStack(indexModifier: (index: number) => number, groupId?: GroupIdentifier): [
        readonly IEditorIdentifier[],
        number
    ] {
        let editors: readonly IEditorIdentifier[];
        let index: number;
        const group = typeof groupId === "number"
            ? this.editorGroupService.getGroup(groupId)
            : undefined;
        // Across groups
        if (!group) {
    ...
    private handleEditorCloseEventInReopen(event: IEditorCloseEvent): void {
        ...
        // Bounding
        if (this.recentlyClosedEditors.length >
    ...
    private readonly resourceExcludeMatcher = this._register(new WindowIdleValue(mainWindow, () => {
        const matcher = this._register(this.instantiationService.createInstance(ResourceGlobMatcher, (root) => getExcludes(root
            ? this.configurationService.getValue<ISearchConfiguration>({ resource: root })
            : this.configurationService.getValue<ISearchConfiguration>()) || Object.create(null), (event) => event.affectsConfiguration(FILES_EXCLUDE_CONFIG) ||
...
class EditorSelectionState {
    constructor(private readonly editorIdentifier: IEditorIdentifier, readonly selection: IEditorPaneSelection | undefined, private readonly reason: EditorPaneSelectionChangeReason | undefined) { }
    justifiesNewNavigationEntry(other: EditorSelectionState): boolean {
        ...
        if (result === EditorPaneSelectionCompareResult.SIMILAR &&
...
class EditorNavigationStacks extends Disposable implements IEditorNavigationStacks {
    ...
    readonly onDidChange = Event.any(this.selectionsStack.onDidChange, this.editsStack.onDidChange, this.navigationsStack.onDidChange);
    constructor(private readonly scope: GoScope, 
    @IInstantiationService
    private readonly instantiationService: IInstantiationService) {
        super();
    }
    ...
    handleActiveEditorSelectionChange(editorPane: IEditorPaneWithSelection, event: IEditorPaneSelectionChangeEvent): void {
        ...
        // the navigation stack.
        else if ((event.reason === EditorPaneSelectionChangeReason.NAVIGATION ||
            ...
            if (event.reason === EditorPaneSelectionChangeReason.JUMP &&
...
export class EditorNavigationStack extends Disposable {
    ...
    constructor(private readonly filter: GoFilter, private readonly scope: GoScope, 
    @IInstantiationService
    private readonly instantiationService: IInstantiationService, 
    @IEditorService
