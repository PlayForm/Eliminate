...
import { Chord, Keybinding, KeyCodeChord, ResolvedKeybinding, ScanCodeChord, SingleModifierChord, } from "../../../../base/common/keybindings.js";
import { IMMUTABLE_CODE_TO_KEY_CODE, IMMUTABLE_KEY_CODE_TO_CODE, KeyCode, KeyCodeUtils, ScanCode, ScanCodeUtils, } from "../../../../base/common/keyCodes.js";
...
export class NativeResolvedKeybinding extends BaseResolvedKeybinding<ScanCodeChord> {
    ...
    protected _getSingleModifierChordDispatch(chord: ScanCodeChord): SingleModifierChord | null {
        if ((chord.scanCode === ScanCode.ControlLeft ||
        ...
        if ((chord.scanCode === ScanCode.AltLeft ||
        ...
        if ((chord.scanCode === ScanCode.ShiftLeft ||
        ...
        if ((chord.scanCode === ScanCode.MetaLeft ||
...
class ScanCodeCombo {
    ...
    public getProducedChar(mapping: IMacLinuxKeyMapping): string {
        ...
        if (charCode >= CharCode.U_Combining_Grave_Accent &&
...
class ScanCodeKeyCodeMapper {
    ...
    public registerIfUnknown(scanCodeCombo: ScanCodeCombo, keyCodeCombo: KeyCodeCombo): void {
        ...
        const keyCodeIsDigit = keyCodeCombo.keyCode >= KeyCode.Digit0 &&
            keyCodeCombo.keyCode <= KeyCode.Digit9;
        const keyCodeIsLetter = keyCodeCombo.keyCode >= KeyCode.KeyA &&
...
export class MacLinuxKeyboardMapper implements IKeyboardMapper {
    ...
    constructor(private readonly _isUSStandard: boolean, rawMappings: IMacLinuxKeyboardMapping, private readonly _mapAltGrToCtrlAlt: boolean, private readonly _OS: OperatingSystem) {
        this._codeInfo = [];
        this._scanCodeKeyCodeMapper = new ScanCodeKeyCodeMapper();
        this._scanCodeToLabel = [];
        this._scanCodeToDispatch = [];
        const _registerIfUnknown = (hwCtrlKey: 0 | 1, hwShiftKey: 0 | 1, hwAltKey: 0 | 1, scanCode: ScanCode, kbCtrlKey: 0 | 1, kbShiftKey: 0 | 1, kbAltKey: 0 | 1, keyCode: KeyCode): void => {
        ...
        for (const strScanCode in rawMappings) {
            if (rawMappings.hasOwnProperty(strScanCode)) {
                ...
                const rawMapping = missingLatinLettersOverride[strScanCode] ||
        ...
        for (let i = mappings.length - 1; i >= 0; i--) {
            ...
            if (withAltGr === mapping.withShift ||
    ...
    public keyCodeChordToScanCodeChord(chord: KeyCodeChord): ScanCodeChord[] {
        ...
        if (chord.keyCode === KeyCode.Enter) {
            return [
                new ScanCodeChord(chord.ctrlKey, chord.shiftKey, chord.altKey, chord.metaKey, ScanCode.Enter),
            ];
        }
    ...
    private static _charCodeToKb(charCode: number): {
        keyCode: KeyCode;
        shiftKey: boolean;
    } | null {
