...
class UserKeybindings extends Disposable {
    ...
    constructor(private readonly userDataProfileService: IUserDataProfileService, private readonly uriIdentityService: IUriIdentityService, private readonly fileService: IFileService, logService: ILogService) {
        super();
        this.watch();
        ...
        this._register(Event.filter(this.fileService.onDidFilesChange, (e) => e.contains(this.userDataProfileService.currentProfile
            .keybindingsResource))(() => {
            logService.debug("Keybindings file changed");
            this.reloadConfigurationScheduler.schedule();
        }));
        this._register(this.fileService.onDidRunOperation((e) => {
            if (e.operation === FileOperation.WRITE &&
                e.resource.toString() ===
    ...
    private watch(): void {
        ...
        this.watchDisposables.add(this.fileService.watch(dirname(this.userDataProfileService.currentProfile
            .keybindingsResource)));
    ...
    updateSchema(additionalContributions: readonly IJSONSchema[]) {
        ...
        const addKnownCommand = (commandId: string, description?: string | ILocalizedString | undefined) => {
            if (!/^_/.test(commandId)) {
                if (!knownCommands.has(commandId)) {
                    ...
                    this.commandsEnumDescriptions.push(isLocalizedString(description)
                        ? description.value
                        : description);
        ...
        for (const [commandId, command] of allCommands) {
            ...
            addKnownCommand(commandId, commandMetadata?.description ??
                MenuRegistry.getCommand(commandId)?.title);
            if (!commandMetadata ||
