...
import { FileKind, IFileService, IFileStat, IFileStatWithPartialMetadata, } from "../../../../platform/files/common/files.js";
import { IKeybindingService } from "../../../../platform/keybinding/common/keybinding.js";
import { ILabelService } from "../../../../platform/label/common/label.js";
import { INotificationService } from "../../../../platform/notification/common/notification.js";
import { IQuickInputService, IQuickPick, IQuickPickItem, ItemActivation, } from "../../../../platform/quickinput/common/quickInput.js";
...
export class SimpleFileDialog extends Disposable implements ISimpleFileDialog {
    ...
    private remoteUriFrom(path: string, hintUri?: URI): URI {
        ...
        const uri: URI = this.scheme === Schemas.file
            ? URI.file(path)
            : URI.from({
                scheme: this.scheme,
                path,
        ...
        const authority = uri.scheme === Schemas.file
            ? undefined
    ...
    private async pickResource(isSave: boolean = false): Promise<URI | undefined> {
        ...
        return new Promise<URI | undefined>((resolve) => {
            ...
            this._register(this.filePickBox.onDidCustom(() => {
                ...
                isResolving++;
                if (this.options.availableFileSystems &&
                    this.options.availableFileSystems.length > 1) {
                    this.options.availableFileSystems =
            ...
            if (!newFolderIsOldFolder &&
                ...
                if (stat &&
                    ...
                    if (!resources.extUriIgnorePathCase.isEqual(currentFolderWithoutSep, inputUriDirnameWithoutSep) &&
                        ...
                        if (statWithoutTrailing &&
                            statWithoutTrailing.isDirectory) {
                            this.badPath = undefined;
                            inputUriDirname =
                                this.tryAddTrailingSeparatorToDirectory(inputUriDirname, statWithoutTrailing);
    ...
    private addPostfix(uri: URI): URI {
        ...
        if (this.requiresTrailing &&
            ...
            if (!hasExt) {
                result = resources.joinPath(resources.dirname(uri), resources.basename(uri) +
                    "." +
    ...
    private async createItems(folder: IFileStat | undefined, currentFolder: URI, token: CancellationToken): Promise<FileQuickPickItem[]> {
        ...
        try {
            ...
            const items = folder.children
                ? await Promise.all(folder.children.map((child) => this.createItem(child, currentFolder, token)))
                : [];
