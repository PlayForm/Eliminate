...
import { FileFilter, IFileDialogService, IOpenDialogOptions, IPickAndOpenOptions, IPromptButton, ISaveDialogOptions, } from "../../../../platform/dialogs/common/dialogs.js";
...
export class FileDialogService extends AbstractFileDialogService implements IFileDialogService {
    ...
    private getFilePickerTypes(filters?: FileFilter[]): FilePickerAcceptType[] | undefined {
        return filters
            ...
            const extensions = filter.extensions.filter((ext) => ext.indexOf("-") < 0 &&
                ext.indexOf("*") < 0 &&
                ext.indexOf("_") < 0);
            accept[getMediaOrTextMime(`fileName.${filter.extensions[0]}`) ??
    ...
    async showOpenDialog(options: IOpenDialogOptions): Promise<URI[] | undefined> {
        ...
        try {
            if (options.canSelectFiles) {
                ...
                uri =
                    await this.fileSystemProvider.registerDirectoryHandle(handle);
            }
        }
        catch (error) {
