...
import { equals as arrayEquals, delta, distinct, } from "../../../../base/common/arrays.js";
import { Barrier, Delayer, Promises, Queue, } from "../../../../base/common/async.js";
...
import { ConfigurationTarget, ConfigurationTargetToString, IConfigurationChange, IConfigurationChangeEvent, IConfigurationData, IConfigurationOverrides, IConfigurationService, IConfigurationUpdateOptions, IConfigurationUpdateOverrides, IConfigurationValue, isConfigurationOverrides, isConfigurationUpdateOverrides, } from "../../../../platform/configuration/common/configuration.js";
...
import { allSettings, applicationSettings, configurationDefaultsSchemaId, ConfigurationScope, Extensions, IConfigurationPropertySchema, IConfigurationRegistry, keyFromOverrideIdentifiers, machineOverridableSettings, machineSettings, OVERRIDE_PROPERTY_PATTERN, resourceLanguageSettingsSchemaId, resourceSettings, windowSettings, } from "../../../../platform/configuration/common/configurationRegistry.js";
...
import { Workspace as BaseWorkspace, IAnyWorkspaceIdentifier, IEmptyWorkspaceIdentifier, ISingleFolderWorkspaceIdentifier, isSingleFolderWorkspaceIdentifier, isWorkspaceFolder, isWorkspaceIdentifier, IWorkspaceContextService, IWorkspaceFolder, IWorkspaceFoldersChangeEvent, IWorkspaceFoldersWillChangeEvent, IWorkspaceIdentifier, toWorkspaceFolder, WorkbenchState, WorkspaceFolder, } from "../../../../platform/workspace/common/workspace.js";
import { IWorkspaceTrustManagementService } from "../../../../platform/workspace/common/workspaceTrust.js";
import { getStoredWorkspaceFolder, isStoredWorkspaceFolder, IStoredWorkspaceFolder, IWorkspaceFolderCreationData, toWorkspaceFolders, } from "../../../../platform/workspaces/common/workspaces.js";
...
import { APPLY_ALL_PROFILES_SETTING, defaultSettingsSchemaId, FOLDER_CONFIG_FOLDER_NAME, folderSettingsSchemaId, IConfigurationCache, IWorkbenchConfigurationService, LOCAL_MACHINE_PROFILE_SCOPES, LOCAL_MACHINE_SCOPES, machineSettingsSchemaId, PROFILE_SCOPES, profileSettingsSchemaId, RestrictedSettings, userSettingsSchemaId, workspaceSettingsSchemaId, } from "../common/configuration.js";
...
import { ApplicationConfiguration, DefaultConfiguration, FolderConfiguration, RemoteUserConfiguration, UserConfiguration, WorkspaceConfiguration, } from "./configuration.js";
...
export class WorkspaceService extends Disposable implements IWorkbenchConfigurationService, IWorkspaceContextService {
    ...
    constructor({ remoteAuthority, configurationCache, }: {
    ...
    }, environmentService: IBrowserWorkbenchEnvironmentService, private readonly userDataProfileService: IUserDataProfileService, private readonly userDataProfilesService: IUserDataProfilesService, private readonly fileService: IFileService, private readonly remoteAgentService: IRemoteAgentService, private readonly uriIdentityService: IUriIdentityService, private readonly logService: ILogService, policyService: IPolicyService) {
        super();
        ...
        if (remoteAuthority) {
            const remoteUserConfiguration = (this.remoteUserConfiguration =
                this._register(new RemoteUserConfiguration(remoteAuthority, configurationCache, fileService, uriIdentityService, remoteAgentService, logService)));
    ...
    private createApplicationConfiguration(): void {
        ...
        if (this.userDataProfileService.currentProfile.isDefault ||
            ...
            this.applicationConfiguration =
                this.applicationConfigurationDisposables.add(this._register(new ApplicationConfiguration(this.userDataProfilesService, this.fileService, this.uriIdentityService, this.logService)));
    ...
    public isCurrentWorkspace(workspaceIdOrFolder: IWorkspaceIdentifier | ISingleFolderWorkspaceIdentifier | URI): boolean {
        switch (this.getWorkbenchState()) {
            ...
            case WorkbenchState.WORKSPACE:
                return (isWorkspaceIdentifier(workspaceIdOrFolder) &&
        ...
        if (this.isWorkspaceTrusted !== trusted) {
            ...
            for (const folder of this.workspace.folders) {
                ...
                if (folderConfiguration) {
                    configurationModel =
                        folderConfiguration.updateWorkspaceTrust(this.isWorkspaceTrusted);
    ...
    isSettingAppliedForAllProfiles(key: string): boolean {
        if (this.configurationRegistry.getConfigurationProperties()[key]
    ...
    private compareFolders(currentFolders: IWorkspaceFolder[], newFolders: IWorkspaceFolder[]): IWorkspaceFoldersChangeEvent {
        ...
        result.added = newFolders.filter((newFolder) => !currentFolders.some((currentFolder) => newFolder.uri.toString() ===
        ...
        for (let currentIndex = 0; currentIndex < currentFolders.length; currentIndex++) {
            ...
            for (newIndex = 0; newIndex < newFolders.length &&
                currentFolder.uri.toString() !==
                    newFolders[newIndex].uri.toString(); newIndex++) { }
    ...
    private async loadConfiguration(applicationConfigurationModel: ConfigurationModel, userConfigurationModel: ConfigurationModel, remoteUserConfigurationModel: ConfigurationModel, trigger: boolean): Promise<void> {
        ...
        if (trigger) {
            ...
            this.triggerConfigurationChange(change, {
    ...
    private updateRestrictedSettings(): void {
        ...
        const workspace = this.getWorkbenchState() === WorkbenchState.WORKSPACE
    ...
    private async writeConfigurationValue(key: string, value: any, target: ConfigurationTarget, overrides: IConfigurationUpdateOverrides | undefined, options?: IConfigurationUpdateOverrides): Promise<void> {
        ...
        if (target === ConfigurationTarget.MEMORY) {
            ...
            this.triggerConfigurationChange({
                keys: overrides?.overrideIdentifiers?.length
                    ? [
                        keyFromOverrideIdentifiers(overrides.overrideIdentifiers),
                        key,
                    ]
                    : [key],
                overrides: overrides?.overrideIdentifiers?.length
                    ? overrides.overrideIdentifiers.map((overrideIdentifier) => [
                        overrideIdentifier,
                        [key],
                    ])
                    : [],
        ...
        switch (editableConfigurationTarget) {
            ...
            case EditableConfigurationTarget.WORKSPACE_FOLDER: {
                const workspaceFolder = overrides && overrides.resource
    ...
    private async createConfigurationEditingService(instantiationService: IInstantiationService): Promise<ConfigurationEditing> {
        const remoteSettingsResource = (await this.remoteAgentService.getEnvironment())?.settingsPath ??
            null;
    ...
    private triggerConfigurationChange(change: IConfigurationChange, previous: {
        data: IConfigurationData;
        workspace?: Workspace;
    } | undefined, target: ConfigurationTarget): void {
    ...
    private toEditableConfigurationTarget(target: ConfigurationTarget, key: string): EditableConfigurationTarget | null {
        ...
        if (target === ConfigurationTarget.USER) {
            if (this.remoteUserConfiguration) {
                const scope = this.configurationRegistry.getConfigurationProperties()[key]
                    ?.scope;
                if (scope === ConfigurationScope.MACHINE ||
...
class RegisterConfigurationSchemasContribution extends Disposable implements IWorkbenchContribution {
    ...
    lifecycleService: ILifecycleService) {
        ...
        extensionService.whenInstalledExtensionsRegistered().then(() => {
            ...
            this._register(Event.any(configurationRegistry.onDidUpdateConfiguration, configurationRegistry.onDidSchemaChange, workspaceTrustManagementService.onDidChangeTrust)(() => delayer.trigger(() => this.registerConfigurationSchemas(), lifecycleService.phase === LifecyclePhase.Eventually
                ? undefined
    ...
    private registerConfigurationSchemas(): void {
        ...
        const folderSettingsSchema: IJSONSchema = WorkbenchState.WORKSPACE ===
    ...
    extensionService: IExtensionService) {
        ...
        if (configurationService.hasCachedConfigurationDefaultsOverrides()) {
            extensionService
                ...
                .then(() => configurationService.reloadConfiguration(ConfigurationTarget.DEFAULT));
        }
    }
}
class UpdateExperimentalSettingsDefaults extends Disposable implements IWorkbenchContribution {
    ...
    private async processExperimentalSettings(properties: Iterable<string>): Promise<void> {
        ...
        for (const property of properties) {
            ...
            if (!tags ||
                (!tags.includes("experimental") && !tags.includes("onExP"))) {
                continue;
            }
