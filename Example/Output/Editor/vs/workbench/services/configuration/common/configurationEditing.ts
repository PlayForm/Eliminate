...
import { Extensions as ConfigurationExtensions, ConfigurationScope, IConfigurationRegistry, keyFromOverrideIdentifiers, OVERRIDE_PROPERTY_REGEX, } from "../../../../platform/configuration/common/configurationRegistry.js";
...
import { FOLDER_SCOPES, FOLDER_SETTINGS_PATH, IWorkbenchConfigurationService, LAUNCH_CONFIGURATION_KEY, TASKS_CONFIGURATION_KEY, TASKS_DEFAULT, USER_STANDALONE_CONFIGURATIONS, WORKSPACE_STANDALONE_CONFIGURATIONS, } from "./configuration.js";
...
export class ConfigurationEditing {
    ...
    private defaultResourceValue(resource: URI): string {
        ...
        const configurationValue: string = basename.substr(0, basename.length -
    ...
    private async validate(target: EditableConfigurationTarget, operation: IConfigurationEditOperation, checkDirty: boolean, overrides: IConfigurationUpdateOverrides): Promise<void> {
        if (this.configurationService.inspect(operation.key).policyValue !==
            undefined) {
        ...
        if (target === EditableConfigurationTarget.WORKSPACE_FOLDER) {
            ...
            if (!operation.workspaceStandAloneConfigurationKey &&
                !OVERRIDE_PROPERTY_REGEX.test(operation.key)) {
                if (configurationScope !== undefined &&
    ...
    private getConfigurationEditOperation(target: EditableConfigurationTarget, config: IConfigurationValue, overrides: IConfigurationUpdateOverrides): IConfigurationEditOperation {
        ...
        if (config.key) {
            const standaloneConfigurationMap = target === EditableConfigurationTarget.USER_LOCAL
            ...
            for (const key of standaloneConfigurationKeys) {
                ...
                // Check for prefix
                if (config.key === key) {
                    const jsonPath = this.isWorkspaceConfigurationResource(resource)
                        ? [key]
                        : [];
                    return {
