...
import { INTERACTIVE_INPUT_CURSOR_BOUNDARY, InteractiveWindowSetting, } from "../../interactive/browser/interactiveCommon.js";
...
export class ReplEditor extends EditorPane implements IEditorPaneWithScrolling {
    ...
    extensionService: IExtensionService) {
        ...
        this._register(notebookExecutionStateService.onDidChangeExecution((e) => {
            if (e.type === NotebookExecutionType.cell &&
                isEqual(e.notebook, this._notebookWidget.value?.viewModel?.notebookDocument
                    .uri)) {
    ...
    private _computeEditorOptions(): IEditorOptions {
        ...
        const editorOptions = deepClone(this._configurationService.getValue<IEditorOptions>("editor", {
            overrideIdentifier,
        }));
    ...
    override async setInput(input: ReplEditorInput, options: InteractiveEditorOptions | undefined, context: IEditorOpenContext, token: CancellationToken): Promise<void> {
        ...
        this._codeEditorWidget = this._instantiationService.createInstance(CodeEditorWidget, this._inputEditorContainer, this._editorOptions, {
            ...{
                ...
                contributions: getDefaultNotebookCreationOptions()
        ...
        if (this._lastLayoutDimensions) {
            ...
            this._notebookWidget.value!.layout(new DOM.Dimension(this._lastLayoutDimensions.dimension.width, this._lastLayoutDimensions.dimension.height -
                this.inputCellContainerHeight), this._notebookEditorContainer);
            const leftMargin = this._notebookOptions.getCellEditorContainerLeftMargin();
        ...
        this._widgetDisposableStore.add(this._codeEditorWidget.onDidChangeCursorPosition((e) => this._onDidChangeSelection.fire({
        ...
        this._widgetDisposableStore.add(this._codeEditorWidget.onDidChangeCursorPosition(({ position }) => {
            ...
            const lastLine = viewPosition.lineNumber === lastLineNumber &&
    ...
    private _scrollIfNecessary(cvm: ICellViewModel) {
        ...
        if (index === this._notebookWidget.value!.getLength() - 1) {
            ...
            if (this._configurationService.getValue<boolean>(InteractiveWindowSetting.interactiveWindowAlwaysScrollOnNewCell) ||
    ...
    private _layoutWidgets(dimension: DOM.Dimension, position: DOM.IDomPosition) {
        ...
        this._codeEditorWidget.layout(this._validateDimension(dimension.width -
            leftMargin -
            INPUT_CELL_HORIZONTAL_PADDING_RIGHT, maxHeight));
