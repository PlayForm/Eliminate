...
class LanguageDetectionStatusContribution implements IWorkbenchContribution {
    ...
    private async _doUpdate(): Promise<void> {
        ...
        const enabled = typeof enablementConfig === "object" &&
        ...
        if (disableLightbulb || !editorUri) {
            ...
            if (lang && lang !== existing && skip[existing] !== lang) {
                ...
                if (!this._combinedEntry) {
                    this._combinedEntry = this._statusBarService.addEntry(props, LanguageDetectionStatusContribution._id, StatusbarAlignment.RIGHT, {
    ...
    async run(accessor: ServicesAccessor): Promise<void> {
        ...
        if (editorUri) {
            ...
            if (lang) {
                editor
                    .getModel()
                    ?.setLanguage(lang, LanguageDetectionLanguageEventSource);
            }
            else {
