...
import { INamedProblemMatcher, isNamedProblemMatcher, ProblemMatcher, Config as ProblemMatcherConfig, ProblemMatcherParser, ProblemMatcherRegistry, } from "./problemMatcher.js";
...
namespace CommandOptions {
    const properties: IMetaData<Tasks.CommandOptions, Tasks.IShellConfiguration>[] = [
        { property: "cwd" },
        { property: "env" },
...
namespace CommandConfiguration {
    export namespace PresentationOptions {
        ...
        export function fillDefaults(value: Tasks.IPresentationOptions, context: IParseContext): Tasks.IPresentationOptions | undefined {
            const defaultEcho = context.engine === Tasks.ExecutionEngine.Terminal
                ? true
                : false;
...
export namespace ProblemMatcherConverter {
    ...
    export function fromWithOsConfig(this: void, external: IConfigurationProperties & {
        [key: string]: any;
    ...
    export function from(this: void, config: ProblemMatcherConfig.ProblemMatcherType | undefined, context: IParseContext): TaskConfigurationValueWithErrors<ProblemMatcher[]> {
        ...
        else if (kind === ProblemMatcherKind.String ||
...
export namespace GroupKind {
    export function from(this: void, external: string | IGroupKind | undefined): Tasks.TaskGroup | undefined {
        ...
        else if (Types.isString(external.kind) &&
            Tasks.TaskGroup.is(external.kind)) {
            const group: string = external.kind;
            const isDefault: boolean | string = Types.isUndefined(external.isDefault)
                ? false
                : external.isDefault;
...
namespace ConfigurationProperties {
    ...
    export function from(this: void, external: IConfigurationProperties & {
        [key: string]: any;
    }, context: IParseContext, includeCommandOptions: boolean, source: TaskConfigSource, properties?: IJSONSchemaMap): TaskConfigurationValueWithErrors<Tasks.IConfigurationProperties> {
        if (!external) {
            return {};
        }
        const result: Tasks.IConfigurationProperties & {
            [key: string]: any;
        } = {};
        if (properties) {
...
namespace ConfiguringTask {
    ...
    export function from(this: void, external: IConfiguringTask, context: IParseContext, index: number, source: TaskConfigSource, registry?: Partial<ITaskDefinitionRegistry>): Tasks.ConfiguringTask | undefined {
        ...
        const result: Tasks.ConfiguringTask = new Tasks.ConfiguringTask(`${typeDeclaration.extensionId}.${taskIdentifier._key}`, taskSource, undefined, type, taskIdentifier, RunOptions.fromConfiguration(external.runOptions), { hide: external.hide });
...
namespace CustomTask {
    export function from(this: void, external: ICustomTask, context: IParseContext, index: number, source: TaskConfigSource): Tasks.CustomTask | undefined {
        ...
        if (type !== Tasks.CUSTOMIZED_TASK_TYPE &&
            type !== "shell" &&
            type !== "process") {
        ...
        const result: Tasks.CustomTask = new Tasks.CustomTask(context.uuidMap.getUUID(taskName), taskSource, taskName, Tasks.CUSTOMIZED_TASK_TYPE, undefined, false, RunOptions.fromConfiguration(external.runOptions), {
            name: taskName,
            identifier: taskName,
        });
    ...
    export function createCustomTask(contributedTask: Tasks.ContributedTask, configuredProps: Tasks.ConfiguringTask | Tasks.CustomTask): Tasks.CustomTask {
        const result: Tasks.CustomTask = new Tasks.CustomTask(configuredProps._id, Object.assign({}, configuredProps._source, {
            customizes: contributedTask.defines,
        }), configuredProps.configurationProperties.name ||
            contributedTask._label, Tasks.CUSTOMIZED_TASK_TYPE, contributedTask.command, false, contributedTask.runOptions, {
        ...
        result.command.presentation =
            CommandConfiguration.PresentationOptions.assignProperties(result.command.presentation!, configuredProps.configurationProperties.presentation)!;
        ...
        result.command.presentation =
            CommandConfiguration.PresentationOptions.fillProperties(result.command.presentation, contributedConfigProps.presentation)!;
        ...
        for (let index = 0; index < externals.length; index++) {
            ...
            if (definition &&
                ...
                for (const key of Object.keys(builtinTypeContextMap)) {
                    if (external.type === key) {
                        typeNotSupported =
                            !ShellExecutionSupportedContext.evaluate(context.contextKeyService.getContext(null));
                        break;
                    }
                }
            }
            ...
            if (isCustomTask(external)) {
                ...
                if (customTask) {
                    ...
                    if (schema2_0_0) {
                        if ((customTask.command === undefined ||
                    ...
                    if (customTask.configurationProperties.group ===
                        Tasks.TaskGroup.Build &&
                        defaultBuildTask.rank < 2) {
                        defaultBuildTask.task = customTask;
                        defaultBuildTask.rank = 2;
                    }
                    else if (customTask.configurationProperties.group ===
...
class ConfigurationParser {
    ...
    private createTaskRunnerConfiguration(fileConfig: IExternalTaskRunnerConfiguration, context: IParseContext, source: TaskConfigSource): ITaskParseResult {
        ...
        if (context.schemaVersion === Tasks.JsonSchemaVersion.V2_0_0 &&
            globalTasks &&
        ...
        if ((!result.custom || result.custom.length === 0) &&
            ...
            const task: Tasks.CustomTask = new Tasks.CustomTask(context.uuidMap.getUUID(name), Object.assign({}, source, "workspace", {
                config: {
                    index: -1,
                    element: fileConfig,
                    workspaceFolder: context.workspaceFolder,
                },
            }) satisfies Tasks.IWorkspaceTaskSource, name, Tasks.CUSTOMIZED_TASK_TYPE, {
                name: undefined,
                runtime: undefined,
                presentation: undefined,
