...
import { IProblemReporter, Parser, ValidationState, ValidationStatus, } from "../../../../base/common/parsers.js";
...
import { ExtensionMessageCollector, ExtensionsRegistry, } from "../../../services/extensions/common/extensionsRegistry.js";
...
abstract class AbstractLineMatcher implements ILineMatcher {
    ...
    private appendProperty(data: IProblemData, property: keyof IProblemData, pattern: IProblemPattern, matches: RegExpExecArray, trim: boolean = false): void {
        ...
        else if (!Types.isUndefined(patternProperty) &&
...
export class ProblemPatternParser extends Parser {
    ...
    private validateProblemPattern(values: IProblemPattern[]): boolean {
        ...
        const locationKind = values[0].kind === undefined
        ...
        if (locationKind === ProblemLocationKind.Location &&
            !(location || line)) {
                    ...
                    delta.added.forEach((extension) => {
                        ...
                        for (const pattern of problemPatterns) {
                            if (Config.NamedMultiLineCheckedProblemPattern.is(pattern)) {
                                ...
                                if (parser.problemReporter.status.state <
...
export class ProblemMatcherParser extends Parser {
    ...
    private createProblemMatcher(description: Config.ProblemMatcher): ProblemMatcher | null {
        ...
        if (Types.isString(description.base)) {
            ...
            if (variableName.length > 1 && variableName[0] === "$") {
                ...
                if (base) {
                    ...
                    if (description.owner !== undefined &&
                        owner !== undefined) {
                        result.owner = owner;
                    }
                    ...
                    if (description.fileLocation !== undefined &&
                        fileLocation !== undefined) {
                        result.fileLocation = fileLocation;
                        result.filePrefix = filePrefix;
                    }
                    if (description.pattern !== undefined &&
                        pattern !== undefined &&
                        pattern !== null) {
                    ...
                    if (description.applyTo !== undefined &&
                        applyTo !== undefined) {
        ...
        if (Config.isNamedProblemMatcher(description)) {
            ...
            (result as INamedProblemMatcher).label = Types.isString(description.label)
                ? description.label
                : description.name;
        }
        return result;
