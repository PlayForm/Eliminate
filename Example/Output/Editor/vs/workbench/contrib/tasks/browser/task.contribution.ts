...
import { ITaskEvent, TASK_RUNNING_STATE, TaskEventKind, TaskGroup, TASKS_CATEGORY, TaskSettingId, } from "../common/tasks.js";
...
import { ManageAutomaticTaskRunning, RunAutomaticTasks, } from "./runAutomaticTasks.js";
...
export class TaskStatusBarContributions extends Disposable implements IWorkbenchContribution {
        ...
        this._register(this._taskService.onDidStateChange((event) => {
            ...
            if (promise &&
                ...
                this._progressService
                    ...
                    progress.report({
                        message: nls.localize("building", "Building..."),
                    });
                    return promise!;
                })
                    .then(() => {
    ...
    private _ignoreEventForUpdateRunningTasksCount(event: ITaskEvent): boolean {
        ...
        if ((isString(event.group) ? event.group : event.group?._id) !==
            TaskGroup.Build._id) {
            return true;
        }
        return (event.__task.configurationProperties.problemMatchers ===
            undefined ||
