...
import { IResolvedVariables, IResolveSet, ITaskExecuteResult, ITaskResolver, ITaskSummary, ITaskSystem, ITaskSystemInfo, ITaskSystemInfoResolver, ITaskTerminateResponse, TaskError, TaskErrors, TaskExecuteKind, Triggers, } from "../common/taskSystem.js";
...
export class TerminalTaskSystem extends Disposable implements ITaskSystem {
    ...
    taskShellIntegrationStartSequence(cwd: string | URI | undefined): string {
        return (VSCodeSequence(VSCodeOscPt.PromptStart) +
    ...
    constructor(private _terminalService: ITerminalService, private _terminalGroupService: ITerminalGroupService, private _outputService: IOutputService, private _paneCompositeService: IPaneCompositePartService, private _viewsService: IViewsService, private _markerService: IMarkerService, private _modelService: IModelService, private _configurationResolverService: IConfigurationResolverService, private _contextService: IWorkspaceContextService, private _environmentService: IWorkbenchEnvironmentService, private _outputChannelId: string, private _fileService: IFileService, private _terminalProfileResolverService: ITerminalProfileResolverService, private _pathService: IPathService, private _viewDescriptorService: IViewDescriptorService, private _logService: ILogService, private _notificationService: INotificationService, instantiationService: IInstantiationService, taskSystemInfoResolver: ITaskSystemInfoResolver) {
        super();
        ...
        this._register((this._terminalStatusManager =
    ...
    public run(task: Task, resolver: ITaskResolver, trigger: string = Triggers.command): ITaskExecuteResult {
        ...
        const instances = InMemoryTask.is(task) || this._isTaskEmpty(task)
            ? []
            : this._getInstances(task);
        const validInstance = instances.length <
    ...
    public rerun(): ITaskExecuteResult | undefined {
        if (this._lastTask && this._lastTask.verify()) {
            if (this._lastTask.task.runOptions.reevaluateOnRerun !==
                undefined &&
    ...
    private _createInactiveDependencyPromise(task: Task): Promise<ITaskSummary> {
        return new Promise<ITaskSummary>((resolve) => {
            const taskInactiveDisposable = this.onDidStateChange((taskEvent) => {
                if (taskEvent.kind === TaskEventKind.Inactive &&
    ...
    private _resolveVariablesFromSet(taskSystemInfo: ITaskSystemInfo | undefined, workspaceFolder: IWorkspaceFolder | undefined, task: CustomTask | ContributedTask, variables: Set<string>, alreadyResolved: Map<string, string>): Promise<IResolvedVariables | undefined> {
        ...
        const options = task.command && task.command.options
        ...
        if (taskSystemInfo && workspaceFolder) {
            ...
            return new Promise<IResolvedVariables | undefined>((resolve, reject) => {
                this._configurationResolverService
                    ...
                    .then(async (resolvedVariablesMap: Map<string, string> | undefined) => {
                    if (resolvedVariablesMap) {
                        ...
                        if (isProcess) {
                            ...
                            if (Platform.isWindows) {
                                processVarValue =
                                    await this._resolveAndFindExecutable(taskSystemInfo, workspaceFolder, task, cwd, envPath);
                            }
                            else {
                                processVarValue =
                                    await this._configurationResolverService.resolveAsync(workspaceFolder, CommandString.value(task.command.name!));
                            }
    ...
    private _executeCommand(task: CustomTask | ContributedTask, trigger: string, alreadyResolved: Map<string, string>): Promise<ITaskSummary> {
        ...
        const systemInfo: ITaskSystemInfo | undefined = (this._currentTask.systemInfo =
    ...
    private _reexecuteCommand(task: CustomTask | ContributedTask, trigger: string, alreadyResolved: Map<string, string>): Promise<ITaskSummary> {
        ...
        variables.forEach((value) => {
            if (value.substring(2, value.length - 1) in
    ...
    private async _executeInTerminal(task: CustomTask | ContributedTask, trigger: string, resolver: VariableResolver, workspaceFolder: IWorkspaceFolder | undefined): Promise<ITaskSummary> {
        ...
        if (task.configurationProperties.isBackground) {
            ...
            if (trigger === Triggers.reconnect && !!terminal.xterm) {
                ...
                const startRegex = new RegExp(watchingProblemMatcher.beginPatterns
    ...
    private _createTerminalName(task: CustomTask | ContributedTask): string {
        const needsFolderQualification = this._contextService.getWorkbenchState() ===
        ...
        if (isShellCommand) {
            ...
            if (task.configurationProperties.icon?.id) {
                ...
                const taskGroupKind = task.configurationProperties.group
                    ...
                    : undefined;
                const kindId = typeof taskGroupKind === "string"
                    ? taskGroupKind
                    : taskGroupKind?.kind;
                icon =
                    kindId === "test"
            ...
            if (task.command.presentation && task.command.presentation.echo) {
                ...
                shellLaunchConfig.initialText = {
                    text: this.taskShellIntegrationStartSequence(cwd) +
                        this.taskShellIntegrationOutputSequence,
    ...
    private async _reconnectToTerminal(task: Task): Promise<ITerminalInstance | undefined> {
        ...
        for (let i = 0; i < this._reconnectedTerminals.length; i++) {
            ...
            if (getReconnectionData(terminal)?.lastTask ===
    ...
    private async _createTerminal(task: CustomTask | ContributedTask, resolver: VariableResolver, workspaceFolder: IWorkspaceFolder | undefined): Promise<[
        ...
        if (task.command.runtime === RuntimeType.CustomExecution) {
            ...
            this._currentTask.shellLaunchConfig = launchConfigs =
                await this._createShellLaunchConfig(task, workspaceFolder, resolver, platform, options, command, args, waitOnExit);
            if (launchConfigs === undefined) {
                return [
                    undefined,
                    new TaskError(Severity.Error, nls.localize("TerminalTaskSystem", "Can't execute a shell command on an UNC drive using cmd.exe."), TaskErrors.UnknownError),
                ];
            }
        }
    ...
    private _buildShellCommandLine(platform: Platform.Platform, shellExecutable: string, shellOptions: IShellConfiguration | undefined, command: CommandString, originalCommand: CommandString | undefined, args: CommandString[]): string {
        ...
        function needsQuotes(value: string): boolean {
            ...
            for (let i = 0; i < value.length; i++) {
                ...
                else if (ch === shellQuoteOptions.strong ||
        ...
        function quote(value: string, kind: ShellQuoting): [
            string,
            boolean
        ] {
            ...
            else if (kind === ShellQuoting.Escape &&
        ...
        function quoteIfNecessary(value: CommandString): [
            string,
            boolean
        ] {
    ...
    private async _resolveCommandAndArgs(resolver: VariableResolver, commandConfig: ICommandConfiguration): Promise<{
        command: CommandString;
        args: CommandString[];
    }> {
    ...
    private async _resolveMatchers(resolver: VariableResolver, values: Array<string | ProblemMatcher> | undefined): Promise<ProblemMatcher[]> {
        ...
        for (const value of values) {
            ...
            const hasUriProvider = taskSystemInfo !== undefined &&
...
function getWaitOnExitValue(presentationOptions: IPresentationOptions, configurationProperties: IConfigurationProperties) {
    if (presentationOptions.close === undefined ||
