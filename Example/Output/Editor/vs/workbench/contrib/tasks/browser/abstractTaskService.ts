...
class TaskMap {
    ...
    public static getKey(workspaceFolder: IWorkspace | IWorkspaceFolder | string): string {
        ...
        if (Types.isString(workspaceFolder)) {
            ...
            const uri: URI | null | undefined = isWorkspaceFolder(workspaceFolder)
                ? workspaceFolder.uri
...
export abstract class AbstractTaskService extends Disposable implements ITaskService {
    ...
    private readonly _instantiationService: IInstantiationService) {
        ...
        this._register(this.onDidStateChange((e) => {
            ...
            else if (e.kind === TaskEventKind.Start &&
                e.__task &&
    ...
    private async _findWorkspaceTasksInGroup(group: TaskGroup, isDefault: boolean): Promise<(ConfiguringTask | Task)[]> {
        return this._findWorkspaceTasks((task) => {
            ...
            if (taskGroup && typeof taskGroup !== "string") {
                return (taskGroup._id === group._id &&
    ...
    public createSorter(): TaskSorter {
        return new TaskSorter(this._contextService.getWorkspace()
    ...
    public async getSavedTasks(type: "persistent" | "historical"): Promise<(Task | ConfiguringTask)[]> {
        ...
        for (const entry of storedTasks.entries()) {
            try {
                ...
                addTaskToMap(folderInfo.isWorkspaceFile
                    ? workspaceToTaskMap
                    : folderToTasksMap, folderInfo.folder, task);
            }
            catch (error) {
    ...
    private _shouldAttachProblemMatcher(task: Task): boolean {
        ...
        if (CustomTask.is(task)) {
            const configProperties: TaskConfig.IConfigurationProperties = task._source.config.element;
            return (configProperties.problemMatcher === undefined &&
    ...
    public needsFolderQualification(): boolean {
        return (this._contextService.getWorkbenchState() ===
    ...
    private _createCustomizableTask(task: ContributedTask | CustomTask | ConfiguringTask): TaskConfig.ICustomTask | TaskConfig.IConfiguringTask | undefined {
        ...
        const taskConfig = CustomTask.is(task) || ConfiguringTask.is(task)
            ? task._source.config
            : undefined;
    ...
    private _writeConfiguration(workspaceFolder: IWorkspaceFolder, key: string, value: any, source?: string): Promise<void> | undefined {
        ...
        switch (source) {
            ...
            case TaskSourceKind.WorkspaceFile:
                ...
                break;
            default:
                if (this._contextService.getWorkbenchState() ===
                    WorkbenchState.FOLDER) {
                    target = ConfigurationTarget.WORKSPACE;
                }
                else if (this._contextService.getWorkbenchState() ===
                    WorkbenchState.WORKSPACE) {
    ...
    private _createRunnableTask(tasks: TaskMap, group: TaskGroup): {
        task: Task;
        ...
        if (extensionTasks.length === 1) {
            ...
            const task: InMemoryTask = new InMemoryTask(id, { kind: TaskSourceKind.InMemory, label: "inMemory" }, id, "inMemory", { reevaluateOnRerun: true }, {
                identifier: id,
    ...
    private _createResolver(grouped?: TaskMap): ITaskResolver {
        ...
        async function fullResolve(that: AbstractTaskService, uri: URI | string, identifier: string | ITaskIdentifier) {
            ...
            if (Types.isString(identifier)) {
                return (data.label.get(identifier) ||
        ...
        return {
            resolve: async (uri: URI | string, identifier: string | ITaskIdentifier | undefined) => {
                ...
                if (resolverData === undefined && grouped === undefined) {
                    return ((await quickResolve(this, uri, identifier)) ??
    ...
    private async _executeTask(task: Task, resolver: ITaskResolver, runSource: TaskRunSource): Promise<ITaskSummary> {
        ...
        const executeResult = runSource === TaskRunSource.Reconnect
    ...
    private async _restart(task: Task): Promise<void> {
        ...
        if (response.success) {
            ...
            this._notificationService.warn(nls.localize("TaskSystem.restartFailed", "Failed to terminate and restart task {0}", Types.isString(task)
                ? task
    ...
    private _getCustomTaskPromises(customTasksKeyValuePairs: [
        string,
        IWorkspaceFolderTaskResult
    ][], filter: ITaskFilter | undefined, result: TaskMap, contributedTasks: TaskMap, waitToActivate: boolean | undefined) {
    ...
    private async _getAFolder(): Promise<IWorkspaceFolder> {
        let folder = this.workspaceFolders.length > 0
    ...
    private get _jsonTasksSupported(): boolean {
        return (ShellExecutionSupportedContext.getValue(this._contextKeyService) ===
            true &&
    ...
    private _testParseExternalConfig(config: TaskConfig.IExternalTaskRunnerConfiguration | undefined, location: string): {
        config: TaskConfig.IExternalTaskRunnerConfiguration | undefined;
    ...
    private _handleError(err: any): void {
        ...
        if (err instanceof TaskError) {
            ...
            const needsConfig = buildError.code === TaskErrors.NotConfigured ||
    ...
    private async _createTaskQuickPickEntries(tasks: Task[], group: boolean = false, sort: boolean = false, selectedEntry?: ITaskQuickPickEntry, includeRecents: boolean = true): Promise<ITaskQuickPickEntry[]> {
        let encounteredTasks: {
            [key: string]: ITaskQuickPickEntry[];
        } = {};
    ...
    private _needsRecentTasksMigration(): boolean {
        return (this.getRecentlyUsedTasksV1().size > 0 &&
    ...
    private _showIgnoredFoldersMessage(): Promise<void> {
        if (this.ignoredWorkspaceFolders.length === 0 ||
        ...
        this._showIgnoredFoldersMessage().then(() => {
            if (this._configurationService.getValue(USE_SLOW_PICKER)) {
                ...
                this._showQuickPick(tasks ? tasks : taskResult!.tasks, placeholder, {
                    label: "$(plus) " +
                        nls.localize("TaskService.noEntryToRun", "Configure a Task"),
                    task: null,
    ...
    private _isTaskEntry(value: IQuickPickItem): value is IQuickPickItem & {
        task: Task;
    } {
        const candidate: IQuickPickItem & {
            task: Task;
        } = value as any;
        return candidate && !!candidate.task;
    }
    private _isSettingEntry(value: IQuickPickItem): value is IQuickPickItem & {
        settingType: string;
    } {
        const candidate: IQuickPickItem & {
            settingType: string;
    ...
    private async _runConfigureTasks(): Promise<void> {
        ...
        const stats = this._contextService
            .getWorkspace()
            .folders.map<Promise<IFileStatWithPartialMetadata | undefined>>((folder) => {
            ...
            this.tasks().then((tasks) => {
                ...
                this._showIgnoredFoldersMessage().then(async () => {
                    ...
                    this._quickInputService
                        ...
                        const task: Task | undefined | null = entry && "task" in entry
                            ? entry.task
                            : undefined;
                        if (task === undefined || task === null) {
                            return;
    ...
    public async runShowTasks(): Promise<void> {
        ...
        if (activeTasks.length === 1) {
            ...
            activeTasks.every((task) => {
                ...
                return (task.command.presentation?.group &&
    ...
    private async _createTasksDotOld(folder: IWorkspaceFolder): Promise<[
        URI,
        URI
    ] | undefined> {
    ...
    private _upgradeTask(task: Task, suppressTaskName: boolean, globalConfig: {
