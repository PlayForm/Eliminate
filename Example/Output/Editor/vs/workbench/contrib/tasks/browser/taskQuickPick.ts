...
import { createColorStyleElement, getColorClass, } from "../../terminal/browser/terminalIcon.js";
import { ConfiguringTask, ContributedTask, CustomTask, KeyedTaskIdentifier, Task, TaskSorter, } from "../common/tasks.js";
...
export class TaskQuickPick extends Disposable {
    ...
    private _createTaskEntry(task: Task | ConfiguringTask, extraButtons: IQuickInputButton[] = []): ITaskTwoLevelQuickPickEntry {
        ...
        const entry: ITaskTwoLevelQuickPickEntry = {
            label: TaskQuickPick.getTaskLabelWithIcon(task, this._guessTaskLabel(task)),
    ...
    private _createEntriesForGroup(entries: QuickPickInput<ITaskTwoLevelQuickPickEntry>[], tasks: (Task | ConfiguringTask)[], groupLabel: string, extraButtons: IQuickInputButton[] = []) {
    ...
    private _dedupeConfiguredAndRecent(recentTasks: (Task | ConfiguringTask)[], configuredTasks: (Task | ConfiguringTask)[]): {
    ...
    public async show(placeHolder: string, defaultEntry?: ITaskQuickPickEntry, startAtType?: string, name?: string): Promise<Task | undefined | null> {
        ...
        disposables.add(picker.onDidTriggerItemButton(async (context) => {
            ...
            if (context.button.iconClass ===
                ThemeIcon.asClassName(removeTaskIcon)) {
                const key = task && !Types.isString(task)
                    ? task.getKey()
                    : undefined;
                if (key) {
            ...
            else if (context.button.iconClass ===
    ...
    private async _doPickerFirstLevel(picker: IQuickPick<ITaskTwoLevelQuickPickEntry, {
        useSeparators: true;
    }>, taskQuickPickEntries: QuickPickInput<ITaskTwoLevelQuickPickEntry>[], disposables: DisposableStore): Promise<Task | ConfiguringTask | string | null | undefined> {
    ...
    public async doPickerSecondLevel(picker: IQuickPick<ITaskTwoLevelQuickPickEntry, {
        useSeparators: true;
    }>, disposables: DisposableStore, type: string, name?: string) {
        picker.busy = true;
        if (type === SHOW_ALL) {
    ...
    public static allSettingEntries(configurationService: IConfigurationService): (ITaskTwoLevelQuickPickEntry & {
        settingType: string;
    })[] {
    ...
    public static getSettingEntry(configurationService: IConfigurationService, type: string): (ITaskTwoLevelQuickPickEntry & {
        settingType: string;
    }) | undefined {
