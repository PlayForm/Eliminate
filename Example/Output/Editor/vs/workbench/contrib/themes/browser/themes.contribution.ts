...
import { Extensions as ColorRegistryExtensions, IColorRegistry, } from "../../../../platform/theme/common/colorRegistry.js";
...
import { IWorkbenchColorTheme, IWorkbenchFileIconTheme, IWorkbenchProductIconTheme, IWorkbenchTheme, IWorkbenchThemeService, ThemeSettingDefaults, ThemeSettings, ThemeSettingTarget, } from "../../../services/themes/common/workbenchThemeService.js";
...
class MarketplaceThemesPicker {
    ...
    constructor(private readonly getMarketplaceColorThemes: (publisher: string, name: string, version: string) => Promise<IWorkbenchTheme[]>, private readonly marketplaceQuery: string, 
...
interface InstalledThemesPickerOptions {
    ...
    readonly onToggle?: (toggle: IQuickInputToggle, quickInput: IQuickPick<ThemeItem, {
        useSeparators: boolean;
    }>) => Promise<void>;
}
class InstalledThemesPicker {
    constructor(private readonly options: InstalledThemesPickerOptions, private readonly setTheme: (theme: IWorkbenchTheme | undefined, settingsTarget: ThemeSettingTarget) => Promise<any>, private readonly getMarketplaceColorThemes: (publisher: string, name: string, version: string) => Promise<IWorkbenchTheme[]>, 
    @IQuickInputService
    private readonly quickInputService: IQuickInputService, 
    @IExtensionGalleryService
    private readonly extensionGalleryService: IExtensionGalleryService, 
    ...
    public async openQuickPick(picks: QuickPickInput<ThemeItem>[], currentTheme: IWorkbenchTheme) {
        ...
        if (this.extensionGalleryService.isEnabled()) {
            if (this.extensionResourceLoaderService
                .supportsExtensionGalleryResources &&
                this.options.browseMessage) {
                marketplaceThemePicker =
                    this.instantiationService.createInstance(MarketplaceThemesPicker, this.getMarketplaceColorThemes.bind(this), this.options.marketplaceTag);
                picks = [
...
registerAction2(class extends Action2 {
    ...
    override async run(accessor: ServicesAccessor) {
        ...
        const options = {
            ...
            browseMessage: "$(plus) " +
                localize("browseColorThemes", "Browse Additional Color Themes..."),
...
registerAction2(class extends Action2 {
    ...
    override async run(accessor: ServicesAccessor) {
        ...
        const picks: QuickPickInput<ThemeItem>[] = [
            {
                type: "separator",
                label: localize("fileIconThemeCategory", "file icon themes"),
            },
            {
                id: "",
...
registerAction2(class extends Action2 {
    ...
    override async run(accessor: ServicesAccessor) {
        ...
        const picks: QuickPickInput<ThemeItem>[] = [
            {
                type: "separator",
                label: localize("productIconThemeCategory", "product icon themes"),
            },
            {
...
CommandsRegistry.registerCommand("workbench.action.previewColorTheme", async function (accessor: ServicesAccessor, extension: {
    publisher: string;
    name: string;
    version: string;
...
function findBuiltInThemes(themes: IWorkbenchColorTheme[], extension: {
    publisher: string;
    name: string;
    ...
    override async run(accessor: ServicesAccessor) {
        ...
        const selectTheme = (theme: IWorkbenchTheme | undefined, applyTheme: boolean) => {
            ...
            selectThemeTimeout = mainWindow.setTimeout(() => {
                ...
                themeService
                    .setColorTheme(newTheme as IWorkbenchColorTheme, applyTheme ? "auto" : "preview")
                    .then(undefined, (err) => {
                    onUnexpectedError(err);
...
class DefaultThemeUpdatedNotificationContribution implements IWorkbenchContribution {
    ...
    private async _showYouGotMigratedNotification(): Promise<void> {
        const usingLight = this._workbenchThemeService.getColorTheme().type ===
            ColorScheme.LIGHT;
        ...
        if (newTheme) {
            ...
            }, "Visual Studio Code now ships with a new default theme '{0}'. If you prefer, you can switch back to the old theme or try one of the many other color themes available.", newTheme.label), choices, {
                onCancel: () => this._writeTelemetry("cancel"),
            });
        }
    }
    private async _tryNewThemeNotification(): Promise<void> {
        const newThemeSettingsId = this._workbenchThemeService.getColorTheme().type ===
            ColorScheme.LIGHT
            ? ThemeSettingDefaults.COLOR_THEME_LIGHT
            : ThemeSettingDefaults.COLOR_THEME_DARK;
        ...
        if (theme) {
            ...
            await this._notificationService.prompt(Severity.Info, localize({
                ...
                comment: ["{0} is the name of the new default theme"],
            }, "Visual Studio Code now ships with a new default theme '{0}'. Do you want to give it a try?", theme.label), choices, { onCancel: () => this._writeTelemetry("cancel") });
        }
    }
