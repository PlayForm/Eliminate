...
import { getOrMakeSearchEditorInput, SEARCH_EDITOR_EXT, SearchEditorInput, } from "./searchEditorInput.js";
...
Registry.as<IEditorPaneRegistry>(EditorExtensions.EditorPane).registerEditorPane(EditorPaneDescriptor.create(SearchEditor, SearchEditor.ID, localize("searchEditor", "Search Editor")), [new SyncDescriptor(SearchEditorInput)]);
//#endregion
    ...
    instantiationService: IInstantiationService) {
        editorResolverService.registerEditor("*" + SEARCH_EDITOR_EXT, {
            ...
            createEditorInput: ({ resource }) => {
                return {
                    editor: instantiationService.invokeFunction(getOrMakeSearchEditorInput, { from: "existingFile", fileUri: resource }),
                };
            },
        });
    }
...
Registry.as<IEditorFactoryRegistry>(EditorExtensions.EditorFactory).registerEditorSerializer(SearchEditorInput.ID, SearchEditorInputSerializer);
//#endregion
//#region Commands
    ...
    async run(accessor: ServicesAccessor) {
        ...
        if (contextService.getValue(SearchEditorConstants.InSearchEditor.serialize())) {
            (accessor.get(IEditorService)
                .activeEditorPane as SearchEditor).deleteResultBlock();
        }
    }
});
registerAction2(class extends Action2 {
...
class SearchEditorWorkingCopyEditorHandler extends Disposable implements IWorkbenchContribution, IWorkingCopyEditorHandler {
    ...
    handles(workingCopy: IWorkingCopyIdentifier): boolean {
        return (workingCopy.resource.scheme ===
    ...
    isOpen(workingCopy: IWorkingCopyIdentifier, editor: EditorInput): boolean {
        ...
        return (editor instanceof SearchEditorInput &&
