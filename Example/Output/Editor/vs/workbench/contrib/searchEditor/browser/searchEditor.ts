...
import { InSearchEditor, SearchConfiguration, SearchEditorID, SearchEditorInputTypeId, } from "./constants.js";
...
export class SearchEditor extends AbstractTextCodeEditor<SearchEditorViewState> {
    ...
    protected override createEditor(parent: HTMLElement) {
        ...
        this.createQueryEditor(this.queryEditorContainer, this._register(this.instantiationService.createChild(new ServiceCollection([
            IContextKeyService,
    ...
    private createQueryEditor(container: HTMLElement, scopedInstantiationService: IInstantiationService, inputBoxFocusedContextKey: IContextKey<boolean>) {
        ...
        this.inputPatternIncludes.onSubmit((triggeredOnType) => this.triggerSearch({
            resetCursor: false,
        ...
        this.inputPatternExcludes.onSubmit((triggeredOnType) => this.triggerSearch({
            resetCursor: false,
            delay: triggeredOnType
                ? this.searchConfig.searchOnTypeDebouncePeriod
                : 0,
        ...
        this.searchResultEditor.onMouseUp((e) => {
            if (e.event.detail === 1) {
                ...
                if (position && behaviour === "peekDefinition") {
                    const line = this.searchResultEditor
                        .getModel()
                        ?.getLineContent(position.lineNumber) ?? "";
                    if (line.match(FILE_LINE_REGEX) ||
    ...
    deleteResultBlock() {
        ...
        model.pushEditOperations(this.searchResultEditor.getSelections(), [...linesToDelete].map((line) => ({
    ...
    private iterateThroughMatches(reverse: boolean) {
        ...
        const fallbackStart = reverse
            ...
            : new Position(1, 1);
        const currentPosition = this.searchResultEditor.getSelection()?.getStartPosition() ??
            fallbackStart;
