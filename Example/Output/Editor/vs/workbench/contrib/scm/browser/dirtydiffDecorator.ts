...
import { IDiffEditorModel, IEditorContribution, IEditorDecorationsCollection, IEditorModel, ScrollType, } from "../../../../editor/common/editorCommon.js";
import { EditorContextKeys } from "../../../../editor/common/editorContextKeys.js";
import { IModelDecorationOptions, ITextModel, MinimapPosition, OverviewRulerLane, shouldSynchronizeModel, } from "../../../../editor/common/model.js";
...
class UIEditorAction extends Action {
    ...
    constructor(editor: ICodeEditor, action: EditorAction, cssClass: string, 
    @IKeybindingService
...
class DirtyDiffWidget extends PeekViewWidget {
    ...
    constructor(editor: ICodeEditor, private model: DirtyDiffModel, 
    @IThemeService
    ...
    private switchQuickDiff(event?: IQuickDiffSelectItem) {
        ...
        const closestIndex = Math.abs(this.model.changes[closestGreaterIndex].change
            .modifiedEndLineNumber -
            this.model.changes[this._index].change
                .modifiedEndLineNumber) <
            Math.abs(this.model.changes[closestLesserIndex].change
                .modifiedEndLineNumber -
                this.model.changes[this._index].change
                    .modifiedEndLineNumber)
            ? closestGreaterIndex
    ...
    protected override _fillHead(container: HTMLElement): void {
        ...
        this.dropdown = this.instantiationService.createInstance(SwitchQuickDiffViewItem, new SwitchQuickDiffBaseAction((event?: IQuickDiffSelectItem) => this.switchQuickDiff(event)), this.model.quickDiffs.map((quickDiffer) => quickDiffer.label), this.model.changes[this._index].label);
...
export class DirtyDiffController extends Disposable implements DirtyDiffContribution {
    ...
    next(lineNumber?: number): void {
        ...
        if (this.editor.hasModel() &&
            ...
            index = this.model.findNextClosestChange(typeof lineNumber === "number"
                ? lineNumber
                : this.editor.getPosition().lineNumber, true, this.widget.provider);
        }
        else {
            const providerChanges: number[] = this.model.mapChanges.get(this.widget.provider) ??
    ...
    previous(lineNumber?: number): void {
        ...
        if (this.editor.hasModel() && typeof lineNumber === "number") {
            index = this.model.findPreviousClosestChange(typeof lineNumber === "number"
                ? lineNumber
                : this.editor.getPosition().lineNumber, true, this.widget.provider);
        }
        else {
    ...
    private assertWidget(): boolean {
        ...
        const onDidModelChange = Event.chain(model.onDidChange, ($) => $.filter((e) => e.diff.length > 0).map((e) => e.diff));
...
class DirtyDiffDecorator extends Disposable {
    static createDecoration(className: string, tooltip: string | null, options: {
        gutter: boolean;
        overview: {
            active: boolean;
            color: string;
    ...
    constructor(editorModel: ITextModel, private readonly codeEditor: ICodeEditor, private model: DirtyDiffModel, 
    @IConfigurationService
    private readonly configurationService: IConfigurationService) {
        super();
...
export class DirtyDiffModel extends Disposable {
    ...
    constructor(textFileModel: IResolvedTextFileEditorModel, 
    @ISCMService
    ...
    private triggerDiff(): Promise<void> {
        ...
        return this.diffDelayer
            ...
            if (!result ||
                ...
                originalModels.some((originalModel) => originalModel.isDisposed())) {
                return; // disposed
            }
            if (originalModels.every((originalModel) => originalModel.textEditorModel.getValueLength() ===
                0)) {
                result.changes = [];
            }
            if (!result.changes) {
    ...
    private getQuickDiffsPromise(): Promise<QuickDiff[]> {
        ...
        this._quickDiffsPromise = this.getOriginalResource().then(async (quickDiffs) => {
            ...
            if (equals(this._quickDiffs, quickDiffs, (a, b) => a.originalResource.toString() ===
    ...
    private async getOriginalResource(): Promise<QuickDiff[]> {
        ...
        if (session &&
            session.entries
                .get()
                .find((v) => isEqual(v.modifiedURI, uri) &&
                v.state.get() === WorkingSetEntryState.Modified)) {
            // disable dirty diff when doing chat edits
            return Promise.resolve([]);
        }
        return this.quickDiffService.getQuickDiffs(uri, this._model.getLanguageId(), this._model.textEditorModel
    ...
    private onEditorsChanged(): void {
            ...
            if (isCodeEditor(editor)) {
                ...
                if (textModel &&
                    ...
                    if (textFileModel?.isResolved()) {
                        ...
                        this.items
                            .get(textModel.uri)
                            ?.set(editor.getId(), new DirtyDiffItem(dirtyDiffModel, decorator));
                    }
                }
            }
        }
        for (const [uri, item] of this.items) {
            for (const editorId of item.keys()) {
                if (!this.editorService.visibleTextEditorControls.find((editor) => isCodeEditor(editor) &&
