...
import { HISTORY_VIEW_PANE_ID, ISCMProvider, ISCMService, ISCMViewService, REPOSITORIES_VIEW_PANE_ID, VIEW_PANE_ID, VIEWLET_ID, } from "../common/scm.js";
import { SCMService } from "../common/scmService.js";
import { SCMActiveRepositoryController, SCMActiveResourceContextKeyController, } from "./activity.js";
...
Registry.as<IWorkbenchContributionsRegistry>(WorkbenchExtensions.Workbench).registerWorkbenchContribution(DirtyDiffWorkbenchController, LifecyclePhase.Restored);
...
viewsRegistry.registerViews([
    {
        ...
        // Repository Count
        ContextKeyExpr.and(ContextKeyExpr.has("scm.providerCount"), ContextKeyExpr.notEquals("scm.providerCount", 0)), 
        // Not Serverless
        ContextKeyExpr.and(IsWebContext, RemoteNameContext.isEqualTo(""))?.negate()),
        containerIcon: sourceControlViewIcon,
    },
], viewContainer);
Registry.as<IWorkbenchContributionsRegistry>(WorkbenchExtensions.Workbench).registerWorkbenchContribution(SCMActiveRepositoryController, LifecyclePhase.Restored);
Registry.as<IWorkbenchContributionsRegistry>(WorkbenchExtensions.Workbench).registerWorkbenchContribution(SCMActiveResourceContextKeyController, LifecyclePhase.Restored);
registerWorkbenchContribution2(SCMWorkingSetController.ID, SCMWorkingSetController, WorkbenchPhase.AfterRestored);
