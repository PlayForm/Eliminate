...
import { ISCMMenus, ISCMProvider, ISCMRepository, ISCMRepositorySortKey, ISCMService, ISCMViewService, ISCMViewVisibleRepositoryChangeEvent, } from "../common/scm.js";
...
export class SCMViewService implements ISCMViewService {
    ...
    set visibleRepositories(visibleRepositories: ISCMRepository[]) {
        ...
        if (this._repositories.find((r) => r.focused && r.selectionIndex === -1)) {
            this.focus(this._repositories.find((r) => r.selectionIndex !== -1)
                ?.repository);
        }
    }
    ...
    readonly onDidChangeVisibleRepositories = Event.any(this._onDidSetVisibleRepositories.event, Event.debounce(this._onDidChangeRepositories.event, (last, e) => {
        if (!last) {
            return e;
        }
     ...
     */
    private readonly _activeRepository = latestChangedValue(this, [
    ...
    isVisible(repository: ISCMRepository): boolean {
        return (this._repositories.find((r) => r.repository === repository)
            ?.selectionIndex !== -1);
    }
