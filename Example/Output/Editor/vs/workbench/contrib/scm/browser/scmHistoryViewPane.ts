...
import { autorun, autorunWithStore, constObservable, derived, IObservable, latestChangedValue, observableFromEvent, observableSignal, observableValue, runOnChange, waitForState, } from "../../../../base/common/observable.js";
...
import { historyItemHoverAdditionsForeground, historyItemHoverDefaultLabelBackground, historyItemHoverDefaultLabelForeground, historyItemHoverDeletionsForeground, historyItemHoverLabelForeground, renderSCMHistoryGraphPlaceholder, renderSCMHistoryItemGraph, SWIMLANE_WIDTH, toISCMHistoryItemViewModelArray, } from "./scmHistory.js";
...
class SCMHistoryItemRefsActionViewItem extends ActionViewItem {
    constructor(private readonly _repository: ISCMRepository, private readonly _historyItemsFilter: "all" | "auto" | ISCMHistoryItemRef[], action: IAction, options?: IDropdownMenuActionViewItemOptions) {
        super(null, action, { ...options, icon: false, label: true });
    }
    protected override updateLabel(): void {
        if (this.options.label && this.label) {
...
registerAction2(class extends Action2 {
    ...
    override async run(accessor: ServicesAccessor, provider: ISCMProvider, ...historyItems: ISCMHistoryItem[]) {
        ...
        const historyItemParentId = historyItemLast.parentIds.length > 0
...
class HistoryItemRenderer implements ITreeRenderer<SCMHistoryItemViewModelTreeElement, LabelFuzzyScore, HistoryItemTemplate> {
    ...
    renderElement(node: ITreeNode<SCMHistoryItemViewModelTreeElement, LabelFuzzyScore>, index: number, templateData: HistoryItemTemplate, height: number | undefined): void {
        ...
        const historyItemRef = provider.historyProvider
            .get()
            ?.historyItemRef?.get();
        const extraClasses = historyItemRef?.revision === historyItem.id
    ...
    private _renderBadge(historyItemRefs: ISCMHistoryItemRef[], showDescription: boolean, templateData: HistoryItemTemplate): void {
        if (historyItemRefs.length === 0 ||
            !ThemeIcon.isThemeIcon(historyItemRefs[0].icon)) {
            return;
        }
        ...
        elements.count.textContent =
            historyItemRefs.length > 1 ? historyItemRefs.length.toString() : "";
        elements.icon.classList.add(...ThemeIcon.asClassNameArray(historyItemRefs[0].icon));
        elements.description.textContent = showDescription
            ? historyItemRefs[0].name
            : "";
    ...
    private _getHoverContent(element: SCMHistoryItemViewModelTreeElement): IManagedHoverTooltipMarkdownString {
        ...
        if (historyItem.author) {
            ...
            if (historyItem.timestamp) {
                const dateFormatter = new Intl.DateTimeFormat(platform.language, {
                    year: "numeric",
                    month: "long",
                    day: "numeric",
                    hour: "numeric",
    ...
    private _processMatches(historyItemViewModel: ISCMHistoryItemViewModel, filterData: LabelFuzzyScore | undefined): [
        IMatch[] | undefined,
        IMatch[] | undefined
    ] {
        if (!filterData) {
...
class HistoryItemLoadMoreRenderer implements ITreeRenderer<SCMHistoryItemLoadMoreTreeElement, void, LoadMoreTemplate> {
    ...
    constructor(private readonly _isLoadingMore: IObservable<boolean>, private readonly _loadMoreCallback: () => void, 
    @IConfigurationService
    private readonly _configurationService: IConfigurationService) { }
...
class HistoryItemHoverDelegate extends WorkbenchHoverDelegate {
    constructor(private readonly _viewContainerLocation: ViewContainerLocation | null, 
...
class SCMHistoryViewModel extends Disposable {
    private readonly _closedRepository = observableFromEvent(this, this._scmService.onDidRemoveRepository, (repository) => repository);
    ...
    getCurrentHistoryItemTreeElement(): SCMHistoryItemViewModelTreeElement | undefined {
        ...
        return state.viewModels.find((viewModel) => viewModel.historyItemViewModel.historyItem.id ===
    ...
    loadMore(cursor?: string): void {
        ...
        this._repositoryState.set(repository, {
            ...state,
            loadMore: cursor ?? true,
        });
    }
    async getHistoryItems(): Promise<SCMHistoryItemViewModelTreeElement[]> {
        ...
        if (!state || state.loadMore !== false) {
            ...
            state = {
                historyItemsFilter: historyItemRefs,
                viewModels,
                loadMore: false,
            };
...
class HistoryItemRefPicker extends Disposable {
    ...
    constructor(private readonly _historyProvider: ISCMHistoryProvider, private readonly _historyItemsFilter: "all" | "auto" | ISCMHistoryItemRef[], 
    @IQuickInputService
    private readonly _quickInputService: IQuickInputService) {
        super();
    }
                ...
                if (added.length > 0) {
                    if (added[0].historyItemRef === "all" ||
                        ...
                        quickPick.selectedItems = [
                            ...quickPick.selectedItems.filter((i) => i.historyItemRef !== "all" &&
...
export class SCMHistoryViewPane extends ViewPane {
    ...
    async revealCurrentHistoryItem(): Promise<void> {
        ...
        const revealTreeNode = (): boolean => {
            ...
            if (historyItemTreeElement &&
                this._tree.hasNode(historyItemTreeElement)) {
    ...
    private async _onDidOpen(e: IOpenEvent<TreeElement | undefined>): Promise<void> {
        if (!e.element) {
            ...
            const historyItemParentId = historyItem.parentIds.length > 0
