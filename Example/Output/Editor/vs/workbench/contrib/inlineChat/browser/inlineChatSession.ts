...
import { CTX_INLINE_CHAT_HAS_STASHED_SESSION, EditMode, } from "../common/inlineChat.js";
...
export class Session {
    ...
    constructor(readonly editMode: EditMode, readonly headless: boolean, 
    /**
     ...
     */
    readonly textModelN: ITextModel, readonly agent: IChatAgent, readonly wholeRange: SessionWholeRange, readonly hunkData: HunkData, readonly chatModel: ChatModel, versionsByRequest?: [
        string,
        number
    ][]) {
        this._teldata = {
...
export class StashedSession {
    ...
    constructor(editor: ICodeEditor, session: Session, private readonly _undoCancelEdits: IValidEditOperation[], 
    @IContextKeyService
...
export class HunkData {
    ...
    private readonly _editorWorkerService: IEditorWorkerService, private readonly _textModel0: ITextModel, private readonly _textModelN: ITextModel) {
    ...
    async recompute(editState: IChatTextEditGroupState, diff?: IDocumentDiff | null) {
        ...
        if (diff && diff.changes.length > 0) {
            ...
            for (let i = 1; i < diff.changes.length; i++) {
                ...
                if (thisChange.modified.startLineNumber -
                    lastChange.modified.endLineNumberExclusive <=
                    HunkData._HUNK_THRESHOLD) {
                    mergedChanges[mergedChanges.length - 1] =
                        new DetailedLineRangeMapping(lastChange.original.join(thisChange.original), lastChange.modified.join(thisChange.modified), (lastChange.innerChanges ?? []).concat(thisChange.innerChanges ?? []));
                }
                else {
...
class RawHunk {
    constructor(readonly original: LineRange, readonly modified: LineRange, readonly changes: RangeMapping[]) { }
}
type RawHunkData = {
