...
export class InlineChatWidget {
    ...
    constructor(location: IChatWidgetLocationOptions, private readonly _options: IInlineChatWidgetConstructionOptions, 
    ...
    private readonly _hoverService: IHoverService) {
            ...
            filter: (item) => {
                if (isResponseVM(item) &&
                    ...
                    if (item.response.value.length > 0 &&
                        item.response.value.every((item) => item.kind === "textEditGroup" &&
                ...
                ctxResponseVote.set(isResponseVM(last)
                    ? last.vote === ChatAgentVoteDirection.Down
                        ...
                        : last.vote === ChatAgentVoteDirection.Up
                            ...
                            : ""
                    : "");
                ctxResponseError.set(isResponseVM(last) &&
                    last.errorDetails !== undefined);
                ctxResponseErrorFiltered.set(!!(isResponseVM(last) &&
                    last.errorDetails?.responseIsFiltered));
                ctxResponseSupportIssues.set(isResponseVM(last) &&
        ...
        const statusMenuId = _options.statusMenuId instanceof MenuId
            ? _options.statusMenuId
            : _options.statusMenuId.menu;
        // BUTTON bar
        const statusMenuOptions = _options.statusMenuId instanceof MenuId
            ? undefined
            : _options.statusMenuId.options;
    ...
    private _updateAriaLabel(): void {
        this._elements.root.ariaLabel =
            this._accessibleViewService.getOpenAriaHint(AccessibilityVerbositySettingId.InlineChat);
...
export class EditorBasedInlineChatWidget extends InlineChatWidget {
    ...
    constructor(location: IChatWidgetLocationOptions, private readonly _parentEditor: ICodeEditor, options: IInlineChatWidgetConstructionOptions, 
    @IContextKeyService
    ...
    showAccessibleHunk(session: Session, hunkData: HunkInformation): void {
        ...
        this._accessibleViewer.value =
            this._instantiationService.createInstance(HunkAccessibleDiffViewer, this._elements.accessibleViewer, session, hunkData, new AccessibleHunk(this._parentEditor, session, hunkData));
        this._onDidChangeHeight.fire();
    }
}
class HunkAccessibleDiffViewer extends AccessibleDiffViewer {
    ...
    constructor(parentNode: HTMLElement, session: Session, hunk: HunkInformation, models: IAccessibleDiffViewerModel, 
    @IInstantiationService
    instantiationService: IInstantiationService) {
...
class AccessibleHunk implements IAccessibleDiffViewerModel {
    constructor(private readonly _editor: ICodeEditor, private readonly _session: Session, private readonly _hunk: HunkInformation) { }
