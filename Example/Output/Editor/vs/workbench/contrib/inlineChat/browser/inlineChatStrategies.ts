...
import { HunkInformation, HunkState, Session } from "./inlineChatSession.js";
...
export abstract class EditModeStrategy {
    ...
    constructor(protected readonly _session: Session, protected readonly _editor: ICodeEditor, protected readonly _zone: InlineChatZoneWidget, 
    @ITextFileService
    ...
    getWholeRangeDecoration(): IModelDeltaDecoration[] {
        ...
        const newDecorations = ranges.map((range) => range.isEmpty()
            ? undefined
...
export class PreviewStrategy extends EditModeStrategy {
    ...
    constructor(session: Session, editor: ICodeEditor, zone: InlineChatZoneWidget, 
    @IModelService
...
export class LiveStrategy extends EditModeStrategy {
    ...
    constructor(session: Session, editor: ICodeEditor, zone: InlineChatZoneWidget, private readonly _showOverlayToolbar: boolean, 
    @IContextKeyService
    contextKeyService: IContextKeyService, 
                ...
                for (const hunkData of this._session.hunkData.getInfo()) {
                    ...
                    if (!data) {
                        ...
                        const result = renderLines(source, renderOptions, [
                            new InlineDecoration(new Range(originalRange.startLineNumber, 1, originalRange.startLineNumber, 1), "", InlineDecorationType.Regular),
                        ], domNode);
                        ...
                        const move = (next: boolean) => {
                            ...
                            const nextIdx = (idx + (next ? 1 : -1) + keys.length) %
                                keys.length;
                            if (nextIdx !== idx) {
                    ...
                    if (hunkData.getState() === HunkState.Pending &&
