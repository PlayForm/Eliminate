...
export class InlineChatSessionServiceImpl implements IInlineChatSessionService {
    ...
    async createSession(editor: IActiveCodeEditor, options: {
        editMode: EditMode;
        headless?: boolean;
        wholeRange?: Range;
        session?: Session;
        ...
        const chatModel = options.session?.chatModel ??
        ...
        store.add(toDisposable(() => {
            ...
            ].some((data) => data.session !== session &&
        ...
        const textModel0 = store.add(this._modelService.createModel(createTextBufferFactoryFromSnapshot(textModel.createSnapshot()), {
...
export class InlineChatEnabler {
    ...
    editorService: IEditorService) {
        ...
        const updateEditor = () => {
            ...
            const isCodeEditorLike = isCodeEditor(ctrl) ||
                isDiffEditor(ctrl) ||
