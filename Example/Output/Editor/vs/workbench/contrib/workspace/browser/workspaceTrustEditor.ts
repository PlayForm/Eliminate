...
class WorkspaceTrustedUrisTable extends Disposable {
        ...
        const entries = this.workspaceTrustManagementService
            ...
            for (const workspaceUri of currentWorkspaceUris) {
                relatedToCurrentWorkspace =
                    relatedToCurrentWorkspace ||
                        this.uriService.extUri.isEqualOrParent(workspaceUri, uri);
            }
            return {
                uri,
    ...
    layout(): void {
        this.table.layout(this.trustedUriEntries.length *
...
export class WorkspaceTrustEditor extends EditorPane {
    ...
    private getFeaturesHeaderText(trusted: boolean): [
        string,
        string
    ] {
        let title: string = "";
    ...
    private async render() {
        ...
        const settingsRequiringTrustedWorkspaceCount = restrictedSettings.default.filter((key) => {
            ...
            if (property.scope === ConfigurationScope.APPLICATION ||
    ...
    private createButtonRow(parent: HTMLElement, buttonInfo: {
        action: Action;
        ...
        if (!this.workspaceTrustManagementService.isWorkspaceTrustForced()) {
            textElement.innerText =
                this.workspaceService.getWorkbenchState() ===
                    ...
                    : localize("untrustedFolderReason", "This folder is trusted via the bolded entries in the the trusted folders below.");
        }
        else {
