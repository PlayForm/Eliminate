    ...
    private registerListeners(): void {
        ...
        this._register(this.workspaceTrustRequestService.onDidInitiateOpenFilesTrustRequest(async () => {
            ...
            await this.dialogService.prompt<void>({
                type: Severity.Info,
                message: this.workspaceContextService.getWorkbenchState() !==
                    WorkbenchState.EMPTY
...
export class WorkspaceTrustUXHandler extends Disposable implements IWorkbenchContribution {
    ...
    //#region Dialog
    private async doShowModal(question: string, trustedOption: {
        label: string;
        sublabel: string;
    }, untrustedOption: {
        label: string;
        sublabel: string;
    }, markdownStrings: string[], trustParentString?: string): Promise<void> {
    ...
    private async showModalOnStart(): Promise<void> {
        ...
        if (this.startupPromptSetting === "once" &&
            this.storageService.getBoolean(STARTUP_PROMPT_SHOWN_KEY, StorageScope.WORKSPACE, false)) {
            this.updateWorkbenchIndicators(false);
            return;
        }
        ...
        return await this.fileService
            ...
            if (result) {
                try {
                    ...
                    if (workspaces.indexOf(this.workspaceContextService
                        .getWorkspace()
                        .folders[0].uri.toString()) > -1) {
                        return true;
                    }
                }
                catch (e) {
...
Registry.as<IEditorFactoryRegistry>(EditorExtensions.EditorFactory).registerEditorSerializer(WorkspaceTrustEditorInput.ID, WorkspaceTrustEditorInputSerializer);
Registry.as<IEditorPaneRegistry>(EditorExtensions.EditorPane).registerEditorPane(EditorPaneDescriptor.create(WorkspaceTrustEditor, WorkspaceTrustEditor.ID, localize("workspaceTrustEditor", "Workspace Trust Editor")), [new SyncDescriptor(WorkspaceTrustEditorInput)]);
/*
 * Actions
 */
    ...
    private async logWorkspaceTrust(isTrusted: boolean): Promise<void> {
        ...
        if (isTrusted) {
            ...
            for (const folder of this.workspaceContextService.getWorkspace()
                ...
                this.telemetryService.publicLog2<WorkspaceTrustFolderInfoEvent, WorkspaceTrustFolderInfoEventClassification>("workspaceFolderDepthBelowTrustedFolder", {
                    workspaceFolderDepth,
                    trustedFolderDepth,
                    delta,
                });
...
Registry.as<IWorkbenchContributionsRegistry>(WorkbenchExtensions.Workbench).registerWorkbenchContribution(WorkspaceTrustTelemetryContribution, LifecyclePhase.Restored);
