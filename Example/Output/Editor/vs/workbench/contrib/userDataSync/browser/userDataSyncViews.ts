...
import { ALL_SYNC_RESOURCES, getLastSyncResourceUri, ISyncResourceHandle as IResourceHandle, ISyncUserDataProfile, IUserDataAutoSyncService, IUserDataSyncEnablementService, IUserDataSyncResourceProviderService, IUserDataSyncService, SyncResource, SyncStatus, UserDataSyncError, UserDataSyncErrorCode, } from "../../../../platform/userDataSync/common/userDataSync.js";
import { isWebPlatform, IUserDataSyncMachine, IUserDataSyncMachinesService, } from "../../../../platform/userDataSync/common/userDataSyncMachines.js";
...
import { AccountStatus, CONTEXT_ACCOUNT_STATE, CONTEXT_ENABLE_ACTIVITY_VIEWS, CONTEXT_ENABLE_SYNC_CONFLICTS_VIEW, CONTEXT_HAS_CONFLICTS, CONTEXT_SYNC_STATE, getSyncAreaLabel, IUserDataSyncWorkbenchService, SYNC_CONFLICTS_VIEW_ID, SYNC_TITLE, } from "../../../services/userDataSync/common/userDataSync.js";
...
export class UserDataSyncDataViews extends Disposable {
    ...
    private registerActivityView(container: ViewContainer, remote: boolean): void {
        ...
        this._register(Event.any(this.userDataSyncEnablementService
            .onDidChangeResourceEnablement, this.userDataSyncEnablementService.onDidChangeEnablement, this.userDataSyncService.onDidResetLocal, this.userDataSyncService.onDidResetRemote)(() => treeView.refresh()));
        const viewsRegistry = Registry.as<IViewsRegistry>(Extensions.ViewsRegistry);
        const viewDescriptor: ITreeViewDescriptor = {
            id,
            name,
    ...
    private registerDataViewActions(viewId: string) {
        ...
        this._register(registerAction2(class extends Action2 {
            ...
            async run(accessor: ServicesAccessor, handle: TreeViewItemHandleArg): Promise<void> {
                ...
                return commandService.executeCommand(API_OPEN_DIFF_EDITOR_COMMAND_ID, remoteResource, localResource, localize("remoteToLocalDiff", "{0} ↔ {1}", localize({
                    key: "leftResourceName",
                    comment: ["remote as in file in cloud"],
                }, "{0} (Remote)", basename(remoteResource)), localize({
                    key: "rightResourceName",
                    comment: ["local as in file in disk"],
                }, "{0} (Local)", basename(localResource))), undefined);
            }
        }));
        this._register(registerAction2(class extends Action2 {
            constructor() {
...
abstract class UserDataSyncActivityViewDataProvider<T = Profile> implements ITreeViewDataProvider {
    ...
    async getChildren(element?: ITreeItem): Promise<ITreeItem[]> {
        try {
            ...
            if ((<ProfileTreeItem>element).profile ||
                element.handle ===
...
class ExtractedUserDataSyncActivityViewDataProvider extends UserDataSyncActivityViewDataProvider<ISyncUserDataProfile> {
    ...
    private getMachines(): Promise<IUserDataSyncMachine[]> {
        if (this.machinesPromise === undefined) {
            this.machinesPromise =
                this.userDataSyncResourceProviderService.getLocalSyncedMachines(this.activityDataLocation);
        }
        return this.machinesPromise;
    }
}
class UserDataSyncMachinesViewDataProvider implements ITreeViewDataProvider {
        ...
        try {
            ...
            return machines.map(({ id, name, isCurrent, platform }) => ({
                ...
                description: isCurrent
                    ...
                    : undefined,
                themeIcon: platform && isWebPlatform(platform)
                    ? Codicon.globe
                    : Codicon.vm,
    ...
    async rename(machineId: string): Promise<boolean> {
        ...
        const validateMachineName = (machineName: string): string | null => {
            ...
            return machineName &&
                !machines.some((m) => m.id !== machineId && m.name === machineName)
                ? machineName
                : null;
        };
        ...
        if (!element) {
            return [
                ...
                },
                {
                    ...
                    collapsibleState: TreeItemCollapsibleState.Collapsed,
                    label: {
                        label: localize("last sync states", "Last Synced Remotes"),
                    },
                    themeIcon: Codicon.folder,
                },
            ];
