...
import { Change, IResourcePreview, IUserDataSyncEnablementService, IUserDataSyncResource, IUserDataSyncService, MergeState, } from "../../../../platform/userDataSync/common/userDataSync.js";
import { TreeViewPane } from "../../../browser/parts/views/treeView.js";
import { IViewletViewOptions } from "../../../browser/parts/views/viewsViewlet.js";
import { DEFAULT_EDITOR_ASSOCIATION } from "../../../common/editor.js";
import { ITreeItem, IViewDescriptorService, TreeItemCollapsibleState, TreeViewItemHandleArg, } from "../../../common/views.js";
...
export class UserDataSyncConflictsViewPane extends TreeViewPane implements IUserDataSyncConflictsView {
    ...
    private async getTreeItems(): Promise<ITreeItem[]> {
        ...
        for (const previewResource of conflictResources) {
            let result = conflictResourcesByProfile[conflictResourcesByProfile.length - 1]?.[0].id === previewResource.profile.id
            ...
            for (const resource of resources) {
                ...
                const treeItem = {
                    ...
                    label: {
                        ...
                        strikethrough: resource.mergeState === MergeState.Accepted &&
        ...
        this._register(registerAction2(class AcceptRemoteAction extends Action2 {
            ...
            async run(accessor: ServicesAccessor, handle: TreeViewItemHandleArg): Promise<void> {
                ...
                await that.userDataSyncWorkbenchService.accept({
                    syncResource: conflict.syncResource,
                    profile: conflict.profile,
                }, conflict.remoteResource, undefined, that.userDataSyncEnablementService.isEnabled());
            }
        }));
        this._register(registerAction2(class AcceptLocalAction extends Action2 {
            ...
            async run(accessor: ServicesAccessor, handle: TreeViewItemHandleArg): Promise<void> {
                ...
                await that.userDataSyncWorkbenchService.accept({
                    syncResource: conflict.syncResource,
                    profile: conflict.profile,
                }, conflict.localResource, undefined, that.userDataSyncEnablementService.isEnabled());
            }
        }));
    }
    async open(conflictToOpen: IResourcePreview): Promise<void> {
        ...
        }, "{0} (Remote)", basename(conflictToOpen.remoteResource));
