...
import { IUserDataAutoSyncService, IUserDataSyncService, registerConfiguration, SyncResource, SyncStatus, UserDataSyncError, UserDataSyncErrorCode, USER_DATA_SYNC_SCHEME, IUserDataSyncEnablementService, IResourcePreview, IUserDataSyncStoreManagementService, UserDataSyncStoreType, IUserDataSyncStore, IUserDataSyncResourceConflicts, IUserDataSyncResource, IUserDataSyncResourceError, USER_DATA_SYNC_LOG_ID } from '../../../../platform/userDataSync/common/userDataSync.js';
...
type ConfigureSyncQuickPickItem = {
    id: SyncResource;
    label: string;
    description?: string;
};
...
export class UserDataSyncWorkbenchContribution extends Disposable implements IWorkbenchContribution {
                    ...
                    const handle = this.notificationService.prompt(Severity.Warning, localize('conflicts detected', "Unable to sync due to conflicts in {0}. Please resolve them to continue.", conflictsArea.toLowerCase()), [
                        {
                            ...
                            run: () => {
                                this.telemetryService.publicLog2<{
                                    source: string;
                                    action: string;
                                }, SyncConflictsClassification>('sync/handleConflicts', { source: conflict.syncResource, action: 'acceptLocal' });
                        ...
                        },
                        {
                            ...
                            run: () => {
                                this.telemetryService.publicLog2<{
                                    source: string;
                                    action: string;
                                }, SyncConflictsClassification>('sync/handleConflicts', { source: conflict.syncResource, action: 'acceptRemote' });
                        ...
                        },
                        {
                            ...
                            run: () => {
                                this.telemetryService.publicLog2<{
                                    source: string;
                                    action?: string;
                                }, SyncConflictsClassification>('sync/showConflicts', { source: conflict.syncResource });
    ...
    private handleTooLargeError(resource: SyncResource, message: string, error: UserDataSyncError): void {
        ...
        this.notificationService.notify({
            ...
            message: operationId ? `${message} ${operationId}` : message,
            actions: {
                primary: [new Action('open sync file', localize('open file', "Open {0} File", getSyncAreaLabel(resource)), undefined, true, () => resource === SyncResource.Settings ? this.preferencesService.openUserSettings({ jsonEditor: true }) : this.preferencesService.openGlobalKeybindingSettings(true))]
            }
        });
    }
    ...
    private handleInvalidContentError({ profile, syncResource: source }: IUserDataSyncResource): void {
        ...
        const handle = this.notificationService.notify({
            ...
            actions: {
                primary: [new Action('open sync file', localize('open file', "Open {0} File", errorArea), undefined, true, () => source === SyncResource.Settings ? this.preferencesService.openUserSettings({ jsonEditor: true }) : this.preferencesService.openGlobalKeybindingSettings(true))]
            }
        });
