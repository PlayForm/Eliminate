...
import { CONTEXT_VARIABLE_NAME, CONTEXT_VARIABLE_TYPE, CONTEXT_VARIABLE_VALUE, IDebugSession, IDebugVisualization, IDebugVisualizationContext, IDebugVisualizationTreeItem, IExpression, IExpressionContainer, MainThreadDebugVisualization, } from "./debug.js";
...
export class DebugVisualizerService implements IDebugVisualizerService {
    ...
    /* extId + \0 + vizId */ string, VisualizerHandle>();
    private readonly trees = new Map<
    /* extId + \0 + treeId */ string, VisualizerTreeHandle>();
    ...
    public async getApplicableFor(variable: IExpression, token: CancellationToken): Promise<IReference<DebugVisualizer[]>> {
        ...
        const maybeVisualizers = await Promise.all(this.registrations.map(async (registration) => {
            ...
            return (handle && {
                handle,
                result: await handle.provideDebugVisualizers(context, token),
            });
        }));
        const ref = {
    ...
    private getVariableContext(threadId: number, variable: Variable) {
        const context: IDebugVisualizationContext = {
            ...
            containerId: variable.parent instanceof Variable
                ? variable.reference
                : undefined,
            threadId,
            variable: {
