 ...
 */
export function filterExceptionsFromTelemetry<T extends {
    [key: string]: unknown;
}>(data: T): Partial<T> {
...
export function getExactExpressionStartAndEnd(lineContent: string, looseStart: number, looseEnd: number): {
    start: number;
    end: number;
} {
    ...
    if (matchingExpression) {
        ...
        while ((subExpressionResult = subExpression.exec(matchingExpression))) {
            const subEnd = subExpressionResult.index +
                1 +
                startOffset +
...
export async function getEvaluatableExpressionAtPosition(languageFeaturesService: ILanguageFeaturesService, model: ITextModel, position: Position, token?: CancellationToken): Promise<{
    range: IRange;
        ...
        case "event": {
            ...
            switch (event.event) {
                ...
                case "loadedSource":
                    ...
                    break;
                case "breakpoint":
                    fixSourcePath(false, (<DebugProtocol.BreakpointEvent>event).body.breakpoint
                        .source);
                    break;
                default:
                    break;
        ...
        case "request": {
            ...
            switch (request.command) {
                ...
                case "breakpointLocations":
                    ...
                    break;
                case "source":
                    fixSourcePath(true, (<DebugProtocol.SourceArguments>request.arguments)
                        .source);
                    break;
                case "gotoTargets":
                    fixSourcePath(true, (<DebugProtocol.GotoTargetsArguments>request.arguments)
                        .source);
                    break;
                case "launchVSCode":
            ...
            if (response.success && response.body) {
                switch (response.command) {
                    ...
                    case "disassemble":
                        ...
                        break;
                    case "locations":
                        fixSourcePath(false, (<DebugProtocol.LocationsResponse>response).body
                            ?.source);
                        break;
                    default:
                        break;
...
export function getVisibleAndSorted<T extends {
...
export const sourcesEqual = (a: DebugProtocol.Source | undefined, b: DebugProtocol.Source | undefined): boolean => !a || !b
    ? a === b
    : a.name === b.name &&
        a.path === b.path &&
