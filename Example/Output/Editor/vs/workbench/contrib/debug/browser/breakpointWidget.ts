...
import { EditorCommand, registerEditorCommand, ServicesAccessor, } from "../../../../editor/browser/editorExtensions.js";
...
import { BREAKPOINT_EDITOR_CONTRIBUTION_ID, BreakpointWidgetContext as Context, CONTEXT_BREAKPOINT_WIDGET_VISIBLE, CONTEXT_IN_BREAKPOINT_WIDGET, DEBUG_SCHEME, IBreakpoint, IBreakpointEditorContribution, IBreakpointUpdateData, IDebugService, } from "../common/debug.js";
const $ = dom.$;
...
export class BreakpointWidget extends ZoneWidget implements IPrivateBreakpointWidgetService {
    ...
    private get placeholder(): string {
        const acceptString = this.keybindingService
            .lookupKeybinding(AcceptBreakpointWidgetInputAction.ID)
            ?.getLabel() || "Enter";
        const closeString = this.keybindingService
    ...
    private setInputMode(): void {
        if (this.editor.hasModel()) {
            ...
            const languageId = this.context === Context.LOG_MESSAGE
    ...
    private createTriggerBreakpointInput(container: HTMLElement) {
        ...
        const selectBreakpointBox = (this.selectBreakpointBox = new SelectBox(breakpointOptions, index + 1, this.contextViewService, defaultSelectBoxStyles, {
            ariaLabel: nls.localize("selectBreakpoint", "Select breakpoint"),
        }));
                    ...
                    suggestionsPromise = provideSuggestionItems(this.languageFeaturesService.completionProvider, underlyingModel, new Position(this.lineNumber, 1), new CompletionOptions(undefined, new Set<CompletionItemKind>().add(CompletionItemKind.Snippet)), _context, token).then((suggestions) => {
                        ...
                        if (this.context === Context.CONDITION) {
                            ...
                            const value = this.input
                                .getModel()
                                .getValue();
                            while (position.column - 2 - overwriteBefore >=
                                0 &&
                        ...
                        return {
                            suggestions: suggestions.items.map((s) => {
                                s.completion.range =
                                    Range.fromPositions(position.delta(0, -overwriteBefore), position);
                                return s.completion;
                            }),
                        };
                    });
