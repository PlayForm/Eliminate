    ...
    async run(accessor: ServicesAccessor, editor: ICodeEditor): Promise<void> {
        ...
        if (position &&
            ...
            editor
                ...
                ?.showBreakpointWidget(position.lineNumber, undefined, BreakpointWidgetContext.CONDITION);
        }
    }
}
class LogPointAction extends EditorAction {
    ...
    async run(accessor: ServicesAccessor, editor: ICodeEditor): Promise<void> {
        ...
        if (position &&
            ...
            editor
                .getContribution<IBreakpointEditorContribution>(BREAKPOINT_EDITOR_CONTRIBUTION_ID)
                ?.showBreakpointWidget(position.lineNumber, position.column, BreakpointWidgetContext.LOG_MESSAGE);
        }
    }
}
class TriggerByBreakpointAction extends EditorAction {
    ...
    async run(accessor: ServicesAccessor, editor: ICodeEditor): Promise<void> {
        ...
        if (position &&
            ...
            editor
                .getContribution<IBreakpointEditorContribution>(BREAKPOINT_EDITOR_CONTRIBUTION_ID)
                ?.showBreakpointWidget(position.lineNumber, position.column, BreakpointWidgetContext.TRIGGER_POINT);
        }
    }
}
...
class ToggleDisassemblyViewSourceCodeAction extends Action2 {
    ...
    run(accessor: ServicesAccessor, editor: ICodeEditor, ...args: any[]): void {
        ...
        if (configService) {
            const value = configService.getValue<IDebugConfiguration>("debug")
...
class StepIntoTargetsAction extends EditorAction {
    ...
    async run(accessor: ServicesAccessor, editor: ICodeEditor): Promise<void> {
        ...
        if (selection) {
                ...
                if (target.line) {
                    positionalTargets.push({
                        start: new Position(target.line, target.column || 1),
                            ...
                            ? new Position(target.endLine, target.endColumn || 1)
                            : undefined,
                        target,
                    });
                }
            }
            positionalTargets.sort((a, b) => b.start.lineNumber - a.start.lineNumber ||
            ...
            const best = positionalTargets.find((t) => t.end &&
                needle.isBefore(t.end) &&
                t.start.isBeforeOrEqual(needle)) ||
                positionalTargets.find((t) => t.end === undefined && t.start.isBeforeOrEqual(needle));
            if (best) {
        ...
        if (editor.hasModel()) {
            ...
            let moveBreakpoint = this.isNext
                ? allEnabledBreakpoints
                    .filter((bp) => uriIdentityService.extUri.isEqual(bp.uri, currentUri) && bp.lineNumber > currentLine)
                    .shift()
                : allEnabledBreakpoints
                    .filter((bp) => uriIdentityService.extUri.isEqual(bp.uri, currentUri) && bp.lineNumber < currentLine)
                    .pop();
            ...
            if (!moveBreakpoint) {
                moveBreakpoint = this.isNext
                    ? allEnabledBreakpoints
                        .filter((bp) => bp.uri.toString() > currentUri.toString())
                        .shift()
                    : allEnabledBreakpoints
                        .filter((bp) => bp.uri.toString() < currentUri.toString())
                        .pop();
