...
import { CALLSTACK_VIEW_ID, CONTEXT_BREAKPOINTS_EXIST, CONTEXT_DEBUG_STATE, CONTEXT_DEBUG_TYPE, CONTEXT_DEBUG_UX, CONTEXT_DISASSEMBLY_VIEW_FOCUS, CONTEXT_HAS_DEBUGGED, CONTEXT_IN_DEBUG_MODE, DEBUG_MEMORY_SCHEME, DEBUG_SCHEME, debuggerDisabledMessage, getStateLabel, IAdapterManager, IBreakpoint, IBreakpointData, IBreakpointUpdateData, ICompound, IConfig, IConfigurationManager, IDebugConfiguration, IDebugModel, IDebugService, IDebugSession, IDebugSessionOptions, IEnablement, IExceptionBreakpoint, IGlobalConfig, ILaunch, IStackFrame, IThread, IViewModel, REPL_VIEW_ID, State, VIEWLET_ID, } from "../common/debug.js";
...
export class DebugService implements IDebugService {
    ...
    private readonly testService: ITestService) {
        ...
        this.disposables.add(Event.any(this.adapterManager.onDidRegisterDebugger, this.configurationManager.onDidSelectConfiguration)(() => {
            const debugUxValue = this.state !== State.Inactive ||
        ...
        this.disposables.add(this.model.onDidChangeCallStack(() => {
            ...
            if (numberOfSessions > 0) {
                ...
                if (viewContainer) {
                    this.activity =
                        this.activityService.showViewContainerActivity(viewContainer.id, {
    ...
    private onStateChange(): void {
        ...
        if (this.previousState !== state) {
            this.contextKeyService.bufferChangeEvents(() => {
                ...
                const debugUxValue = (state !== State.Inactive &&
        ...
        try {
            ...
            if (compound) {
                ...
                if (compound.preLaunchTask) {
                    const taskResult = await this.taskRunner.runTaskAndCheckErrors(launch?.workspace ||
                        this.contextService.getWorkspace(), compound.preLaunchTask);
                ...
                const values = await Promise.all(compound.configurations.map((configData) => {
                    const name = typeof configData === "string"
                        ? configData
                        : configData.name;
            ...
            if (configOrName && !config) {
                const message = !!launch
                    ? nls.localize("configMissing", "Configuration '{0}' is missing in 'launch.json'.", typeof configOrName === "string"
                        ? configOrName
                        : configOrName.name)
    ...
    private async createSession(launch: ILaunch | undefined, config: IConfig | undefined, options?: IDebugSessionOptions): Promise<boolean> {
        ...
        if (configByProviders && configByProviders.type) {
            try {
                ...
                const cfg = await this.configurationManager.resolveDebugConfigurationWithSubstitutedVariables(launch && launch.workspace
                    ? launch.workspace.uri
                    : undefined, resolvedConfig.type, resolvedConfig, initCancellationToken.token);
                if (!cfg) {
                    if (launch &&
                        type &&
                        cfg === null &&
                ...
                if (!dbg ||
                    ...
                    let message: string;
                    if (configByProviders.request !== "attach" &&
                        configByProviders.request !== "launch") {
                ...
                else if (this.contextService.getWorkbenchState() ===
     ...
     */
    private async doCreateSession(sessionId: string, root: IWorkspaceFolder | undefined, configuration: {
        resolved: IConfig;
        ...
        listenerDisposables.add(session.onDidEndAdapter(async (adapterExitEvent) => {
            if (adapterExitEvent) {
                if (adapterExitEvent.error) {
                    this.notificationService.error(nls.localize("debugAdapterCrash", "Debug adapter process has terminated unexpectedly ({0})", adapterExitEvent.error.message ||
            ...
            if (this.model.getSessions().length === 0) {
                ...
                if (this.layoutService.isVisible(Parts.SIDEBAR_PART) &&
                    this.configurationService.getValue<IDebugConfiguration>("debug").openExplorerOnEnd) {
    ...
    async restartSession(session: IDebugSession, restartData?: any): Promise<void> {
        ...
        if (launch && needsToSubstitute && unresolved) {
            ...
            if (resolvedByProviders) {
                ...
                if (resolved &&
                    !initCancellationToken.token.isCancellationRequested) {
                    resolved =
                        await this.configurationManager.resolveDebugConfigurationWithSubstitutedVariables(launch && launch.workspace
                            ? launch.workspace.uri
                            : undefined, resolved.type, resolved, initCancellationToken.token);
                }
            }
            else {
        ...
        for (const breakpoint of this.model.getBreakpoints({
            triggeredOnly: true,
        })) {
        ...
        const shouldFocus = !!this.viewModel.focusedSession &&
    ...
    private async showError(message: string, errorActions: ReadonlyArray<IAction> = [], promptLaunchJson = true): Promise<void> {
        ...
        const actions = errorActions.filter((action) => action.id.endsWith(".command"))
            .length > 0
            ? errorActions
            : [
                ...errorActions,
    ...
    //---- focus management
    async focusStackFrame(_stackFrame: IStackFrame | undefined, _thread?: IThread, _session?: IDebugSession, options?: {
        explicit?: boolean;
                ...
                if (editor.input === DisassemblyViewInput.instance) {
                    ...
                    if (stackFrame &&
                        ...
                        if (lineNumber >= 1 &&
                            ...
                            }, "{0}, debugging paused {1}, {2}:{3}", lineContent, thread && thread.stoppedDetails
    ...
    async updateFunctionBreakpoint(id: string, update: {
        name?: string;
        hitCondition?: string;
        condition?: string;
    }): Promise<void> {
    ...
    async updateDataBreakpoint(id: string, update: {
        hitCondition?: string;
        condition?: string;
    }): Promise<void> {
    ...
    private unlinkTriggeredBreakpoints(allBreakpoints: readonly IBreakpoint[], removedBreakpoints: readonly IBreakpoint[]): uri[] {
        ...
        for (const removed of removedBreakpoints) {
            for (const existing of allBreakpoints) {
                if (!removedBreakpoints.includes(existing) &&
                    existing.triggeredBy === removed.getId()) {
                    this.model.updateBreakpoints(new Map([
    ...
    private async makeTriggeredBreakpointsMatchEnablement(enable: boolean, breakpoint: Breakpoint) {
        ...
        await Promise.all(this.model
            .getBreakpoints()
            .filter((bp) => bp.triggeredBy === breakpoint.getId() &&
            bp.enabled !== enable)
    ...
    private onFileChanges(fileChangesEvent: FileChangesEvent): void {
        const toRemove = this.model
            .getBreakpoints()
            .filter((bp) => fileChangesEvent.contains(bp.originalUri, FileChangeType.DELETED));
        if (toRemove.length) {
            this.model.removeBreakpoints(toRemove);
        }
        const toSend: URI[] = [];
        ...
        for (const sessionId of breakpoint.sessionsThatVerified) {
            ...
            if (bestScore < Score.VerifiedAndPausedInFile) {
                const pausedInThisFile = threads.find((t) => {
                    ...
                    return (top &&
                        this.uriIdentityService.extUri.isEqual(top.source.uri, uri));
                });
                if (pausedInThisFile) {
...
export function getStackFrameThreadAndSessionToFocus(model: IDebugModel, stackFrame: IStackFrame | undefined, thread?: IThread, session?: IDebugSession, avoidSession?: IDebugSession): {
