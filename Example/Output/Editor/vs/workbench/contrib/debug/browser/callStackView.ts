...
export function getContextForContributedActions(element: CallStackItem | null): string | number {
    if (element instanceof StackFrame) {
        if (element.source.inMemory) {
            return (element.source.raw.path ||
...
export class CallStackView extends ViewPane {
    ...
    constructor(private options: IViewletViewOptions, 
    @IContextMenuService
    ...
    private readonly menuService: IMenuService) {
        ...
        this.onCallStackChangeScheduler = this._register(new RunOnceScheduler(async () => {
            ...
            const thread = sessions.length === 1 &&
                sessions[0].getAllThreads().length === 1
                ? sessions[0].getAllThreads()[0]
                : undefined;
            const stoppedDetails = sessions.length === 1
    ...
    protected override renderBody(container: HTMLElement): void {
        ...
        this._register(this.tree.onDidOpen(async (e) => {
            ...
            const focusStackFrame = (stackFrame: IStackFrame | undefined, thread: IThread | undefined, session: IDebugSession, options: {
                explicit?: boolean;
...
function getSessionContextOverlay(session: IDebugSession): [
    string,
    any
][] {
    return [
...
function getThreadContextOverlay(thread: IThread): [
    string,
    any
][] {
    return [
...
function getStackFrameContextOverlay(stackFrame: IStackFrame): [
    string,
    any
][] {
    return [
...
class StackFramesRenderer implements ICompressibleTreeRenderer<IStackFrame, FuzzyScore, IStackFrameTemplateData> {
    ...
    renderElement(element: ITreeNode<IStackFrame, FuzzyScore>, index: number, data: IStackFrameTemplateData): void {
        ...
        data.stackFrame.classList.toggle("disabled", !stackFrame.source ||
...
class ShowMoreRenderer implements ICompressibleTreeRenderer<IStackFrame[], FuzzyScore, ILabelTemplateData> {
    ...
    renderElement(element: ITreeNode<IStackFrame[], FuzzyScore>, index: number, data: ILabelTemplateData): void {
        ...
        if (stackFrames.every((sf) => !!(sf.source &&
            sf.source.origin &&
...
function stoppedDescription(stoppedDetails: IRawStoppedDetails): string {
    return (stoppedDetails.description ||
        (stoppedDetails.reason
            ...
            }, "Paused on {0}", stoppedDetails.reason)
            ...
            children.forEach((child, index) => {
                if (child instanceof StackFrame &&
                    ...
                    if (this.deemphasizedStackFramesToShow.indexOf(child) === -1) {
                        ...
                        const nextChild = index < children.length - 1
                            ? children[index + 1]
                            : undefined;
