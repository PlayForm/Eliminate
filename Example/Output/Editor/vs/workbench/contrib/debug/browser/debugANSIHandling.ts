...
export function handleANSIOutput(text: string, linkDetector: ILinkDetector, workspaceFolder: IWorkspaceFolder | undefined): HTMLSpanElement {
    ...
    while (currentPos < textLength) {
        ...
        if (text.charCodeAt(currentPos) === 27 &&
    ...
    function setBasicFormatters(styleCodes: number[]): void {
        for (const code of styleCodes) {
            switch (code) {
                ...
                case 4: {
                    // underline
                    styleNames = styleNames.filter((style) => style !== `code-underline` &&
                ...
                case 73: {
                    // superscript
                    styleNames = styleNames.filter((style) => style !== `code-superscript` &&
 ...
 */
export function appendStylizedStringToContainer(root: HTMLElement, stringContent: string, cssClasses: string[], linkDetector: ILinkDetector, workspaceFolder: IWorkspaceFolder | undefined, customTextColor?: RGBA | string, customBackgroundColor?: RGBA | string, customUnderlineColor?: RGBA | string): void {
