...
export class ConfigurationManager implements IConfigurationManager {
    ...
    constructor(private readonly adapterManager: IAdapterManager, 
    ...
    hasDebugConfigurationProvider(debugType: string, triggerKind?: DebugConfigurationProviderTriggerKind): boolean {
        ...
        const provider = this.configProviders.find((p) => p.provideDebugConfigurations &&
            p.type === debugType &&
    ...
    async resolveDebugConfigurationWithSubstitutedVariables(folderUri: uri | undefined, type: string | undefined, config: IConfig, token: CancellationToken): Promise<IConfig | null | undefined> {
        ...
        const providers = this.configProviders
            ...
            .concat(this.configProviders.filter((p) => p.type === "*" &&
            p.resolveDebugConfigurationWithSubstitutedVariables));
        let result: IConfig | null | undefined = config;
        await sequence(providers.map((provider) => async () => {
            ...
            if (result) {
                result =
                    await provider.resolveDebugConfigurationWithSubstitutedVariables!(folderUri, result, token);
            }
        }));
        return result;
    }
    ...
    getLaunch(workspaceUri: uri | undefined): ILaunch | undefined {
        ...
        return this.launches.find((l) => l.workspace &&
    ...
    async selectConfiguration(launch: ILaunch | undefined, name?: string, config?: IConfig, dynamicConfig?: {
        type?: string;
    }): Promise<void> {
        ...
        if (name && names.indexOf(name) >= 0) {
            ...
            if (!config) {
                ...
                this.getSelectedConfig = async () => {
                    ...
                    const provider = activatedProviders.length > 0
...
abstract class AbstractLaunch implements ILaunch {
    ...
    abstract openConfigFile(options: {
        preserveFocus: boolean;
        type?: string | undefined;
        suppressInitialConfigs?: boolean | undefined;
    }, token?: CancellationToken | undefined): Promise<{
        editor: IEditorPane | null;
        created: boolean;
    }>;
    constructor(protected configurationManager: ConfigurationManager, private readonly adapterManager: IAdapterManager) { }
...
class Launch extends AbstractLaunch implements ILaunch {
    constructor(configurationManager: ConfigurationManager, adapterManager: IAdapterManager, public workspace: IWorkspaceFolder, 
    @IFileService
    ...
    async openConfigFile({ preserveFocus, type, suppressInitialConfigs, }: {
        preserveFocus: boolean;
        type?: string;
...
class WorkspaceLaunch extends AbstractLaunch implements ILaunch {
    constructor(configurationManager: ConfigurationManager, adapterManager: IAdapterManager, 
    @IEditorService
    private readonly editorService: IEditorService, 
    @IConfigurationService
    private readonly configurationService: IConfigurationService, 
    ...
    async openConfigFile({ preserveFocus, type, useInitialConfigs, }: {
        preserveFocus: boolean;
        type?: string;
