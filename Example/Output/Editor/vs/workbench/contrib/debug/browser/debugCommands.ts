...
import { CONTEXT_BREAKPOINT_INPUT_FOCUSED, CONTEXT_BREAKPOINTS_FOCUSED, CONTEXT_DEBUG_STATE, CONTEXT_DEBUGGERS_AVAILABLE, CONTEXT_DISASSEMBLY_VIEW_FOCUS, CONTEXT_EXPRESSION_SELECTED, CONTEXT_FOCUSED_SESSION_IS_ATTACH, CONTEXT_IN_DEBUG_MODE, CONTEXT_IN_DEBUG_REPL, CONTEXT_JUMP_TO_CURSOR_SUPPORTED, CONTEXT_STEP_INTO_TARGETS_SUPPORTED, CONTEXT_VARIABLES_FOCUSED, CONTEXT_WATCH_EXPRESSIONS_FOCUSED, EDITOR_CONTRIBUTION_ID, getStateLabel, IConfig, IDebugConfiguration, IDebugEditorContribution, IDebugService, IDebugSession, IEnablement, isFrameDeemphasized, IStackFrame, IThread, REPL_VIEW_ID, State, VIEWLET_ID, } from "../common/debug.js";
...
CommandsRegistry.registerCommand({
    ...
    handler: async (accessor: ServicesAccessor, session: IDebugSession) => {
        ...
        const stoppedChildSession = debugService
            .getModel()
            .getSessions()
            .find((s) => s.parentSession === session && s.state === State.Stopped);
...
CommandsRegistry.registerCommand({
    ...
    handler: async (accessor: ServicesAccessor, debugType: string | unknown, debugStartOptions?: {
        noDebug?: boolean;
    }) => {
        const quickInputService = accessor.get(IQuickInputService);
        const debugService = accessor.get(IDebugService);
...
KeybindingsRegistry.registerCommandAndKeybindingRule({
    ...
    handler: async (accessor: ServicesAccessor, debugStartOptions?: {
