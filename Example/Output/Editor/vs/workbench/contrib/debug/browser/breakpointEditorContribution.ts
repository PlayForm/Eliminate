...
export function createBreakpointDecorations(accessor: ServicesAccessor, model: ITextModel, breakpoints: ReadonlyArray<IBreakpoint>, state: State, breakpointsActivated: boolean, showBreakpointsInOverviewRuler: boolean): {
    range: Range;
...
async function requestBreakpointCandidateLocations(model: ITextModel, lineNumbers: number[], session: IDebugSession): Promise<BreakpointsForLine[]> {
    ...
    return await Promise.all(distinct(lineNumbers, (l) => l).map(async (lineNumber) => {
        try {
            return {
                lineNumber,
                positions: await session.breakpointsLocations(model.uri, lineNumber),
            };
        }
        catch {
...
function createCandidateDecorations(model: ITextModel, breakpointDecorations: IBreakpointDecoration[], lineBreakpoints: BreakpointsForLine[]): {
    range: Range;
    options: IModelDecorationOptions;
    ...
    for (const { positions, lineNumber } of lineBreakpoints) {
        ...
        positions.forEach((p) => {
            ...
            if ((p.column <= firstColumn &&
                !breakpointDecorations.some((bp) => bp.range.startColumn > firstColumn &&
            ...
            if (e.event.rightButton ||
                ...
                if (breakpoints.length) {
                    ...
                    if (isShiftPressed) {
                        ...
                        breakpoints.some((bp) => !!bp.condition ||
                            !!bp.logMessage ||
                            !!bp.hitCondition ||
                            !!bp.triggeredBy)) {
    ...
    private getContextMenuActions(breakpoints: ReadonlyArray<IBreakpoint>, uri: URI, lineNumber: number, column?: number): IAction[] {
        ...
        if (breakpoints.length === 1) {
            ...
            actions.push(new SubmenuAction("debug.enableDisableBreakpoints", nls.localize("enableDisableBreakpoints", "Enable/Disable Breakpoints"), sorted.map((bp) => new Action(bp.enabled
                ...
                : "enableColumnBreakpoint", bp.enabled
                ? bp.column
            ...
            actions.push(new Action("addBreakpoint", nls.localize("addBreakpoint", "Add Breakpoint"), undefined, true, () => this.debugService.addBreakpoints(uri, [
                { lineNumber, column },
            ])));
            actions.push(new Action("addConditionalBreakpoint", nls.localize("addConditionalBreakpoint", "Add Conditional Breakpoint..."), undefined, true, () => Promise.resolve(this.showBreakpointWidget(lineNumber, column, BreakpointWidgetContext.CONDITION))));
            actions.push(new Action("addLogPoint", nls.localize("addLogPoint", "Add Logpoint..."), undefined, true, () => Promise.resolve(this.showBreakpointWidget(lineNumber, column, BreakpointWidgetContext.LOG_MESSAGE))));
            actions.push(new Action("addTriggeredBreakpoint", nls.localize("addTriggeredBreakpoint", "Add Triggered Breakpoint..."), undefined, true, () => Promise.resolve(this.showBreakpointWidget(lineNumber, column, BreakpointWidgetContext.TRIGGER_POINT))));
        }
        if (this.debugService.state === State.Stopped) {
            actions.push(new Separator());
            actions.push(new Action("runToLine", nls.localize("runToLine", "Run to Line"), undefined, true, () => this.debugService
    ...
    private marginFreeFromNonDebugDecorations(line: number): boolean {
        ...
        if (decorations) {
            for (const { options } of decorations) {
                ...
                const hasSomeActionableCodicon = !(clz.includes("codicon-") ||
        ...
        const setCandidateDecorations = (changeAccessor: IModelDecorationsChangeAccessor, desiredCandidatePositions: BreakpointsForLine[]) => {
            ...
            this.candidateDecorations = candidateDecorationIds.map((decorationId, index) => {
                ...
                const icon = candidate.breakpoint
                    ? getBreakpointMessageAndIcon(this.debugService.state, this.debugService
                        .getModel()
                        .areBreakpointsActivated(), candidate.breakpoint, this.labelService, this.debugService.getModel()).icon
...
class InlineBreakpointWidget implements IContentWidget, IDisposable {
    ...
    private toDispose: IDisposable[] = [];
    constructor(private readonly editor: IActiveCodeEditor, private readonly decorationId: string, cssClass: string | null | undefined, private readonly breakpoint: IBreakpoint | undefined, private readonly debugService: IDebugService, private readonly contextMenuService: IContextMenuService, private readonly getContextMenuActions: () => IAction[]) {
    ...
    private create(cssClass: string | null | undefined): void {
        ...
        this.toDispose.push(this.editor.onDidChangeConfiguration((c) => {
            if (c.hasChanged(EditorOption.fontSize) ||
...
const debugIconBreakpointCurrentStackframeForeground = registerColor("debugIcon.breakpointCurrentStackframeForeground", {
    ...
    hcDark: "#FFCC00",
    hcLight: "#BE8700",
}, nls.localize("debugIcon.breakpointCurrentStackframeForeground", "Icon color for the current breakpoint stack frame."));
