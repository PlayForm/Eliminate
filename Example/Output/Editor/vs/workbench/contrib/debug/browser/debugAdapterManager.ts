...
import { CONTEXT_DEBUG_EXTENSION_AVAILABLE, CONTEXT_DEBUGGERS_AVAILABLE, IAdapterDescriptor, IAdapterManager, IConfig, IDebugAdapter, IDebugAdapterDescriptorFactory, IDebugAdapterFactory, IDebugConfiguration, IDebugSession, INTERNAL_CONSOLE_OPTIONS_SCHEMA, } from "../common/debug.js";
...
export class AdapterManager extends Disposable implements IAdapterManager {
    ...
    constructor(delegate: IAdapterManagerDelegate, 
    @IEditorService
                ...
                added.value.forEach((rawAdapter) => {
                    ...
                    if (rawAdapter.type !== "*") {
                        ...
                        if (existing) {
                            ...
                            dbg.when
                                ?.keys()
                                .forEach((key) => this.debuggerWhenKeys.add(key));
    ...
    canSetBreakpointsIn(model: ITextModel): boolean {
        ...
        if (this.configurationService.getValue<IDebugConfiguration>("debug")
    ...
    async guessDebugger(gettingConfigurations: boolean): Promise<Debugger | undefined> {
        ...
        if (candidates.length === 0 && languageLabel) {
            ...
            const { confirmed } = await this.dialogService.confirm({
                ...
                }, "&&Find {0} extension", languageLabel),
            });
            if (confirmed) {
        ...
        return this.quickInputService
            ...
            label: string;
            debugger?: Debugger;
        } | IQuickPickItem>(picks, { activeItem: picks[0], placeHolder })
            .then(async (picked) => {
