    ...
    nameValueMap.forEach((_value, name) => {
        ...
        if (lineNumbers) {
            for (const lineNumber of lineNumbers) {
                if (ranges.some((r) => lineNumber >= r.startLineNumber &&
...
export class DebugEditorContribution implements IDebugEditorContribution {
    ...
    private updateHoverConfiguration(): void {
        ...
        if (model) {
            this.editorHoverOptions =
                this.configurationService.getValue<IEditorHoverOptions>("editor.hover", {
                    resource: model.uri,
    ...
    private preventDefaultEditorHover() {
        if (this.defaultHoverLockout.value ||
    ...
    private onEditorMouseDown(mouseEvent: IEditorMouseEvent): void {
        this.mouseDown = true;
        if (mouseEvent.target.type === MouseTargetType.CONTENT_WIDGET &&
    ...
    private onEditorMouseMove(mouseEvent: IEditorMouseEvent): void {
        ...
        if ((target.type === MouseTargetType.CONTENT_WIDGET &&
    ...
    private async toggleExceptionWidget(): Promise<void> {
        ...
        const exceptionSf = callStack.find((sf) => !!(sf &&
            sf.source &&
            sf.source.available &&
    ...
    async addLaunchConfiguration(): Promise<void> {
        ...
        const getConfigurationPosition = () => {
            ...
            visit(model.getValue(), {
                ...
                onArrayBegin: (offset: number) => {
                    if (lastProperty === "configurations" &&
                        depthInArray === 0) {
        ...
        const insertLine = (position: Position): Promise<any> => {
            ...
            if (model.getLineLastNonWhitespaceColumn(position.lineNumber) >
                position.column) {
    ...
    private get updateInlineValuesScheduler(): RunOnceScheduler {
        const model = this.editor.getModel();
        return new RunOnceScheduler(async () => await this.updateInlineValueDecorations(this.debugService.getViewModel().focusedStackFrame), model
        ...
        if (this.languageFeaturesService.inlineValuesProvider.has(model)) {
            const findVariable = async (_key: string, caseSensitiveLookup: boolean): Promise<string | undefined> => {
                ...
                for (const scope of scopes) {
                    ...
                    const found = variables.find((v) => caseSensitiveLookup
                        ? v.name === key
                    ...
                    for (const iv of result) {
                        ...
                        switch (iv.type) {
                            ...
                            case "variable": {
                                ...
                                if (!va) {
                                    const lineContent = model.getLineContent(iv.range
                                        .startLineNumber);
            ...
            /* line */ number, Map</* var */ string, /* value */ string>>();
