...
export class SettingsChangeRelauncher extends Disposable implements IWorkbenchContribution {
    ...
    private onConfigurationChange(e: IConfigurationChangeEvent): void {
        if (e &&
            !SettingsChangeRelauncher.SETTINGS.some((key) => e.affectsConfiguration(key))) {
            return;
        }
    ...
    private update(askToRelaunch: boolean): void {
        ...
        if (isNative) {
            // Titlebar style
            processChanged((config.window.titleBarStyle === TitlebarStyle.NATIVE ||
                config.window.titleBarStyle === TitlebarStyle.CUSTOM) &&
                this.titleBarStyle.handleChange(config.window?.titleBarStyle));
            // macOS: Native tabs
            processChanged(isMacintosh &&
                this.nativeTabs.handleChange(config.window?.nativeTabs));
            // macOS: Native fullscreen
            processChanged(isMacintosh &&
                this.nativeFullScreen.handleChange(config.window?.nativeFullScreen));
            // macOS: Click through (accept first mouse)
            processChanged(isMacintosh &&
                this.clickThroughInactive.handleChange(config.window?.clickThroughInactive));
            // Linux: WCO
            processChanged(isLinux &&
                this.linuxWindowControlOverlay.handleChange(config.window?.experimentalControlOverlay));
            // Update mode
        ...
        if (askToRelaunch && changed && this.hostService.hasFocus) {
            this.doConfirm(isNative
                ...
                : localize({
                    key: "restartWeb",
                    comment: ["&& denotes a mnemonic"],
                }, "&&Reload"), () => this.hostService.restart());
        }
    }
        ...
        if (this.contextService.getWorkbenchState() === WorkbenchState.WORKSPACE) {
            ...
            if (!this.onDidChangeWorkspaceFoldersUnbind) {
                this.onDidChangeWorkspaceFoldersUnbind =
                    this.contextService.onDidChangeWorkspaceFolders(() => this.onDidChangeWorkspaceFolders());
            }
        }
