...
class LanguageStatusViewModel {
    constructor(readonly combined: readonly ILanguageStatus[], readonly dedicated: readonly ILanguageStatus[]) { }
    isEqual(other: LanguageStatusViewModel) {
        return (equals(this.combined, other.combined) &&
            equals(this.dedicated, other.dedicated));
    }
}
class StoredCounter {
    constructor(
    @IStorageService
    private readonly _storageService: IStorageService, private readonly _key: string) { }
    get value() {
...
class LanguageStatus {
    ...
    private _update(): void {
        ...
        if (model.combined.length === 0) {
            ...
            if (!this._combinedEntry) {
                this._combinedEntry = this._statusBarService.addEntry(props, LanguageStatus._id, StatusbarAlignment.RIGHT, {
        ...
        for (const status of model.dedicated) {
            ...
            if (!entry) {
                entry = this._statusBarService.addEntry(props, status.id, StatusbarAlignment.RIGHT, {
                    id: "status.editor.mode",
    ...
    private _renderStatus(status: ILanguageStatus, showSeverity: boolean, isPinned: boolean, store: DisposableStore): HTMLElement {
        ...
        const labelValue = typeof status.label === "string"
            ? status.label
            : status.label.value;
    ...
    private static _accessibilityInformation(status: ILanguageStatus): IAccessibilityInformation {
        ...
        const textValue = typeof status.label === "string"
            ? status.label
            : status.label.value;
        if (status.detail) {
            return {
    ...
    private static _asStatusbarEntry(item: ILanguageStatus): IStatusbarEntry {
        ...
        return {
            ...
            tooltip: item.command?.tooltip ||
                new MarkdownString(item.detail, {
                    isTrusted: true,
