...
export class ListSettingWidget<TListDataItem extends IListDataItem> extends AbstractListSettingWidget<TListDataItem> {
    ...
    protected renderItem(item: TListDataItem, idx: number): RowElementGroup {
        ...
        siblingElement.textContent = item.sibling
            ? `when: ${item.sibling}`
            : null;
...
export class ExcludeSettingWidget extends ListSettingWidget<IIncludeExcludeDataItem> {
    ...
    protected override addTooltipsToRow(rowElementGroup: RowElementGroup, item: IIncludeExcludeDataItem): void {
        ...
        const { rowElement } = rowElementGroup;
        this.listDisposables.add(this.hoverService.setupManagedHover(getDefaultHoverDelegate("mouse"), rowElement, {
...
export class ObjectSettingDropdownWidget extends AbstractListSettingWidget<IObjectDataItem> {
    ...
    override setValue(listData: IObjectDataItem[], options?: IObjectSetValueOptions): void {
        ...
        if (isDefined(options) &&
            options.settingKey !== this.currentSettingKey) {
            this.model.setEditKey("none");
    ...
    private renderEditWidget(keyOrValue: ObjectKey | ObjectValue, options: IObjectRenderEditWidgetOptions) {
    ...
    private renderStringEditWidget(keyOrValue: IObjectStringData, { idx, isKey, originalItem, changedItem, update, }: IObjectRenderEditWidgetOptions) {
        const wrapper = $(isKey
    ...
    private renderEnumEditWidget(keyOrValue: IObjectEnumData, { isKey, changedItem, update }: IObjectRenderEditWidgetOptions) {
    ...
    protected addTooltipsToRow(rowElementGroup: RowElementGroup, item: IObjectDataItem): void {
        ...
        const markdownString = {
            markdown: new MarkdownString().appendMarkdown(accessibleDescription),
            markdownNotSupportedFallback: accessibleDescription,
        };
        const keyDescription: string | IManagedHoverTooltipMarkdownString = this.getEnumDescription(item.key) ??
            item.keyDescription ??
            markdownString;
...
export class ObjectSettingCheckboxWidget extends AbstractListSettingWidget<IBoolObjectDataItem> {
    ...
    private renderEditWidget(value: boolean, checkboxDescription: string, onValueChange: (newValue: boolean) => void) {
