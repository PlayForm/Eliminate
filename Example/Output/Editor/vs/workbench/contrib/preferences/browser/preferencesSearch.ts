...
import { IGroupFilter, ISearchResult, ISetting, ISettingMatch, ISettingsEditorModel, ISettingsGroup, SettingMatchType, } from "../../../services/preferences/common/preferences.js";
...
export class PreferencesSearchService extends Disposable implements IPreferencesSearchService {
    ...
    private get remoteSearchAllowed(): boolean {
        const workbenchSettings = this.configurationService.getValue<IWorkbenchSettingsConfiguration>()
            .workbench.settings;
...
export class SettingMatches {
    ...
    constructor(searchString: string, setting: ISetting, requireFullQueryMatch: boolean, private searchDescription: boolean, valuesMatcher: (filter: string, setting: ISetting) => IRange[], 
    @IConfigurationService
    private readonly configurationService: IConfigurationService) {
...
class AiRelatedInformationSearchKeysProvider {
    ...
    constructor(private readonly aiRelatedInformationService: IAiRelatedInformationService) { }
    ...
    private async getAiRelatedInformationItems(token?: CancellationToken | undefined) {
        ...
        for (const info of relatedInformation) {
            if (filterMatches.length ===
                AiRelatedInformationSearchProvider.AI_RELATED_INFORMATION_MAX_PICKS) {
                break;
            }
    ...
    private async getTfIdfItems(token?: CancellationToken | undefined): Promise<ISettingMatch[]> {
        ...
        for (const info of tfIdfRankings) {
            if (info.score / maxScore <
                TfIdfSearchProvider.TF_IDF_POST_NORMALIZE_THRESHOLD ||
...
class RemoteSearchProvider implements IRemoteSearchProvider {
    ...
    searchModel(preferencesModel: ISettingsEditorModel, token?: CancellationToken): Promise<ISearchResult | null> {
        ...
        return this.adaSearchProvider
            .searchModel(preferencesModel, token)
            .then((results) => {
            return results?.filterMatches.length
                ? results
                : this.tfIdfSearchProvider!.searchModel(preferencesModel, token);
        });
    }
}
