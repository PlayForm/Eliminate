...
class EditSettingRenderer extends Disposable {
    ...
    constructor(private editor: ICodeEditor, private primarySettingsModel: ISettingsEditorModel, private settingHighlighter: SettingHighlighter, 
    @IConfigurationService
    private readonly configurationService: IConfigurationService, 
    @IInstantiationService
    ...
    private getSettings(lineNumber: number): IIndexedSetting[] {
        ...
        return this.getSettingsAtLineNumber(lineNumber).filter((setting) => {
            ...
            if (configurationNode) {
                ...
                if (configurationNode.type === "boolean" ||
                    configurationNode.enum) {
                    if ((<SettingsEditorModel>this.primarySettingsModel)
                        .configurationTarget !==
                        ConfigurationTarget.WORKSPACE_FOLDER) {
                        return true;
                    }
                    if (configurationNode.scope ===
    ...
    private getSettingsAtLineNumber(lineNumber: number): IIndexedSetting[] {
        ...
        for (const group of this.settingsGroups) {
            ...
            if (lineNumber >= group.range.startLineNumber &&
    ...
    private toAbsoluteCoords(position: Position): {
        x: number;
        y: number;
    } {
...
class UnsupportedSettingsRenderer extends Disposable implements languages.CodeActionProvider {
    ...
    constructor(private readonly editor: ICodeEditor, private readonly settingsEditorModel: SettingsEditorModel, 
    @IMarkerService
    ...
    private handlePolicyConfiguration(setting: ISetting, configuration: IConfigurationPropertySchema, markerData: IMarkerData[]): boolean {
        ...
        if (this.configurationService.inspect(setting.key).policyValue ===
            undefined) {
            return false;
        }
        if (this.settingsEditorModel.configurationTarget ===
    ...
    private handleLocalUserConfiguration(setting: ISetting, configuration: IConfigurationPropertySchema, markerData: IMarkerData[]): void {
        if (!this.userDataProfileService.currentProfile.isDefault &&
            !this.userDataProfileService.currentProfile.useDefaultFlags
                ?.settings) {
            if (isEqual(this.userDataProfilesService.defaultProfile
                .settingsResource, this.settingsEditorModel.uri) &&
