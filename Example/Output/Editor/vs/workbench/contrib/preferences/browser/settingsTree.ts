...
import { ConfigurationTarget, getLanguageTagSettingPlainKey, IConfigurationService, } from "../../../../platform/configuration/common/configuration.js";
...
import { compareTwoNullableNumbers, LANGUAGE_SETTING_TAG, SETTINGS_EDITOR_COMMAND_SHOW_CONTEXT_MENU, } from "../common/preferences.js";
...
import { getIndicatorsLabelAriaLabel, ISettingOverrideClickEvent, SettingsTreeIndicatorsLabel, } from "./settingsEditorSettingIndicators.js";
import { ITOCEntry } from "./settingsLayout.js";
import { inspectSetting, ISettingsEditorViewState, objectSettingSupportsRemoveDefaultValue, settingKeyToDisplayFormat, SettingsTreeElement, SettingsTreeGroupChild, SettingsTreeGroupElement, SettingsTreeNewExtensionsElement, SettingsTreeSettingElement, } from "./settingsTreeModels.js";
import { ExcludeSettingWidget, IBoolObjectDataItem, IIncludeExcludeDataItem, IListDataItem, IncludeSettingWidget, IObjectDataItem, IObjectEnumOption, IObjectKeySuggester, IObjectValueSuggester, ListSettingWidget, ObjectSettingCheckboxWidget, ObjectSettingDropdownWidget, ObjectValue, SettingListEvent, } from "./settingsWidgets.js";
const $ = DOM.$;
...
function getObjectDisplayValue(element: SettingsTreeSettingElement): IObjectDataItem[] {
    ...
    return Object.keys(data)
        ...
        const removable = defaultValue === undefined ||
...
function createObjectValueSuggester(element: SettingsTreeSettingElement): IObjectValueSuggester {
    ...
    return (key: string) => {
        ...
        const patternSchema = suggestedSchema ??
...
export function resolveSettingsTree(tocData: ITOCEntry<string>, coreSettingsGroups: ISettingsGroup[], logService: ILogService): {
...
export async function createTocTreeForExtensionSettings(extensionService: IExtensionService, groups: ISettingsGroup[]): Promise<ITOCEntry<ISetting>> {
    ...
    const processGroupEntry = async (group: ISettingsGroup) => {
        const flatSettings = group.sections
...
export abstract class AbstractSettingRenderer extends Disposable implements ITreeRenderer<SettingsTreeElement, never, any> {
    ...
    constructor(private readonly settingActions: IAction[], private readonly disposableActionFactory: (setting: ISetting, settingTarget: SettingsTarget) => IAction[], 
    @IThemeService
...
export class SettingComplexRenderer extends AbstractSettingRenderer implements ITreeRenderer<SettingsTreeSettingElement, never, ISettingComplexItemTemplate> {
    ...
    private renderValidations(dataElement: SettingsTreeSettingElement, template: ISettingComplexItemTemplate) {
        const errMsg = dataElement.isConfigured &&
    ...
    protected renderValue(dataElement: SettingsTreeSettingElement, template: ISettingObjectItemTemplate, onChange: (value: Record<string, unknown> | undefined) => void): void {
        ...
        template.objectDropdownWidget!.setValue(items, {
            ...
            showAddButton: objectAdditionalProperties === false
                ? !areAllPropertiesDefined(Object.keys(objectProperties ?? {}), items) || isDefined(objectPatternProperties)
                : true,
...
abstract class SettingIncludeExcludeRenderer extends AbstractSettingRenderer implements ITreeRenderer<SettingsTreeSettingElement, never, ISettingIncludeExcludeItemTemplate> {
    ...
    private onDidChangeIncludeExclude(template: ISettingIncludeExcludeItemTemplate, e: SettingListEvent<IListDataItem>): void {
        if (template.context) {
            ...
            if (e.type !== "add") {
                if (e.originalItem.value.data.toString() in
                    template.context.defaultValue) {
            ...
            if (e.type === "change" || e.type === "add" || e.type === "move") {
                if (e.newItem.value.data.toString() in
                    template.context.defaultValue &&
                    !e.newItem.sibling) {
            ...
            function sortKeys<T extends object>(obj: T) {
                const sortedKeys = Object.keys(obj).sort((a, b) => a.localeCompare(b)) as Array<keyof T>;
            ...
            this._onDidChangeSetting.fire({
                ...
                value: Object.keys(newValue).length === 0
                    ? undefined
                    : sortKeys(newValue),
                type: template.context.valueType,
                manualReset: false,
    ...
    protected renderValue(dataElement: SettingsTreeSettingElement, template: ISettingEnumItemTemplate, onChange: (value: string) => void): void {
        ...
        const displayOptions: ISelectOptionItem[] = settingEnum
            ...
            return {
                ...
                decoratorRight: data === stringifiedDefaultValue ||
...
class SettingNumberRenderer extends AbstractSettingRenderer implements ITreeRenderer<SettingsTreeSettingElement, never, ISettingNumberItemTemplate> {
    ...
    protected renderValue(dataElement: SettingsTreeSettingElement, template: ISettingNumberItemTemplate, onChange: (value: number | null) => void): void {
        const numParseFn = dataElement.valueType === "integer" ||
            dataElement.valueType === "nullable-integer"
            ? parseInt
            : parseFloat;
        const nullNumParseFn = dataElement.valueType === "nullable-integer" ||
    ...
    protected renderValue(dataElement: SettingsTreeSettingElement, template: ISettingExtensionToggleItemTemplate, onChange: (_: undefined) => void): void {
        ...
        template.elementDisposables.add(template.actionButton.onDidClick(async () => {
            this._telemetryService.publicLog2<{
                extensionId: String;
            }, ManageExtensionClickTelemetryClassification>("ManageExtensionClick", { extensionId });
        ...
        template.elementDisposables.add(template.dismissButton.onDidClick(async () => {
            this._telemetryService.publicLog2<{
                extensionId: String;
            }, ManageExtensionClickTelemetryClassification>("DismissExtensionClick", { extensionId });
...
export class SettingTreeRenderers extends Disposable {
    ...
    private getActionsForSetting(setting: ISetting, settingTarget: SettingsTarget): IAction[] {
        ...
        if (this._userDataSyncEnablementService.isEnabled() &&
    ...
    getKeyForDOMElementInSetting(element: HTMLElement): string | null {
        ...
        return (settingElement &&
            settingElement.getAttribute(AbstractSettingRenderer.SETTING_KEY_ATTR));
    }
 ...
 */
function renderArrayValidations(dataElement: SettingsTreeSettingElement, template: ISettingListItemTemplate | ISettingObjectItemTemplate, value: string[] | Record<string, unknown> | undefined, calledOnStartup: boolean): boolean {
    template.containerElement.classList.add("invalid-input");
    if (dataElement.setting.validator) {
...
class SettingsTreeDelegate extends CachedListVirtualDelegate<SettingsTreeGroupChild> {
    getTemplateId(element: SettingsTreeGroupElement | SettingsTreeSettingElement | SettingsTreeNewExtensionsElement): string {
        ...
        if (element instanceof SettingsTreeSettingElement) {
            ...
            const invalidTypeError = element.isConfigured &&
...
export class SettingsTree extends WorkbenchObjectTree<SettingsTreeElement> {
    constructor(container: HTMLElement, viewState: ISettingsEditorViewState, renderers: ITreeRenderer<any, void, any>[], 
    @IContextKeyService
...
class SyncSettingAction extends Action {
    ...
    constructor(private readonly setting: ISetting, 
    @IConfigurationService
    ...
    override async run(): Promise<void> {
        ...
        let currentValue = [
            ...this.configService.getValue<string[]>("settingsSync.ignoredSettings"),
        ];
