...
export class FolderSettingsActionViewItem extends BaseActionViewItem {
    ...
    override render(container: HTMLElement): void {
        ...
        this.dropDownElement = DOM.$(".dropdown-icon.hide" +
    ...
    private onWorkspaceFoldersChanged(): void {
        ...
        if (oldFolder) {
            this._folder =
                workspace.folders.filter((folder) => isEqual(folder.uri, oldFolder.uri))[0] || workspace.folders[0];
        }
        ...
        if (this.contextService.getWorkbenchState() ===
            ...
            actions.push(...workspaceFolders.map((folder, index) => {
                ...
                return {
                    ...
                    checked: !!this.folder &&
                        isEqual(this.folder.uri, folder.uri),
                    enabled: true,
                    class: undefined,
...
export class SettingsTargetsWidget extends Widget {
    ...
    constructor(parent: HTMLElement, options: ISettingsTargetsWidgetOptions | undefined, 
    @IWorkspaceContextService
    private readonly contextService: IWorkspaceContextService, 
    ...
    private resetLabels() {
        ...
        const hostLabel = remoteAuthority &&
            this.labelService.getHostLabel(Schemas.vscodeRemote, remoteAuthority);
    ...
    private create(parent: HTMLElement): void {
        ...
        this.settingsSwitcherBar = this._register(new ActionBar(settingsTabsWidget, {
            ...
            ariaRole: "tablist",
            actionViewItemProvider: (action: IAction, options: IActionViewItemOptions) => action.id === "folderSettings"
                ? this.folderSettings
                : undefined,
        }));
        ...
        this.folderSettingsAction = new Action("folderSettings", "", ".settings-tab", false, async (folder) => {
            this.updateTarget(isWorkspaceFolder(folder)
                ? folder.uri
    ...
    set settingsTarget(settingsTarget: SettingsTarget | null) {
        ...
        this.userLocalSettings.checked =
            ConfigurationTarget.USER_LOCAL === this.settingsTarget;
        this.userRemoteSettings.checked =
            ConfigurationTarget.USER_REMOTE === this.settingsTarget;
        this.workspaceSettings.checked =
            ConfigurationTarget.WORKSPACE === this.settingsTarget;
    ...
    private onWorkbenchStateChanged(): void {
        ...
        this.update();
        if (this.settingsTarget === ConfigurationTarget.WORKSPACE &&
    ...
    private async update(): Promise<void> {
        ...
        this.userRemoteSettings.enabled = !!(this.options.enableRemoteSettings &&
...
export class SearchWidget extends Widget {
    ...
    private getControlsWidth(): number {
        const countWidth = this.countElement
