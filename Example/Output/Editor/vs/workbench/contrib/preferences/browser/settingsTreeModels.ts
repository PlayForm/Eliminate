...
import { compareTwoNullableNumbers, ENABLE_EXTENSION_TOGGLE_SETTINGS, ENABLE_LANGUAGE_FILTER, MODIFIED_SETTING_TAG, POLICY_SETTING_TAG, REQUIRE_TRUSTED_WORKSPACE_SETTING_TAG, } from "../common/preferences.js";
...
export class SettingsTreeSettingElement extends SettingsTreeElement {
    ...
    private initSettingDescription() {
        if (this.setting.description.length >
    ...
    private initSettingValueType() {
        if (isExtensionToggleSetting(this.setting, this.productService)) {
            ...
            if (this.setting.editPresentation ===
    ...
    private update(inspectResult: IInspectResult, isWorkspaceTrusted: boolean): void {
        ...
        if (inspected.policyValue) {
            ...
            const overrideValueSource = source instanceof Map
    ...
    matchesAnyExtension(extensionFilters?: Set<string>): boolean {
        ...
        return Array.from(extensionFilters).some((extensionId) => extensionId.toLowerCase() ===
...
export class SettingsTreeModel {
    ...
    private updateRequireTrustedTargetElements(): void {
        this.reinspectSettings([...this._treeElementsBySettingName.values()]
            .flat()
...
export function inspectSetting(key: string, target: SettingsTarget, languageFilter: string | undefined, configurationService: IWorkbenchConfigurationService): IInspectResult {
    ...
    const targetSelector = target === ConfigurationTarget.APPLICATION
        ? "applicationValue"
    ...
    const targetOverrideSelector = target === ConfigurationTarget.APPLICATION
        ? "application"
        : target === ConfigurationTarget.USER_LOCAL
            ? "userLocal"
            : target === ConfigurationTarget.USER_REMOTE
...
export function settingKeyToDisplayFormat(key: string, groupId: string = "", isLanguageTagSetting: boolean = false): {
    category: string;
    label: string;
} {
...
function isObjectRenderableSchema({ type }: IJSONSchema, key: string): boolean {
    ...
    if (objectSettingSupportsRemoveDefaultValue(key) &&
        Array.isArray(type) &&
        type.length === 2) {
...
function isObjectSetting({ key, type, objectProperties, objectPatternProperties, objectAdditionalProperties, }: ISetting): boolean {
    if (type !== "object") {
        return false;
    }
    ...
    if (objectAdditionalProperties &&
        typeof objectAdditionalProperties === "object") {
        schemas.push(objectAdditionalProperties);
    }
    // Flatten anyof schemas
    const flatSchemas = schemas
        .map((schema): IJSONSchema[] => {
...
export class SearchResultModel extends SettingsTreeModel {
    ...
    updateChildren(): void {
        ...
        this.root.children = this.root.children.filter((child) => child instanceof SettingsTreeSettingElement &&
