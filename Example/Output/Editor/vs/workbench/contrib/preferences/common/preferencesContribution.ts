    ...
    private handleSettingsEditorRegistration(): void {
        ...
        if (!!this.configurationService.getValue(USE_SPLIT_JSON_SETTING) ||
            ...
            this.editorOpeningListener =
                this.editorResolverService.registerEditor("**/settings.json", {
                    ...
                    createEditorInput: ({ resource, options, }): EditorInputWithOptions => {
                        ...
                        if (isEqual(resource, this.userDataProfileService.currentProfile
                            .settingsResource)) {
                            return {
                                editor: this.preferencesService.createSplitJsonEditorInput(ConfigurationTarget.USER_LOCAL, resource),
                                options,
                            };
                        }
                        ...
                        if (state === WorkbenchState.FOLDER) {
                            ...
                            if (isEqual(resource, folders[0].toResource(FOLDER_SETTINGS_PATH))) {
                                return {
                                    editor: this.preferencesService.createSplitJsonEditorInput(ConfigurationTarget.WORKSPACE, resource),
                                    options,
                                };
                            }
                        }
                        // Multi Folder Workspace Settings File
                            ...
                            for (const folder of folders) {
                                if (isEqual(resource, folder.toResource(FOLDER_SETTINGS_PATH))) {
                                    return {
                                        editor: this.preferencesService.createSplitJsonEditorInput(ConfigurationTarget.WORKSPACE_FOLDER, resource),
                                        options,
                                    };
                                }
                            }
                        ...
                        return {
                            editor: this.textEditorService.createTextEditor({ resource }),
                            options,
                        };
                    },
                });
