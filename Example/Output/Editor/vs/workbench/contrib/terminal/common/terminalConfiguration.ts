...
const terminalDescriptors = "\n- " +
    [
        ...
        "`${local}`: " +
            localize("local", "indicates a local terminal in a remote workspace"),
        "`${process}`: " +
            localize("process", "the name of the terminal process"),
        "`${separator}`: " +
            localize("separator", "a conditional separator {0} that only shows when surrounded by variables with values or static text.", "(` - `)"),
        "`${sequence}`: " +
            localize("sequence", "the name provided to the terminal by the process"),
        "`${task}`: " +
            localize("task", "indicates this terminal is associated with a task"),
