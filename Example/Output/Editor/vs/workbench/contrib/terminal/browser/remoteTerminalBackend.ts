...
import { REMOTE_TERMINAL_CHANNEL_NAME, RemoteTerminalChannelClient, } from "../common/remote/remoteTerminalChannel.js";
...
class RemoteTerminalBackend extends BaseTerminalBackend implements ITerminalBackend {
    ...
    private readonly _onRestoreCommands = this._register(new Emitter<{
        id: number;
        commands: ISerializedTerminalCommand[];
    }>());
    readonly onRestoreCommands = this._onRestoreCommands.event;
    constructor(readonly remoteAuthority: string | undefined, private readonly _remoteTerminalChannel: RemoteTerminalChannelClient, 
    @IRemoteAgentService
    ...
    async createProcess(shellLaunchConfig: IShellLaunchConfig, cwd: string, // TODO: This is ignored
    cols: number, rows: number, unicodeVersion: "6" | "11", env: IProcessEnvironment, // TODO: This is ignored
    options: ITerminalProcessOptions, shouldPersist: boolean): Promise<ITerminalChildProcess> {
        if (!this._remoteTerminalChannel) {
    ...
    async getWslPath(original: string, direction: "unix-to-win" | "win-to-unix"): Promise<string> {
        ...
        return (this._remoteTerminalChannel.getWslPath(original, direction) ||
            original);
    }
    ...
    async getTerminalLayoutInfo(): Promise<ITerminalsLayoutInfo | undefined> {
        ...
        if (reviveBufferState && reviveBufferState.length > 0) {
            try {
                ...
                this._logService.warn("RemoteTerminalBackend#getTerminalLayoutInfo Error", e && typeof e === "object" && "message" in e
                    ? e.message
                    : e);
            }
        }
