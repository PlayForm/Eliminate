...
export class TerminalTabList extends WorkbenchList<ITerminalInstance> {
    ...
    private readonly _hoverService: IHoverService) {
        ...
        this.disposables.add(this.onMouseDblClick(async (e) => {
            ...
            if (this._terminalService.getEditingTerminal()?.instanceId ===
                e.element?.instanceId) {
                return;
            }
            if (this._getFocusMode() === "doubleClick" &&
    ...
    private _updateContextKey() {
        ...
        this._isSplitContextKey.set(instance.length > 0 &&
...
class TerminalTabsRenderer implements IListRenderer<ITerminalInstance, ITerminalTabEntryTemplate> {
    ...
    constructor(private readonly _container: HTMLElement, private readonly _labels: ResourceLabels, private readonly _getSelection: () => ITerminalInstance[], 
    @IInstantiationService
            ...
            hoverDelegate: {
                ...
                showHover: (options) => {
                    return this._hoverService.showHover({
                        ...
                        position: {
                            hoverPosition: this._terminalConfigurationService.config.tabs
                                .location === "left"
                                ? HoverPosition.RIGHT
                                : HoverPosition.LEFT,
                        },
