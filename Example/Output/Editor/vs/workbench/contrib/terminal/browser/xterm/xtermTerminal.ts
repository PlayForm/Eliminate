...
function getFullBufferLineAsString(lineIndex: number, buffer: IBuffer): {
...
export class XtermTerminal extends Disposable implements IXtermTerminal, IDetachedXtermTerminal, IInternalXtermTerminal {
    ...
    private readonly _onDidRequestRunCommand = this._register(new Emitter<{
        command: ITerminalCommand;
        noNewLine?: boolean;
    }>());
    readonly onDidRequestRunCommand = this._onDidRequestRunCommand.event;
    private readonly _onDidRequestCopyAsHtml = this._register(new Emitter<{
        command: ITerminalCommand;
    }>());
    ...
    private readonly _onDidChangeFindResults = this._register(new Emitter<{
        resultIndex: number;
        resultCount: number;
    }>());
     ...
     */
    constructor(xtermCtor: typeof RawXtermTerminal, options: IXtermTerminalOptions, 
    ...
    layoutService: ILayoutService) {
        super();
        this._xtermAddonLoader =
            options.xtermAddonImpoter ?? new XtermAddonImporter();
        ...
        this._xtermAddonLoader
            ...
            this._clipboardAddon =
                this._instantiationService.createInstance(ClipboardAddon, undefined, {
    ...
    *getBufferReverseIterator(): IterableIterator<string> {
    ...
    private _shouldLoadWebgl(): boolean {
        return ((this._terminalConfigurationService.config.gpuAcceleration ===
            "auto" &&
            XtermTerminal._suggestedRendererType === undefined) ||
    ...
    private _getSearchAddon(): Promise<SearchAddonType> {
        if (!this._searchAddonPromise) {
            this._searchAddonPromise = this._xtermAddonLoader
                .importAddon("search")
                .then((AddonCtor) => {
                if (this._store.isDisposed) {
    ...
    private _getWrappedLineCount(index: number, buffer: IBuffer): {
        lineCount: number;
        currentIndex: number;
        endSpaces: number;
    } {
    ...
    private async _refreshImageAddon(): Promise<void> {
        ...
        if (this._terminalConfigurationService.config.enableImages &&
            this._webglAddon) {
            if (!this._imageAddon) {
    ...
    private async _updateUnicodeVersion(): Promise<void> {
        ...
        if (this.raw.unicode.activeVersion !==
