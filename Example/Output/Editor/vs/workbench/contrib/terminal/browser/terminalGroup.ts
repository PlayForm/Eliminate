...
import { IView, Orientation, Sizing, SplitView, } from "../../../../base/browser/ui/splitview/splitview.js";
...
class SplitPaneContainer extends Disposable {
    ...
    resizePanes(relativeSizes: number[]): void {
        ...
        relativeSizes[relativeSizes.length - 1] +=
            1 -
                relativeSizes.reduce((totalValue, currentValue) => totalValue + currentValue, 0);
        let totalSize = 0;
        for (let i = 0; i < this._splitView.length; i++) {
...
export class TerminalGroup extends Disposable implements ITerminalGroup {
    ...
    constructor(private _container: HTMLElement | undefined, shellLaunchConfigOrInstance: IShellLaunchConfig | ITerminalInstance | undefined, 
    ...
    private readonly _instantiationService: IInstantiationService) {
        ...
        this._onPanelOrientationChanged.fire(this._terminalLocation === ViewContainerLocation.Panel &&
    ...
    getLayoutInfo(isActive: boolean): ITerminalTabLayoutInfoById {
        const instances = this.terminalInstances.filter((instance) => typeof instance.persistentProcessId === "number" &&
    ...
    private _removeInstance(instance: ITerminalInstance) {
        ...
        if (wasActiveInstance && this._terminalInstances.length > 0) {
            const newIndex = index < this._terminalInstances.length
                ? index
    ...
    attachToElement(element: HTMLElement): void {
        ...
        if (!this._splitPaneContainer) {
            ...
            this._splitPaneContainer =
                this._instantiationService.createInstance(SplitPaneContainer, this._groupElement, orientation);
    ...
    private _getBellTitle(instance: ITerminalInstance) {
        if (this._terminalConfigurationService.config.enableBell &&
            instance.statusList.statuses.some((e) => e.id === TerminalStatus.Bell)) {
            return "*";
        }
        return "";
    }
    ...
    resizePane(direction: Direction): void {
        ...
        const shouldResizePart = (isHorizontalResize && groupOrientation === Orientation.VERTICAL) ||
            (!isHorizontalResize &&
        ...
        if (charSize) {
            ...
            if (shouldResizePart) {
                ...
                const shouldShrink = (position === Position.LEFT &&
