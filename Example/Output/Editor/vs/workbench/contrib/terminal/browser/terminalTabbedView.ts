...
export class TerminalTabbedView extends Disposable {
    ...
    private readonly _hoverService: IHoverService) {
        ...
        this._tabTreeIndex =
            this._terminalConfigurationService.config.tabs.location === "left"
                ? 0
                : 1;
        ...
        this._register(_configurationService.onDidChangeConfiguration((e) => {
            if (e.affectsConfiguration(TerminalSettingId.TabsEnabled) ||
                ...
                this._tabTreeIndex =
                    this._terminalConfigurationService.config.tabs
                        .location === "left"
                        ? 0
                        : 1;
                this._terminalContainerIndex =
    ...
    private _shouldShowTabs(): boolean {
        ...
        if (hide === "singleGroup" &&
            this._terminalGroupService.groups.length > 1) {
            return true;
        }
        return false;
    }
    private _refreshShowTabs() {
        if (this._shouldShowTabs()) {
            ...
            if (this._splitView.length === 2 &&
                !this._terminalTabsMouseContextKey.get()) {
    ...
    private _getLastListWidth(): number {
        const widthKey = this._panelOrientation === Orientation.VERTICAL
            ? TerminalStorageKeys.TabsListWidthVertical
    ...
    private _getAdditionalWidth(instance: ITerminalInstance): number {
        ...
        const statusIconWidth = instance.statusList.statuses.length > 0
            ? WidthConstants.StatusIcon
            : 0;
        const splitAnnotationWidth = (this._terminalGroupService.getGroupForInstance(instance)
            ?.terminalInstances.length || 0) > 1
            ? WidthConstants.SplitAnnotation
            : 0;
    ...
    private _updateListWidth(width: number): void {
        if (width < TerminalTabsListSizes.MidpointViewWidth &&
            width >= TerminalTabsListSizes.NarrowViewWidth) {
            width = TerminalTabsListSizes.NarrowViewWidth;
            this._splitView.resizeView(this._tabTreeIndex, width);
        }
        else if (width >= TerminalTabsListSizes.MidpointViewWidth &&
    ...
    private _updateHasText() {
        const hasText = this._tabListElement.clientWidth >
            TerminalTabsListSizes.MidpointViewWidth;
    ...
    private _attachEventListeners(parentDomElement: HTMLElement, terminalContainer: HTMLElement): void {
        ...
        this._register(dom.addDisposableListener(terminalContainer, "contextmenu", (event: MouseEvent) => {
            const rightClickBehavior = this._terminalConfigurationService.config
                .rightClickBehavior;
        ...
        this._register(dom.addDisposableListener(this._tabContainer, "contextmenu", (event: MouseEvent) => {
            ...
            if (!this._cancelContextMenu) {
                ...
                if (!emptyList) {
                    this._terminalGroupService.lastAccessedMenu =
                        "tab-list";
                }
                ...
                if (focusedInstance) {
                    selectedInstances.splice(selectedInstances.findIndex((e) => e.instanceId ===
                        focusedInstance.instanceId), 1);
                    selectedInstances.unshift(focusedInstance);
                }
                openContextMenu(dom.getWindow(this._tabContainer), event, selectedInstances, emptyList
                    ? this._tabsListEmptyMenu
                    : this._tabsListMenu, this._contextMenuService, emptyList ? this._getTabActions() : undefined);
            }
            event.preventDefault();
            event.stopImmediatePropagation();
            this._cancelContextMenu = false;
    ...
    private _getTabActions(): IAction[] {
        return [
            new Separator(),
            this._configurationService.inspect(TerminalSettingId.TabsLocation)
                .userValue === "left"
    ...
    focus() {
        if (this._terminalService.connectionState ===
