...
export async function getCwdForSplit(instance: ITerminalInstance, folders: IWorkspaceFolder[] | undefined, commandService: ICommandService, configService: ITerminalConfigurationService): Promise<string | URI | undefined> {
...
export const terminalSendSequenceCommand = async (accessor: ServicesAccessor, args: unknown) => {
    ...
    if (instance) {
        const text = isObject(args) && "text" in args
...
export function registerTerminalAction(options: IAction2Options & {
    ...
    delete (strictOptions as IAction2Options & {
        run?: (c: ITerminalServicesCollection, accessor: ServicesAccessor, args?: unknown) => void | Promise<unknown>;
    })["run"];
    // Register
 ...
 */
export function registerContextualInstanceAction(options: IAction2Options & {
    /**
 ...
 */
export function registerActiveInstanceAction(options: IAction2Options & {
 ...
 */
export function registerActiveXtermAction(options: IAction2Options & {
...
export function registerTerminalActions() {
    ...
    registerTerminalAction({
        ...
        run: async (c, _, args) => {
            const cwd = isObject(args) && "cwd" in args
    ...
    registerActiveInstanceAction({
        ...
        run: async (activeInstance, c, accessor, args) => {
            const notificationService = accessor.get(INotificationService);
            const name = isObject(args) && "name" in args
    ...
    newWithProfileAction = registerAction2(class extends Action2 {
        ...
        async run(accessor: ServicesAccessor, eventOrOptionsOrProfile: MouseEvent | ICreateTerminalOptions | ITerminalProfile | {
            ...
            if (isObject(eventOrOptionsOrProfile) &&
                ...
                const config = c.profileService.availableProfiles.find((profile) => profile.profileName ===
...
function getResourceOrActiveInstance(c: ITerminalServicesCollection, resource: unknown): ITerminalInstance | undefined {
    return (c.service.getInstanceFromResource(toOptionalUri(resource)) ||
        c.service.activeInstance);
}
async function pickTerminalCwd(accessor: ServicesAccessor, cancel?: CancellationToken): Promise<WorkspaceFolderCwdPair | undefined> {
    ...
    const folderPicks: Item[] = shrinkedPairs.map((pair) => {
        ...
        const description = pair.isOverridden
            ? localize("workbench.action.terminal.overriddenCwdDescription", "(Overriden) {0}", labelService.getUriLabel(pair.cwd, {
...
async function resolveWorkspaceFolderCwd(folder: IWorkspaceFolder, configurationService: IConfigurationService, configurationResolverService: IConfigurationResolverService): Promise<WorkspaceFolderCwdPair> {
    ...
    return isAbsolute(resolvedCwdConfig) ||
        resolvedCwdConfig.startsWith(AbstractVariableResolverService.VARIABLE_LHS)
        ? {
            folder,
            isAbsolute: true,
            isOverridden: true,
