...
export class TerminalService extends Disposable implements ITerminalService {
    ...
    @IRemoteAgentService
    private _remoteAgentService: IRemoteAgentService, 
    @IViewsService
    ...
    private readonly _timerService: ITimerService) {
        ...
        this._processSupportContextKey =
            TerminalContextKeys.processSupported.bindTo(this._contextKeyService);
        this._processSupportContextKey.set(!isWeb || this._remoteAgentService.getConnection() !== null);
        this._terminalHasBeenCreated =
            TerminalContextKeys.terminalHasBeenCreated.bindTo(this._contextKeyService);
        this._terminalCountContextKey = TerminalContextKeys.count.bindTo(this._contextKeyService);
        this._terminalEditorActive =
            TerminalContextKeys.terminalEditorActive.bindTo(this._contextKeyService);
        this._register(this.onDidChangeActiveInstance((instance) => {
            this._terminalEditorActive.set(!!instance?.target &&
    ...
    private async _initializePrimaryBackend() {
        ...
        const isPersistentRemote = !!this._environmentService.remoteAuthority &&
        ...
        reconnectedPromise.then(async () => {
            ...
            await Promise.all(Array.from(this._terminalInstanceService.getRegisteredBackends()).map(async (backend) => {
                this._timerService.setPerformanceMarks(backend.remoteAuthority === undefined
                    ? "localPtyHost"
                    : "remotePtyHost", await backend.getPerformanceMarks());
                backend.setReady();
            }));
    ...
    private async _onBeforeShutdownAsync(reason: ShutdownReason): Promise<boolean> {
        ...
        try {
            ...
            const shouldPersistProcesses = this._terminalConfigurationService.config
                .enablePersistentSessions &&
                reason === ShutdownReason.RELOAD;
            if (!shouldPersistProcesses) {
                const hasDirtyInstances = (this._terminalConfigurationService.config.confirmOnExit ===
                    "always" &&
    ...
    private _shouldReviveProcesses(reason: ShutdownReason): boolean {
        ...
        switch (this._terminalConfigurationService.config
            ...
            case "onExit": {
                ...
                return (reason === ShutdownReason.LOAD ||
    ...
    private _onWillShutdown(e: WillShutdownEvent): void {
        ...
        const shouldPersistTerminals = this._terminalConfigurationService.config
    ...
    private _updateTitle(instance: ITerminalInstance | undefined): void {
        if (!this._terminalConfigurationService.config
            .enablePersistentSessions ||
            !instance ||
    ...
    async moveToTerminalView(source?: ITerminalInstance | URI, target?: ITerminalInstance, side?: "before" | "after"): Promise<void> {
        ...
        await this._terminalGroupService.showPanel(true);
        if (target && side) {
            const index = group.terminalInstances.indexOf(target) +
                (side === "after" ? 1 : 0);
    ...
    async createTerminal(options?: ICreateTerminalOptions): Promise<ITerminalInstance> {
        ...
        const splitActiveTerminal = typeof options?.location === "object" &&
        ...
        const location = (await this.resolveLocation(options?.location)) ||
            this.defaultLocation;
...
class TerminalEditorStyle extends Themable {
    ...
    private _registerListeners(): void {
        ...
        this._register(this._editorService.onDidActiveEditorChange(() => {
            if (this._editorService.activeEditor instanceof
                TerminalEditorInput) {
                this.updateStyles();
            }
        }));
