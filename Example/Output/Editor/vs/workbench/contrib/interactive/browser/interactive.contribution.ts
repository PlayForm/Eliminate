...
Registry.as<IEditorPaneRegistry>(EditorExtensions.EditorPane).registerEditorPane(EditorPaneDescriptor.create(InteractiveEditor, INTERACTIVE_WINDOW_EDITOR_ID, "Interactive Window"), [new SyncDescriptor(InteractiveEditorInput)]);
export class InteractiveDocumentContribution extends Disposable implements IWorkbenchContribution {
    ...
    private readonly instantiationService: IInstantiationService) {
        ...
        editorResolverService.registerEditor(`${Schemas.vscodeInteractiveInput}:/**`, {
            ...
            createEditorInput: ({ resource }) => {
                const editorInput = editorService
                    ...
                    .find((editor) => editor.editor instanceof
        ...
        editorResolverService.registerEditor(`*.interactive`, {
            ...
            canSupportResource: (uri) => (uri.scheme === Schemas.untitled &&
...
function createEditor(resource: URI, instantiationService: IInstantiationService): EditorInput {
    ...
    const inputBoxPath = counter && counter[1]
...
class InteractiveWindowWorkingCopyEditorHandler extends Disposable implements IWorkbenchContribution, IWorkingCopyEditorHandler {
    ...
    isOpen(workingCopy: IWorkingCopyIdentifier, editor: EditorInput): boolean {
        ...
        return (editor instanceof InteractiveEditorInput &&
...
export class InteractiveEditorSerializer implements IEditorSerializer {
    ...
    canSerialize(editor: EditorInput): editor is InteractiveEditorInput {
        ...
        return (URI.isUri(editor.primary.resource) &&
...
Registry.as<IEditorFactoryRegistry>(EditorExtensions.EditorFactory).registerEditorSerializer(InteractiveEditorSerializer.ID, InteractiveEditorSerializer);
...
registerAction2(class extends Action2 {
    ...
    async run(accessor: ServicesAccessor, showOptions?: number | {
        viewColumn?: number;
        preserveFocus?: boolean;
    }, resource?: URI, id?: string, title?: string): Promise<{
        notebookUri: URI;
        inputUri: URI;
        ...
        const group = columnToEditorGroup(editorGroupService, configurationService, typeof showOptions === "number"
            ? showOptions
            : showOptions?.viewColumn);
        const editorOptions = {
            activation: EditorActivation.PRESERVE,
            preserveFocus: typeof showOptions !== "number"
...
registerAction2(class extends Action2 {
    ...
    async run(accessor: ServicesAccessor, context?: UriComponents): Promise<void> {
        ...
        if (editorControl &&
            isReplEditorControl(editorControl) &&
            editorControl.notebookEditor) {
            const notebookDocument = editorControl.notebookEditor.textModel;
            const textModel = editorControl.activeCodeEditor?.getModel();
            const activeKernel = editorControl.notebookEditor.activeKernel;
            const language = activeKernel?.supportedLanguages[0] ??
                PLAINTEXT_LANGUAGE_ID;
            if (notebookDocument &&
                textModel &&
                editorControl.activeCodeEditor) {
    ...
    async run(accessor: ServicesAccessor): Promise<void> {
        ...
        if (editorControl &&
            ...
            const interactiveWindow = Iterable.find(openEditors, (identifier) => {
                return (identifier.editor.typeId ===
