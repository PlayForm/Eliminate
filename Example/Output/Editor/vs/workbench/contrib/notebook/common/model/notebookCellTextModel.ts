...
export class NotebookCellTextModel extends Disposable implements ICell {
    ...
    set internalMetadata(newInternalMetadata: NotebookCellInternalMetadata) {
        const lastRunSuccessChanged = this._internalMetadata.lastRunSuccess !==
    ...
    get textBuffer() {
        ...
        this._textBuffer = this._register(createTextBuffer(this._source, model.DefaultEndOfLine.LF)
            .textBuffer);
    ...
    private setRegisteredLanguage(languageService: ILanguageService, newLanguage: string, currentLanguage: string) {
        ...
        if (!languageService.isRegisteredLanguageId(currentLanguage) &&
            isFallBackLanguage) {
    ...
    private _outputNotEqualFastCheck(left: ICellOutput[], right: ICellOutput[]) {
        ...
        for (let i = 0; i < this.outputs.length; i++) {
            ...
            for (let k = 0; k < l.outputs.length; k++) {
                ...
                if (l.outputs[k].data.byteLength !==
    ...
    fastEqual(b: ICellDto2): boolean {
        ...
        if (this.internalMetadata?.executionOrder !==
