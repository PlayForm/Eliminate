...
import { CellEditType, CellKind, CellUri, diff, ICell, ICellDto2, ICellEditOperation, ICellOutput, INotebookTextModel, IOutputDto, IOutputItemDto, ISelectionState, NotebookCellCollapseState, NotebookCellDefaultCollapseConfig, NotebookCellInternalMetadata, NotebookCellMetadata, NotebookCellOutputsSplice, NotebookCellsChangeType, NotebookCellTextModelSplice, NotebookDocumentMetadata, NotebookTextModelChangedEvent, NotebookTextModelWillAddRemoveEvent, NullablePartialNotebookCellInternalMetadata, NullablePartialNotebookCellMetadata, TransientOptions, } from "../notebookCommon.js";
...
class StackOperation implements IWorkspaceUndoRedoElement {
    ...
    constructor(readonly textModel: NotebookTextModel, readonly undoRedoGroup: UndoRedoGroup | undefined, private _pauseableEmitter: PauseableEmitter<NotebookTextModelChangedEvent>, private _postUndoRedo: (alternativeVersionId: string) => void, selectionState: ISelectionState | undefined, beginAlternativeVersionId: string) {
...
class NotebookOperationManager {
    ...
    constructor(private readonly _textModel: NotebookTextModel, private _undoService: IUndoRedoService, private _pauseableEmitter: PauseableEmitter<NotebookTextModelChangedEvent>, private _postUndoRedo: (alternativeVersionId: string) => void) { }
    isUndoStackEmpty(): boolean {
        return (this._pendingStackOperation === null ||
...
export class NotebookTextModel extends Disposable implements INotebookTextModel {
    ...
    constructor(readonly viewType: string, readonly uri: URI, cells: ICellDto2[], metadata: NotebookDocumentMetadata, options: TransientOptions, 
    @IUndoRedoService
    private readonly _undoService: IUndoRedoService, 
    @IModelService
    ...
    private readonly _languageDetectionService: ILanguageDetectionService) {
        ...
        this._operationManager = new NotebookOperationManager(this, this._undoService, this._pauseableEmitter, (alternativeVersionId: string) => {
    ...
    private _getCellIndexWithOutputIdHandleFromEdits(outputId: string, rawEdits: ICellEditOperation[]) {
        const edit = rawEdits.find((e) => "outputs" in e &&
    ...
    reset(cells: ICellDto2[], metadata: NotebookDocumentMetadata, transientOptions: TransientOptions): void {
        ...
        this.applyEdits([...edits, { editType: CellEditType.DocumentMetadata, metadata }], true, undefined, () => undefined, undefined, false);
    }
    static computeEdits(model: NotebookTextModel, cells: ICellDto2[]) {
        ...
        if (model.cells.length === cells.length &&
    ...
    private static _commonSuffix(a: readonly NotebookCellTextModel[], aLen: number, aDelta: number, b: ICellDto2[], bLen: number, bDelta: number): number {
        ...
        for (let i = 0; i < maxResult &&
            a[aDelta + aLen - i - 1].fastEqual(b[bDelta + bLen - i - 1]); i++) {
            result++;
        }
        return result;
    }
        ...
        const editsWithDetails = rawEdits
            ...
            if ("index" in edit) {
                ...
                if (this._indexIsInvalid(cellIndex)) {
                    ...
                    cellIndex =
                        this._getCellIndexWithOutputIdHandleFromEdits(edit.outputId, rawEdits.slice(0, index));
                }
                if (this._indexIsInvalid(cellIndex)) {
    ...
    private _isDocumentMetadataChanged(a: NotebookDocumentMetadata, b: NotebookDocumentMetadata) {
        ...
        for (const key of keys) {
            ...
            else if (a[key as keyof NotebookDocumentMetadata] !==
    ...
    private _isCellMetadataChanged(a: NotebookCellMetadata, b: NotebookCellMetadata) {
        ...
        for (const key of keys) {
            if (a[key as keyof NotebookCellMetadata] !==
