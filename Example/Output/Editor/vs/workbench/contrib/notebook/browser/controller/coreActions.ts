...
import { cellRangeToViewCells, getNotebookEditorFromEditorPane, IActiveNotebookEditor, ICellOutputViewModel, ICellViewModel, } from "../notebookBrowser.js";
...
export abstract class NotebookMultiCellAction extends Action2 {
    ...
    async run(accessor: ServicesAccessor, ...additionalArgs: any[]): Promise<void> {
        ...
        if (editor) {
            const selectedCellRange: ICellRange[] = editor.getSelections().length === 0
                ? [editor.getFocus()]
...
export function parseMultiCellExecutionArgs(accessor: ServicesAccessor, ...args: any[]): INotebookCommandContext | undefined {
    ...
    if (isMultiCellArgs(firstArg)) {
        ...
        const selectedCells = ranges
            .map((range) => editor.getCellsInRange(range).slice(0))
            .flat();
