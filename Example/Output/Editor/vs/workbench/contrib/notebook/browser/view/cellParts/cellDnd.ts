...
export class CellDragAndDropController extends Disposable {
    ...
    constructor(private notebookEditor: INotebookEditorDelegate, private readonly notebookListContainer: HTMLElement) {
        super();
    ...
    private onCellDragover(event: CellDragEvent): void {
        ...
        const insertionIndicatorAbsolutePos = dropDirection === "above"
            ? event.cellTop
    ...
    private updateInsertIndicator(dropDirection: string, insertionIndicatorAbsolutePos: number) {
        ...
        const insertionIndicatorTop = insertionIndicatorAbsolutePos -
            this.list.scrollTop +
            bottomToolbarGap / 2;
    ...
    private _dropImpl(draggedCell: ICellViewModel, dropDirection: "above" | "below", ctx: {
        ctrlKey: boolean;
        altKey: boolean;
    }, draggedOverCell: ICellViewModel) {
        const cellTop = this.list.getCellViewScrollTop(draggedOverCell);
        ...
        const isCopy = (ctx.ctrlKey && !platform.isMacintosh) ||
    ...
    public explicitDrop(cell: ICellViewModel, ctx: {
        dragOffsetY: number;
        ctrlKey: boolean;
        altKey: boolean;
    }) {
...
export function performCellDropEdits(editor: INotebookEditorDelegate, draggedCell: ICellViewModel, dropDirection: "above" | "below", draggedOverCell: ICellViewModel): void {
    ...
    if (typeof draggedCellIndex !== "number" ||
