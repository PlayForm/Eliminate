...
import { CellFoldingState, getNotebookEditorFromEditorPane, INotebookEditor, INotebookEditorContribution, INotebookEditorMouseEvent, } from "../notebookBrowser.js";
...
export class FoldingController extends Disposable implements INotebookEditorContribution {
    ...
    setFoldingStateUp(index: number, state: CellFoldingState, levels: number) {
        ...
        const regions = this._foldingModel.getAllRegionsAtLine(index + 1, (region, level) => region.isCollapsed !== (state === CellFoldingState.Collapsed) &&
            level <= levels);
        regions.forEach((r) => this._foldingModel!.setCollapsed(r.regionIndex, state === CellFoldingState.Collapsed));
        this._updateEditorFoldingRanges();
    }
    ...
    onMouseUp(e: INotebookEditorMouseEvent) {
        ...
        if (target.classList.contains("codicon-notebook-collapsed") ||
            ...
            this.setFoldingStateUp(modelIndex, state === CellFoldingState.Collapsed
                ? CellFoldingState.Expanded
                : CellFoldingState.Collapsed, 1);
...
registerAction2(class extends Action2 {
    ...
    async run(accessor: ServicesAccessor, args?: {
        ...
        if (index !== undefined) {
            const targetCell = index < 0 || index >= editor.getLength()
                ? undefined
                : editor.cellAt(index);
            if (targetCell?.cellKind === CellKind.Code &&
                direction === "down") {
                return;
            }
            if (direction === "up") {
