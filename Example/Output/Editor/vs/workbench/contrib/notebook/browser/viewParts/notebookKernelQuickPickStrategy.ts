...
abstract class KernelPickerStrategyBase implements IKernelPickerStrategy {
    ...
    async showQuickPick(editor: IActiveNotebookEditor, wantedId?: string, skipAutoRun?: boolean): Promise<boolean> {
        ...
        const quickPick = localDisposableStore.add(this._quickInputService.createQuickPick<KernelQuickPickItem>({
            useSeparators: true,
        }));
        ...
        quickPick.placeholder = selected
            ? localize("prompt.placeholder.change", "Change kernel for '{0}'", this._labelService.getUriLabel(notebook.uri, {
                relative: true,
            }))
            : localize("prompt.placeholder.select", "Select kernel for '{0}'", this._labelService.getUriLabel(notebook.uri, {
                relative: true,
            }));
        quickPick.busy =
        ...
        const kernelDetectionTaskListener = this._notebookKernelService.onDidChangeKernelDetectionTasks(() => {
            quickPick.busy =
                this._notebookKernelService.getKernelDetectionTasks(notebook).length > 0;
        });
        ...
        const kernelChangeEventListener = Event.debounce<void, void>(Event.any(this._notebookKernelService.onDidChangeSourceActions, this._notebookKernelService.onDidAddKernel, this._notebookKernelService.onDidRemoveKernel, this._notebookKernelService.onDidChangeNotebookAffinity), (last, _current) => last, KERNEL_PICKER_UPDATE_DEBOUNCE)(async () => {
            ...
            for (const item of currentActiveItems) {
                if (isKernelPick(item)) {
                    ...
                    const sameItem = quickPickItems.find((pi) => isSourcePick(pi) &&
                        pi.action.action.id === item.action.action.id) as SourcePick | undefined;
                    if (sameItem) {
                        activeItems.push(sameItem);
                    }
                }
    ...
    protected async _showKernelExtension(extensionWorkbenchService: IExtensionsWorkbenchService, extensionService: IExtensionService, viewType: string, extIds: string[], isInsiders?: boolean) {
    ...
    private async _showInstallKernelExtensionRecommendation(notebookTextModel: NotebookTextModel, quickPick: IQuickPick<KernelQuickPickItem, {
        useSeparators: true;
    }>, extensionWorkbenchService: IExtensionsWorkbenchService, token: CancellationToken) {
        quickPick.busy = true;
    ...
    protected async _getKernelRecommendationsQuickPickItems(notebookTextModel: NotebookTextModel, extensionWorkbenchService: IExtensionsWorkbenchService): Promise<QuickPickInput<SearchMarketplacePick | InstallExtensionPick>[] | undefined> {
        ...
        const suggestedExtension: INotebookExtensionRecommendation | undefined = language
            ? this.getSuggestedKernelFromLanguage(notebookTextModel.viewType, language)
            : undefined;
        if (suggestedExtension) {
            await extensionWorkbenchService.queryLocal();
            const extensions = extensionWorkbenchService.installed.filter((e) => (e.enablementState ===
            ...
            quickPickItems.push({
                id: "installSuggested",
                description: suggestedExtension.displayName ??
...
export class KernelPickerMRUStrategy extends KernelPickerStrategyBase {
    ...
    private async displaySelectAnotherQuickPick(editor: IActiveNotebookEditor, kernelListEmpty: boolean): Promise<boolean> {
        ...
        const quickPick = disposables.add(this._quickInputService.createQuickPick<KernelQuickPickItem>({
            useSeparators: true,
        }));
        const quickPickItem = await new Promise<KernelQuickPickItem | IQuickInputButton | undefined>((resolve) => {
            ...
            disposables.add(quickPick.onDidTriggerItemButton(async (e) => {
                if (isKernelSourceQuickPickItem(e.item) &&
            ...
            disposables.add(Event.debounce<void, void>(Event.any(this._notebookKernelService.onDidChangeSourceActions, this._notebookKernelService.onDidAddKernel, this._notebookKernelService.onDidRemoveKernel), (last, _current) => last, KERNEL_PICKER_UPDATE_DEBOUNCE)(async () => {
                quickPick.busy = true;
            ...
            if (isKernelSourceQuickPickItem(selectedKernelPickItem)) {
                try {
                    ...
                    if (selectedKernelId) {
                        ...
                        const kernel = all.find((kernel) => kernel.id ===
    ...
    private async _calculdateKernelSources(editor: IActiveNotebookEditor) {
        ...
        const matchResult = this._getMatchingResult(notebook);
        if (sourceActionCommands.length === 0 &&
        ...
        for (const group of groupBy(others, (a, b) => a.extension.value === b.extension.value ? 0 : 1)) {
            ...
            const source = extension?.displayName ??
        ...
        quickPickItems.push(...validActions.map((action) => {
            const buttons = action.documentation
                ...
                : [];
            return {
                id: typeof action.command! === "string"
                    ? action.command
                    : action.command!.id,
                label: action.label,
