...
import { CellUri, IOrderedMimeType, isTextStreamMime, NotebookCellExecutionState, NotebookCellOutputsSplice, RENDERER_NOT_AVAILABLE, } from "../../../common/notebookCommon.js";
...
class CellOutputElement extends Disposable {
    ...
    constructor(private notebookEditor: INotebookEditorDelegate, private viewCell: CodeCellViewModel, private cellOutputContainer: CellOutputContainer, private outputContainer: FastDomNode<HTMLElement>, readonly output: ICellOutputViewModel, 
    @INotebookService
    ...
    detach() {
        ...
        if (this.innerContainer) {
            for (let i = 0; i < this.innerContainer.childNodes.length; i++) {
                if ((this.innerContainer.childNodes[i] as HTMLElement)
    ...
    rerender() {
        ...
        if (!this.innerContainer) {
            ...
            const previousSibling = currOutputIndex > 0 &&
                !!this.cellOutputContainer.renderedOutputEntries[currOutputIndex - 1].element.innerContainer?.parentElement
    ...
    private shouldEnableCopy(mimeTypes: readonly IOrderedMimeType[]) {
        if (!mimeTypes.find((mimeType) => TEXT_BASED_MIMETYPES.indexOf(mimeType.mimeType) ||
    ...
    private _generateRendererInfo(renderId: string): string {
        ...
        if (renderInfo) {
            const displayName = renderInfo.displayName !== ""
...
class OutputEntryViewHandler {
    constructor(readonly model: ICellOutputViewModel, readonly element: CellOutputElement) { }
}
...
export class CellOutputContainer extends CellContentPart {
    ...
    constructor(private notebookEditor: INotebookEditorDelegate, private viewCell: CodeCellViewModel, private readonly templateData: CodeCellRenderTemplate, private options: {
        limit: number;
    }, 
    @IOpenerService
    ...
    private _doRender() {
        if (this.viewCell.outputsViewModels.length > 0) {
            ...
            for (let index = 0; index <
                Math.min(this.options.limit, this.viewCell.outputsViewModels.length); index++) {
    ...
    private _renderNow(splice: NotebookCellOutputsSplice, context: CellOutputUpdateContext) {
        ...
        if (firstGroupEntries.length +
            newlyInserted.length +
            secondGroupEntries.length >
            this.options.limit) {
            // exceeds limit again
            if (firstGroupEntries.length + newlyInserted.length >
                this.options.limit) {
                ...
                const elementsPushedOutOfView = secondGroupEntries.slice(this.options.limit -
                ...
                this._outputEntries = [
                    ...firstGroupEntries,
                    ...newlyInsertedEntries,
                    ...secondGroupEntries.slice(0, this.options.limit -
                        firstGroupEntries.length -
                        newlyInserted.length),
                ];
            ...
            if (firstGroupEntries.length +
                ...
                outputsNewlyAvailable = this.viewCell.outputsViewModels
                    .slice(firstGroupEntries.length +
                    newlyInsertedEntries.length +
                    secondGroupEntries.length, last)
                    .map((output) => {
            ...
            for (let i = 0; i < outputsNewlyAvailable.length; i++) {
                this._outputEntries[firstGroupEntries.length +
                    newlyInserted.length +
                    secondGroupEntries.length +
                    i].element.render(undefined);
            }
        }
        if (this.viewCell.outputsViewModels.length > this.options.limit) {
            DOM.show(this.templateData.outputShowMoreContainer.domNode);
        ...
        this._validateFinalOutputHeight(context === CellOutputUpdateContext.Other &&
