...
import { CellOverflowToolbarGroups, INotebookActionContext, INotebookCellActionContext, NOTEBOOK_EDITOR_WIDGET_ACTION_WEIGHT, NOTEBOOK_OUTPUT_WEBVIEW_ACTION_WEIGHT, NotebookAction, NotebookCellAction, } from "../../controller/coreActions.js";
...
function getFocusedEditor(accessor: ServicesAccessor) {
    ...
    if (view &&
        view.viewCells.every((cell) => !cell.outputIsFocused && !cell.outputIsHovered)) {
        return;
    }
...
export function runCutCells(accessor: ServicesAccessor, editor: INotebookEditor, targetCell: ICellViewModel | undefined): boolean {
    ...
    if (targetCell) {
        ...
        if (!containingSelection) {
            ...
            const newFocus = focus.end <= targetCellIndex
                ? focus
