...
class WorkbenchAlwaysLabelStrategy implements IActionLayoutStrategy {
    constructor(readonly notebookEditor: INotebookEditorDelegate, readonly editorToolbar: NotebookEditorWorkbenchToolbar, readonly goToMenu: IMenu, readonly instantiationService: IInstantiationService) { }
...
export class NotebookEditorWorkbenchToolbar extends Disposable {
    ...
    constructor(readonly notebookEditor: INotebookEditorDelegate, readonly contextKeyService: IContextKeyService, readonly notebookOptions: NotebookOptions, readonly domNode: HTMLElement, 
    ...
    private readonly experimentService: IWorkbenchAssignmentService) {
        ...
        this._buildBody();
        this._register(Event.debounce<void, void>(this.editorService.onDidActiveEditorChange, (last, _current) => last, 200)(this._updatePerEditorChange, this));
            ...
            if (this._renderLabel !== RenderLabel.Never) {
                ...
                if (a && a.renderLabel) {
                    return action instanceof MenuItemAction
                        ? this.instantiationService.createInstance(ActionViewWithLabel, action, { hoverDelegate: options.hoverDelegate })
                        : undefined;
                }
                else {
                    return action instanceof MenuItemAction
        ...
        this._register(this.configurationService.onDidChangeConfiguration((e) => {
            if (e.affectsConfiguration(NotebookSetting.globalToolbarShowLabel)) {
                ...
                this._notebookLeftToolbar =
                    this.instantiationService.createInstance(WorkbenchToolBar, this._notebookTopLeftToolbarContainer, leftToolbarOptions);
                this._register(this._notebookLeftToolbar);
                this._notebookLeftToolbar.context = context;
                this._showNotebookActionsinEditorToolbar();
                return;
    ...
    private _computeSizes() {
        ...
        if (toolbar &&
            ...
            const kernelWidth = (rightToolbar.getItemsLength()
...
export function workbenchCalculateActions(initialPrimaryActions: IActionModel[], initialSecondaryActions: IAction[], leftToolbarContainerMaxWidth: number): {
...
export function workbenchDynamicCalculateActions(initialPrimaryActions: IActionModel[], initialSecondaryActions: IAction[], leftToolbarContainerMaxWidth: number): {
    primaryActions: IActionModel[];
    secondaryActions: IAction[];
} {
    ...
    const totalWidthWithLabels = initialPrimaryActions
    ...
    if (visibleActionLength * ICON_ONLY_ACTION_WIDTH +
        (visibleActionLength - 1) * ACTION_PADDING >
...
function actionOverflowHelper(initialPrimaryActions: IActionModel[], initialSecondaryActions: IAction[], leftToolbarContainerMaxWidth: number, iconOnly: boolean): {
    primaryActions: IActionModel[];
    secondaryActions: IAction[];
} {
