...
export abstract class DiffElementViewModelBase extends Disposable {
    ...
    constructor(public readonly mainDocumentTextModel: INotebookTextModel, public readonly editorEventDispatcher: NotebookDiffEditorEventDispatcher, public readonly initData: {
...
export class NotebookDocumentMetadataViewModel extends DiffElementViewModelBase {
    ...
    constructor(public readonly originalDocumentTextModel: INotebookTextModel, public readonly modifiedDocumentTextModel: INotebookTextModel, public readonly type: "unchangedMetadata" | "modifiedMetadata", editorEventDispatcher: NotebookDiffEditorEventDispatcher, initData: {
        metadataStatusHeight: number;
        outputStatusHeight: number;
        fontInfo: FontInfo | undefined;
    }, notebookService: INotebookService, private readonly editorHeightCalculator: IDiffEditorHeightCalculatorService) {
    ...
    public async computeHeights() {
        if (this.type === "unchangedMetadata") {
            this.editorHeight =
                this.editorHeightCalculator.computeHeightFromLines(this.originalMetadata.textBuffer.getLineCount());
        }
        else {
            const original = this.originalMetadata.uri;
            const modified = this.modifiedMetadata.uri;
            this.editorHeight =
                await this.editorHeightCalculator.diffAndComputeHeight(original, modified);
        }
    }
    layoutChange() {
    ...
    protected _layout(delta: ILayoutInfoDelta) {
        ...
        const editorHeight = delta.editorHeight !== undefined
            ? delta.editorHeight
            : this._layoutInfo.editorHeight;
        const editorMargin = delta.editorMargin !== undefined
            ? delta.editorMargin
        ...
        const bodyMargin = delta.bodyMargin !== undefined
            ? delta.bodyMargin
...
export abstract class DiffElementCellViewModelBase extends DiffElementViewModelBase {
    ...
    protected _stateChangeEmitter = this._register(new Emitter<{
    ...
    constructor(mainDocumentTextModel: INotebookTextModel, original: NotebookCellTextModel | undefined, modified: NotebookCellTextModel | undefined, readonly type: "unchanged" | "insert" | "delete" | "modified", editorEventDispatcher: NotebookDiffEditorEventDispatcher, initData: {
        metadataStatusHeight: number;
        outputStatusHeight: number;
        fontInfo: FontInfo | undefined;
    }, notebookService: INotebookService, public readonly index: number, private readonly configurationService: IConfigurationService, public readonly diffEditorHeightCalculator: IDiffEditorHeightCalculatorService) {
        super(mainDocumentTextModel, editorEventDispatcher, initData);
    ...
    protected _layout(delta: ILayoutInfoDelta) {
        ...
        const editorMargin = delta.editorMargin !== undefined
            ? delta.editorMargin
            : this._layoutInfo.editorMargin;
        const metadataHeight = delta.metadataHeight !== undefined
        ...
        const metadataStatusHeight = delta.metadataStatusHeight !== undefined
            ...
            : this._layoutInfo.metadataStatusHeight;
        const rawOutputHeight = delta.rawOutputHeight !== undefined
        ...
        if (newLayout.metadataStatusHeight !==
            this._layoutInfo.metadataStatusHeight) {
        ...
        if (newLayout.outputMetadataHeight !==
            this._layoutInfo.outputMetadataHeight) {
...
export class SideBySideDiffElementViewModel extends DiffElementCellViewModelBase {
    ...
    constructor(mainDocumentTextModel: NotebookTextModel, readonly otherDocumentTextModel: NotebookTextModel, original: NotebookCellTextModel, modified: NotebookCellTextModel, type: "unchanged" | "modified", editorEventDispatcher: NotebookDiffEditorEventDispatcher, initData: {
        metadataStatusHeight: number;
        outputStatusHeight: number;
        fontInfo: FontInfo | undefined;
        ...
        this._register(this.modified.textModel.onDidChangeContent(() => {
            if (mainDocumentTextModel.transientOptions.cellContentMetadata) {
                const cellMetadataKeys = [
                    ...Object.keys(mainDocumentTextModel.transientOptions
                        .cellContentMetadata),
                ];
    ...
    override checkIfInputModified(): false | {
        reason: string | undefined;
    } {
        if (this.original.textModel.getTextBufferHash() ===
    ...
    checkIfOutputsModified() {
        ...
        return {
            reason: ret === OutputComparison.Metadata
    ...
    checkMetadataIfModified() {
        ...
        const modified = hash(getFormattedMetadataJSON(this.mainDocumentTextModel.transientOptions
            .transientCellMetadata, this.original?.metadata || {}, this.original?.language)) !==
            hash(getFormattedMetadataJSON(this.mainDocumentTextModel.transientOptions
                .transientCellMetadata, this.modified?.metadata ?? {}, this.modified?.language));
        if (modified) {
    ...
    private async computeModifiedMetadataEditorHeight() {
        if (this.checkMetadataIfModified()) {
            ...
            this._layoutInfo.metadataHeight =
                await this.diffEditorHeightCalculator.diffAndComputeHeight(originalMetadataUri, modifiedMetadataUri);
        }
    }
...
export class SingleSideDiffElementViewModel extends DiffElementCellViewModelBase {
    ...
    constructor(mainDocumentTextModel: NotebookTextModel, readonly otherDocumentTextModel: NotebookTextModel, original: NotebookCellTextModel | undefined, modified: NotebookCellTextModel | undefined, type: "insert" | "delete", editorEventDispatcher: NotebookDiffEditorEventDispatcher, initData: {
        metadataStatusHeight: number;
        outputStatusHeight: number;
        fontInfo: FontInfo | undefined;
    ...
    isOutputEmpty() {
        ...
        // outputs are not changed
        return ((this.original?.outputs || this.modified?.outputs || []).length ===
            0);
    }
    getRichOutputTotalHeight() {
...
function outputsEqual(original: ICellOutput[], modified: ICellOutput[]) {
    ...
    for (let i = 0; i < len; i++) {
        ...
        for (let j = 0; j < a.outputs.length; j++) {
            ...
            if (aOutputItem.data.buffer.length !==
