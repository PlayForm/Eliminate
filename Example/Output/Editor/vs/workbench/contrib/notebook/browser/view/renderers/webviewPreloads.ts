...
declare function requestIdleCallback(callback: (args: IdleDeadline) => void, options?: {
    timeout: number;
}): number;
...
async function webviewPreloads(ctx: PreloadContext) {
    ...
    const runWhenIdle: (callback: (idle: IdleDeadline) => void, timeout?: number) => IDisposable = typeof requestIdleCallback !== "function" ||
        ...
        for (const node of event.composedPath()) {
            if (node instanceof HTMLAnchorElement && node.href) {
                if (node.href.startsWith("blob:")) {
                    ...
                    if (scrollTarget) {
                        const scrollTop = scrollTarget.getBoundingClientRect().top +
                            event.view.scrollY;
    ...
    const dimensionUpdater = new (class {
        ...
        updateHeight(id: string, height: number, options: {
            init?: boolean;
            isOutput?: boolean;
        }) {
        ...
        constructor() {
            this._observer = new ResizeObserver((entries) => {
                for (const entry of entries) {
                    ...
                    const shouldUpdatePadding = (hasContent &&
    ...
    function _internalHighlightRange(range: Range, tagName = "mark", attributes = {}) {
        ...
        function _textNodesInRange(range: Range): Text[] {
            ...
            const walker = range.startContainer.ownerDocument.createTreeWalker(range.commonAncestorContainer, NodeFilter.SHOW_TEXT, (node) => range.intersectsNode(node)
    ...
    function createOutputItem(id: string, mime: string, metadata: unknown, valueBytes: Uint8Array, allOutputItemData: ReadonlyArray<{
        ...
        function create(id: string, mime: string, metadata: unknown, valueBytes: Uint8Array, appended?: {
            valueBytes: Uint8Array;
            previousVersion: number;
        }): ExtendedOutputItem {
        ...
        /*mime*/ string, Promise<(rendererApi.OutputItem & ExtendedOutputItem) | undefined>>();
    ...
    class CSSHighlighter implements IHighlighter {
        ...
        _refreshRegistry(updateMatchesHighlight = true) {
            ...
            this._activeHighlightInfo.forEach((highlightInfo) => {
                ...
                if (highlightInfo.currentMatchIndex <
                    highlightInfo.matches.length &&
                    highlightInfo.currentMatchIndex >= 0) {
                    this._currentMatchesHighlight.add(highlightInfo.matches[highlightInfo.currentMatchIndex]
                        .originalRange);
                }
            });
        }
    ...
    const find = (query: string, options: {
            ...
            while (find && matches.length < 500) {
                ...
                if (find) {
                    ...
                    if (options.includeOutput &&
                        ...
                        // output container
                        const cellId = selection.getRangeAt(0).startContainer
                            .parentElement!.id;
        ...
        matches = matches.filter((match) => options.findIds.length
    ...
    const renderers = new (class {
        ...
        public async render(item: ExtendedOutputItem, preferredRendererId: string | undefined, element: HTMLElement, signal: AbortSignal): Promise<void> {
            ...
            if (!(await this._doRender(item, element, primaryRenderer, signal))
                .continue) {
                return;
            }
        ...
        private async _doRender(item: rendererApi.OutputItem, element: HTMLElement, renderer: Renderer, signal: AbortSignal): Promise<{
            continue: boolean;
        }> {
            try {
        ...
        private findRenderer(preferredRendererId: string | undefined, info: rendererApi.OutputItem) {
            ...
            if (typeof preferredRendererId === "string") {
                ...
                const renderers = Array.from(this._renderers.values()).filter((renderer) => renderer.data.mimeTypes.includes(info.mime) &&
    ...
    class OutputCell {
        ...
        /*outputId*/ string, OutputContainer>();
    ...
    function postNotebookMessage<T extends webviewMessages.FromWebviewMessage>(type: T["type"], properties: Omit<T, "__vscode_notebook_message" | "type">) {
        vscode.postMessage({
            __vscode_notebook_message: true,
            type,
            ...properties,
