 ...
 *--------------------------------------------------------------------------------------------*/
import { Dimension, getWindow, IFocusTracker, scheduleAtNextAnimationFrame, trackFocus, WindowIntervalTimer, } from "../../../../../../base/browser/dom.js";
import { CancelablePromise, createCancelablePromise, DeferredPromise, disposableTimeout, Queue, } from "../../../../../../base/common/async.js";
...
class NotebookChatWidget extends Disposable implements INotebookViewZone {
    ...
    constructor(private readonly _notebookEditor: INotebookEditor, readonly id: string, readonly notebookViewZone: INotebookViewZone, readonly domNode: HTMLElement, readonly widgetContainer: HTMLElement, readonly inlineChatWidget: InlineChatWidget, readonly parentEditor: CodeEditorWidget, private readonly _languageService: ILanguageService) {
        super();
        const updateHeight = () => {
    ...
    async getOrCreateEditingCell(): Promise<{
        cell: ICellViewModel;
    ...
    private _layoutWidget(inlineChatWidget: InlineChatWidget, widgetContainer: HTMLElement) {
        ...
        const maxWidth = 640;
        const width = Math.min(maxWidth, this._notebookEditor.getLayoutInfo().width -
            leftMargin -
            rightMargin);
...
export class NotebookChatController extends Disposable implements INotebookEditorContribution {
    ...
    private readonly _onDidChangePromptCache = this._register(new Emitter<{
        cell: URI;
    }>());
    ...
    private readonly _chatService: IChatService) {
        ...
        this._ctxOuterFocusPosition =
            CTX_NOTEBOOK_CHAT_OUTER_FOCUS_POSITION.bindTo(this._contextKeyService);
            ...
            chatWidgetViewOptions: {
                rendererOptions: {
                    renderTextEditsAsSummary: (uri) => {
                        return (isEqual(uri, this._widget?.parentEditor.getModel()
                            ?.uri) ||
    ...
    async acceptInput() {
        ...
        if (this._widget.editingCell &&
            this._widget.editingCell.textBuffer.getLength() > 0) {
    ...
    dismiss(discard: boolean) {
        ...
        const isWidgetFocused = currentFocus.start === widgetIndex &&
            currentFocus.end === widgetIndex;
        if (widget && isWidgetFocused) {
            ...
            const shouldFocusAboveCell = widgetIndex !== 0 &&
    ...
    async makeChanges(editor: IActiveCodeEditor, edits: ISingleEditOperation[]): Promise<void> {
        const cursorStateComputerAndInlineDiffCollection: ICursorStateComputer = (undoEdits) => {
            ...
            for (const edit of undoEdits) {
                last =
                    !last || last.isBefore(edit.range.getEndPosition())
