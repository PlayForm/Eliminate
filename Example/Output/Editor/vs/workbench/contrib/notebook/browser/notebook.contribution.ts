...
import { DefaultEndOfLine, ITextBuffer, ITextBufferFactory, ITextModel, } from "../../../../editor/common/model.js";
...
import { CellKind, CellUri, ICell, ICellOutput, IResolvedNotebookEditorModel, NotebookCellsChangeType, NotebookMetadataUri, NotebookSetting, NotebookWorkingCopyTypeIdentifier, } from "../common/notebookCommon.js";
...
/*--------------------------------------------------------------------------------------------- */
Registry.as<IEditorPaneRegistry>(EditorExtensions.EditorPane).registerEditorPane(EditorPaneDescriptor.create(NotebookEditor, NotebookEditor.ID, "Notebook Editor"), [new SyncDescriptor(NotebookEditorInput)]);
Registry.as<IEditorPaneRegistry>(EditorExtensions.EditorPane).registerEditorPane(EditorPaneDescriptor.create(NotebookTextDiffEditor, NotebookTextDiffEditor.ID, "Notebook Diff Editor"), [new SyncDescriptor(NotebookDiffEditorInput)]);
Registry.as<IEditorPaneRegistry>(EditorExtensions.EditorPane).registerEditorPane(EditorPaneDescriptor.create(NotebookMultiTextDiffEditor, NotebookMultiTextDiffEditor.ID, "Notebook Diff Editor"), [new SyncDescriptor(NotebookMultiDiffEditorInput)]);
...
Registry.as<IEditorFactoryRegistry>(EditorExtensions.EditorFactory).registerEditorSerializer(NotebookEditorInput.ID, NotebookEditorSerializer);
Registry.as<IEditorFactoryRegistry>(EditorExtensions.EditorFactory).registerEditorSerializer(NotebookDiffEditorInput.ID, NotebookDiffEditorSerializer);
    ...
    private updateCellUndoRedoComparisonKey(configurationService: IConfigurationService, undoRedoService: IUndoRedoService) {
        ...
        if (!undoRedoPerCell) {
            ...
            if (!this._uriComparisonKeyComputer) {
                this._uriComparisonKeyComputer =
                    undoRedoService.registerUriComparisonKeyComputer(CellUri.scheme, {
            ...
            if (cell.uri.toString() === resource.toString()) {
                const bufferFactory: ITextBufferFactory = {
                    create: (defaultEOL) => {
                        ...
                        return {
                            textBuffer: cell.textBuffer as ITextBuffer,
...
class CellInfoContentProvider {
        ...
        for (const cell of ref.object.notebook.cells) {
            if (cell.handle === data.handle) {
                ...
                this._disposables.push(disposables.add(ref.object.notebook.onDidChangeContent((e) => {
                    if (result &&
                        e.rawEvents.some((event) => (event.kind ===
                            NotebookCellsChangeType.ChangeCellMetadata ||
                            event.kind ===
                                NotebookCellsChangeType.ChangeCellLanguage) &&
                            event.index === cellIndex)) {
                        const value = getFormattedMetadataJSON(ref.object.notebook.transientOptions
                            .transientCellMetadata, cell.metadata, cell.language);
                        if (result.getValue() !== value) {
                            result.setValue(getFormattedMetadataJSON(ref.object.notebook.transientOptions
                                .transientCellMetadata, cell.metadata, cell.language));
                        }
                    }
                })));
    ...
    private parseStreamOutput(op?: ICellOutput): {
        content: string;
        mode: ILanguageSelection;
    } | undefined {
        if (!op) {
            return;
        }
    ...
    private _getResult(data: {
        ...
        const op = cell.outputs.find((op) => op.outputId === data.outputId ||
            op.alternativeOutputId === data.outputId);
    ...
    async provideMetadataTextContent(resource: URI): Promise<ITextModel | null> {
        ...
        this._disposables.push(disposables.add(ref.object.notebook.onDidChangeContent((e) => {
            if (result &&
                ...
                const value = getFormattedNotebookMetadataJSON(ref.object.notebook.transientOptions
                    .transientDocumentMetadata, ref.object.notebook.metadata);
                if (result.getValue() !== value) {
...
class NotebookEditorManager implements IWorkbenchContribution {
    ...
    editorGroups: IEditorGroupsService) {
        ...
        this._disposables.add(Event.debounce<E, E[]>(this._notebookEditorModelService.onDidChangeDirty, (last, current) => (!last ? [current] : [...last, current]), 100)(this._openMissingDirtyNotebookEditors, this));
...
class SimpleNotebookWorkingCopyEditorHandler extends Disposable implements IWorkbenchContribution, IWorkingCopyEditorHandler {
    ...
    private _getViewType(workingCopy: IWorkingCopyIdentifier) {
        ...
        if (notebookType &&
            notebookType.viewType === notebookType.notebookType) {
...
function isConfigurationPropertySchema(x: IConfigurationPropertySchema | {
    ...
    properties: {
        ...
        [NotebookSetting.outputScrolling]: {
            ...
            default: typeof product.quality === "string" &&
                product.quality !== "stable", // only enable as default in insiders
        },
