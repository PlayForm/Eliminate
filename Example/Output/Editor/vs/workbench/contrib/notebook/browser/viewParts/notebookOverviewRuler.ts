    ...
    private _render(ctx: CanvasRenderingContext2D, width: number, height: number, scrollHeight: number, ratio: number) {
        ...
        if (viewModel) {
            for (let i = 0; i < viewModel.viewCells.length; i++) {
                ...
                const cellHeight = (viewCell.layoutInfo.totalHeight / scrollHeight) *
                    ratio *
                    height;
                decorations
                    ...
                    const lineHeight = Math.min(fontInfo.lineHeight, (viewCell.layoutInfo.editorHeight /
                        scrollHeight /
                    ...
                    const width = overviewRuler.position ===
                    ...
                    if (overviewRuler.includeOutput) {
                        ctx.fillStyle = fillStyle;
                        const outputOffset = (viewCell.layoutInfo.editorHeight /
                            scrollHeight) *
                            ratio *
                            height;
                        const decorationHeight = (fontInfo.lineHeight / scrollHeight) *
                            ratio *
                            height;
