...
import { CELL_TITLE_CELL_GROUP_ID, getContextFromActiveEditor, getEditorFromArgsOrActivePane, INotebookActionContext, INotebookCellActionContext, NotebookAction, NotebookCellAction, } from "../coreActions.js";
...
registerAction2(class extends NotebookCellAction {
    ...
    async runWithContext(accessor: ServicesAccessor, context: INotebookCellActionContext) {
        ...
        const newFocusMode = newCell.cellKind === CellKind.Markup &&
...
async function startChat(accessor: ServicesAccessor, context: INotebookActionContext, index: number, input?: string, autoSend?: boolean, source?: string) {
    ...
    if (configurationService.getValue<boolean>(NotebookSetting.cellGenerate) ||
...
registerAction2(class extends NotebookAction {
    constructor() {
        super({
            ...
            title: {
                value: "$(sparkle) " +
                    localize("notebookActions.menu.insertCodeCellWithChat", "Generate"),
    ...
    override getEditorContextFromArgsOrActive(accessor: ServicesAccessor, ...args: any[]): IInsertCellWithChatArgs | undefined {
        ...
        const cell = firstArg.index <= 0
            ? undefined
            : notebookEditor.cellAt(firstArg.index - 1);
        return {
            cell,
            notebookEditor,
    ...
    async runWithContext(accessor: ServicesAccessor, context: IInsertCellWithChatArgs) {
        const index = Math.max(0, context.cell
            ...
            menu: [
                {
                    ...
                    group: "0_main",
                    order: 2,
                    when: ContextKeyExpr.and(NOTEBOOK_EDITOR_EDITABLE.isEqualTo(true), ChatContextKeys.inputHasText.toNegated(), CTX_INLINE_CHAT_REQUEST_IN_PROGRESS.toNegated(), CTX_INLINE_CHAT_RESPONSE_TYPE.isEqualTo(InlineChatResponseType.MessagesAndEdits)),
                },
            ],
        });
    }
