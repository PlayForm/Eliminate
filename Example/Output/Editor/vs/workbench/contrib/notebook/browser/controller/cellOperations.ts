...
export async function changeCellToKind(kind: CellKind, context: INotebookActionContext, language?: string, mime?: string): Promise<void> {
    ...
    if (context.ui && context.cell) {
        ...
        await notebookEditor.focusNotebookCell(newCell, cell.getEditState() === CellEditState.Editing
            ? "editor"
            : "container");
    }
...
export function runDeleteAction(editor: IActiveNotebookEditor, cell: ICellViewModel) {
    ...
    const containingSelection = selections.find((selection) => selection.start <= targetCellIndex &&
    ...
    if (containingSelection) {
        ...
        textModel.applyEdits(edits, true, {
            ...
            if (nextCellAfterContainingSelection) {
                const cellIndex = textModel.cells.findIndex((cell) => cell.handle ===
...
export async function joinNotebookCells(editor: IActiveNotebookEditor, range: ICellRange, direction: "above" | "below", constraint?: CellKind): Promise<{
    edits: ResourceEdit[];
    cell: ICellViewModel;
    endFocus: ICellRange;
...
export function insertCell(languageService: ILanguageService, editor: IActiveNotebookEditor, index: number, type: CellKind, direction: "above" | "below" = "above", initialText: string = "", ui: boolean = false) {
    const viewModel = editor.getViewModel() as NotebookViewModel;
    ...
    if (type === CellKind.Code) {
        const supportedLanguages = activeKernel?.supportedLanguages ??
        ...
        if (cell?.cellKind === CellKind.Code) {
            ...
            if (cell === undefined && direction === "above") {
                ...
                language =
                    viewModel.viewCells.find((cell) => cell.cellKind === CellKind.Code)?.language || defaultLanguage;
            }
            else {
                language = defaultLanguage;
            }
