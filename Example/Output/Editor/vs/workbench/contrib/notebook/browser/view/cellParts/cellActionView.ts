...
export class UnifiedSubmenuActionView extends SubmenuEntryActionViewItem {
    ...
    override render(container: HTMLElement): void {
        ...
        this._actionLabel = document.createElement("a");
        container.appendChild(this._actionLabel);
        this._hover = this._register(this._hoverService.setupManagedHover(this.options.hoverDelegate ??
            getDefaultHoverDelegate("element"), this._actionLabel, ""));
        this.updateLabel();
        for (const event of [
            DOM.EventType.CLICK,
        ...
        if (this._actionLabel) {
            ...
            if (primaryAction && primaryAction instanceof MenuItemAction) {
                ...
                if (this.renderLabel) {
                    this._actionLabel.classList.add("notebook-label");
                    this._actionLabel.innerText = this._action.label;
                    this._hover?.update(primaryAction.tooltip.length
                ...
                if (this.renderLabel) {
                    this._actionLabel.classList.add("notebook-label");
                    this._actionLabel.innerText = this._action.label;
                    this._hover?.update(this._action.tooltip.length
