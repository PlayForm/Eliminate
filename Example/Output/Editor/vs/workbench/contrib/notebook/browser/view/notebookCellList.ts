...
import { CellKind, diff, NOTEBOOK_EDITOR_CURSOR_BOUNDARY, NOTEBOOK_EDITOR_CURSOR_LINE_BOUNDARY, SelectionStateType, } from "../../common/notebookCommon.js";
import { NOTEBOOK_CELL_LIST_FOCUSED } from "../../common/notebookContextKeys.js";
import { INotebookExecutionStateService } from "../../common/notebookExecutionStateService.js";
import { cellRangesEqual, cellRangesToIndexes, ICellRange, reduceCellRanges, } from "../../common/notebookRange.js";
import { CellEditState, CellRevealRangeType, CellRevealType, CursorAtBoundary, CursorAtLineBoundary, ICellOutputViewModel, ICellViewModel, INotebookViewZoneChangeAccessor, } from "../notebookBrowser.js";
...
export class NotebookCellList extends WorkbenchList<CellViewModel> implements IDisposable, IStyleController, INotebookCellList {
    ...
    constructor(private listUser: string, container: HTMLElement, private readonly notebookOptions: NotebookOptions, delegate: IListVirtualDelegate<CellViewModel>, renderers: IListRenderer<CellViewModel, BaseCellRenderTemplate>[], contextKeyService: IContextKeyService, options: IWorkbenchListOptions<CellViewModel>, 
    @IListService
    ...
    notebookExecutionStateService: INotebookExecutionStateService) {
        ...
        const updateVisibleRanges = () => {
            ...
            if (bottomModelIndex - topModelIndex ===
    ...
    setHiddenAreas(_ranges: ICellRange[], triggerViewUpdate: boolean): boolean {
        ...
        if (newRanges.length === oldRanges.length) {
            ...
            for (let i = 0; i < newRanges.length; i++) {
                if (!(newRanges[i].start === oldRanges[i].start &&
        ...
        const hiddenAreaIds = newRanges
            .map((range) => this._viewModel!.setTrackedRange(null, range, TrackedRangeStickiness.GrowsOnlyWhenTypingAfter))
        ...
        if (elementTop >= scrollTop && elementTop < wrapperBottom) {
            ...
            if (endElementTop < wrapperBottom) {
                ...
                if (endElementTop + endElementHeight - wrapperBottom <
                    ...
                    return this.view.setScrollTop(scrollTop +
                        endElementTop +
                        endElementHeight -
                        wrapperBottom);
                }
        ...
        switch (revealPosition) {
            ...
            case CellRevealPosition.Bottom:
                if (firstLine) {
                    ...
                    const padding = this.notebookOptions.getLayoutConfiguration()
                        .cellTopMargin +
    ...
    private async _revealRangeInCenterIfOutsideViewportInternalAsync(viewIndex: number, range: Selection | Range): Promise<void> {
        ...
        if (positionOffset < scrollTop || positionOffset > wrapperBottom) {
            ...
            const newPositionOffset = this.view.elementTop(viewIndex) +
    ...
    override domFocus() {
        ...
        if (!isMacintosh &&
            ...
            !!DOM.findParentWithClass(<HTMLElement>this.view.domNode.ownerDocument.activeElement, "context-view")) {
            return;
        }
        super.domFocus();
    }
