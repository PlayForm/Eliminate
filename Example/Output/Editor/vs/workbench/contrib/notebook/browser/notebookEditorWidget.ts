...
export class NotebookEditorWidget extends Disposable implements INotebookEditorDelegate, INotebookEditor {
    ...
    constructor(readonly creationOptions: INotebookEditorCreationOptions, dimension: DOM.Dimension | undefined, 
    ...
    private readonly keybindingService: IKeybindingService) {
        ...
        this._scrollBeyondLastLine =
            this.configurationService.getValue<boolean>("editor.scrollBeyondLastLine");
        ...
        new RawContextKey<boolean>(PreventDefaultContextMenuItemsContextKeyName, false)
    ...
    private _updateForNotebookConfiguration() {
        ...
        if (cellToolbarInteraction === "hover" ||
        ...
        const execute = () => {
            try {
                ...
                const firstMarkupCell = this.viewModel.viewCells.find((cell) => cell.cellKind === CellKind.Markup &&
                    ...
                    !this.cellIsHidden(cell)) as MarkupCellViewModel | undefined;
                if (!firstMarkupCell) {
                    return;
                }
        ...
        if (cellOptions) {
            ...
            if (cell) {
                ...
                if (selection) {
                    ...
                    await this.revealRangeInCenterIfOutsideViewportAsync(cell, new Range(selection.startLineNumber, selection.startColumn, selection.endLineNumber ||
                        selection.startLineNumber, selection.endColumn || selection.startColumn));
                }
                else {
                    this._list.revealCell(cell, options?.cellRevealType ??
                ...
                if (editor) {
                    if (cellOptions.options?.selection) {
                        ...
                        const editorSelection = new Range(selection.startLineNumber, selection.startColumn, selection.endLineNumber ||
                            selection.startLineNumber, selection.endColumn || selection.startColumn);
    ...
    private async _attachModel(textModel: NotebookTextModel, viewType: string, viewState: INotebookEditorViewState | undefined, perf?: NotebookPerfMarks) {
        ...
        this._lastCellWithEditorFocus =
            this.viewModel.viewCells.find((viewCell) => this.getActiveCell() === viewCell &&
                viewCell.focusMode === CellFocusMode.Editor) ?? null;
        this._localStore.add(this.viewModel.onDidChangeViewCells((e) => {
            if (this._isDisposed) {
                return;
            }
    ...
    private _validateCellFocusMode(cell: ICellViewModel) {
        ...
        if (this._lastCellWithEditorFocus &&
    ...
    private getBodyHeight(dimensionHeight: number) {
        return Math.max(dimensionHeight -
    ...
    updateCellFocusMode() {
        ...
        if (activeCell?.focusMode === CellFocusMode.Output &&
    ...
    hasOutputTextSelection() {
        ...
        while (container && container !== this._body) {
            if ((container as HTMLElement).classList &&
    ...
    get activeKernel() {
        return (this.textModel &&
            this.notebookKernelService.getSelectedOrSuggestedKernel(this.textModel));
    }
    ...
    async find(query: string, options: INotebookFindOptions, token: CancellationToken, skipWarmup: boolean = false, shouldGetSearchPreviewInfo = false, ownerID?: string): Promise<CellFindMatchWithIndex[]> {
        ...
        const matchMap: {
            [key: string]: CellFindMatchWithIndex;
        } = {};
    ...
    /* outputId */ string, IAckOutputHeight>();
    ...
    private _didStartDragMarkupCell(cellId: string, event: {
        dragOffsetY: number;
    }): void {
        const cell = this._getCellById(cellId);
        if (cell instanceof MarkupCellViewModel) {
    ...
    private _didDragMarkupCell(cellId: string, event: {
        dragOffsetY: number;
    }): void {
        const cell = this._getCellById(cellId);
        if (cell instanceof MarkupCellViewModel) {
    ...
    private _didDropMarkupCell(cellId: string, event: {
        dragOffsetY: number;
        ctrlKey: boolean;
        altKey: boolean;
    }): void {
    ...
    private _updatePerformanceMetadata(cellId: string, executionId: string, duration: number, rendererId: string): void {
        ...
        if (cell?.internalMetadata.executionId === executionId &&
            cellIndex !== undefined) {
