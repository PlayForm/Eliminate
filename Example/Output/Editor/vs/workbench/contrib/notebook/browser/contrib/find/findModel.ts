...
export class FindModel extends Disposable {
    ...
    constructor(private readonly _notebookEditor: INotebookEditor, private readonly _state: FindReplaceState<NotebookFindFilters>, 
        ...
        const updateEditingState = () => {
            ...
            for (let i = 0; i < viewModel.length; i++) {
                ...
                if (cell && cell.cellKind === CellKind.Markup) {
                    const foundContentMatch = contentMatches.find((m) => m.cell.handle === cell.handle &&
                    ...
                    if (currentEditingState === CellEditState.Editing &&
    ...
    getCurrentMatch() {
        ...
        return {
            cell,
            match,
            isModelMatch: nextIndex.remainder <
    ...
    refreshCurrentMatch(focus: {
        ...
        const matchesBefore = findMatchIndex === 0
            ? 0
    ...
    find(option: {
        ...
        if (!this._findMatchesStarts) {
            ...
            if ("index" in option) {
                ...
                const nextVal = (this._currentMatch +
                    (option.previous ? -1 : 1) +
                    totalVal) %
                    totalVal;
    ...
    async _research() {
        ...
        if (this._findMatchDecorationModel.currentMatchDecorations.kind ===
            "input") {
            ...
            if (findMatches[matchAfterSelection].index > oldCurrMatchCellIndex) {
                ...
                let currMatchRangeInEditor = cell.editorAttached &&
