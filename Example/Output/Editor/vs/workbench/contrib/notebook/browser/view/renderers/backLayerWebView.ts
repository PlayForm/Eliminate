...
import { FileAccess, matchesScheme, matchesSomeScheme, Schemas, } from "../../../../../../base/common/network.js";
...
export class BackLayerWebView<T extends ICommonCellInfo> extends Themable {
    ...
    constructor(public notebookEditor: INotebookDelegateForWebview, private readonly id: string, public readonly notebookViewType: string, public readonly documentUri: URI, private options: BacklayerWebviewOptions, private readonly rendererMessaging: IScopedRendererMessaging | undefined, 
    @IWebviewService
    ...
    private _generateStyles() {
        return {
            ...
            "notebook-markup-font-size": typeof this.options.markupFontSize === "number" &&
                this.options.markupFontSize > 0
                ? `${this.options.markupFontSize}px`
                : `calc(${this.options.fontSize}px * 1.2)`,
            "notebook-markdown-line-height": typeof this.options.markdownLineHeight === "number" &&
    ...
    private resolveOutputId(id: string): {
        cellInfo: T;
        ...
        this._register(this.webview.onMessage(async (message) => {
            ...
            switch (data.type) {
                ...
                case "initializedMarkup": {
                    if (this.initializeMarkupPromise?.requestId ===
                        data.requestId) {
                    ...
                    if (resolvedResult) {
                        ...
                        if (latestCell) {
                            ...
                            this.notebookEditor.focusNotebookCell(latestCell, "output", {
                                outputId: resolvedResult.output.model
                                    .outputId,
                                skipReveal: true,
        ...
        if (executionMatch) {
            ...
            if (!isNaN(executionCount)) {
                ...
                const cell = notebookModel?.cells
                    ...
                    .reverse()
                    .find((cell) => {
                    return (cell.internalMetadata.executionOrder ===
                        executionCount);
                });
                if (cell?.uri) {
    ...
    private _openUri(uri: URI) {
        ...
        if (match) {
            const selection: ITextEditorSelection | undefined = lineNumber !== undefined && column !== undefined
    ...
    private _getResourceRootsCache(): URI[] {
        ...
        return [
            ...
            ...Array.from(this.notebookService.getStaticPreloads(this.notebookViewType), (x) => [dirname(x.entrypoint), ...x.localResourceRoots]),
            workspaceFolders,
            notebookDir,
    ...
    private _createOutputCreationMessage(cellInfo: T, content: IInsetRenderOutput, cellTop: number, offset: number, createOnIdle: boolean, initiallyHidden: boolean): {
    ...
    async find(query: string, options: {
        wholeWord?: boolean;
