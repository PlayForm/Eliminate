...
export class NotebookMultiTextDiffEditor extends EditorPane {
    ...
    override async setInput(input: NotebookMultiDiffEditorInput, options: IMultiDiffEditorOptions | undefined, context: IEditorOpenContext, token: CancellationToken): Promise<void> {
        ...
        await this.viewModel.computeDiff(this.modelSpecificResources.add(new CancellationTokenSource())
            .token);
        ...
        this.modelSpecificResources.add(autorun((reader) => {
            ...
            transaction((tx) => {
                items.forEach((item) => {
                    ...
                    const diffItem = diffItems.find((d) => d.modifiedUri?.toString() ===
    ...
    override hasFocus(): boolean {
        return (this._multiDiffEditorWidget?.getActiveControl()?.hasTextFocus() ||
            super.hasFocus());
    }
                    ...
                    if (uri.scheme === Schemas.vscodeNotebookCell) {
                        ...
                        if (notebookDocument && cellIndex !== undefined) {
                            ...
                            const nb = notebookDocument
                                ...
                                : undefined;
                            const cellLanguage = nb && cellIndex !== undefined
