...
export class NotebookStickyLine extends Disposable {
    constructor(public readonly element: HTMLElement, public readonly foldingIcon: StickyFoldingIcon, public readonly header: HTMLElement, public readonly entry: OutlineEntry, public readonly notebookEditor: INotebookEditor) {
        super();
    ...
    private toggleFoldRange(currentState: CellFoldingState) {
        ...
        const newFoldingState = currentState === CellFoldingState.Collapsed
...
class StickyFoldingIcon {
    ...
    constructor(public isCollapsed: boolean, public dimension: number) {
...
export class NotebookStickyScroll extends Disposable {
    ...
    private currentStickyLines = new Map<OutlineEntry, {
    ...
    private setCurrentStickyLines(newStickyLines: Map<OutlineEntry, {
        line: NotebookStickyLine;
        rendered: boolean;
    }>) {
        this.currentStickyLines = newStickyLines;
    }
    private compareStickyLineMaps(mapA: Map<OutlineEntry, {
        line: NotebookStickyLine;
        rendered: boolean;
    }>, mapB: Map<OutlineEntry, {
        line: NotebookStickyLine;
        rendered: boolean;
    }>): boolean {
    ...
    constructor(private readonly domNode: HTMLElement, private readonly notebookEditor: INotebookEditor, private readonly notebookCellList: INotebookCellList, private readonly layoutFn: (delta: number) => void, 
    @IContextMenuService
    private readonly _contextMenuService: IContextMenuService, 
    @IInstantiationService
    private readonly instantiationService: IInstantiationService) {
        super();
        if (this.notebookEditor.notebookOptions.getDisplayOptions()
            .stickyScrollEnabled) {
            this.init();
        }
    ...
    private onContextMenu(e: MouseEvent) {
        ...
        const selectedOutlineEntry = Array.from(this.currentStickyLines.values()).find((entry) => entry.line.element.contains(selectedElement))?.line
            .entry;
    ...
    private updateContent(newMap: Map<OutlineEntry, {
        line: NotebookStickyLine;
        rendered: boolean;
    }>) {
        DOM.clearNode(this.domNode);
    ...
    static checkCollapsedStickyLines(entry: OutlineEntry | undefined, numLinesToRender: number, notebookEditor: INotebookEditor) {
        ...
        const newMap = new Map<OutlineEntry, {
            line: NotebookStickyLine;
            rendered: boolean;
        }>();
    ...
    private renderStickyLines(stickyMap: Map<OutlineEntry, {
        line: NotebookStickyLine;
        rendered: boolean;
    ...
    static createStickyElement(entry: OutlineEntry, notebookEditor: INotebookEditor) {
        ...
        const indentMode = notebookEditor.notebookOptions.getLayoutConfiguration()
            .stickyScrollMode;
...
export function computeContent(notebookEditor: INotebookEditor, notebookCellList: INotebookCellList, notebookOutlineEntries: OutlineEntry[], renderedStickyHeight: number): Map<OutlineEntry, {
    line: NotebookStickyLine;
    rendered: boolean;
}> {
