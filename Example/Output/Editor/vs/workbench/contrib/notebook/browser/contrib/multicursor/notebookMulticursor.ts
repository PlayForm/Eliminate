        ...
        this.anchorDisposables.add(this.anchorCell[1].onDidChangeCursorSelection((e) => {
            ...
            this.trackedMatches.forEach((match) => {
                ...
                const newSelections = controller
                    .getSelections()
                    .map((selection) => {
                    const newStartCol = selection.startColumn +
                        translation.deltaStartCol;
                    const newStartLine = selection.startLineNumber +
                        translation.deltaStartLine;
                    const newEndCol = selection.endColumn + translation.deltaEndCol;
                    const newEndLine = selection.endLineNumber +
    ...
    public async findAndTrackNextSelection(cell: ICellViewModel): Promise<void> {
        if (this.state === NotebookMultiCursorState.Idle) {
            ...
            const findResult = notebookTextModel.findNextMatch(this.word, {
                cellIndex: index,
                position: cell
                    .getSelections()[cell.getSelections().length - 1].getEndPosition(),
            }, false, true, USUAL_WORD_SEPARATORS);
            if (!findResult) {
            ...
            if (findResult.cell.handle === cell.handle) {
                ...
                if (this.notebookEditor.activeCellAndCodeEditor?.[0].handle !==
                    resultCellViewModel.handle) {
                    // should not happen
