...
function createHideUnchangedRegionOptions(configurationService: IConfigurationService): UnchangedEditorRegionOptions & {
    dispose: () => void;
} {
    ...
    disposables.add(configurationService.onDidChangeConfiguration((e) => {
        if (e.affectsConfiguration("diffEditor.hideUnchangedRegions.minimumLineCount")) {
            result.options.minimumLineCount =
                configurationService.getValue<number>("diffEditor.hideUnchangedRegions.minimumLineCount");
        }
        if (e.affectsConfiguration("diffEditor.hideUnchangedRegions.contextLineCount")) {
            result.options.contextLineCount =
                configurationService.getValue<number>("diffEditor.hideUnchangedRegions.contextLineCount");
        }
        if (e.affectsConfiguration("diffEditor.hideUnchangedRegions.revealLineCount")) {
            result.options.revealLineCount =
                configurationService.getValue<number>("diffEditor.hideUnchangedRegions.revealLineCount");
        }
