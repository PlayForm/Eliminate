...
export class NotebookOptions extends Disposable {
    ...
    constructor(readonly targetWindow: CodeWindow, private isReadonly: boolean, private readonly overrides: {
        cellToolbarInteraction: string;
        globalToolbar: boolean;
        stickyScrollEnabled: boolean;
        dragAndDropEnabled: boolean;
    ...
    private readonly codeEditorService: ICodeEditorService) {
        ...
        const globalToolbar = overrides?.globalToolbar ??
            this.configurationService.getValue<boolean | undefined>(NotebookSetting.globalToolbar) ??
            true;
        const stickyScrollEnabled = overrides?.stickyScrollEnabled ??
            this.configurationService.getValue<boolean | undefined>(NotebookSetting.stickyScrollEnabled) ??
            false;
        ...
        const dragAndDropEnabled = overrides?.dragAndDropEnabled ??
            this.configurationService.getValue<boolean | undefined>(NotebookSetting.dragAndDropEnabled) ??
            true;
        const cellToolbarLocation = this.configurationService.getValue<string | {
            [key: string]: string;
        }>(NotebookSetting.cellToolbarLocation) ?? { "default": "right" };
        const cellToolbarInteraction = overrides?.cellToolbarInteraction ??
            this.configurationService.getValue<string>(NotebookSetting.cellToolbarVisibility);
        ...
        if (deprecatedOutputLineHeightSetting !== undefined) {
            ...
            outputLineHeightSettingValue =
                this.configurationService.getValue<number>(NotebookSetting.outputLineHeight);
        }
        ...
        if (deprecatedOutputFontSizeSetting !== undefined) {
            ...
            outputFontSize =
                this.configurationService.getValue<number>(NotebookSetting.outputFontSize) || fontSize;
        }
    ...
    private _computeEditorTopPadding(): number {
        ...
        const onDidAddDecorationType = (e: string) => {
            ...
            try {
                ...
                if (options.afterContentClassName ||
                    ...
                    if (cssRules !== null) {
                        for (let i = 0; i < cssRules.length; i++) {
                            ...
                            if (((cssRules[i] as CSSStyleRule).selectorText.endsWith("::after") ||
                                (cssRules[i] as CSSStyleRule).selectorText.endsWith("::after")) &&
    ...
    private _updateConfiguration(e: IConfigurationChangeEvent) {
        ...
        if (cellStatusBarVisibility) {
            configuration.showCellStatusBar =
                this.configurationService.getValue<ShowCellStatusBarType>(NotebookSetting.showCellStatusBar);
        }
        ...
        if (globalToolbar && this.overrides?.globalToolbar === undefined) {
            configuration.globalToolbar =
                this.configurationService.getValue<boolean>(NotebookSetting.globalToolbar) ?? true;
        }
        if (stickyScrollEnabled &&
            this.overrides?.stickyScrollEnabled === undefined) {
            configuration.stickyScrollEnabled =
                this.configurationService.getValue<boolean>(NotebookSetting.stickyScrollEnabled) ?? false;
        }
        if (stickyScrollMode) {
            configuration.stickyScrollMode =
                this.configurationService.getValue<"flat" | "indented">(NotebookSetting.stickyScrollMode) ?? "flat";
        }
        if (consolidatedOutputButton) {
            configuration.consolidatedOutputButton =
                this.configurationService.getValue<boolean>(NotebookSetting.consolidatedOutputButton) ?? true;
        }
        if (consolidatedRunButton) {
            configuration.consolidatedRunButton =
                this.configurationService.getValue<boolean>(NotebookSetting.consolidatedRunButton) ?? true;
        }
        ...
        if (dragAndDropEnabled) {
            configuration.dragAndDropEnabled =
                this.configurationService.getValue<boolean>(NotebookSetting.dragAndDropEnabled) ?? true;
        }
        if (fontSize) {
        ...
        if (outputFontSize || fontSize) {
            configuration.outputFontSize =
                this.configurationService.getValue<number>(NotebookSetting.outputFontSize) || configuration.fontSize;
        }
        if (markupFontSize) {
            configuration.markupFontSize =
                this.configurationService.getValue<number>(NotebookSetting.markupFontSize);
        }
        if (markdownLineHeight) {
            configuration.markdownLineHeight =
                this.configurationService.getValue<number>(NotebookSetting.markdownLineHeight);
        }
        if (outputFontFamily) {
            configuration.outputFontFamily =
                this.configurationService.getValue<string>(NotebookSetting.outputFontFamily);
        }
        if (editorOptionsCustomizations) {
            configuration.editorOptionsCustomizations =
                this.configurationService.getValue(NotebookSetting.cellEditorOptionsCustomizations);
        }
        if (interactiveWindowCollapseCodeCells) {
            configuration.interactiveWindowCollapseCodeCells =
                this.configurationService.getValue(NotebookSetting.interactiveWindowCollapseCodeCells);
        }
        ...
        if (outputWordWrap) {
            configuration.outputWordWrap =
                this.configurationService.getValue<boolean>(NotebookSetting.outputWordWrap);
        }
        if (outputScrolling) {
            configuration.outputScrolling =
                this.configurationService.getValue<boolean>(NotebookSetting.outputScrolling);
        }
        if (outputLinkifyFilePaths) {
            configuration.outputLinkifyFilePaths =
                this.configurationService.getValue<boolean>(NotebookSetting.LinkifyOutputFilePaths);
        }
        if (minimalError) {
            configuration.outputMinimalError =
                this.configurationService.getValue<boolean>(NotebookSetting.minimalErrorRendering);
        }
    ...
    private _computeInsertToolbarPositionOption(isReadOnly: boolean) {
        return isReadOnly
            ? "hidden"
            : (this.configurationService.getValue<"betweenCells" | "notebookToolbar" | "both" | "hidden">(NotebookSetting.insertToolbarLocation) ?? "both");
    }
    ...
    private _computeBottomToolbarDimensions(compactView: boolean, insertToolbarPosition: "betweenCells" | "notebookToolbar" | "both" | "hidden", insertToolbarAlignment: "left" | "center", cellToolbar: "right" | "left" | "hidden"): {
        ...
        if (insertToolbarPosition === "betweenCells" ||
    ...
    computeCellToolbarLocation(viewType?: string): "right" | "left" | "hidden" {
        ...
        if (typeof cellToolbarLocation === "string") {
            ...
            if (viewType) {
                const notebookSpecificSetting = cellToolbarLocation[viewType] ??
                    cellToolbarLocation["default"];
    ...
    computeTopInsertToolbarHeight(viewType?: string): number {
        if (this._layoutConfiguration.insertToolbarPosition ===
            "betweenCells" ||
    ...
    private statusBarIsVisible(internalMetadata: NotebookCellInternalMetadata, cellUri: URI): boolean {
        ...
        else if (this._layoutConfiguration.showCellStatusBar ===
            "visibleAfterExecute") {
            return (typeof internalMetadata.lastRunSuccess === "boolean" ||
                exe !== undefined);
        }
        else {
            return false;
