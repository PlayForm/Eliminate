...
export class NotebookOptions extends Disposable {
                ...
                if (options.afterContentClassName || options.beforeContentClassName) {
                    ...
                    if (cssRules !== null) {
                        for (let i = 0; i < cssRules.length; i++) {
                            ...
                            if (((cssRules[i] as CSSStyleRule).selectorText.endsWith('::after') || (cssRules[i] as CSSStyleRule).selectorText.endsWith('::after'))
    ...
    private _updateConfiguration(e: IConfigurationChangeEvent) {
        ...
        if (cellToolbarLocation) {
            configuration.cellToolbarLocation = this.configurationService.getValue<string | {
                [key: string]: string;
            }>(NotebookSetting.cellToolbarLocation) ?? { 'default': 'right' };
        }
