...
import { Disposable, dispose, IDisposable, IReference, MutableDisposable, } from "../../../../../base/common/lifecycle.js";
...
export abstract class BaseCellViewModel extends Disposable {
    ...
    private readonly _cellDecorationsChanged = this._register(new Emitter<{
    ...
    constructor(readonly viewType: string, readonly model: NotebookCellTextModel, public id: string, private readonly _viewContext: ViewContext, private readonly _configurationService: IConfigurationService, private readonly _modelService: ITextModelService, private readonly _undoRedoService: IUndoRedoService, private readonly _codeEditorService: ICodeEditorService) {
        super();
        ...
        this._commentOptions =
            this._configurationService.getValue<IEditorCommentsOptions>("editor.comments", { overrideIdentifier: this.language });
        this._register(this._configurationService.onDidChangeConfiguration((e) => {
            ...
            if (estimatedHasHorizontalScrolling) {
                this._restoreViewState({
                    ...
                    viewState: {
                        ...
                        firstPositionDeltaTop: this._viewContext.notebookOptions.getLayoutConfiguration()
                            .editorTopPadding,
                    },
                });
            }
    ...
    getCellDecorationRange(decorationId: string): Range | null {
        if (this._textEditor) {
            ...
            return (this._textEditor.getModel()?.getDecorationRange(decorationId) ??
                null);
        }
        return null;
    }
    ...
    getPositionScrollTopOffset(range: Selection | Range): number {
        ...
        const position = range instanceof Selection
            ? range.getPosition()
    ...
    async resolveTextModel(): Promise<model.ITextModel> {
        if (!this._textModelRef || !this.textModel) {
            ...
            this._textModelRefChangeDisposable.value =
                this.textModel!.onDidChangeContent(() => this.onDidChangeTextModelContent());
        }
        return this.textModel!;
    }
    ...
    override dispose() {
        ...
        if (this._undoRedoService.getUriComparisonKey(this.uri) ===
            this.uri.toString()) {
