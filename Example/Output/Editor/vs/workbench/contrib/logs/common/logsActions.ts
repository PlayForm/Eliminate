...
import { ILoggerService, isLogLevel, LogLevel, LogLevelToLocalizedString, } from "../../../../platform/log/common/log.js";
...
type LogLevelQuickPickItem = IQuickPickItem & {
    level: LogLevel;
};
...
export class SetLogLevelAction extends Action {
    ...
    private async selectLogLevelOrChannel(): Promise<LogChannelQuickPickItem | LogLevel | null> {
        ...
        for (const channel of this.outputService.getChannelDescriptors()) {
            ...
            const item: LogChannelQuickPickItem = {
                ...
                label: channel.label,
                description: channelLogLevel !== logLevel
    ...
    private async setLogLevelForChannel(logChannel: LogChannelQuickPickItem): Promise<void> {
        ...
        const currentLogLevel = this.loggerService.getLogLevel(logChannel.resource) ??
            defaultLogLevel;
        ...
        return [
            {
                ...
                buttons: button && defaultLogLevel !== LogLevel.Trace
                    ? [button]
                    : undefined,
            },
            {
                ...
                buttons: button && defaultLogLevel !== LogLevel.Debug
                    ? [button]
                    : undefined,
            },
            {
                ...
                buttons: button && defaultLogLevel !== LogLevel.Info
                    ? [button]
                    : undefined,
            },
            {
                ...
                buttons: button && defaultLogLevel !== LogLevel.Warning
                    ? [button]
                    : undefined,
            },
            {
                ...
                buttons: button && defaultLogLevel !== LogLevel.Error
                    ? [button]
                    : undefined,
            },
            {
                ...
                buttons: button && defaultLogLevel !== LogLevel.Off
                    ? [button]
                    : undefined,
            },
        ];
...
export class OpenWindowSessionLogFileAction extends Action {
    ...
    override async run(): Promise<void> {
        const sessionResult = await this.quickInputService.pick(this.getSessions().then((sessions) => sessions.map((s, index): IQuickPickItem => ({
            id: s.toString(),
            label: basename(s),
            description: index === 0
