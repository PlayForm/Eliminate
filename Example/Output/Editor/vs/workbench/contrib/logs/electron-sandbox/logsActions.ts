...
export class OpenLogsFolderAction extends Action {
    ...
    override run(): Promise<void> {
        return this.nativeHostService.showItemInFolder(joinPath(this.environmentService.logsHome, "main.log").with({
            scheme: Schemas.file,
        }).fsPath);
    }
}
export class OpenExtensionLogsFolderAction extends Action {
    ...
    override async run(): Promise<void> {
        ...
        if (folderStat.children && folderStat.children[0]) {
            return this.nativeHostService.showItemInFolder(folderStat.children[0].resource.with({ scheme: Schemas.file })
                .fsPath);
        }
    }
}
