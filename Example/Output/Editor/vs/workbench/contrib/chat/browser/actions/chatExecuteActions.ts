...
class SubmitWithoutDispatchingAction extends Action2 {
    ...
    constructor() {
        super({
            ...
            f1: false,
            category: CHAT_CATEGORY,
            precondition: ContextKeyExpr.and(ChatContextKeys.inputHasText, ChatContextKeys.requestInProgress.negate(), ContextKeyExpr.and(ChatContextKeys.location.isEqualTo(ChatAgentLocation.Panel))),
            keybinding: {
                when: ChatContextKeys.inChatInput,
...
MenuRegistry.appendMenuItem(MenuId.ChatExecute, {
    ...
    order: 3,
    group: 'navigation',
    when: ContextKeyExpr.and(ChatContextKeys.languageModelsAreUserSelectable, ContextKeyExpr.or(ContextKeyExpr.equals(ChatContextKeys.location.key, ChatAgentLocation.Panel), ContextKeyExpr.equals(ChatContextKeys.location.key, ChatAgentLocation.EditingSession), ContextKeyExpr.equals(ChatContextKeys.location.key, ChatAgentLocation.Editor))),
});
