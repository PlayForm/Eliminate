...
export class ChatEditingService extends Disposable implements IChatEditingService {
    ...
    private readonly _workbenchAssignmentService: IWorkbenchAssignmentService) {
        super();
    ...
    async startOrContinueEditingSession(chatSessionId: string, options?: {
        silent: boolean;
    ...
    private async _createEditingSession(chatSessionId: string, options?: {
        silent: boolean;
    }): Promise<IChatEditingSession> {
    ...
    private async _continueEditingSession(builder: (stream: IChatEditingSessionStream, token: CancellationToken) => Promise<void>, options?: {
        silent?: boolean;
    }): Promise<void> {
        const session = this._currentSessionObs.get();
        if (!session) {
...
class ChatDecorationsProvider extends Disposable implements IDecorationsProvider {
    ...
    constructor(private readonly _session: IObservable<IChatEditingSession | null>) {
        super();
    }
...
export class ChatEditingMultiDiffSourceResolver implements IMultiDiffSourceResolver {
    ...
    constructor(private readonly _currentSession: IObservable<ChatEditingSession | null>, 
    @IInstantiationService
    private readonly _instantiationService: IInstantiationService) { }
