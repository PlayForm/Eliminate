...
MenuRegistry.appendMenuItem(MenuId.CommandCenter, {
    ...
    icon: defaultChat.icon,
    when: ContextKeyExpr.and(ContextKeyExpr.has('config.chat.commandCenter.enabled'), ContextKeyExpr.or(ChatContextKeys.panelParticipantRegistered, ChatContextKeys.installEntitled)),
    order: 10001,
});
registerAction2(class ToggleChatControl extends ToggleTitleBarConfigAction {
    constructor() {
        super('chat.commandCenter.enabled', localize('toggle.chatControl', 'Chat Controls'), localize('toggle.chatControlsDescription', "Toggle visibility of the Chat Controls in title bar"), 3, false, ContextKeyExpr.and(ContextKeyExpr.has('config.window.commandCenter'), ContextKeyExpr.or(ChatContextKeys.panelParticipantRegistered, ChatContextKeys.installEntitled)));
    }
});
export class ChatCommandCenterRendering implements IWorkbenchContribution {
    ...
    instantiationService: IInstantiationService) {
        this._store.add(actionViewItemService.register(MenuId.CommandCenter, MenuId.ChatCommandCenter, (action, options) => {
            ...
            return instantiationService.createInstance(DropdownWithPrimaryActionViewItem, primaryAction, dropdownAction, action.actions, '', {
                ...options,
