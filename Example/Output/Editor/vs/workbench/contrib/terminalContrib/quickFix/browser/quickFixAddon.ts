...
import { ITerminalQuickFix, ITerminalQuickFixCommandAction, ITerminalQuickFixInternalOptions, ITerminalQuickFixOpenerAction, ITerminalQuickFixOptions, ITerminalQuickFixProviderSelector, ITerminalQuickFixResolvedExtensionOptions, ITerminalQuickFixService, ITerminalQuickFixTerminalCommandAction, ITerminalQuickFixUnresolvedExtensionOptions, TerminalQuickFixType, } from "./quickFix.js";
...
export class TerminalQuickFixAddon extends Disposable implements ITerminalAddon, ITerminalQuickFixAddon {
    ...
    constructor(private readonly _aliases: string[][] | undefined, private readonly _capabilities: ITerminalCapabilityStore, 
    ...
    private async _resolveQuickFixes(command: ITerminalCommand, aliases?: string[][]): Promise<void> {
        ...
        const resolver = async (selector: ITerminalQuickFixOptions, lines?: string[]) => {
            ...
            return this._quickFixService.providers
                .get(id)
                ?.provideTerminalQuickFixes(command, lines, {
                type: "resolved",
                commandLineMatcher: selector.commandLineMatcher,
                outputMatcher: selector.outputMatcher,
                commandExitResult: selector.commandExitResult,
...
export async function getQuickFixesForCommand(aliases: string[][] | undefined, terminal: Terminal, terminalCommand: ITerminalCommand, quickFixOptions: Map<string, ITerminalQuickFixOptions[]>, commandService: ICommandService, openerService: IOpenerService, labelService: ILabelService, onDidRequestRerunCommand?: Emitter<{
    command: string;
    shouldExecute?: boolean;
    ...
    for (const options of quickFixOptions.values()) {
        for (const option of options) {
            if ((option.commandExitResult === "success" &&
                    ...
                    if ("type" in quickFix) {
                        switch (quickFix.type) {
                            ...
                            case TerminalQuickFixType.Opener: {
                                ...
                                const isUrl = fix.uri.scheme === Schemas.http ||
...
class TerminalQuickFixItem {
    ...
    constructor(readonly action: ITerminalAction, readonly type: TerminalQuickFixType, readonly source: string, readonly title: string | undefined, readonly kind: "fix" | "explain" = "fix") { }
}
