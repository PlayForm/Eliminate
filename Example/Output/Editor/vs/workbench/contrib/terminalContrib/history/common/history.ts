...
let directoryHistory: ITerminalPersistedHistory<{
    remoteAuthority?: string;
}> | undefined = undefined;
export function getDirectoryHistory(accessor: ServicesAccessor): ITerminalPersistedHistory<{
    remoteAuthority?: string;
}> {
    if (!directoryHistory) {
        directoryHistory = accessor
            .get(IInstantiationService)
            .createInstance(TerminalPersistedHistory, "dirs") as TerminalPersistedHistory<{
            remoteAuthority?: string;
        }>;
    }
    return directoryHistory;
}
let commandHistory: ITerminalPersistedHistory<{
    shellType: TerminalShellType;
}> | undefined = undefined;
export function getCommandHistory(accessor: ServicesAccessor): ITerminalPersistedHistory<{
    shellType: TerminalShellType | undefined;
}> {
    if (!commandHistory) {
        commandHistory = accessor
            .get(IInstantiationService)
            .createInstance(TerminalPersistedHistory, "commands") as TerminalPersistedHistory<{
            shellType: TerminalShellType;
        }>;
    }
...
export class TerminalPersistedHistory<T> extends Disposable implements ITerminalPersistedHistory<T> {
    ...
    constructor(private readonly _storageDataKey: string, 
    ...
    private readonly _storageService: IStorageService) {
        ...
        this._register(this._storageService.onDidChangeValue(StorageScope.APPLICATION, this._getTimestampStorageKey(), this._store)(() => {
            if (!this._isStale) {
                this._isStale =
                    this._storageService.getNumber(this._getTimestampStorageKey(), StorageScope.APPLICATION, 0) !== this._timestamp;
            }
        }));
    }
...
export async function fetchPwshHistory(accessor: ServicesAccessor) {
    ...
    let filePath: string;
    const remoteEnvironment = await remoteAgentService.getEnvironment();
    const isFileWindows = remoteEnvironment?.os === OperatingSystem.Windows ||
        (!remoteEnvironment && isWindows);
    if (isFileWindows) {
...
async function fetchFileContents(folderPrefix: string | undefined, filePath: string, isFileWindows: boolean, fileService: Pick<IFileService, "readFile">, remoteAgentService: Pick<IRemoteAgentService, "getConnection">): Promise<string | undefined> {
    if (!folderPrefix) {
        return undefined;
    }
