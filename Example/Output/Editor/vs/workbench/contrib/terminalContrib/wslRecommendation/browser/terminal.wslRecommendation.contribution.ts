...
export class TerminalWslRecommendationContribution extends Disposable implements IWorkbenchContribution {
    ...
    terminalService: ITerminalService) {
        ...
        let listener: IDisposable | undefined = terminalService.onDidCreateInstance(async (instance) => {
            ...
            listener?.dispose();
            listener = undefined;
            const extId = Object.keys(exeBasedExtensionTips.wsl.recommendations).find((extId) => exeBasedExtensionTips.wsl.recommendations[extId]
                .important);
            ...
            notificationService.prompt(Severity.Info, localize("useWslExtension.title", "The '{0}' extension is recommended for opening a terminal in WSL.", exeBasedExtensionTips.wsl.friendlyName), [
                {
                    ...
                    run: () => {
                        instantiationService
                            .createInstance(InstallRecommendedExtensionAction, extId)
                            .run();
                    },
                },
            ], {
