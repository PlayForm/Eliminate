...
export class TerminalSearchLinkOpener implements ITerminalLinkOpener {
    ...
    async open(link: ITerminalSimpleLink): Promise<void> {
        ...
        if (link.contextLine) {
            ...
            const matchingParsedLink = parsedLinks.find((parsedLink) => parsedLink.suffix &&
                link.text.startsWith(parsedLink.path.text));
        ...
        this._workspaceContextService
            ...
            if (text.substring(0, folder.name.length + 1) ===
    ...
    private async _getExactMatch(sanitizedLink: string): Promise<IResourceMatch | undefined> {
        ...
        let absolutePath: string | undefined = isAbsolute
            ? sanitizedLink
            : undefined;
        ...
        if (!resourceMatch) {
            ...
            if (results.results.length > 0) {
                if (results.results.length === 1) {
                    ...
                    const results = await this._searchService.fileSearch(this._fileQueryBuilder.file(this._workspaceContextService.getWorkspace()
                        .folders, {
                        filePattern: `**/${sanitizedLink}`,
                    }));
...
export class TerminalUrlLinkOpener implements ITerminalLinkOpener {
    constructor(private readonly _isRemote: boolean, 
    @IOpenerService
    ...
    async open(link: ITerminalSimpleLink): Promise<void> {
        ...
        this._openerService.open(link.text, {
            allowTunneling: this._isRemote &&
                this._configurationService.getValue("remote.forwardOnOpen"),
