...
export class TerminalLinkManager extends DisposableStore {
    ...
    constructor(private readonly _xterm: Terminal, private readonly _processInfo: ITerminalProcessInfo, capabilities: ITerminalCapabilityStore, private readonly _linkResolver: ITerminalLinkResolver, 
    ...
    protected async _getLinksForType(y: number, type: "word" | "url" | "localFile" | "localFolder"): Promise<ILink[] | undefined> {
        switch (type) {
            case "word":
                return await new Promise<ILink[] | undefined>((r) => this._standardLinkProviders
            ...
            case "url":
                return await new Promise<ILink[] | undefined>((r) => this._standardLinkProviders
                    ...
                    ?.provideLinks(y, r));
            case "localFile": {
                const links = await new Promise<ILink[] | undefined>((r) => this._standardLinkProviders
                    .get(TerminalLocalLinkDetector.id)
                    ?.provideLinks(y, r));
                return links?.filter((link) => (link as TerminalLink).type ===
    ...
    private _showHover(targetOptions: ILinkHoverTargetOptions, text: IMarkdownString, actions: IHoverAction[] | undefined, linkHandler: (url: string) => void, link?: TerminalLink): IDisposable | undefined {
        if (this._widgetManager) {
