...
export class TerminalAccessibilityHelpProvider extends Disposable implements IAccessibleViewContentProvider {
    ...
    provideContent(): string {
        ...
        if (this._hasShellIntegration) {
            ...
            content.push("- " +
                localize("goToNextCommand", "Go to Next Command<keybinding:{0}> in the accessible view", TerminalAccessibilityCommandId.AccessibleBufferGoToNextCommand));
            content.push("- " +
                localize("goToPreviousCommand", "Go to Previous Command<keybinding:{0}> in the accessible view", TerminalAccessibilityCommandId.AccessibleBufferGoToPreviousCommand));
            content.push("- " +
                localize("goToSymbol", "Go to Symbol<keybinding:{0}>", AccessibilityCommandId.GoToSymbol));
            content.push("- " +
                localize("runRecentCommand", "Run Recent Command<keybinding:{0}>", TerminalHistoryCommandId.RunRecentCommand));
            content.push("- " +
                localize("goToRecentDirectory", "Go to Recent Directory<keybinding:{0}>", TerminalHistoryCommandId.GoToRecentDirectory));
        }
        else {
