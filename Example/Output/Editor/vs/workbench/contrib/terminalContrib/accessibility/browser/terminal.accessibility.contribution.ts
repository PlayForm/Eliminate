...
export class TerminalAccessibleViewContribution extends Disposable implements ITerminalContribution {
    ...
    private _xterm: (Pick<IXtermTerminal, "shellIntegration" | "getFont"> & {
        raw: Terminal;
    }) | undefined;
    ...
    private readonly _terminalService: ITerminalService) {
        ...
        this._register(AccessibleViewAction.addImplementation(90, "terminal", () => {
            if (this._terminalService.activeInstance !==
                this._ctx.instance) {
                return false;
            }
            this.show();
    ...
    show(): void {
        ...
        const position = this._configurationService.getValue(TerminalAccessibilitySettingId.AccessibleViewPreserveCursorPosition)
            ? this._accessibleViewService.getPosition(AccessibleViewProviderId.Terminal)
            : undefined;
    ...
    navigateToCommand(type: NavigationType): void {
        ...
        const filteredCommands = type === NavigationType.Previous
            ? commands
...
registerTerminalAction({
    ...
    run: (c, accessor) => accessor
        ...
        ?.setPosition(new Position(1, 1), true),
});
// #endregion
