...
export class TerminalStickyScrollOverlay extends Disposable {
    ...
    constructor(private readonly _instance: ITerminalInstance, private readonly _xterm: IXtermTerminal & {
        raw: RawXtermTerminal;
    }, private readonly _xtermColorProvider: IXtermColorProvider, private readonly _commandDetection: ICommandDetectionCapability, xtermCtor: Promise<typeof XTermTerminal>, 
    ...
    private readonly _themeService: IThemeService) {
        ...
        this._register(Event.runAndSubscribe(this._xterm.raw.buffer.onBufferChange, (buffer) => {
            this._setState((buffer ?? this._xterm.raw.buffer.active).type ===
                "normal"
                ? OverlayState.On
                : OverlayState.Off);
        }));
        ...
        this._register(Event.runAndSubscribe(configurationService.onDidChangeConfiguration, (e) => {
            if (!e ||
                e.affectsConfiguration(TerminalStickyScrollSettingId.MaxLineCount)) {
    ...
    private _refreshNow(): void {
        ...
        if (!("marker" in command)) {
            ...
            if (partialCommand?.commandStartMarker &&
    ...
    private _updateContent(command: ITerminalCommand | ICurrentPartialCommand, startMarker: IMarker) {
        ...
        const stickyScrollLineCount = Math.min(promptRowCount + commandRowCount - 1, maxLineCount) -
            rowOffset;
            ...
            if (this._element) {
                ...
                if (termBox.height > 0) {
                    ...
                    if (!isPartialCommand &&
                        ...
                        if (buffer.viewportY + stickyScrollLineCount >
                            command.endMarker.line) {
                            const diff = buffer.viewportY +
    ...
    private _ensureElement() {
        ...
        const scrollBarWidth = (this._xterm.raw as any as {
            _core: IXtermCore;
        })
