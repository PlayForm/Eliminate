...
import { IXtermCore, XtermAttributes, } from "../../../terminal/browser/xterm-private.js";
...
const isTenativeCharacterPrediction = (p: unknown): p is TentativeBoundary & {
...
class CursorMovePrediction implements IPrediction {
    ...
    constructor(private readonly _direction: CursorMoveDirection, private readonly _moveByWords: boolean, private readonly _amount: number) { }
    apply(buffer: IBuffer, cursor: Cursor) {
    ...
    rollback(cursor: Cursor) {
        ...
        return (cursor.move(this._applied.prevPosition, cursor.y) +
...
export class TypeAheadAddon extends Disposable implements ITerminalAddon {
    ...
    constructor(private _processManager: ITerminalProcessManager, 
        ...
        this._register(this._configurationService.onDidChangeConfiguration((e) => {
            if (e.affectsConfiguration(TERMINAL_CONFIG_SECTION)) {
                ...
                this._typeaheadThreshold =
                    this._configurationService.getValue<ITerminalTypeAheadConfiguration>(TERMINAL_CONFIG_SECTION).localEchoLatencyThreshold;
    ...
    protected _reevaluatePredictorStateNow(stats: PredictionStats, timeline: PredictionTimeline) {
        ...
        else if (stats.sampleSize > StatsConstants.StatsMinSamplesToTurnOn &&
    ...
    private _onUserData(data: string): void {
        ...
        const addLeftNavigating = (p: IPrediction) => this._timeline!.tentativeCursor(buffer).x <=
            this._lastRow!.startingX
            ? this._timeline!.addBoundary(buffer, p)
            : this._timeline!.addPrediction(buffer, p);
        const addRightNavigating = (p: IPrediction) => this._timeline!.tentativeCursor(buffer).x >=
        ...
        while (reader.remaining > 0) {
            if (reader.eatCharCode(127)) {
                ...
                if (this._timeline.tentativeCursor(buffer).x <=
                    this._lastRow.startingX) {
