...
const pwshTypeToIconMap: {
    [type: string]: ThemeIcon | undefined;
} = {
...
export class SuggestAddon extends Disposable implements ITerminalAddon, ISuggestController {
    ...
    constructor(private readonly _cachedPwshCommands: Set<SimpleCompletionItem>, private readonly _capabilities: ITerminalCapabilityStore, private readonly _terminalSuggestWidgetVisibleContextKey: IContextKey<boolean>, 
            ...
            if (commandDetection) {
                if (this._promptInputModel !==
                    ...
                    this._promptInputModelSubscriptions.value =
                        combinedDisposable(this._promptInputModel.onDidChangeInput((e) => this._sync(e)), this._promptInputModel.onDidFinishInput(() => this.hideSuggestWidget()));
                }
            }
            else {
    ...
    private _requestCompletions(): void {
        ...
        if (!this._codeCompletionsRequested &&
        ...
        if (!this._gitCompletionsRequested &&
    ...
    private _sync(promptInputState: IPromptInputModelState): void {
        ...
        if (!this._mostRecentPromptInputState ||
            ...
            if (!this._terminalSuggestWidgetVisibleContextKey.get()) {
                if (config.quickSuggestions) {
                    if (promptInputState.cursorIndex === 1 ||
        ...
        if (this._currentPromptInputState.cursorIndex <
        ...
        if (this._terminalSuggestWidgetVisibleContextKey.get()) {
            this._cursorIndexDelta =
                this._currentPromptInputState.cursorIndex -
                    (this._replacementIndex + this._replacementLength);
            let normalizedLeadingLineContent = this._currentPromptInputState.value.substring(this._replacementIndex, this._replacementIndex +
    ...
    private _handleCompletionsSequence(terminal: Terminal, data: string, command: string, args: string[]): void {
        ...
        this._leadingLineContent =
            this._currentPromptInputState.prefix.substring(replacementIndex, replacementIndex + replacementLength + this._cursorIndexDelta);
        const payload = data.slice(command.length +
            args[0].length +
            args[1].length +
            args[2].length +
            4 /*semi-colons*/);
        const rawCompletions: PwshCompletion | PwshCompletion[] | CompressedPwshCompletion[] | CompressedPwshCompletion = args.length === 0 || payload.length === 0
            ? undefined
            : JSON.parse(payload);
        const completions = parseCompletionsFromShell(rawCompletions);
        const firstChar = this._leadingLineContent.length === 0
            ? ""
        ...
        if (this._mostRecentCompletion?.isDirectory &&
        ...
        if (replacementIndex !== 100 && completionList.length > 0) {
            ...
            if (!this._cachedFirstWord) {
                ...
                this._cachedFirstWord.sort((a, b) => {
                    ...
                    const isANonAlpha = aCode < 65 || (aCode > 90 && aCode < 97) || aCode > 122
                        ? 1
                        : 0;
                    const isBNonAlpha = bCode < 65 || (bCode > 90 && bCode < 97) || bCode > 122
                        ? 1
                        : 0;
    ...
    private _getTerminalDimensions(): {
        ...
        const cssCellDims = (this._terminal as any as {
            _core: IXtermCore;
        })
    ...
    acceptSelectedSuggestion(suggestion?: Pick<ISimpleSelectedSuggestion, "item" | "model">, respectRunOnEnter?: boolean): void {
        ...
        (currentPromptInputState.ghostTextIndex === -1 ||
            ...
            const spaceIndex = currentPromptInputState.value
                ...
                : currentPromptInputState.ghostTextIndex)
                .indexOf(" ");
            rightSideReplacementText = currentPromptInputState.value.substring(currentPromptInputState.cursorIndex, spaceIndex === -1
                ? undefined
...
function rawCompletionToSimpleCompletionItem(rawCompletion: PwshCompletion): SimpleCompletionItem {
    ...
    const isExecutable = rawCompletion.ResultType === 2 &&
...
function getIcon(resultType: number, customIconId?: string): ThemeIcon {
    if (customIconId) {
        const icon: ThemeIcon | undefined = customIconId in Codicon
            ? (Codicon as {
                [id: string]: ThemeIcon | undefined;
            })[customIconId]
            : Codicon.symbolText;
        if (icon) {
            return icon;
