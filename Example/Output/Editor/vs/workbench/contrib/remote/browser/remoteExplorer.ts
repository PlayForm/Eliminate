...
import { ForwardPortAction, OpenPortInBrowserAction, OpenPortInPreviewAction, openPreviewEnabledContext, TunnelPanel, TunnelPanelDescriptor, TunnelViewModel, } from "./tunnelView.js";
            ...
            this.contextKeyListener.value =
                this.contextKeyService.onDidChangeContext((e) => {
                    if (e.affectsSome(new Set([
                        ...forwardedPortsFeaturesEnabled.keys(),
                        ...forwardedPortsViewEnabled.keys(),
                    ]))) {
                        this.enableForwardedPortsFeatures();
                    }
                });
...
export class AutomaticPortForwarding extends Disposable implements IWorkbenchContribution {
    ...
    private readonly preferencesService: IPreferencesService) {
        ...
        configurationService
            ...
            this._register(configurationService.onDidChangeConfiguration((e) => {
                ...
                else if (e.affectsConfiguration(PORT_AUTO_FALLBACK_SETTING) &&
                    !this.portListener) {
                    this.listenForPorts();
                }
            }));
    ...
    private getPortAutoFallbackNumber(): number {
        ...
        if (inspectSource.applicationValue ===
...
class OnAutoForwardedAction extends Disposable {
    ...
    private async portNumberHeuristicDelay(): Promise<RemoteTunnel | undefined> {
        ...
        else if (firstTunnel.tunnelRemotePort < 10000 &&
    ...
    private elevateChoice(tunnel: RemoteTunnel): IPromptChoice {
        return {
            ...
            run: async () => {
                ...
                this.lastNotification = this.notificationService.prompt(Severity.Info, (await this.basicMessage(newTunnel)) + this.linkMessage(), [
...
class OutputAutomaticPortForwarding extends Disposable {
    ...
    constructor(private readonly terminalService: ITerminalService, readonly notificationService: INotificationService, readonly openerService: IOpenerService, readonly externalOpenerService: IExternalUriOpenerService, private readonly remoteExplorerService: IRemoteExplorerService, private readonly configurationService: IConfigurationService, private readonly debugService: IDebugService, readonly tunnelService: ITunnelService, readonly hostService: IHostService, readonly logService: ILogService, readonly contextKeyService: IContextKeyService, readonly privilegedOnly: () => boolean) {
        ...
        this.tryStartStopUrlFinder();
        if (configurationService.getValue(PORT_AUTO_SOURCE_SETTING) ===
            PORT_AUTO_SOURCE_SETTING_HYBRID) {
...
class ProcAutomaticPortForwarding extends Disposable {
    ...
    private async forwardCandidates(): Promise<RemoteTunnel[] | undefined> {
        ...
        for (const value of this.remoteExplorerService.tunnelModel.candidates) {
            ...
            if (!attributes) {
                attributes =
                    await this.remoteExplorerService.tunnelModel.getAttributes(this.remoteExplorerService.tunnelModel.candidates);
            }
            ...
            if (this.notifiedOnly.has(address) ||
    ...
    private async handleCandidateUpdate(removed: Map<string, {
        host: string;
        port: number;
    }>) {
