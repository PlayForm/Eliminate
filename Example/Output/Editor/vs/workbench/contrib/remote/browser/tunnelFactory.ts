...
import { ITunnel, ITunnelService, RemoteTunnel, TunnelCreationOptions, TunnelOptions, TunnelPrivacyId, TunnelProtocol, } from "../../../../platform/tunnel/common/tunnel.js";
...
export class TunnelFactoryContribution extends Disposable implements IWorkbenchContribution {
    ...
    contextKeyService: IContextKeyService) {
        ...
        if (tunnelFactory) {
            ...
            let privacyOptions = environmentService.options?.tunnelProvider?.features
                ?.privacyOptions ?? [];
            ...
            const tunnelInformation = environmentService.options?.tunnelProvider
                ...
                ? {
                    features: {
                        elevation: !!environmentService.options?.tunnelProvider
                            ?.features?.elevation,
                        public: !!environmentService.options?.tunnelProvider
                            ?.features?.public,
                        privacyOptions,
                        protocol: environmentService.options?.tunnelProvider
