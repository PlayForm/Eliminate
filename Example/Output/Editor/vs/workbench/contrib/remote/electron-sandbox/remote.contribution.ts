...
import { IWorkbenchContribution, IWorkbenchContributionsRegistry, registerWorkbenchContribution2, Extensions as WorkbenchContributionsExtensions, WorkbenchPhase, } from "../../../common/contributions.js";
import { OpenLocalFileCommand, OpenLocalFileFolderCommand, OpenLocalFolderCommand, RemoteFileDialogContext, SaveLocalFileCommand, } from "../../../services/dialogs/browser/simpleFileDialog.js";
...
class RemoteAgentDiagnosticListener implements IWorkbenchContribution {
    ...
    labelService: ILabelService) {
        ipcRenderer.on("vscode:getDiagnosticInfo", (event: unknown, request: {
            replyChannel: string;
                ...
                remoteAgentService
                    ...
                    if (info) {
                        (info as IRemoteDiagnosticInfo).hostName =
                            hostName;
                        if (remoteConnectionLatencyMeasurer.latency
                            ?.high) {
                            (info as IRemoteDiagnosticInfo).latency = {
                                average: remoteConnectionLatencyMeasurer
                                    .latency.average,
                                current: remoteConnectionLatencyMeasurer
                                    .latency.current,
                            };
                        }
                    }
...
class RemoteEmptyWorkbenchPresentation extends Disposable implements IWorkbenchContribution {
    ...
    contextService: IWorkspaceContextService) {
        ...
        function shouldShowExplorer(): boolean {
            ...
            return (startupEditor !== "welcomePage" &&
