...
export class ExtensionsAutoProfiler implements IWorkbenchContribution {
    ...
    timerService: ITimerService) {
        timerService.perfBaseline.then((value) => {
            ...
            this._unresponsiveListener =
                _extensionService.onDidChangeResponsiveChange(this._onDidChangeResponsiveChange, this);
        });
    }
    dispose(): void {
    ...
    private async _processCpuProfile(profile: IExtensionHostProfile) {
        ...
        if (this._configService.getValue("application.experimental.rendererProfiling")) {
            ...
            searchTree.fill(this._extensionService.extensions.map((e) => [
                e.extensionLocation,
                e,
            ]));
            await this._profileAnalysisService.analyseBottomUp(profile.data, (url) => searchTree.findSubstr(URI.parse(url))?.identifier.value ??
                "<<not-found>>", this._perfBaseline, false);
        }
