...
class RecommendationsNotification extends Disposable {
    ...
    private updateNotificationHandle(notificationHandle: INotificationHandle) {
        ...
        this.onDidChangeVisibilityDisposable.value =
            this.notificationHandle.onDidChangeVisibility((e) => this._onDidChangeVisibility.fire(e));
    }
}
...
export class ExtensionRecommendationNotificationService extends Disposable implements IExtensionRecommendationNotificationService {
    ...
    get ignoredRecommendations(): string[] {
        return distinct([
            ...(<string[]>(JSON.parse(this.storageService.get(ignoreImportantExtensionRecommendationStorageKey, StorageScope.PROFILE, "[]")))),
        ].map((i) => i.toLowerCase()));
    }
    ...
    hasToIgnoreRecommendationNotifications(): boolean {
        ...
        return (config.ignoreRecommendations ||
    ...
    async promptImportantExtensionsInstallNotification(extensionRecommendations: IExtensionRecommendations): Promise<RecommendationsNotificationResult> {
        ...
        return this.promptRecommendationsNotification({ ...extensionRecommendations, extensions }, {
            onDidInstallRecommendedExtensions: (extensions: IExtension[]) => extensions.forEach((extension) => this.telemetryService.publicLog2<{
                ...
                extensionId: string;
                source: string;
            }, ExtensionRecommendationsNotificationClassification>("extensionRecommendations:popup", {
    ...
    async promptWorkspaceRecommendations(recommendations: Array<string | URI>): Promise<void> {
        ...
        installed = installed.filter((l) => this.extensionEnablementService.getEnablementState(l) !==
            EnablementState.DisabledByExtensionKind); // Filter extensions disabled by kind
        ...
        await this.promptRecommendationsNotification({
            ...
            onDidInstallRecommendedExtensions: () => this.telemetryService.publicLog2<{
                userReaction: string;
            }, ExtensionWorkspaceRecommendationsNotificationClassification>("extensionWorkspaceRecommendations:popup", {
                userReaction: "install",
            }),
    ...
    private async promptRecommendationsNotification({ extensions: extensionIds, source, name, searchValue, }: ExtensionRecommendations, recommendationsNotificationActions: RecommendationsNotificationActions): Promise<RecommendationsNotificationResult> {
        ...
        if (extensions.length === 1) {
            ...
            const publishers = [
                ...extensions.reduce((result, extension) => result.add(extension.publisherDisplayName), new Set<string>()),
            ];
        ...
        const donotShowAgainLabel = source === RecommendationSource.WORKSPACE
            ...
            const installExtensions = async (isMachineScoped: boolean) => {
                ...
                await Promises.settled<any>([
                    ...
                    galleryExtensions.length
                        ? this.extensionManagementService.installGalleryExtensions(galleryExtensions.map((e) => ({
                            extension: e,
    ...
    private getNextPendingNotificationIndex(): number {
        ...
        if (this.pendingNotificaitons.length) {
            for (let i = 0; i < this.pendingNotificaitons.length; i++) {
                if (this.pendingNotificaitons[i].source <=
