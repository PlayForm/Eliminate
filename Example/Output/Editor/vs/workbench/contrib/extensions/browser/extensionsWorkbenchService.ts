...
import { DidUninstallExtensionEvent, EXTENSION_IDENTIFIER_REGEX, IDeprecationInfo, IExtensionGalleryService, IExtensionInfo, IExtensionQueryOptions, IExtensionsControlManifest, IGalleryExtension, ILocalExtension, InstallExtensionEvent, InstallExtensionInfo, InstallExtensionResult, InstallOperation, InstallOptions, IProductVersion, IQueryOptions, isTargetPlatformCompatible, UninstallExtensionInfo, WEB_EXTENSION_TAG, } from "../../../../platform/extensionManagement/common/extensionManagement.js";
import { areSameExtensions, getGalleryExtensionId, getGalleryExtensionTelemetryData, getLocalExtensionTelemetryData, groupByExtension, } from "../../../../platform/extensionManagement/common/extensionManagementUtil.js";
...
import { DefaultIconPath, EnablementState, extensionsConfigurationNodeBase, IExtensionManagementServer, IExtensionManagementServerService, IResourceExtension, IWorkbenchExtensionEnablementService, IWorkbenchExtensionManagementService, } from "../../../services/extensionManagement/common/extensionManagement.js";
...
export class Extension implements IExtension {
    ...
    constructor(private stateProvider: IExtensionStateProvider<ExtensionState>, private runtimeStateProvider: IExtensionStateProvider<ExtensionRuntimeState | undefined>, public readonly server: IExtensionManagementServer | undefined, public local: ILocalExtension | undefined, private _gallery: IGalleryExtension | undefined, private readonly resourceExtensionInfo: {
        ...
        if (this.gallery) {
            ...
            if (!cache) {
                if (this.gallery.assets.manifest) {
                    this.galleryResourcesCache.set("manifest", (cache = this.galleryService
...
class Extensions extends Disposable {
    ...
    constructor(readonly server: IExtensionManagementServer, private readonly stateProvider: IExtensionStateProvider<ExtensionState>, private readonly runtimeStateProvider: IExtensionStateProvider<ExtensionRuntimeState | undefined>, private readonly isWorkspaceServer: boolean, 
    @IExtensionGalleryService
    private readonly galleryService: IExtensionGalleryService, 
    ...
    private async mapInstalledExtensionWithCompatibleGalleryExtension(galleryExtensions: IGalleryExtension[], productVersion: IProductVersion): Promise<[
        Extension,
        IGalleryExtension
    ][]> {
    ...
    private mapInstalledExtensionWithGalleryExtension(galleryExtensions: IGalleryExtension[]): [
        Extension,
        IGalleryExtension
    ][] {
    ...
    private async updateMetadata(localExtension: ILocalExtension, gallery: IGalleryExtension): Promise<ILocalExtension> {
        ...
        if (localExtension.manifest.version !== gallery.version) {
            ...
            const galleryWithLocalVersion: IGalleryExtension | undefined = (await this.galleryService.getExtensions([
                {
    ...
    private async onDidInstallExtensions(results: readonly InstallExtensionResult[]): Promise<void> {
        for (const event of results) {
            ...
            this._onChange.fire(!local || !extension
                ? undefined
    ...
    getExtensionState(extension: Extension): ExtensionState {
        if (extension.gallery &&
            this.installing.some((e) => !!e.gallery &&
                areSameExtensions(e.gallery.identifier, extension.gallery!.identifier))) {
...
export class ExtensionsWorkbenchService extends Disposable implements IExtensionsWorkbenchService, IURLHandler {
    ...
    private initializeAutoUpdate(): void {
        ...
        this._register(this.updateService.onStateChange((e) => {
            if ((e.type === StateType.CheckingForUpdates && e.explicit) ||
        ...
        if (this.configurationService.getValue(AutoRestartConfigurationKey) ===
            ...
            this.autoRestartListenerDisposable.value =
                this.hostService.onDidChangeFocus((focus) => {
                    if (!focus &&
                        this.configurationService.getValue(AutoRestartConfigurationKey) === true) {
    ...
    private reportInstalledExtensionsTelemetry() {
        ...
        this.telemetryService.publicLog2<InstalledExtensionsEvent, ExtensionsLoadClassification>("installedExtensions", {
    ...
    async queryGallery(arg1: any, arg2?: any): Promise<IPager<IExtension>> {
        ...
        const options: IQueryOptions = CancellationToken.isCancellationToken(arg1)
            ? {}
            : arg1;
        const token: CancellationToken = CancellationToken.isCancellationToken(arg1)
            ? arg1
            : arg2;
    ...
    private updateExtensionsNotificaiton(): void {
        ...
        if (this.extensionsNotification?.message !==
    ...
    private computeExtensionsNotifications(): Array<Omit<IExtensionsNotification, "dismiss"> & {
        key: string;
    }> {
        const computedNotificiations: Array<Omit<IExtensionsNotification, "dismiss"> & {
            key: string;
        }> = [];
        const invalidExtensions = this.local.filter((e) => e.enablementState ===
    ...
    private getInstalledExtensionMatchingLocation(location: URI): IExtension | null {
        return (this.local.find((e) => e.local &&
            this.uriIdentityService.extUri.isEqualOrParent(location, e.local?.location)) ?? null);
    }
    ...
    async updateRunningExtensions(auto: boolean = false): Promise<void> {
        ...
        if (toAdd.length || toRemove.length) {
            if (await this.extensionService.stopExtensionHosts(nls.localize("restart", "Enable or Disable extensions"), auto)) {
                ...
                this.telemetryService.publicLog2<ExtensionsAutoRestartEvent, ExtensionsAutoRestartClassification>("extensions:autorestart", {
    ...
    private getRuntimeState(extension: IExtension): ExtensionRuntimeState | undefined {
        ...
        const reloadAction = this.extensionManagementServerService
            ...
            : ExtensionRuntimeActionType.RestartExtensions;
        const reloadActionLabel = reloadAction === ExtensionRuntimeActionType.ReloadWindow
            ? nls.localize("reload", "reload window")
            : nls.localize("restart extensions", "restart extensions");
        if (isUninstalled) {
            const canRemoveRunningExtension = runningExtension &&
                        ...
                        if (this.extensionsServers.length > 1) {
                            ...
                            if (extensionInOtherServer) {
                                ...
                                if (runningExtensionServer ===
                                    ...
                                    return {
                                        action: reloadAction,
                                        reason: nls.localize("enable remote", "Please {0} to enable this extension in {1}.", reloadActionLabel, this
            ...
            if (extension.gallery) {
                toUpdate.push({
                    ...
                    options: {
                        ...
                        profileLocation: this.userDataProfileService.currentProfile
                            .extensionsResource,
    ...
    async shouldRequireConsentToUpdate(extension: IExtension): Promise<string | undefined> {
        ...
        if (extension.local?.manifest.main ||
    ...
    async canInstall(extension: IExtension): Promise<boolean> {
        ...
        if (extension.resourceExtension &&
            (await this.extensionManagementService.canInstall(extension.resourceExtension))) {
            return true;
        }
        return false;
    }
            ...
            if (isString(arg)) {
                ...
                if (!extension?.isBuiltin) {
                    installableInfo = {
                        ...
                        preRelease: installOptions.installPreReleaseVersion ??
                            this.preferPreReleases,
                    };
                }
            }
        ...
        if (installable) {
            if (installOptions.justification) {
                const syncCheck = isUndefined(installOptions.isMachineScoped) &&
                    ...
                    this.userDataSyncEnablementService.isResourceEnabled(SyncResource.Extensions);
                const buttons: IPromptButton<boolean>[] = [];
                buttons.push({
                    label: isString(installOptions.justification) ||
                        ...
                        : nls.localize({
                            ...
                            comment: ["&& denotes a mnemonic"],
                        }, "&&Install Extension and {0}", installOptions.justification.action),
                    run: () => true,
                });
                if (!extension) {
                    buttons.push({
                    ...
                    const result = await this.dialogService.confirm({
                        ...
                        primaryButton: isString(installOptions.justification)
                            ...
                            : nls.localize({
                                ...
                                comment: ["&& denotes a mnemonic"],
                            }, "&&Enable Extension and {0}", installOptions.justification.action),
                    });
                    if (!result.confirmed) {
                        throw new CancellationError();
                    }
                }
                await this.setEnablement(extension, extension.enablementState ===
                    EnablementState.DisabledWorkspace
                    ? EnablementState.EnabledWorkspace
                    : EnablementState.EnabledGlobally);
            }
    ...
    reinstall(extension: IExtension): Promise<IExtension> {
        return this.doInstall(extension, () => {
            const ext = extension.local
                ? extension
                : this.local.filter((e) => areSameExtensions(e.identifier, extension.identifier))[0];
    ...
    async toggleExtensionIgnoredToSync(extension: IExtension): Promise<void> {
        ...
        if (extension.local && isIgnored) {
            (<Extension>extension).local =
                await this.updateSynchronizingInstalledExtension(extension.local, true);
            this._onChange.fire(extension);
        }
        else {
    ...
    async toggleApplyExtensionToAllProfiles(extension: IExtension): Promise<void> {
        ...
        await Promise.all(this.getAllExtensions().map(async (extensions) => {
            ...
            if (local &&
                ...
                await this.extensionManagementService.toggleAppliationScope(local, this.userDataProfileService.currentProfile
                    .extensionsResource);
            }
        }));
    }
    ...
    private async waitAndGetInstalledExtension(identifier: IExtensionIdentifier): Promise<IExtension> {
        ...
        if (!installedExtension) {
            await Event.toPromise(Event.filter(this.onChange, (e) => !!e &&
                this.local.some((local) => areSameExtensions(local.identifier, identifier))));
        }
    ...
    private promptAndSetEnablement(extensions: IExtension[], enablementState: EnablementState): Promise<any> {
        const enable = enablementState === EnablementState.EnabledGlobally ||
    ...
    private getExtensionsRecursively(extensions: IExtension[], installed: IExtension[], enablementState: EnablementState, options: {
        dependencies: boolean;
        pack: boolean;
    }, checked: IExtension[] = []): IExtension[] {
        ...
        if (toCheck.length) {
            ...
            const extensionsToEanbleOrDisable = installed.filter((i) => {
                ...
                const enable = enablementState === EnablementState.EnabledGlobally ||
                    enablementState === EnablementState.EnabledWorkspace;
                const isExtensionEnabled = i.enablementState === EnablementState.EnabledGlobally ||
    ...
    private async doSetEnablement(extensions: IExtension[], enablementState: EnablementState): Promise<boolean[]> {
        ...
        for (let i = 0; i < changed.length; i++) {
            if (changed[i]) {
                ...
                */
                this.telemetryService.publicLog(enablementState === EnablementState.EnabledGlobally ||
                    enablementState === EnablementState.EnabledWorkspace
                    ? "extension:enable"
                    : "extension:disable", extensions[i].telemetryData);
            }
        }
        return changed;
    }
    ...
    private reportProgressFromOtherSources(): void {
        if (this.installed.some((e) => e.state === ExtensionState.Installing ||
    ...
    private get dismissedNotificationsValue(): string {
        if (!this._dismissedNotificationsValue) {
            this._dismissedNotificationsValue =
                this.getDismissedNotificationsValue();
        }
