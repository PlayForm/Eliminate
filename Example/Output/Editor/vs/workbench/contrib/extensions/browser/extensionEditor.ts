 ...
 *--------------------------------------------------------------------------------------------*/
import { $, addDisposableListener, append, Dimension, hide, setParentFlowTo, show, } from "../../../../base/browser/dom.js";
...
import { ButtonWithDropDownExtensionAction, ButtonWithDropdownExtensionActionViewItem, ClearLanguageAction, DisableDropDownAction, DropDownExtensionAction, EnableDropDownAction, ExtensionEditorManageExtensionAction, ExtensionRuntimeStateAction, ExtensionStatusAction, ExtensionStatusLabelAction, InstallAnotherVersionAction, InstallDropdownAction, InstallingLabelAction, LocalInstallAction, MigrateDeprecatedExtensionAction, RemoteInstallAction, SetColorThemeAction, SetFileIconThemeAction, SetLanguageAction, SetProductIconThemeAction, ToggleAutoUpdateForExtensionAction, TogglePreReleaseExtensionAction, UninstallAction, UpdateAction, WebInstallAction, } from "./extensionsActions.js";
...
import { ExtensionRecommendationWidget, ExtensionStatusWidget, ExtensionWidget, InstallCountWidget, onClick, RatingsWidget, RemoteBadgeWidget, SponsorWidget, VerifiedPublisherWidget, } from "./extensionsWidgets.js";
...
export class ExtensionEditor extends EditorPane {
    ...
    protected createEditor(parent: HTMLElement): void {
        ...
        this.showPreReleaseVersionContextKey =
            CONTEXT_SHOW_PRE_RELEASE_VERSION.bindTo(this._scopedContextKeyService.value);
        ...
        const extensionContainers: ExtensionContainers = this.instantiationService.createInstance(ExtensionContainers, [
            ...actions,
            ...widgets,
    ...
    private async render(extension: IExtension, template: IExtensionEditorTemplate, preserveFocus: boolean): Promise<void> {
        ...
        this.extensionReadme = new Cache(() => gallery
        ...
        template.publisher.parentElement?.classList.toggle("hide", !!extension.resourceExtension ||
        ...
        const location = extension.resourceExtension?.location ??
            (extension.local?.source === "resource"
        ...
        if (extRecommendations[extension.identifier.id.toLowerCase()]) {
            recommendationsData = {
                recommendationReason: extRecommendations[extension.identifier.id.toLowerCase()]
                    .reasonId,
            };
        }
        /* __GDPR__
    ...
    private onNavbarChange(extension: IExtension, { id, focus }: {
        id: string | null;
        focus: boolean;
    }, template: IExtensionEditorTemplate): void {
        this.contentDisposables.clear();
        template.content.innerText = "";
        this.activeElement = null;
        if (id) {
    ...
    private renderInstallInfo(container: HTMLElement, extension: ILocalExtension): void {
        ...
        if (extension.source !== "gallery") {
            const element = $("div", undefined, extension.source === "vsix"
                ? localize("vsix", "VSIX")
            ...
            if (isNative &&
                ...
                this.transientDisposables.add(onClick(element, () => this.openerService.open(extension.location, {
                    openExternal: true,
                })));
            }
        }
        if (extension.size) {
            ...
            append(installInfo, $(".more-info-entry", undefined, $("div", {
                title: localize("size when installed", "Size when installed"),
            }, localize("size", "Size")), element));
        ...
        this.getCacheLocation(extension).then((cacheLocation) => {
            ...
            computeSize(cacheLocation, this.fileService).then((cacheSize) => {
                ...
                append(installInfo, $(".more-info-entry", undefined, $("div", {
                    title: localize("disk space used", "Cache size"),
                }, localize("cache size", "Cache")), element));
                if (isNative && extension.location.scheme === Schemas.file) {
                    element.classList.add("link");
    ...
    private async renderExtensionPack(manifest: IExtensionManifest, parent: HTMLElement, token: CancellationToken): Promise<IActiveElement | null> {
        ...
        this.contentDisposables.add(toDisposable(arrays.insert(this.layoutParticipants, {
