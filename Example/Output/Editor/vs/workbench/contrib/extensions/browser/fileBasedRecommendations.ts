...
import { IFileContentCondition, IFileLanguageCondition, IFileOpenCondition, IFilePathCondition, } from "../../../../base/common/product.js";
...
export class FileBasedRecommendations extends ExtensionRecommendations {
    ...
    private readonly fileBasedRecommendations = new Map<string, {
            ...
            for (const condition of conditions) {
                ...
                if ((<IFilePathCondition>condition).pathGlob) {
                    ...
                    if (processedPathGlobs.get(pathGlob) ??
                        match((<IFilePathCondition>condition).pathGlob, uri.with({ fragment: "" }).toString())) {
    ...
    private promptFromRecommendations(uri: URI, model: ITextModel, extensionRecommendations: IStringDictionary<IFileOpenCondition[]>): void {
        ...
        if (importantRecommendations.size &&
            this.promptRecommendedExtensionForFileType(languageName &&
    ...
    private promptRecommendedExtensionForFileType(name: string, language: string, recommendations: string[]): boolean {
        ...
        const promptedRecommendations = language !== PLAINTEXT_LANGUAGE_ID
    ...
    private getCachedRecommendations(): IStringDictionary<number> {
        ...
        if (Array.isArray(storedRecommendations)) {
            storedRecommendations = storedRecommendations.reduce<IStringDictionary<number>>((result, id) => {
                result[id] = Date.now();
                return result;
            }, {});
        }
