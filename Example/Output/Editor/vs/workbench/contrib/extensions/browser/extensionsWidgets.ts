...
import { $, addDisposableListener, append, EventType, finalHandler, reset, } from "../../../../base/browser/dom.js";
...
import { ExtensionEditorTab, ExtensionState, IExtension, IExtensionContainer, IExtensionsWorkbenchService, } from "../common/extensions.js";
...
export function onClick(element: HTMLElement, callback: () => void): IDisposable {
    ...
    disposables.add(addDisposableListener(element, EventType.KEY_UP, (e) => {
        ...
        if (keyboardEvent.equals(KeyCode.Space) ||
...
export class SponsorWidget extends ExtensionWidget {
    ...
    render(): void {
        ...
        this.disposables.add(onClick(sponsor, () => {
            ...
            this.telemetryService.publicLog2<SponsorExtensionEvent, SponsorExtensionClassification>("extensionsAction.sponsorExtension", {
...
export class RemoteBadgeWidget extends ExtensionWidget {
    ...
    private element: HTMLElement;
    constructor(parent: HTMLElement, private readonly tooltip: boolean, 
...
class RemoteBadge extends Disposable {
    ...
    constructor(private readonly tooltip: boolean, 
    @IHoverService
    hoverService: IHoverService, 
    @ILabelService
    ...
    private render(): void {
        ...
        const applyBadgeStyle = () => {
            ...
            const bgColor = this.themeService
                .getColorTheme()
                .getColor(EXTENSION_BADGE_REMOTE_BACKGROUND);
            const fgColor = this.themeService
                .getColorTheme()
                .getColor(EXTENSION_BADGE_REMOTE_FOREGROUND);
        ...
        if (this.tooltip) {
            const updateTitle = () => {
                if (this.element &&
                    this.extensionManagementServerService
                        .remoteExtensionManagementServer) {
                    this.elementHover.update(localize("remote extension title", "Extension in {0}", this.extensionManagementServerService
...
export class SyncIgnoredWidget extends ExtensionWidget {
    ...
    render(): void {
        ...
        if (this.extension &&
            ...
            const element = append(this.container, $("span.extension-sync-ignored" +
...
export class ExtensionActivationStatusWidget extends ExtensionWidget {
    constructor(private readonly container: HTMLElement, private readonly small: boolean, 
    @IExtensionService
...
export class ExtensionHoverWidget extends ExtensionWidget {
    ...
    constructor(private readonly options: ExtensionHoverOptions, private readonly extensionStatusAction: ExtensionStatusAction, 
    @IExtensionsWorkbenchService
    private readonly extensionsWorkbenchService: IExtensionsWorkbenchService, 
    @IHoverService
    private readonly hoverService: IHoverService, 
    ...
    private getHoverMarkdown(): MarkdownString | undefined {
        ...
        if (location) {
            if (this.extension.isWorkspaceScoped &&
                this.contextService.isInsideWorkspace(location)) {
        ...
        if (extensionRuntimeStatus ||
            ...
            if (extensionRuntimeStatus) {
                ...
                if (extensionRuntimeStatus.runtimeErrors.length ||
                    extensionRuntimeStatus.messages.length) {
                    ...
                    const errorsLink = extensionRuntimeStatus.runtimeErrors
                        .length
                        ? `[${extensionRuntimeStatus.runtimeErrors.length === 1 ? localize("uncaught error", "1 uncaught error") : localize("uncaught errors", "{0} uncaught errors", extensionRuntimeStatus.runtimeErrors.length)}](${URI.parse(`command:extension.open?${encodeURIComponent(JSON.stringify([this.extension.identifier.id, ExtensionEditorTab.Features]))}`)})`
                        : undefined;
                    const messageLink = extensionRuntimeStatus.messages.length
                        ? `[${extensionRuntimeStatus.messages.length === 1 ? localize("message", "1 message") : localize("messages", "{0} messages", extensionRuntimeStatus.messages.length)}](${URI.parse(`command:extension.open?${encodeURIComponent(JSON.stringify([this.extension.identifier.id, ExtensionEditorTab.Features]))}`)})`
                        : undefined;
    ...
    private getRecommendationMessage(extension: IExtension): string | undefined {
        ...
        const bgColor = this.themeService
            .getColorTheme()
            .getColor(extensionButtonProminentBackground);
...
export class ExtensionStatusWidget extends ExtensionWidget {
    ...
    constructor(private readonly container: HTMLElement, private readonly extensionStatusAction: ExtensionStatusAction, 
    @IOpenerService
    private readonly openerService: IOpenerService) {
        super();
        this.render();
...
export class ExtensionRecommendationWidget extends ExtensionWidget {
    ...
    private getRecommendationStatus(): {
        ...
        const extRecommendations = this.extensionRecommendationsService.getAllRecommendationsWithReason();
        if (extRecommendations[this.extension.identifier.id.toLowerCase()]) {
            const reasonText = extRecommendations[this.extension.identifier.id.toLowerCase()]
                .reasonText;
            if (reasonText) {
...
export const extensionRatingIconColor = registerColor("extensionIcon.starForeground", {
    light: "#DF6100",
    dark: "#FF8E00",
    hcDark: "#FF8E00",
    hcLight: textLinkForeground,
}, localize("extensionIconStarForeground", "The icon color for extension ratings."), true);
...
export const extensionPreReleaseIconColor = registerColor("extensionIcon.preReleaseForeground", {
    dark: "#1d9271",
    light: "#1d9271",
    hcDark: "#1d9271",
    hcLight: textLinkForeground,
}, localize("extensionPreReleaseForeground", "The icon color for pre-release extension."), true);
