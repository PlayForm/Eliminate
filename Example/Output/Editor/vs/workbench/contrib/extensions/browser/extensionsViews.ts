...
import { DelayedPagedModel, IPagedModel, IPager, PagedModel, } from "../../../../base/common/paging.js";
...
import { ExtensionGalleryError, ExtensionGalleryErrorCode, SortBy as GallerySortBy, IQueryOptions as IGalleryQueryOptions, InstallExtensionInfo, SortOrder, } from "../../../../platform/extensionManagement/common/extensionManagement.js";
...
import { Delegate, IExtensionsViewState, Renderer } from "./extensionsList.js";
...
export class ExtensionsListView extends ViewPane {
    ...
    constructor(protected readonly options: ExtensionsListViewOptions, viewletViewOptions: IViewletViewOptions, 
    ...
    private readonly logService: ILogService) {
        super({
            ...
            maximumBodySize: options.flexibleHeight
                ? storageService.getNumber(`${viewletViewOptions.id}.size`, StorageScope.PROFILE, 0)
                    ? undefined
                    : 0
                : undefined,
        ...
        if (e.element) {
            ...
            const extension = e.element
                ? this.extensionsWorkbenchService.local.find((local) => areSameExtensions(local.identifier, e.element!.identifier) &&
                    (!e.element!.server ||
                        e.element!.server === local.server)) || e.element
                : e.element;
    ...
    private async queryLocal(query: Query, options: IQueryOptions): Promise<IQueryResult> {
        ...
        if (canIncludeInstalledExtensions) {
            ...
            disposables.add(Event.debounce(Event.any(Event.filter(this.extensionsWorkbenchService.onChange, (e) => e?.state === ExtensionState.Installed), this.extensionService.onDidChangeExtensions), () => undefined)(async () => {
    ...
    private async filterLocal(local: IExtension[], runningExtensions: readonly IExtensionDescription[], query: Query, options: IQueryOptions): Promise<{
    ...
    private filterInstalledExtensions(local: IExtension[], runningExtensions: readonly IExtensionDescription[], query: Query, options: IQueryOptions): IExtension[] {
        ...
        value = value
            ...
            .trim()
            .toLowerCase();
        const matchingText = (e: IExtension) => (e.name.toLowerCase().indexOf(value) > -1 ||
            e.displayName.toLowerCase().indexOf(value) > -1 ||
        ...
        if (options.sortBy !== undefined) {
            ...
            result = local.filter((e) => (!e.isBuiltin ||
                e.outdated ||
    ...
    private filterWorkspaceUnsupportedExtensions(local: IExtension[], query: Query, options: IQueryOptions): IExtension[] {
        ...
        const hasRestrictedSupportType = (extension: IExtension, supportType: ExtensionUntrustedWorkspaceSupportType) => {
            ...
            if (enablementState !== EnablementState.EnabledGlobally &&
    ...
    private filterExtensionsByFeature(local: IExtension[], query: Query, options: IQueryOptions): IExtension[] {
        ...
        const renderer = feature.renderer
            ? this.instantiationService.createInstance<IExtensionFeatureRenderer>(feature.renderer)
            : undefined;
        try {
            const result = local.filter((e) => {
                ...
                return (renderer?.shouldRender(e.local.manifest) ||
                    this.extensionFeaturesManagementService.getAccessData(new ExtensionIdentifier(e.identifier.id), featureId));
            });
            return this.sortExtensions(result, options);
        }
        finally {
    ...
    private sortExtensions(extensions: IExtension[], options: IQueryOptions): IExtension[] {
        switch (options.sortBy) {
            case GallerySortBy.InstallCount:
                extensions = extensions.sort((e1, e2) => typeof e2.installCount === "number" &&
            ...
            case GallerySortBy.WeightedRating:
                extensions = extensions.sort((e1, e2) => typeof e2.rating === "number" &&
                ...
                try {
                    ...
                    for (const extension of extensions) {
                        if (extension.gallery &&
                            ...
                            (await this.extensionManagementService.canInstall(extension.gallery))) {
    ...
    private openExtension(extension: IExtension, options: {
        sideByside?: boolean;
...
export class DefaultPopularExtensionsView extends ExtensionsListView {
    override async show(): Promise<IPagedModel<IExtension>> {
        const query = this.extensionManagementServerService
...
export class StaticQueryExtensionsView extends ExtensionsListView {
    constructor(protected override readonly options: StaticQueryExtensionsViewOptions, viewletViewOptions: IViewletViewOptions, 
    @INotificationService
...
export class WorkspaceRecommendedExtensionsView extends ExtensionsListView implements IWorkspaceRecommendedExtensionsView {
    ...
    override async show(query: string): Promise<IPagedModel<IExtension>> {
        const shouldShowEmptyView = query &&
    ...
    private async getInstallableWorkspaceRecommendations(): Promise<IExtension[]> {
        const installed = (await this.extensionsWorkbenchService.queryLocal()).filter((l) => l.enablementState !== EnablementState.DisabledByExtensionKind); // Filter extensions disabled by kind
