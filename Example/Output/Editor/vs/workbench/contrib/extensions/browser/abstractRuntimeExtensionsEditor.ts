...
export abstract class AbstractRuntimeExtensionsEditor extends EditorPane {
    ...
    private async _resolveExtensions(): Promise<IRuntimeExtension[]> {
        ...
        const profileTime = (extension: IRuntimeExtension): number => extension.profileInfo?.totalTime ?? 0;
        const activationTime = (extension: IRuntimeExtension): number => (extension.status.activationTimes?.codeLoadingTime ?? 0) +
    ...
    protected createEditor(parent: HTMLElement): void {
        ...
        const renderer: IListRenderer<IRuntimeExtension, IRuntimeExtensionTemplateData> = {
            ...
            renderElement: (element: IRuntimeExtension, index: number, data: IRuntimeExtensionTemplateData): void => {
                ...
                data.elementDisposables.push(addDisposableListener(data.icon, "error", () => (data.icon.src =
                    element.marketplaceInfo?.iconUrlFallback ||
                        DefaultIconPath), { once: true }));
                data.icon.src =
                ...
                data.name.textContent = (element.marketplaceInfo?.displayName ||
                    element.description.identifier.value).substr(0, 50);
                data.version.textContent = element.description.version;
                const activationTimes = element.status.activationTimes;
                if (activationTimes) {
                    const syncTime = activationTimes.codeLoadingTime +
                        activationTimes.activateCallTime;
                    data.activationTime.textContent = activationTimes
                ...
                if (element.status.messages &&
                    element.status.messages.length > 0) {
                ...
                else if (element.description.extensionLocation.scheme ===
                ...
                else if (element.status.runningLocation &&
                    element.status.runningLocation.affinity > 0) {
                    extraLabel =
                        element.status.runningLocation instanceof
        ...
        this._list = <WorkbenchList<IRuntimeExtension>>(this._instantiationService.createInstance(WorkbenchList, "RuntimeExtensions", parent, delegate, [renderer], {
...
export class ShowRuntimeExtensionsAction extends Action2 {
    ...
    async run(accessor: ServicesAccessor): Promise<void> {
        await accessor
            .get(IEditorService)
            .openEditor(RuntimeExtensionsInput.instance, { pinned: true });
    }
}
