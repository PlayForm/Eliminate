...
export abstract class AbstractUserDataProfileElement extends Disposable {
    ...
    constructor(name: string, icon: string | undefined, flags: UseDefaultProfileFlags | undefined, workspaces: readonly URI[] | undefined, isActive: boolean, 
    ...
    protected toUserDataProfileResourceChildElement(child: IProfileResourceChildTreeItem, primaryActions?: IAction[], contextMenuActions?: IAction[]): IProfileResourceTypeChildElement {
        return {
            ...
            description: isString(child.description)
                ? child.description
                : undefined,
    ...
    openWorkspace(workspace: URI): void {
        if (this.uriIdentityService.extUri.extname(workspace) ===
            WORKSPACE_SUFFIX) {
            this.hostService.openWindow([{ workspaceUri: workspace }], {
                forceNewWindow: true,
            });
        }
        else {
            this.hostService.openWindow([{ folderUri: workspace }], {
                forceNewWindow: true,
            });
        }
    }
    ...
    protected async saveProfile(profile: IUserDataProfile): Promise<IUserDataProfile | undefined> {
        ...
        return await this.userDataProfileManagementService.updateProfile(profile, {
            name: this.name,
            icon: this.icon,
            useDefaultFlags: profile.useDefaultFlags && !useDefaultFlags
                ? {}
                : useDefaultFlags,
...
export class UserDataProfileElement extends AbstractUserDataProfileElement {
    ...
    constructor(private _profile: IUserDataProfile, readonly titleButtons: [
        Action[],
        Action[]
    ], readonly actions: [
        IAction[],
    ...
    instantiationService: IInstantiationService) {
        ...
        this._register(this.userDataProfileService.onDidChangeCurrentProfile(() => (this.active =
    ...
    protected override async getChildrenForResourceType(resourceType: ProfileResourceType): Promise<IProfileChildElement[]> {
        if (resourceType === ProfileResourceType.Extensions) {
            const children = await this.instantiationService
                ...
                .getChildren();
            return children.map<IProfileResourceTypeChildElement>((child) => this.toUserDataProfileResourceChildElement(child, undefined, [
                {
...
export class NewProfileElement extends AbstractUserDataProfileElement {
    ...
    constructor(name: string, copyFrom: URI | IUserDataProfile | undefined, readonly titleButtons: [
        Action[],
        Action[]
    ], readonly actions: [
        IAction[],
    ...
    private async getChildrenFromProfileTemplate(profileTemplate: IUserDataProfileTemplate, resourceType: ProfileResourceType): Promise<IProfileResourceTypeChildElement[]> {
        const profile = toUserDataProfile(generateUuid(), this.name, URI.file("/root").with({
...
export class UserDataProfilesEditorModel extends EditorModel {
    ...
    static getInstance(instantiationService: IInstantiationService): UserDataProfilesEditorModel {
        if (!UserDataProfilesEditorModel.INSTANCE) {
            UserDataProfilesEditorModel.INSTANCE =
                instantiationService.createInstance(UserDataProfilesEditorModel);
        }
    ...
    get profiles(): AbstractUserDataProfileElement[] {
        return this._profiles
            ...
            if (a instanceof UserDataProfileElement &&
                a.profile.isDefault) {
                return -1;
            }
            if (b instanceof UserDataProfileElement &&
                b.profile.isDefault) {
                return 1;
            }
    ...
    private onDidChangeProfiles(e: DidChangeProfilesEvent): void {
        ...
        for (const profile of e.removed) {
            ...
            const index = this._profiles.findIndex(([p]) => p instanceof UserDataProfileElement &&
    ...
    private createProfileElement(profile: IUserDataProfile): [
    ...
    async createNewProfile(copyFrom?: URI | IUserDataProfile): Promise<AbstractUserDataProfileElement | undefined> {
        ...
        if (!this.newProfileElement) {
            ...
            if (isWeb && copyFrom instanceof URI && isProfileURL(copyFrom)) {
                primaryActions.push(new Action("userDataProfile.createInDesktop", localize("import in desktop", "Create in {0}", this.productService.nameLong), undefined, true, () => this.openerService.open(copyFrom, {
                    openExternal: true,
                })));
            }
            ...
            const updateCreateActionLabel = () => {
                if (createAction.enabled) {
                    if (this.newProfileElement?.copyFrom &&
                        this.userDataProfilesService.profiles.some((p) => !p.isTransient &&
                            p.name === this.newProfileElement?.name)) {
    ...
    async saveNewProfile(transient?: boolean, token?: CancellationToken): Promise<IUserDataProfile | undefined> {
        ...
        try {
            if (this.newProfileElement.previewProfile) {
                if (!transient) {
                    profile =
                        await this.userDataProfileManagementService.updateProfile(this.newProfileElement.previewProfile, { transient: false });
                }
            }
            else {
                ...
                if (copyFrom instanceof URI) {
                    ...
                    if (template) {
                        ...
                        profile =
                            await this.userDataProfileImportExportService.createProfileFromTemplate(template, {
                                name,
                                useDefaultFlags,
                                icon,
                    ...
                    profile =
                        await this.userDataProfileImportExportService.createFromProfile(copyFrom, {
                            name,
                            useDefaultFlags,
                            icon: icon,
                            resourceTypeFlags: this.newProfileElement.copyFlags,
