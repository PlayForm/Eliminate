...
import { CURRENT_PROFILE_CONTEXT, HAS_PROFILES_CONTEXT, IS_CURRENT_PROFILE_TRANSIENT_CONTEXT, isProfileURL, IUserDataProfileManagementService, IUserDataProfileService, PROFILES_CATEGORY, PROFILES_TITLE, } from "../../../services/userDataProfile/common/userDataProfile.js";
...
export class UserDataProfilesWorkbenchContribution extends Disposable implements IWorkbenchContribution {
    ...
    private registerEditor(): void {
        Registry.as<IEditorPaneRegistry>(EditorExtensions.EditorPane).registerEditorPane(EditorPaneDescriptor.create(UserDataProfilesEditor, UserDataProfilesEditor.ID, localize("userdataprofilesEditor", "Profiles Editor")), [new SyncDescriptor(UserDataProfilesEditorInput)]);
    ...
    private registerProfileEntryAction(profile: IUserDataProfile): IDisposable {
        ...
        return registerAction2(class ProfileEntryAction extends Action2 {
            ...
            async run(accessor: ServicesAccessor) {
                if (that.userDataProfileService.currentProfile.id !==
                    profile.id) {
    ...
    private registerSwitchProfileAction(): IDisposable {
        ...
        return registerAction2(class SwitchProfileAction extends Action2 {
            ...
            async run(accessor: ServicesAccessor) {
                const quickInputService = accessor.get(IQuickInputService);
                const items: Array<IQuickPickItem & {
    ...
    private async reportWorkspaceProfileInfo(): Promise<void> {
        ...
        if (this.userDataProfilesService.profiles.length > 1) {
            this.telemetryService.publicLog2<UserProfilesCountEvent, UserProfilesCountClassification>("profiles:count", {
