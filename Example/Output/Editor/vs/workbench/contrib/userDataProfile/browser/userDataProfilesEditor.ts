...
import { AbstractUserDataProfileElement, IProfileChildElement, IProfileResourceTypeChildElement, IProfileResourceTypeElement, isProfileResourceChildElement, isProfileResourceTypeElement, NewProfileElement, UserDataProfileElement, UserDataProfilesEditorModel, } from "./userDataProfilesEditorModel.js";
...
export class UserDataProfilesEditor extends EditorPane implements IUserDataProfilesEditor {
    ...
    private renderSidebar(parent: HTMLElement): void {
        ...
        this.profilesList = this._register(this.instantiationService.createInstance(WorkbenchList<AbstractUserDataProfileElement>, "ProfilesList", append(parent, $(".profiles-list")), delegate, [renderer], {
    ...
    selectProfile(profile: IUserDataProfile): void {
        const index = this.model?.profiles.findIndex((p) => p instanceof UserDataProfileElement &&
    ...
    private updateProfilesList(elementToSelect?: AbstractUserDataProfileElement): void {
        ...
        if (elementToSelect) {
            ...
            const elementToSelect = this.model.profiles.find((profile) => profile.active) ??
...
class ProfileWidget extends Disposable {
    ...
    private readonly _profileElement = this._register(new MutableDisposable<{
    ...
    private readonly instantiationService: IInstantiationService) {
        ...
        this.profileTree = this._register(this.instantiationService.createInstance(WorkbenchAsyncDataTree<AbstractUserDataProfileElement, ProfileTreeElement>, "ProfileEditor-Tree", this.profileTreeContainer, delegate, [
    ...
    render(profileElement: AbstractUserDataProfileElement): void {
        ...
        if (this._profileElement.value?.element instanceof
    ...
    protected estimateHeight({ element, root }: ProfileTreeElement): number {
        switch (element) {
            ...
            case "contents":
                return 258;
            case "workspaces":
                return ((root.workspaces ? root.workspaces.length * 24 + 30 : 0) +
                    112);
        }
    }
}
...
class CopyFromProfileRenderer extends ProfilePropertyRenderer {
    ...
    renderTemplate(parent: HTMLElement): IProfilePropertyRendererTemplate {
        ...
        const render = (profileElement: NewProfileElement, copyFromOptions: (ISelectOptionItem & {
            id?: string;
        ...
        return {
            set element(element: ProfileTreeElement) {
                ...
                if (profileElement.root instanceof NewProfileElement) {
                    ...
                    copyFromSelectBox.setEnabled(!newProfileElement.previewProfile &&
    ...
    private getCopyFromOptions(profileElement: NewProfileElement): (ISelectOptionItem & {
        id?: string;
        source?: IUserDataProfile | URI;
    })[] {
        const separator = {
...
class ContentsProfileRenderer extends ProfilePropertyRenderer {
    ...
    private readonly _onDidChangeSelection = this._register(new Emitter<{
        element: ProfileTreeElement;
        selected: boolean;
    }>());
    ...
    renderTemplate(parent: HTMLElement): IProfilePropertyRendererTemplate {
        ...
        const profilesContentTree = (this.profilesContentTree = disposables.add(this.instantiationService.createInstance(WorkbenchAsyncDataTree<AbstractUserDataProfileElement, ProfileContentTreeElement>, "ProfileEditor-ContentsTree", append(configureRowContainer, $(".profile-content-tree.file-icon-themable-tree.show-file-icons")), delegate, [
            ...
            accessibilityProvider: {
                getAriaLabel(element: ProfileContentTreeElement | null): string {
                    if ((<IProfileResourceTypeElement>element?.element)
                        .resourceType) {
...
class ProfileWorkspacesRenderer extends ProfilePropertyRenderer {
    ...
    renderTemplate(parent: HTMLElement): IProfilePropertyRendererTemplate {
        ...
        const table = (this.workspacesTable = disposables.add(this.instantiationService.createInstance(WorkbenchTable<WorkspaceTableElement>, "ProfileEditor-AssociationsTable", workspacesTableContainer, new (class implements ITableVirtualDelegate<URI> {
        ...
        })(), [
            {
                ...
                project(row: WorkspaceTableElement): WorkspaceTableElement {
                    return row;
                },
            },
            {
        ...
        const updateTable = () => {
            if (profileElement?.root instanceof UserDataProfileElement &&
                ...
                table.splice(0, table.length, profileElement.root.workspaces
                    .map((workspace) => ({
                    workspace,
                    profileElement: <UserDataProfileElement>(profileElement!.root),
                }))
    ...
    renderElement({ element: profileResourceTreeElement, }: ITreeNode<ProfileContentTreeElement, void>, index: number, templateData: INewProfileResourceTemplateData, height: number | undefined): void {
        ...
        const renderRadioItems = () => {
            ...
            if (root.copyFrom && name) {
                ...
                templateData.radio.setActiveItem(root.getCopyFlag(element.resourceType)
                    ? 0
...
class ChangeProfileAction extends Action {
    constructor(private readonly item: WorkspaceTableElement, 
    @IUserDataProfilesService
    private readonly userDataProfilesService: IUserDataProfilesService) {
...
class WorkspaceUriActionsColumnRenderer implements ITableRenderer<WorkspaceTableElement, IActionsColumnTemplateData> {
    ...
    private createDeleteAction(item: WorkspaceTableElement): IAction {
        return {
            ...
            enabled: this.userDataProfileManagementService.getDefaultProfileToUse()
                .id !== item.profileElement.profile.id,
            id: "deleteTrustedUri",
