...
import { isURLDomainTrusted, ITrustedDomainService, } from "./trustedDomainService.js";
export class OpenerValidatorContributions implements IWorkbenchContribution {
    ...
    async validateLink(resource: URI | string, openOptions?: OpenOptions): Promise<boolean> {
        if (!matchesScheme(resource, Schemas.http) &&
        ...
        if (await this._trustedDomainService.isValid(resourceUri)) {
            ...
            const { result } = await this._dialogService.prompt<boolean>({
                ...
                detail: typeof originalResource === "string"
                    ? originalResource
                    : formattedLink,
                buttons: [
                    {
