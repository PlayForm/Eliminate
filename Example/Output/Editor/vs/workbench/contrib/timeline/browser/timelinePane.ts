...
import { ITimelineService, Timeline, TimelineChangeEvent, TimelineItem, TimelineOptions, TimelineProvidersChangeEvent, TimelineRequest, } from "../common/timeline.js";
const ItemHeight = 22;
...
class TimelineAggregate {
    ...
    add(timeline: Timeline, options: TimelineOptions) {
        ...
        if (timeline.items.length !== 0 && this.items.length !== 0) {
            ...
            if ((timeline.items[timeline.items.length - 1]?.timestamp ?? 0) >=
...
export class TimelinePane extends ViewPane {
    ...
    get pageOnScroll() {
        if (this._pageOnScroll === undefined) {
            this._pageOnScroll =
                this.configurationService.getValue<boolean | null | undefined>("timeline.pageOnScroll") ?? false;
        }
    ...
    get pageSize() {
        ...
        if (pageSize === undefined || pageSize === null) {
            ...
            pageSize = Math.max(20, Math.floor((this.tree?.renderHeight ?? 0 / ItemHeight) +
    ...
    private onActiveEditorChanged() {
        ...
        if ((this.uriIdentityService.extUri.isEqual(uri, this.uri) &&
            uri !== undefined) ||
    ...
    protected override renderBody(container: HTMLElement): void {
        ...
        this.tree = <WorkbenchObjectTree<TreeElement, FuzzyScore>>(this.instantiationService.createInstance(WorkbenchObjectTree, "TimelinePane", this.$tree, new TimelineListVirtualDelegate(), [this.treeRenderer], {
    ...
    ensureValidItems() {
        ...
        if (!this.hasVisibleItems ||
            !this.timelineService
                .getSources()
                .some(({ id }) => !this.excludedSources.has(id) &&
                this.timelinesBySource.has(id))) {
            this.tree.setChildren(null, undefined);
            this._isEmpty = true;
            this.setLoadingUriMessage();
...
class TimelineElementTemplate implements IDisposable {
    ...
    constructor(container: HTMLElement, actionViewItemProvider: IActionViewItemProvider, hoverDelegate: IHoverDelegate) {
...
class TimelineTreeRenderer implements ITreeRenderer<TreeElement, FuzzyScore, TimelineElementTemplate> {
    ...
    constructor(private readonly commands: TimelinePaneCommands, 
    @IInstantiationService
...
class TimelinePaneCommands extends Disposable {
    ...
    constructor(private readonly pane: TimelinePane, 
    @ITimelineService
    ...
    private readonly menuService: IMenuService) {
        ...
        this._register(CommandsRegistry.registerCommand("timeline.toggleFollowActiveEditor", (accessor: ServicesAccessor, ...args: any[]) => (pane.followActiveEditor = !pane.followActiveEditor)));
    ...
    private getActions(menuId: MenuId, context: {
        key: string;
        value?: string;
    }): {
        primary: IAction[];
