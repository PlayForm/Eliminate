...
import { HasSpeechProvider, ISpeechProvider, ISpeechService, ISpeechToTextSession, ITextToSpeechSession, KeywordRecognitionStatus, SPEECH_LANGUAGE_CONFIG, speechLanguageConfigToLanguage, SpeechToTextInProgress, SpeechToTextStatus, TextToSpeechInProgress, TextToSpeechStatus, } from "../common/speechService.js";
    ...
    async recognizeKeyword(token: CancellationToken): Promise<KeywordRecognitionStatus> {
        ...
        const recognizeKeyword = () => {
            ...
            const currentRecognizeKeywordSession = (activeRecognizeKeywordSession = this.doRecognizeKeyword(cts.token).then((status) => {
                if (currentRecognizeKeywordSession ===
