...
export class InputCodeEditorView extends CodeEditorView {
    ...
    constructor(public readonly inputNumber: 1 | 2, viewModel: IObservable<MergeEditorViewModel | undefined>, 
    ...
    configurationService: IConfigurationService) {
        ...
        this._register(instantiationService.createInstance(TitleMenu, inputNumber === 1
            ? MenuId.MergeInput1Toolbar
            : MenuId.MergeInput2Toolbar, this.htmlElements.toolbar));
        this._register(autorunOpts({
            ...
            this.editor.setModel(this.inputNumber === 1
            ...
            reset(this.htmlElements.description, ...(description
...
export class ModifiedBaseRangeGutterItemModel implements IGutterItemInfo {
    ...
    constructor(public readonly id: string, private readonly baseRange: ModifiedBaseRange, private readonly inputNumber: 1 | 2, private readonly viewModel: MergeEditorViewModel) { }
    ...
    public readonly state: IObservable<{
        handled: boolean;
        focused: boolean;
    }> = derived(this, (reader) => {
    ...
    public getContextMenuActions(): readonly IAction[] {
        ...
        return [
            ...
            this.baseRange.isConflicting
                ? setFields(action("mergeEditor.acceptBoth", localize("mergeEditor.acceptBoth", "Accept Both"), state
                    .withInputValue(1, !both)
                    .withInputValue(2, !both), both), { enabled: this.baseRange.canBeCombined })
                : undefined,
            new Separator(),
            this.baseRange.isConflicting
                ? setFields(action("mergeEditor.swap", localize("mergeEditor.swap", "Swap"), state.swap(), false), {
                    enabled: !state.kind &&
...
export class MergeConflictGutterItemView extends Disposable implements IGutterItemView<ModifiedBaseRangeGutterItemModel> {
    ...
    constructor(item: ModifiedBaseRangeGutterItemModel, target: HTMLElement, contextMenuService: IContextMenuService) {
        ...
        this._register(autorun((reader) => {
            ...
            const iconMap: Record<InputState, {
