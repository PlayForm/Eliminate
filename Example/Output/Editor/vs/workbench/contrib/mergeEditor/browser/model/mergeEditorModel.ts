...
export class MergeEditorModel extends EditorModel {
    ...
    private readonly modifiedBaseRangeResultStates = derived(this, (reader) => {
        const map = new Map<ModifiedBaseRange, ModifiedBaseRangeData>(this.modifiedBaseRanges
            .read(reader)
            .map<[
            ModifiedBaseRange,
            ModifiedBaseRangeData
        ]>((s) => [s, new ModifiedBaseRangeData(s)]));
        return map;
    });
    private readonly resultSnapshot = this.resultTextModel.createSnapshot();
    constructor(readonly base: ITextModel, readonly input1: InputData, readonly input2: InputData, readonly resultTextModel: ITextModel, private readonly diffComputer: IMergeDiffComputer, private readonly options: {
        resetResult: boolean;
    ...
    public readonly isUpToDate = derived(this, (reader) => this.diffComputingState.read(reader) ===
    ...
    public setState(baseRange: ModifiedBaseRange, state: ModifiedBaseRangeState, _markInputAsHandled: boolean | InputNumber, tx: ITransaction, _pushStackElement: boolean = false): void {
    ...
    public resetDirtyConflictsToBase(): void {
        transaction((tx) => {
            ...
            for (const range of this.modifiedBaseRanges.get()) {
                if (this.getState(range).get().kind ===
                    ModifiedBaseRangeStateKind.unrecognized) {
...
class MarkAsHandledUndoRedoElement implements IResourceUndoRedoElement {
    ...
    constructor(public readonly resource: URI, private readonly mergeEditorModelRef: WeakRef<MergeEditorModel>, private readonly stateRef: WeakRef<ModifiedBaseRangeData>, private readonly input1Handled: boolean, private readonly input2Handled: boolean) { }
    public redo() {
