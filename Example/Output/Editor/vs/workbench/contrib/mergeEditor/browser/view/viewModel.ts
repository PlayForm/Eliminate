...
export class MergeEditorViewModel extends Disposable {
    ...
    private readonly notificationService: INotificationService) {
        ...
        this._register(resultCodeEditorView.editor.onDidChangeModelContent((e) => {
            ...
            for (const change of e.changes) {
                ...
                const baseRanges = this.model.findModifiedBaseRangesInRange(new LineRange(rangeInBase.startLineNumber, rangeInBase.endLineNumber -
    ...
    public readonly shouldUseAppendInsteadOfAccept = observableConfigValue<boolean>("mergeEditor.shouldUseAppendInsteadOfAccept", false, this.configurationService);
    private counter = 0;
    ...
    public setState(baseRange: ModifiedBaseRange, state: ModifiedBaseRangeState, tx: ITransaction, inputNumber: InputNumber): void {
    ...
    public toggleActiveConflict(inputNumber: 1 | 2): void {
        ...
        transaction((tx) => {
            ...
            this.setState(activeModifiedBaseRange, this.model
                .getState(activeModifiedBaseRange)
                .get()
                .toggle(inputNumber), tx, inputNumber);
        });
    }
    public acceptAll(inputNumber: 1 | 2): void {
        transaction((tx) => {
            ...
            for (const range of this.model.modifiedBaseRanges.get()) {
                this.setState(range, this.model
                    .getState(range)
                    .get()
                    .withInputValue(inputNumber, true), tx, inputNumber);
            }
        });
    }
}
        ...
        this._register(model.onDidChangeContent((e) => {
            ...
            if (e.isRedoing) {
                for (const item of this.attachedHistory) {
                    if (this.previousAltId < item.altId &&
