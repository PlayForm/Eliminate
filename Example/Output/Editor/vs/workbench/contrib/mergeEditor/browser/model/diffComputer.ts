...
export class MergeDiffComputer implements IMergeDiffComputer {
    private readonly mergeAlgorithm = observableConfigValue<"smart" | "experimental" | "legacy" | "advanced">("mergeEditor.diffAlgorithm", "advanced", this.configurationService).map((v) => v === "smart" ? "legacy" : v === "experimental" ? "advanced" : v);
    constructor(
    ...
    async computeDiff(textModel1: ITextModel, textModel2: ITextModel, reader: IReader): Promise<IMergeDiffComputerResult> {
        ...
        if (inputVersion !== newInputVersion ||
        ...
        assertFn(() => {
            for (const c of changes) {
                ...
                for (const map of c.rangeMappings) {
                    let inputRangesValid = inputRange.startLineNumber - 1 <=
                    ...
                    let outputRangesValid = outputRange.startLineNumber - 1 <=
            ...
            return (changes.length === 0 ||
                (changes[0].inputRange.startLineNumber ===
                    ...
                    checkAdjacentItems(changes, (m1, m2) => m2.inputRange.startLineNumber -
