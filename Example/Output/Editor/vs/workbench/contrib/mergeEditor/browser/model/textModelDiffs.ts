...
export class TextModelDiffs extends Disposable {
    ...
    private _recompute(reader: IReader): void {
        ...
        transaction((tx) => {
            ...
            this._state.set(this._isInitializing
                ? TextModelDiffState.initializing
                : TextModelDiffState.updating, tx, TextModelDiffChangeReason.other);
        });
    ...
    public removeDiffs(diffToRemoves: DetailedLineRangeMapping[], transaction: ITransaction | undefined, group?: UndoRedoGroup): void {
        ...
        for (const diffToRemove of diffToRemoves) {
            ...
            diffs = diffs.map((d) => d.outputRange.isAfter(diffToRemove.outputRange)
                ? d.addOutputLineDelta(diffToRemove.inputRange.lineCount -
