...
export class ViewZoneComputer {
    ...
    constructor(private readonly input1Editor: ICodeEditor, private readonly input2Editor: ICodeEditor, private readonly resultEditor: ICodeEditor) { }
    public computeViewZones(reader: IReader, viewModel: MergeEditorViewModel, options: {
        ...
        for (const m of baseRangeWithStoreAndTouchingDiffs) {
            ...
            for (const { input1Line, baseLine, input2Line, resultLine, } of alignedLines) {
                ...
                const input1Line_ = input1Line !== undefined
                    ? input1Line + input1LinesAdded
                    : -1;
                const input2Line_ = input2Line !== undefined
                    ? input2Line + input2LinesAdded
                    : -1;
                const baseLine_ = baseLine + baseLinesAdded;
                const resultLine_ = resultLine !== undefined
...
export class MergeEditorViewZones {
    constructor(public readonly input1ViewZones: readonly MergeEditorViewZone[], public readonly input2ViewZones: readonly MergeEditorViewZone[], public readonly baseViewZones: readonly MergeEditorViewZone[], public readonly resultViewZones: readonly MergeEditorViewZone[]) { }
}
/**
...
class Spacer extends MergeEditorViewZone {
    constructor(private readonly afterLineNumber: number, private readonly heightInLines: number) {
        super();
    }
...
class Placeholder extends MergeEditorViewZone {
    constructor(private readonly afterLineNumber: number, private readonly heightPx: number) {
        super();
    }
    override create(viewZoneChangeAccessor: IViewZoneChangeAccessor, viewZoneIdsToCleanUp: string[], disposableStore: DisposableStore): void {
        viewZoneIdsToCleanUp.push(viewZoneChangeAccessor.addZone({
...
class CommandViewZone extends MergeEditorViewZone {
    constructor(private readonly conflictActionsFactory: ConflictActionsFactory, private readonly lineNumber: number, private readonly items: IObservable<IContentWidgetAction[]>) {
        super();
    }
    override create(viewZoneChangeAccessor: IViewZoneChangeAccessor, viewZoneIdsToCleanUp: string[], disposableStore: DisposableStore): void {
