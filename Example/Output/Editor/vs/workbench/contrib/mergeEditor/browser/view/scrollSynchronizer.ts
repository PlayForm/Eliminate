...
export class ScrollSynchronizer extends Disposable {
    ...
    private get shouldAlignBase() {
        return (this.layout.get().kind === "mixed" &&
    ...
    constructor(private readonly viewModel: IObservable<MergeEditorViewModel | undefined>, private readonly input1View: InputCodeEditorView, private readonly input2View: InputCodeEditorView, private readonly baseView: IObservable<BaseCodeEditorView | undefined>, private readonly inputResultView: ResultCodeEditorView, private readonly layout: IObservable<IMergeEditorLayout>) {
        ...
        const handleInput1OnScroll = (this.updateScrolling = () => {
            ...
            if (baseView) {
                if (this.shouldAlignBase) {
                    this.baseView
                        .get()
                        ?.editor.setScrollTop(this.input1View.editor.getScrollTop(), ScrollType.Immediate);
                }
                else {
        ...
        this._store.add(this.input1View.editor.onDidScrollChange(this.reentrancyBarrier.makeExclusiveOrSkip((c) => {
            ...
            if (c.scrollLeftChanged) {
                this.baseView
                    .get()
                    ?.editor.setScrollLeft(c.scrollLeft, ScrollType.Immediate);
            ...
            if (c.scrollTopChanged) {
                ...
                if (baseView && this.model) {
                    if (this.shouldAlignBase) {
                        this.baseView
                            .get()
                            ?.editor.setScrollTop(c.scrollTop, ScrollType.Immediate);
                    }
                    else {
    ...
    private synchronizeScrolling(scrollingEditor: CodeEditorWidget, targetEditor: CodeEditorWidget, mapping: DocumentLineRangeMap | undefined) {
        ...
        const factor = Math.min((scrollingEditor.getScrollTop() - sourceStartTopPx) /
            (sourceEndPx - sourceStartTopPx), 1);
