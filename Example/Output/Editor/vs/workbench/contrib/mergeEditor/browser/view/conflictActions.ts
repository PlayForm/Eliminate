...
export class ConflictActionsFactory extends Disposable {
    ...
    private _getLayoutInfo() {
        const lineHeightFactor = Math.max(1.3, this._editor.getOption(EditorOption.lineHeight) /
...
export class ActionsSource {
    constructor(private readonly viewModel: MergeEditorViewModel, private readonly modifiedBaseRange: ModifiedBaseRange) { }
...
class ActionsContentWidget extends FixedZoneWidget {
    ...
    constructor(editor: ICodeEditor, viewZoneAccessor: IViewZoneChangeAccessor, afterLineNumber: number, height: number, className: string, items: IObservable<IContentWidgetAction[]>, viewZoneIdsToCleanUp: string[]) {
