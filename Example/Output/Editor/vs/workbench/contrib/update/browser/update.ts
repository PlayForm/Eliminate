...
import { DisablementReason, IUpdate, IUpdateService, StateType, State as UpdateState, } from "../../../../platform/update/common/update.js";
    ...
    private async onUpdateStateChange(state: UpdateState): Promise<void> {
        ...
        switch (state.type) {
            ...
            case StateType.Idle:
                ...
                else if (this.state.type === StateType.CheckingForUpdates &&
                    this.state.explicit &&
            ...
            case StateType.Ready: {
                ...
                if (productVersion) {
                    ...
                    this.majorMinorUpdateAvailableContextKey.set(Boolean(currentVersion &&
                        nextVersion &&
...
export class SwitchProductQualityContribution extends Disposable implements IWorkbenchContribution {
    ...
    private registerGlobalActivityActions(): void {
        ...
        if (productQualityChangeHandler &&
                    ...
                    try {
                        ...
                        if (userDataSyncStore &&
                            ...
                            userDataSyncStoreType =
                                await this.selectSettingsSyncService(dialogService);
                        ...
                        if (res.confirmed) {
                            ...
                            if (userDataSyncService.status ===
                                SyncStatus.Syncing) {
                                promises.push(Event.toPromise(Event.filter(userDataSyncService.onDidChangeStatus, (status) => status !==
                                    SyncStatus.Syncing)));
                            }
