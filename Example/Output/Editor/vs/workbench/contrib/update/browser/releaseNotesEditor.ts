...
export class ReleaseNotesManager {
    ...
    public async show(version: string, useCurrentFile: boolean): Promise<boolean> {
        ...
        if (this._currentReleaseNotes) {
            ...
            this._currentReleaseNotes.webview.setHtml(html);
            this._webviewWorkbenchService.revealWebview(this._currentReleaseNotes, activeEditorPane
                ? activeEditorPane.group
                : this._editorGroupService.activeGroup, false);
        }
        else {
            ...
            disposables.add(this._currentReleaseNotes.webview.onMessage((e) => {
                if (e.message.type === "showReleaseNotes") {
                    ...
                    const x = this._currentReleaseNotes?.webview.container
                        .offsetLeft + e.message.value.x;
                    const y = this._currentReleaseNotes?.webview.container
    ...
    private async renderBody(fileContent: {
        text: string;
        base: URI;
    }) {
