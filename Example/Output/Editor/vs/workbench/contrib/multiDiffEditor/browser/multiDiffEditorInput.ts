...
import { autorun, constObservable, derived, mapObservableArrayCached, observableFromEvent, observableFromValueWithChangeEvent, ObservableLazyPromise, observableValue, recomputeInitiallyAndOnChange, ValueWithChangeEventFromObservable, } from "../../../../base/common/observable.js";
...
export class MultiDiffEditorInput extends EditorInput implements ILanguageSupport {
    public static fromResourceMultiDiffEditorInput(input: IResourceMultiDiffEditorInput, instantiationService: IInstantiationService): MultiDiffEditorInput {
        ...
        return instantiationService.createInstance(MultiDiffEditorInput, multiDiffSource, input.label, input.resources?.map((resource) => {
    ...
    public static fromSerialized(data: ISerializedMultiDiffEditorInput, instantiationService: IInstantiationService): MultiDiffEditorInput {
        return instantiationService.createInstance(MultiDiffEditorInput, URI.parse(data.multiDiffSourceUri), data.label, data.resources?.map((resource) => new MultiDiffEditorItem(resource.originalUri
            ? URI.parse(resource.originalUri)
            : undefined, resource.modifiedUri
            ? URI.parse(resource.modifiedUri)
            : undefined, resource.goToFileUri
    ...
    constructor(public readonly multiDiffSource: URI, public readonly label: string | undefined, public readonly initialResources: readonly MultiDiffEditorItem[] | undefined, public readonly isTransient: boolean = false, 
    @ITextModelService
        ...
        const documentsWithPromises = mapObservableArrayCached(this, source.resources, async (r, store) => {
            ...
            try {
                [original, modified] = await Promise.all([
                    r.originalUri
                        ? this._textModelService.createModelReference(r.originalUri)
                        : undefined,
                    r.modifiedUri
                        ? this._textModelService.createModelReference(r.modifiedUri)
                        : undefined,
                ]);
                if (original) {
    ...
    override matches(otherInput: EditorInput | IUntypedEditorInput): boolean {
        ...
        if (otherInput instanceof MultiDiffEditorInput) {
            return (this.multiDiffSource.toString() ===
                otherInput.multiDiffSource.toString());
        }
        return false;
    }
    public readonly resources = derived(this, (reader) => this._resolvedSource.cachedPromiseResult
        .read(reader)
        ?.data?.resources.read(reader));
    private readonly textFileServiceOnDidChange = new FastEventDispatcher<ITextFileEditorModel, URI>(this._textFileService.files.onDidChangeDirty, (item) => item.resource.toString(), (uri) => uri.toString());
    private readonly _isDirtyObservables = mapObservableArrayCached(this, this.resources.map((r) => r ?? []), (res) => {
        ...
        /** @description modifiedDirty||originalDirty */ isModifiedDirty.read(reader) || isOriginalDirty.read(reader));
    }, (i) => i.getKey());
    private readonly _isDirtyObservable = derived(this, (reader) => this._isDirtyObservables
        .read(reader)
        .some((isDirty) => isDirty.read(reader))).keepObserved(this._store);
...
class FastEventDispatcher<T, TKey> {
    ...
    constructor(private readonly _event: Event<T>, private readonly _getEventArgsKey: (item: T) => string, private readonly _keyToString: (key: TKey) => string) { }
