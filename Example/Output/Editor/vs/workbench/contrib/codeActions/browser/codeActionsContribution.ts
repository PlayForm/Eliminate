...
export class CodeActionsContribution extends Disposable implements IWorkbenchContribution {
    ...
    constructor(codeActionsExtensionPoint: IExtensionPoint<CodeActionsExtensionPoint[]>, 
    @IKeybindingService
        ...
        providers.forEach((provider) => {
            if (provider.providedCodeActionKinds) {
                provider.providedCodeActionKinds.forEach((kind) => {
                    if (!this.settings.has(kind) &&
                        CodeActionKind.Source.contains(new HierarchicalKind(kind))) {
    ...
    private updateConfigurationSchema(codeActionContributions: readonly CodeActionsExtensionPoint[]) {
        ...
        for (const [sourceAction, props] of this.getSourceActions(codeActionContributions)) {
            ...
            newNotebookProperties[sourceAction] =
                createNotebookCodeActionsAutoSave(nls.localize("codeActionsOnSave.generic", "Controls whether '{0}' actions should be run on file save.", props.title));
        }
        codeActionsOnSaveSchema.properties = newProperties;
        notebookCodeActionsOnSaveSchema.properties = newNotebookProperties;
        Registry.as<IConfigurationRegistry>(Extensions.Configuration).notifyConfigurationSchemaUpdated(editorConfiguration);
    }
        ...
        for (const codeActionKind of this.settings) {
            if (!properties[codeActionKind]) {
                ...
                notebookProperties[codeActionKind] =
                    createNotebookCodeActionsAutoSave(nls.localize("codeActionsOnSave.generic", "Controls whether '{0}' actions should be run on file save.", codeActionKind));
            }
        }
                                    ...
                                    anyOf: [
                                        {
                                            ...
                                            enumDescriptions: actions.map((action) => action.description ??
                                                action.title),
                                        },
                                        { type: "string" },
                                    ],
