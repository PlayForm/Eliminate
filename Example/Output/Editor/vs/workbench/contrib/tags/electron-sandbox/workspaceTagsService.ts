...
import { IFileService, IFileStat, IFileStatResult, } from "../../../../platform/files/common/files.js";
...
import { GradleDependencyCompactRegex, GradleDependencyLooseRegex, JavaLibrariesToLookFor, MavenArtifactIdRegex, MavenDependenciesRegex, MavenDependencyRegex, MavenGroupIdRegex, } from "../common/javaWorkspaceTags.js";
    ...
    getHashedRemotesFromUri(workspaceUri: URI, stripEndingDotGit: boolean = false): Promise<string[]> {
        ...
        return this.fileService.exists(uri).then((exists) => {
            ...
            return this.textFileService
                ...
                .then((content) => getHashedRemotesFromConfig(content.value, stripEndingDotGit), (err) => []);
        });
    }
    /* __GDPR__FRAGMENT__
        "WorkspaceTags" : {
    ...
    */
    private async resolveWorkspaceTags(): Promise<Tags> {
        ...
        return this.fileService
            ...
            tags["workspace.ASP5"] =
                nameSet.has("project.json") &&
                    this.searchArray(names, /^.+\.cs$/i);
            tags["workspace.sln"] = this.searchArray(names, /^.+\.sln$|^.+\.csproj$/i);
            tags["workspace.unity"] =
                nameSet.has("assets") &&
                    nameSet.has("library") &&
                    nameSet.has("projectsettings");
            tags["workspace.npm"] =
                nameSet.has("package.json") || nameSet.has("node_modules");
            tags["workspace.bower"] =
                nameSet.has("bower.json") ||
                    nameSet.has("bower_components");
            ...
            const mainActivity = nameSet.has("mainactivity.cs") ||
                nameSet.has("mainactivity.fs");
            const appDelegate = nameSet.has("appdelegate.cs") ||
                nameSet.has("appdelegate.fs");
            ...
            const pomPromises = getFilePromises("pom.xml", this.fileService, this.textFileService, (content) => {
                try {
                    ...
                    while ((dependenciesContent =
                        MavenDependenciesRegex.exec(content.value))) {
                        let dependencyContent;
                        while ((dependencyContent =
                            MavenDependencyRegex.exec(dependenciesContent[1]))) {
