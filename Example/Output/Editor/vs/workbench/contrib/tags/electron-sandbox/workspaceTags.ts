...
import { getHashedRemotesFromConfig as baseGetHashedRemotesFromConfig, IWorkspaceTagsService, Tags, } from "../common/workspaceTags.js";
        ...
        Promise.all<string[]>(workspaceUris.map((workspaceUri) => {
            ...
            return this.fileService.exists(uri).then((exists) => {
                ...
                return this.textFileService
                    ...
                    .then((content) => getDomainsOfRemotes(content.value, AllowedSecondLevelDomains), (err) => []);
            });
        })).then((domains) => {
    ...
    private reportAzureNode(workspaceUris: URI[], tags: Tags): Promise<Tags> {
        ...
        return this.fileService
            ...
            const names = (<IFileStat[]>[])
                .concat(...results.map((result) => result.success
    ...
    private reportAzureJava(workspaceUris: URI[], tags: Tags): Promise<Tags> {
        return Promise.all(workspaceUris.map((workspaceUri) => {
            ...
            return this.fileService.exists(uri).then((exists) => {
                ...
                return this.textFileService
                    .read(uri, { acceptTextOnly: true })
                    .then((content) => !!content.value.match(/azure/i), (err) => false);
            });
        })).then((javas) => {
