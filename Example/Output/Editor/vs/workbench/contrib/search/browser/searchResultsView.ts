...
import { ISearchModel, ISearchTreeFileMatch, ISearchTreeFolderMatch, ISearchTreeMatch, isPlainTextSearchHeading, isSearchTreeFileMatch, isSearchTreeFolderMatch, isSearchTreeFolderMatchNoRoot, isSearchTreeFolderMatchWorkspaceRoot, isSearchTreeMatch, isTextSearchHeading, ITextSearchHeading, RenderableMatch, } from "./searchTreeModel/searchTreeCommon.js";
...
export class TextSearchResultRenderer extends Disposable implements ICompressibleTreeRenderer<ITextSearchHeading, any, ITextSearchResultTemplate> {
    ...
    async renderElement(node: ITreeNode<ITextSearchHeading, any>, index: number, templateData: IFolderMatchTemplate, height: number | undefined): Promise<void> {
        if (isPlainTextSearchHeading(node.element)) {
            ...
            const aiName = await node.element
                .parent()
                .searchModel.getAITextResultProviderName();
...
export class FolderMatchRenderer extends Disposable implements ICompressibleTreeRenderer<ISearchTreeFolderMatch, any, IFolderMatchTemplate> {
    ...
    constructor(private searchView: SearchView, private labels: ResourceLabels, 
    @IWorkspaceContextService
    protected contextService: IWorkspaceContextService, 
    @ILabelService
    ...
    renderTemplate(container: HTMLElement): IFolderMatchTemplate {
        ...
        const instantiationService = this._register(this.instantiationService.createChild(new ServiceCollection([
            IContextKeyService,
