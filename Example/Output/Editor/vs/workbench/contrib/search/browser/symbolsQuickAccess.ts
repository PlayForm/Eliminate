...
export class SymbolsQuickAccessProvider extends PickerQuickAccessProvider<ISymbolQuickPickItem> {
    ...
    async getSymbolPicks(filter: string, options: {
        skipLocal?: boolean;
        skipSorting?: boolean;
        delay?: number;
    ...
    private async doGetSymbolPicks(query: IPreparedQuery, options: {
        skipLocal?: boolean;
        skipSorting?: boolean;
    } | undefined, token: CancellationToken): Promise<Array<ISymbolQuickPickItem>> {
        ...
        for (const { symbol, provider } of workspaceSymbols) {
            ...
            if (options?.skipLocal &&
                !SymbolsQuickAccessProvider.TREAT_AS_GLOBAL_SYMBOL_TYPES.has(symbol.kind) &&
            ...
            if (!skipContainerQuery &&
                containerQuery &&
                containerQuery.original.length > 0) {
                if (containerLabel) {
    ...
    private async openSymbol(provider: IWorkspaceSymbolProvider, symbol: IWorkspaceSymbol, token: CancellationToken, options: {
        ...
        if (typeof provider.resolveWorkspaceSymbol === "function") {
            symbolToOpen =
                (await provider.resolveWorkspaceSymbol(symbol, token)) ||
                    symbol;
            if (token.isCancellationRequested) {
                return;
            }
