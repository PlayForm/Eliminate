...
import { ISearchResult, isSearchResult, isSearchTreeFileMatch, isSearchTreeFolderMatch, isSearchTreeFolderMatchNoRoot, isSearchTreeFolderMatchWorkspaceRoot, isSearchTreeMatch, isTextSearchHeading, RenderableMatch, } from "./searchTreeModel/searchTreeCommon.js";
//#region Actions
            ...
            menu: [
                {
                    ...
                    group: "navigation",
                    order: 4,
                    when: ContextKeyExpr.and(ContextKeyExpr.equals("view", VIEW_ID), ContextKeyExpr.or(Constants.SearchContext.HasSearchResults.negate(), Constants.SearchContext
                        .ViewHasSomeCollapsibleKey)),
                },
            ],
        });
    }
 ...
 */
export async function forcedExpandRecursively(viewer: WorkbenchCompressibleAsyncDataTree<ISearchResult, RenderableMatch, void>, element: RenderableMatch | undefined) {
    if (element) {
...
function collapseDeepestExpandedLevel(accessor: ServicesAccessor) {
    ...
    if (searchView) {
        ...
        if (isSearchTreeFolderMatchWorkspaceRoot(node) ||
            ...
            while ((node = navigator.next())) {
                ...
                if (searchView.isTreeLayoutViewVisible &&
                    !canCollapseFirstLevel) {
                    let nodeToTest = node;
                    if (isSearchTreeFolderMatch(node)) {
                        const compressionStartNode = viewer.getCompressedTreeNode(node)?.elements[0]
                            .element;
