...
import { FileMatchOrMatch, IChangeEvent, ISearchModel, ISearchResult, ISearchTreeFileMatch, ISearchTreeFolderMatch, ISearchTreeMatch, isSearchResult, isSearchTreeFileMatch, isSearchTreeFolderMatch, isSearchTreeFolderMatchNoRoot, isSearchTreeFolderMatchWithResource, isSearchTreeFolderMatchWorkspaceRoot, isSearchTreeMatch, isTextSearchHeading, ITextSearchHeading, RenderableMatch, SearchModelLocation, } from "./searchTreeModel/searchTreeCommon.js";
...
export class SearchView extends ViewPane {
    ...
    @IReplaceService
    private readonly replaceService: IReplaceService, 
    @ITextFileService
    ...
    private readonly accessibilitySignalService: IAccessibilitySignalService) {
        ...
        // globals
        this.viewletVisible =
            Constants.SearchContext.SearchViewVisibleKey.bindTo(this.contextKeyService);
        this.firstMatchFocused =
            Constants.SearchContext.FirstMatchFocusKey.bindTo(this.contextKeyService);
        this.fileMatchOrMatchFocused =
            Constants.SearchContext.FileMatchOrMatchFocusKey.bindTo(this.contextKeyService);
        ...
        this.folderMatchWithResourceFocused =
            Constants.SearchContext.ResourceFolderFocusKey.bindTo(this.contextKeyService);
        this.hasSearchResultsKey =
            Constants.SearchContext.HasSearchResults.bindTo(this.contextKeyService);
        ...
        this.hasSearchPatternKey =
            Constants.SearchContext.ViewHasSearchPatternKey.bindTo(this.contextKeyService);
        this.hasReplacePatternKey =
            Constants.SearchContext.ViewHasReplacePatternKey.bindTo(this.contextKeyService);
        this.hasFilePatternKey =
            Constants.SearchContext.ViewHasFilePatternKey.bindTo(this.contextKeyService);
        this.hasSomeCollapsibleResultKey =
            Constants.SearchContext.ViewHasSomeCollapsibleKey.bindTo(this.contextKeyService);
        ...
        this.inputBoxFocused =
            Constants.SearchContext.InputBoxFocusedKey.bindTo(this.contextKeyService);
        this.inputPatternIncludesFocused =
            Constants.SearchContext.PatternIncludesFocusedKey.bindTo(this.contextKeyService);
        this.inputPatternExclusionsFocused =
            Constants.SearchContext.PatternExcludesFocusedKey.bindTo(this.contextKeyService);
        this.isEditableItem = Constants.SearchContext.IsEditableItemKey.bindTo(this.contextKeyService);
        this.instantiationService = this._register(this.instantiationService.createChild(new ServiceCollection([
            IContextKeyService,
        ...
        this.treeAccessibilityProvider =
            this.instantiationService.createInstance(SearchAccessibilityProvider, this);
    ...
    private refreshInputs() {
        ...
        this.inputPatternExcludes.setUseExcludesAndIgnoreFiles(!this.viewModel.searchResult.query
    ...
    protected override renderBody(parent: HTMLElement): void {
        ...
        const useExcludesAndIgnoreFiles = typeof this.viewletState["query.useExcludesAndIgnoreFiles"] ===
            "boolean"
    ...
    private createSearchWidget(container: HTMLElement): void {
        ...
        const updateHasPatternKey = () => this.hasSearchPatternKey.set(this.searchWidget.searchInput
    ...
    private async refreshAndUpdateCount(event?: IChangeEvent): Promise<void> {
        ...
        this.updateSearchResultCount(this.viewModel.searchResult.query!
            .userDisabledExcludesAndIgnoreFiles, this.viewModel.searchResult.query?.onlyOpenEditors, event?.clearingAll);
    ...
    private createSearchResultsView(container: HTMLElement): void {
        ...
        this.tree = this._register(this.instantiationService.createInstance(WorkbenchCompressibleAsyncDataTree<ISearchResult, RenderableMatch>, "SearchView", this.resultsElement, delegate, {
        ...
        this._register(Event.any<any>(this.tree.onDidFocus, this.tree.onDidChangeFocus)(() => {
            ...
            if (this.tree.isDOMFocused()) {
                ...
                this.fileMatchOrFolderMatchFocus.set(isSearchTreeFileMatch(focus) ||
                    isSearchTreeFolderMatch(focus));
                this.fileMatchOrFolderMatchWithResourceFocus.set(isSearchTreeFileMatch(focus) ||
                    isSearchTreeFolderMatchWithResource(focus));
    ...
    private hasSomeCollapsible(): boolean {
        ...
        do {
            if (node &&
                !viewer.isCollapsed(node) &&
                (!shouldShowAI || !isTextSearchHeading(node))) {
    ...
    async selectPreviousMatch(): Promise<void> {
        ...
        while (!prev ||
            (!isSearchTreeMatch(prev) && !this.tree.isCollapsed(prev))) {
    ...
    private updateTextFromSelection({ allowUnselectedWord = true, allowSearchOnType = true }, editor?: IEditor): boolean {
        const seedSearchStringFromSelection = this.configurationService.getValue<IEditorOptions>("editor").find!
    ...
    setSearchParameters(args: IFindInFilesArgs = {}): void {
        ...
        if (typeof args.replace === "string") {
            ...
            if (this.searchWidget.replaceInput &&
    ...
    private _searchWithIncludeOrExclude(include: boolean, folderPaths: string[]) {
        ...
        (include
            ...
            : this.inputPatternExcludes).setValue(folderPaths.join(", "));
        ...
        if (fileCount > 0) {
            ...
            if (onlyOpenEditors) {
                const searchingInOpenMessage = " - " +
                    nls.localize("onlyOpenEditors", "searching only in open files") +
                    " ";
            ...
            const openInEditorTooltip = appendKeyBindingLabel(nls.localize("openInEditor.tooltip", "Copy current search results to an editor"), this.keybindingService.lookupKeybinding(Constants.SearchCommandIds.OpenInEditorCommandId));
    ...
    private showSearchWithoutFolderMessage(): void {
        ...
        const openFolderButton = this.messageDisposables.add(new SearchLinkButton(nls.localize("openFolder", "Open Folder"), () => {
            this.commandService
                .executeCommand(env.isMacintosh && env.isNative
    ...
    private onFocus(lineMatch: ISearchTreeMatch, preserveFocus?: boolean, sideBySide?: boolean, pinned?: boolean): Promise<any> {
        const useReplacePreview = this.configurationService.getValue<ISearchConfiguration>().search
            .useReplacePreview;
        const resource = isSearchTreeMatch(lineMatch)
            ? lineMatch.parent().resource
            : (<ISearchTreeFileMatch>lineMatch).resource;
    ...
    async open(element: FileMatchOrMatch, preserveFocus?: boolean, sideBySide?: boolean, pinned?: boolean, resourceInput?: URI): Promise<void> {
        ...
        const oldParentMatches = isSearchTreeMatch(element)
            ? element.parent().matches()
            : [];
...
class SearchViewDataSource implements IAsyncDataSource<ISearchResult, RenderableMatch> {
    ...
    getChildren(element: RenderableMatch | ISearchResult): Iterable<RenderableMatch> | Promise<Iterable<RenderableMatch>> {
        if (isSearchResult(element)) {
            ...
            if (element.isAIContributed &&
                !this.searchView.model.hasAIResults) {
...
class RefreshTreeController extends Disposable {
    ...
    constructor(private readonly searchView: SearchView, private readonly geSearchConfig: () => ISearchConfigurationProperties, 
    @IFileService
    private readonly fileService: IFileService) {
        super();
    ...
    private async refreshTreeUsingQueue(): Promise<void> {
        const aggregateChangeEvent: IChangeEvent | undefined = this.queuedIChangeEvents.length === 0
            ? undefined
            : {
                elements: this.queuedIChangeEvents
                    .map((e) => e.elements)
                    .flat(),
