...
import { DEFAULT_MAX_SEARCH_RESULTS, IFolderQuery, ISearchComplete, ISearchConfigurationProperties, ISearchProgressItem, ISearchService, ITextQuery, pathIncludedInQuery, QueryType, } from "../../../../services/search/common/search.js";
...
import { contentMatchesToTextSearchMatches, INotebookCellMatchWithModel, INotebookFileMatchWithModel, webviewMatchesToTextSearchMatches, } from "./searchNotebookHelpers.js";
...
export class NotebookSearchService implements INotebookSearchService {
    ...
    private async getClosedNotebookResults(textQuery: ITextQuery, scannedFiles: ResourceSet, token: CancellationToken): Promise<IClosedNotebookSearchResults> {
            ...
            if (notebook.selectors.length > 0) {
                promises.push((async () => {
                    const includes = notebook.selectors.map((selector) => {
                        const globPattern = (selector as INotebookExclusiveDocumentFilter)
                            .include ||
        ...
        for (const result of results) {
            ...
            if (!scannedFiles.has(result.resource) &&
    ...
    private async getLocalNotebookResults(query: ITextQuery, token: CancellationToken, widgets: Array<NotebookEditorWidget>, searchID: string): Promise<IOpenNotebookSearchResults> {
        ...
        for (const widget of widgets) {
            ...
            let matches = await widget.find(query.contentPattern.pattern, {
                ...
                includeMarkupInput: query.contentPattern.notebookInfo
                    ?.isInNotebookMarkdownInput ?? true,
                includeMarkupPreview: query.contentPattern.notebookInfo
                    ?.isInNotebookMarkdownPreview ?? true,
                includeCodeInput: query.contentPattern.notebookInfo
                    ?.isInNotebookCellInput ?? true,
                includeOutput: query.contentPattern.notebookInfo
