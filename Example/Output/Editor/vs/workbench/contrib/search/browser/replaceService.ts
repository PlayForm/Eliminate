...
import { FileMatchOrMatch, ISearchTreeFileMatch, ISearchTreeMatch, isSearchTreeFileMatch, isSearchTreeMatch, } from "./searchTreeModel/searchTreeCommon.js";
...
class ReplacePreviewModel extends Disposable {
    ...
    async resolve(replacePreviewUri: URI): Promise<ITextModel> {
        ...
        const fileMatch = <ISearchTreeFileMatch>(this.searchWorkbenchService.searchModel.searchResult
            .matches(false)
...
export class ReplaceService implements IReplaceService {
    ...
    private readonly textModelResolverService: ITextModelService, 
    @IBulkEditService
    private readonly bulkEditorService: IBulkEditService, 
    @ILabelService
        ...
        const rawTextPromises = edits.map(async (e) => {
            if (e.resource.scheme === network.Schemas.vscodeNotebookCell) {
                ...
                if (notebookResource) {
                    ...
                    try {
                        ref =
                            await this.notebookEditorModelResolverService.resolve(notebookResource);
    ...
    async openReplacePreview(element: FileMatchOrMatch, preserveFocus?: boolean, sideBySide?: boolean, pinned?: boolean): Promise<any> {
        const fileMatch = isSearchTreeMatch(element)
            ? element.parent()
            : element;
        ...
        if (arg instanceof Array) {
            arg.forEach((element) => {
                ...
                if (fileMatch.count() > 0) {
                    edits.push(...fileMatch
                        .matches()
                        .flatMap((match) => this.createEdits(match, resource)));
                }
            });
        }
        return edits;
