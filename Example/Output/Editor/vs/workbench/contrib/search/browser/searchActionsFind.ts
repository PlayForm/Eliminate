...
import { ISearchResult, ISearchTreeFileMatch, ISearchTreeFolderMatchWithResource, isSearchTreeFileMatch, isSearchTreeMatch, RenderableMatch, } from "./searchTreeModel/searchTreeCommon.js";
//#region Interfaces
...
registerAction2(class RevealInSideBarForSearchResultsAction extends Action2 {
    ...
    override async run(accessor: ServicesAccessor, args: any): Promise<any> {
        ...
        paneCompositeService
            .openPaneComposite(VIEWLET_ID_FILES, ViewContainerLocation.Sidebar, false)
            .then((viewlet) => {
            if (!viewlet) {
                return;
            }
            ...
            if (uri && contextService.isInsideWorkspace(uri)) {
                ...
                explorerService
                    .select(uri, true)
                    .then(() => explorerView.focus(), onUnexpectedError);
            }
        });
    }
});
    ...
    const resolvedResources = fileService
        ...
        results.forEach((result) => {
            if (result.success && result.stat) {
                folders.push(result.stat.isDirectory
...
function getMultiSelectedSearchResources(viewer: WorkbenchCompressibleAsyncDataTree<ISearchResult, RenderableMatch, void>, currElement: RenderableMatch | undefined, sortConfig: ISearchConfigurationProperties): URI[] {
    return getElementsToOperateOn(viewer, currElement, sortConfig)
        .map((renderableMatch) => isSearchTreeMatch(renderableMatch)
        ? null
        : renderableMatch.resource)
