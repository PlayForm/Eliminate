        ...
        // Prompt to select a workspace from many
            ...
            this.notificationService.prompt(Severity.Info, localize({
                ...
                comment: ['{Locked="]({0})"}']
                    ...
                    run: () => {
                        this.quickInputService.pick(workspaces.map(workspace => ({ label: workspace } satisfies IQuickPickItem)), { placeHolder: localize('selectToOpen', "Select a workspace to open") }).then(pick => {
                            if (pick) {
