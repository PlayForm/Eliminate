...
export class BaseIssueReporterService extends Disposable {
    ...
    constructor(public disableExtensions: boolean, public data: IssueReporterData, public os: {
        type: string;
        arch: string;
        release: string;
    }, public product: IProductConfiguration, public readonly window: Window, public readonly isWeb: boolean, 
    @IIssueFormService
    public readonly issueFormService: IIssueFormService, 
    @IThemeService
    public readonly themeService: IThemeService) {
        super();
        const targetExtension = data.extensionId
            ? data.enabledExtensions.find((extension) => extension.id.toLocaleLowerCase() ===
    ...
    private updateExtensionSelector(extensions: IssueReporterExtensionData[]): void {
        ...
        if (extensionsSelector) {
            ...
            this.addEventListener("extension-selector", "change", async (e: Event) => {
                ...
                const selectedExtensionId = (<HTMLInputElement>e.target)
                    .value;
    ...
    public setEventHandlers(): void {
        ...
        for (let i = 0; i < showInfoElements.length; i++) {
            ...
            (showInfo as HTMLAnchorElement).addEventListener("click", (e: MouseEvent) => {
                ...
                if (label) {
                    const containingElement = label.parentElement &&
                        label.parentElement.parentElement;
                    const info = containingElement &&
        ...
        this.addEventListener("issue-source", "change", (e: Event) => {
            ...
            this.render();
            const title = (<HTMLInputElement>this.getElementById("issue-title"))
                .value;
        ...
        this.addEventListener("issue-title", "input", (e: Event) => {
            ...
            if (title &&
                this.getIssueUrlWithTitle(title, issueUrl).length >
                    MAX_URL_LENGTH) {
            ...
            if (isMacintosh) {
                if (cmdOrCtrlKey && e.key === "a" && e.target) {
                    if (isHTMLInputElement(e.target) ||
                        isHTMLTextAreaElement(e.target)) {
    ...
    private isPreviewEnabled() {
        ...
        if (this.isWeb) {
            ...
            if (issueType === IssueType.PerformanceIssue &&
        ...
        if (results.length) {
            ...
            for (let i = 0; i < this.numberOfSearchResultsDisplayed; i++) {
                ...
                if (issue.state) {
                    ...
                    issueIcon.appendChild(renderIcon(issue.state === "open"
                        ? Codicon.issueOpened
                        : Codicon.issueClosed));
    ...
    private setUpTypes(): void {
        ...
        reset(typeSelect, makeOption(IssueType.Bug, localize("bugReporter", "Bug Report")), makeOption(IssueType.FeatureRequest, localize("featureRequest", "Feature Request")), makeOption(IssueType.PerformanceIssue, localize("performanceIssue", "Performance Issue (freeze, slow, crash)")));
    ...
    public async submitToGitHub(issueTitle: string, issueBody: string, gitHubDetails: {
        owner: string;
    ...
    public parseGitHubUrl(url: string): undefined | {
        repositoryName: string;
        owner: string;
    } {
    ...
    private getExtensionGitHubUrl(): string {
        ...
        if (bugsUrl &&
            bugsUrl.match(/^https?:\/\/github\.com\/([^\/]*)\/([^\/]*)\/?(\/issues)?$/)) {
    ...
    public async updateExtensionStatus(extension: IssueReporterExtensionData) {
        ...
        if (template) {
            const descriptionTextArea = this.getElementById("description")!;
            const descriptionText = (descriptionTextArea as HTMLTextAreaElement)
                .value;
            if (descriptionText === "" ||
                !descriptionText.includes(template.toString())) {
                const fullTextArea = descriptionText +
                    (descriptionText === "" ? "" : "\n") +
                    template.toString();
    ...
    private getExtensionTableHtml(extensions: IssueReporterExtensionData[]): HTMLTableElement {
        return $("table", undefined, $("tr", undefined, $("th", undefined, "Extension"), $("th", undefined, "Author (truncated)" as string), $("th", undefined, "Version")), ...extensions.map((extension) => $("tr", undefined, $("td", undefined, extension.name), $("td", undefined, extension.publisher?.substr(0, 3) ?? "N/A"), $("td", undefined, extension.version))));
    }
