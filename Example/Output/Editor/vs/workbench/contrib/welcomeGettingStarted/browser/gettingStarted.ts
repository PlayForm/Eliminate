 ...
 *--------------------------------------------------------------------------------------------*/
import { $, addDisposableListener, append, clearNode, Dimension, reset, } from "../../../../base/browser/dom.js";
...
import { IRecentFolder, IRecentlyOpened, IRecentWorkspace, isRecentFolder, isRecentWorkspace, IWorkspacesService, } from "../../../../platform/workspaces/common/workspaces.js";
...
import { hiddenEntriesConfigurationKey, IResolvedWalkthrough, IResolvedWalkthroughStep, IWalkthroughsService, parseDescription, } from "./gettingStartedService.js";
import { restoreWalkthroughsConfigurationKey, RestoreWalkthroughsConfigurationValue, } from "./startupPage.js";
...
type RecentEntry = (IRecentFolder | IRecentWorkspace) & {
    id: string;
};
...
export class GettingStartedPage extends EditorPane {
    ...
    private readonly accessibilityService: IAccessibilityService) {
        ...
        this._register(this.gettingStartedService.onDidChangeWalkthrough((category) => {
            ...
            this.container
                ...
                .forEach((step) => ((step as HTMLDivElement).innerText =
                ourCategory.title));
            this.container
        ...
        this._register(this.storageService.onWillSaveState((e) => {
            ...
            if (this.workspaceContextService.getWorkspace().folders
                .length !== 0) {
                return;
            }
            if (!this.editorInput ||
    ...
    private async selectStep(id: string | undefined, delayFocus = true) {
        if (id) {
            ...
            if (!stepElement) {
                ...
                stepElement =
                    this.container.querySelector<HTMLDivElement>(`[data-step-id]`);
                if (!stepElement) {
            ...
            setTimeout(() => (stepElement as HTMLElement).focus(), delayFocus && this.shouldAnimate()
    ...
    private updateMediaSourceForColorMode(element: HTMLImageElement, sources: {
        hcDark: URI;
        hcLight: URI;
        dark: URI;
        light: URI;
    ...
    private async buildCategoriesSlide() {
        ...
        const showOnStartupCheckbox = new Toggle({
            ...
            isChecked: this.configurationService.getValue(configurationKey) ===
                "welcomePage",
        ...
        if (this.editorInput.showTelemetryNotice &&
            ...
            const daysSinceFirstSession = (+new Date() - +new Date(firstSessionDateString)) /
                1000 /
                60 /
                60 /
                24;
            ...
            if (fistContentBehaviour === "openToFirstCategory") {
                const first = this.gettingStartedCategories.filter((c) => !c.when ||
                    this.contextService.contextMatchesRules(c.when))[0];
                if (first) {
                    this.hasScrolledToFirstCategory = true;
                    this.currentWalkthrough = first;
                    this.editorInput.selectedCategory =
                        this.currentWalkthrough?.id;
                    this.editorInput.walkthroughPageTitle =
                        this.currentWalkthrough.walkthroughPageTitle;
    ...
    private buildRecentlyOpenedList(): GettingStartedIndexList<RecentEntry> {
        ...
        this.recentlyOpened
            ...
            const workspacesWithID = workspaces
                .filter((recent) => !this.workspaceContextService.isCurrentWorkspace(isRecentWorkspace(recent)
                ? recent.workspace
                : recent.folderUri))
                .map((recent) => ({
                ...recent,
    ...
    private buildStartList(): GettingStartedIndexList<IWelcomePageStartEntry> {
        const renderStartEntry = (entry: IWelcomePageStartEntry): HTMLElement => $("li", {}, $("button.button-link", {
    ...
    private buildGettingStartedWalkthroughsList(): GettingStartedIndexList<IResolvedWalkthrough> {
        const renderGetttingStaredWalkthrough = (category: IResolvedWalkthrough): HTMLElement => {
            const renderNewBadge = (category.newItems || category.newEntry) &&
                !category.isFeatured;
    ...
    private async scrollToCategory(categoryID: string, stepId?: string) {
        ...
        this.inProgressScroll = this.inProgressScroll.then(async () => {
            ...
            this.editorInput.walkthroughPageTitle =
                ourCategory.walkthroughPageTitle;
            this.currentWalkthrough = ourCategory;
            this.buildCategorySlide(categoryID, stepId);
            this.setSlide("details");
        });
    }
    private iconWidgetFor(category: IResolvedWalkthrough | {
        icon: {
            type: "icon";
            icon: ThemeIcon;
        };
    ...
    private buildCategorySlide(categoryID: string, selectedStep?: string) {
        ...
        this.detailsPageDisposables.add(addDisposableListener(stepListContainer, "keydown", (e) => {
            ...
            if (event.keyCode === KeyCode.UpArrow) {
                const toExpand = category.steps.filter((step, index) => index < currentStepIndex() &&
            ...
            if (event.keyCode === KeyCode.DownArrow) {
                const toExpand = category.steps.find((step, index) => index > currentStepIndex() &&
                    this.contextService.contextMatchesRules(step.when));
                if (toExpand) {
        ...
        const buildStepList = () => {
            ...
            reset(stepListContainer, ...renderedSteps.map((step) => {
                const codicon = $(".codicon" +
                    (step.done
                        ? ".complete" +
                            ThemeIcon.asCSSSelector(gettingStartedCheckedCodicon)
        ...
        const toExpand = category.steps.find((step) => this.contextService.contextMatchesRules(step.when) &&
            !step.done) ?? category.steps[0];
    ...
    private async scrollPrev() {
        this.inProgressScroll = this.inProgressScroll.then(async () => {
            if (this.prevWalkthrough &&
                ...
                if (this.gettingStartedCategories.length !==
    ...
    private setSlide(toEnable: "details" | "categories", firstLaunch: boolean = false) {
        ...
        if (toEnable === "categories") {
            ...
            prevButton!.style.display =
                this.editorInput.showWelcome || this.prevWalkthrough
                    ? "block"
                    : "none";
