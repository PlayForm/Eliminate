...
export class WalkthroughsService extends Disposable implements IWalkthroughsService {
    ...
    private readonly productService: IProductService) {
        super();
        this.metadata = new Map(JSON.parse(this.storageService.get(walkthroughMetadataConfigurationKey, StorageScope.PROFILE, "[]")));
    ...
    private registerWalkthroughs() {
        walkthroughs.forEach(async (category, index) => {
            this._registerWalkthrough({
                ...
                when: ContextKeyExpr.deserialize(category.when) ??
                    ContextKeyExpr.true(),
                steps: category.content.steps.map((step, index) => {
                    return {
                        ...
                        order: index,
                        when: ContextKeyExpr.deserialize(step.when) ??
                            ContextKeyExpr.true(),
    ...
    private initCompletionEventListeners() {
        ...
        this._register(this.extensionManagementService.onDidInstallExtensions((result) => {
            if (result.some((e) => ExtensionIdentifier.equals(this.productService.gitHubEntitlement
                ?.extensionId, e.identifier.id) &&
                !e?.context?.[EXTENSION_INSTALL_SKIP_WALKTHROUGH_CONTEXT])) {
                result.forEach((e) => {
    ...
    private async registerExtensionWalkthroughContributions(extension: IExtensionDescription) {
        ...
        const convertExtensionRelativePathsToBrowserURIs = (path: string | {
            hc: string;
            hcLight?: string;
            dark: string;
            light: string;
        ...
        await Promise.all(extension.contributes?.walkthroughs?.map(async (walkthrough, index) => {
            ...
            const steps = (walkthrough.steps ?? []).map((step, index) => {
                ...
                const fullyQualifiedID = extension.identifier.value +
                    "#" +
                    walkthrough.id +
                    "#" +
                    step.id;
                ...
                if (!step.media) {
                    throw Error("missing media in walkthrough step: " +
                        walkthrough.id +
                        "@" +
                        step.id);
                }
                ...
                // Throw error for unknown walkthrough format
                    ...
                    throw new Error("Unknown walkthrough format detected for " +
                        fullyQualifiedID);
                }
                return {
                    description,
            ...
            if (walkthrough.featuredFor) {
                ...
                isFeatured =
                    await this.instantiationService.invokeFunction((a) => checkGlobFileExists(a, folders, walkthrough.featuredFor!, token.token));
            }
        ...
        if (hadLastFoucs &&
            sectionToOpen &&
            this.configurationService.getValue<string>("workbench.welcomePage.walkthroughs.openOnInstall")) {
    ...
    getWalkthroughs(): IResolvedWalkthrough[] {
        ...
        const categoriesWithCompletion = registeredCategories
            ...
            .filter((category) => category.content.type !== "steps" ||
    ...
    private resolveWalkthrough(category: IWalkthrough): IResolvedWalkthrough {
        ...
        if (firstSeenDate) {
            ...
            recencyBonus = Math.max(0, (NEW_WALKTHROUGH_TIME - timeSinceFirstSeen) /
    ...
    _registerWalkthrough(walkthroughDescriptor: IWalkthrough): void {
        ...
        walkthroughDescriptor.steps.forEach((step) => {
            if (this.steps.has(step.id)) {
                throw Error("Attempting to register step with id " +
                    step.id +
        ...
        if (!step.completionEvents.length) {
            step.completionEvents = coalesce(step.description
                ...
                if (href.startsWith("command:")) {
                    return ("onCommand:" +
                        href.slice("command:".length, href.includes("?")
                            ? href.indexOf("?")
                            : undefined));
                }
            ...
            switch (eventType) {
                ...
                case "onContext": {
                    ...
                    if (expression) {
                        ...
                        expression
                            .keys()
                            .forEach((key) => this.stepCompletionContextKeys.add(key));
    ...
    private getStep(id: string): IWalkthroughStep {
        ...
        if (!step) {
            throw Error("Attempting to access step which does not exist in registry " +
                id);
        }
        return step;
    }
}
export const parseDescription = (desc: string): LinkedText[] => desc
    .split("\n")
    .filter((x) => x)
...
const convertInternalMediaPathsToBrowserURIs = (path: string | {
    hc: string;
    hcLight?: string;
    dark: string;
    light: string;
