...
registerAction2(class extends Action2 {
    ...
    public run(accessor: ServicesAccessor, walkthroughID: string | {
        ...
        if (walkthroughID) {
            const selectedCategory = typeof walkthroughID === "string"
                ? walkthroughID
...
Registry.as<IEditorPaneRegistry>(EditorExtensions.EditorPane).registerEditorPane(EditorPaneDescriptor.create(GettingStartedPage, GettingStartedPage.ID, localize("welcome", "Welcome")), [new SyncDescriptor(GettingStartedInput)]);
...
class WorkspacePlatformContribution {
    ...
    private readonly contextService: IContextKeyService) {
        this.remoteAgentService.getEnvironment().then((env) => {
            ...
            const remotePlatform = remoteOS === OS.Macintosh
                ? "mac"
                : remoteOS === OS.Windows
                    ? "windows"
                    : remoteOS === OS.Linux
