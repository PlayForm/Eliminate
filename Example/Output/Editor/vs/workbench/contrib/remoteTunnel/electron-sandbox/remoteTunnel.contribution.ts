...
import { CONFIGURATION_KEY_HOST_NAME, CONFIGURATION_KEY_PREFIX, CONFIGURATION_KEY_PREVENT_SLEEP, ConnectionInfo, INACTIVE_TUNNEL_MODE, IRemoteTunnelService, IRemoteTunnelSession, LOG_ID, LOGGER_NAME, TunnelStatus, } from "../../../../platform/remoteTunnel/common/remoteTunnel.js";
import { IStorageService, StorageScope, StorageTarget, } from "../../../../platform/storage/common/storage.js";
import { isUntitledWorkspace, IWorkspaceContextService, } from "../../../../platform/workspace/common/workspace.js";
...
export class RemoteTunnelWorkbenchContribution extends Disposable implements IWorkbenchContribution {
    ...
    private remoteTunnelService: IRemoteTunnelService, 
    @ICommandService
    ...
    private async initialize(): Promise<void> {
        ...
        const doInitialStateDiscovery = async (progress?: IProgress<IProgressStep>) => {
            ...
            const status = await this.remoteTunnelService.initialize(mode.active && newSession
    ...
    private async createQuickpickItems(sessions: ExistingSessionItem[]): Promise<(ExistingSessionItem | AuthenticationProviderOption | IQuickPickSeparator | (IQuickPickItem & {
                    ...
                    notificationService.notify({
                        ...
                        actions: {
                            primary: [
                                ...
                                new Action("showExtension", localize("action.showExtension", "Show Extension"), undefined, true, () => {
                                    return commandService.executeCommand("workbench.extensions.action.showExtensionsWithIds", [
        ...
        return new Promise<void>((c, e) => {
            ...
            if (this.connectionInfo) {
                quickPick.title = this.connectionInfo.isAttached
                    ...
                    }, "Remote Tunnel Access enabled for {0} (launched externally)", this.connectionInfo.tunnelName)
                    : localize({
                        key: "manage.title.orunning",
                        comment: ["{0} is the tunnel name"],
                    }, "Remote Tunnel Access enabled for {0}", this.connectionInfo.tunnelName);
                items.push({
                    id: RemoteTunnelCommandIds.copyToClipboard,
