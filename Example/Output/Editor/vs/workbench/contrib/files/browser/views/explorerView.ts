...
import { ExplorerCompressionDelegate, ExplorerDataSource, ExplorerDelegate, FileDragAndDrop, FilesFilter, FileSorter, FilesRenderer, ICompressedNavigationController, isCompressedFolderName, } from "./explorerViewer.js";
...
export function getContext(focus: ExplorerItem[], selection: ExplorerItem[], respectMultiSelection: boolean, compressedNavigationControllerProvider: {
    ...
    const compressedNavigationController = compressedNavigationControllers &&
...
class ExplorerFindProvider implements IAsyncFindProvider<ExplorerItem> {
    ...
    private async *createResultItems(folderResults: {
        folder: URI;
        result: ISearchComplete;
    }[], pattern: string, isFuzzyMatch: boolean): AsyncIterable<IAsyncFindResult<ExplorerItem>> {
    ...
    private createItem(resource: URI, root: ExplorerItem): ExplorerItem | undefined {
        ...
        for (const stat of pathSegments) {
            currentResource = currentResource.with({
                path: `${currentResource.path}/${stat}`,
            });
...
export class ExplorerView extends ViewPane implements IExplorerView {
    ...
    constructor(options: IExplorerViewPaneOptions, 
    @IContextMenuService
    ...
    openerService: IOpenerService) {
        ...
        this.parentReadonlyContext =
            ExplorerResourceParentReadOnlyContext.bindTo(contextKeyService);
    ...
    @memoize
    private get fileCopiedContextKey(): IContextKey<boolean> {
        return FileCopiedContext.bindTo(this.contextKeyService);
    }
    @memoize
    private get resourceCutContextKey(): IContextKey<boolean> {
    ...
    private createTree(container: HTMLElement): void {
        ...
        const getFileNestingSettings = (item?: ExplorerItem) => this.configurationService.getValue<IFilesConfiguration>({
            resource: item?.root.resource,
        }).explorer.fileNesting;
        const rootsSupportFindProvider = this.explorerService.roots.every((root) => root.resource.scheme === Schemas.file ||
            root.resource.scheme === Schemas.vscodeRemote);
        this.tree = <WorkbenchCompressibleAsyncDataTree<ExplorerItem | ExplorerItem[], ExplorerItem, FuzzyScore>>this.instantiationService.createInstance(WorkbenchCompressibleAsyncDataTree, "FileExplorer", container, new ExplorerDelegate(), new ExplorerCompressionDelegate(), [this.renderer], this.instantiationService.createInstance(ExplorerDataSource, this.filter), {
            ...
            findResultsProvider: rootsSupportFindProvider
                ? this.instantiationService.createInstance(ExplorerFindProvider)
                : undefined,
        });
        ...
        this._register(onDidChangeCompressionConfiguration((_) => this.tree.updateOptions({
            compressionEnabled: isCompressionEnabled(),
        })));
        // Bind context keys
        ...
        this._register(this.tree.onDidOpen(async (e) => {
            ...
            const shiftDown = DOM.isKeyboardEvent(e.browserEvent) &&
    ...
    private onFocusChanged(elements: readonly ExplorerItem[]): void {
        ...
        this.setContextKeys(stat);
        if (stat) {
            const enableTrash = Boolean(this.configurationService.getValue<IFilesConfiguration>().files
                ?.enableTrash);
        ...
        if (Array.isArray(treeInput)) {
            if (hasExpandedRootChild(this.tree, treeInput)) {
                treeInput.forEach((folder) => {
                    folder.children.forEach((child) => this.tree.hasNode(child) &&
