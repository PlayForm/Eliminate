...
export class BrowserFileUpload {
    ...
    private async doUploadEntry(entry: IWebkitDataTransferItemEntry, parentResource: URI, target: ExplorerItem | undefined, progress: IProgress<IProgressStep>, operation: IBrowserUploadOperation, token: CancellationToken): Promise<{
        isFile: boolean;
        resource: URI;
    } | undefined> {
        if (token.isCancellationRequested ||
        ...
        const reportProgress = (fileSize: number, bytesUploaded: number): void => {
            ...
            const bytesUploadedPerSecond = operation.totalBytesUploaded /
    ...
    private async importResources(target: ExplorerItem, resources: URI[], token: CancellationToken): Promise<void> {
        if (resources && resources.length > 0) {
            ...
            const resourcesFiltered = coalesce(await Promises.settled(resources.map(async (resource) => {
                ...
                if (targetNames.has(caseSensitive
                    ? basename(resource)
            ...
            const undoLevel = this.configurationService.getValue<IFilesConfiguration>()
            ...
            await this.explorerService.applyBulkEdit(resourceFileEdits, {
                ...
                confirmBeforeUndo: undoLevel === UndoConfirmLevel.Verbose ||
                    undoLevel === UndoConfirmLevel.Default,
            });
            // if we only add one file, just open it directly
            const autoOpen = this.configurationService.getValue<IFilesConfiguration>()
...
export class FileDownload {
    ...
    private async doDownloadBrowser(resource: URI, progress: IProgress<IProgressStep>, cts: CancellationTokenSource): Promise<void> {
        ...
        if (preferFileSystemAccessWebApis &&
    ...
    private reportProgress(name: string, fileSize: number, bytesDownloaded: number, operation: IDownloadOperation): void {
        ...
        const bytesDownloadedPerSecond = operation.totalBytesDownloaded /
            ((Date.now() - operation.startTime) / 1000);
        // Small file
        let message: string;
        if (fileSize < ByteSize.MB) {
