...
import { getFileNamesMessage, IConfirmationResult, IDialogService, } from "../../../../platform/dialogs/common/dialogs.js";
...
import { IFilesConfiguration, UndoConfirmLevel, VIEW_ID, VIEWLET_ID, } from "../common/files.js";
import { NEW_UNTITLED_FILE_COMMAND_ID, REVEAL_IN_EXPLORER_COMMAND_ID, SAVE_ALL_IN_GROUP_COMMAND_ID, } from "./fileConstants.js";
...
async function deleteFiles(explorerService: IExplorerService, workingCopyFileService: IWorkingCopyFileService, dialogService: IDialogService, configurationService: IConfigurationService, elements: ExplorerItem[], useTrash: boolean, skipConfirm = false, ignoreIfNotExists = false): Promise<void> {
    ...
    if (skipConfirm ||
        (useTrash &&
            configurationService.getValue<boolean>(CONFIRM_DELETE_SETTING_KEY) === false)) {
...
export async function findValidPasteFileTarget(explorerService: IExplorerService, fileService: IFileService, dialogService: IDialogService, targetFolder: ExplorerItem, fileToPaste: {
    resource: URI | string;
    isDirectory?: boolean;
...
export class GlobalCompareResourcesAction extends Action2 {
    ...
    override async run(accessor: ServicesAccessor): Promise<void> {
        ...
        if (activeResource &&
            ...
            if (picks?.length === 1) {
                const resource = (picks[0] as unknown as {
                    resource: unknown;
                })
                    .resource;
...
export function validateFileName(pathService: IPathService, item: ExplorerItem, name: string, os: OperatingSystem): {
    content: string;
    severity: Severity;
} | null {
        ...
        if (resource &&
            ...
            if (!this.registrationDisposal) {
                ...
                this.registrationDisposal =
                    textModelService.registerTextModelContentProvider(scheme, provider);
            }
        ...
        onFinish: async (value, success) => {
            if (success) {
                ...
                if (stat.resource.toString() !== targetResource.toString()) {
                    try {
                        await explorerService.applyBulkEdit([
                            new ResourceFileEdit(stat.resource, targetResource),
                        ], {
                            confirmBeforeUndo: configurationService.getValue<IFilesConfiguration>()
...
export const pasteFileHandler = async (accessor: ServicesAccessor, fileList?: FileList) => {
    ...
    const confirmPasteNative = hasNativeFilesToPaste &&
    ...
    if (confirmPasteNative && toPaste.files.length >= 1) {
        const message = toPaste.files.length > 1
            ...
            : nls.localize("confirmPasteNative", "Are you sure you want to paste '{0}'?", basename(toPaste.type === "paths"
    ...
    const element = context.length ? context[0] : explorerService.roots[0];
    const incrementalNaming = configurationService.getValue<IFilesConfiguration>().explorer
        .incrementalNaming;
    ...
    async function applyCopyResourceEdit(targets: readonly URI[], resourceFileEdits: ResourceFileEdit[]) {
        const undoLevel = configurationService.getValue<IFilesConfiguration>().explorer
            .confirmUndo;
        const options = {
            confirmBeforeUndo: undoLevel === UndoConfirmLevel.Default ||
...
async function getFilesToPaste(fileList: FileList | undefined, clipboardService: IClipboardService, hostService: IHostService): Promise<FilesToPaste> {
    if (fileList && fileList.length > 0) {
        ...
        return {
            type: "data",
            files: [...fileList].filter((file) => !getPathForFile(file)),
        };
    }
    else {
...
class BaseSetActiveEditorReadonlyInSession extends Action2 {
    constructor(id: string, title: ILocalizedString, private readonly newReadonlyState: true | false | "toggle" | "reset") {
        super({
            id,
            title,
            f1: true,
