...
import { ITestResult, ITestRunTaskResults, LiveTestResult, maxCountPriority, TestResultItemChangeReason, } from "../../common/testResult.js";
import { ITestResultService } from "../../common/testResultService.js";
import { InternalTestItem, IRichLocation, ITestItemContext, ITestMessage, ITestMessageMenuArgs, TestMessageType, TestResultItem, TestResultState, testResultStateToContextValues, TestRunProfileBitset, } from "../../common/testTypes.js";
import { getTestItemContextOverlay } from "../explorerProjections/testItemContextOverlay.js";
import * as icons from "../icons.js";
import { renderTestMessageAsText } from "../testMessageColorizer.js";
import { getMessageArgs, InspectSubject, mapFindTestMessage, MessageSubject, TaskSubject, TestOutputSubject, } from "./testResultsSubject.js";
...
class CoverageElement implements ITreeElement {
    ...
    constructor(private readonly results: ITestResult, public readonly task: ITestRunTaskResults, private readonly coverageService: ITestCoverageService) {
...
class TestCaseElement implements ITreeElement {
    ...
    constructor(public readonly results: ITestResult, public readonly test: TestResultItem, public readonly taskIndex: number) { }
}
...
class TestMessageElement implements ITreeElement {
    ...
    constructor(public readonly result: ITestResult, public readonly test: TestResultItem, public readonly taskIndex: number, public readonly messageIndex: number) {
...
export class OutputPeekTree extends Disposable {
    ...
    @ITestResultService
    results: ITestResultService, 
    @IInstantiationService
    instantiationService: IInstantiationService, 
    @ITestExplorerFilterState
    explorerFilter: ITestExplorerFilterState, 
    @ITestCoverageService
    coverageService: ITestCoverageService, 
    @IProgressService
    ...
    telemetryService: ITelemetryService) {
        ...
        this.tree = this._register(instantiationService.createInstance(WorkbenchCompressibleObjectTree, "Test Output Peek", container, {
            ...
            sorter: {
                compare(a, b) {
                    if (a instanceof TestCaseElement &&
        ...
        const getTaskChildren = (taskElem: TaskElement): Iterable<ICompressedTreeElement<TreeElement>> => {
            ...
            const tests = Iterable.filter(results.tests, (test) => test.tasks[index].state >= TestResultState.Running ||
            ...
            if (task.coverage.get()) {
                result = Iterable.concat(Iterable.single<ICompressedTreeElement<TreeElement>>({
        ...
        const getTestChildren = (result: ITestResult, test: TestResultItem, taskIndex: number): Iterable<ICompressedTreeElement<TreeElement>> => {
            return test.tasks[taskIndex].messages
                .map((m, messageIndex) => m.type === TestMessageType.Error
                ? {
        ...
        this._register(this.tree.onDidChangeCollapseState((e) => {
            if (e.node.element instanceof OlderResultsElement &&
                !e.node.collapsed) {
...
class TestRunElementRenderer implements ICompressibleTreeRenderer<ITreeElement, FuzzyScore, TemplateData> {
    ...
    constructor(private readonly treeActions: TreeActionsProvider, 
    @IInstantiationService
    ...
    public renderCompressedElements(node: ITreeNode<ICompressedTreeNode<ITreeElement>, FuzzyScore>, _index: number, templateData: TemplateData): void {
        ...
        if ((lastElement instanceof TaskElement ||
...
class TreeActionsProvider {
    constructor(private readonly showRevealLocationOnMessages: boolean, private readonly requestReveal: Emitter<InspectSubject>, 
    @IContextKeyService
    ...
    public provideActionBar(element: ITreeElement) {
        ...
        if (element instanceof TaskElement) {
            ...
            if (element.task.running) {
                primary.push(new Action("testing.outputPeek.cancel", localize("testing.cancelRun", "Cancel Test Run"), ThemeIcon.asClassName(icons.testingCancelIcon), undefined, () => this.commandService.executeCommand(TestCommandId.CancelTestRunAction, element.results.id, element.task.id)));
            }
            else {
                primary.push(new Action("testing.outputPeek.rerun", localize("testing.reRunLastRun", "Rerun Last Run"), ThemeIcon.asClassName(icons.testingRerunIcon), undefined, () => this.commandService.executeCommand(TestCommandId.ReRunLastRun, element.results.id)));
                primary.push(new Action("testing.outputPeek.debug", localize("testing.debugLastRun", "Debug Last Run"), ThemeIcon.asClassName(icons.testingDebugIcon), undefined, () => this.commandService.executeCommand(TestCommandId.DebugLastRun, element.results.id)));
            }
        }
        ...
        if (element instanceof TestCaseElement ||
            ...
            if (capabilities & TestRunProfileBitset.Run) {
                primary.push(new Action("testing.outputPeek.runTest", localize("run test", "Run Test"), ThemeIcon.asClassName(icons.testingRunIcon), undefined, () => this.commandService.executeCommand("vscode.runTestsById", TestRunProfileBitset.Run, extId)));
            }
            if (capabilities & TestRunProfileBitset.Debug) {
                primary.push(new Action("testing.outputPeek.debugTest", localize("debug test", "Debug Test"), ThemeIcon.asClassName(icons.testingDebugIcon), undefined, () => this.commandService.executeCommand("vscode.runTestsById", TestRunProfileBitset.Debug, extId)));
            }
        }
