...
import { createActionViewItem, createAndFillInActionBarActions, createAndFillInContextMenuActions, MenuEntryActionViewItem, } from "../../../../platform/actions/browser/menuEntryActionViewItem.js";
...
import { getTestingConfiguration, TestingConfigKeys, TestingCountBadge, } from "../common/configuration.js";
import { labelForTestInState, TestCommandId, TestExplorerViewMode, TestExplorerViewSorting, Testing, } from "../common/constants.js";
...
import { InternalTestItem, ITestRunProfile, TestControllerCapability, TestItemExpandState, TestResultState, testResultStateToContextValues, TestRunProfileBitset, } from "../common/testTypes.js";
...
import { collectTestStateCounts, CountSummary, getTestProgressText, } from "./testingProgressUiService.js";
...
export class TestingExplorerView extends ViewPane {
    ...
    public getTreeIncludeExclude(profileOrBitset: ITestRunProfile | TestRunProfileBitset, withinItems?: InternalTestItem[], filterToType: "visible" | "selected" = "visible") {
        ...
        for (const root of withinItems ||
            ...
            if (typeof profileOrBitset === "object" &&
            ...
            if (!this.viewModel.tree.hasElement(element)) {
                const visibleChildren = [...element.children].reduce((acc, c) => this.viewModel.tree.hasElement(c) &&
    ...
    private getTestConfigGroupActions(group: TestRunProfileBitset) {
        ...
        if (profileActions.length > 1) {
            postActions.push(new Action("selectDefaultTestConfigurations", localize("selectDefaultConfigs", "Select Default Profile"), undefined, undefined, () => this.commandService.executeCommand<ITestRunProfile>(TestCommandId.SelectDefaultTestProfiles, group)));
        }
        if (hasConfigurable) {
            postActions.push(new Action("configureTestProfiles", localize("configureTestProfiles", "Configure Test Profiles"), undefined, undefined, () => this.commandService.executeCommand<ITestRunProfile>(TestCommandId.ConfigureTestProfilesAction, group)));
        }
...
class TestingExplorerViewModel extends Disposable {
    ...
    constructor(listContainer: HTMLElement, onDidChangeVisibility: Event<boolean>, 
    @IConfigurationService
    configurationService: IConfigurationService, 
    @IEditorService
    ...
    @ITestResultService
    private readonly testResults: ITestResultService, 
    @ITestingPeekOpener
    private readonly peekOpener: ITestingPeekOpener, 
    @ITestProfileService
    private readonly testProfileService: ITestProfileService, 
    @ITestingContinuousRunService
    private readonly crService: ITestingContinuousRunService, 
    ...
    commandService: ICommandService) {
        ...
        this._register(this.crService.onDidChange((testId) => {
            if (testId) {
                ...
                this.tree.resort(elem?.parent && this.tree.hasElement(elem.parent)
                    ? elem.parent
                    : null, false);
            }
        }));
        ...
        this._register(testResults.onTestChanged((evt) => {
            ...
            if (evt.item.ownComputedState !== TestResultState.Running &&
    ...
    private handleExecuteKeypress(evt: IKeyboardEvent) {
        ...
        const toRun = targeted.filter((e): e is TestItemTreeElement => e instanceof TestItemTreeElement);
        if (toRun.length) {
...
class TestsFilter implements ITreeFilter<TestExplorerTreeElement> {
    ...
    constructor(private readonly collection: IMainThreadTestCollection, 
    @ITestExplorerFilterState
    ...
    private testFilterText(element: TestItemTreeElement) {
        ...
        for (let e: TestItemTreeElement | null = element; e; e = e.parent) {
            ...
            let included = this.state.globList[0].include === false
                ? FilterResult.Include
                : FilterResult.Inherit;
...
class TreeSorter implements ITreeSorter<TestExplorerTreeElement> {
    ...
    public compare(a: TestExplorerTreeElement, b: TestExplorerTreeElement): number {
        if (a instanceof TestTreeErrorMessage ||
            b instanceof TestTreeErrorMessage) {
            return ((a instanceof TestTreeErrorMessage ? -1 : 0) +
                (b instanceof TestTreeErrorMessage ? 1 : 0));
        }
        const durationDelta = (b.duration || 0) - (a.duration || 0);
        if (this.viewModel.viewSorting === TestExplorerViewSorting.ByDuration &&
            durationDelta !== 0) {
            return durationDelta;
        }
        const stateDelta = cmpPriority(a.state, b.state);
        if (this.viewModel.viewSorting === TestExplorerViewSorting.ByStatus &&
            stateDelta !== 0) {
            return stateDelta;
        }
        ...
        if (a instanceof TestItemTreeElement &&
            ...
            b.test.item.range) {
            inSameLocation = true;
            const delta = a.test.item.range.startLineNumber -
...
class NoTestsForDocumentWidget extends Disposable {
    ...
    @ITestExplorerFilterState
    filterState: ITestExplorerFilterState) {
        super();
...
class TestItemRenderer extends Disposable implements ITreeRenderer<TestItemTreeElement, FuzzyScore, ITestElementTemplateData> {
    public static readonly ID = "testItem";
    constructor(private readonly actionRunner: TestExplorerActionRunner, 
    @IMenuService
    private readonly menuService: IMenuService, 
    @ITestService
    protected readonly testService: ITestService, 
    @ITestProfileService
    protected readonly profiles: ITestProfileService, 
    @IContextKeyService
    private readonly contextKeyService: IContextKeyService, 
    @IInstantiationService
    private readonly instantiationService: IInstantiationService, 
    ...
    public _renderElement(node: ITreeNode<TestItemTreeElement, FuzzyScore>, data: ITestElementTemplateData): void {
        ...
        const icon = icons.testingStatesToIcons.get(node.element.test.expand === TestItemExpandState.BusyExpanding ||
...
const getActionableElementActions = (contextKeyService: IContextKeyService, menuService: IMenuService, testService: ITestService, crService: ITestingContinuousRunService, profiles: ITestProfileService, element: TestItemTreeElement) => {
    const test = element instanceof TestItemTreeElement ? element.test : undefined;
    const contextKeys: [
        string,
        unknown
    ][] = getTestItemContextOverlay(test, test ? profiles.capabilitiesForTest(test.item) : 0);
    ...
    if (test) {
        ...
        ], [TestingContextKeys.supportsContinuousRun.key, supportsCr], [TestingContextKeys.testResultOutdated.key, element.retired], [
