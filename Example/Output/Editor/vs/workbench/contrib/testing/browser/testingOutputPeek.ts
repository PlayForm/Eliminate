...
import { IPeekViewService, peekViewTitleForeground, peekViewTitleInfoForeground, PeekViewWidget, } from "../../../../editor/contrib/peekView/browser/peekView.js";
...
import { AutoOpenPeekViewWhen, getTestingConfiguration, TestingConfigKeys, } from "../common/configuration.js";
...
import { ITestResult, resultItemParents, TestResultItemChange, TestResultItemChangeReason, } from "../common/testResult.js";
...
import { InspectSubject, inspectSubjectHasStack, mapFindTestMessage, MessageSubject, TaskSubject, TestOutputSubject, } from "./testResultsView/testResultsSubject.js";
...
export class TestingPeekOpener extends Disposable implements ITestingPeekOpener {
    ...
    private getActiveControl(): InspectSubject | undefined {
        ...
        return (controller?.subject ??
            this.viewsService.getActiveViewWithId<TestResultsView>(Testing.ResultsViewId)?.subject);
    }
    /** @inheritdoc */
    ...
    private openPeekOnFailure(evt: TestResultItemChange) {
        ...
        if (evt.result.request.continuous &&
            !getTestingConfiguration(this.configuration, TestingConfigKeys.AutoOpenPeekViewDuringContinuousRun)) {
            return;
        }
        ...
        switch (cfg) {
            case AutoOpenPeekViewWhen.FailureVisible: {
                ...
                const editorUris = new Set(visibleEditors
                    .filter(isCodeEditor)
                    .map((e) => e.getModel()?.uri.toString()));
...
class TestResultsPeek extends PeekViewWidget {
    ...
    @IPeekViewService
    peekViewService: IPeekViewService, 
    @ITestingPeekOpener
    private readonly testingPeek: ITestingPeekOpener, 
    @IContextKeyService
    ...
    private applyTheme() {
        ...
        const isError = this.current instanceof MessageSubject &&
    ...
    protected override _fillContainer(container: HTMLElement): void {
        if (!this.scopedContextKeyService) {
            ...
            TestingContextKeys.isInPeek
                ...
                .set(true);
            const instaService = this._disposables.add(this.instantiationService.createChild(new ServiceCollection([
                IContextKeyService,
    ...
    public setModel(subject: InspectSubject): Promise<void> {
        ...
        const peekLines = TestResultsPeek.lastHeightInLines ||
            Math.max(inspectSubjectHasStack(subject)
                ? Math.ceil(this.getVisibleEditorLines() / 2)
                : 0, hintMessagePeekHeight(message));
    ...
    private getVisibleEditorLines() {
        ...
        return Math.round(this.editor.getDomNode()!.clientHeight /
...
const hintMessagePeekHeight = (msg: ITestMessage) => {
    const msgHeight = ITestMessage.isDiffable(msg)
        ...
        : hintPeekStrHeight(typeof msg.message === "string"
            ? msg.message
            : msg.message.value);
