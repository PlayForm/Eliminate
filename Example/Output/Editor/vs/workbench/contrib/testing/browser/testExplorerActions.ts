...
import { TestCommandId, testConfigurationGroupNames, TestExplorerViewMode, TestExplorerViewSorting, Testing, } from "../common/constants.js";
...
import { expandAndGetTestById, IMainThreadTestCollection, IMainThreadTestController, ITestService, testsInFile, testsUnderUri, } from "../common/testService.js";
import { ExtTestRunProfileKind, InternalTestItem, ITestRunProfile, TestItemExpandState, TestRunProfileBitset, } from "../common/testTypes.js";
...
const continuousMenus = (whenIsContinuousOn: boolean): IAction2Options["menu"] => [
    {
        ...
        group: "navigation",
        order: ActionOrder.RunUsing,
        when: ContextKeyExpr.and(ContextKeyExpr.equals("view", Testing.ExplorerViewId), TestingContextKeys.supportsContinuousRun.isEqualTo(true), TestingContextKeys.isContinuousModeOn.isEqualTo(whenIsContinuousOn)),
    },
    {
        id: MenuId.CommandPalette,
...
function selectContinuousRunProfiles(crs: ITestingContinuousRunService, notificationService: INotificationService, quickInputService: IQuickInputService, profilesToPickFrom: Iterable<Readonly<{
    controller?: IMainThreadTestController;
    profiles: ITestRunProfile[];
}>>): Promise<ITestRunProfile[]> {
    type ItemType = IQuickPickItem & {
    ...
    const quickpick = disposables.add(quickInputService.createQuickPick<IQuickPickItem & {
        profile: ITestRunProfile;
    }>({ useSeparators: true }));
            ...
            menu: [
                {
                    ...
                    group: "navigation",
                    when: ContextKeyExpr.and(ContextKeyExpr.equals("view", Testing.ExplorerViewId), TestingContextKeys.isRunning.isEqualTo(false), TestingContextKeys.capabilityToContextKey[group].isEqualTo(true)),
                },
            ],
            category,
            viewId: Testing.ExplorerViewId,
...
export class RunAllAction extends RunOrDebugAllTestsAction {
    constructor() {
        ...
        }, TestRunProfileBitset.Run, localize("noTestProvider", "No tests found in this workspace. You may need to install a test provider extension"));
    }
}
export class DebugAllAction extends RunOrDebugAllTestsAction {
    constructor() {
        ...
        }, TestRunProfileBitset.Debug, localize("noDebugTestProvider", "No debuggable tests found in this workspace. You may need to install a test provider extension"));
    }
}
export class CoverageAllAction extends RunOrDebugAllTestsAction {
    constructor() {
        ...
        }, TestRunProfileBitset.Coverage, localize("noCoverageTestProvider", "No tests with coverage runners found in this workspace. You may need to install a test provider extension"));
    }
}
            ...
            menu: [
                {
                    ...
                    group: "navigation",
                    when: ContextKeyExpr.and(ContextKeyExpr.equals("view", Testing.ExplorerViewId), ContextKeyExpr.equals(TestingContextKeys.isRunning.serialize(), true)),
                },
            ],
        });
    }
...
export class GoToTest extends Action2 {
    ...
    public override async run(accessor: ServicesAccessor, element?: TestExplorerTreeElement, preserveFocus?: boolean) {
        if (!element) {
            const view = accessor
                .get(IViewsService)
                .getActiveViewWithId<TestingExplorerView>(Testing.ExplorerViewId);
...
abstract class ExecuteTestAtCursor extends Action2 {
    ...
    public async run(accessor: ServicesAccessor) {
        ...
        // the closest one before the position. Again, if we find several tests
        // whose range is equal to the closest one, we run them all.
        const testsToRun = await showDiscoveringWhile(progressService, getTestsAtCursor(testService, uriIdentityService, model.uri, position, (test) => !!(profileService.capabilitiesForTest(test.item) &
            this.group)));
...
export const discoverAndRunTests = async (collection: IMainThreadTestCollection, progress: IProgressService, ids: ReadonlyArray<string>, runTests: (tests: ReadonlyArray<InternalTestItem>) => Promise<ITestResult>): Promise<ITestResult | undefined> => {
...
const refreshMenus = (whenIsRefreshing: boolean): IAction2Options["menu"] => [
    ...
    },
    {
        ...
        group: "navigation",
        order: ActionOrder.Refresh,
        when: ContextKeyExpr.and(ContextKeyExpr.equals("view", Testing.ExplorerViewId), TestingContextKeys.canRefreshTests.isEqualTo(true), TestingContextKeys.isRefreshingTests.isEqualTo(whenIsRefreshing)),
    },
    {
        id: MenuId.CommandPalette,
