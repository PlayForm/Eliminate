...
import { DefaultGutterClickAction, getTestingConfiguration, TestingConfigKeys, } from "../common/configuration.js";
...
import { getContextForTestItem, ITestService, simplifyTestsToExecute, testsInFile, } from "../common/testService.js";
import { IncrementalTestCollectionItem, InternalTestItem, ITestMessage, ITestRunProfile, TestDiffOpType, TestMessageType, TestResultItem, TestResultState, TestRunProfileBitset, } from "../common/testTypes.js";
...
export class TestingDecorationService extends Disposable implements ITestingDecorationsService {
    ...
    private readonly modelService: IModelService) {
        ...
        this._register(Event.any(this.results.onResultsChanged, this.results.onTestChanged, this.testService.excluded.onTestExclusionsChanged, this.testService.showInlineOutput.onDidChange, Event.filter(configurationService.onDidChangeConfiguration, (e) => e.affectsConfiguration(TestingConfigKeys.GutterEnabled)))(() => {
            if (!debounceInvalidate.isScheduled()) {
                debounceInvalidate.schedule();
            }
        }));
    ...
    public updateDecorationsAlternateAction(resource: URI, isAlt: boolean) {
        ...
        model.changeDecorations((accessor) => {
            for (const decoration of cached.value) {
                if (decoration instanceof RunTestDecoration &&
                    decoration.editorDecoration.alternate) {
                    accessor.changeDecorationOptions(decoration.id, isAlt
                        ? decoration.editorDecoration.alternate
                    ...
                    for (const kind of [
                        ...
                        for (let i = 0; i < state.messages.length; i++) {
                            ...
                            if (!messageLines.has(line)) {
                                const decoration = lastDecorations.getMessage(m) ||
                                    this.instantiationService.createInstance(TestMessageDecoration, m, buildTestUri({
...
export class TestingDecorations extends Disposable implements IEditorContribution {
    ...
    private readonly uriIdentityService: IUriIdentityService) {
        ...
        this._register(dom.addDisposableListener(win, "keydown", (e) => {
            if (new StandardKeyboardEvent(e).keyCode === KeyCode.Alt &&
                this._currentUri) {
        ...
        this._register(this.editor.onMouseDown((e) => {
            if (e.target.position && this.currentUri) {
                const modelDecorations = editor
                    .getModel()
                    ?.getLineDecorations(e.target.position.lineNumber) ?? [];
        ...
        const updateFontFamilyVar = () => {
            this.editor
                .getContainerDomNode()
                .style.setProperty("--testMessageDecorationFontFamily", editor.getOption(EditorOption.fontFamily));
            this.editor
                .getContainerDomNode()
                .style.setProperty("--testMessageDecorationFontSize", `${editor.getOption(EditorOption.fontSize)}px`);
        };
...
const createRunTestDecoration = (tests: readonly IncrementalTestCollectionItem[], states: readonly (TestResultItem | undefined)[], visible: boolean, defaultGutterAction: DefaultGutterClickAction): IModelDeltaDecoration & {
...
abstract class TitleLensContentWidget {
    ...
    private applyStyling() {
        ...
        if (!fontSize || fontSize < 5) {
            ...
            height =
                (fontSize *
                    Math.max(1.3, this.editor.getOption(EditorOption.lineHeight) /
...
abstract class RunTestDecoration {
    ...
    constructor(protected tests: readonly {
        test: IncrementalTestCollectionItem;
        resultItem: TestResultItem | undefined;
    }[], private visible: boolean, protected readonly model: ITextModel, 
    @ICodeEditorService
    private readonly codeEditorService: ICodeEditorService, 
    @ITestService
    ...
    protected readonly menuService: IMenuService) {
        ...
        this.editorDecoration = createRunTestDecoration(tests.map((t) => t.test), tests.map((t) => t.resultItem), visible, getTestingConfiguration(this.configurationService, TestingConfigKeys.DefaultGutterClickAction));
    ...
    public replaceOptions(newTests: readonly {
        ...
        const { options, alternate } = createRunTestDecoration(newTests.map((t) => t.test), newTests.map((t) => t.resultItem), visible, getTestingConfiguration(this.configurationService, TestingConfigKeys.DefaultGutterClickAction));
    ...
    private showContextMenu(e: IEditorMouseEvent) {
        ...
        editor
            ?.getContribution<EditorLineNumberContextMenu>(EditorLineNumberContextMenu.ID)
            ?.show(e);
    }
    ...
    protected getTestContextMenuActions(test: InternalTestItem, resultItem?: TestResultItem): IReference<IAction[]> {
        ...
        [
            ...
            if (capabilities & bitset) {
                testActions.push(new Action(`testing.gutter.${bitset}`, label, undefined, undefined, () => this.testService.runTests({
                    group: bitset,
                    tests: [test],
                })));
            }
...
class MultiRunTestDecoration extends RunTestDecoration implements ITestDecoration {
    constructor(tests: readonly {
        test: IncrementalTestCollectionItem;
        resultItem: TestResultItem | undefined;
    }[], visible: boolean, model: ITextModel, 
    @ICodeEditorService
    codeEditorService: ICodeEditorService, 
    @ITestService
    ...
    @ITestProfileService
    testProfileService: ITestProfileService, 
    @IContextKeyService
    ...
    public override getContextMenuActions() {
        ...
        [
            ...
            const canRun = this.tests.some(({ test }) => this.testProfileService.capabilitiesForTest(test.item) &
                bitset);
            if (canRun) {
...
class RunSingleTestDecoration extends RunTestDecoration implements ITestDecoration {
    constructor(test: IncrementalTestCollectionItem, resultItem: TestResultItem | undefined, model: ITextModel, visible: boolean, 
    @ICodeEditorService
    codeEditorService: ICodeEditorService, 
    @ITestService
    testService: ITestService, 
    ...
    configurationService: IConfigurationService, 
    @ITestProfileService
    testProfiles: ITestProfileService, 
    @IContextKeyService
    contextKeyService: IContextKeyService, 
    @IMenuService
    menuService: IMenuService) {
...
class TestMessageDecoration implements ITestDecoration {
    ...
    constructor(public readonly testMessage: ITestMessage, private readonly messageUri: URI | undefined, textModel: ITextModel, 
    @ITestingPeekOpener
