...
import { combinedDisposable, Disposable, DisposableStore, IDisposable, IReference, MutableDisposable, toDisposable, } from "../../../../../base/common/lifecycle.js";
...
import { getMarkId, ITestMessage, TestMessageType, } from "../../common/testTypes.js";
...
class SimpleDiffEditorModel extends EditorModel {
    ...
    constructor(private readonly _original: IReference<IResolvedTextEditorModel>, private readonly _modified: IReference<IResolvedTextEditorModel>) {
        super();
    }
...
export class DiffContentProvider extends Disposable implements IPeekOutputRenderer {
    ...
    constructor(private readonly editor: ICodeEditor | undefined, private readonly container: HTMLElement, 
...
export class MarkdownTestMessagePeek extends Disposable implements IPeekOutputRenderer {
    private readonly markdown = new Lazy(() => this._register(this.instantiationService.createInstance(MarkdownRenderer, {})));
...
class ScrollHelper {
    constructor(private readonly hasMultipleFrames: boolean, private readonly contentHeight: number, private readonly viewHeight: number) { }
    public onScrolled(evt: ScrollEvent, container: HTMLElement | undefined | null, editor: ICodeEditor | undefined) {
        if (!editor || !container) {
            return;
        }
        let delta = Math.max(0, evt.scrollTop -
...
export class TerminalMessagePeek extends Disposable implements IPeekOutputRenderer {
    ...
    constructor(private readonly container: HTMLElement, private readonly isInPeekView: boolean, 
    @ITerminalService
    ...
    private async updateForTestSubject(subject: TestOutputSubject | MessageSubject) {
        const that = this;
        const testItem = subject instanceof TestOutputSubject
            ? subject.test.item
            : subject.test;
        const terminal = await this.updateGenerically<ITaskRawOutput>({
            ...
            *doInitialWrite(output, results) {
                ...
                const state = subject instanceof TestOutputSubject
                    ? subject.test
    ...
    private layoutTerminal({ xterm }: IDetachedTerminalInstance, width = this.dimensions?.width ?? this.container.clientWidth, height = this.dimensions?.height ?? this.container.clientHeight) {
