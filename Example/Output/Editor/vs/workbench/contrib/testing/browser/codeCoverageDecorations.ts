...
export class CodeCoverageDecorations extends Disposable implements IEditorContribution {
    ...
    private readonly coverage: ITestCoverageService, 
    ...
    private hoverInlineDecoration(model: ITextModel, position: Position) {
        ...
        const decoration = mapFindFirst(allDecorations, ({ id }) => this.decorationIds.has(id)
...
export class CoverageDetailsModel {
        ...
        for (const { range, metadata: { detail }, } of detailRanges) {
            if (detail.type === DetailType.Statement && detail.branches) {
                for (let i = 0; i < detail.branches.length; i++) {
                    ...
                    detailRanges.push({
                        range: tidyLocation(detail.branches[i].location ||
    ...
    public describe(detail: CoverageDetailsWithBranch, model: ITextModel): IMarkdownString | undefined {
        if (detail.type === DetailType.Declaration) {
            ...
            const text = wrapName(model.getValueInRange(tidyLocation(detail.location)).trim() ||
                `<empty statement>`);
...
class CoverageToolbarWidget extends Disposable implements IOverlayWidget {
    ...
    private setActions() {
        ...
        if (current.testId) {
            ...
            this.actionBar.push(new ActionWithIcon("perTestFilter", coverUtils.labels.showingFilterFor(testItem.label), testingFilterIcon, undefined, () => this.commandService.executeCommand(TestCommandId.CoverageFilterToTestInEditor, this.current, this.editor)));
        }
        else if (current.coverage.perTestData?.size) {
            this.actionBar.push(new ActionWithIcon("perTestFilter", localize("testing.coverageForTestAvailable", "{0} test(s) ran code in this file", current.coverage.perTestData.size), testingFilterIcon, undefined, () => this.commandService.executeCommand(TestCommandId.CoverageFilterToTestInEditor, this.current, this.editor)));
        }
    ...
    private show() {
        ...
        ds.add(this.configurationService.onDidChangeConfiguration((e) => {
            if (this.current &&
                (e.affectsConfiguration(TestingConfigKeys.CoverageBarThresholds) ||
                    e.affectsConfiguration(TestingConfigKeys.CoveragePercent))) {
                ...
                if (!entry.testId) {
                    ...
                    coverage
                        ...
                        if (!cts.token
                            ...
                            first) {
                            activeEditor?.revealLineNearTop(first.location instanceof
                                Position
