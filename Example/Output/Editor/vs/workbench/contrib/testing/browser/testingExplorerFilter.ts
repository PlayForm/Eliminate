...
export class TestingExplorerFilter extends BaseActionViewItem {
    ...
    private readonly history: StoredValue<{
        values: string[];
        lastValue: string;
    } | string[]> = this._register(this.instantiationService.createInstance(StoredValue, {
...
class FiltersDropdownMenuActionViewItem extends DropdownMenuActionViewItem {
    constructor(action: IAction, options: IActionViewItemOptions, private readonly filters: ITestExplorerFilterState, actionRunner: IActionRunner, 
    @IContextMenuService
