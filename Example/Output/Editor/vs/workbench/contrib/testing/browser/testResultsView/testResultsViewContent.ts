...
class MessageStackFrame extends CustomStackFrame {
    ...
    constructor(private readonly message: HTMLElement, private readonly followup: FollowupActionWidget, private readonly subject: InspectSubject, 
...
function runInLast(accessor: ServicesAccessor, bitset: TestRunProfileBitset, subject: InspectSubject) {
    ...
    if (subject instanceof TaskSubject) {
        return accessor
            .get(ICommandService)
            .executeCommand(bitset === TestRunProfileBitset.Debug
            ? TestCommandId.DebugLastRun
            : TestCommandId.ReRunLastRun, subject.result.id);
    }
...
export class TestResultsViewContent extends Disposable {
    ...
    private readonly didReveal = this._register(new Emitter<{
    ...
    constructor(private readonly editor: ICodeEditor | undefined, private readonly options: {
    ...
    public fillBody(containerElement: HTMLElement): void {
        ...
        this.contextKeyTestMessage =
            TestingContextKeys.testMessageContext.bindTo(this.messageContextKeyService);
        this.contextKeyResultOutdated =
            TestingContextKeys.testResultOutdated.bindTo(this.messageContextKeyService);
        const treeContainer = dom.append(containerElement, dom.$(".test-output-peek-tree"));
        const tree = this._register(this.instantiationService.createInstance(OutputPeekTree, treeContainer, this.didReveal.event, {
    ...
    private async prepareTopFrame(subject: InspectSubject, callFrames: ITestMessageStackFrame[]) {
        ...
        const topFrame = (this.currentTopFrame =
            this.instantiationService.createInstance(MessageStackFrame, this.messageContainer, this.followupWidget, subject));
    ...
    private populateFloatingClick(subject: InspectSubject) {
        ...
        if (subject.result instanceof LiveTestResult) {
            this.contextKeyResultOutdated.set(subject.result.getStateById(subject.test.extId)?.retired ??
                false);
        ...
        const instaService = this.currentSubjectStore.add(this.instantiationService.createChild(new ServiceCollection([
            IContextKeyService,
...
class FollowupActionWidget extends Disposable {
    ...
    constructor(private readonly editor: ICodeEditor | undefined, 
    @ITestService
