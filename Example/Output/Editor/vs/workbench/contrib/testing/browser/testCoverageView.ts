...
import { BypassedFileCoverage, ComputedFileCoverage, FileCoverage, getTotalCoveragePercent, TestCoverage, } from "../common/testCoverage.js";
        ...
        this._register(autorun((reader) => {
            ...
            if (coverage) {
                const t = (this.tree.value ??=
                    this.instantiationService.createInstance(TestCoverageTree, container, labels, this.sortOrder));
...
class DeclarationCoverageNode {
    ...
    constructor(public readonly uri: URI, private readonly data: IDeclarationCoverage, details: readonly CoverageDetails[]) {
...
const shouldShowDeclDetailsOnExpand = (c: CoverageTreeElement): c is IPrefixTreeNode<FileCoverage> => isFileCoverage(c) &&
...
class TestCoverageTree extends Disposable {
    ...
    constructor(container: HTMLElement, labels: ResourceLabels, sortOrder: IObservable<CoverageSortOrder>, 
    @IInstantiationService
    instantiationService: IInstantiationService, 
    @IEditorService
    editorService: IEditorService) {
        super();
        this.tree = <WorkbenchCompressibleObjectTree<CoverageTreeElement, void>>instantiationService.createInstance(WorkbenchCompressibleObjectTree, "TestCoverageView", container, new TestCoverageTreeListDelegate(), [
    ...
    public setInput(coverage: TestCoverage, showOnlyTest?: TestId) {
        ...
        for (let node of tree.nodes) {
            ...
            while (!(node.value instanceof FileCoverage) &&
...
class TestCoverageTreeListDelegate implements IListVirtualDelegate<CoverageTreeElement> {
    ...
    getTemplateId(element: CoverageTreeElement): string {
        ...
        if (element instanceof LoadingDetails ||
...
class Sorter implements ITreeSorter<CoverageTreeElement> {
    ...
    compare(a: CoverageTreeElement, b: CoverageTreeElement): number {
        ...
        if (isFileCoverage(a) && isFileCoverage(b)) {
            ...
            switch (order) {
                case CoverageSortOrder.Location:
                    return Position.compare(a.location instanceof Range
                        ? a.location.getStartPosition()
                        : a.location, b.location instanceof Range
...
registerAction2(class TestCoverageChangePerTestFilterAction extends Action2 {
    ...
    override run(accessor: ServicesAccessor): void {
        ...
        quickInputService
            .pick(items, {
            activeItem: items.find((item): item is TItem => "testId" in item &&
