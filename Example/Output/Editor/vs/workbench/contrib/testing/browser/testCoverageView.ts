...
const shouldShowDeclDetailsOnExpand = (c: CoverageTreeElement): c is IPrefixTreeNode<FileCoverage> => isFileCoverage(c) && c.value instanceof FileCoverage && !!c.value.declaration?.total;
class TestCoverageTree extends Disposable {
    ...
    public setInput(coverage: TestCoverage, showOnlyTest?: TestId) {
        ...
        this.inputDisposables.add(onObservableChange(coverage.didAddCoverage, nodes => {
            ...
            if (toRender) {
                this.tree.setChildren(toRender, Iterable.map(toRender.children?.values() || [], toChild), { diffIdentityProvider: { getId: el => (el as TestCoverageFileNode).value!.id } });
            }
        }));
