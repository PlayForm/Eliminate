...
const isRunningTests = (service: ITestResultService) => service.results.length > 0 && service.results[0].completedAt === undefined;
...
export class TestResultService extends Disposable implements ITestResultService {
    ...
    @ITestResultStorage
    private readonly storage: ITestResultStorage, 
    @ITestProfileService
     ...
     * @inheritdoc
     */
    public getStateById(extId: string): [
        results: ITestResult,
        item: TestResultItem
    ] | undefined {
    ...
    public push<T extends ITestResult>(result: T): T {
        if (result.completedAt === undefined) {
            ...
            const index = findFirstIdxMonotonousOrArrLen(this.results, (r) => r.completedAt !== undefined &&
    ...
    private resort() {
        this.results.sort((a, b) => (b.completedAt ?? Number.MAX_SAFE_INTEGER) -
