...
import { addDisposableListener, computeScreenAwareSize, EventType, getActiveDocument, getActiveElement, getActiveWindow, getClientArea, getWindow, getWindowId, getWindows, IDimension, isActiveDocument, isAncestorUsingFlowTo, position, size, } from "../../base/browser/dom.js";
...
import { CustomTitleBarVisibility, getMenuBarVisibility, hasCustomTitlebar, hasNativeTitlebar, IPath, TitleBarSetting, useWindowControlsOverlay, } from "../../platform/window/common/window.js";
...
import { ActivityBarPosition, EditorActionsLocation, EditorTabsMode, isHorizontal, IWorkbenchLayoutService, LayoutSettings, MULTI_WINDOW_PARTS, PanelAlignment, panelOpensMaximizedFromString, PanelOpensMaximizedOptions, Parts, Position, positionFromString, positionToString, shouldShowCustomTitleBar, SINGLE_WINDOW_PARTS, ZenModeSettings, } from "../services/layout/browser/layoutService.js";
...
export abstract class Layout extends Disposable implements IWorkbenchLayoutService {
    ...
    private readonly _onDidChangeWindowMaximized = this._register(new Emitter<{
        windowId: number;
        maximized: boolean;
    }>());
    ...
    private readonly _onDidLayoutContainer = this._register(new Emitter<{
        container: HTMLElement;
        dimension: IDimension;
    }>());
    readonly onDidLayoutContainer = this._onDidLayoutContainer.event;
    private readonly _onDidAddContainer = this._register(new Emitter<{
        container: HTMLElement;
    ...
    private computeContainerOffset(targetWindow: Window) {
        ...
        const isCommandCenterVisible = titlebarVisible &&
            this.configurationService.getValue<boolean>(LayoutSettings.COMMAND_CENTER) !== false;
    ...
    private registerLayoutListeners(): void {
        ...
        this._register(this.configurationService.onDidChangeConfiguration((e) => {
            if ([
                ...
                const editorActionsMovedToTitlebar = e.affectsConfiguration(LayoutSettings.EDITOR_ACTIONS_LOCATION) &&
                    this.configurationService.getValue<EditorActionsLocation>(LayoutSettings.EDITOR_ACTIONS_LOCATION) === EditorActionsLocation.TITLEBAR;
        ...
        const showingCustomMenu = (isWindows || isLinux || isWeb) &&
    ...
    private onFullscreenChanged(windowId: number): void {
        ...
        if (this.state.runtime.mainWindowFullscreen) {
            ...
            if (zenModeExitInfo.transitionedToFullScreen &&
            ...
            for (let i = 0; i < resolvedFilesToOpenOrCreate.length; i++) {
                ...
                if (resolvedFileToOpenOrCreate) {
                    filesToOpenOrCreate.push({
                        ...
                        viewColumn: initialEditorsState.filesToOpenOrCreate?.[i]
    ...
    private getInitialEditorsState(): IInitialEditorsState | undefined {
        ...
        if ((defaultLayout?.editors?.length ||
    ...
    protected restoreParts(): void {
        ...
        layoutReadyPromises.push((async () => {
            ...
            const viewlet = await this.paneCompositeService.openPaneComposite(this.state.initialization.views.containerToRestore
        ...
        layoutReadyPromises.push((async () => {
            ...
            // restored, as such we need to test again
            await restoreDefaultViewsPromise;
            if (!this.state.initialization.views.containerToRestore
                .auxiliaryBar) {
                return;
            }
            mark("code/willRestoreAuxiliaryBar");
            const panel = await this.paneCompositeService.openPaneComposite(this.state.initialization.views.containerToRestore
    ...
    private focusPanelOrEditor(): void {
        ...
        if ((this.hasFocus(Parts.PANEL_PART) ||
    ...
    getMaximumEditorDimensions(container: HTMLElement): IDimension {
        ...
        if (container === this.mainContainer) {
            ...
            const takenWidth = (this.isVisible(Parts.ACTIVITYBAR_PART)
                ...
                (this.isVisible(Parts.AUXILIARYBAR_PART)
                    ...
                    : 0);
            const takenHeight = (this.isVisible(Parts.TITLEBAR_PART, targetWindow)
    ...
    toggleZenMode(skipLayout?: boolean, restoring = false): void {
        ...
        if (this.isZenModeActive()) {
            ...
            if (config.showTabs !== this.editorGroupService.partOptions.showTabs) {
                this.state.runtime.zenMode.transitionDisposables.set(ZenModeSettings.SHOW_TABS, this.editorGroupService.mainPart.enforcePartOptions({
            ...
            this.state.runtime.zenMode.transitionDisposables.set("configurationChange", this.configurationService.onDidChangeConfiguration((e) => {
                ...
                if (e.affectsConfiguration(ZenModeSettings.SILENT_NOTIFICATIONS)) {
                    ...
                    if (zenModeExitInfo.handleNotificationsDoNotDisturbMode) {
                        this.notificationService.setFilter(zenModeSilentNotifications
    ...
    private setSideBarHidden(hidden: boolean, skipLayout?: boolean): void {
        ...
        if (hidden &&
            this.paneCompositeService.getActivePaneComposite(ViewContainerLocation.Sidebar)) {
    ...
    private adjustPartPositions(sideBarPosition: Position, panelAlignment: PanelAlignment, panelPosition: Position): void {
        ...
        const preMovePanelWidth = !this.isVisible(Parts.PANEL_PART)
            ? Sizing.Invisible(this.workbenchGrid.getViewCachedVisibleSize(this.panelPartView) ?? this.panelPartView.minimumWidth)
            : this.workbenchGrid.getViewSize(this.panelPartView).width;
        const preMovePanelHeight = !this.isVisible(Parts.PANEL_PART)
            ? Sizing.Invisible(this.workbenchGrid.getViewCachedVisibleSize(this.panelPartView) ?? this.panelPartView.minimumHeight)
            : this.workbenchGrid.getViewSize(this.panelPartView).height;
        const preMoveSideBarSize = !this.isVisible(Parts.SIDEBAR_PART)
            ? Sizing.Invisible(this.workbenchGrid.getViewCachedVisibleSize(this.sideBarPartView) ?? this.sideBarPartView.minimumWidth)
        ...
        const preMoveAuxiliaryBarSize = !this.isVisible(Parts.AUXILIARYBAR_PART)
            ? Sizing.Invisible(this.workbenchGrid.getViewCachedVisibleSize(this.auxiliaryBarPartView) ?? this.auxiliaryBarPartView.minimumWidth)
        ...
        if (sideBarPosition === Position.LEFT) {
            ...
            this.workbenchGrid.moveView(this.sideBarPartView, preMoveSideBarSize, sideBarSiblingToEditor
                ? this.editorPartView
                : this.activityBarPartView, sideBarSiblingToEditor ? Direction.Left : Direction.Right);
            ...
            this.workbenchGrid.moveView(this.sideBarPartView, preMoveSideBarSize, sideBarSiblingToEditor
                ? this.editorPartView
                : this.activityBarPartView, sideBarSiblingToEditor ? Direction.Right : Direction.Left);
            if (auxiliaryBarSiblingToEditor) {
        ...
        if (isPanelVertical) {
            this.workbenchGrid.moveView(this.panelPartView, preMovePanelWidth, this.editorPartView, panelPosition === Position.LEFT
                ? Direction.Left
                : Direction.Right);
            this.workbenchGrid.resizeView(this.panelPartView, {
    ...
    private setPanelHidden(hidden: boolean, skipLayout?: boolean): void {
        ...
        if (hidden &&
            this.paneCompositeService.getActivePaneComposite(ViewContainerLocation.Panel)) {
    ...
    private panelOpensMaximized(): boolean {
        ...
        return (panelOpensMaximized === PanelOpensMaximizedOptions.ALWAYS ||
    ...
    private setAuxiliaryBarHidden(hidden: boolean, skipLayout?: boolean): void {
        ...
        if (hidden &&
            this.paneCompositeService.getActivePaneComposite(ViewContainerLocation.AuxiliaryBar)) {
    ...
    isPanelMaximized(): boolean {
        // the workbench grid currently prevents us from supporting panel maximization with non-center panel alignment
        return ((this.getPanelAlignment() === "center" ||
            !isHorizontal(this.getPanelPosition())) &&
            !this.isVisible(Parts.EDITOR_PART, mainWindow));
    }
    ...
    setPanelPosition(position: Position): void {
        ...
        if (position === Position.BOTTOM) {
            this.workbenchGrid.moveView(this.panelPartView, editorHidden
                ? size.height
                : this.stateModel.getRuntimeValue(LayoutStateKeys.PANEL_LAST_NON_MAXIMIZED_HEIGHT), this.editorPartView, Direction.Down);
        }
        else if (position === Position.TOP) {
            this.workbenchGrid.moveView(this.panelPartView, editorHidden
                ? size.height
                : this.stateModel.getRuntimeValue(LayoutStateKeys.PANEL_LAST_NON_MAXIMIZED_HEIGHT), this.editorPartView, Direction.Up);
        }
        else if (position === Position.RIGHT) {
            this.workbenchGrid.moveView(this.panelPartView, editorHidden
                ? size.width
                : this.stateModel.getRuntimeValue(LayoutStateKeys.PANEL_LAST_NON_MAXIMIZED_WIDTH), this.editorPartView, Direction.Right);
        }
        else {
            this.workbenchGrid.moveView(this.panelPartView, editorHidden
                ? size.width
                : this.stateModel.getRuntimeValue(LayoutStateKeys.PANEL_LAST_NON_MAXIMIZED_WIDTH), this.editorPartView, Direction.Left);
        }
    ...
    getVisibleNeighborPart(part: Parts, direction: Direction): Parts | undefined {
        ...
        for (const neighborView of neighborViews) {
            const neighborPart = [
                ...
                Parts.TITLEBAR_PART,
            ].find((partId) => this.getPart(partId) === neighborView &&
    ...
    private arrangeEditorNodes(nodes: {
        editor: ISerializedNode;
        sideBar?: ISerializedNode;
        auxiliaryBar?: ISerializedNode;
    }, availableHeight: number, availableWidth: number): ISerializedNode {
    ...
    private arrangeMiddleSectionNodes(nodes: {
        ...
        if (!isHorizontal(panelPostion)) {
            ...
            result.push({
                type: "branch",
                data: panelPostion === Position.BOTTOM
...
class LayoutStateModel extends Disposable {
    ...
    constructor(private readonly storageService: IStorageService, private readonly configurationService: IConfigurationService, private readonly contextService: IWorkspaceContextService, private readonly container: HTMLElement) {
        super();
        ...
        this._register(this.storageService.onDidChangeValue(StorageScope.PROFILE, undefined, this._store)((storageChangeEvent) => {
            ...
            for (key in LayoutStateKeys) {
                ...
                if (stateKey instanceof RuntimeStateKey &&
                    ...
                    if (`${LayoutStateModel.STORAGE_PREFIX}${stateKey.name}` ===
