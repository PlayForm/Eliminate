...
import { IActivity, IBadge, IconBadge, NumberBadge, ProgressBadge, } from "../../services/activity/common/activity.js";
import { Before2D, CompositeDragAndDropObserver, ICompositeDragAndDrop, toggleDropEffect, } from "../dnd.js";
...
export class CompositeBarActionViewItem extends BaseActionViewItem {
    ...
    constructor(action: CompositeBarAction, options: ICompositeBarActionViewItemOptions, private readonly badgesEnabled: (compositeId: string) => boolean, 
    @IThemeService
    ...
    protected updateStyles(): void {
        ...
        if (this.label) {
            ...
            this.container.style.setProperty("--insert-border-color", colors.dragAndDropBorder
        ...
        if (this.badgeContent) {
            ...
            const badgeFg = badgeStyles?.badgeForeground ??
                colors.badgeForeground ??
                theme.getColor(badgeForeground);
            const badgeBg = badgeStyles?.badgeBackground ??
...
export class CompositeOverflowActivityActionViewItem extends CompositeBarActionViewItem {
    constructor(action: CompositeBarAction, private getOverflowingComposites: () => {
        id: string;
        name?: string;
    }[], private getActiveCompositeId: () => string | undefined, private getBadge: (compositeId: string) => IBadge, private getCompositeOpenAction: (compositeId: string) => IAction, colors: (theme: IColorTheme) => ICompositeBarColors, hoverOptions: IActivityHoverOptions, 
    @IContextMenuService
...
export class CompositeActionViewItem extends CompositeBarActionViewItem {
    ...
    configurationService: IConfigurationService) {
        ...
        if (!CompositeActionViewItem.manageExtensionAction) {
            CompositeActionViewItem.manageExtensionAction =
                instantiationService.createInstance(ManageExtensionAction);
        }
    }
    override render(container: HTMLElement): void {
        super.render(container);
        ...
        this._register(CompositeDragAndDropObserver.INSTANCE.registerDraggable(this.container, () => {
            ...
            onDragOver: (e) => {
                const isValidMove = e.dragAndDropData.getData().id !==
            ...
            onDragStart: (e) => {
                if (e.dragAndDropData.getData().id !==
                    this.compositeBarActionItem.id) {
                    return;
                }
    ...
    private showContextMenu(container: HTMLElement): void {
        ...
        if ((<any>this.compositeActivityAction.compositeBarActionItem)
            .extensionId) {
...
export class ToggleCompositePinnedAction extends Action {
    constructor(private activity: ICompositeBarActionItem | undefined, private compositeBar: ICompositeBar) {
...
export class ToggleCompositeBadgeAction extends Action {
    constructor(private compositeBarActionItem: ICompositeBarActionItem | undefined, private compositeBar: ICompositeBar) {
