...
import { INotificationChangeEvent, INotificationsModel, IStatusMessageChangeEvent, IStatusMessageViewItem, NotificationChangeType, StatusMessageChangeType, } from "../../../common/notifications.js";
...
export class NotificationsStatus extends Disposable {
    ...
    private updateNotificationsCenterStatusItem(): void {
        ...
        if (!this.notificationsCenterStatusItem) {
            this.notificationsCenterStatusItem = this.statusbarService.addEntry(statusProperties, "status.notifications", StatusbarAlignment.RIGHT, -Number.MAX_VALUE /* towards the far end of the right hand side */);
        }
        else {
    ...
    private getTooltip(notificationsInProgress: number): string {
        ...
        if (notificationsInProgress === 0) {
            ...
            }, "{0} New Notifications", this.newNotificationsCount);
        }
        if (this.newNotificationsCount === 0) {
            return localize({
                key: "noNotificationsWithProgress",
                comment: ["{0} will be replaced by a number"],
            }, "No New Notifications ({0} in progress)", notificationsInProgress);
        }
        if (this.newNotificationsCount === 1) {
            return localize({
                key: "oneNotificationWithProgress",
                comment: ["{0} will be replaced by a number"],
            }, "1 New Notification ({0} in progress)", notificationsInProgress);
        }
        return localize({
            key: "notificationsWithProgress",
            comment: ["{0} and {1} will be replaced by a number"],
        }, "{0} New Notifications ({1} in progress)", this.newNotificationsCount, notificationsInProgress);
    }
    ...
    private doSetStatusMessage(item: IStatusMessageViewItem): void {
        ...
        const showAfter = item.options && typeof item.options.showAfter === "number"
            ? item.options.showAfter
            : 0;
        const hideAfter = item.options && typeof item.options.hideAfter === "number"
        ...
        let showHandle: any = setTimeout(() => {
            statusMessageEntry = this.statusbarService.addEntry({
                ...
                text: message,
                ariaLabel: message,
            }, "status.message", StatusbarAlignment.LEFT, -Number.MAX_VALUE /* far right on left hand side */);
            showHandle = null;
        }, showAfter);
