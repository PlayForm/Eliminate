 ...
 *--------------------------------------------------------------------------------------------*/
import { hide, isAncestorOfActiveElement, show, } from "../../../../base/browser/dom.js";
...
export class StatusbarViewModel extends Disposable {
    ...
    private readonly _onDidChangeEntryVisibility = this._register(new Emitter<{
        id: string;
        visible: boolean;
    }>());
    ...
    private sort(): void {
        ...
        sortedEntriesWithNumberedPriority.sort((entryA, entryB) => {
            if (entryA.alignment === entryB.alignment) {
                ...
                if (entryA.priority.primary !== entryB.priority.primary) {
                    return (Number(entryB.priority.primary) -
        ...
        if (mapEntryWithRelativePriority.size > 0) {
            ...
            for (const entry of sortedEntriesWithNumberedPriority) {
                ...
                if (relativeEntries) {
                    sortedEntries.push(...relativeEntries.filter((entry) => isStatusbarEntryLocation(entry.priority.primary) &&
