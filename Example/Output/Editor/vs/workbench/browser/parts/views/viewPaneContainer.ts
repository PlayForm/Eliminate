...
class ViewPaneDropOverlay extends Themable {
    ...
    constructor(private paneElement: HTMLElement, private orientation: Orientation | undefined, private bounds: BoundingRect | undefined, protected location: ViewContainerLocation, themeService: IThemeService) {
        super(themeService);
    ...
    override updateStyles(): void {
        ...
        this.overlay.style.backgroundColor =
            this.getColor(this.location === ViewContainerLocation.Panel
...
class ViewContainerMenuActions extends CompositeMenuActions {
    constructor(element: HTMLElement, viewContainer: ViewContainer, 
...
export class ViewPaneContainer extends Component implements IViewPaneContainer {
    ...
    constructor(id: string, private options: IViewPaneContainerOptions, 
    ...
    create(parent: HTMLElement): void {
        ...
        const getOverlayBounds: () => BoundingRect = () => {
            ...
            const top = this.orientation === Orientation.VERTICAL
                ? lastPane.bottom
                : fullSize.top;
            const left = this.orientation === Orientation.HORIZONTAL
                ? lastPane.right
                : fullSize.left;
            return {
                top,
        ...
        const inBounds = (bounds: BoundingRect, pos: {
            x: number;
            y: number;
        }) => {
    ...
    private saveViewSizes(): void {
        ...
        if (this.didLayout) {
            this.viewContainerModel.setSizes(this.panes.map((view) => ({
                id: view.id,
    ...
    private computeInitialSizes(): Map<string, number> {
        ...
        if (this.dimension) {
            ...
            for (const viewDescriptor of this.viewContainerModel
                ...
                if (this.orientation === Orientation.VERTICAL) {
                    sizes.set(viewDescriptor.id, (this.dimension.height *
                        (viewDescriptor.weight || 20)) /
                        totalWeight);
                }
                else {
                    sizes.set(viewDescriptor.id, (this.dimension.width * (viewDescriptor.weight || 20)) /
                        totalWeight);
                }
            }
        }
        ...
        store.add(CompositeDragAndDropObserver.INSTANCE.registerTarget(pane.dropTargetElement, {
            onDragEnter: (e) => {
                if (!overlay) {
                    ...
                    if (dropData.type === "composite" &&
                        ...
                        if (!viewsToMove.some((v) => !v.canMoveView) &&
                            viewsToMove.length > 0) {
                            overlay = new ViewPaneDropOverlay(pane.dropTargetElement, this.orientation ??
                                Orientation.VERTICAL, undefined, this.viewDescriptorService.getViewContainerLocation(this.viewContainer)!, this.themeService);
                        }
                    }
                }
            },
            ...
            onDrop: (e) => {
                if (overlay) {
                    ...
                    if (anchorView) {
                        if (overlay.currentDropOperation ===
                            DropDirection.DOWN ||
    ...
    isViewMergedWithContainer(): boolean {
        if (!(this.options.mergeViewWithContainerWhenSingleView &&
    ...
    private onDidSashReset(index: number) {
        ...
        for (let i = index; i >= 0; i--) {
            if (this.paneItems[i].pane?.isVisible() &&
...
registerAction2(class MoveViews extends Action2 {
    ...
    async run(accessor: ServicesAccessor, options: {
        ...
        if (!Array.isArray(options?.viewIds) ||
