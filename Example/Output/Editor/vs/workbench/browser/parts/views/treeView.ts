...
export class TreeViewPane extends ViewPane {
    ...
    override shouldShowWelcome(): boolean {
        return ((this.treeView.dataProvider === undefined ||
...
abstract class AbstractTreeView extends Disposable implements ITreeView {
    ...
    protected createTree() {
        ...
        this.tree = this.treeDisposables.add(this.instantiationService.createInstance(Tree, this.id, this.treeContainer!, new TreeViewDelegate(), [renderer], dataSource, {
            ...
            expandOnlyOnTwistieClick: (e: ITreeItem) => {
                return (!!e.command ||
                    !!e.checkbox ||
                    this.configurationService.getValue<"singleClick" | "doubleClick">("workbench.tree.expandMode") === "doubleClick");
            },
            collapseByDefault: (e: ITreeItem): boolean => {
                return (e.collapsibleState !==
        ...
        this.treeDisposables.add(this.tree.onDidOpen(async (e) => {
            ...
            if (e.browserEvent.target &&
                (e.browserEvent.target as HTMLElement).classList.contains(TreeItemCheckbox.checkboxClass)) {
                return;
            }
        ...
        for (const line of lines) {
            ...
            if (linkedText.nodes.length === 1 &&
                ...
                if (href.scheme === Schemas.command) {
                    ...
                    if (preConditions) {
                        button.enabled =
                            this.contextKeyService.contextMatchesRules(preConditions);
    ...
    private showMessage(message: string | IMarkdownString): void {
        ...
        if (typeof this._messageValue === "string" &&
    ...
    private updateCollapseAllToggle() {
        if (this.showCollapseAllAction) {
            ...
            this.collapseAllToggleContext?.set(!!this.root.children &&
                ...
                this.root.children.some((value) => value.collapsibleState !==
...
class TreeDataSource implements IAsyncDataSource<ITreeItem, ITreeItem> {
    constructor(private treeView: ITreeView, private withProgress: <T>(task: Promise<T>) => Promise<T>) { }
    hasChildren(element: ITreeItem): boolean {
        return (!!this.treeView.dataProvider &&
    ...
    async getChildren(element: ITreeItem): Promise<ITreeItem[]> {
        ...
        if (this.treeView.dataProvider) {
            try {
                result =
                    (await this.withProgress(this.treeView.dataProvider.getChildren(element))) ?? [];
            }
            catch (e) {
...
class TreeRenderer extends Disposable implements ITreeRenderer<ITreeItem, FuzzyScore, ITreeExplorerTemplateData> {
    ...
    private _renderedElements = new Map<string, {
        original: ITreeNode<ITreeItem, FuzzyScore>;
        rendered: ITreeExplorerTemplateData;
    }[]>(); // tree item handle to template data
    constructor(private treeViewId: string, private menus: TreeMenus, private labels: ResourceLabels, private actionViewItemProvider: IActionViewItemProvider, private aligner: Aligner, private checkboxStateHandler: CheckboxStateHandler, private readonly manuallyManageCheckboxes: () => boolean, 
    @IThemeService
    private readonly themeService: IThemeService, 
    @IConfigurationService
    private readonly configurationService: IConfigurationService, 
    ...
    renderElement(element: ITreeNode<ITreeItem, FuzzyScore>, index: number, templateData: ITreeExplorerTemplateData): void {
        ...
        const matches = treeItemLabel && treeItemLabel.highlights && label
            ...
            : undefined;
        const icon = this.themeService.getColorTheme().type === ColorScheme.LIGHT
            ? node.icon
            : node.iconDark;
        ...
        if (this._tree) {
            ...
            if (this.hasIcon(parent)) {
                return (!!parent.children &&
                    parent.children.some((c) => c.collapsibleState !==
                        TreeItemCollapsibleState.None &&
                        !this.hasIcon(c)));
            }
            return (!!parent.children &&
                parent.children.every((c) => c.collapsibleState === TreeItemCollapsibleState.None ||
                    !this.hasIcon(c)));
        }
        else {
            return false;
...
class TreeMenus implements IDisposable {
    ...
    private getActions(menuId: MenuId, elements: ITreeItem[]): {
        primary: IAction[];
        secondary: IAction[];
    } {
...
export class CustomTreeView extends AbstractTreeView {
    constructor(id: string, title: string, private readonly extensionId: string, 
    @IThemeService
    themeService: IThemeService, 
    @IInstantiationService
    instantiationService: IInstantiationService, 
...
export class CustomTreeViewDragAndDrop implements ITreeDragAndDrop<ITreeItem> {
    ...
    constructor(private readonly treeId: string, 
    @ILabelService
    private readonly labelService: ILabelService, 
    @IInstantiationService
    private readonly instantiationService: IInstantiationService, 
    ...
    async drop(data: IDragAndDropData, targetNode: ITreeItem | undefined, targetIndex: number | undefined, targetSector: ListViewTargetSector | undefined, originalEvent: DragEvent): Promise<void> {
        ...
        for (const [type, item] of originalDataTransfer) {
            if (type === this.treeMimeType ||
                ...
                (item.asFile() &&
                    dndController.dropMimeTypes.includes(DataTransfers.FILES.toLowerCase()))) {
