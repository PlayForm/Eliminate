...
import { $, addDisposableListener, append, clearNode, EventType, isAncestor, } from "../../../../base/browser/dom.js";
...
export class ActivitybarPart extends Part {
    ...
    constructor(private readonly paneCompositePart: IPaneCompositePart, 
    ...
    private createCompositeBar(): PaneCompositeBar {
        return this.instantiationService.createInstance(ActivityBarCompositeBar, {
            ...
            activityHoverOptions: {
                position: () => this.layoutService.getSideBarPosition() ===
                    Position.LEFT
                    ? HoverPosition.RIGHT
                    : HoverPosition.LEFT,
            },
    ...
    override updateStyles(): void {
        ...
        const borderColor = this.getColor(ACTIVITY_BAR_BORDER) ||
...
export class ActivityBarCompositeBar extends PaneCompositeBar {
    ...
    constructor(options: IPaneCompositeBarOptions, part: Parts, paneCompositePart: IPaneCompositePart, showGlobalActivities: boolean, 
    @IInstantiationService
    ...
    layoutService: IWorkbenchLayoutService) {
        ...
        this._register(this.configurationService.onDidChangeConfiguration((e) => {
            if (e.affectsConfiguration("window.menuBarVisibility")) {
                if (getMenuBarVisibility(this.configurationService) ===
                    "compact") {
                    this.installMenubar();
                }
                else {
        ...
        if (menuBarVisibility === "compact" ||
            ...
            actions.unshift(...[
                toAction({
                    ...
                    run: () => this.configurationService.updateValue("window.menuBarVisibility", menuBarVisibility === "compact"
                        ? "toggle"
                        : "compact"),
                }),
                new Separator(),
    ...
    async run(accessor: ServicesAccessor): Promise<void> {
        ...
        for (let i = 0; i < visibleViewletIds.length; i++) {
            if (visibleViewletIds[i] === activeViewlet.getId()) {
                targetViewletId =
                    visibleViewletIds[(i + visibleViewletIds.length + this.offset) %
