...
import { EditorGroupLayout, GroupDirection, GroupLocation, GroupOrientation, GroupsArrangement, GroupsOrder, IAuxiliaryEditorPart, IEditorDropTargetDelegate, IEditorGroupContextKeyProvider, IEditorGroupsService, IEditorPart, IEditorSideGroup, IEditorWorkingSet, IEditorWorkingSetOptions, IFindGroupScope, IMergeGroupOptions, } from "../../../services/editor/common/editorGroupsService.js";
...
export class EditorParts extends MultiWindowParts<EditorPart> implements IEditorGroupsService, IEditorPartsView {
    ...
    getScopedInstantiationService(part: IEditorPart): IInstantiationService {
        ...
        return (this.mapPartToInstantiationService.get(part.windowId) ??
    ...
    async applyWorkingSet(workingSet: IEditorWorkingSet | "empty", options?: IEditorWorkingSetOptions): Promise<boolean> {
        ...
        if (workingSet === "empty") {
            ...
            workingSetState =
                this.editorWorkingSets[this.indexOfWorkingSet(workingSet) ?? -1];
        }
        if (!workingSetState) {
            return false;
        }
        ...
        const applied = await this.applyState(workingSetState === "empty"
            ? workingSetState
            : workingSetState.auxiliary);
        if (!applied) {
            return false;
        }
        await this.mainPart.applyState(workingSetState === "empty"
            ? workingSetState
            : workingSetState.main, options);
    ...
    findGroup(scope: IFindGroupScope, source: IEditorGroupView | GroupIdentifier = this.activeGroup, wrap?: boolean): IEditorGroupView | undefined {
        ...
        if (this._parts.size > 1) {
            ...
            if (scope.location === GroupLocation.FIRST ||
