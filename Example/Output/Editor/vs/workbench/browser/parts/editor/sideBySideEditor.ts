...
import { $, clearNode, Dimension, multibyteAwareBtoa, } from "../../../../base/browser/dom.js";
...
import { EditorExtensions, EditorPaneSelectionCompareResult, IEditorControl, IEditorOpenContext, IEditorPane, IEditorPaneSelection, IEditorPaneSelectionChangeEvent, IEditorPaneWithSelection, isEditorPaneWithSelection, SideBySideEditor as Side, SIDE_BY_SIDE_EDITOR_ID, } from "../../../common/editor.js";
...
function isSideBySideEditorViewState(thing: unknown): thing is ISideBySideEditorViewState {
    ...
    return (typeof candidate?.primary === "object" &&
...
export class SideBySideEditor extends AbstractEditorWithViewState<ISideBySideEditorViewState> implements IEditorPaneWithSelection {
    ...
    //#endregion
    //#region Events
    private onDidCreateEditors = this._register(new Emitter<{
        width: number;
        height: number;
    } | undefined>());
    private _onDidChangeSizeConstraints = this._register(new Relay<{
        width: number;
        height: number;
    } | undefined>());
    ...
    private orientation = this.configurationService.getValue<"vertical" | "horizontal">(SideBySideEditor.SIDE_BY_SIDE_LAYOUT_SETTING) === "vertical"
    ...
    private onConfigurationUpdated(event: IConfigurationChangeEvent): void {
        if (event.affectsConfiguration(SideBySideEditor.SIDE_BY_SIDE_LAYOUT_SETTING)) {
            this.orientation =
                this.configurationService.getValue<"vertical" | "horizontal">(SideBySideEditor.SIDE_BY_SIDE_LAYOUT_SETTING) === "vertical"
                    ? Orientation.VERTICAL
                    : Orientation.HORIZONTAL;
    ...
    private createSplitView(parent: HTMLElement, ratio?: number): void {
        ...
        if (ratio) {
            ...
            this.splitview.layout(this.orientation === Orientation.HORIZONTAL
                ? this.dimension.width
                : this.dimension.height);
        }
        // Secondary (left)
        ...
        this.splitview.addView({
            ...
            minimumSize: this.orientation === Orientation.HORIZONTAL
    ...
    private loadViewState(input: SideBySideEditorInput, options: ISideBySideEditorOptions | undefined, context: IEditorOpenContext): {
    ...
    getSelection(): IEditorPaneSelection | undefined {
        ...
        if (isEditorPaneWithSelection(lastFocusedEditorPane)) {
            ...
            if (selection) {
                return new SideBySideAwareEditorPaneSelection(selection, lastFocusedEditorPane === this.primaryEditorPane
                    ? Side.PRIMARY
                    : Side.SECONDARY);
            }
        }
    ...
    layout(dimension: Dimension): void {
        ...
        splitview.layout(this.orientation === Orientation.HORIZONTAL
            ? dimension.width
            : dimension.height);
    }
    ...
    private layoutPane(pane: EditorPane | undefined, size: number): void {
        pane?.layout(this.orientation === Orientation.HORIZONTAL
...
class SideBySideAwareEditorPaneSelection implements IEditorPaneSelection {
    constructor(private readonly selection: IEditorPaneSelection, private readonly side: Side.PRIMARY | Side.SECONDARY) { }
