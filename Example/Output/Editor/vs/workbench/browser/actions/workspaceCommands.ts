...
CommandsRegistry.registerCommand({
    ...
    handler: (accessor: ServicesAccessor) => accessor
        .get(IFileDialogService)
        .pickFileFolderAndOpen({ forceNewWindow: true }),
});
CommandsRegistry.registerCommand({
    id: "_files.pickFolderAndOpen",
    handler: (accessor: ServicesAccessor, options: {
...
CommandsRegistry.registerCommand({
    ...
    handler: (accessor: ServicesAccessor) => accessor
        .get(IFileDialogService)
        .pickFolderAndOpen({ forceNewWindow: true }),
});
CommandsRegistry.registerCommand({
    id: "workbench.action.files.openFileInNewWindow",
    handler: (accessor: ServicesAccessor) => accessor
        .get(IFileDialogService)
        .pickFileAndOpen({ forceNewWindow: true }),
});
CommandsRegistry.registerCommand({
    id: "workbench.action.openWorkspaceInNewWindow",
    handler: (accessor: ServicesAccessor) => accessor
        .get(IFileDialogService)
        .pickWorkspaceAndOpen({ forceNewWindow: true }),
});
CommandsRegistry.registerCommand({
...
CommandsRegistry.registerCommand({
    ...
    handler: (accessor: ServicesAccessor, uriComponents?: UriComponents, arg?: boolean | IOpenFolderAPICommandOptions) => {
        ...
        const uriToOpen: IWindowOpenable = hasWorkspaceFileExtension(uri) || uri.scheme === Schemas.untitled
            ? { workspaceUri: uri }
            : { folderUri: uri };
        ...
        args: [
            {
                ...
                constraint: (value: any) => value === undefined ||
                    value === null ||
