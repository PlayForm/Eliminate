...
import { AuxiliaryBarVisibleContext, FocusedViewContext, InEditorZenModeContext, IsAuxiliaryWindowFocusedContext, IsMainEditorCenteredLayoutContext, IsMainWindowFullscreenContext, MainEditorAreaVisibleContext, PanelAlignmentContext, PanelPositionContext, PanelVisibleContext, SideBarVisibleContext, TitleBarStyleContext, } from "../../common/contextkeys.js";
...
export class EditorActionsDefaultAction extends Action2 {
    ...
    constructor() {
        super({
            ...
            category: Categories.View,
            precondition: ContextKeyExpr.and(ContextKeyExpr.equals(`config.${LayoutSettings.EDITOR_ACTIONS_LOCATION}`, EditorActionsLocation.DEFAULT).negate(), ContextKeyExpr.equals(`config.${LayoutSettings.EDITOR_TABS_MODE}`, EditorTabsMode.NONE).negate()),
            metadata: {
...
class MoveFocusedViewAction extends Action2 {
    ...
    run(accessor: ServicesAccessor, viewId?: string): void {
        ...
        const isViewSolo = viewDescriptorService.getViewContainerModel(currentContainer)
        ...
        items.push(...pinnedViewlets
            .filter((viewletId) => {
            if (viewletId ===
                viewDescriptorService.getViewContainerByViewId(focusedViewId)!.id) {
                return false;
            }
...
registerAction2(class extends Action2 {
    ...
    run(accessor: ServicesAccessor): void {
        ...
        if (!defaultContainer ||
            defaultContainer ===
                viewDescriptorService.getViewContainerByViewId(viewDescriptor.id)) {
            return;
        }
...
ToggleVisibilityActions.push(...[
    ...
    CreateToggleLayoutItem(ToggleAuxiliaryBarAction.ID, AuxiliaryBarVisibleContext, localize("secondarySideBar", "Secondary Side Bar"), {
        whenA: ContextKeyExpr.equals("config.workbench.sideBar.location", "left"),
    ...
    run(accessor: ServicesAccessor): void {
        ...
        disposables.add(contextKeyService.onDidChangeContext((changeEvent) => {
            if (changeEvent.affectsSome(LayoutContextKeySet)) {
                quickPick.items = this.getItems(contextKeyService, keybindingService);
                if (selectedItem) {
                    quickPick.activeItems = quickPick.items.filter((item) => (item as CustomizeLayoutItem).id ===
                        selectedItem?.id) as IQuickPickItem[];
                }
