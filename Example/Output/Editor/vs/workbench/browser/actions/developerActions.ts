...
import { $, append, createCSSRule, createStyleSheet, getActiveDocument, getDomNodePagePosition, getWindows, onDidRegisterWindow, } from "../../../base/browser/dom.js";
...
import { DisposableInfo, DisposableStore, DisposableTracker, dispose, IDisposable, setDisposableTracker, toDisposable, } from "../../../base/common/lifecycle.js";
...
class ToggleScreencastModeAction extends Action2 {
    ...
    run(accessor: ServicesAccessor): void {
        ...
        disposables.add(onKeyDown.event((e) => {
            ...
            if (shortcut.kind === ResultKind.KbFound &&
                shortcut.commandId &&
            ...
            if (commandDetails) {
                if ((options.showCommandGroups ?? false) &&
                    commandDetails.category) {
                ...
                if (this._isKbFound(shortcut) && shortcut.commandId) {
                    ...
                    if (keybindings.length > 0) {
                        keyLabel =
                            keybindings[keybindings.length - 1].getLabel();
                    }
                }
            }
            ...
            if ((options.showKeys ?? true) ||
                ((options.showKeybindings ?? true) &&
                    this._isKbFound(shortcut))) {
    ...
    private getCommandDetails(commandId: string): {
        title: string;
        category?: string;
    } | undefined {
        ...
        if (fromCommandsRegistry &&
            fromCommandsRegistry.metadata?.description) {
            return {
                title: typeof fromCommandsRegistry.metadata.description ===
                    "string"
                    ? fromCommandsRegistry.metadata.description
                ...
                for (const key of storageService.keys(scope, target)) {
                    ...
                    if (value &&
                        (!environmentService.isBuilt /* show all keys in dev */ ||
                            value.length >
                                RemoveLargeStorageEntriesAction.SIZE_THRESHOLD)) {
                        items.push({
                            key,
                            scope,
                            target,
                            ...
                            detail: localize("largeStorageItemDetail", "Scope: {0}, Target: {1}", scope === StorageScope.APPLICATION
                                ? localize("global", "Global")
                                : scope === StorageScope.PROFILE
                                    ? localize("profile", "Profile")
                                    : localize("workspace", "Workspace"), target === StorageTarget.MACHINE
...
class SnapshotTrackedDisposables extends Action2 {
    ...
    run(accessor: ServicesAccessor): void {
        ...
        trackedDisposables = new Set(tracker
