...
import { connectManagedSocket, ManagedSocket, RemoteSocketHalf, } from "../../../platform/remote/common/managedSocket.js";
...
export function getSockets(stdout: string): Record<string, {
    pid: number;
    socket: number;
}> {
...
export function loadListeningPorts(...stdouts: string[]): {
    socket: number;
    ip: string;
    port: number;
}[] {
...
export async function findPorts(connections: {
    socket: number;
    ip: string;
    port: number;
...
export function tryFindRootPorts(connections: {
    socket: number;
    ip: string;
    port: number;
    ...
    const ports: Map<number, CandidatePort & {
        ppid: number;
    }> = new Map();
...
export class NodeExtHostTunnelService extends ExtHostTunnelService {
    ...
    private _foundRootPorts: Map<number, CandidatePort & {
        ppid: number;
    }> = new Map();
    ...
    override async $registerCandidateFinder(enable: boolean): Promise<void> {
        ...
        while (this._candidateFindingEnabled) {
            ...
            const newPorts = (await this.findCandidatePorts()).filter((candidate) => isLocalhost(candidate.host) ||
...
class ExtHostManagedSocket extends ManagedSocket {
    public static connect(passing: vscode.ManagedMessagePassing, path: string, query: string, debugLabel: string): Promise<ExtHostManagedSocket> {
    ...
    constructor(private readonly passing: vscode.ManagedMessagePassing, debugLabel: string, half: RemoteSocketHalf) {
