...
import { CustomExecutionDTO, ExtHostTaskBase, HandlerData, TaskDTO, TaskHandleDTO, } from "../common/extHostTask.js";
...
export class ExtHostTask extends ExtHostTaskBase {
    ...
    @IExtHostWorkspace
    private readonly workspaceService: IExtHostWorkspace, 
    ...
    private async getAFolder(workspaceFolders: vscode.WorkspaceFolder[] | undefined): Promise<IWorkspaceFolder> {
        let folder = workspaceFolders && workspaceFolders.length > 0
            ? workspaceFolders[0]
            : undefined;
        if (!folder) {
    ...
    public async $resolveVariables(uriComponents: UriComponents, toResolve: {
        process?: {
            name: string;
            cwd?: string;
            path?: string;
        ...
        if (toResolve.process !== undefined) {
            ...
            result.process = await win32.findExecutable(await resolver.resolveAsync(ws, toResolve.process.name), toResolve.process.cwd !== undefined
