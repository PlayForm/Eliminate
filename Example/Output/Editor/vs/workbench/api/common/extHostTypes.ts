...
export class Disposable {
    static from(...inDisposables: {
        ...
        let disposables: ReadonlyArray<{
            dispose(): any;
        }> | undefined = inDisposables;
...
export class Position {
    ...
    translate(lineDeltaOrChange: number | undefined | {
        lineDelta?: number;
        characterDelta?: number;
    ...
    with(lineOrChange: number | undefined | {
        line?: number;
        character?: number;
    }, character: number = this.character): Position {
...
export class Range {
    ...
    constructor(startLineOrStart: number | Position | vscode.Position, startColumnOrEnd: number | Position | vscode.Position, endLine?: number, endColumn?: number) {
        ...
        else if (Position.isPosition(startLineOrStart) &&
    ...
    with(startOrChange: Position | undefined | {
        start?: Position;
        end?: Position;
    }, end: Position = this.end): Range {
...
export class WorkspaceEdit implements vscode.WorkspaceEdit {
    ...
    // --- text (Maplike)
    has(uri: URI): boolean {
        return this._edits.some((edit) => edit._type === FileEditType.Text &&
    ...
    set(uri: URI, edits: null | undefined | ReadonlyArray<TextEdit | SnippetTextEdit | NotebookEdit | [
        NotebookEdit,
        vscode.WorkspaceEditEntryMetadata | undefined
    ] | [
        TextEdit | SnippetTextEdit,
...
export class ShellExecution implements vscode.ShellExecution {
    ...
    public computeId(): string {
        ...
        if (this._command !== undefined) {
            props.push(typeof this._command === "string"
                ? this._command
                : this._command.value);
        }
        if (this._args && this._args.length > 0) {
...
export class TreeItem {
    ...
    static isTreeItem(thing: any, extension: IExtensionDescription): thing is TreeItem {
        ...
        if (treeItemThing.checkboxState !== undefined) {
            const checkbox = isNumber(treeItemThing.checkboxState)
                ...
                : isObject(treeItemThing.checkboxState) &&
                    ...
                    : undefined;
            const tooltip = !isNumber(treeItemThing.checkboxState) &&
                isObject(treeItemThing.checkboxState)
        ...
        if (treeItemThing.resourceUri !== undefined &&
        ...
        if (treeItemThing.command !== undefined &&
        ...
        if (treeItemThing.contextValue !== undefined &&
...
export class DebugStackFrame implements vscode.DebugStackFrame {
    constructor(public readonly session: vscode.DebugSession, readonly threadId: number, readonly frameId: number) { }
}
export class DebugThread implements vscode.DebugThread {
    constructor(public readonly session: vscode.DebugSession, readonly threadId: number) { }
}
@es5ClassCompat
...
export class NotebookCellOutput {
    static isNotebookCellOutput(candidate: any): candidate is vscode.NotebookCellOutput {
        ...
        return (typeof (<NotebookCellOutput>candidate).id === "string" &&
...
export class TestRunRequest implements vscode.TestRunRequest {
    constructor(public readonly include: vscode.TestItem[] | undefined = undefined, public readonly exclude: vscode.TestItem[] | undefined = undefined, public readonly profile: vscode.TestRunProfile | undefined = undefined, public readonly continuous = false, public readonly preserveFocus = true) { }
}
@es5ClassCompat
...
export class FileCoverage implements vscode.FileCoverage {
    ...
    constructor(public readonly uri: vscode.Uri, public statementCoverage: vscode.TestCoverageCount, public branchCoverage?: vscode.TestCoverageCount, public declarationCoverage?: vscode.TestCoverageCount, public fromTests: vscode.TestItem[] = []) { }
}
export class StatementCoverage implements vscode.StatementCoverage {
    ...
    constructor(public executed: number | boolean, public location: Position | Range, public branches: vscode.BranchCoverage[] = []) { }
}
...
export class ChatRequestTurn implements vscode.ChatRequestTurn {
    constructor(readonly prompt: string, readonly command: string | undefined, readonly references: vscode.ChatPromptReference[], readonly participant: string, readonly toolReferences: vscode.ChatLanguageModelToolReference[]) { }
}
export class ChatResponseTurn implements vscode.ChatResponseTurn {
    constructor(readonly response: ReadonlyArray<ChatResponseMarkdownPart | ChatResponseFileTreePart | ChatResponseAnchorPart | ChatResponseCommandButtonPart>, readonly result: vscode.ChatResult, readonly participant: string, readonly command?: string) { }
}
...
export class ChatRequestEditorData implements vscode.ChatRequestEditorData {
    constructor(readonly document: vscode.TextDocument, readonly selection: vscode.Selection, readonly wholeRange: vscode.Range) { }
}
