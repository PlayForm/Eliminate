...
import { InputBoxValidationSeverity, QuickInputButtons, QuickPickItemKind, ThemeIcon, } from "./extHostTypes.js";
...
export function createExtHostQuickOpen(mainContext: IMainContext, workspace: IExtHostWorkspaceProvider, commands: ExtHostCommands): ExtHostQuickOpenShape & ExtHostQuickOpen {
                    ...
                    for (let handle = 0; handle < items.length; handle++) {
                        ...
                        if (typeof item === "string") {
                            ...
                            pickItems.push({
                                ...
                                tooltip: allowedTooltips
                                    ? MarkdownString.fromStrict(item.tooltip)
                                    : undefined,
                                handle,
                            });
                        }
    ...
    class ExtHostQuickInput implements QuickInput {
        ...
        set buttons(buttons: QuickInputButton[]) {
            ...
            if (!allowedButtonLocation &&
                buttons.some((button) => button.location)) {
            ...
            this.update({
                buttons: buttons.map<TransferQuickInputButton>((button, i) => {
                    return {
                        ...
                        location: allowedButtonLocation
                            ? button.location
                            : undefined,
                    };
                }),
    ...
    function getIconUris(iconPath: QuickInputButton["iconPath"]): {
        ...
        const dark = getDarkIconUri(iconPath as URI | {
            light: URI;
            dark: URI;
        });
        const light = getLightIconUri(iconPath as URI | {
            light: URI;
            dark: URI;
        });
    ...
    function getLightIconUri(iconPath: URI | {
        light: URI;
        dark: URI;
    }) {
        return typeof iconPath === "object" && "light" in iconPath
            ? iconPath.light
            : iconPath;
    }
    function getDarkIconUri(iconPath: URI | {
        light: URI;
        dark: URI;
    }) {
        return typeof iconPath === "object" && "dark" in iconPath
            ? iconPath.dark
            : iconPath;
    }
    ...
    class ExtHostInputBox extends ExtHostQuickInput implements InputBox {
        ...
        private _valueSelection: readonly [
            number,
            number
        ] | undefined;
        ...
        set valueSelection(valueSelection: readonly [
            number,
            number
        ] | undefined) {
