...
import { IRevealOptions, ITreeItem, ITreeItemCheckboxState, ITreeItemLabel, NoTreeViewError, TreeCommand, TreeViewItemHandleArg, TreeViewPaneHandleArg, } from "../../common/views.js";
...
import { DataTransfer, MarkdownString, ViewBadge, } from "./extHostTypeConverters.js";
    ...
    if (label && typeof label === "object" && typeof label.label === "string") {
        ...
        if (Array.isArray(label.highlights)) {
            highlights = (<[
                number,
                number
            ][]>label.highlights).filter((highlight) => highlight.length === 2 &&
...
export class ExtHostTreeViews extends Disposable implements ExtHostTreeViewsShape {
    ...
    constructor(private _proxy: MainThreadTreeViewsShape, private commands: ExtHostCommands, private logService: ILogService) {
        super();
    ...
    async $handleDrop(destinationViewId: string, requestId: number, treeDataTransferDTO: DataTransferDTO, targetItemHandle: string | undefined, token: CancellationToken, operationUuid?: string, sourceViewId?: string, sourceTreeItemHandles?: string[]): Promise<void> {
    ...
    private async addAdditionalTransferItems(treeDataTransfer: vscode.DataTransfer, treeView: ExtHostTreeView<any>, sourceTreeItemHandles: string[], token: CancellationToken, operationUuid?: string): Promise<vscode.DataTransfer | undefined> {
...
class ExtHostTreeView<T> extends Disposable {
    ...
    get selectedElements(): T[] {
        return <T[]>(this._selectedHandles
    ...
    get focusedElement(): T | undefined {
        return <T | undefined>((this._focusedHandle
    ...
    constructor(private viewId: string, options: vscode.TreeViewOptions<T>, private proxy: MainThreadTreeViewsShape, private commands: CommandsConverter, private logService: ILogService, private extension: IExtensionDescription) {
        ...
        const onDidChangeData = Event.debounce<TreeData<T>, {
            message: boolean;
            elements: (T | Root)[];
        }>(this._onDidChangeData.event, (result, current) => {
            if (!result) {
    ...
    reveal(element: T | undefined, options?: IRevealOptions): Promise<void> {
        ...
        if (element) {
            return this.refreshPromise
                ...
                .then((parentChain) => this.resolveTreeNode(element, parentChain[parentChain.length - 1]).then((treeNode) => this.proxy.$reveal(this.viewId, {
                item: treeNode.item,
                parentChain: parentChain.map((p) => p.item),
            }, { select, focus, expand })), (error) => this.logService.error(error));
        }
        else {
    ...
    async handleDrag(sourceTreeItemHandles: TreeItemHandle[], treeDataTransfer: vscode.DataTransfer, token: CancellationToken): Promise<vscode.DataTransfer | undefined> {
        ...
        if (!this.dndController?.handleDrag ||
    ...
    private async fetchChildrenNodes(parentElement?: T): Promise<TreeNode[] | undefined> {
        ...
        try {
            ...
            const items = treeItems.map((item, index) => item
                ? this.createAndRegisterTreeNode(coalescedElements[index], item, parentNode)
                : null);
