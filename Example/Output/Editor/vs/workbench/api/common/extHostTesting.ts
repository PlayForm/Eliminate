...
import { AbstractIncrementalTestCollection, CoverageDetails, ICallProfileRunHandler, IncrementalChangeCollector, IncrementalTestCollectionItem, InternalTestItem, ISerializedTestResults, isStartControllerTests, IStartControllerTests, IStartControllerTestsResult, ITestErrorMessage, ITestItem, ITestItemContext, ITestMessageMenuArgs, ITestRunProfile, TestControllerCapability, TestMessageFollowupRequest, TestMessageFollowupResponse, TestResultState, TestRunProfileBitset, TestsDiff, TestsDiffOp, } from "../../contrib/testing/common/testTypes.js";
...
/* controllerId */ string, Map</* profileId */ number, boolean>>;
...
export class ExtHostTesting extends Disposable implements ExtHostTestingShape {
    ...
    /* controller ID */ string, ControllerInfo>();
    ...
    @IExtHostCommands
    private readonly commands: IExtHostCommands, 
            ...
            processArgument: (arg) => {
                switch (arg?.$mid) {
                    case MarshalledId.TestItemContext: {
                        ...
                        return (controller?.collection.tree.get(targetTest)
    ...
    public async $provideTestFollowups(req: TestMessageFollowupRequest, token: CancellationToken): Promise<TestMessageFollowupResponse[]> {
        ...
        const test = results &&
            findTestInResultSnapshot(TestId.fromString(req.extId), results?.results);
        if (!test) {
            return [];
        }
    ...
    private async runControllerTestRequest(req: ICallProfileRunHandler | ICallProfileRunHandler, isContinuous: boolean, token: CancellationToken): Promise<IStartControllerTestsResult> {
        ...
        const excludeTests = req.excludeExtIds
            ...
            .filter(isDefined)
            .filter((exclude) => includeTests.some((include) => include.fullId.compare(exclude.fullId) ===
...
class TestRunTracker extends Disposable {
    ...
    constructor(private readonly dto: TestRunDto, private readonly proxy: MainThreadTestingShape, private readonly logService: ILogService, private readonly profile: vscode.TestRunProfile | undefined, private readonly extension: IExtensionDescription, parentToken?: CancellationToken) {
        super();
    ...
    public createRun(name: string | undefined): vscode.TestRun {
        ...
        const guardTestMutation = <Args extends unknown[]>(fn: (test: vscode.TestItem, ...args: Args) => void) => (test: vscode.TestItem, ...args: Args) => {
            if (ended) {
        ...
        const run: vscode.TestRun = {
            ...
            addCoverage: (coverage) => {
                ...
                this.publishedCoverage.set(id, {
                    report: coverage,
                    extIds: fromTests.map((t) => TestId.fromExtHostTestItem(t, ctrlId).toString()),
                });
...
export class TestRunCoordinator {
    ...
    constructor(private readonly proxy: MainThreadTestingShape, private readonly logService: ILogService) { }
    /**
...
export class TestRunDto {
    ...
    constructor(public readonly controllerId: string, public readonly id: string, public readonly isPersisted: boolean, public readonly colllection: ExtHostTestItemCollection) { }
}
/**
 * @private
 */
...
export class TestRunProfileImpl implements vscode.TestRunProfile {
    ...
    constructor(proxy: MainThreadTestingShape, profiles: Map<number, vscode.TestRunProfile>, activeProfiles: Set<number>, onDidChangeActiveProfiles: Event<DefaultProfileChangeEvent>, public readonly controllerId: string, public readonly profileId: number, private _label: string, public readonly kind: vscode.TestRunProfileKind, public runHandler: (request: vscode.TestRunRequest, token: vscode.CancellationToken) => Thenable<void> | void, _isDefault = false, public _tag: vscode.TestTag | undefined = undefined, private _supportsContinuousRun = false) {
        this.#proxy = proxy;
