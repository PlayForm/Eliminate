...
import { INotebookContributionData, INotebookExclusiveDocumentFilter, } from "../../contrib/notebook/common/notebookCommon.js";
...
import { genericCellMatchesToTextSearchMatches, INotebookCellMatchNoModel, INotebookFileMatchNoModel, IRawClosedNotebookFileMatch, } from "../../contrib/search/common/searchNotebookHelpers.js";
...
export class ExtHostNotebookController implements ExtHostNotebookShape {
    ...
    constructor(mainContext: IMainContext, commands: ExtHostCommands, private _textDocumentsAndEditors: ExtHostDocumentsAndEditors, private _textDocuments: ExtHostDocuments, private _extHostFileSystem: IExtHostConsumerFileSystem, private _extHostSearch: IExtHostSearch, private _logService: ILogService) {
    ...
    private readonly _notebookSerializer = new Map<number, {
        viewType: string;
     ...
     */
    async $searchInNotebooks(handle: number, textQuery: ITextQuery, viewTypeFileTargets: NotebookPriorityInfo[], otherViewTypeFileTargets: NotebookPriorityInfo[], token: CancellationToken): Promise<{
        ...
        if (delta.value.removedDocuments) {
            for (const uri of delta.value.removedDocuments) {
                ...
                for (const editor of this._editors.values()) {
                    if (editor.notebookData.uri.toString() ===
                        revivedUri.toString()) {
    ...
    private static _registerApiCommands(extHostCommands: ExtHostCommands) {
        ...
        const commandDataToNotebook = new ApiCommand("vscode.executeDataToNotebook", "_executeDataToNotebook", "Invoke notebook serializer", [
            notebookTypeArg,
            new ApiCommandArgument<Uint8Array, VSBuffer>("data", "Bytes to convert to data", (v) => v instanceof Uint8Array, (v) => VSBuffer.wrap(v)),
        ...
        const commandNotebookToData = new ApiCommand("vscode.executeNotebookToData", "_executeNotebookToData", "Invoke notebook serializer", [
            notebookTypeArg,
            new ApiCommandArgument<vscode.NotebookData, SerializableObjectWithBuffers<NotebookDataDto>>("NotebookData", "Notebook data to convert to bytes", (v) => true, (v) => new SerializableObjectWithBuffers(typeConverters.NotebookData.from(v))),
