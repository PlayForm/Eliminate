...
import { ExtHostConfigurationShape, IConfigurationInitData, MainContext, MainThreadConfigurationShape, } from "./extHost.protocol.js";
...
function isResourceLanguage(thing: any): thing is {
    uri: URI;
    languageId: string;
} {
    return (thing &&
...
function isLanguage(thing: any): thing is {
    languageId: string;
} {
    return (thing &&
        !thing.uri &&
...
export class ExtHostConfigProvider {
            ...
            inspect: <T>(key: string): ConfigurationInspect<T> | undefined => {
                ...
                if (config) {
                    return {
                        ...
                        globalLanguageValue: deepClone(config.user?.override ??
        ...
        const readonlyProxy = (target: any): any => {
            return isObject(target)
                ? new Proxy(target, {
                    ...
                    set: (_target: any, property: PropertyKey, _value: any) => {
                    ...
                    defineProperty: (_target: any, property: PropertyKey) => {
    ...
    private _toConfigurationChangeEvent(change: IConfigurationChange, previous: {
