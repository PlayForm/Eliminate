...
export abstract class ExtHostDebugServiceBase extends DisposableCls implements IExtHostDebugService, ExtHostDebugServiceShape {
    ...
    private readonly _debugVisualizationElements = new Map<number, {
        provider: string;
        item: vscode.DebugTreeItem;
        children?: number[];
    }>();
    private _signService: ISignService | undefined;
    private readonly _visualizers = new Map<number, {
    ...
    private _variableResolver: IExtHostVariableResolverProvider, 
    @IExtHostCommands
    private _commands: IExtHostCommands, 
    @IExtHostTesting
    private _testing: IExtHostTesting) {
        super();
        ...
        this._extensionService
            ...
            .then((extensionRegistry: ExtensionDescriptionRegistry) => {
    ...
    public startDebugging(folder: vscode.WorkspaceFolder | undefined, nameOrConfig: string | vscode.DebugConfiguration, options: vscode.DebugSessionOptions): Promise<boolean> {
        ...
        return this._debugServiceProxy.$startDebugging(folder ? folder.uri : undefined, nameOrConfig, {
            ...
            repl: options.consoleMode === DebugConsoleMode.MergeWithParent
                ? "mergeWithParent"
                : "separate",
            ...
            for (const bpd of delta.changed) {
                if (bpd.id) {
                    ...
                    if (bp) {
                        ...
                        else if (bp instanceof SourceBreakpoint &&
                            bpd.type === "source") {
                            const sbp = <any>bp;
    ...
    private getIconUris(iconPath: vscode.DebugVisualization["iconPath"]): {
        dark: URI;
        light?: URI;
    } | {
        id: string;
        ...
        const dark = typeof iconPath === "object" && "dark" in iconPath
            ? iconPath.dark
            : iconPath;
        const light = typeof iconPath === "object" && "light" in iconPath
            ? iconPath.light
            : iconPath;
        return {
...
export class ExtHostDebugSession {
    ...
    constructor(private _debugServiceProxy: MainThreadDebugServiceShape, private _id: DebugSessionUUID, private _type: string, private _name: string, private _workspaceFolder: vscode.WorkspaceFolder | undefined, private _configuration: vscode.DebugConfiguration, private _parentSession: vscode.DebugSession | undefined) { }
...
class MultiTracker implements vscode.DebugAdapterTracker {
    ...
    onWillReceiveMessage(message: any): void {
        this.trackers.forEach((t) => t.onWillReceiveMessage
