...
type ExtHostSelectKernelArgs = ControllerInfo | {
    notebookEditor: vscode.NotebookEditor;
} | (ControllerInfo & {
    notebookEditor: vscode.NotebookEditor;
}) | undefined;
type SelectKernelReturnArgs = ControllerInfo | {
...
export class ExtHostNotebookKernels implements ExtHostNotebookKernelsShape {
    ...
    constructor(mainContext: IMainContext, private readonly _initData: IExtHostInitDataService, private readonly _extHostNotebook: ExtHostNotebookController, private _commands: ExtHostCommands, 
    @ILogService
    private readonly _logService: ILogService) {
        ...
        const requestKernelVariablesApiCommand = new ApiCommand("vscode.executeNotebookVariableProvider", "_executeNotebookVariableProvider", "Execute notebook variable provider", [ApiCommandArgument.Uri], new ApiCommandResult<VariablesResult[], vscode.VariablesResult[]>("A promise that resolves to an array of variables", (value, apiArgs) => {
    ...
    registerKernelSourceActionProvider(extension: IExtensionDescription, viewType: string, provider: vscode.NotebookKernelSourceActionProvider) {
        ...
        const eventHandle = typeof provider.onDidChangeNotebookKernelSourceActions ===
            "function"
            ? handle
            : undefined;
        const that = this;
...
class NotebookCellExecutionTask extends Disposable {
    ...
    constructor(controllerId: string, private readonly _cell: ExtHostCell, private readonly _proxy: MainThreadNotebookKernelsShape) {
        super();
                    ...
                    ? {
                        ...
                        location: executionError?.location
                            ? {
                                ...
                                startColumn: executionError.location.start
                                    .character,
                                endLineNumber: executionError.location.end.line,
                                endColumn: executionError.location.end
                                    .character,
                            }
                            : undefined,
...
class NotebookExecutionTask extends Disposable {
    ...
    private readonly _tokenSource = this._register(new CancellationTokenSource());
    constructor(controllerId: string, private readonly _notebook: ExtHostNotebookDocument, private readonly _proxy: MainThreadNotebookKernelsShape) {
        super();
...
class TimeoutBasedCollector<T> {
    ...
    constructor(private readonly delay: number, private readonly callback: (items: T[]) => Promise<void>) { }
