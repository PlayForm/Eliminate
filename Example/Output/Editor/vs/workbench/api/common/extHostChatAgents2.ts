...
class ChatAgentResponseStream {
    ...
    constructor(private readonly _extension: IExtensionDescription, private readonly _request: IChatAgentRequest, private readonly _proxy: MainThreadChatAgentsShape2, private readonly _commandsConverter: CommandsConverter, private readonly _sessionDisposables: DisposableStore) { }
    close() {
        this._isClosed = true;
    }
                            ...
                            if (matchingVarData.references?.length) {
                                references = matchingVarData.references.map((r) => ({
                                    ...
                                    reference: {
                                        ...
                                        value: r.reference as URI | Location,
                                    },
...
export class ExtHostChatAgents2 extends Disposable implements ExtHostChatAgentsShape2 {
    ...
    constructor(mainContext: IMainContext, private readonly _logService: ILogService, private readonly _commands: ExtHostCommands, private readonly _documents: ExtHostDocuments, private readonly _languageModels: ExtHostLanguageModels) {
        super();
    ...
    async $detectChatParticipant(handle: number, requestDto: Dto<IChatAgentRequest>, context: {
    ...
    private async _createRequest(requestDto: Dto<IChatAgentRequest>, context: {
        history: IChatAgentHistoryEntryDto[];
    }) {
    ...
    async $invokeAgent(handle: number, requestDto: Dto<IChatAgentRequest>, context: {
        history: IChatAgentHistoryEntryDto[];
    ...
    private async prepareHistoryTurns(agentId: string, context: {
        history: IChatAgentHistoryEntryDto[];
    ...
    async $provideFollowups(requestDto: Dto<IChatAgentRequest>, handle: number, result: IChatAgentResult, context: {
        history: IChatAgentHistoryEntryDto[];
...
class ExtHostParticipantDetector {
    constructor(public readonly extension: IExtensionDescription, public readonly provider: vscode.ChatParticipantDetectionProvider) { }
}
class ExtHostChatAgent {
    ...
    constructor(public readonly extension: IExtensionDescription, public readonly id: string, private readonly _proxy: MainThreadChatAgentsShape2, private readonly _handle: number, private _requestHandler: vscode.ChatExtendedRequestHandler) { }
    ...
    async provideTitle(context: vscode.ChatContext, token: CancellationToken): Promise<string | undefined> {
        ...
        return ((await this._titleProvider.provideChatTitle(context, token)) ??
            undefined);
    }
        ...
        const updateMetadataSoon = () => {
            ...
            queueMicrotask(() => {
                this._proxy.$updateAgent(this._handle, {
                    ...
                    helpTextPostfix: !this._helpTextPostfix ||
