...
class LanguageModelResponseStream {
    ...
    constructor(readonly option: number, stream?: AsyncIterableSource<vscode.LanguageModelTextPart | vscode.LanguageModelToolCallPart>) {
        this.stream =
            stream ??
                new AsyncIterableSource<vscode.LanguageModelTextPart | vscode.LanguageModelToolCallPart>();
    }
}
...
export class ExtHostLanguageModels implements ExtHostLanguageModelsShape {
    ...
    private readonly _allLanguageModelData = new Map<string, {
    ...
    private readonly _pendingRequest = new Map<number, {
        languageModelId: string;
        res: LanguageModelResponse;
    }>();
    ...
    registerLanguageModel(extension: IExtensionDescription, identifier: string, provider: vscode.ChatResponseProvider, metadata: vscode.ChatResponseProviderMetadata): IDisposable {
        ...
        if (metadata.auth) {
            auth = {
                ...
                accountLabel: typeof metadata.auth === "object"
                    ? metadata.auth.label
                    : undefined,
            };
        }
    ...
    async $startChatRequest(handle: number, requestId: number, from: ExtensionIdentifier, messages: IChatMessage[], options: vscode.LanguageModelChatRequestOptions, token: CancellationToken): Promise<void> {
        ...
        const progress = new Progress<vscode.ChatResponseFragment2>(async (fragment) => {
            ...
            if (fragment.part instanceof
                extHostTypes.LanguageModelToolCallPart) {
                part = {
                    type: "tool_use",
                    name: fragment.part.name,
                    parameters: fragment.part.input ?? fragment.part.parameters,
        ...
        if (data.provider.provideLanguageModelResponse2) {
            ...
            p = Promise.resolve(data.provider.provideLanguageModelResponse(messages.map(typeConvert.LanguageModelChatMessage.to), options, ExtensionIdentifier.toKey(from), progress, token));
        }
        p.then(() => {
    ...
    private async _sendChatRequest(extension: IExtensionDescription, languageModelId: string, messages: vscode.LanguageModelChatMessage[], options: vscode.LanguageModelChatRequestOptions, token: CancellationToken) {
        ...
        if (this._isUsingAuth(from, metadata)) {
            const success = await this._getAuthAccess(extension, {
                identifier: metadata.extension,
                displayName: metadata.auth.providerLabel,
            }, options.justification, false);
            if (!success ||
        ...
        if (options.tools) {
            ...
            type OldChatTool = vscode.LanguageModelChatTool & {
    ...
    private async _getAuthAccess(from: IExtensionDescription, to: {
        identifier: ExtensionIdentifier;
        displayName: string;
    }, justification: string | undefined, silent: boolean | undefined): Promise<boolean> {
    ...
    private _isUsingAuth(from: ExtensionIdentifier, toMetadata: ILanguageModelChatMetadata): toMetadata is ILanguageModelChatMetadata & {
    ...
    private async _computeTokenLength(languageModelId: string, value: string | vscode.LanguageModelChatMessage, token: vscode.CancellationToken): Promise<number> {
        ...
        return this._proxy.$countTokens(languageModelId, typeof value === "string"
            ? value
            : typeConvert.LanguageModelChatMessage.from(value), token);
    }
