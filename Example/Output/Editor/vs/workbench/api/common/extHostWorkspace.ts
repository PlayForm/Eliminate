...
function delta(oldFolders: vscode.WorkspaceFolder[], newFolders: vscode.WorkspaceFolder[], compare: (a: vscode.WorkspaceFolder, b: vscode.WorkspaceFolder, extHostFileSystemInfo: IExtHostFileSystemInfo) => number, extHostFileSystemInfo: IExtHostFileSystemInfo): {
...
class ExtHostWorkspaceImpl extends Workspace {
    static toExtHostWorkspace(data: IWorkspaceData | null, previousConfirmedWorkspace: ExtHostWorkspaceImpl | undefined, previousUnconfirmedWorkspace: ExtHostWorkspaceImpl | undefined, extHostFileSystemInfo: IExtHostFileSystemInfo): {
        ...
        if (previousConfirmedWorkspace) {
            ...
            newWorkspaceFolders.push(...folders.map(({ uri, name, index }) => ({
                uri: URI.revive(uri),
                name,
                index,
            })));
...
export class ExtHostWorkspace implements ExtHostWorkspaceShape, IExtHostWorkspaceProvider {
    ...
    uriTransformerService: IURITransformerService) {
    ...
    updateWorkspaceFolders(extension: IExtensionDescription, index: number, deleteCount: number, ...workspaceFoldersToAdd: {
        ...
        for (let i = 0; i < newWorkspaceFolders.length; i++) {
            ...
            if (newWorkspaceFolders.some((otherFolder, index) => index !== i &&
                isFolderEqual(folder.uri, otherFolder.uri, this._extHostFileSystemInfo))) {
    ...
    include: vscode.GlobPattern | undefined, filePatterns: vscode.GlobPattern[] | undefined, options: vscode.FindFiles2Options, token: vscode.CancellationToken = CancellationToken.None): Promise<vscode.Uri[]> {
        ...
        const queryOptions: QueryOptions<IFileQueryBuilderOptions>[] = filePatternsToUse?.map((filePattern) => {
            ...
            const fileQueries: IFileQueryBuilderOptions = {
                ignoreSymlinks: typeof options.followSymlinks === "boolean"
                ...
                excludePattern: excludePatterns.length > 0
                    ? excludePatterns
                    : undefined,
    ...
    findTextInFiles2(query: vscode.TextSearchQuery2, options: vscode.FindTextInFilesOptions2 | undefined, extensionId: ExtensionIdentifier, token: vscode.CancellationToken = CancellationToken.None): vscode.FindTextInFilesResponse {
            ...
            return {
                options: {
                    ...
                    previewOptions: options.previewOptions
                        ? {
                            matchLines: options.previewOptions?.numMatchLines ??
                                100,
                            charsPerLine: options.previewOptions?.charsPerLine ??
                                10000,
                        }
                        : undefined,
        ...
        const queryOptionsRaw: (QueryOptions<ITextQueryBuilderOptions> | undefined)[] = options?.include?.map((include) => getOptions(include)) ?? [
    ...
    async findTextInFiles(query: vscode.TextSearchQuery, options: vscode.FindTextInFilesOptions & {
        ...
        const queryOptions: ITextQueryBuilderOptions = {
            ...
            disregardParentIgnoreFiles: typeof options.useParentIgnoreFiles === "boolean"
                ...
                : undefined,
            disregardExcludeSettings: typeof options.useDefaultExcludes === "boolean"
                ? !options.useDefaultExcludes
                : true,
            disregardSearchExcludeSettings: typeof options.useSearchExclude === "boolean"
    ...
    getOnWillCreateEditSessionIdentityEvent(extension: IExtensionDescription): Event<vscode.EditSessionIdentityWillCreateEvent> {
        return (listener, thisArg, disposables) => {
            const wrappedListener: IExtensionListener<vscode.EditSessionIdentityWillCreateEvent> = function wrapped(e) {
...
function parseSearchExcludeInclude(include: string | IRelativePatternDto | undefined | null): {
    pattern: string;
    folder?: URI;
} | undefined {
...
function globsToISearchPatternBuilder(excludes: vscode.GlobPattern[] | undefined): ISearchPatternBuilder<URI>[] {
    return (excludes?.map((exclude): ISearchPatternBuilder<URI> | undefined => {
        if (typeof exclude === "string") {
            if (exclude === "") {
                return undefined;
            }
