...
import { MainContext, MainThreadTelemetryShape } from "./extHost.protocol.js";
...
export abstract class RequireInterceptor {
    ...
    constructor(private _apiFactory: IExtensionApiFactory, private _extensionRegistry: IExtensionRegistries, 
...
class NodeModuleAliasingModuleFactory implements IAlternativeModuleProvider {
    ...
    initData: IExtHostInitDataService) {
        if (initData.environment.appRoot &&
...
class VSCodeNodeModuleFactory implements INodeModuleFactory {
    ...
    constructor(private readonly _apiFactory: IExtensionApiFactory, private readonly _extensionPaths: ExtensionPaths, private readonly _extensionRegistry: IExtensionRegistries, private readonly _configProvider: ExtHostConfigProvider, private readonly _logService: ILogService) { }
...
class OpenNodeModuleFactory implements INodeModuleFactory {
    ...
    constructor(private readonly _extensionPaths: ExtensionPaths, private readonly _appUriScheme: string, 
    @IExtHostRpcService
    ...
    private sendShimmingTelemetry(): void {
        ...
        this._mainThreadTelemetry.$publicLog2<{
            extension: string;
        }, ShimmingOpenClassification>("shimming.open", { extension: this._extensionId });
    }
    private sendNoForwardTelemetry(): void {
        ...
        this._mainThreadTelemetry.$publicLog2<{
            extension: string;
        }, ShimmingOpenCallNoForwardClassification>("shimming.open.call.noForward", { extension: this._extensionId });
    }
}
//#endregion
