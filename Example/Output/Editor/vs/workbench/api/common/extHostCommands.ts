...
import { ExtHostCommandsShape, ICommandDto, ICommandMetadataDto, MainContext, MainThreadCommandsShape, MainThreadTelemetryShape, } from "./extHost.protocol.js";
...
export class ExtHostCommands implements ExtHostCommandsShape {
            ...
            },
            {
                processArgument(arg) {
                    return cloneAndChange(arg, function (obj) {
                        ...
                        if (Range.isIRange((obj as languages.Location).range) &&
    ...
    private async _doExecuteCommand<T>(id: string, args: any[], retry: boolean): Promise<T> {
        if (this._commands.has(id)) {
            ...
            try {
                const result = await this.#proxy.$executeCommand(id, hasBuffers
    ...
    $getContributedCommandMetadata(): Promise<{
        ...
        const result: {
            [id: string]: string | ICommandMetadata;
        } = Object.create(null);
...
export class CommandsConverter implements extHostTypeConverter.Command.ICommandsConverter {
    ...
    constructor(private readonly _commands: ExtHostCommands, private readonly _lookupApiCommand: (id: string) => ApiCommand | undefined, private readonly _logService: ILogService) {
...
export class ApiCommandArgument<V, O = V> {
    ...
    static Arr<T, K = T>(element: ApiCommandArgument<T, K>) {
        return new ApiCommandArgument(`${element.name}_array`, `Array of ${element.name}, ${element.description}`, (v: unknown) => Array.isArray(v) && v.every((e) => element.validate(e)), (v: T[]) => v.map((e) => element.convert(e)));
    }
    ...
    constructor(readonly name: string, readonly description: string, readonly validate: (v: V) => boolean, readonly convert: (v: V) => O) { }
    optional(): ApiCommandArgument<V | undefined | null, O | undefined | null> {
        return new ApiCommandArgument(this.name, `(optional) ${this.description}`, (value) => value === undefined || value === null || this.validate(value), (value) => value === undefined
            ? undefined
            : value === null
                ? null
                : this.convert(value));
...
export class ApiCommandResult<V, O = V> {
    ...
    constructor(readonly description: string, readonly convert: (v: V, apiArgs: any[], cmdConverter: CommandsConverter) => O) { }
}
export class ApiCommand {
    constructor(readonly id: string, readonly internalId: string, readonly description: string, readonly args: ApiCommandArgument<any, any>[], readonly result: ApiCommandResult<any, any>) { }
}
