...
import { IStatusbarEntry, IStatusbarEntryAccessor, IStatusbarEntryPriority, IStatusbarService, StatusbarAlignment as MainThreadStatusBarAlignment, StatusbarAlignment, StatusbarEntryKind, } from "../../services/statusbar/browser/statusbar.js";
...
class ExtensionStatusBarItemService implements IExtensionStatusBarItemService {
    ...
    private readonly _entries: Map<string, {
        ...
        statusBarItemsExtensionPoint.setHandler((extensions) => {
            ...
            for (const entry of extensions) {
                ...
                for (const candidate of Iterable.wrap(value)) {
                    ...
                    const kind = statusBarItemsService.setOrUpdateEntry(fullItemId, fullItemId, ExtensionIdentifier.toKey(entry.description.identifier), candidate.name ??
                        entry.description.displayName ??
                        entry.description.name, candidate.text, candidate.tooltip, candidate.command
                        ? { id: candidate.command, title: candidate.name }
                        : undefined, undefined, undefined, candidate.alignment === "left", candidate.priority, candidate.accessibilityInformation);
