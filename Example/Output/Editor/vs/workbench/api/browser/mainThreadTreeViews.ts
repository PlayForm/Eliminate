...
import { Extensions, IRevealOptions, ITreeItem, ITreeView, ITreeViewDataProvider, ITreeViewDescriptor, ITreeViewDragAndDropController, IViewBadge, IViewsRegistry, NoTreeViewError, ResolvableTreeItem, } from "../../common/views.js";
import { IExtensionService } from "../../services/extensions/common/extensions.js";
import { extHostNamedCustomer, IExtHostContext, } from "../../services/extensions/common/extHostCustomers.js";
import { IViewsService } from "../../services/views/common/viewsService.js";
import { CheckboxUpdate, ExtHostContext, ExtHostTreeViewsShape, MainContext, MainThreadTreeViewsShape, } from "../common/extHost.protocol.js";
import * as typeConvert from "../common/extHostTypeConverters.js";
import { DataTransferFileCache } from "../common/shared/dataTransferCache.js";
@extHostNamedCustomer(MainContext.MainThreadTreeViews)
export class MainThreadTreeViews extends Disposable implements MainThreadTreeViewsShape {
    private readonly _proxy: ExtHostTreeViewsShape;
    private readonly _dataProviders: DisposableMap<string, {
    ...
    async $registerTreeViewDataProvider(treeViewId: string, options: {
    ...
    $refresh(treeViewId: string, itemsToRefreshByHandle: {
        ...
        if (viewer && dataProvider) {
            ...
            return viewer.refresh(itemsToRefresh.items.length ? itemsToRefresh.items : undefined, itemsToRefresh.checkboxes.length
                ? itemsToRefresh.checkboxes
                : undefined);
        }
    ...
    private async reveal(treeView: ITreeView, dataProvider: TreeViewDataProvider, itemIn: ITreeItem, parentChain: ITreeItem[], options: IRevealOptions): Promise<void> {
        ...
        let expand = Math.min(isNumber(options.expand)
            ? options.expand
            : options.expand === true
                ? 1
                : 0, 3);
...
class TreeViewDragAndDropController implements ITreeViewDragAndDropController {
    ...
    constructor(private readonly treeViewId: string, readonly dropMimeTypes: string[], readonly dragMimeTypes: string[], readonly hasWillDrop: boolean, private readonly _proxy: ExtHostTreeViewsShape) { }
    async handleDrop(dataTransfer: VSDataTransfer, targetTreeItem: ITreeItem | undefined, token: CancellationToken, operationUuid?: string, sourceTreeId?: string, sourceTreeItemHandles?: string[]): Promise<void> {
...
class TreeViewDataProvider implements ITreeViewDataProvider {
    ...
    constructor(private readonly treeViewId: string, private readonly _proxy: ExtHostTreeViewsShape, private readonly notificationService: INotificationService) {
        ...
        if (itemsToRefreshByHandle) {
            for (const newTreeItemHandle of Object.keys(itemsToRefreshByHandle)) {
                ...
                if (currentTreeItem) {
                    ...
                    if (currentTreeItem.checkbox?.isChecked !==
    ...
    private updateTreeItem(current: ITreeItem, treeItem: ITreeItem): void {
        ...
        if (current) {
            const properties = distinct([
                ...Object.keys(current instanceof ResolvableTreeItem
                    ? current.asTreeItem()
                    : current),
