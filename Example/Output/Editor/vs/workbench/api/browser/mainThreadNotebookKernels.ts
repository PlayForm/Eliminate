...
export class MainThreadNotebookKernels implements MainThreadNotebookKernelsShape {
    ...
    private readonly _kernels = new Map<number, [
        kernel: MainThreadKernel,
        registraion: IDisposable
    ]>();
    private readonly _kernelDetectionTasks = new Map<number, [
        task: MainThreadKernelDetectionTask,
        registraion: IDisposable
    ]>();
    private readonly _kernelSourceActionProviders = new Map<number, [
    ...
    async $postMessage(handle: number, editorId: string | undefined, message: any): Promise<boolean> {
        ...
        for (const [editor] of this._editors) {
            ...
            if (this._notebookKernelService.getMatchingKernel(editor.textModel)
                .selected !== kernel) {
                // different kernel
                continue;
            }
