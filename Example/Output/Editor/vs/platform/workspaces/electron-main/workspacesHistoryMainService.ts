...
import { Event as CommonEvent, Emitter } from "../../../base/common/event.js";
...
export class WorkspacesHistoryMainService extends Disposable implements IWorkspacesHistoryMainService {
    ...
    async removeRecentlyOpened(recentToRemove: URI[]): Promise<void> {
        ...
        if (workspaces.length !== mru.workspaces.length ||
    ...
    private containsWorkspace(recents: IRecent[], candidate: IWorkspaceIdentifier): boolean {
        return !!recents.find((recent) => isRecentWorkspace(recent) &&
            recent.workspace.id === candidate.id);
    }
    private containsFolder(recents: IRecent[], candidate: URI): boolean {
        return !!recents.find((recent) => isRecentFolder(recent) &&
    ...
    private async updateWindowsJumpList(): Promise<void> {
        ...
        if ((await this.getRecentlyOpened()).workspaces.length > 0) {
            ...
            const items: JumpListItem[] = coalesce((await this.getRecentlyOpened()).workspaces
    ...
    private getWindowsJumpListLabel(workspace: IWorkspaceIdentifier | URI, recentLabel: string | undefined): {
        title: string;
        description: string;
    } {
        // Prefer recent label
    ...
    private async updateMacOSRecentDocuments(): Promise<void> {
        ...
        for (let i = 0; i < mru.workspaces.length &&
            entries <
                WorkspacesHistoryMainService.MAX_MACOS_DOCK_RECENT_WORKSPACES; i++) {
        ...
        for (let i = 0; i < mru.files.length &&
            entries <
                WorkspacesHistoryMainService.MAX_MACOS_DOCK_RECENT_ENTRIES_TOTAL; i++) {
