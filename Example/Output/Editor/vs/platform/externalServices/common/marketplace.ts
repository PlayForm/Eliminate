...
export async function resolveMarketplaceHeaders(version: string, productService: IProductService, environmentService: IEnvironmentService, configurationService: IConfigurationService, fileService: IFileService, storageService: IStorageService | undefined, telemetryService: ITelemetryService): Promise<IHeaders> {
    ...
    if (supportsTelemetry(productService, environmentService) && getTelemetryLevel(configurationService) === TelemetryLevel.USAGE) {
        ...
        ({
            ...
            await getServiceMachineId(environmentService, fileService, storageService));
        ...
        ({
            'X-Market-Client-Id': `VSCode ${version}`,
            'User-Agent': `VSCode ${version} (${productService.nameShort})`
        }['VSCode-SessionId'] = telemetryService.machineId ||
            await getServiceMachineId(environmentService, fileService, storageService));
    }
    return {
        'X-Market-Client-Id': `VSCode ${version}`,
