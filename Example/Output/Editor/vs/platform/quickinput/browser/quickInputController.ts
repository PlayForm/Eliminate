...
import { IInputBox, IInputOptions, IKeyMods, IPickOptions, IQuickInput, IQuickInputButton, IQuickNavigateConfiguration, IQuickPick, IQuickPickItem, IQuickWidget, QuickInputHideReason, QuickPickFocus, QuickPickInput, } from "../common/quickInput.js";
import { backButton, EndOfQuickInputBoxContextKey, InputBox, InQuickInputContextKey, IQuickInputOptions, IQuickInputStyles, QuickInputTypeContextKey, QuickInputUI, QuickPick, QuickWidget, Visibilities, Writeable, } from "./quickInput.js";
...
export class QuickInputController extends Disposable {
    ...
    private getUI(showInActiveContainer?: boolean) {
        if (this.ui) {
            ...
            if (showInActiveContainer) {
                if (dom.getWindow(this._container) !==
                ...
                case KeyCode.Tab:
                    if (!event.altKey &&
                        ...
                        if (event.shiftKey &&
                            ...
                            dom.isAncestor(event.target, stops[stops.length - 1])) {
                            dom.EventHelper.stop(event, true);
                            stops[0].focus();
                        }
                    }
    ...
    pick<T extends IQuickPickItem, O extends IPickOptions<T>>(picks: Promise<QuickPickInput<T>[]> | QuickPickInput<T>[], options: IPickOptions<T> = {}, token: CancellationToken = CancellationToken.None): Promise<(O extends {
        canPickMany: true;
    } ? T[] : T) | undefined> {
        type R = (O extends {
            canPickMany: true;
        ...
        return new Promise<R>((doResolve, reject) => {
            ...
            const disposables = [
                ...
                input.onDidChangeValue((value) => {
                    if (activeItem &&
                        !value &&
                        (input.activeItems.length !== 1 ||
                            input.activeItems[0] !== activeItem)) {
            ...
            input.matchOnLabel =
                options.matchOnLabel === undefined || options.matchOnLabel; // default to true
    ...
    createQuickPick<T extends IQuickPickItem>(options: {
        useSeparators: boolean;
    } = { useSeparators: false }): IQuickPick<T, {
        useSeparators: boolean;
    }> {
    ...
    private setVisibilities(visibilities: Visibilities) {
        ...
        ui.customButtonContainer.style.display = visibilities.customButton
            ? ""
            : "none";
        ui.message.style.display = visibilities.message ? "" : "none";
        ui.progressBar.getContainer().style.display = visibilities.progressBar
            ? ""
            : "none";
    ...
    toggle() {
        if (this.isVisible() &&
            this.controller instanceof QuickPick &&
            this.controller.canSelectMany) {
    ...
    private updateStyles() {
        if (this.ui) {
            ...
            this.ui.container.style.border = widgetBorder
                ? `1px solid ${widgetBorder}`
                : "";
            this.ui.container.style.boxShadow = widgetShadow
                ? `0 0 8px 2px ${widgetShadow}`
                : "";
