...
export class LoggerChannelClient extends AbstractLoggerService implements ILoggerService {
    constructor(private readonly windowId: number | undefined, logLevel: LogLevel, logsHome: URI, loggers: ILoggerResource[], private readonly channel: IChannel) {
        ...
        this._register(channel.listen<LogLevel | [
            URI,
            LogLevel
        ]>("onDidChangeLogLevel", windowId)((arg) => {
            if (isLogLevel(arg)) {
        ...
        this._register(channel.listen<[
            URI,
            boolean
        ]>("onDidChangeVisibility", windowId)(([resource, visibility]) => super.setVisibility(URI.revive(resource), visibility)));
    ...
    listen(context: any, event: string): Event<any> {
        ...
        switch (event) {
            case "onDidChangeLoggers":
                return Event.map<DidChangeLoggersEvent, DidChangeLoggersEvent>(this.loggerService.onDidChangeLoggers, (e) => ({
            ...
            case "onDidChangeLogLevel":
                return Event.map<LogLevel | [
                    URI,
                    LogLevel
                ], LogLevel | [
                    URI,
                    LogLevel
                ]>(this.loggerService.onDidChangeLogLevel, (e) => isLogLevel(e)
                    ? e
...
export class RemoteLoggerChannelClient extends Disposable {
    constructor(loggerService: ILoggerService, channel: IChannel) {
        ...
        this._register(channel.listen<[
            URI,
            boolean
        ]>("onDidChangeVisibility")(([resource, visibility]) => loggerService.setVisibility(URI.revive(resource), visibility)));
