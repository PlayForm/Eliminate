...
function formatUsageTexts(usageTexts: [
    string,
    string
][], columns: number) {
...
function wrapText(text: string, columns: number): string[] {
    const lines: string[] = [];
    while (text.length) {
        const index = text.length < columns
            ? text.length
...
export function buildHelpMessage(productName: string, executableName: string, version: string, options: OptionDescriptions<any>, capabilities?: {
    noPipe?: boolean;
    noInputFiles: boolean;
}): string {
    const columns = (process.stdout.isTTY && process.stdout.columns) || 80;
    const inputFiles = capabilities?.noInputFiles !== true
