...
export function extractLocalHostUriMetaDataForPortMapping(uri: URI): {
    address: string;
    port: number;
} | undefined {
...
export function extractQueryLocalHostUriMetaDataForPortMapping(uri: URI): {
    address: string;
    port: number;
} | undefined {
...
export class DisposableTunnel {
    ...
    constructor(public readonly remoteAddress: {
        port: number;
        host: string;
    }, public readonly localAddress: {
        port: number;
        host: string;
    } | string, private readonly _dispose: () => Promise<void>) { }
...
export abstract class AbstractTunnelService extends Disposable implements ITunnelService {
    ...
    private _onTunnelClosed: Emitter<{
        host: string;
        port: number;
    }> = new Emitter();
    public onTunnelClosed: Event<{
        host: string;
        port: number;
    ...
    protected readonly _tunnels = new Map</*host*/ string, Map</* port */ number, {
        refcount: number;
    ...
    protected readonly configurationService: IConfigurationService) { super(); }
    ...
    private async tryDisposeTunnel(remoteHost: string, remotePort: number, tunnel: {
        refcount: number;
        readonly value: Promise<RemoteTunnel | string | undefined>;
    }): Promise<void> {
