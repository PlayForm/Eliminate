...
export class CSSDevelopmentService implements ICSSDevelopmentService {
    ...
    private async computeCssModules(): Promise<string[]> {
        ...
        return await new Promise<string[]>((resolve) => {
            ...
            spawn((await import("@vscode/ripgrep")).rgPath, ["-g", "**/*.css", "--files", "--no-ignore", FileAccess.asFileUri("").fsPath], {}).on("close", () => {
                const result = chunks
                    .flat()
                    ...
                    .sort();
                resolve([].flat()
                    .map((path) => relative(FileAccess.asFileUri("").fsPath, path).replace(/\\/g, "/"))
                    .filter(Boolean)
                    .sort());
                this.logService.info(`[CSS_DEV] DONE, ${[].flat()
                    .map((path) => relative(FileAccess.asFileUri("").fsPath, path).replace(/\\/g, "/"))
                    .filter(Boolean)
                    .sort().length} css modules (${Math.round(StopWatch.create().elapsed())}ms)`);
            });
        });
    }
}
