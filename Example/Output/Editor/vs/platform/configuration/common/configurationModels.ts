...
import { ConfigurationScope, Extensions, IConfigurationPropertySchema, IConfigurationRegistry, OVERRIDE_PROPERTY_REGEX, overrideIdentifiersFromKey, } from "./configurationRegistry.js";
...
export class ConfigurationModel implements IConfigurationModel {
    ...
    constructor(private readonly _contents: any, private readonly _keys: string[], private readonly _overrides: IOverrides[], readonly raw: ReadonlyArray<IStringDictionary<any> | ConfigurationModel> | undefined, private readonly logService: ILogService) { }
    ...
    get rawConfiguration(): ConfigurationModel {
        if (!this._rawConfiguration) {
            if (this.raw?.length) {
                ...
                this._rawConfiguration = rawConfigurationModels.reduce((previous, current) => current === previous
                    ? current
                    : previous.merge(current), rawConfigurationModels[0]);
            }
            else {
        ...
        return {
            ...
            get override() {
                return overrideIdentifier
                    ? freeze(that.rawConfiguration.getOverrideValue<V>(section, overrideIdentifier))
                    : undefined;
            },
            get merged() {
    ...
    merge(...others: ConfigurationModel[]): ConfigurationModel {
        ...
        return new ConfigurationModel(contents, keys, overrides, raws.every((raw) => raw instanceof ConfigurationModel)
            ? undefined
            : raws, this.logService);
    }
...
export class ConfigurationModelParser {
    ...
    constructor(protected readonly _name: string, protected readonly logService: ILogService) { }
    ...
    private filter(properties: any, configurationProperties: {
        [qualifiedKey: string]: IConfigurationPropertySchema | undefined;
    }, filterOverriddenProperties: boolean, options?: ConfigurationParseOptions): {
        raw: {};
        restricted: string[];
        hasExcludedProperties: boolean;
    } {
...
export class UserSettings extends Disposable {
    ...
    constructor(private readonly userSettingsResource: URI, protected parseOptions: ConfigurationParseOptions, extUri: IExtUri, private readonly fileService: IFileService, private readonly logService: ILogService) {
        super();
        ...
        this._register(Event.any(Event.filter(this.fileService.onDidFilesChange, (e) => e.contains(this.userSettingsResource)), Event.filter(this.fileService.onDidRunOperation, (e) => (e.isOperation(FileOperation.CREATE) ||
...
class ConfigurationInspectValue<V> implements IConfigurationValue<V> {
    constructor(private readonly key: string, private readonly overrides: IConfigurationOverrides, private readonly _value: V | undefined, readonly overrideIdentifiers: string[] | undefined, private readonly defaultConfiguration: ConfigurationModel, private readonly policyConfiguration: ConfigurationModel | undefined, private readonly applicationConfiguration: ConfigurationModel | undefined, private readonly userConfiguration: ConfigurationModel, private readonly localUserConfiguration: ConfigurationModel, private readonly remoteUserConfiguration: ConfigurationModel, private readonly workspaceConfiguration: ConfigurationModel | undefined, private readonly folderConfigurationModel: ConfigurationModel | undefined, private readonly memoryConfigurationModel: ConfigurationModel) { }
    ...
    private get userLocalInspectValue(): InspectValue<V> {
        if (!this._userLocalInspectValue) {
            this._userLocalInspectValue =
                this.localUserConfiguration.inspect<V>(this.key, this.overrides.overrideIdentifier);
        }
    ...
    private get userRemoteInspectValue(): InspectValue<V> {
        if (!this._userRemoteInspectValue) {
            this._userRemoteInspectValue =
                this.remoteUserConfiguration.inspect<V>(this.key, this.overrides.overrideIdentifier);
        }
    ...
    private get workspaceInspectValue(): InspectValue<V> | null {
        if (this._workspaceInspectValue === undefined) {
            this._workspaceInspectValue = this.workspaceConfiguration
                ? this.workspaceConfiguration.inspect<V>(this.key, this.overrides.overrideIdentifier)
                : null;
        }
    ...
    private get workspaceFolderInspectValue(): InspectValue<V> | null {
        if (this._workspaceFolderInspectValue === undefined) {
            this._workspaceFolderInspectValue = this.folderConfigurationModel
                ? this.folderConfigurationModel.inspect<V>(this.key, this.overrides.overrideIdentifier)
                : null;
        }
...
export class Configuration {
    ...
    inspect<C>(key: string, overrides: IConfigurationOverrides, workspace: Workspace | undefined): IConfigurationValue<C> {
        ...
        return new ConfigurationInspectValue<C>(key, overrides, consolidateConfigurationModel.getValue<C>(key), overrideIdentifiers.size ? [...overrideIdentifiers] : undefined, this._defaultConfiguration, this._policyConfiguration.isEmpty()
            ? undefined
            : this._policyConfiguration, this.applicationConfiguration.isEmpty()
            ? undefined
            : this.applicationConfiguration, this.userConfiguration, this.localUserConfiguration, this.remoteUserConfiguration, workspace ? this._workspaceConfiguration : undefined, folderConfigurationModel ? folderConfigurationModel : undefined, memoryConfigurationModel);
    }
        ...
        for (const key of keys) {
            for (const overrideIdentifier of overrideIdentifiersFromKey(key)) {
                ...
                const keys = [
                    ...
                    ...fromKeys.filter((key) => !objects.equals(this._defaultConfiguration
                        .override(overrideIdentifier)
                        .getValue(key), defaults
                        .override(overrideIdentifier)
                        .getValue(key))),
                ];
    ...
    private getWorkspaceConsolidatedConfiguration(): ConfigurationModel {
        if (!this._workspaceConsolidatedConfiguration) {
            this._workspaceConsolidatedConfiguration =
                this._defaultConfiguration.merge(this.applicationConfiguration, this.userConfiguration, this._workspaceConfiguration, this._memoryConfiguration);
        }
    ...
    static parse(data: IConfigurationData, logService: ILogService): Configuration {
        ...
        return new Configuration(defaultConfiguration, policyConfiguration, applicationConfiguration, userConfiguration, ConfigurationModel.createEmptyModel(logService), workspaceConfiguration, folders, ConfigurationModel.createEmptyModel(logService), new ResourceMap<ConfigurationModel>(), logService);
    }
...
export class ConfigurationChangeEvent implements IConfigurationChangeEvent {
    ...
    constructor(readonly change: IConfigurationChange, private readonly previous: {
        workspace?: Workspace;
        data: IConfigurationData;
    } | undefined, private readonly currentConfiguraiton: Configuration, private readonly currentWorkspace: Workspace | undefined, private readonly logService: ILogService) {
...
function compareConfigurationContents(to: {
    keys: string[];
    contents: any;
} | undefined, from: {
    keys: string[];
