...
function transformIncomingOptions<O extends {
    profileLocation?: UriComponents;
}>(options: O | undefined, transformer: IURITransformer | null): O | undefined {
...
function transformOutgoingExtension(extension: ILocalExtension, transformer: IURITransformer | null): ILocalExtension {
    return transformer
        ? cloneAndChange(extension, (value) => value instanceof URI
    ...
    listen(context: any, event: string): Event<any> {
        ...
        switch (event) {
            ...
            case "onDidInstallExtensions": {
                return Event.map<readonly InstallExtensionResult[], readonly InstallExtensionResult[]>(this.onDidInstallExtensions, (results) => results.map((i) => ({
                    ...i,
                    local: i.local
...
export class ExtensionManagementChannelClient extends Disposable implements IExtensionManagementService {
    ...
    constructor(private readonly channel: IChannel) {
        super();
        this._register(this.channel.listen<InstallExtensionEvent>("onInstallExtension")((e) => this.onInstallExtensionEvent({
            ...e,
            source: this.isUriComponents(e.source)
                ? URI.revive(e.source)
                : e.source,
            profileLocation: URI.revive(e.profileLocation),
        })));
        this._register(this.channel.listen<readonly InstallExtensionResult[]>("onDidInstallExtensions")((results) => this.onDidInstallExtensionsEvent(results.map((e) => ({
            ...e,
            local: e.local
                ? transformIncomingExtension(e.local, null)
                : e.local,
            source: this.isUriComponents(e.source)
                ? URI.revive(e.source)
                : e.source,
        ...
        this._register(this.channel.listen<UninstallExtensionEvent>("onUninstallExtension")((e) => this.onUninstallExtensionEvent({
            ...e,
            profileLocation: URI.revive(e.profileLocation),
        })));
        this._register(this.channel.listen<DidUninstallExtensionEvent>("onDidUninstallExtension")((e) => this.onDidUninstallExtensionEvent({
            ...e,
            profileLocation: URI.revive(e.profileLocation),
        })));
        this._register(this.channel.listen<DidUpdateExtensionMetadata>("onDidUpdateExtensionMetadata")((e) => this.onDidUpdateExtensionMetadataEvent({
            profileLocation: URI.revive(e.profileLocation),
    ...
    zip(extension: ILocalExtension): Promise<URI> {
        return Promise.resolve(this.channel
            .call<UriComponents>("zip", [extension])
            .then((result) => URI.revive(result)));
    }
    ...
    installFromLocation(location: URI, profileLocation: URI): Promise<ILocalExtension> {
        return Promise.resolve(this.channel.call<ILocalExtension>("installFromLocation", [
            location,
            profileLocation,
    ...
    installFromGallery(extension: IGalleryExtension, installOptions?: InstallOptions): Promise<ILocalExtension> {
        return Promise.resolve(this.channel.call<ILocalExtension>("installFromGallery", [
            extension,
            installOptions,
        ])).then((local) => transformIncomingExtension(local, null));
    }
    ...
    reinstallFromGallery(extension: ILocalExtension): Promise<ILocalExtension> {
        return Promise.resolve(this.channel.call<ILocalExtension>("reinstallFromGallery", [
            extension,
        ])).then((local) => transformIncomingExtension(local, null));
    }
    getInstalled(type: ExtensionType | null = null, extensionsProfileResource?: URI, productVersion?: IProductVersion): Promise<ILocalExtension[]> {
        return Promise.resolve(this.channel.call<ILocalExtension[]>("getInstalled", [
            type,
    ...
    updateMetadata(local: ILocalExtension, metadata: Partial<Metadata>, extensionsProfileResource?: URI): Promise<ILocalExtension> {
        return Promise.resolve(this.channel.call<ILocalExtension>("updateMetadata", [
            local,
            metadata,
            extensionsProfileResource,
    ...
    resetPinnedStateForAllUserExtensions(pinned: boolean): Promise<void> {
        return this.channel.call<void>("resetPinnedStateForAllUserExtensions", [
            pinned,
        ]);
    }
    toggleAppliationScope(local: ILocalExtension, fromProfileLocation: URI): Promise<ILocalExtension> {
        return this.channel
            .call<ILocalExtension>("toggleAppliationScope", [
            local,
            fromProfileLocation,
        ])
            .then((extension) => transformIncomingExtension(extension, null));
    ...
    copyExtensions(fromProfileLocation: URI, toProfileLocation: URI): Promise<void> {
        return this.channel.call<void>("copyExtensions", [
            fromProfileLocation,
            toProfileLocation,
        ]);
    }
    ...
    async download(extension: IGalleryExtension, operation: InstallOperation, donotVerifySignature: boolean): Promise<URI> {
        const result = await this.channel.call<UriComponents>("download", [
            extension,
            operation,
