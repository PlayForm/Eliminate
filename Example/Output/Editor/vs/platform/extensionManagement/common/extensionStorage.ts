...
export class ExtensionStorageService extends Disposable implements IExtensionStorageService {
    ...
    private readonly logService: ILogService) {
        ...
        this.extensionsWithKeysForSync =
            ExtensionStorageService.readAllExtensionsWithKeysForSync(storageService);
    ...
    private onDidChangeStorageValue(e: IProfileStorageValueChangeEvent): void {
        ...
        if (extensionIdWithVersion) {
            if (this.storageService.get(e.key, StorageScope.PROFILE) ===
                undefined) {
    ...
    addToMigrationList(from: string, to: string): void {
        if (from !== to) {
            ...
            const migrationList: [
                string,
                string
            ][] = this.migrationList.filter((entry) => !entry.includes(from) && !entry.includes(to));
    ...
    private set migrationList(migrationList: [
        string,
        string
    ][]) {
