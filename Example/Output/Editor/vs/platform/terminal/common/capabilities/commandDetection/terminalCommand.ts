...
export class TerminalCommand implements ITerminalCommand {
    ...
    constructor(private readonly _xterm: Terminal, private readonly _properties: ITerminalCommandProperties) { }
    static deserialize(xterm: Terminal, serialized: ISerializedTerminalCommand & Required<Pick<ISerializedTerminalCommand, "endLine">>, isCommandStorageDisabled: boolean): TerminalCommand | undefined {
        ...
        const promptStartMarker = serialized.promptStartLine !== undefined
            ? xterm.registerMarker(serialized.promptStartLine -
        ...
        const executedMarker = serialized.executedLine !== undefined
            ? xterm.registerMarker(serialized.executedLine -
                (buffer.baseY + buffer.cursorY))
            : undefined;
    ...
    getOutputMatch(outputMatcher: ITerminalOutputMatcher): ITerminalOutputMatch | undefined {
        ...
        const linesToCheck = typeof matcher === "string"
            ? 1
...
export class PartialTerminalCommand implements ICurrentPartialCommand {
    ...
    promoteToFullCommand(cwd: string | undefined, exitCode: number | undefined, ignoreCommandLine: boolean, markProperties: IMarkProperties | undefined): TerminalCommand | undefined {
        ...
        if ((this.command !== undefined && !this.command.startsWith("\\")) ||
            ignoreCommandLine) {
    ...
    markFinishedTime() {
        if (this.commandDuration === undefined &&
...
function extractCommandLine(buffer: IBuffer, cols: number, commandStartMarker: IXtermMarker | undefined, commandStartX: number | undefined, commandExecutedMarker: IXtermMarker | undefined, commandExecutedX: number | undefined): string {
...
function getCommandRowCount(command: ITerminalCommand | ICurrentPartialCommand): number {
    const marker = "hasOutput" in command ? command.marker : command.commandStartMarker;
    const executedMarker = "hasOutput" in command
