...
export class PromptInputModel extends Disposable implements IPromptInputModel {
    ...
    constructor(private readonly _xterm: Terminal, onCommandStart: Event<ITerminalCommand>, onCommandExecuted: Event<ITerminalCommand>, 
    @ILogService
    private readonly _logService: ILogService) {
        ...
        this._register(onCommandStart((e) => this._handleCommandStart(e as {
            marker: IMarker;
        })));
    ...
    private _handleCommandStart(command: {
        marker: IMarker;
    }) {
    ...
    private _doSync() {
        ...
        for (let y = absoluteCursorY + 1; y < buffer.baseY + this._xterm.rows; y++) {
            ...
            const lineText = line?.translateToString(true);
            if (lineText && line) {
                if (this._continuationPrompt === undefined ||
                    this._lineContainsContinuationPrompt(lineText)) {
        ...
        // Adjust trailing whitespace
        {
            ...
            if (isMultiLine) {
                ...
                const continuationOffset = (valueLines.length - 1) *
    ...
    private _lineContainsContinuationPrompt(lineText: string): boolean {
        return !!(this._continuationPrompt &&
    ...
    private _getRelativeCursorIndex(startCellX: number, buffer: IBuffer, line: IBufferLine): number {
        return (line?.translateToString(true, startCellX, buffer.cursorX).length ??
            0);
    }
