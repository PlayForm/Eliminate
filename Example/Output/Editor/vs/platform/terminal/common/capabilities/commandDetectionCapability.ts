...
export class CommandDetectionCapability extends Disposable implements ICommandDetectionCapability {
    ...
    private _handleCursorMove() {
        ...
        if (this._terminal.buffer.active === this._terminal.buffer.normal &&
            ...
            if (this._terminal.buffer.active.baseY +
    ...
    getCommandForLine(line: number): ITerminalCommand | ICurrentPartialCommand | undefined {
        ...
        if ((this._commands[0].promptStartMarker ?? this._commands[0].marker!)
            .line > line) {
            return undefined;
        }
        // Iterate backwards through commands to find the right one
        for (let i = this.commands.length - 1; i >= 0; i--) {
            if ((this.commands[i].promptStartMarker ?? this.commands[i].marker!)
                .line <= line) {
    ...
    handleCommandStart(options?: IHandleCommandOptions): void {
        ...
        if (this._currentCommand.commandStartMarker?.line ===
            this._terminal.buffer.active.cursorY) {
    ...
    handleCommandFinished(exitCode: number | undefined, options?: IHandleCommandOptions): void {
        ...
        if (this._currentCommand.commandStartMarker === undefined ||
            ...
            if (!e.endLine) {
                ...
                this._currentCommand.promptStartMarker =
                    e.promptStartLine !== undefined
                        ? this._terminal.registerMarker(e.promptStartLine -
...
// All common methods
(UnixPtyHeuristics | WindowsPtyHeuristics) & IDisposable;
/**
 * Non-Windows-specific behavior.
 */
class UnixPtyHeuristics extends Disposable {
    constructor(private readonly _terminal: Terminal, private readonly _capability: CommandDetectionCapability, private readonly _hooks: ICommandDetectionHeuristicsHooks, private readonly _logService: ILogService) {
        super();
...
class WindowsPtyHeuristics extends Disposable {
    ...
    constructor(private readonly _terminal: Terminal, private readonly _capability: CommandDetectionCapability, private readonly _hooks: ICommandDetectionHeuristicsHooks, 
    @ILogService
    private readonly _logService: ILogService) {
        super();
        this._register(_terminal.parser.registerCsiHandler({ final: "J" }, (params) => {
        ...
        if (rowsDifference > 0) {
            this._waitForCursorMove().then(() => {
                ...
                for (let i = this._capability.commands.length - 1; i >= 0; i--) {
                    ...
                    if (shiftedLine?.translateToString(true) !==
                        command.commandStartLineContent) {
                        continue;
                    }
                ...
                if (prompt) {
                    ...
                    if (typeof prompt === "object" && prompt.likelySingleLine) {
                        ...
                        this._capability.currentCommand.promptStartMarker =
                            cloneMarker(this._terminal, this._capability.currentCommand
                                .commandStartMarker);
                        ...
                        if (lastCommand &&
                            ...
                            lastCommand.endMarker = cloneMarker(this._terminal, this._capability.currentCommand
                                .commandStartMarker);
                        }
                    }
        ...
        if (!this._capability.currentCommand.commandExecutedMarker) {
            this._onCursorMoveListener.value = this._terminal.onCursorMove(() => {
                if (this._hooks.commandMarkers.length === 0 ||
                    this._hooks.commandMarkers[this._hooks.commandMarkers.length - 1].line !== this._terminal.buffer.active.cursorY) {
    ...
    postHandleCommandFinished(): void {
        ...
        for (let i = commandLine; i <= executedLine; i++) {
            ...
            for (let j = 0; j < text.length; j++) {
                ...
                // end of the line
                while (commandText.length < current &&
                    commandText[current] === " ") {
                    current++;
                }
                // Character match
                ...
                if (current === commandText.length) {
                    ...
                    currentCommand.commandExecutedMarker =
                        this._terminal.registerMarker(i -
    ...
    private _cursorOnNextLine(): boolean {
        ...
        const cursorYAbsolute = this._terminal.buffer.active.baseY +
            this._terminal.buffer.active.cursorY;
        // If the cursor position is within the last command, we should poll.
        const lastCommandYAbsolute = (lastCommand.endMarker
    ...
    private _getWindowsPrompt(y: number = this._terminal.buffer.active.baseY +
        this._terminal.buffer.active.cursorY): string | {
        prompt: string;
...
function cloneMarker(xterm: Terminal, marker: IXtermMarker, offset: number = 0): IXtermMarker | undefined {
    return xterm.registerMarker(marker.line -
