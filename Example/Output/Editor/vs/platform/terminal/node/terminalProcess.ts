...
import { FlowControlConstants, GeneralShellType, IProcessProperty, IProcessPropertyMap, IProcessReadyEvent, IProcessReadyWindowsPty, IShellLaunchConfig, ITerminalChildProcess, ITerminalLaunchError, ITerminalProcessOptions, PosixShellType, ProcessPropertyType, TerminalShellType, } from "../common/terminal.js";
...
export class TerminalProcess extends Disposable implements ITerminalChildProcess {
    ...
    constructor(readonly shellLaunchConfig: IShellLaunchConfig, cwd: string, cols: number, rows: number, env: IProcessEnvironment, 
    /**
     * environment used for `findExecutable`
     */
    private readonly _executableEnv: IProcessEnvironment, private readonly _options: ITerminalProcessOptions, 
    @ILogService
    ...
    private readonly _productService: IProductService) {
        ...
        if (isWindows) {
            ...
            this.onProcessReady((e) => {
                ...
                this._register(this._windowsShellHelper.onShellTypeChanged((e) => this._onDidChangeProperty.fire({
                    type: ProcessPropertyType.ShellType,
                    value: e,
                })));
                this._register(this._windowsShellHelper.onShellNameChanged((e) => this._onDidChangeProperty.fire({
    ...
    private async _validateExecutable(): Promise<undefined | ITerminalLaunchError> {
        ...
        const envPaths: string[] | undefined = slc.env && slc.env.PATH
    ...
    private async setupPtyProcess(shellLaunchConfig: IShellLaunchConfig, options: IPtyForkOptions, shellIntegrationInjection: IShellIntegrationConfigInjection | undefined): Promise<void> {
        ...
        this._childProcessMonitor.onDidChangeHasChildProcesses((value) => this._onDidChangeProperty.fire({
    ...
    private async _throttleKillSpawn(): Promise<void> {
        ...
        while (Date.now() - TerminalProcess._lastKillOrStart <
            Constants.KillSpawnThrottleInterval) {
            this._logService.trace("Throttling kill/spawn call");
            await timeout(Constants.KillSpawnThrottleInterval -
    ...
    private _sendProcessTitle(ptyProcess: IPty): void {
        ...
        if (sanitizedTitle.toLowerCase().startsWith("python")) {
            ...
            const shellTypeValue = posixShellTypeMap.get(sanitizedTitle) ||
    ...
    getWindowsPty(): IProcessReadyWindowsPty | undefined {
        return isWindows
            ? {
                backend: "useConpty" in this._ptyOptions &&
                    this._ptyOptions.useConpty
                    ? "conpty"
                    : "winpty",
...
class DelayedResizer extends Disposable {
    ...
    private readonly _onTrigger = this._register(new Emitter<{
        rows?: number;
        cols?: number;
    }>());
    get onTrigger(): Event<{
