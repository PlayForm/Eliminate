...
export function computeStats(requests: IUniversalWatchRequest[], failedRecursiveRequests: number, recursiveWatcher: ParcelWatcher, nonRecursiveWatcher: NodeJSWatcher): string {
    const lines: string[] = [];
...
function computeRequestStatus(requests: IUniversalWatchRequest[], watcher: ParcelWatcher | NodeJSWatcher): {
    suspended: number;
    polling: number;
} {
    let polling = 0;
...
function fillNonRecursiveWatcherStats(lines: string[], nonRecursiveWatcher: NodeJSWatcher): void {
    ...
    const activeWatchers = allWatchers.filter((watcher) => !watcher.instance.failed &&
        !watcher.instance.isReusingRecursiveWatcher);
