    ...
    private _collectContextKeysAndSubmenuIds(item: IMenuItem | ISubmenuItem): void {
        ...
        if (isIMenuItem(item)) {
            ...
            if (item.command.toggled) {
                const toggledExpression: ContextKeyExpression = (item.command.toggled as {
...
class MenuInfo extends MenuInfoSnapshot {
    constructor(_id: MenuId, private readonly _hiddenStates: PersistedMenuHideState, _collectContextKeysForSubmenus: boolean, 
    @ICommandService
    private readonly _commandService: ICommandService, 
    @IKeybindingService
    private readonly _keybindingService: IKeybindingService, 
    ...
    createActionGroups(options: IMenuActionOptions | undefined): [
        string,
        Array<MenuItemAction | SubmenuItemAction>
    ][] {
        const result: [
...
class MenuImpl implements IMenu {
    ...
    getActions(options?: IMenuActionOptions | undefined): [
        string,
