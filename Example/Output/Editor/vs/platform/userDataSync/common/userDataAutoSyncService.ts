...
import { IUserDataAutoSyncService, IUserDataManifest, IUserDataSyncEnablementService, IUserDataSyncLogService, IUserDataSyncService, IUserDataSyncStoreManagementService, IUserDataSyncStoreService, IUserDataSyncTask, UserDataAutoSyncError, UserDataSyncError, UserDataSyncErrorCode, } from "./userDataSync.js";
...
export class UserDataAutoSyncService extends Disposable implements IUserDataAutoSyncService {
    ...
    private readonly storageService: IStorageService) {
        ...
        if (this.syncUrl) {
            ...
            this._register(userDataSyncStoreManagementService.onDidChangeUserDataSyncStore(() => {
                if (!isEqual(this.syncUrl, userDataSyncStoreManagementService
            ...
            this._register(Event.filter(this.userDataSyncEnablementService
                .onDidChangeResourceEnablement, ([, enabled]) => enabled)(() => this.triggerSync(["resourceEnablement"], false, false)));
    ...
    private async onDidFinishSync(error: Error | undefined): Promise<void> {
        ...
        if (userDataSyncError instanceof UserDataAutoSyncError) {
            this.telemetryService.publicLog2<{
                code: string;
                service: string;
            }, AutoSyncErrorClassification>(`autosync/error`, {
        ...
        else if (userDataSyncError.code === UserDataSyncErrorCode.UpgradeRequired ||
    ...
    async triggerSync(sources: string[], skipIfSyncedRecently: boolean, disableCache: boolean): Promise<void> {
        ...
        return this.syncTriggerDelayer.trigger(async () => {
            ...
            const providerId = this.userDataSyncAccountService.account
            ...
            ? this.getSyncTriggerDelayTime() *
                1 *
                Math.min(Math.pow(2, this.successiveFailures), 60) /* Delay exponentially until max 1 minute */
            : this.getSyncTriggerDelayTime());
    }
    ...
    private async createAndRunSyncTask(disableCache: boolean, token: CancellationToken): Promise<void> {
        ...
        if (this.manifest === null) {
            try {
                ...
                throw new UserDataAutoSyncError(toErrorMessage(error), error instanceof UserDataSyncError
                    ? error.code
