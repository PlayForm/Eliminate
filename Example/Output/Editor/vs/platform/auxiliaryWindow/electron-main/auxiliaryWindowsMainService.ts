 ...
 *--------------------------------------------------------------------------------------------*/
import { app, BrowserWindow, BrowserWindowConstructorOptions, HandlerDetails, WebContents, } from "electron";
...
import { defaultAuxWindowState, IWindowState, WindowMode, } from "../../window/electron-main/window.js";
import { defaultBrowserWindowOptions, getLastFocused, IDefaultBrowserWindowOptionsOverrides, WindowStateValidator, } from "../../windows/electron-main/windows.js";
...
export class AuxiliaryWindowsMainService extends Disposable implements IAuxiliaryWindowsMainService {
    ...
    private registerListeners(): void {
        ...
        app.on("browser-window-created", (_event, browserWindow) => {
            ...
            // This is a main window, listen to child windows getting created to claim it
                ...
                disposables.add(Event.fromNodeEventEmitter(browserWindow.webContents, "did-create-window", (browserWindow, details) => ({
                    browserWindow,
                    details,
    ...
    registerWindow(webContents: WebContents): void {
        ...
        disposables.add(auxiliaryWindow.onDidEnterFullScreen(() => this._onDidChangeFullScreen.fire({
            window: auxiliaryWindow,
            fullscreen: true,
        })));
        disposables.add(auxiliaryWindow.onDidLeaveFullScreen(() => this._onDidChangeFullScreen.fire({
            window: auxiliaryWindow,
            fullscreen: false,
        })));
        disposables.add(auxiliaryWindow.onDidTriggerSystemContextMenu(({ x, y }) => this._onDidTriggerSystemContextMenu.fire({
            window: auxiliaryWindow,
            x,
            y,
        })));
