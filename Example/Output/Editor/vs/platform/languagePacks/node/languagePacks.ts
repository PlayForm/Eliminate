...
class LanguagePacksCache extends Disposable {
    ...
    update(extensions: ILocalExtension[]): Promise<{
        [language: string]: ILanguagePack;
    }> {
    ...
    private createLanguagePacksFromExtensions(languagePacks: {
        [language: string]: ILanguagePack;
    }, ...extensions: ILocalExtension[]): void {
    ...
    private createLanguagePacksFromExtension(languagePacks: {
        [language: string]: ILanguagePack;
    }, extension: ILocalExtension): void {
        const extensionIdentifier = extension.identifier;
        const localizations = extension.manifest.contributes &&
            extension.manifest.contributes.localizations
            ? extension.manifest.contributes.localizations
            : [];
    ...
    private updateHash(languagePack: ILanguagePack): void {
        if (languagePack) {
            ...
            for (const extension of languagePack.extensions) {
                md5.update(extension.extensionIdentifier.uuid ||
                    extension.extensionIdentifier.id).update(extension.version); // CodeQL [SM01510] The extension UUID is not sensitive info and is not manually created by a user
            }
            languagePack.hash = md5.digest("hex");
        }
    }
    private withLanguagePacks<T>(fn: (languagePacks: {
        [language: string]: ILanguagePack;
        ...
        return this.languagePacksFileLimiter.queue(() => {
            ...
            return fs.promises
                ...
                .then(undefined, (err) => err.code === "ENOENT"
                ? Promise.resolve("{}")
                : Promise.reject(err))
                .then<{
                [language: string]: ILanguagePack;
