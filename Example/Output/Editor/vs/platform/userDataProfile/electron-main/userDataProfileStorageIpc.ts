...
export class ProfileStorageChangesListenerChannel extends Disposable implements IServerChannel {
    ...
    constructor(private readonly storageMainService: IStorageMainService, private readonly userDataProfilesService: IUserDataProfilesService, private readonly logService: ILogService) {
        super();
    ...
    private registerStorageChangeListeners(): IDisposable {
        ...
        disposables.add(Event.debounce(this.storageMainService.onDidChangeProfileStorage, (changes: Map<string, {
            ...
            if (!profileChanges) {
                changes.set(e.profile.id, (profileChanges = {
                    profile: e.profile,
                    keys: [],
                    storage: e.storage,
                }));
    ...
    private onDidChangeApplicationStorage(keys: string[]): void {
        const targetChangedProfiles: IUserDataProfile[] = keys.includes(TARGET_KEY)
    ...
    private onDidChangeProfileStorage(changes: Map<string, {
        profile: IUserDataProfile;
        keys: string[];
        storage: IStorageMain;
    }>): void {
