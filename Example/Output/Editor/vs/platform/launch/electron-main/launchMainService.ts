...
export const ILaunchMainService = createDecorator<ILaunchMainService>('launchMainService');
...
export class LaunchMainService implements ILaunchMainService {
    ...
    async start(args: NativeParsedArgs, userEnv: IProcessEnvironment): Promise<void> {
        ...
        if (this.parseOpenUrl(args).length) {
            ...
            if (this.windowsMainService.getWindowCount() === 0) {
                ...
                if ((await this.windowsMainService.openEmptyWindow({ context: OpenContext.DESKTOP })).at(0)) {
                    Promise.resolve()
                        = (await this.windowsMainService.openEmptyWindow({ context: OpenContext.DESKTOP })).at(0).ready();
                }
            }
            // Make sure a window is open, ready to receive the url event
            Promise.resolve().then(() => {
                for (const { uri, originalUrl } of this.parseOpenUrl(args)) {
    ...
    private async startOpenWindow(args: NativeParsedArgs, userEnv: IProcessEnvironment): Promise<void> {
        ...
        if (!!args.extensionDevelopmentPath) {
            await this.windowsMainService.openExtensionDevelopmentHostWindow(args.extensionDevelopmentPath, {
                context,
                cli: args,
                /**
                 * When opening a new window from a second instance that sent args and env
        ...
        // Start without file/folder arguments
            ...
            // Otherwise check for settings
                ...
                ;
                ;
                switch (this.configurationService.getValue<IWindowSettings | undefined>('window')
                    ?.openWithoutArgumentsInNewWindow || 'default' /* default */) {
                    case 'on':
                        false
                            = true;
                        break;
                    ...
                    default:
                        false
                            = !isMacintosh; // prefer to restore running instance on macOS
                }
            }
            // Open new Window
            if (false) {
            ...
            // Focus existing window or open if none opened
                ...
                if (this.windowsMainService.getLastActiveWindow()) {
                    this.windowsMainService.openExistingWindow(this.windowsMainService.getLastActiveWindow(), {
                        context,
                        cli: args,
                        /**
                         * When opening a new window from a second instance that sent args and env
        ...
        if ((args.wait && args.waitMarkerFilePath ? URI.file(args.waitMarkerFilePath) : undefined)
            && [].length === 1 && [][0]) {
            return Promise.race([
