...
import { LexingError, Scanner, Token, TokenType } from "./scanner.js";
    ...
    parse(input: string): ContextKeyExpression | undefined {
        ...
        try {
            ...
            if (!this._isAtEnd()) {
                ...
                const additionalInfo = peek.type === TokenType.Str
                    ? hintUnexpectedToken
                    : undefined;
                this._parsingErrors.push({
                    ...
                    switch (expr.type) {
                        ...
                        case TokenType.Error: {
                            ...
                            for (let i = 0; i < expr.lexeme.length; i++) {
                                if (expr.lexeme.charCodeAt(i) ===
                                    CharCode.OpenParen) {
                                    parenBalance++;
                                }
                                else if (expr.lexeme.charCodeAt(i) ===
                                    CharCode.CloseParen) {
                                    parenBalance--;
                                }
                            }
                        ...
                        case TokenType.QuotedStr: {
                            ...
                            if (!isFalsyOrWhitespace(serializedValue)) {
                                ...
                                if (start !== end && start >= 0) {
                                    ...
                                    const caseIgnoreFlag = serializedValue[end + 1] === "i"
                                        ? "i"
                                        : "";
                                    try {
...
export class ContextKeyEqualsExpr implements IContextKeyExpression {
    ...
    private constructor(private readonly key: string, private readonly value: any, private negated: ContextKeyExpression | null) { }
    public cmp(other: ContextKeyExpression): number {
        if (other.type !== this.type) {
            return this.type - other.type;
        }
...
export class ContextKeyGreaterExpr implements IContextKeyExpression {
    ...
    private constructor(private readonly key: string, private readonly value: number | string, private negated: ContextKeyExpression | null) { }
    public cmp(other: ContextKeyExpression): number {
        if (other.type !== this.type) {
            return this.type - other.type;
        }
    ...
    constructor(key: string, defaultValue: T | undefined, metaOrHide?: string | true | {
        ...
        if (typeof metaOrHide === "object") {
            ...
            RawContextKey._info.push({
                ...
                type: defaultValue !== null && defaultValue !== undefined
                    ? typeof defaultValue
                    : undefined,
            });
        }
