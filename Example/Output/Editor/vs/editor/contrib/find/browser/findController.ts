...
import { CONTEXT_FIND_INPUT_FOCUSED, CONTEXT_FIND_WIDGET_VISIBLE, CONTEXT_REPLACE_INPUT_FOCUSED, FIND_IDS, FindModelBoundToEditorModel, ToggleCaseSensitiveKeybinding, TogglePreserveCaseKeybinding, ToggleRegexKeybinding, ToggleSearchScopeKeybinding, ToggleWholeWordKeybinding, } from "./findModel.js";
...
export function getSelectionSearchString(editor: ICodeEditor, seedSearchStringFromSelection: "single" | "multiple" = "single", seedSearchStringFromNonEmptySelection: boolean = false): string | null {
    ...
    if ((seedSearchStringFromSelection === "single" &&
        ...
        if (selection.isEmpty()) {
            ...
            if (wordAtPosition &&
                false === seedSearchStringFromNonEmptySelection) {
                return wordAtPosition.word;
            }
        }
        else {
            if (editor.getModel().getValueLengthInRange(selection) <
...
export class CommonFindController extends Disposable implements IEditorContribution {
            ...
            if (this._editor.hasModel()) {
                ...
                selections = selections
                    ...
                    if (selection.endColumn === 1 &&
                        ...
                        selection = selection.setEndPosition(selection.endLineNumber - 1, this._editor
                            .getModel()!
    ...
    protected async _start(opts: IFindStartOptions, newState?: INewFindReplaceState): Promise<void> {
        ...
        else if (opts.seedSearchStringFromSelection === "multiple" &&
    ...
    protected override async _start(opts: IFindStartOptions, newState?: INewFindReplaceState): Promise<void> {
        ...
        switch (this._editor.getOption(EditorOption.find).autoFindInSelection) {
            ...
            case "never":
                ...
                break;
            case "multiline": {
                const isSelectionMultipleLine = !!selection &&
                    selection.startLineNumber !== selection.endLineNumber;
...
StartFindAction.addImplementation(0, (accessor: ServicesAccessor, editor: ICodeEditor, args: any): boolean | Promise<void> => {
    ...
    return controller.start({
        ...
        seedSearchStringFromSelection: editor.getOption(EditorOption.find)
            ...
            ? "single"
            : "none",
        seedSearchStringFromNonEmptySelection: editor.getOption(EditorOption.find)
