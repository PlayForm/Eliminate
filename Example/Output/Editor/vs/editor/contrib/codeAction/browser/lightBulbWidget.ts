...
export class LightBulbWidget extends Disposable implements IContentWidget {
    ...
    private readonly _onClick = this._register(new Emitter<{
        readonly x: number;
        readonly y: number;
    ...
    private readonly _keybindingService: IKeybindingService) {
        ...
        this._register(this._editor.onMouseDown(async (e: IEditorMouseEvent) => {
            if (!e.target.element ||
                !this.lightbulbClasses.some((cls) => e.target.element &&
    ...
    public update(actions: CodeActionSet, trigger: CodeActionTrigger, atPosition: IPosition) {
        ...
        if (!lineHasSpace) {
            ...
            if (lineNumber > 1 && !isFolded(lineNumber - 1)) {
                ...
                else if (nextLineEmptyOrIndented ||
            ...
            effectiveColumnNumber = /^\S\s*$/.test(model.getLineContent(effectiveLineNumber))
                ? 2
                : 1;
        }
