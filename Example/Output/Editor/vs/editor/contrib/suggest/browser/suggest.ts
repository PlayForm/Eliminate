...
export async function provideSuggestionItems(registry: LanguageFeatureRegistry<languages.CompletionItemProvider>, model: ITextModel, position: Position, options: CompletionOptions = CompletionOptions.default, context: languages.CompletionContext = {
    triggerKind: languages.CompletionTriggerKind.Invoke,
}, token: CancellationToken = CancellationToken.None): Promise<CompletionItemModel> {
    ...
    const snippetCompletions = (async () => {
        ...
        if (options.providerFilter.size > 0 &&
