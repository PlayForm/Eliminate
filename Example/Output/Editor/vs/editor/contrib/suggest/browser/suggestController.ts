...
import { CompletionItem, ISuggestItemPreselector, Context as SuggestContext, suggestWidgetStatusbarMenu, } from "./suggest.js";
...
class LineSuffix {
    ...
    delta(position: IPosition): number {
        ...
        if (this._marker) {
            ...
            return (this._model.getLineMaxColumn(position.lineNumber) -
                position.column);
        }
    }
}
...
export class SuggestController implements IEditorContribution {
    ...
    private readonly _telemetryService: ITelemetryService) {
        ...
        this.widget = this._toDispose.add(new WindowIdleValue(getWindow(editor.getDomNode()), () => {
            ...
            this._toDispose.add(widget.onDetailsKeyDown((e) => {
                ...
                if (e
                    .toKeyCodeChord()
                    .equals(new KeyCodeChord(true, false, false, false, KeyCode.KeyC)) ||
                    (platform.isMacintosh &&
                        e
                            .toKeyCodeChord()
                            .equals(new KeyCodeChord(false, false, false, true, KeyCode.KeyC)))) {
                    e.stopPropagation();
                    return;
                }
        ...
        this._toDispose.add(this.model.onDidSuggest((e) => {
            ...
            if (e.triggerOptions.auto) {
                ...
                if (options.selectionMode === "never" ||
        ...
        const updateFromConfig = () => {
            ...
            acceptSuggestionsOnEnter.set(acceptSuggestionOnEnter === "on" ||
    ...
    protected _insertSuggestion(event: ISelectedSuggestion | undefined, flags: InsertFlags): void {
        ...
        if (Array.isArray(item.completion.additionalTextEdits)) {
            ...
            this.editor.executeEdits("suggestController.additionalTextEdits.sync", item.completion.additionalTextEdits.map((edit) => {
                ...
                if (range.startLineNumber === item.position.lineNumber &&
                    ...
                    const columnDelta = this.editor.getPosition()!.column -
            ...
            tasks.push(item
                .resolve(cts.token)
                .then(() => {
                if (!item.completion.additionalTextEdits ||
        ...
        if (!(item.completion.insertTextRules! &
            CompletionItemInsertTextRule.InsertAsSnippet)) {
        ...
        snippetController.insert(insertText, {
            ...
            undoStopAfter: false,
            adjustWhitespace: !(item.completion.insertTextRules! &
        ...
        if (item.completion.command) {
            if (item.completion.command.id === TriggerSuggestAction.id) {
                ...
                tasks.push(this._commandService
                    .executeCommand(item.completion.command.id, ...(item.completion.command.arguments
        ...
        if (flags & InsertFlags.KeepAlternativeSuggestions) {
            this._alternatives.value.set(event, (next) => {
                ...
                while (model.canUndo()) {
                    ...
                    this._insertSuggestion(next, InsertFlags.NoBeforeUndoStop |
    ...
    getOverwriteInfo(item: CompletionItem, toggleMode: boolean): {
        ...
        let replace = this.editor.getOption(EditorOption.suggest).insertMode ===
            "replace";
        if (toggleMode) {
            replace = !replace;
        }
        const overwriteBefore = item.position.column - item.editStart.column;
        const overwriteAfter = (replace ? item.editReplaceEnd.column : item.editInsertEnd.column) -
            item.position.column;
    ...
    triggerSuggestAndAcceptBest(arg: {
        ...
        Event.once(this.model.onDidTrigger)((_) => {
            ...
            this.model.onDidSuggest(({ completionModel }) => {
                ...
                this.editor.pushUndoStop();
                this._insertSuggestion({ index, item, model: completionModel }, InsertFlags.KeepAlternativeSuggestions |
