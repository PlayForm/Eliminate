...
export class SuggestDetailsWidget {
    ...
    getLayoutInfo() {
        const lineHeight = this._editor.getOption(EditorOption.suggestLineHeight) ||
    ...
    renderItem(item: CompletionItem, explainMode: boolean): void {
        ...
        // --- details
        if (detail) {
            const cappedDetail = detail.length > 100000
...
export class SuggestDetailsOverlay implements IOverlayWidget {
    ...
    constructor(readonly widget: SuggestDetailsWidget, private readonly _editor: ICodeEditor) {
    ...
    _placeAtAnchor(anchorBox: dom.IDomNodePagePosition, size: dom.Dimension, preferAlignAtTop: boolean) {
        ...
        const eastPlacement: Placement = (function () {
            ...
            const maxSizeTop = new dom.Dimension(width, bodyBox.height -
                anchorBox.top -
                info.borderHeight -
                info.verticalPadding);
            const maxSizeBottom = maxSizeTop.with(undefined, anchorBox.top +
                anchorBox.height -
                info.borderHeight -
                info.verticalPadding);
            return {
        ...
        const southPacement: Placement = (function () {
            ...
            const maxSizeBottom = new dom.Dimension(anchorBox.width - info.borderHeight, bodyBox.height -
                anchorBox.top -
                anchorBox.height -
                info.verticalPadding);
            return {
        ...
        const placement = placements.find((p) => p.fit >= 0) ??
