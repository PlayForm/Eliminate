    ...
    public isValid(model: ITextModel, selection: Selection, decorations: IEditorDecorationsCollection): boolean {
        ...
        for (let i = 0, len = decorations.length; !requestIsValid && i < len; i++) {
            ...
            if (range && range.startLineNumber === lineNumber) {
                if (range.startColumn <= startColumn &&
...
class WordHighlighter {
    ...
    constructor(editor: IActiveCodeEditor, providers: LanguageFeatureRegistry<DocumentHighlightProvider>, multiProviders: LanguageFeatureRegistry<MultiDocumentHighlightProvider>, contextKeyService: IContextKeyService, 
    @ITextModelService
    ...
    logService: ILogService) {
        ...
        this.occurrencesHighlightDelay =
            this.configurationService.getValue<number>("editor.occurrencesHighlightDelay");
        ...
        this.toUnhook.add(editor.onDidBlurEditorWidget(() => {
            ...
            else if (activeEditor.getModel()?.uri.scheme ===
    ...
    private _stopSingular(): void {
        ...
        if (this.editor.hasTextFocus()) {
            if (this.editor.getModel()?.uri.scheme !==
                Schemas.vscodeNotebookCell &&
    ...
    private _beginRenderDecorations(noDelay?: boolean): void {
        ...
        const minimumRenderTime = this.lastCursorPositionChangeTime +
            (noDelay ? 0 : this.occurrencesHighlightDelay);
        ...
        for (const editor of currentEditors) {
            ...
            if (uri && this.workerRequestValue.has(uri)) {
                ...
                WordHighlighter.storedDecorationIDs =
                    WordHighlighter.storedDecorationIDs.set(uri, newDecorationIDs);
