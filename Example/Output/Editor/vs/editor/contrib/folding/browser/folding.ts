        ...
        if (this.updateScheduler) {
            ...
            this.foldingModelPromise = this.updateScheduler
                ...
                const foldingRegionPromise = (this.foldingRegionPromise =
                    createCancelablePromise((token) => provider.compute(token)));
                return foldingRegionPromise.then((foldingRanges) => {
                    if (foldingRanges &&
                        ...
                        if (this._foldingImportsByDefault &&
                            !this._currentModelHasFoldedImports) {
                            const hasChanges = foldingRanges.setCollapsedAllOfType(FoldingRangeKind.Imports.value, true);
                            if (hasChanges) {
                                scrollState =
                                    StableEditorScrollState.capture(this.editor);
                                this._currentModelHasFoldedImports =
                                    hasChanges;
                            }
                        }
                ...
                if (selections && selections.length > 0) {
                    ...
                    for (const selection of selections) {
                        ...
                        if (this.hiddenRangeModel &&
                            ...
                            toToggle.push(...foldingModel.getAllRegionsAtLine(lineNumber, (r) => r.isCollapsed &&
    ...
    private onEditorMouseDown(e: IEditorMouseEvent): void {
        ...
        switch (e.target.type) {
            ...
            case MouseTargetType.CONTENT_EMPTY: {
                if (this._unfoldOnClickAfterEndOfLine &&
    ...
    private onEditorMouseUp(e: IEditorMouseEvent): void {
        ...
        if (region && region.startLineNumber === lineNumber) {
            ...
            if (iconClicked || isCollapsed) {
                ...
                let toToggle = [];
                if (surrounding) {
                    const filter = (otherRegion: FoldingRegion) => !otherRegion.containedBy(region) &&
...
function foldingArgumentsConstraint(args: any) {
    if (!types.isUndefined(args)) {
        ...
        if (!types.isUndefined(foldingArgs.levels) &&
            !types.isNumber(foldingArgs.levels)) {
            return false;
        }
        if (!types.isUndefined(foldingArgs.direction) &&
