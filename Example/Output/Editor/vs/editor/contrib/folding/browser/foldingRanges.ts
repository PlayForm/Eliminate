...
export class FoldingRegions {
    ...
    private ensureParentIndices() {
        if (!this._parentsComputed) {
            ...
            for (let i = 0, len = this._startIndexes.length; i < len; i++) {
                ...
                if (startLineNumber > MAX_LINE_NUMBER ||
                    endLineNumber > MAX_LINE_NUMBER) {
                    throw new Error("startLineNumber or endLineNumber must not exceed " +
                        MAX_LINE_NUMBER);
                }
                ...
                const parentIndex = parentIndexes.length > 0
    ...
    public contains(index: number, line: number) {
        return (this.getStartLineNumber(index) <= line &&
     ...
     */
    public static sanitizeAndMerge(rangesA: FoldingRegions | FoldRange[], rangesB: FoldingRegions | FoldRange[], maxLineNumber: number | undefined, selection?: SelectedLines): FoldRange[] {
...
export class FoldingRegion {
    ...
    containedBy(range: ILineRange): boolean {
        return (range.startLineNumber <= this.startLineNumber &&
