...
export class MarkdownHover implements IHoverPart {
    constructor(public readonly owner: IEditorHoverParticipant<MarkdownHover>, public readonly range: Range, public readonly contents: IMarkdownString[], public readonly isBeforeContent: boolean, public readonly ordinal: number, public readonly source: HoverSource | undefined = undefined) { }
...
class HoverSource {
    constructor(readonly hover: Hover, readonly hoverProvider: HoverProvider, readonly hoverPosition: Position) { }
...
export class MarkdownHoverParticipant implements IEditorHoverParticipant<MarkdownHover> {
    ...
    public createLoadingMessage(anchor: HoverAnchor): MarkdownHover | null {
        return new MarkdownHover(this, anchor.range, [
            new MarkdownString().appendText(nls.localize("modesContentHover.loading", "Loading...")),
        ], false, 2000);
    }
    ...
    private _getMarkdownHovers(hoverProviderRegistry: LanguageFeatureRegistry<HoverProvider>, model: ITextModel, anchor: HoverRangeAnchor, token: CancellationToken): AsyncIterableObject<MarkdownHover> {
        ...
        const markdownHovers = hoverProviderResults
            ...
            .map((item) => {
            const range = item.hover.range
                ? Range.lift(item.hover.range)
                : anchor.range;
    ...
    public updateMarkdownHoverVerbosityLevel(action: HoverVerbosityAction, index: number, focus?: boolean): Promise<{
...
class RenderedMarkdownHoverPart implements IRenderedHoverPart<MarkdownHover> {
    constructor(public readonly hoverPart: MarkdownHover, public readonly hoverElement: HTMLElement, public readonly disposables: DisposableStore) { }
...
class MarkdownRenderedHoverParts implements IRenderedHoverParts<MarkdownHover> {
    ...
    private _ongoingHoverOperations: Map<HoverProvider, {
        verbosityDelta: number;
        tokenSource: CancellationTokenSource;
    }> = new Map();
    private readonly _disposables = new DisposableStore();
    constructor(hoverParts: MarkdownHover[], hoverPartsContainer: DocumentFragment, private readonly _hoverParticipant: MarkdownHoverParticipant, private readonly _editor: ICodeEditor, private readonly _languageService: ILanguageService, private readonly _openerService: IOpenerService, private readonly _commandService: ICommandService, private readonly _keybindingService: IKeybindingService, private readonly _hoverService: IHoverService, private readonly _configurationService: IConfigurationService, private readonly _onFinishedRendering: () => void) {
    ...
    private _renderHoverParts(hoverParts: MarkdownHover[], hoverPartsContainer: DocumentFragment, onFinishedRendering: () => void): RenderedMarkdownHoverPart[] {
    ...
    private _renderHoverPart(hoverPart: MarkdownHover, onFinishedRendering: () => void): RenderedMarkdownHoverPart {
    ...
    private _renderMarkdownHover(markdownHover: MarkdownHover, onFinishedRendering: () => void): IRenderedHoverPart<MarkdownHover> {
    ...
    private _renderHoverExpansionAction(container: HTMLElement, action: HoverVerbosityAction, actionEnabled: boolean): DisposableStore {
        ...
        const actionElement = dom.append(container, $(ThemeIcon.asCSSSelector(isActionIncrease
            ? increaseHoverVerbosityIcon
            : decreaseHoverVerbosityIcon)));
    ...
    public async updateMarkdownHoverPartVerbosityLevel(action: HoverVerbosityAction, index: number, focus: boolean = true): Promise<{
        hoverPart: MarkdownHover;
        hoverElement: HTMLElement;
    } | undefined> {
        const model = this._editor.getModel();
        ...
        if (!hoverRenderedPart ||
            !hoverSource?.supportsVerbosityAction(action)) {
            return undefined;
        }
...
export function renderMarkdownHovers(context: IEditorHoverRenderContext, markdownHovers: MarkdownHover[], editor: ICodeEditor, languageService: ILanguageService, openerService: IOpenerService): IRenderedHoverParts<MarkdownHover> {
...
function renderMarkdownInContainer(editor: ICodeEditor, markdownHover: MarkdownHover, languageService: ILanguageService, openerService: IOpenerService, onFinishedRendering: () => void): IRenderedHoverPart<MarkdownHover> {
    const disposables = new DisposableStore();
