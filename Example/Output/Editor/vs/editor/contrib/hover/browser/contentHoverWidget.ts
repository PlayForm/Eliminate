...
export class ContentHoverWidget extends ResizableContentWidget {
    ...
    public isMouseGettingCloser(posx: number, posy: number): boolean {
        ...
        if (this._renderedHover.closestMouseDistance === undefined) {
            this._renderedHover.closestMouseDistance =
                computeDistanceFromPointToRectangle(this._renderedHover.initialMousePosX, this._renderedHover.initialMousePosY, widgetRect.left, widgetRect.top, widgetRect.width, widgetRect.height);
        }
    ...
    public show(renderedHover: RenderedContentHover): void {
        ...
        const hoverFocused = this._hover.containerDomNode.ownerDocument.activeElement ===
        ...
        const accessibleViewHint = hoverFocused &&
            getHoverAccessibleViewHint(this._configurationService.getValue("accessibility.verbosity.hover") === true &&
                this._accessibilityService.isScreenReaderOptimized(), this._keybindingService
