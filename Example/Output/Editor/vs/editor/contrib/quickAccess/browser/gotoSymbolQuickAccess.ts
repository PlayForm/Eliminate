...
import { DocumentSymbol, getAriaLabelForSymbol, SymbolKind, SymbolKinds, SymbolTag, } from "../../../common/languages.js";
...
export abstract class AbstractGotoSymbolQuickAccessProvider extends AbstractEditorNavigationQuickAccessProvider {
    ...
    private readonly _outlineModelService: IOutlineModelService, options: IGotoSymbolQuickAccessProviderOptions = Object.create(null)) {
        super(options);
    ...
    private doProvideWithoutEditorSymbols(context: IQuickAccessTextEditorContext, model: ITextModel, picker: IQuickPick<IGotoSymbolQuickPickItem, {
        useSeparators: true;
    ...
    private doProvideWithEditorSymbols(context: IQuickAccessTextEditorContext, model: ITextModel, picker: IQuickPick<IGotoSymbolQuickPickItem, {
        ...
        const updatePickerItems = async (positionToEnclose: Position | undefined) => {
            ...
            picker.busy = true;
            try {
                const query = prepareQuery(picker.value
                    .substr(AbstractGotoSymbolQuickAccessProvider.PREFIX.length)
                    .trim());
                ...
                if (items.length > 0) {
                    ...
                    if (positionToEnclose && query.original.length === 0) {
                        const candidate = <IGotoSymbolQuickPickItem>(findLast(items, (item) => Boolean(item.type !== "separator" &&
                            item.range &&
        ...
        if (filterBySymbolKind) {
            ...
            function updateLastSeparatorLabel(): void {
                if (lastSeparator &&
                    ...
                    lastSeparator.label = format(NLS_SYMBOL_KIND_CACHE[lastSymbolKind] ||
                        FALLBACK_NLS_SYMBOL_KIND, lastSymbolKindCounter);
                }
            }
    ...
    private compareByScore(symbolA: IGotoSymbolQuickPickItem, symbolB: IGotoSymbolQuickPickItem): number {
        if (typeof symbolA.score !== "number" &&
            typeof symbolB.score === "number") {
            return 1;
        }
        else if (typeof symbolA.score === "number" &&
            typeof symbolB.score !== "number") {
            return -1;
        }
        if (typeof symbolA.score === "number" &&
