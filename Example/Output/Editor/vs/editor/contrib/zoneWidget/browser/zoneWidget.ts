...
export abstract class ZoneWidget implements IHorizontalSashLayoutProvider {
    ...
    private _showImpl(where: Range, heightInLines: number): void {
        ...
        this.editor.changeViewZones((accessor: IViewZoneChangeAccessor) => {
            ...
            this._viewZone = new ViewZoneDelegate(viewZoneDomNode, position.lineNumber, position.column, heightInLines, (top: number) => this._onViewZoneTop(top), (height: number) => this._onViewZoneHeight(height), this.options.showInHiddenAreas, this.options.ordinal);
    ...
    private _initSash(): void {
        ...
        this._disposables.add(this._resizeSash.onDidChange((evt: ISashEvent) => {
            if (data) {
                const lineDelta = (evt.currentY - data.startY) /
                    this.editor.getOption(EditorOption.lineHeight);
                const roundedLineDelta = lineDelta < 0
                    ? Math.ceil(lineDelta)
    ...
    getHorizontalSashTop() {
        return ((this.domNode.style.height === null
            ? 0
