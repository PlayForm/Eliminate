...
import { ConfigurationChangedEvent, EditorOption, RenderLineNumbersType, } from "../../../common/config/editorOptions.js";
...
export class StickyScrollController extends Disposable implements IEditorContribution, IStickyScrollController {
    ...
    private readonly _contextKeyService: IContextKeyService) {
        ...
        this._stickyScrollVisibleContextKey =
            EditorContextKeys.stickyScrollVisible.bindTo(this._contextKeyService);
        ...
        this._register(focusTracker.onDidBlur((_) => {
            ...
            if (this._positionRevealed === false &&
    ...
    public focusNext(): void {
        if (this._focusedStickyElementIndex <
    ...
    private _registerMouseListeners(): void {
        ...
        const getMouseEventTarget = (mouseEvent: ClickLinkMouseEvent): {
            range: Range;
            ...
            if (mouseEvent.target.type !== MouseTargetType.OVERLAY_WIDGET ||
        ...
        this._register(dom.addStandardDisposableListener(stickyScrollWidgetDomNode, dom.EventType.MOUSE_MOVE, (mouseEvent: IMouseEvent) => {
            if (mouseEvent.shiftKey) {
                ...
                if (currentEndForLineIndex === null ||
        ...
        this._register(gesture.onExecute(async (e) => {
            ...
            this._instaService.invokeFunction(goToDefinitionWithLocation, e, this._editor as IActiveCodeEditor, {
    ...
    private _needsUpdate(event: IModelTokensChangedEvent) {
        ...
        for (const stickyLineNumber of stickyLineNumbers) {
            for (const range of event.ranges) {
                if (stickyLineNumber >= range.fromLineNumber &&
    ...
    private async _renderStickyScroll(rebuildFromLine?: number): Promise<void> {
        ...
        const shouldUpdateState = stickyWidgetVersion === undefined ||
    ...
    private _updateAndGetMinRebuildFromLine(rebuildFromLine: number | undefined): number | undefined {
        if (rebuildFromLine !== undefined) {
            const minRebuildFromLineOrInfinity = this._minRebuildFromLine !== undefined
        ...
        if (arrayVisibleRanges.length !== 0) {
            ...
            for (const range of candidateRanges) {
                ...
                if (end - start > 0) {
                    ...
                    if (topOfElementAtDepth > topOfEndLine &&
                        topOfElementAtDepth <= bottomOfEndLine) {
                        startLineNumbers.push(start);
                        endLineNumbers.push(end + 1);
                        if (topOfElementAtDepth >
                    ...
                    else if (bottomOfElementAtDepth > bottomOfBeginningLine &&
