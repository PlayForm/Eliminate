...
class InlineProgressWidget extends Disposable implements IContentWidget {
    ...
    constructor(private readonly typeId: string, private readonly editor: ICodeEditor, private readonly range: Range, title: string, private readonly delegate: InlineProgressDelegate) {
        super();
        this.create(title);
...
export class InlineProgressManager extends Disposable {
    ...
    constructor(private readonly id: string, private readonly _editor: ICodeEditor, 
        ...
        this._showPromise.value = disposableTimeout(() => {
            ...
            if (decorationIds.length > 0) {
                this._currentWidget.value =
                    this._instantiationService.createInstance(InlineProgressWidget, this.id, this._editor, range, title, delegate);
            }
