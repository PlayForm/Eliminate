...
export class RenameWidget implements IRenameWidget, IContentWidget, IDisposable {
    ...
    constructor(private readonly _editor: ICodeEditor, private readonly _acceptKeybindings: [
        string,
        string
    ], 
    @IThemeService
     ...
     */
    private _getSelection(where: IRange, currentName: string): {
        start: number;
        end: number;
    } {
    ...
    private async _updateRenameCandidates(candidates: ProviderResult<NewSymbolName[]>[], currentName: string, token: CancellationToken) {
        ...
        const newNames = namesListResults.flatMap((namesListResult) => namesListResult.status === "fulfilled" &&
    ...
    get domNode() {
        if (!this._domNode) {
            ...
            this._disposables.add(dom.addDisposableListener(this.input, dom.EventType.KEY_DOWN, (e) => {
                const keyEvent = new StandardKeyboardEvent(e);
                if (keyEvent.keyCode === KeyCode.LeftArrow ||
...
class RenameCandidateView {
    ...
    public static getLayoutInfo({ lineHeight }: {
        lineHeight: number;
    }): {
        totalHeight: number;
    } {
