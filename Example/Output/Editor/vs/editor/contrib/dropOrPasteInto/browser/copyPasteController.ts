...
export class CopyPasteController extends Disposable implements IEditorContribution {
    ...
    private _pasteAsActionContext?: {
        readonly preferred?: PastePreference;
    };
    ...
    private showPasteAsNoEditMessage(selections: readonly Selection[], preference: PastePreference) {
        MessageController.get(this._editor)?.showMessage(localize("pasteAsError", "No paste edits for '{0}' found", preference instanceof HierarchicalKind
            ? preference.value
        ...
        const p = createCancelablePromise(async (pToken) => {
            ...
            try {
                ...
                if (editSession.edits.length) {
                    const canShowWidget = editor.getOption(EditorOption.pasteAs)
                            ...
                            (async () => {
                                try {
                                    ...
                                    const resolved = resolveP &&
                                        (await this._pasteProgressManager.showWhile(selections[0].getEndPosition(), localize("resolveProcess", "Resolving paste edit. Click to cancel"), Promise.race([
                                            showP.p,
                                            resolveP,
                                        ]), {
                                            cancel: () => {
        ...
        const p = createCancelablePromise(async (token) => {
            ...
            try {
                ...
                const context: DocumentPasteContext = {
                    ...
                    only: preference && preference instanceof HierarchicalKind
                        ? preference
                        : undefined,
                };
                ...
                if (preference) {
                    ...
                    const selected = await this._quickInputService.pick(editSession.edits.map((edit): IQuickPickItem & {
    ...
    private async mergeInDataFromCopy(dataTransfer: VSDataTransfer, metadata: CopyMetadata | undefined, token: CancellationToken): Promise<void> {
        if (metadata?.id &&
            ...
            const toMergeDataTransfer = await CopyPasteController._currentCopyOperation
                .dataTransferPromise;
            if (token.isCancellationRequested) {
                return;
            }
