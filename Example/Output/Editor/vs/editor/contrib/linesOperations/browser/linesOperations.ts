...
export class TrimTrailingWhitespaceAction extends EditorAction {
    ...
    public run(_accessor: ServicesAccessor, editor: ICodeEditor, args: any): void {
        ...
        const model = editor.getModel();
        const trimInRegexAndStrings = config.getValue<boolean>("files.trimTrailingWhitespaceInRegexAndStrings", {
...
export class DeleteLinesAction extends EditorAction {
    ...
    private _getLinesToRemove(editor: IActiveCodeEditor): IDeleteLinesOperation[] {
        ...
        for (let i = 1; i < operations.length; i++) {
            if (previousOperation.endLineNumber + 1 >=
        ...
        rangesToDelete = rangesToDelete.map((selection) => {
            if (selection.isEmpty()) {
                if (selection.startColumn === 1) {
                    ...
                    const deleteFromColumn = selection.startLineNumber === 1
                        ? 1
    ...
    public run(_accessor: ServicesAccessor, editor: ICodeEditor): void {
        ...
        const lastSelection = selections.reduce((previousValue, currentValue) => {
            if (previousValue.isEmpty()) {
                if (previousValue.endLineNumber ===
                ...
                if (currentValue.startLineNumber >
        ...
        for (let i = 0, len = reducedSelections.length; i < len; i++) {
            ...
            const selectionEndPositionOffset = model.getLineLength(selection.endLineNumber) -
                selection.endColumn;
            if (selection.isEmpty() ||
            ...
            if (!deleteSelection.isEmpty()) {
                ...
                if (selection.isEmpty()) {
                    ...
                    if (selection.startLineNumber === selection.endLineNumber) {
                        ...
                        edits.push(EditOperation.replace(deleteSelection, trimmedLinesContent));
                        resultSelection = new Selection(selection.startLineNumber - lineOffset, selection.startColumn, selection.startLineNumber - lineOffset, trimmedLinesContent.length -
                            selectionEndPositionOffset);
                    }
                }
                if (Range.intersectRanges(deleteSelection, primaryCursor) !==
                    null) {
...
class BackwardsCompatibleRegExp {
    ...
    constructor(private readonly _pattern: string, private readonly _flags: string) {
        this._actual = null;
...
export class PascalCaseAction extends AbstractCaseAction {
    ...
    protected _modifyText(text: string, wordSeparators: string): string {
        ...
        const words = wordsWithMaintainBoundaries
            .map((word: string) => word.split(wordBoundary))
            .flat();
        return words
            .map((word: string) => word.substring(0, 1).toLocaleUpperCase() +
            word.substring(1))
...
if (SnakeCaseAction.caseBoundary.isSupported() &&
