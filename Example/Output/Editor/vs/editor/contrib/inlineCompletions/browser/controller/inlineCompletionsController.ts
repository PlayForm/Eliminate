...
import { autorun, constObservable, derived, derivedDisposable, derivedObservableWithCache, ITransaction, mapObservableArrayCached, observableFromEvent, observableSignal, runOnChange, runOnChangeWithStore, transaction, waitForState, } from "../../../../../base/common/observable.js";
...
export class InlineCompletionsController extends Disposable {
    ...
    private readonly _positions = derived(this, (reader) => this._editorObs.selections
        .read(reader)
        ?.map((s) => s.getEndPosition()) ?? [new Position(1, 1)]);
    private readonly _suggestWidgetAdaptor = this._register(new SuggestWidgetAdaptor(this.editor, () => {
    ...
    private readonly _editorDictationInProgress = observableFromEvent(this, this._contextKeyService.onDidChangeContext, () => this._contextKeyService
    ...
    private readonly _debounceValue = this._debounceService.for(this._languageFeaturesService.inlineCompletionsProvider, "InlineCompletionsDebounce", { min: 50, max: 50 });
    ...
    public readonly model = derivedDisposable<InlineCompletionsModel | undefined>(this, (reader) => {
        ...
        const model: InlineCompletionsModel = this._instantiationService.createInstance(InlineCompletionsModel, textModel, this._suggestWidgetSelectedItem, this._editorObs.versionId, this._positions, this._debounceValue, this.optionPreview, this.optionPreviewMode, this.optionMode, this._enabled, this.optionInlineEditsEnabled, this._shouldHideInlineEdit, this.editor);
        return model;
    ...
    private readonly _ghostTextWidgets = mapObservableArrayCached(this, this._stablizedGhostTexts, (ghostText, store) => derivedDisposable((reader) => this._instantiationService.createInstance(readHotReloadableExport(GhostTextView, reader), this.editor, {
        ghostText: ghostText,
    ...
    private readonly _accessibilityService: IAccessibilityService) {
        ...
        this._register(runOnChange(this._editorObs.selections, (_value, _, changes) => {
            if (changes.some((e) => e.reason === CursorChangeReason.Explicit ||
                e.source === "api")) {
        ...
        this._register(autorun((reader) => {
            ...
            const state = this.model
                .read(reader)
                ?.inlineCompletionState.read(reader);
...
.monaco-editor .ghost-text {
    ...
    private _provideScreenReaderUpdate(content: string): void {
        ...
        if (!accessibleViewShowing &&
            ...
            this.editor.getOption(EditorOption.inlineCompletionsAccessibilityVerbose)) {
    ...
    public shouldShowHoverAtViewZone(viewZoneId: string): boolean {
        return (this._ghostTextWidgets.get()[0]?.get().ownsViewZone(viewZoneId) ??
            false);
    }
    public reject() {
        transaction((tx) => {
