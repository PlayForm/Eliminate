...
import { autorun, derived, derivedHandleChanges, derivedOpts, IObservable, IReader, ITransaction, observableSignal, observableValue, recomputeInitiallyAndOnChange, subtransaction, transaction, } from "../../../../../base/common/observable.js";
...
import { InlineCompletionsSource, InlineCompletionWithUpdatedRange, } from "./inlineCompletionsSource.js";
...
export class InlineCompletionsModel extends Disposable {
    ...
    constructor(public readonly textModel: ITextModel, public readonly selectedSuggestItem: IObservable<SuggestItemInfo | undefined>, public readonly _textModelVersionId: IObservable<number | null, IModelContentChangedEvent | undefined>, private readonly _positions: IObservable<readonly Position[]>, private readonly _debounceValue: IFeatureDebounceInformation, private readonly _suggestPreviewEnabled: IObservable<boolean>, private readonly _suggestPreviewMode: IObservable<"prefix" | "subword" | "subwordSmart">, private readonly _inlineSuggestMode: IObservable<"prefix" | "subword" | "subwordSmart">, private readonly _enabled: IObservable<boolean>, private readonly _inlineEditsEnabled: IObservable<boolean>, private readonly _shouldHideInlineEdit: IObservable<boolean>, private readonly _editor: ICodeEditor, 
    ...
    private readonly _fetchInlineCompletionsPromise = derivedHandleChanges({
        ...
        const shouldUpdate = (this._enabled.read(reader) &&
    ...
    private readonly _inlineCompletionItems = derivedOpts({ owner: this }, (reader) => {
        ...
        for (const completion of c.inlineCompletions) {
            if (!completion.inlineCompletion.sourceInlineCompletion
                .isInlineEdit) {
                ...
                completion.inlineCompletion.sourceInlineCompletion
                    .isInlineEdit) {
    ...
    public readonly activeCommands = derivedOpts<Command[]>({ owner: this, equalsFn: itemsEquals() }, (r) => this.selectedInlineCompletion.read(r)?.inlineCompletion.source
        .inlineCompletions.commands ?? []);
    public readonly lastTriggerKind: IObservable<InlineCompletionTriggerKind | undefined> = this._source.inlineCompletions.map(this, (v) => v?.request.context.triggerKind);
    public readonly inlineCompletionsCount = derived<number | undefined>(this, (reader) => {
        if (this.lastTriggerKind.read(reader) ===
            InlineCompletionTriggerKind.Explicit) {
    ...
    public readonly state = derivedOpts<{
        ...
        equalsFn: (a, b) => {
            ...
            if (a.kind === "ghostText" && b.kind === "ghostText") {
                ...
                return (a.inlineEdit.equals(b.inlineEdit) &&
                    a.cursorAtInlineEdit === b.cursorAtInlineEdit);
            }
            return false;
        },
    }, (reader) => {
        ...
        if (item?.inlineEditCompletion) {
            ...
            if (!shouldShow &&
                item.inlineEditCompletion.request.context.triggerKind ===
                    InlineCompletionTriggerKind.Automatic &&
                this._shouldHideInlineEdit.read(reader) &&
                !cursorAtInlineEdit) {
                return undefined;
            }
            ...
            const currentItemIsCollapsed = !disableCollapsing &&
                cursorDist > 1 &&
                this._collapsedInlineEditId.read(reader) ===
                    item.inlineEditCompletion.semanticId;
            const commands = item.inlineEditCompletion.inlineCompletion.source
                .inlineCompletions.commands;
            const inlineEdit = new InlineEdit(edit, currentItemIsCollapsed, false, commands ?? []);
            return {
                kind: "inlineEdit",
                inlineEdit,
                inlineCompletion: item.inlineEditCompletion,
                edits: [edit],
                cursorAtInlineEdit,
            };
        }
    ...
    private async _deltaSelectedInlineCompletionIndex(delta: 1 | -1): Promise<void> {
        ...
        if (completions.length > 0) {
            const newIdx = (this.selectedInlineCompletionIndex.get() +
                delta +
                completions.length) %
                completions.length;
    ...
    private async _acceptNext(editor: ICodeEditor, getAcceptUntilIndex: (position: Position, text: string) => number, kind: PartialAcceptTriggerKind): Promise<void> {
        ...
        if (acceptUntilIndexExclusive === ghostTextVal.length &&
        ...
        try {
            ...
            try {
                ...
                const newText = editor.getModel()!.getValueInRange(replaceRange) +
                    partialGhostTextVal;
    ...
    public extractReproSample(): Repro {
        ...
        const item = this.state
            .get()
            ?.inlineCompletion?.toInlineCompletion(undefined);
        return {
            documentValue: value,
