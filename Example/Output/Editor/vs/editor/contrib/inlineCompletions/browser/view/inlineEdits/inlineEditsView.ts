...
export class InlineEditsViewAndDiffProducer extends Disposable {
    ...
    private readonly _originalModel = derivedDisposable(() => this._modelService.createModel("", null, this._modelUriGenerator.getUniqueUri())).keepObserved(this._store);
    private readonly _modifiedModel = derivedDisposable(() => this._modelService.createModel("", null, this._modelUriGenerator.getUniqueUri())).keepObserved(this._store);
    private readonly _differ = new LRUCachedFunction({ getCacheKey: JSON.stringify }, (arg: {
        original: string;
        modified: string;
    }) => {
        this._originalModel.get().setValue(arg.original);
        this._modifiedModel.get().setValue(arg.modified);
        const diffAlgo = this._diffProviderFactoryService.createDiffProvider({
    ...
    private readonly _inlineEditPromise = derived<IObservable<InlineEditWithChanges | undefined> | undefined>(this, (reader) => {
        ...
        const diffResult = this._differ.get({
            original: this._editor.getModel()!.getValueInRange(edit.range),
            modified: edit.text,
        });
        return diffResult.promiseResult.map((p) => {
            ...
            const edits = innerChanges.map((c) => new SingleTextEdit(addRangeToPos(rangeStartPos, c.originalRange), this._modifiedModel
                .get()!
                .getValueInRange(c.modifiedRange)));
    ...
    constructor(private readonly _editor: ICodeEditor, private readonly _edit: IObservable<InlineEdit | undefined>, private readonly _model: IObservable<InlineCompletionsModel | undefined>, 
...
export class InlineEditWithChanges {
    ...
    public readonly modifiedLineRange = this.lineEdit
        .toLineEdit()
        .getNewLineRanges()[0];
    constructor(public readonly originalText: AbstractText, public readonly edit: TextEdit, public readonly isCollapsed: boolean, public readonly showInlineIfPossible: boolean, public readonly commands: readonly Command[]) { }
...
export const modifiedBackgroundColor = registerColor("inlineEdit.modifiedBackground", transparent(diffInserted, 0.4), "", true);
export const border = registerColor("inlineEdit.border", {
    light: darken(editorLineHighlightBorder, 0.15),
    dark: lighten(editorLineHighlightBorder, 0.5),
...
export class InlineEditsView extends Disposable {
    ...
    constructor(private readonly _editor: ICodeEditor, private readonly _edit: IObservable<InlineEditWithChanges | undefined>, private readonly _model: IObservable<InlineCompletionsModel | undefined>, 
    @IInstantiationService
    private readonly _instantiationService: IInstantiationService, 
    @ICommandService
    ...
    private readonly _uiState = derived(this, (reader) => {
        ...
        else if (edit.showInlineIfPossible &&
    ...
    private readonly _previewTextModel = this._register(this._instantiationService.createInstance(TextModel, "", this._editor.getModel()!.getLanguageId(), {
    ...
    private readonly _previewEditor = this._register(this._instantiationService.createInstance(EmbeddedCodeEditorWidget, this._elements.editor, {
        glyphMargin: false,
        lineNumbers: "off",
    ...
    private readonly _previewEditorLayoutInfo = derived(this, (reader) => {
        ...
        const selectionTop = this._editor.getTopForLineNumber(range.startLineNumber) -
            this._editorObs.scrollTop.read(reader);
        const selectionBottom = this._editor.getTopForLineNumber(range.endLineNumberExclusive) -
            this._editorObs.scrollTop.read(reader);
        ...
        const editHeight = this._editor.getOption(EditorOption.lineHeight) *
            inlineEdit.modifiedLineRange.length;
    ...
    protected readonly _indicator = this._register(new InlineEditsIndicator(this._editorObs, derived<IInlineEditsIndicatorState | undefined>((reader) => {
        const state = this._uiState.read(reader);
        ...
        return {
            editTopLeft: edit1,
            showAlways: state.state === "collapsed" || state.state === "inline",
        };
    }), this._model));
}
