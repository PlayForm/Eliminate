...
export class InlineCompletionContextKeys extends Disposable {
    ...
    constructor(private readonly contextKeyService: IContextKeyService, private readonly model: IObservable<InlineCompletionsModel | undefined>) {
        super();
        this._register(bindContextKey(InlineCompletionContextKeys.inlineEditVisible, this.contextKeyService, (reader) => this.model.read(reader)?.inlineEditState.read(reader) !==
            undefined));
        this._register(autorun((reader) => {
            /** @description update context key: inlineCompletionVisible, suppressSuggestions */
            const model = this.model.read(reader);
            const state = model?.inlineCompletionState.read(reader);
            const isInlineCompletionVisible = !!state?.inlineCompletion &&
            ...
            if (state?.primaryGhostText && state?.inlineCompletion) {
                this.suppressSuggestions.set(state.inlineCompletion.inlineCompletion.source
