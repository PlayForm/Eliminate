...
import { IModelDeltaDecoration, InjectedTextCursorStops, ITextModel, PositionAffinity, } from "../../../../../common/model.js";
...
export class GhostTextView extends Disposable {
    ...
    constructor(private readonly _editor: ICodeEditor, private readonly _model: IGhostTextWidgetModel, 
    @ILanguageService
    ...
    private readonly uiState = derived(this, (reader) => {
        ...
        const replacedRange = ghostText instanceof GhostTextReplacement
        ...
        const tokens = syntaxHighlightingEnabled
            ? textModel.tokenization.tokenizeLineWithEdit(ghostText.lineNumber, new LineEditWithAdditionalLines(edit, additionalLines.map((l) => l.content)))
            : undefined;
        ...
        const tokenizedAdditionalLines: LineData[] = additionalLines.map((l, idx) => ({
            content: tokens?.additionalLines?.[idx] ??
    ...
    private readonly additionalLinesWidget = this._register(new AdditionalLinesWidget(this._editor, derived((reader) => {
        /** @description lines */
        const uiState = this.uiState.read(reader);
        return uiState
            ? {
...
export class AdditionalLinesWidget extends Disposable {
    ...
    private readonly editorOptionsChanged = observableSignalFromEvent("editorOptionChanged", Event.filter(this.editor.onDidChangeConfiguration, (e) => e.hasChanged(EditorOption.disableMonospaceOptimizations) ||
    ...
    constructor(private readonly editor: ICodeEditor, private readonly lines: IObservable<{
