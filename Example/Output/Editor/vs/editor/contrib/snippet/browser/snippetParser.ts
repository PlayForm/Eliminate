...
export class Scanner {
    ...
    next(): Token {
        ...
        if (Scanner.isVariableCharacter(ch)) {
            ...
            do {
                ch = this.value.charCodeAt(pos + ++len);
            } while (Scanner.isVariableCharacter(ch) ||
...
export abstract class Marker {
    ...
    appendChild(child: Marker): this {
        if (child instanceof Text &&
            ...
            (<Text>this._children[this._children.length - 1]).value +=
                child.value;
        }
        else {
...
export class Transform extends Marker {
    ...
    clone(): Transform {
        ...
        ret.regexp = new RegExp(this.regexp.source, "" +
...
export class FormatString extends Marker {
    constructor(readonly index: number, readonly shorthandName?: string, readonly ifValue?: string, readonly elseValue?: string) {
        super();
    }
    resolve(value?: string): string {
...
export class SnippetParser {
    ...
    parseFragment(value: string, snippet: TextmateSnippet): readonly Marker[] {
        ...
        snippet.walk((marker) => {
            if (marker instanceof Placeholder) {
                ...
                else if (!placeholderDefaultValues.has(marker.index) &&
    ...
    private _parseTransform(parent: TransformableMarker): boolean {
        ...
        while (true) {
            ...
            if (this._parseFormatString(transform) ||
