...
export class OneSnippet {
    ...
    constructor(private readonly _editor: IActiveCodeEditor, private readonly _snippet: TextmateSnippet, private readonly _snippetLineLeadingWhitespace: string) {
    ...
    move(fwd: boolean | undefined): Selection[] {
        ...
        if (this._placeholderGroupsIdx >= 0) {
            ...
            for (const placeholder of this._placeholderGroups[this._placeholderGroupsIdx]) {
                ...
                if (placeholder.transform) {
                    ...
                    const currentValue = this._editor
                        .getModel()
                    ...
                    for (let i = 1; i < transformedValueLines.length; i++) {
                        transformedValueLines[i] = this._editor
                            .getModel()
                            .normalizeIndentation(this._snippetLineLeadingWhitespace +
                            transformedValueLines[i]);
                    }
        ...
        const newSelections = this._editor
            .getModel()
            .changeDecorations((accessor) => {
                ...
                for (const nestedPlaceholder of nested._snippet.placeholderInfo
                    ...
                    if (nestedPlaceholder.isFinalTabstop) {
                        nestedPlaceholder.index =
                            placeholder.index +
                                (indexLastPlaceholder + 1) / this._nestingLevel;
                    }
                    else {
                        nestedPlaceholder.index =
                            placeholder.index +
                                nestedPlaceholder.index / this._nestingLevel;
                    }
                }
...
export class SnippetSession {
        ...
        snippet.walk((marker) => {
            ...
            if (adjustIndentation) {
                ...
                if (offset === 0) {
                    ...
                    if (prevChar === CharCode.LineFeed ||
                        prevChar === CharCode.CarriageReturn) {
    ...
    static createEditsAndSnippetsFromSelections(editor: IActiveCodeEditor, template: string, overwriteBefore: number, overwriteAfter: number, enforceFinalTabstop: boolean, adjustWhitespace: boolean, clipboardText: string | undefined, overtypingCapturer: OvertypingCapturer | undefined, languageConfigurationService: ILanguageConfigurationService): {
        ...
        for (const { selection, idx } of indexedSelections) {
            ...
            const snippetLineLeadingWhitespace = SnippetSession.adjustWhitespace(model, start, adjustWhitespace ||
                (idx > 0 &&
                    firstLineFirstNonWhitespace !==
                        model.getLineFirstNonWhitespaceColumn(selection.positionLineNumber)), snippet);
            snippet.resolveVariables(new CompositeSnippetVariableResolver([
                modelBasedVariableResolver,
                new ClipboardBasedVariableResolver(readClipboardText, idx, indexedSelections.length, editor.getOption(EditorOption.multiCursorPaste) ===
                    "spread"),
    ...
    constructor(private readonly _editor: IActiveCodeEditor, private readonly _template: string | ISnippetEdit[], private readonly _options: ISnippetSessionInsertOptions = _defaultOptions, 
    @ILanguageConfigurationService
