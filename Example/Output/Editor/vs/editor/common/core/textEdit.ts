...
export class TextEdit {
    ...
    public static insert(position: Position, newText: string): TextEdit {
        return new TextEdit([
            new SingleTextEdit(Range.fromPositions(position, position), newText),
        ]);
    }
    constructor(public readonly edits: readonly SingleTextEdit[]) {
        assertFn(() => checkAdjacentItems(edits, (a, b) => a.range
            .getEndPosition()
    ...
    mapPosition(position: Position): Position | Range {
        ...
        for (const edit of this.edits) {
            ...
            if (position.isBefore(end)) {
                const startPos = new Position(start.lineNumber + lineDelta, start.column +
...
export class SingleTextEdit {
    ...
    public extendToFullLine(initialValue: AbstractText): SingleTextEdit {
        const newRange = new Range(this.range.startLineNumber, 1, this.range.endLineNumber, initialValue
            .getTransformer()
            .getLineLength(this.range.endLineNumber) + 1);
...
export class LineBasedText extends AbstractText {
    constructor(private readonly _getLineContent: (lineNumber: number) => string, private readonly _lineCount: number) {
        assert(_lineCount >= 1);
        super();
    }
...
export class BasedTextEdit {
    constructor(public readonly base: AbstractText, public readonly edit: TextEdit) { }
    toString() {
