...
import { ApplyEditsResult, EndOfLinePreference, FindMatch, IInternalModelContentChange, ISingleEditOperationIdentifier, ITextBuffer, ITextSnapshot, IValidEditOperation, SearchData, ValidAnnotatedEditOperation, } from "../../model.js";
    ...
    public getCharacterCountInRange(range: Range, eol: EndOfLinePreference = EndOfLinePreference.TextDefined): number {
        if (this._mightContainNonBasicASCII) {
            ...
            for (let lineNumber = fromLineNumber; lineNumber <= toLineNumber; lineNumber++) {
                ...
                const toOffset = lineNumber === toLineNumber
                    ? range.endColumn - 1
                    : lineContent.length;
    ...
    public applyEdits(rawOperations: ValidAnnotatedEditOperation[], recordTrimAutoWhitespace: boolean, computeUndoEdits: boolean): ApplyEditsResult {
        ...
        for (let i = 0; i < rawOperations.length; i++) {
            ...
            if (op.text) {
                ...
                if (!mightContainUnusualLineTerminators &&
