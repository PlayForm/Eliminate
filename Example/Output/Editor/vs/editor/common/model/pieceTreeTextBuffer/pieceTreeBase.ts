...
import { fixInsert, leftest, NodeColor, rbDelete, righttest, SENTINEL, TreeNode, updateTreeMetadata, } from "./rbTreeBase.js";
...
class LineStarts {
    constructor(public readonly lineStarts: Uint32Array | Uint16Array | number[], public readonly cr: number, public readonly lf: number, public readonly crlf: number, public readonly isBasicASCII: boolean) { }
}
...
export class PieceTreeBase {
    ...
    public findMatchesLineByLine(searchRange: Range, searchData: SearchData, captureMatches: boolean, limitResultCount: number): FindMatch[] {
        ...
        while (currentNode !== endPosition.node) {
            ...
            if (lineBreakCnt >= 1) {
                ...
                const startColumn = startLineNumber === searchRange.startLineNumber
    ...
    public delete(offset: number, cnt: number): void {
        ...
        if (startNode === endNode) {
            ...
            if (startPosition.nodeStartOffset + startNode.piece.length ===
                offset + cnt) {
    ...
    public getLineRawContent(lineNumber: number, endOffset: number = 0): string {
        ...
        if (cache) {
            ...
            const buffer = this._buffers[x.piece.bufferIndex].buffer;
            const startOffset = this.offsetInBuffer(x.piece.bufferIndex, x.piece.start);
            if (cache.nodeStartLineNumber + x.piece.lineFeedCnt ===
                lineNumber) {
    ...
    private getIndexOf(node: TreeNode, accumulatedValue: number): {
        index: number;
        remainder: number;
    } {
        const piece = node.piece;
    ...
    private appendToNode(node: TreeNode, value: string): void {
        ...
        const hitCRLF = this.shouldCheckCRLF() &&
        ...
        this._buffers[0].lineStarts = (<number[]>(this._buffers[0].lineStarts)).concat(<number[]>lineStarts.slice(1));
        const endIndex = this._buffers[0].lineStarts.length - 1;
        const endColumn = this._buffers[0].buffer.length -
    ...
    private nodeCharCodeAt(node: TreeNode, offset: number): number {
        ...
        const buffer = this._buffers[node.piece.bufferIndex];
        const newOffset = this.offsetInBuffer(node.piece.bufferIndex, node.piece.start) +
            offset;
    ...
    private startWithLF(val: string | TreeNode): boolean {
        ...
        return (this._buffers[piece.bufferIndex].buffer.charCodeAt(startOffset) ===
            10);
    }
    ...
    private fixCRLF(prev: TreeNode, next: TreeNode) {
        ...
        if (prev.piece.end.column === 0) {
            ...
            newEnd = {
                ...
                column: lineStarts[prev.piece.end.line] -
