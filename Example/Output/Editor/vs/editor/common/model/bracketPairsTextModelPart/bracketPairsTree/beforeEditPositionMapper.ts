...
export class BeforeEditPositionMapper {
    ...
    private translateCurToOld(newOffset: Length): Length {
        ...
        if (offsetObj.lineCount - this.deltaOldToNewLineCount ===
    ...
    private adjustNextEdit(offset: Length) {
        while (this.nextEditIdx < this.edits.length) {
            ...
            if (lengthLessThanEqual(nextEditEndOffsetInCur, offset)) {
                ...
                const lineDelta = nextEditEndOffsetInCurObj.lineCount -
                    nextEditEndOffsetBeforeInCurObj.lineCount;
                this.deltaOldToNewLineCount += lineDelta;
                const previousColumnDelta = this.deltaLineIdxInOld ===
                    nextEdit.endOffsetBeforeObj.lineCount
                    ? this.deltaOldToNewColumnCount
                    : 0;
                const columnDelta = nextEditEndOffsetInCurObj.columnCount -
