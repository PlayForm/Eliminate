...
export class BracketPairsTree extends Disposable {
    ...
    */
    ...
    public constructor(private readonly textModel: TextModel, private readonly getLanguageConfiguration: (languageId: string) => ResolvedLanguageConfiguration) {
        super();
        ...
        else if (textModel.tokenization.backgroundTokenizationState ===
    ...
    private handleEdits(edits: TextEditInfo[], tokenChange: boolean): void {
        ...
        if (this.initialAstWithoutTokens && !tokenChange) {
            this.queuedTextEditsForInitialAstWithoutTokens =
                combineTextEditInfos(this.queuedTextEditsForInitialAstWithoutTokens, edits);
        }
    }
    //#endregion
    private flushQueue() {
...
function collectBrackets(node: AstNode, nodeOffsetStart: Length, nodeOffsetEnd: Length, startOffset: Length, endOffset: Length, push: (item: BracketInfo) => boolean, level: number, nestingLevelOfEqualBracketType: number, levelPerBracketType: Map<string, number>, onlyColorizedBrackets: boolean, parentPairIsIncomplete: boolean = false): boolean {
    if (level > 200) {
        return true;
    }
        ...
        switch (node.kind) {
            ...
            case AstNodeKind.Pair: {
                const colorize = !onlyColorizedBrackets ||
                    !node.closingBracket ||
                    (node.closingBracket.bracketInfo as ClosingBracketKind).closesColorized(node.openingBracket.bracketInfo as OpeningBracketKind);
                ...
                for (let i = 0; i < childCount; i++) {
                    ...
                    if (lengthLessThanEqual(nodeOffsetStart, endOffset) &&
                        ...
                        if (colorize ||
                            child.kind !== AstNodeKind.Bracket ||
                            !node.closingBracket) {
                            const shouldContinue = collectBrackets(child, nodeOffsetStart, nodeOffsetEnd, startOffset, endOffset, push, colorize ? level + 1 : level, colorize
                                ? levelPerBracket + 1
                                : levelPerBracket, levelPerBracketType, onlyColorizedBrackets, !node.closingBracket);
                            if (!shouldContinue) {
                                return false;
                            }
                        }
...
class CollectBracketPairsContext {
    constructor(public readonly push: (item: BracketPairWithMinIndentationInfo) => boolean, public readonly includeMinIndentation: boolean, public readonly textModel: ITextModel) { }
}
function collectBracketPairs(node: AstNode, nodeOffsetStart: Length, nodeOffsetEnd: Length, startOffset: Length, endOffset: Length, context: CollectBracketPairsContext, level: number, levelPerBracketType: Map<string, number>): boolean {
    if (level > 200) {
        return true;
    }
