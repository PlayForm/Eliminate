...
export class RenderLineInput {
    ...
    public equals(other: RenderLineInput): boolean {
        return (this.useMonospaceOptimizations ===
            other.useMonospaceOptimizations &&
...
export class CharacterMapping {
    private static getPartIndex(partData: number): number {
        return ((partData & CharacterMappingConstants.PART_INDEX_MASK) >>>
            CharacterMappingConstants.PART_INDEX_OFFSET);
    }
    private static getCharIndex(partData: number): number {
        return ((partData & CharacterMappingConstants.CHAR_INDEX_MASK) >>>
...
export class RenderLineOutput2 {
    constructor(public readonly characterMapping: CharacterMapping, public readonly html: string, public readonly containsRTL: boolean, public readonly containsForeignElements: ForeignElementType) { }
}
...
function resolveRenderLineInput(input: RenderLineInput): ResolvedRenderLineInput {
    ...
    let len: number;
    if (input.stopRenderingLineAfter !== -1 &&
    ...
    if (input.renderWhitespace === RenderWhitespace.All ||
        ...
        (input.renderWhitespace === RenderWhitespace.Trailing &&
            !input.continuesWithWrappedLine)) {
...
function _applyRenderWhitespace(input: RenderLineInput, lineContent: string, len: number, tokens: LinePart[]): LinePart[] {
        ...
        if (wasInWhitespace) {
            ...
            if (!isInWhitespace ||
                ...
                if (generateLinePartForEachWhitespace) {
                    const lastEndIndex = resultLen > 0
                        ? result[resultLen - 1].endIndex
                        : fauxIndentLength;
    ...
    if (wasInWhitespace) {
        ...
        if (continuesWithWrappedLine && onlyBoundary) {
            ...
            const isSingleTrailingSpace = lastCharCode === CharCode.Space &&
...
function _applyInlineDecorations(lineContent: string, len: number, tokens: LinePart[], _lineDecorations: LineDecoration[]): LinePart[] {
    ...
    if (lineDecorationIndex < lineDecorationsLen &&
        ...
        while (lineDecorationIndex < lineDecorationsLen &&
            lineDecorations[lineDecorationIndex].startOffset ===
                lastTokenEndIndex) {
            const lineDecoration = lineDecorations[lineDecorationIndex];
        ...
        if (partRendersWhitespace) {
            let partWidth = 0;
            {
                ...
                for (; _charIndex < partEndIndex; _charIndex++) {
                    ...
                    const charWidth = (charCode === CharCode.Tab
