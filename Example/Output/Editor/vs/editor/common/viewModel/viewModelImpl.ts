...
import { ConfigurationChangedEvent, EDITOR_FONT_DEFAULTS, EditorOption, filterValidationDecorations, } from "../config/editorOptions.js";
...
export class ViewModel extends Disposable implements IViewModel {
    ...
    constructor(editorId: number, configuration: IEditorConfiguration, model: ITextModel, domLineBreaksComputerFactory: ILineBreaksComputerFactory, monospaceLineBreaksComputerFactory: ILineBreaksComputerFactory, scheduleAtNextAnimationFrame: (callback: () => void) => IDisposable, private readonly languageConfigurationService: ILanguageConfigurationService, private readonly _themeService: IThemeService, private readonly _attachedView: IAttachedView, private readonly _transactionalTarget: IBatchableTarget) {
        ...
        if (USE_IDENTITY_LINES_COLLECTION &&
            this.model.isTooLargeForTokenization()) {
    ...
    private _captureStableViewport(): StableViewport {
        ...
        if (this._viewportStart.isValid &&
            this.viewLayout.getCurrentScrollTop() > 0) {
    ...
    private _registerModelEvents(): void {
        this._register(this.model.onDidChangeContentOrInjectedText((e) => {
            try {
                ...
                const changes = e instanceof
                    textModelEvents.InternalModelContentChangeEvent
                    ? e.rawContentChangedEvent.changes
                    : e.changes;
                const versionId = e instanceof
                    textModelEvents.InternalModelContentChangeEvent
                        ...
                        case textModelEvents.RawContentChangedType
                            .LinesInserted:
                            {
                                for (let lineIdx = 0; lineIdx < change.detail.length; lineIdx++) {
                                    ...
                                    if (injectedText) {
                                        injectedText = injectedText.filter((element) => !element.ownerId ||
                                            element.ownerId ===
                                                this._editorId);
                                    }
                        ...
                        case textModelEvents.RawContentChangedType
                            .LineChanged:
                            {
                                ...
                                if (change.injectedText) {
                                    injectedText = change.injectedText.filter((element) => !element.ownerId ||
            ...
            // Recover viewport
            if (!this._hasFocus &&
                this.model.getAttachedEditorCount() >= 2 &&
        ...
        if (hasEmptyRange && emptySelectionClipboard) {
            ...
            for (const modelRange of modelRanges) {
                ...
                if (modelRange.isEmpty()) {
                    ...
                    result.push(this.model.getValueInRange(modelRange, forceCRLF
    ...
    public getRichTextToCopy(modelRanges: Range[], emptySelectionClipboard: boolean): {
        html: string;
        mode: string;
    } | null {
        ...
        const useDefaultFontFamily = hasBadChars ||
            fontInfo.fontFamily === EDITOR_FONT_DEFAULTS.fontFamily;
...
class ViewportStart implements IDisposable {
    ...
    private constructor(private readonly _model: ITextModel, private _viewLineNumber: number, private _isValid: boolean, private _modelTrackedRange: string, private _startLineDelta: number) { }
    public dispose(): void {
...
class StableViewport {
    constructor(public readonly viewportStartModelPosition: Position | null, public readonly startLineDelta: number) { }
