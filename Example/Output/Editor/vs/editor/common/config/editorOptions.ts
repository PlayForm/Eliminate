...
function applyUpdate<T>(value: T | undefined, update: T): ApplyUpdateResult<T> {
    ...
    if (Array.isArray(value) || Array.isArray(update)) {
        const arrayEquals = Array.isArray(value) &&
            Array.isArray(update) &&
            arrays.equals(value, update);
...
function _autoIndentFromString(autoIndent: "none" | "keep" | "brackets" | "advanced" | "full"): EditorAutoIndentStrategy {
    switch (autoIndent) {
        case "none":
            return EditorAutoIndentStrategy.None;
        case "keep":
            return EditorAutoIndentStrategy.Keep;
        case "brackets":
            return EditorAutoIndentStrategy.Brackets;
        case "advanced":
            return EditorAutoIndentStrategy.Advanced;
        case "full":
            return EditorAutoIndentStrategy.Full;
    }
}
//#endregion
...
class EditorFontWeight extends BaseEditorOption<EditorOption.fontWeight, string, string> {
    private static SUGGESTION_VALUES = [
        "normal",
        "bold",
        "100",
        "200",
    ...
    public validate(_input: any): GoToLocationOptions {
        ...
        return {
            ...
            multipleDefinitions: input.multipleDefinitions ??
                stringSet<GoToLocationValues>(input.multipleDefinitions, "peek", ["peek", "gotoAndPeek", "goto"]),
            multipleTypeDefinitions: input.multipleTypeDefinitions ??
                stringSet<GoToLocationValues>(input.multipleTypeDefinitions, "peek", ["peek", "gotoAndPeek", "goto"]),
            multipleDeclarations: input.multipleDeclarations ??
                stringSet<GoToLocationValues>(input.multipleDeclarations, "peek", ["peek", "gotoAndPeek", "goto"]),
            multipleImplementations: input.multipleImplementations ??
                stringSet<GoToLocationValues>(input.multipleImplementations, "peek", ["peek", "gotoAndPeek", "goto"]),
            multipleReferences: input.multipleReferences ??
                stringSet<GoToLocationValues>(input.multipleReferences, "peek", ["peek", "gotoAndPeek", "goto"]),
            multipleTests: input.multipleTests ??
                stringSet<GoToLocationValues>(input.multipleTests, "peek", [
                    "peek",
                    "gotoAndPeek",
                    "goto",
                ]),
...
export class EditorLayoutInfoComputer extends ComputedEditorOption<EditorOption.layoutInfo, EditorLayoutInfo> {
    ...
    public static computeContainedMinimapLineCount(input: {
        ...
        const desiredRatio = (extraLinesBeforeFirstLine +
            input.viewLineCount +
    ...
    private static _computeMinimapLayout(input: IMinimapLayoutInput, memory: ComputeOptionsMemory): EditorMinimapLayoutInfo {
        ...
        const couldUseMemory = stableMinimapLayoutInput &&
            ...
            input.minimap.enabled ===
                stableMinimapLayoutInput.minimap.enabled &&
            ...
            input.minimap.maxColumn ===
                stableMinimapLayoutInput.minimap.maxColumn &&
            input.minimap.scale === stableMinimapLayoutInput.minimap.scale &&
            input.verticalScrollbarWidth ===
                stableMinimapLayoutInput.verticalScrollbarWidth &&
            // && input.viewLineCount === lastMinimapLayoutInput.viewLineCount !!! INTENTIONAL OMITTED
            // && input.remainingWidth === lastMinimapLayoutInput.remainingWidth !!! INTENTIONAL OMITTED
            input.isViewportWrapping ===
                stableMinimapLayoutInput.isViewportWrapping;
        const lineHeight = input.lineHeight;
        const typicalHalfwidthCharacterWidth = input.typicalHalfwidthCharacterWidth;
        const scrollBeyondLastLine = input.scrollBeyondLastLine;
        const minimapRenderCharacters = input.minimap.renderCharacters;
        let minimapScale = pixelRatio >= 2
            ? Math.round(input.minimap.scale * 2)
            : input.minimap.scale;
        ...
        if (minimapSize === "fill" || minimapSize === "fit") {
            ...
            if (ratio > 1) {
                ...
                if (minimapSize === "fit") {
                    const effectiveMinimapHeight = Math.ceil((extraLinesBeforeFirstLine +
                        viewLineCount +
                        extraLinesBeyondLastLine) *
                        minimapLineHeight);
                    if (isViewportWrapping &&
                        ...
                        fitBecomesFill =
                            effectiveMinimapHeight > minimapCanvasInnerHeight;
                    }
                }
                if (minimapSize === "fill" || fitBecomesFill) {
                    ...
                    minimapCharWidth =
                        minimapScale / pixelRatio / minimapWidthMultiplier;
                    minimapCanvasInnerHeight = Math.ceil(Math.max(typicalViewportLineCount, extraLinesBeforeFirstLine +
                        viewLineCount +
                        extraLinesBeyondLastLine) * minimapLineHeight);
                    if (isViewportWrapping) {
        ...
        const minimapWidth = Math.min(minimapMaxWidth, Math.max(0, Math.floor(((remainingWidth - verticalScrollbarWidth - 2) *
            minimapCharWidth) /
        ...
        const minimapLeft = minimapSide === "left"
            ? 0
            : outerWidth - minimapWidth - verticalScrollbarWidth;
        return {
            renderMinimap,
            minimapLeft,
            minimapWidth,
    ...
    public static computeLayout(options: IComputedEditorOptions, env: EditorLayoutInfoComputerEnv): EditorLayoutInfo {
        ...
        const wordWrapOverride1 = wordWrapOverride2 === "inherit"
            ...
            : wordWrapOverride2;
        const wordWrap = wordWrapOverride1 === "inherit"
            ? options.get(EditorOption.wordWrap)
            : wordWrapOverride1;
        const wordWrapColumn = options.get(EditorOption.wordWrapColumn);
        const isDominatedByLongLines = env.isDominatedByLongLines;
        const showGlyphMargin = options.get(EditorOption.glyphMargin);
        const showLineNumbers = options.get(EditorOption.lineNumbers).renderType !==
        ...
        const remainingWidth = outerWidth -
            glyphMarginWidth -
            lineNumbersWidth -
        ...
        if (minimapLayout.renderMinimap !== RenderMinimap.None &&
...
class WrappingStrategy extends BaseEditorOption<EditorOption.wrappingStrategy, "simple" | "advanced", "simple" | "advanced"> {
    ...
    public validate(input: any): "simple" | "advanced" {
        return stringSet<"simple" | "advanced">(input, "simple", [
            "simple",
            "advanced",
        ]);
    }
...
class EditorLightbulb extends BaseEditorOption<EditorOption.lightbulb, IEditorLightbulbOptions, EditorLightbulbOptions> {
    constructor() {
        const defaults: EditorLightbulbOptions = {
...
class EditorStickyScroll extends BaseEditorOption<EditorOption.stickyScroll, IEditorStickyScrollOptions, EditorStickyScrollOptions> {
    ...
    public validate(_input: any): EditorStickyScrollOptions {
        ...
        return {
            ...
            defaultModel: stringSet<"outlineModel" | "foldingProviderModel" | "indentationModel">(input.defaultModel, this.defaultValue.defaultModel, [
                "outlineModel",
                "foldingProviderModel",
                "indentationModel",
            ]),
...
class EditorInlayHints extends BaseEditorOption<EditorOption.inlayHints, IEditorInlayHintsOptions, EditorInlayHintsOptions> {
    constructor() {
        const defaults: EditorInlayHintsOptions = {
            enabled: "on",
            fontSize: 0,
            fontFamily: "",
            padding: false,
    ...
    public validate(_input: any): EditorInlayHintsOptions {
        ...
        return {
            enabled: stringSet<"on" | "off" | "offUnlessPressed" | "onUnlessPressed">(input.enabled, this.defaultValue.enabled, [
                "on",
                "off",
                "offUnlessPressed",
                "onUnlessPressed",
...
class EditorLineHeight extends EditorFloatOption<EditorOption.lineHeight> {
    constructor() {
        super(EditorOption.lineHeight, "lineHeight", EDITOR_FONT_DEFAULTS.lineHeight, (x) => EditorFloatOption.clamp(x, 0, 150), {
...
class EditorMinimap extends BaseEditorOption<EditorOption.minimap, IEditorMinimapOptions, EditorMinimapOptions> {
    ...
    public validate(_input: any): EditorMinimapOptions {
        ...
        return {
            ...
            sectionHeaderFontSize: EditorFloatOption.clamp(input.sectionHeaderFontSize ??
                this.defaultValue.sectionHeaderFontSize, 4, 32),
            sectionHeaderLetterSpacing: EditorFloatOption.clamp(input.sectionHeaderLetterSpacing ??
...
class EditorRulers extends BaseEditorOption<EditorOption.rulers, (number | IRulerOption)[], IRulerOption[]> {
    constructor() {
        ...
        const columnSchema: IJSONSchema = {
            type: "number",
...
class BracketPairColorization extends BaseEditorOption<EditorOption.bracketPairColorization, IBracketPairColorizationOptions, InternalBracketPairColorizationOptions> {
    constructor() {
        const defaults: InternalBracketPairColorizationOptions = {
            ...
            independentColorPoolPerBracketType: EDITOR_MODEL_DEFAULTS.bracketPairColorizationOptions
...
class SmartSelect extends BaseEditorOption<EditorOption.smartSelect, ISmartSelectOptions, SmartSelectOptions> {
    ...
    public validate(input: any): Readonly<Required<ISmartSelectOptions>> {
        ...
        return {
            selectLeadingAndTrailingWhitespace: boolean((input as ISmartSelectOptions)
                .selectLeadingAndTrailingWhitespace, this.defaultValue.selectLeadingAndTrailingWhitespace),
...
class EditorDropIntoEditor extends BaseEditorOption<EditorOption.dropIntoEditor, IDropIntoEditorOptions, EditorDropIntoEditorOptions> {
    constructor() {
        const defaults: EditorDropIntoEditorOptions = {
            enabled: true,
...
class EditorPasteAs extends BaseEditorOption<EditorOption.pasteAs, IPasteAsOptions, EditorPasteAsOptions> {
    constructor() {
        const defaults: EditorPasteAsOptions = {
            enabled: true,
...
export const EditorOptions = {
    acceptSuggestionOnCommitCharacter: register(new EditorBooleanOption(EditorOption.acceptSuggestionOnCommitCharacter, "acceptSuggestionOnCommitCharacter", true, {
    ...
    copyWithSyntaxHighlighting: register(new EditorBooleanOption(EditorOption.copyWithSyntaxHighlighting, "copyWithSyntaxHighlighting", true, {
    ...
    cursorBlinking: register(new EditorEnumOption(EditorOption.cursorBlinking, "cursorBlinking", TextEditorCursorBlinkingStyle.Blink, "blink", ["blink", "smooth", "phase", "expand", "solid"], cursorBlinkingStyleFromString, {
    ...
    cursorStyle: register(new EditorEnumOption(EditorOption.cursorStyle, "cursorStyle", TextEditorCursorStyle.Line, "line", [
        "line",
        "block",
        "underline",
        "line-thin",
    ...
    unfoldOnClickAfterEndOfLine: register(new EditorBooleanOption(EditorOption.unfoldOnClickAfterEndOfLine, "unfoldOnClickAfterEndOfLine", false, {
    ...
    hideCursorInOverviewRuler: register(new EditorBooleanOption(EditorOption.hideCursorInOverviewRuler, "hideCursorInOverviewRuler", false, {
    ...
    matchBrackets: register(new EditorStringEnumOption(EditorOption.matchBrackets, "matchBrackets", "always" as "never" | "near" | "always", ["always", "near", "never"] as const, {
    ...
    multiCursorMergeOverlapping: register(new EditorBooleanOption(EditorOption.multiCursorMergeOverlapping, "multiCursorMergeOverlapping", true, {
    ...
    definitionLinkOpensInPeek: register(new EditorBooleanOption(EditorOption.definitionLinkOpensInPeek, "definitionLinkOpensInPeek", false, {
    ...
    renderFinalNewline: register(new EditorStringEnumOption(EditorOption.renderFinalNewline, "renderFinalNewline", (platform.isLinux ? "dimmed" : "on") as "off" | "on" | "dimmed", ["off", "on", "dimmed"] as const, {
    ...
    renderLineHighlightOnlyWhenFocus: register(new EditorBooleanOption(EditorOption.renderLineHighlightOnlyWhenFocus, "renderLineHighlightOnlyWhenFocus", false, {
    ...
    scrollBeyondLastColumn: register(new EditorIntOption(EditorOption.scrollBeyondLastColumn, "scrollBeyondLastColumn", 4, 0, Constants.MAX_SAFE_SMALL_INTEGER, {
    ...
    suggestOnTriggerCharacters: register(new EditorBooleanOption(EditorOption.suggestOnTriggerCharacters, "suggestOnTriggerCharacters", true, {
    ...
    defaultColorDecorators: register(new EditorBooleanOption(EditorOption.defaultColorDecorators, "defaultColorDecorators", false, {
