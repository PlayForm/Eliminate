...
for (const editorOption of editorOptionsRegistry) {
    ...
    if (typeof schema !== "undefined") {
        if (isConfigurationPropertySchema(schema)) {
            ...
            }.properties![`editor.${editorOption.name}`] =
                schema);
        }
        else {
...
let cachedEditorConfigurationKeys: {
    [key: string]: boolean;
} | null = null;
...
export function isEditorConfigurationKey(key: string): boolean {
    ...
    return getEditorConfigurationKeys()[`editor.${key}`] || false;
}
export function isDiffEditorConfigurationKey(key: string): boolean {
    const editorConfigurationKeys = getEditorConfigurationKeys();
    return getEditorConfigurationKeys()[`diffEditor.${key}`] || false;
}
;
