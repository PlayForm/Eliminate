...
export class IndentationContextProcessor {
    ...
    private _getProcessedPreviousLineTokens(range: Range): IViewLineTokens {
        ...
        const doesLanguageContinueOnPreviousLine = scopedLineTokens.languageId ===
...
class IndentationLineProcessor {
    constructor(private readonly model: IVirtualModel, private readonly languageConfigurationService: ILanguageConfigurationService) { }
    /**
...
export function isLanguageDifferentFromLineStart(model: ITextModel, position: Position): boolean {
    ...
    const languageIsDifferentFromLineStart = !doesScopeStartAtOffsetZero &&
