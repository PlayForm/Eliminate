 ...
 */
export function getInheritIndentForLine(autoIndent: EditorAutoIndentStrategy, model: IVirtualModel, lineNumber: number, honorIntentialIndent: boolean = true, languageConfigurationService: ILanguageConfigurationService): {
    indentation: string;
...
export function getGoodIndentForLine(autoIndent: EditorAutoIndentStrategy, virtualModel: IVirtualModel, languageId: string, lineNumber: number, indentConverter: IIndentConverter, languageConfigurationService: ILanguageConfigurationService): string | null {
    if (autoIndent < EditorAutoIndentStrategy.Full) {
        return null;
    }
...
export function getIndentForEnter(autoIndent: EditorAutoIndentStrategy, model: ITextModel, range: Range, indentConverter: IIndentConverter, languageConfigurationService: ILanguageConfigurationService): {
    beforeEnter: string;
    afterEnter: string;
} | null {
    if (autoIndent < EditorAutoIndentStrategy.Full) {
 ...
 */
export function getIndentActionForType(cursorConfig: CursorConfiguration, model: ITextModel, range: Range, ch: string, indentConverter: IIndentConverter, languageConfigurationService: ILanguageConfigurationService): string | null {
    ...
    if (previousLineNumber > 0) {
        ...
        if (indentRulesSupport.shouldIndentNextLine(previousLine) &&
            ...
            if (inheritedIndentation !== undefined) {
                ...
                const isChFirstNonWhitespaceCharacterAndInAutoClosingPair = autoClosingPairExists &&
...
export function getIndentMetadata(model: ITextModel, lineNumber: number, languageConfigurationService: ILanguageConfigurationService): number | null {
