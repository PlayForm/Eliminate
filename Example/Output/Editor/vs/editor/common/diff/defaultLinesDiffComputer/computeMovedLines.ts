 ...
 *--------------------------------------------------------------------------------------------*/
import { compareBy, numberComparator, pushMany, reverseOrder, } from "../../../../base/common/arrays.js";
...
export function computeMovedLines(changes: DetailedLineRangeMapping[], originalLines: string[], modifiedLines: string[], hashedOriginalLines: number[], hashedModifiedLines: number[], timeout: ITimeout): LineRangeMapping[] {
...
function computeMovesFromSimpleDeletionsToSimpleInsertions(changes: DetailedLineRangeMapping[], originalLines: string[], modifiedLines: string[], timeout: ITimeout) {
...
function computeUnchangedMoves(changes: DetailedLineRangeMapping[], hashedOriginalLines: number[], hashedModifiedLines: number[], originalLines: string[], modifiedLines: string[], timeout: ITimeout) {
    const moves: LineRangeMapping[] = [];
    const original3LineHashes = new SetMap<string, {
        range: LineRange;
    }>();
        ...
        for (let i = change.modified.startLineNumber; i < change.modified.endLineNumberExclusive - 2; i++) {
            ...
            original3LineHashes.forEach(key, ({ range }) => {
                for (const lastMapping of lastMappings) {
                    ...
                    if (lastMapping.originalLineRange.endLineNumberExclusive +
                        1 ===
    ...
    for (const mapping of possibleMappings) {
        const diffOrigToMod = mapping.modifiedLineRange.startLineNumber -
    ...
    for (let i = 0; i < moves.length; i++) {
        ...
        const linesAbove = Math.max(move.original.startLineNumber -
            firstTouchingChangeOrig.original.startLineNumber, move.modified.startLineNumber -
            firstTouchingChangeMod.modified.startLineNumber);
        const lastTouchingChangeOrig = monotonousChanges.findLastMonotonous((c) => c.original.startLineNumber <
            move.original.endLineNumberExclusive)!;
        const lastTouchingChangeMod = findLastMonotonous(changes, (c) => c.modified.startLineNumber <
            move.modified.endLineNumberExclusive)!;
        const linesBelow = Math.max(lastTouchingChangeOrig.original.endLineNumberExclusive -
            move.original.endLineNumberExclusive, lastTouchingChangeMod.modified.endLineNumberExclusive -
            move.modified.endLineNumberExclusive);
        ...
        for (extendToTop = 0; extendToTop < linesAbove; extendToTop++) {
            ...
            if (modifiedSet.contains(modLine) ||
...
function areLinesSimilar(line1: string, line2: string, timeout: ITimeout): boolean {
    ...
    const r = commonNonSpaceCharCount / longerLineLength > 0.6 &&
        longerLineLength > 10;
    return r;
}
function joinCloseConsecutiveMoves(moves: LineRangeMapping[]): LineRangeMapping[] {
    ...
    for (let i = 1; i < moves.length; i++) {
        ...
        const originalDist = current.original.startLineNumber -
            last.original.endLineNumberExclusive;
        const modifiedDist = current.modified.startLineNumber -
...
function removeMovesInSameDiff(changes: DetailedLineRangeMapping[], moves: LineRangeMapping[]) {
    ...
    moves = moves.filter((m) => {
        const diffBeforeEndOfMoveOriginal = changesMonotonous.findLastMonotonous((c) => c.original.startLineNumber <
            m.original.endLineNumberExclusive) || new LineRangeMapping(new LineRange(1, 1), new LineRange(1, 1));
