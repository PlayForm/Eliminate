...
export class DefaultLinesDiffComputer implements ILinesDiffComputer {
    ...
    computeDiff(originalLines: string[], modifiedLines: string[], options: ILinesDiffComputerOptions): LinesDiff {
        ...
        if ((originalLines.length === 1 && originalLines[0].length === 0) ||
            ...
            return new LinesDiff([
                new DetailedLineRangeMapping(new LineRange(1, originalLines.length + 1), new LineRange(1, modifiedLines.length + 1), [
                    new RangeMapping(new Range(1, 1, originalLines.length, originalLines[originalLines.length - 1]
                        .length + 1), new Range(1, 1, modifiedLines.length, modifiedLines[modifiedLines.length - 1]
                        .length + 1)),
                ]),
            ], [], false);
        }
        ...
        for (const diff of lineAlignments) {
            assertFn(() => diff.seq1Range.start - seq1LastStart ===
        ...
        assertFn(() => {
            ...
            function validateRange(range: LineRange, lines: string[]): boolean {
                ...
                if (range.endLineNumberExclusive < 1 ||
