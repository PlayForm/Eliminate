 ...
 *--------------------------------------------------------------------------------------------*/
import { findFirstMonotonous, findLastIdxMonotonous, findLastMonotonous, } from "../../../../base/common/arraysFind.js";
...
export class LinesSliceCharSequence implements ISequence {
    ...
    constructor(public readonly lines: string[], private readonly range: Range, public readonly considerWhitespaceChanges: boolean) {
        ...
        for (let lineNumber = this.range.startLineNumber; lineNumber <= this.range.endLineNumber; lineNumber++) {
            ...
            if (lineNumber === this.range.startLineNumber &&
            ...
            const lineLength = lineNumber === this.range.endLineNumber
                ? Math.min(this.range.endColumn -
                    1 -
                    lineStartOffset -
                    trimmedWsLength, line.length)
                : line.length;
    ...
    public getBoundaryScore(length: number): number {
        ...
        if (prevCategory === CharBoundaryCategory.LineBreakCR &&
        ...
        if (prevCategory !== nextCategory) {
            score += 10;
            if (prevCategory === CharBoundaryCategory.WordLower &&
