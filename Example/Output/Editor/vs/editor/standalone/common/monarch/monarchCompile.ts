...
function createKeywordMatcher(arr: string[], caseInsensitive: boolean = false): (str: string) => boolean {
    ...
    if (caseInsensitive) {
        return function (word) {
            return (hash[word.toLowerCase()] !== undefined &&
        ...
        str = str.replace(/@(\w+)/g, function (s, attr?) {
            ...
            if (typeof lexer[attr] === "string") {
                ...
                if (lexer[attr] === undefined) {
                    throw monarchCommon.createError(lexer, "language definition does not contain attribute '" +
                        attr +
                        "', used at: " +
                        str);
                }
                else {
                    throw monarchCommon.createError(lexer, "attribute reference '" +
                        attr +
...
function createGuard(lexer: monarchCommon.ILexerMin, ruleName: string, tkey: string, val: monarchCommon.FuzzyAction): monarchCommon.IBranch {
    ...
    if ((op === "~" || op === "!~") && /^(\w|\|)*$/.test(pat)) {
        ...
        if (!words) {
            throw monarchCommon.createError(lexer, "the @ match target '" +
                pat +
...
function compileAction(lexer: monarchCommon.ILexerMin, ruleName: string, action: any): monarchCommon.FuzzyAction {
    if (!action) {
        ...
        if (typeof action.token !== "string") {
            throw monarchCommon.createError(lexer, "a 'token' attribute must be of type string, in rule: " +
                ruleName);
        }
        else {
            ...
            if (typeof action.bracket === "string") {
                if (action.bracket === "@open") {
                    ...
                    throw monarchCommon.createError(lexer, "a 'bracket' attribute must be either '@open' or '@close', in rule: " +
                        ruleName);
                }
            }
            if (action.next) {
                if (typeof action.next !== "string") {
                    throw monarchCommon.createError(lexer, "the next state must be a string value in rule: " +
                        ruleName);
                }
                else {
                    ...
                    if (!/^(@pop|@push|@popall)$/.test(next)) {
                        ...
                        if (next.indexOf("$") < 0) {
                            ...
                            if (!monarchCommon.stateExists(lexer, monarchCommon.substituteMatches(lexer, next, "", [], ""))) {
                                throw monarchCommon.createError(lexer, "the next state '" +
                                    action.next +
                                    "' is not defined in rule: " +
                                    ruleName);
                            }
...
class Rule implements monarchCommon.IRule {
    ...
    public setRegex(lexer: monarchCommon.ILexerMin, re: string | RegExp): void {
        ...
        if (typeof re === "string") {
            ...
            throw monarchCommon.createError(lexer, "rules must start with a match string or regular expression: " +
                this.name);
        }
        this.matchOnlyAtLineStart = sregex.length > 0 && sregex[0] === "^";
        this.name = this.name + ": " + sregex;
        this.regex = compileRegExp(lexer, "^(?:" +
        ...
        for (const rule of rules) {
            ...
            if (include) {
                ...
                if (!json.tokenizer[include]) {
                    throw monarchCommon.createError(lexer, "include target '" +
                        include +
                        "' is not defined at: " +
                        state);
                }
