...
class MonarchLineStateFactory {
    ...
    private readonly _maxCacheDepth: number;
    private readonly _entries: {
        [stackElementId: string]: MonarchLineState;
    };
    constructor(maxCacheDepth: number) {
        this._maxCacheDepth = maxCacheDepth;
        this._entries = Object.create(null);
...
class MonarchLineState implements languages.IState {
    ...
    constructor(stack: MonarchStackElement, embeddedLanguageData: EmbeddedLanguageData | null) {
        this.stack = stack;
    ...
    public equals(other: languages.IState): boolean {
        ...
        if (this.embeddedLanguageData === null &&
...
export class MonarchTokenizer extends Disposable implements languages.ITokenizationSupport, IDisposable {
    ...
    private readonly _configurationService: IConfigurationService) {
        ...
        this._maxTokenizationLineLength =
            this._configurationService.getValue<number>("editor.maxTokenizationLineLength", {
    ...
    private _findLeavingNestedLanguageOffset(line: string, state: MonarchLineState): number {
        ...
        for (const rule of rules) {
            if (!monarchCommon.isIAction(rule.action) ||
        ...
        if (!hasEmbeddedPopRule) {
            throw monarchCommon.createError(this._lexer, 'no rule containing nextEmbedded: "@pop" in tokenizer embedded state: ' +
                state.stack.state);
        }
        return popOffset;
    }
    ...
    private _myTokenize(lineWithoutLF: string, hasEOL: boolean, lineState: MonarchLineState, offsetDelta: number, tokensCollector: IMonarchTokensCollector): MonarchLineState {
        ...
        const line = hasEOL && this._lexer.includeLF
            ? lineWithoutLF + "\n"
            : lineWithoutLF;
        ...
        while (forceEvaluation || pos < lineLength) {
            ...
            if (typeof action === "string" || Array.isArray(action)) {
                ...
                if (action.nextEmbedded) {
                    if (action.nextEmbedded === "@pop") {
                        ...
                        enteringEmbeddedLanguage =
                            monarchCommon.substituteMatches(this._lexer, action.nextEmbedded, matched, matches, state);
                    }
                }
                ...
                if (action.switchTo && typeof action.switchTo === "string") {
                    ...
                    if (action.next === "@push") {
                        ...
                        if (stack.depth <= 1) {
                            throw monarchCommon.createError(this._lexer, "trying to pop an empty stack in rule: " +
                                this._safeRuleName(rule));
                        }
                        else {
                            stack = stack.pop()!;
            ...
            // check result
            if (result === null) {
                throw monarchCommon.createError(this._lexer, "lexer rule has no well-defined action in rule: " +
                    this._safeRuleName(rule));
            }
            ...
            if (Array.isArray(result)) {
                ...
                if (matched.length === 0) {
                    if (lineLength === 0 ||
                        ...
                        throw monarchCommon.createError(this._lexer, "no progress in tokenizer in rule: " +
                            this._safeRuleName(rule));
                    }
                }
                ...
                if (monarchCommon.isString(result) &&
                    result.indexOf("@brackets") === 0) {
                    const rest = result.substr("@brackets".length);
                    const bracket = findBracket(this._lexer, matched);
                    if (!bracket) {
                        throw monarchCommon.createError(this._lexer, "@brackets token returned but no bracket defined as: " +
                            matched);
                    }
