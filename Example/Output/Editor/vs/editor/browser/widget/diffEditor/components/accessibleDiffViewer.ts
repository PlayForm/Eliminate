...
export class AccessibleDiffViewer extends Disposable {
    ...
    constructor(private readonly _parentNode: HTMLElement, private readonly _visible: IObservable<boolean>, private readonly _setVisible: (visible: boolean, tx: ITransaction | undefined) => void, private readonly _canClose: IObservable<boolean>, private readonly _width: IObservable<number>, private readonly _height: IObservable<number>, private readonly _diffs: IObservable<DetailedLineRangeMapping[] | undefined>, private readonly _models: IAccessibleDiffViewerModel, 
...
class ViewModel extends Disposable {
    ...
    public readonly currentGroup: IObservable<ViewElementGroup | undefined> = this._currentGroupIdx.map((idx, r) => this._groups.read(r)[idx]);
    public readonly currentGroupIndex: IObservable<number> = this._currentGroupIdx;
    public readonly currentElement: IObservable<ViewElement | undefined> = this._currentElementIdx.map((idx, r) => this.currentGroup.read(r)?.lines[idx]);
    constructor(private readonly _diffs: IObservable<DetailedLineRangeMapping[] | undefined>, private readonly _models: IAccessibleDiffViewerModel, private readonly _setVisible: (visible: boolean, tx: ITransaction | undefined) => void, public readonly canClose: IObservable<boolean>, 
...
class ViewElementGroup {
    constructor(public readonly range: LineRangeMapping, public readonly lines: readonly ViewElement[]) { }
}
...
class DeletedLineViewElement {
    ...
    constructor(public readonly diff: DetailedLineRangeMapping, public readonly originalLineNumber: number) {
    }
}
class AddedLineViewElement {
    public readonly type = LineType.Added;
    public readonly originalLineNumber = undefined;
    constructor(public readonly diff: DetailedLineRangeMapping, public readonly modifiedLineNumber: number) {
    }
}
class UnchangedLineViewElement {
    public readonly type = LineType.Unchanged;
    constructor(public readonly originalLineNumber: number, public readonly modifiedLineNumber: number) {
    }
}
class View extends Disposable {
    ...
    constructor(private readonly _element: HTMLElement, private readonly _model: ViewModel, private readonly _width: IObservable<number>, private readonly _height: IObservable<number>, private readonly _models: IAccessibleDiffViewerModel, 
    @ILanguageService
    ...
    private _render(store: DisposableStore): void {
        ...
        for (const viewItem of group?.lines || []) {
            ...
            if (viewItem.type === LineType.Header) {
                ...
                const getAriaLines = (lines: number) => lines === 0 ? localize('no_lines_changed', "no lines changed")
