...
export class DiffEditorOptions {
    private readonly _options: ISettableObservable<IEditorOptions & Required<IDiffEditorBaseOptions>, {
        changedOptions: IDiffEditorOptions;
    }>;
    public get editorOptions(): IObservable<IEditorOptions, {
    ...
    public readonly couldShowInlineViewBecauseOfSize = derived(this, reader => this._options.read(reader).renderSideBySide && this._diffEditorWidth.read(reader) <= this._options.read(reader).renderSideBySideInlineBreakpoint);
    ...
    private readonly trueInlineDiffRenderingEnabled: IObservable<boolean> = derived(this, reader => this._options.read(reader).experimental.useTrueInlineView!);
    public readonly useTrueInlineDiffRendering: IObservable<boolean> = derived(this, reader => !this.renderSideBySide.read(reader) && this.trueInlineDiffRenderingEnabled.read(reader));
