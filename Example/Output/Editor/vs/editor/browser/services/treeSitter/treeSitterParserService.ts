...
import { EDITOR_EXPERIMENTAL_PREFER_TREESITTER, ITreeSitterParseResult, ITreeSitterParserService, } from "../../../common/services/treeSitterParserService.js";
...
export class TextModelTreeSitter extends Disposable {
    ...
    constructor(readonly model: ITextModel, private readonly _treeSitterLanguages: TreeSitterLanguages, private readonly _treeSitterImporter: TreeSitterImporter, private readonly _logService: ILogService, private readonly _telemetryService: ITelemetryService) {
        super();
    ...
    private async _onDidChangeContent(treeSitterTree: TreeSitterParseResult, changes: IModelContentChange[]) {
        ...
        if (!diff || diff.length > 0) {
            ...
            const ranges = diff
                ? diff.map((r) => new Range(r.startPosition.row + 1, r.startPosition.column + 1, r.endPosition.row + 1, r.endPosition.column + 1))
...
export class TreeSitterParseResult implements IDisposable, ITreeSitterParseResult {
    ...
    constructor(public readonly parser: Parser, public readonly /** exposed for tests **/ language: Parser.Language, private readonly _logService: ILogService, private readonly _telemetryService: ITelemetryService) {
    ...
    private sendParseTimeTelemetry(parseType: TelemetryParseType, languageId: string, time: number, passes: number): void {
        ...
        if (parseType === TelemetryParseType.Full) {
            this._telemetryService.publicLog2<{
                ...
                time: number;
                passes: number;
            }, ParseTimeClassification>(`treeSitter.fullParse`, { languageId, time, passes });
        }
        else {
            this._telemetryService.publicLog2<{
                languageId: string;
                time: number;
                passes: number;
            }, ParseTimeClassification>(`treeSitter.incrementalParse`, { languageId, time, passes });
        }
    }
}
export class TreeSitterLanguages extends Disposable {
    ...
    constructor(private readonly _treeSitterImporter: TreeSitterImporter, private readonly _fileService: IFileService, private readonly _environmentService: IEnvironmentService, private readonly _registeredLanguages: Map<string, string>) {
        super();
    }
