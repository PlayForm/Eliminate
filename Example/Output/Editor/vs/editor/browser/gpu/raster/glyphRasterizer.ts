...
export class GlyphRasterizer extends Disposable implements IGlyphRasterizer {
    ...
    constructor(private readonly _fontSize: number, private readonly _fontFamily: string) {
        super();
     ...
     */
    public rasterizeGlyph(chars: string, metadata: number, colorMap: string[]): Readonly<IRasterizedGlyph> {
        if (chars === '') {
            return {
                source: this._canvas,
    ...
    public _rasterizeGlyph(chars: string, metadata: number, colorMap: string[]): Readonly<IRasterizedGlyph> {
        const devicePixelFontSize = Math.ceil(this._fontSize * getActiveWindow().devicePixelRatio);
