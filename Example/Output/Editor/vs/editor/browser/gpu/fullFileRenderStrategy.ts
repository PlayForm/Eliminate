...
export class FullFileRenderStrategy extends Disposable implements IGpuRenderStrategy {
    ...
    constructor(private readonly _context: ViewContext, private readonly _device: GPUDevice, private readonly _canvas: HTMLCanvasElement, private readonly _atlas: TextureAtlas) {
        super();
        ...
        const bufferSize = FullFileRenderStrategy._lineCount *
    ...
    update(viewportData: ViewportData, viewLineOptions: ViewLineOptions): number {
        ...
        for (y = viewportData.startLineNumber; y <= viewportData.endLineNumber; y++) {
            ...
            for (let tokenIndex = 0, tokensLen = tokens.getCount(); tokenIndex < tokensLen; tokenIndex++) {
                ...
                for (x = tokenStartIndex; x < tokenEndIndex; x++) {
                    ...
                    if (chars === "\t") {
                        xOffset =
                            CursorColumns.nextRenderTabStop(x + xOffset, lineData.tabSize) -
                                x -
                                1;
                        continue;
                    }
                    ...
                    (viewportData.relativeVerticalOffset[y - viewportData.startLineNumber] +
                        ...
                        Math.floor((viewportData.lineHeight -
                            this._context.configuration.options.get(EditorOption.fontSize)) /
                            2)) *
                        dpr);
        ...
        const visibleObjectCount = (viewportData.endLineNumber - viewportData.startLineNumber + 1) *
            lineIndexCount;
        // Only write when there is changed data
        if (dirtyLineStart <= dirtyLineEnd) {
            // Write buffer and swap it out to unblock writes
            this._device.queue.writeBuffer(this._cellBindBuffer, (dirtyLineStart - 1) *
                lineIndexCount *
                Float32Array.BYTES_PER_ELEMENT, cellBuffer.buffer, (dirtyLineStart - 1) *
                lineIndexCount *
                Float32Array.BYTES_PER_ELEMENT, (dirtyLineEnd - dirtyLineStart + 1) *
                lineIndexCount *
                Float32Array.BYTES_PER_ELEMENT);
        }
