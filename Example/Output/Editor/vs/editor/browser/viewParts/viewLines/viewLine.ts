...
import { CharacterMapping, DomPosition, ForeignElementType, LineRange, RenderLineInput, renderViewLine, } from "../../../common/viewLayout/viewLineRenderer.js";
...
export class ViewLine implements IVisibleLine {
    ...
    public onSelectionChanged(): boolean {
        if (isHighContrast(this._options.themeType) ||
    ...
    public renderLine(lineNumber: number, deltaTop: number, lineHeight: number, viewportData: ViewportData, sb: StringBuilder): boolean {
        if (this._options.useGpu &&
            ViewGpuContext.canRender(this._options, viewportData, lineNumber)) {
        ...
        if (isHighContrast(options.themeType) ||
            ...
            for (const selection of selections) {
                if (selection.endLineNumber < lineNumber ||
                    selection.startLineNumber > lineNumber) {
                    // Selection does not intersect line
                    continue;
                }
                const startColumn = selection.startLineNumber === lineNumber
                    ? selection.startColumn
                    : lineData.minColumn;
                const endColumn = selection.endLineNumber === lineNumber
                    ? selection.endColumn
                    : lineData.maxColumn;
...
class FastRenderedViewLine implements IRenderedViewLine {
    ...
    public getVisibleRangesForRange(lineNumber: number, startColumn: number, endColumn: number, context: DomReadingContext): FloatHorizontalRange[] | null {
        ...
        return [
            new FloatHorizontalRange(startPosition, endPosition - startPosition),
        ];
    }
