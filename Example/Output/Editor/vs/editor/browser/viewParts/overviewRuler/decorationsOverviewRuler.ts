...
class Settings {
    ...
    private _initLanes(canvasLeftOffset: number, canvasWidth: number, laneCount: number): [
        number[],
        number[]
    ] {
...
export class DecorationsOverviewRuler extends ViewPart {
    ...
    private _render(): void {
        ...
        if (this._settings.overviewRulerLanes === 0) {
            // overview ruler is off
            this._domNode.setBackgroundColor(backgroundColor
        ...
        if (this._actualShouldRender === ShouldRenderValue.Maybe &&
            !equals(this._renderedCursorPositions, this._cursorPositions, (a, b) => a.position.lineNumber === b.position.lineNumber &&
                a.color === b.color)) {
            this._actualShouldRender = ShouldRenderValue.Needed;
        }
        if (this._actualShouldRender === ShouldRenderValue.Maybe) {
        ...
        for (const decorationGroup of decorations) {
            ...
            for (let i = 0, len = decorationGroupData.length / 3; i < len; i++) {
                ...
                let y1 = (viewLayout.getVerticalOffsetForLineNumber(startLineNumber) *
                    heightRatio) |
                    0;
                let y2 = ((viewLayout.getVerticalOffsetForLineNumber(endLineNumber) +
                    lineHeight) *
                    heightRatio) |
                    0;
                const height = y2 - y1;
                if (height < minDecorationHeight) {
                    ...
                    else if (yCenter + halfMinDecorationHeight >
                        canvasHeight) {
        ...
        if (!this._settings.hideCursor) {
            ...
            for (let i = 0, len = this._cursorPositions.length; i < len; i++) {
                ...
                let yCenter = (viewLayout.getVerticalOffsetForLineNumber(cursor.lineNumber) *
                    heightRatio) |
                    0;
