...
import { editorForeground, minimapBackground, minimapForegroundOpacity, minimapSelection, } from "../../../../platform/theme/common/colorRegistry.js";
import { IEditorConfiguration } from "../../../common/config/editorConfiguration.js";
import { EditorLayoutInfoComputer, EditorOption, MINIMAP_GUTTER_WIDTH, RenderMinimap, } from "../../../common/config/editorOptions.js";
...
class MinimapLayout {
     ...
     */
    public readonly scrollHeight: number, public readonly sliderNeeded: boolean, private readonly _computedSliderRatio: number, 
    /**
     ...
     */
    public intersectWithViewport(range: Range): [
        number,
        number
    ] | null {
    ...
    public getYForLineNumber(lineNumber: number, minimapLineHeight: number): number {
        return (+(lineNumber - this.startLineNumber + this.topPaddingLineCount) *
            minimapLineHeight);
    }
    public static create(options: MinimapOptions, viewportStartLineNumber: number, viewportEndLineNumber: number, viewportStartLineNumberVerticalOffset: number, viewportHeight: number, viewportContainsWhitespaceGaps: boolean, lineCount: number, realLineCount: number, scrollTop: number, scrollHeight: number, previousLayout: MinimapLayout | null): MinimapLayout {
        ...
        if (minimapLinesFitting >=
            ...
            let startLineNumber = Math.max(1, Math.floor(consideringStartLineNumber -
            ...
            const partialLine = (scrollTop - viewportStartLineNumberVerticalOffset) /
                lineHeight;
...
class RenderData {
    ...
    constructor(renderedLayout: MinimapLayout, imageData: ImageData, lines: MinimapLine[]) {
...
class MinimapSamplingState {
    public static compute(options: MinimapOptions, viewLineCount: number, oldSamplingState: MinimapSamplingState | null): [
        MinimapSamplingState | null,
        SamplingStateEvent[]
    ] {
        if (options.renderMinimap === RenderMinimap.None ||
            !options.isSampling) {
            return [null, []];
        }
    ...
    constructor(public readonly samplingRatio: number, public readonly minimapLines: number[]) { }
     ...
     */
    public modelLineRangeToMinimapLineRange(fromLineNumber: number, toLineNumber: number): [
        number,
        number
    ] | null {
        ...
        if (fromLineIndex === toLineIndex) {
            ...
            if (sampledLineNumber < fromLineNumber ||
     ...
     */
    public decorationLineRangeToMinimapLineRange(startLineNumber: number, endLineNumber: number): [
        number,
        number
    ] {
    ...
    public onLinesDeleted(e: viewEvents.ViewLinesDeletedEvent): [
        number,
        number
    ] {
...
export class Minimap extends ViewPart implements IMinimapModel {
    ...
    public render(ctx: RestrictedRenderingContext): void {
        ...
        if (this._samplingState) {
            viewportStartLineNumber =
                this._samplingState.modelLineToMinimapLine(viewportStartLineNumber);
    ...
    public revealLineNumber(lineNumber: number): void {
        ...
        this._context.viewModel.revealRange("mouse", false, new Range(lineNumber, 1, lineNumber, 1), viewEvents.VerticalRevealType.Center, ScrollType.Smooth);
    }
...
class InnerMinimap extends Disposable {
    ...
    constructor(theme: EditorTheme, model: IMinimapModel) {
        ...
        this._pointerDownListener = dom.addStandardDisposableListener(this._domNode.domNode, dom.EventType.POINTER_DOWN, (e) => {
            ...
            const internalOffsetY = (this._model.options.canvasInnerHeight /
    ...
    private _getBuffer(): ImageData | null {
        if (!this._buffers) {
            if (this._model.options.canvasInnerWidth > 0 &&
    ...
    public render(renderingCtx: IMinimapRenderingContext): void {
        ...
        if (renderingCtx.scrollLeft + renderingCtx.viewportWidth >=
    ...
    private _renderSelectionLineHighlights(canvasContext: CanvasRenderingContext2D, selections: Selection[], highlightedLines: ContiguousLineMap<boolean>, layout: MinimapLayout, minimapLineHeight: number): void {
    ...
    private _renderDecorationsLineHighlights(canvasContext: CanvasRenderingContext2D, decorations: ViewModelDecoration[], highlightedLines: ContiguousLineMap<boolean>, layout: MinimapLayout, minimapLineHeight: number): void {
    ...
    private _renderSelectionsHighlights(canvasContext: CanvasRenderingContext2D, selections: Selection[], lineOffsetMap: ContiguousLineMap<number[] | null>, layout: MinimapLayout, lineHeight: number, tabSize: number, characterWidth: number, canvasInnerWidth: number): void {
        if (!this._selectionColor || this._selectionColor.isTransparent()) {
            return;
        }
        for (const selection of selections) {
    ...
    private _renderDecorationsHighlights(canvasContext: CanvasRenderingContext2D, decorations: ViewModelDecoration[], lineOffsetMap: ContiguousLineMap<number[] | null>, layout: MinimapLayout, minimapLineHeight: number, tabSize: number, characterWidth: number, canvasInnerWidth: number): void {
    ...
    private renderDecorationOnLine(canvasContext: CanvasRenderingContext2D, lineOffsetMap: ContiguousLineMap<number[] | null>, decorationRange: Range, decorationColor: Color | undefined, layout: MinimapLayout, lineNumber: number, height: number, minimapLineHeight: number, tabSize: number, charWidth: number, canvasInnerWidth: number): void {
    ...
    private getXOffsetForPosition(lineOffsetMap: ContiguousLineMap<number[] | null>, lineNumber: number, column: number, tabSize: number, charWidth: number, canvasInnerWidth: number): number {
        if (column === 1) {
    ...
    private _renderSectionHeaders(layout: MinimapLayout) {
        ...
        for (const decoration of decorations) {
            ...
            InnerMinimap._renderSectionLabel(canvasContext, headerText, decoration.options.minimap?.sectionHeaderStyle ===
                MinimapSectionHeaderStyle.Underlined, backgroundFill, foregroundFill, canvasInnerWidth, backgroundFillY, backgroundFillHeight, y, separatorY);
        }
    }
    private static _fitSectionHeader(target: CanvasRenderingContext2D, maxWidth: number, headerText: string): string {
        if (!headerText) {
            return headerText;
        }
        const ellipsis = "…";
    ...
    private static _renderSectionLabel(target: CanvasRenderingContext2D, headerText: string | null, hasSeparatorLine: boolean, backgroundFill: string, foregroundFill: string, minimapWidth: number, backgroundFillY: number, backgroundFillHeight: number, textY: number, separatorY: number): void {
        if (headerText) {
    ...
    private renderLines(layout: MinimapLayout): RenderData | null {
        ...
        const minimapCharWidth = this._model.options.minimapCharWidth;
        const baseCharHeight = renderMinimap === RenderMinimap.Text
        ...
        const renderBackground = new RGBA8(Math.round((background.r - defaultBackground.r) * backgroundA +
            defaultBackground.r), Math.round((background.g - defaultBackground.g) * backgroundA +
            defaultBackground.g), Math.round((background.b - defaultBackground.b) * backgroundA +
            defaultBackground.b), 255);
    ...
    private static _renderUntouchedLines(target: ImageData, topPaddingLineCount: number, startLineNumber: number, endLineNumber: number, minimapLineHeight: number, lastRenderData: RenderData | null): [
        number,
        number,
        boolean[]
    ] {
        ...
        const maxDestPixel = (endLineNumber - startLineNumber + 1) *
            minimapLineHeight *
            WIDTH *
            4;
    ...
    private static _renderLine(target: ImageData, backgroundColor: RGBA8, backgroundAlpha: number, useLighterFont: boolean, renderMinimap: RenderMinimap, charWidth: number, colorTracker: MinimapTokensColorTracker, foregroundAlpha: number, minimapCharRenderer: MinimapCharRenderer, dy: number, innerLinePadding: number, tabSize: number, lineData: ViewLineData, fontScale: number, minimapLineHeight: number): void {
...
class ContiguousLineMap<T> {
    ...
    public set(lineNumber: number, value: T): void {
        if (lineNumber < this._startLineNumber ||
