...
export class ViewLinesGpu extends ViewPart implements IViewLines {
    ...
    async initWebgpu() {
        ...
        this._glyphStorageBuffer[0] = this._register(GPULifecycle.createBuffer(this._device, {
            ...
            size: GlyphStorageBufferInfo.BytesPerEntry *
        ...
        this._bindGroup = this._device.createBindGroup({
            ...
            entries: [
                ...
                {
                    binding: BindingId.LayoutInfoUniform,
    ...
    private _updateAtlasStorageBufferAndTexture() {
        for (const [layerIndex, page] of ViewGpuContext.atlas.pages.entries()) {
            ...
            const values = new Float32Array(GlyphStorageBufferInfo.FloatsPerEntry *
                TextureAtlasPage.maximumGlyphCount);
            let entryOffset = 0;
            for (const glyph of page.glyphs) {
                ...
                values[entryOffset +
                    ...
                    1] = glyph.originOffsetY;
                entryOffset += GlyphStorageBufferInfo.FloatsPerEntry;
            }
            if (entryOffset / GlyphStorageBufferInfo.FloatsPerEntry >
                TextureAtlasPage.maximumGlyphCount) {
    ...
    private _visibleRangesForLineRange(lineNumber: number, startColumn: number, endColumn: number): VisibleRanges | null {
        ...
        const result = new VisibleRanges(false, [
            new FloatHorizontalRange((startColumn - 1) * viewLineOptions.spaceWidth, (endColumn - startColumn - 1) * viewLineOptions.spaceWidth),
        ]);
        return result;
    }
