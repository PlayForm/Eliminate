...
export class WhitespaceOverlay extends DynamicViewOverlay {
    ...
    private _applyRenderWhitespace(ctx: RenderingContext, lineNumber: number, selections: LineRange[] | null, lineData: ViewLineData): string {
        ...
        if (this._options.renderWhitespace === "trailing" &&
        ...
        for (let charIndex = fauxIndentLength; charIndex < len; charIndex++) {
            ...
            if (onlyBoundary &&
                ...
                charIndex === len - 1) {
                const prevCharCode = charIndex - 1 >= 0
                    ? lineContent.charCodeAt(charIndex - 1)
                    : CharCode.Null;
                const isSingleTrailingSpace = chCode === CharCode.Space &&
